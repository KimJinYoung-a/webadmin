...
itemname = requestCheckVar(request("itemname"),80)
sellcash = requestCheckVar(request("sellcash"),10)
suplycash = requestCheckVar(request("suplycash"),10)
shopbuyprice = requestCheckVar(request("shopbuyprice"),10)
...
vatcode = requestCheckVar(request("vatcode"),10)
...
songjangdiv = requestCheckVar(request("songjangdiv"),16)
...
if mode ="arrmodi" then
	...
	for i=0 to cnt
		...
		if Not (extbarcodeAlreadyExists) then
			...
			sqlStr = sqlStr + " set extbarcode='" + trim(CStr(extbarcodearr(i))) + "'," + VbCrlf
			sqlStr = sqlStr + " isusing='" + CStr(isusingarr(i)) + "'," + VbCrlf
			sqlStr = sqlStr + " updt=getdate()"+ VbCrlf
			sqlStr = sqlStr + " where itemgubun='" + itemgubunarr(i) + "'"+ VbCrlf
			sqlStr = sqlStr + " and shopitemid=" + CStr(itemarr(i)) + ""+ VbCrlf
			sqlStr = sqlStr + " and itemoption='" + CStr(itemoptionarr(i)) + "'"+ VbCrlf
...
elseif mode ="arradd" then
	...
	for i=0 to cnt
		...
		if shopitemNotExists then
			...
			sqlStr = sqlStr + " , i.makerid, i.itemname, IsNull(v.optionname,'') as optname, i.sellcash + IsNULL(v.optaddprice,0), i.orgprice + IsNULL(v.optaddprice,0), 0, 0"
			sqlStr = sqlStr + " , IsNULL(s.barcode,'') as barcode ,i.vatinclude, i.cate_large, i.cate_mid, i.cate_small " + VbCrlf
			sqlStr = sqlStr + " , Case When i.mwdiv<>'U' then i.mwdiv else NULL end " + VbCrlf
...
elseif mode="addipchullist" then
	...
	sqlStr = sqlStr + " (chargeid,shopid,divcode,vatcode,scheduledate,statecd,songjangdiv,songjangno)" + VbCrlf
	sqlStr = sqlStr + " values('" + chargeid + "'" + VbCrlf
	sqlStr = sqlStr + " ,'" + shopid + "'" + VbCrlf
	sqlStr = sqlStr + " ,'" + divcode + "'" + VbCrlf
	sqlStr = sqlStr + " ,'" + vatcode + "'" + VbCrlf
	sqlStr = sqlStr + " ,'" + scheduledt + "'" + VbCrlf
	sqlStr = sqlStr + " ,'0'" + VbCrlf
	sqlStr = sqlStr + " ,'" + songjangdiv + "'" + VbCrlf
	sqlStr = sqlStr + " ,'" + songjangno + "'" + VbCrlf
	sqlStr = sqlStr + " )"
