...
Dim idx, olist, page, i, state, title, makerid, isusing, research
	...
	menupos	= requestCheckVar(request("menupos"),20)
...
function goView(idx, makerid){
	location.href = "collectionModify.asp?mode=U&idx="+idx+"&makerid="+makerid+"&menupos=<%=menupos%>";
}
...
<img src="/images/icon_arrow_link.gif"> <b>SHOP_collection</b>
...
<form name="frmSortImgSize" method="post" action="/designer/brand/shop/collection/collectionSortNoProcess.asp" style="margin:0px;">
...
<tr>
	...
	<td align="right">
		<input type="button" value="신규등록" class="button" onclick="javascript:location.href='/designer/brand/shop/collection/collectionModify.asp';">
	</td>
</tr>
</table>
<!-- 액션 끝 -->
...
<% end if %>	
	...
	<td>
		<%= getcollectionstatsname(olist.FItemList(i).Fstate) %>
	</td>
	<td>
	...
	<td>
		<input type="button" onClick="goView('<%=olist.FItemList(i).FIdx%>', '<%=olist.FItemList(i).FMakerid%>')" value="수정[상세상품:<%=olist.FItemList(i).fitemcnt%>개]" class="button">
	</td>		
</tr>
<% Next %>
<tr height="25" bgcolor="FFFFFF" >
