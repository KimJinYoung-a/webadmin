...
<%
Dim ohello, omanager, menugubun
SET ohello = new chello
	ohello.FRectMakerid	= session("ssBctID")
	ohello.sbhellomodify
If ohello.FTotalCount <> 0 Then
	SET omanager = new cmanager
		omanager.FRectMakerid	= session("ssBctID")
...
function imgDelProcess(){
	if (confirm('이미지를 삭제 하시겠습니까?')){
	    ...
	    document.dupl.duplid.value = '<%=session("ssBctID")%>';
...
<form name="frm" method="post" action="<%= uploadUrl %>/linkweb/street/doHelloupload.asp" enctype="MULTIPART/FORM-DATA">
