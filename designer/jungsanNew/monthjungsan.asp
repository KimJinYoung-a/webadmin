...
function PopDetail(iidx,tg){
    var uri = 'jungsandetailsumON.asp?id=' + iidx;
    if (tg=="OF") uri = 'jungsandetailsumOF.asp?idx=' + iidx;
    if (tg=="AC") uri = '/lectureadmin/jungsan/jungsandetailsumAC.asp?id=' + iidx;
...
function PopConfirm(mnupos,iidx){
    if (confirm('확인 완료 처리 하시겠습니까?')){
        var popwin = window.open('/designer/jungsanNew/jconfirmProcess.asp?id=' + iidx +'&mode=confirm','popjconfirm','width=400, height=400, scrollbars=1');
	    popwin.focus();
    }
}
...
<tr align="center" bgcolor="#FFFFFF" height="30">
    ...
    <td align="right"><%=FormatNumber(ojungsanTaxCC.FItemList(i).FdlvJungsanSum + ojungsanTaxCC.FItemList(i).FetJungsanSum,0)%></td>
    ...
    <td>
        <% if ojungsanTaxMM.FItemList(i).IsElecTaxExists then %>
      	...
      	<% elseif ojungsanTaxMM.FItemList(i).IsElecSimpleBillCase then %>
      	    <% if (ojungsanTaxMM.FItemList(i).Ffinishflag="1") then %>
...
<tr align="left" bgcolor="#FFFFFF">
    ...
    <td >협력사가 텐바이텐 사이트를 통해 판매한 매출액</td>
