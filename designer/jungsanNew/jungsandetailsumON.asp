...
function ExcelJunsanDetailList(id,gubun,itemvatyn){
    ...
    location.href = '/designer/jungsanNew/jungsandetailsum_excel_exdt.asp?id=' + id + '&gubun=' + gubun + '&itemvatyn='+itemvatyn;    
    <% else %>
    location.href = '/designer/jungsanNew/jungsandetailsum_excel.asp?id=' + id + '&gubun=' + gubun + '&itemvatyn='+itemvatyn;
    <% end if %>
}
</script>
...
<% else %>
    <tr bgcolor="#FFFFFF">
    	<td colspan="<%=CHKIIF(IsCommissionTax,16,12)%>" align="center"><img src="/images/icon_search.jpg" width="16" border="0" align="absbottom">&nbsp;상세내역을 선택하세요.</td>
    </tr>
<% end if %>
</table>
<!-- 주문건별 리스트 끝-->
