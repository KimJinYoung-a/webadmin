...
function downloadDeliveXL(){
	...
	xlfrm.action="dosongjanglistexcel.asp";
	xlfrm.submit();
}
...
function NextStep(frm){
	...
	var sArry		= frm.songjangfile.value.split("\\");	// 선택된 파일의 전체 경로
	var ext			= sArry[sArry.length-1].split(".");		// 확장자 추출
	if(ext[ext.length-1].toLowerCase() != "csv") {
		alert('파일은 CSV(text)형식만 업로드가 능합니다.\n파일형식을 확인해주세요.')
		return;
	}
...
<% if False and (Request.ServerVariables("REMOTE_ADDR") = "61.252.133.67") then %>
TEST!!!!
<form name="frmNext" method="post" action="BatchSongjangInputStep2_TEST_skyer9.asp" onsubmit="return false;" enctype="multipart/form-data">
<% else %>
<form name="frmNext" method="post" action="BatchSongjangInputStep2.asp" onsubmit="return false;" enctype="multipart/form-data">
<% end if %>
<tr bgcolor="#FFFFFF" height="50">
