...
''Data 유효성 검사
function IsValidLine(ioneLine,byref iValidArr, byref errMSG)
    dim buf
    IsValidLine = False
...
''rw ioneLine
    ...
    if Not IsArray(buf) then
    	errMSG = "데이터 오류(1)"
    	Exit Function
    end if

    if UBound(buf)<>7 then
    	errMSG = "데이터 오류(2)"
    	Exit Function
	end if
	...
	if UBound(Split(iOrderserial, "-")) <> 1 then
    	errMSG = "잘못된 CS주문번호"
    	Exit Function
	end if

    ''길이 Check
    if Len(iDetailidx)<6 then
    	errMSG = "잘못된 CS일련번호"
    	Exit Function
    end if
    ...
    if Len(iSongjangDiv)<1 then
    	errMSG = "택배사 코드 없음"
    	Exit Function
    end if
    if Len(iSongjangNo)<7 or Len(iSongjangNo)>32 then
    	errMSG = "송장번호 오류"
    	Exit Function
    end if
    ...
    if Not IsNumeric(iDetailidx) then
    	errMSG = "잘못된 일련번호(숫자)"
    	Exit Function
    end if
    'if Not IsNumeric(iOrderserial) then
    '	errMSG = "잘못된 주문번호(숫자)"
    '	Exit Function
    'end if
    if Not IsNumeric(iSongjangDiv) then
    	errMSG = "잘못된 택배사 코드"
    	Exit Function
    end if
    if Not IsNumeric(iSongjangNo) then
    	errMSG = "송장번호 오류"
    	Exit Function
    end if

    ''ETC Check
    if (iSongjangDiv>99) or (iSongjangDiv<1) then
    	errMSG = "기타 오류(1)"
    	Exit Function
    end if

    ''E+ 엑셀관련
    if InStr(iSongjangNo,"E+")>0 then
    	errMSG = "기타 오류(2)"
    	Exit Function
    end if
    ...
    if (LCASE(session("ssBctId"))="cocorobox") or (LCASE(session("ssBctId"))="cocoroboxdeco") then
    ...
    if IsArray(iLines) then
        ...
        for i=0 to iTotCnt    ''빈줄도 검사
            ...
            elseif (iLines(i)="") then
            	...
            	errMSG = ""
                if (IsValidLine(iLines(i),ValidArr, errMSG)) then
                    iSuccCnt = iSuccCnt + 1
                else
                    iFailCnt = iFailCnt + 1
                    iFailLineStr = iFailLineStr + "<br>" + CStr(i+1) + " : " + errMSG
                end if
            end if
        Next
...
<tr bgcolor="#FFFFFF">
    <td align="center">
        ...
        <font color=red>실패 라인 : <%= iFailLineStr %></font>
        <% end if %>
    </td>
</tr>
<form name="frmSv" method="post" action="upchebeasong_cs_Process.asp">
