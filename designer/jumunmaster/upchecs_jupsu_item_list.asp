	...
	<tr align="center" bgcolor="#FFFFFF" >
		<td align="left">
			...
			<select class="select" name="searchtype">
				...
				<option value="jupsuChulgo"   <% if (searchtype = "jupsuChulgo") then %>selected<% end if %>>교환 CS접수</option>
				<option value="confirmChulgo" <% if (searchtype = "confirmChulgo") then %>selected<% end if %>>교환 업체확인</option>
				<option value="jupsuReturn" <% if (searchtype = "jupsuReturn") then %>selected<% end if %>>반품접수</option>
			</select>
			&nbsp;
...
<tr>
    <td>
    * CS접수 이상 업체처리완료 이전 상태까지의 상품수를 표시합니다.(최근 3개월)
    </td>
...
<p>
	...
	<table width="100%" border="0" align="center" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	    <tr align="center" bgcolor="<%= adminColor("tabletop") %>">
			...
			<td>상품명</td>
			<td width="140">옵션명</td>
			<td width="85">교환 CS접수</td>
			<td width="85">교환 업체확인</td>
			<td width="85">반품접수</td>
	    </tr>
