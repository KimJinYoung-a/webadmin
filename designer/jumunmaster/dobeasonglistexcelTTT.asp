...
function replaceXlText(org)
	...
	'// 줄바꿈문자 제거
	reText = replace(reText,vbCrLf," ")
	reText = replace(reText,vbCr," ")
	reText = replace(reText,vbLf," ")
...
<tr>
	...
	<% end if %>
	<td align="center" x:str><%= replaceXlText(db2html(ojumun.FMasterItemList(ix).FComment)) %></td>
	...
	<td align="center" x:str ><%
		if (Not IsNULL(ojumun.FMasterItemList(ix).Frequiredetail)) then
			...
			%>
			    [<%= i+ 1 %>번 상품 문구]<br>
			    <%= nl2br(replaceXlText(splitValue(ojumun.FMasterItemList(ix).Frequiredetail,CAddDetailSpliter,i))) %><br>
	    <%
	    		next
	    	end if
	    end if
	%></td>
	<% else %>
	<td align="center" x:str ><%= nl2br(replaceXlText(ojumun.FMasterItemList(ix).Frequiredetail)) %></td>
	<% end if %>
	<% if (SheetType="V3") or (SheetType="V4") then %>
...
<% if Not IsNULL(ojumun.FMasterItemList(ix).Freqdate) then %>
	<td align="center" x:str><%= Left(CStr(ojumun.FMasterItemList(ix).Freqdate),10) %>일 <%= (ojumun.FMasterItemList(ix).Freqtime) %>시</td>
