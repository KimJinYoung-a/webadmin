...
<form name="frm" method="get" onsubmit="chksubmit(); return false" style="margin:0px;">
...
<form name="frmbalju" method="post" action="upchebeasong_Process.asp" style="margin:0px;">
...
<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
    ...
    <tr align="center" bgcolor="<%= adminColor("tabletop") %>">
		...
		<td width="65">주문통보일</td>
		<td width="65">출고일<br><font color="#AAAAAA">출고예정일</font></td>
		<td width="40">경과일</td>
		<td width="100"><% drawSelectBoxDeliverCompanyAssign "defaultsongjangdiv","" %></td>
		<td width="130" align="center">운송장번호</td>
		<td width="100" align="center">미출고사유<br>보기</td>
	</tr>
...
<% else %>
	...
	<tr align="center" class="a" bgcolor="#FFFFFF">
		<td>
			<input type="hidden" name="orderserial" value="<%= ojumun.FMasterItemList(ix).Forderserial %>"><input type="checkbox" name="detailidx" value="<%= ojumun.FMasterItemList(ix).Fidx %>" onClick="AnCheckClick(this);" <%= CHKIIF(ojumun.FMasterItemList(ix).FMisendReason="05","disabled","") %>>
		</td>
		...
		<td><acronym title="<%= ojumun.FMasterItemList(ix).Fbeasongdate %>"><%= left(ojumun.FMasterItemList(ix).Fbeasongdate,10) %></acronym></td>
		<td>
			D+
			<% if IsNULL(ojumun.FMasterItemList(ix).Fbaljudate) then %>
			    0
			<% elseif IsNULL(ojumun.FMasterItemList(ix).Fbeasongdate) then %>
			    <%= datediff("d",(left(ojumun.FMasterItemList(ix).Fbaljudate,10)) , (left(now,10)) ) %>
			<% else %>
				<% if datediff("d",(left(ojumun.FMasterItemList(ix).Fbaljudate,10)) , (left(ojumun.FMasterItemList(ix).Fbeasongdate,10))) < 0 then %>
				0
				<% else %>
				<%= datediff("d",(left(ojumun.FMasterItemList(ix).Fbaljudate,10)) , (left(ojumun.FMasterItemList(ix).Fbeasongdate,10)) ) %>
				<% end if %>
			<% end if %>
		</td>
		<td>
		    <% if (IsNULL(ojumun.FMasterItemList(ix).FSongjangdiv) or (ojumun.FMasterItemList(ix).FSongjangdiv=0)) and (Trim(ojumun.FMasterItemList(ix).FCurrstate) <> "7") then  %>
		...
		<td><input type="text" class="text" name="songjangno" size="16" value="<%= ojumun.FMasterItemList(ix).FSongjangno %>" onKeyup="CheckThis(this,'<%= ix %>');" maxlength=32 <%= CHKIIF(ojumun.FMasterItemList(ix).FMisendReason="05","readonly style='background:#EEEEEE'","") %>></td>
		<td>
		    <% if (ojumun.FMasterItemList(ix).isMisendAlreadyInputed) then %>
	        <a href="javascript:popMisendInput('<%= ojumun.FMasterItemList(ix).Fidx %>');"><%= ojumun.FMasterItemList(ix).getMisendText %></a>
	        <% else %>
	        <!-- a href="javascript:popMisendInput('<%= ojumun.FMasterItemList(ix).Fidx %>');">입력</a -->
	        <% end if %>
		</td>
	</tr>
	<% next %>
...
<form name="frmArrInput" method="post" action="BatchSongjangInput.asp">
