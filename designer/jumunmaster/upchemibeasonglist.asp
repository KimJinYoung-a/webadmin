...
function ShowOrderInfo(frm,orderserial){
	...
	window.open("about:blank", "_orderdetail", props);
    frm.target = "_orderdetail";
...
function BaljuReprint(){
	...
	if (!pass) {
	    ...
	    frm.isall.value = "";
 		frm.action = "/designer/jumunmaster/reselectbaljulist.asp";
		frm.submit();
	}
}
...
function BaljuReprintNew() {
	...
	if (!pass) {
	    ...
	    var popwin = window.open("about:blank","BaljuReprintNew","width=850,scrollbars=yes,resizable");
	    popwin.focus();
	    frm.target = "BaljuReprintNew";
	    frm.isall.value = "";
 		frm.action = "/designer/jumunmaster/reselectbaljulistNew.asp";
		frm.submit();
...
<form name="frm" method="get" action="" onsubmit="chksubmit(); return false" style="margin:0px;">
...
<table width="100%" align="center" cellpadding="0" cellspacing="0" class="a" style="padding-top:10;">
	<tr align="center">
		<td align="left">
    		<input type="button" class="button" value="선택내역 발주서 재출력" onclick="BaljuReprint();">
			&nbsp;
    		<input type="button" class="button" value="선택내역 발주서 재출력(New)" onclick="BaljuReprintNew();">
			&nbsp;
    		<input type="button" class="button" value="미출고전체 발주서 재출력" onclick="BaljuReprintAll();">
		</td>
		<td align="right">
			<!--
...
품절출고불가의 경우, 고객센터에서 처리<br>
<font color="red">이미 입력된 출고불가 사유를 수정</font>하려면 고객센터에 문의하세요.

<form name="frmshow" method="post" style="margin:0px;">
	<input type="hidden" name="orderserial" value="">
</form>
<form name="frmbalju" method="post" style="margin:0px;">
	...
	<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
		...
		<tr align="center" class="a" bgcolor="#FFFFFF">
			<td>
				...
				<input type="checkbox" name="chkidx" value="<%= ojumun.FMasterItemList(ix).Fidx %>" onClick="AnCheckClick(this);">
			</td>
			...
			<td><%= ojumun.FMasterItemList(ix).getMisendText %></td>
			<td><%= ojumun.FMasterItemList(ix).FMisendIpgodate %></td>
			<td>
				<% if (ojumun.FMasterItemList(ix).isMisendAlreadyInputed) then %>
				<a href="javascript:popMisendInput('<%= ojumun.FMasterItemList(ix).Fidx %>');">상세보기</a>
				<% else %>
				<a href="javascript:popMisendInput('<%= ojumun.FMasterItemList(ix).Fidx %>');"><font color="#AAAAAA">입력</font></a>
				<% end if %>
			</td>
		</tr>
		<% next %>
