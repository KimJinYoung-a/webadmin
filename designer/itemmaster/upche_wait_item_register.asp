...
if npartner.FResultCount < 1 then
	response.write "<script type='text/javascript'>"
	response.write "	alert('브랜드 정보가 없습니다.');"
...
function SubmitSave() {
 ...
 if (!IsDigit(document.itemreg.itemWeight.value)){
	...
	//마진 변경확인
	if(document.itemreg.margin.value != document.itemreg.defultmargine.value){
		if(!confirm("마진이 기본마진과 다릅니다. 그대로 진행하시겠습니까?")){
			document.itemreg.margin.focus();
			return;
		}
	}
    ...
    if (itemreg.useoptionyn[0].checked == true) {
        ...
        //이중옵션
            //2013/12/18 추가
            var maxbytelen = 32; //(96-1)/2 ,max 32
            if (itemreg.optionTypename3.value.length>0){
                maxbytelen = 30; //(96-2)/3
            }
            ...
            //2014/05/02 옵션이 너무 많이 등록되서 사이트 에러남.
			var oCnt1=0, oCnt2=0, oCnt3=0;
			for(var i=0;i<itemreg.optionName1.length;i++) {
				if(fnTrim(itemreg.optionName1[i].value).length>0) {
					oCnt1++;
				}
			}
			for(var i=0;i<itemreg.optionName2.length;i++) {
				if(fnTrim(itemreg.optionName2[i].value).length>0) {
					oCnt2++;
				}
			}
			for(var i=0;i<itemreg.optionName3.length;i++) {
				if(fnTrim(itemreg.optionName3[i].value).length>0) {
					oCnt3++;
				}
			}
			if(oCnt1==0) oCnt1=1;
            ...
            //2013/12/18 추가
            if (getByteLength(itemreg.optionTypename1.value)>32){
                alert('옵션구분명이 너무깁니다.옵션명을 줄여주세요.');
                itemreg.optionTypename1.focus();
                return;
            }

            if (getByteLength(itemreg.optionTypename2.value)>32){
                alert('옵션구분명이 너무깁니다.옵션명을 줄여주세요.');
                itemreg.optionTypename2.focus();
                return;
            }

            if (getByteLength(itemreg.optionTypename3.value)>32){
                alert('옵션구분명이 너무깁니다.옵션명을 줄여주세요.');
                itemreg.optionTypename3.focus();
                return;
            }
            ...
            for (var i=0;i<itemreg.optionName1.length;i++){
                ...
                //2013/12/18 추가
                if (getByteLength(itemreg.optionName1[i].value)>maxbytelen){
                    alert('옵션명이 너무깁니다.옵션명을 줄여주세요.');
                    itemreg.optionName1[i].focus();
                    return;
                }
            }
            ...
            for (var i=0;i<itemreg.optionName2.length;i++){
                ...
                if (getByteLength(itemreg.optionName2[i].value)>maxbytelen){
                    alert('옵션명이 너무깁니다.옵션명을 줄여주세요.');
                    itemreg.optionName2[i].focus();
                    return;
                }
            }
            ...
            if (itemreg.optionTypename3.value.length>0){
                ...
                for (var i=0;i<itemreg.optionName3.length;i++){
                    ...
                    if (getByteLength(itemreg.optionName3[i].value)>maxbytelen){
                        alert('옵션명이 너무깁니다.옵션명을 줄여주세요.');
                        itemreg.optionName3[i].focus();
                        return;
                    }
                }
    ...
    if(confirm(strWMsg +"상품을 올리시겠습니까? \n담당MD 승인후 반영 됩니다.") == true){
...
function TnCheckUpcheYN(frm){
	if (frm.mwdiv[0].checked || frm.mwdiv[1].checked){
		...
		frm.deliverOverseas.checked=true;
...
function TnCheckOptionYN(frm){
	if (frm.useoptionyn[0].checked == true) {
       ...
       // document.itemreg.optlevel.value= "1"; // 2016/11/10 주석
...
function popDispCateSelect(){
	...
	$.ajax({
		url: "/common/module/act_DispCategorySelectUpche.asp?designerid="+designerid+"&isDft="+dCnt+"&chk="+cCnt,
		cache: false,
		success: function(message) {
...
function addDispCateItem(dcd,cnm,div,dpt) {
	...
	oCell3.innerHTML = "<img src='<%= fixImgUrl %>/photoimg/images/btn_tags_delete_ov.gif' onClick='delDispCateItem()' align=absmiddle>";
...
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
  <tr align="left">
    ...
    <input type="hidden" name="cd3" value="">
      ...
      <input type="text" name="cd1_name" value="" id="[on,off,off,off][카테고리1]" size="20" readonly class="text_ro">
      <input type="text" name="cd2_name" value="" id="[on,off,off,off][카테고리2]" size="20" readonly class="text_ro">
      <input type="text" name="cd3_name" value="" id="[on,off,off,off][카테고리3]" size="20" readonly class="text_ro">
...
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
  ...
  <tr align="left">
  	...
  	<td bgcolor="#FFFFFF" colspan="3">
      <input type="text" name="sourcearea" maxlength="64" size="25" class="text" id="[on,off,off,off][원산지]">&nbsp;(ex:한국,중국,중국OEM,일본 등 / 식품일 경우 국내: 국내산 또는 시군구명, 수입: 미국산, 중국산 등)
  ...
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
		<input type="text" name="itemWeight" maxlength="12" size="8" id="[on,off,off,off][상품무게]" style="text-align:right" value="0">g &nbsp;(그램단위로 입력, ex:1.5kg→ 1500) / 해외배송시 배송비 산출을 위한 것이므로 정확히 입력./숫자만 입력
	</td>
</tr>
  <tr align="left">
  	<td height="30" width="15%" bgcolor="#DDDDFF">검색키워드 :</td>
  	<td bgcolor="#FFFFFF" colspan="3">
      <input type="text" name="keywords" maxlength="260" size="120" class="text" id="[on,off,off,off][검색키워드]">&nbsp;(콤마로구분 ex: 커플,티셔츠,조명)
  	</td>
  </tr>
</table>
...
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
  ...
  <tr align="left">
  	...
  	<td width="35%" bgcolor="#FFFFFF" colspan="3">
		...
		<label><input type="checkbox" name="deliverOverseas" value="Y" title="해외배송은 상품무게가 입력이 돼야 완료됩니다.">해외배송</label>
  	</td>
  </tr>
...
<tr id="opttype" style="display:none" height="40">
    ...
    <td width="85%" bgcolor="#FFFFFF">
        ...
        <input type="radio" name="optlevel" value="2" onClick="TnCheckOptionYN(this.form);"   >이중 옵션 (옵션 구분 최대 3개) <!--<font color="blue">※ 매입특정구분이 업체배송인 경우만 선택가능합니다.</font> //2016.5.20 정윤정 삭제-->
    </td>
</tr>
<!----- 단일 옵션 DIV ----->
...
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
  <tr align="left">
  	...
  	<td bgcolor="#FFFFFF" colspan="3">
      <input type="radio" name="usinghtml" value="N"  >일반TEXT
      <input type="radio" name="usinghtml" value="H" checked>TEXT+HTML
 ...
 <tr bgcolor="#FFFFFF">
  ...
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain2, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain3, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain4, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain5, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain6, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain7, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <!-- 20160601추가한부분 -->
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain8, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain9, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain10, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain11, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <tr align="left">
	...
	<td bgcolor="#FFFFFF" colspan="3">
	  ...
	  <input type="button" value="이미지지우기" class="button" onClick="ClearImage(this.form.mobileimgmain12, 40, 640, 1200)"> (선택,640X1200, Max <%= CMOBILE_IMG_MAXSIZE %>KB,jpg,gif)
	</td>
  </tr>
  <!--// 20160601추가한부분 -->
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="a" bgcolor="F4F4F4">
    <tr valign="top" height="25">
        ...
        <td valign="bottom" align="center" style="padding:10px 0 0 0;">
          <span id="regbtn"><input type="button" value="저장하기" class="button_s" onClick="SubmitSave()"></span>
...
<iframe name="FrameCKP" src="about:blank" frameborder="0" width="210" height="210"></iframe>
<% else %>
