...
iColorCD = requestCheckVar(Request.Querystring("iCD"),20)
itemId = requestCheckVar(Request.Querystring("iid"),20)
...
<!--
	...
	//상품 검색
	function findItemid() {
		var pwnd = window.open("/common/pop_upche_itemSearchList.asp","popFindItem","width=550,height=500,scrollbars=yes");
		pwnd.focus();
	}

	//상품ID 삽입
	function putActItems(iid) {
		document.frmItemColor.iid.value=iid;
	}
//-->
</script>
...
<tr>
	...
	<td bgcolor="#FFFFFF"><input type="text" name="iid" size="10" readonly maxlength="8" onClick="findItemid()"></td>
</tr>
<% else %>
<tr>
	<td bgcolor="<%= adminColor("tabletop") %>">상품코드</td>
...
<% end if %>
<tr>
	<td bgcolor="<%= adminColor("tabletop") %>">색상선택</td>
...
<tr>
	...
	<td bgcolor="#FFFFFF">
		...
		<tr>
			<td colspan="2"><font color="#808080">※이미지는 <font color=red>1000px×1000px</font>, <font color=red>560kb이하</font>의 <font color=red>JPG형식</font></font></td>
		</tr>
		<% IF slistImage <> "" THEN %>
		<tr>
