...
opage = requestCheckVar(request("opage"),20)
ourl = requestCheckVar(request("ourl"),200)
if idx="" then idx=0
...
function ModiThis(frm){
	...
	if(compno){
		...
		}else if(frm.dtstat.value == ''){
				alert('확정수량이 적습니다. 사유를 선택해주세요');
				return false;
		}
...
<table width="100%" border="0" align="center" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	...
	<tr bgcolor="#FFFFFF">
		...
		<td>
			...
			<%= ojumunmaster.FOneItem.GetStateName %> / <%= ojumunmaster.FOneItem.FALinkCode %>
		...
		<td>
			...
			<a href="javascript:jsPopCalendar('frmMaster','scheduleipgodate');"><img src="/images/calicon.gif" border="0" align="absmiddle" height=21></a>
		</td>
	</tr>
	<% if ojumunmaster.FOneItem.FStatecd>="1" then %>
	<tr bgcolor="#FFFFFF">
		...
		<td>
			...
			<a href="javascript:jsPopCalendar('frmMaster', 'beasongdate');"><img src="/images/calicon.gif" border="0" align="absmiddle" height=21></a>
		</td>
...
<% if ojumunmaster.FOneItem.FStatecd>="1" then %>
	...
	if IsNull(ojumundetail.FItemList(i).FDetail_status) then
		ojumundetail.FItemList(i).FDetail_status = ""
	end if
	%>
	...
	<tr align="center" bgcolor="#FFFFFF">
		...
		<% else %>
		<td><input type="text" name="realitemno" value="<%= ojumundetail.FItemList(i).Frealitemno %>"  size="2" maxlength="4" onKeyup="chkRealItemNo(<%= i %>);"></td>
		<td>
			<span id="seldiv<%=i%>">
			...
			<% if ojumundetail.FItemList(i).FDetail_status = "" then %>
				<script>ShowHide("seldiv<%=i%>", false)</script>
			<% end if %>
			<span id="comdiv<%=i%>">
					<input type="text" name="comment" value="<%= ojumundetail.FItemList(i).Fdetail_description %>"  size="10" maxlength="10" readonly>
			</span>
			<% if ojumundetail.FItemList(i).FDetail_status <> "직접입력" and ojumundetail.FItemList(i).FDetail_status <> "일시품절" then %>
				<script>ShowHide("comdiv<%=i%>", false)</script>
			<% end if %>
			<span id="caldiv<%=i%>">
					<a href="javascript:jsPopCalendar('frmBuyPrc_<%=i%>', 'comment');"><img src="/images/calicon.gif" border="0" align="absmiddle" height=21></a>
			</span>
			<% if ojumundetail.FItemList(i).FDetail_status <> "일시품절" then %>
				<script>ShowHide("caldiv<%=i%>", false)</script>
			<% end if %>
		</td>
