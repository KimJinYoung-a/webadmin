...
ojungsan.FRectIdx = idx
'ojungsan.FRectMakerId = session("ssBctID")
ojungsan.FRectGroupID = groupid
if (groupid<>"") then
ojungsan.GetOneOffJungsanMaster
end if
...
''수수료 발행 불가
if ojungsan.FOneItem.IsCommissionTax then
    response.write "<script>alert('수수료 계산서 발행 불가');</script>"
	response.write "<script>window.close()</script>"
	dbget.close()	:	response.End
end if

makerid = ojungsan.FOneItem.FMakerid
...
ogroup.FRectGroupid = ojungsan.FOneItem.Fgroupid
...
if (ojungsan.FOneItem.IsElecTaxCase) then
	stypename = "세금계산서"
elseif (ojungsan.FOneItem.IsElecFreeTaxCase) then
	stypename = "계산서"
else
...
function billTaxEvalFlexApi(pidx){
    ...
    <%
    dim FG_VAT
    if (ojungsan.FOneItem.Ftaxtype="03") then
        FG_VAT = "2"
    elseif (ojungsan.FOneItem.Ftaxtype="02") then
        FG_VAT = "3"
    else
        FG_VAT = "1"
    end if
    ...
    var obj8 = AddNew("AM","<%= ojungsan.FOneItem.GetTotalTaxSuply %>");
    var obj9 = AddNew("AM_VAT","<%= ojungsan.FOneItem.GetTotalTaxvat %>");
    var obj10 = AddNew("AMT","<%= ojungsan.FOneItem.Ftot_jungsanprice %>");

    var obj11 = AddNew("AMT_CASH","0");
    var obj12 = AddNew("AMT_CHECK","0");
    var obj13 = AddNew("AMT_NOTE","0");
    var obj14 = AddNew("YMD_WRITE","<%= Replace(ojungsan.FOneItem.GetPreFixSegumil,"-","") %>");
    ...
    var obj46 = AddNew("BUY_DAM_DEPT","오프라인");
    var obj47 = AddNew("AMT_AR","<%= ojungsan.FOneItem.Ftot_jungsanprice %>");      //외상미수금
    ...
    var obj51 = AddNew("NO_SENDER_PK","DZ_TEN_OF_<%= idx %>_<%= ojungsan.FOneItem.Fdifferencekey %>_<%= ojungsan.FOneItem.Ftot_jungsanprice %>");
    ...
    var objline1 = AddNew("ITEM_STD", "<%= Right(Replace(ojungsan.FOneItem.Fyyyymm,"-",""),4) %>");
    var objline2 = AddNew("NM_ITEM", "오프라인 <%= makerid %> 판매대금");
    var objline3 = AddNew("NO_ITEM", "1");
    var objline4 = AddNew("AM", "<%= ojungsan.FOneItem.GetTotalTaxSuply %>");
    var objline5 = AddNew("AM_VAT", "<%= ojungsan.FOneItem.GetTotalTaxvat %>");
    var objline6 = AddNew("AMT", "<%= ojungsan.FOneItem.Ftot_jungsanprice %>");
    var objline7 = AddNew("DD_WRITE", "<%= Mid(ojungsan.FOneItem.GetPreFixSegumil,9,2) %>");
    var objline8 = AddNew("MM_WRITE", "<%= Mid(ojungsan.FOneItem.GetPreFixSegumil,6,2) %>");
...
<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" class="a">
  <tr valign="bottom">
    <td width="10" height="10" align="right" valign="bottom"><img src="/images/tbl_header_round_01.png" width="10" height="25"></td>
    <td height="10" valign="bottom" background="/images/tbl_header_round_02.png">10x10 Business Communication Tool</td>
    <td width="10" height="10" align="left" valign="bottom"><img src="/images/tbl_header_round_03.png" width="10" height="25"></td>
  </tr>
  <tr valign="top" bgcolor="#F3F3FF">
    <td height="20" valign="middle" background="/images/tbl_blue_round_04.gif"></td>
    <td height="25" valign="middle"><img src="/images/icon_star.gif" align="absbottom">&nbsp;<strong>전자 <%= stypename %> 발행</strong></td>
    <td height="20" background="/images/tbl_blue_round_05.gif"></td>
  </tr>
</table>


<table width="95%" border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#000000" class="a">
<form name="frm" method="post" action="dotaxreg.asp">
<input type=hidden name=jungsanid value="<%= ojungsan.FOneItem.FIdx %>">
<input type=hidden name=jungsanname value="오프라인 <%= ojungsan.FOneItem.Fyyyymm %>월 정산">
<input type=hidden name=jungsangubun value="OF">
...
<input type=hidden name=tax_type value="<%= ojungsan.FOneItem.Ftaxtype %>">
<input type=hidden name=bill_type value="18"> <!-- 영수 01 청구 18 -->
<input type=hidden name=pc_gbn value="C"> <!-- 개인 P 기업 C -->

<input type=hidden name=item_count value="1">
<input type=hidden name=item_nm value="<%= "오프라인 " + makerid %> 판매대금">
<input type=hidden name=item_qty value="1">
<input type=hidden name=item_price value="<%= ojungsan.FOneItem.Ftot_jungsanprice %>">
<input type=hidden name=item_amt value="<%= ojungsan.FOneItem.GetTotalTaxSuply %>">
<input type=hidden name=item_vat value="<%= ojungsan.FOneItem.GetTotalTaxvat %>">
<input type=hidden name=item_remark value="">
<input type=hidden name=credit_amt value="<%= ojungsan.FOneItem.GetTotalTaxSuply + ojungsan.FOneItem.GetTotalTaxvat %>">

<input type=hidden name=cur_u_user_no value="261745"> <!-- DEV 1000394, REAL 244730, ON 261744 -->
...
<input type=hidden name=cur_hp_no3 value="0000">
	...
	<tr bgcolor="#FFFFFF">
   		<td colspan="2" height="20" valign="middle" bgcolor="#FFFFFF">
	...
	<tr bgcolor="#FFFFFF">
   		<td colspan="2" height="20" valign="middle" bgcolor="#FFFFFF">
   		<img src="/images/icon_arrow_down.gif" width="16" height="16" align="absbottom">
   		<strong>등록된 업체정보 확인</strong>
   		</td>
 	</tr>
	<tr bgcolor="#FFFFFF">
		<td bgcolor="#DDDDFF" width="30%">사업자명</td>
		<td><%= ogroup.FOneItem.FCompany_name %></td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td bgcolor="#DDDDFF">대표자명</td>
		<td><%= ogroup.FOneItem.Fceoname %></td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td bgcolor="#DDDDFF">사업자번호</td>
		<td><%= ogroup.FOneItem.Fcompany_no %></td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td bgcolor="#DDDDFF">과세구분</td>
	...
	<tr bgcolor="#FFFFFF">
		...
		<% if False and (ojungsan.FOneItem.Fdifferencekey>0) then %>
		...
		<% else %>
		<td>
			<input type=text name=write_date value="<%= ojungsan.FOneItem.GetPreFixSegumil %>" size="10" maxlength=10 readonly style="border:0"></td>
		</td>
		<% end if %>
	...
	<tr bgcolor="#FFFFFF">
		...
		<td><b><%= FormatNumber(ojungsan.FOneItem.Ftot_jungsanprice,0) %></b> (공급가 : <%= FormatNumber(ojungsan.FOneItem.GetTotalTaxSuply,0) %> 부가세: <%= FormatNumber(ojungsan.FOneItem.GetTotalTaxvat,0) %>)</td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td colspan="2">
	...
	<tr bgcolor="#FFFFFF">
	    ...
	    <% if (ojungsan.FOneItem.Favailneo=1) then %>
	...
	<tr bgcolor="#FFFFFF">
		<td colspan="2" align=center ><input type=button name="evalButton" value="전자 <%= stypename %> 발행" onClick="ActTaxReg(frm)"></td>
	</tr>
</form>
</table>
...
<input type="hidden" name="jungsangubun" value="OF">
<input type="hidden" name="write_date" value="<%= ojungsan.FOneItem.GetPreFixSegumil %>">
<input type="hidden" name="jungsanid" value="<%= ojungsan.FOneItem.FIdx %>">
</form>
