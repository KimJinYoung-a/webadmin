...
if (mode = "base") then
    if checkNotValidHTML(msnmail) or checkNotValidHTML(messenger) or checkNotValidHTML(mywork) then
    	response.write "<script type='text/javascript'>alert('내용에 사용하실수 없는 태그가 있습니다.');</script>"
...
elseif (mode = "mypass") then
	...
	if (rsget.EOF or rsget.BOF) then
	    ...
	    userid=rsget("id")  ''2014/07/14 추가
	end if
	rsget.Close

	if (newuserpass = "") then
		response.write "<script type='text/javascript'>alert('기존 비밀번호가 잘못 입력되었습니다.');</script>"
