...
function ShopReturnUpdateBySheetIdx(iidx)
    dim sqlStr

	if (iidx=0) or (iidx="") then exit function

    sqlStr = " exec [db_summary].[dbo].[sp_Ten_ShopReturn_UpdateBySheetIdx] " & iidx & ""
    dbget.Execute sqlStr
end function

function PreOrderUpdateBySheetIdx(iidx)
    dim sqlStr

	if (iidx=0) or (iidx="") then exit function

    sqlStr = " exec db_summary.dbo.sp_Ten_preOrder_UpdateBySheetIdx " & iidx & ""
    dbget.Execute sqlStr
end function
...
function PreOrderUpdateByBrand(targetid)
	...
	if (targetid="10x10") or (targetid="") then exit function

    sqlStr = " exec db_summary.dbo.sp_ten_preOrder_Update '" & targetid & "','',0,''"
    dbget.Execute sqlStr
    Exit function
