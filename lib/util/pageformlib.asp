...
Function fnDisplayPaging_New2017(strCurrentPage, intTotalRecord, intRecordPerPage, intBlockPerPage, strJsFuncName)
	...
	'## 이전 페이지
	If intStartBlock > 1 Then
		vPageBody = vPageBody & "	<a href=""#"" title=""이전 페이지"" onclick=""" & strJsFuncName & "(" & intStartBlock-1 & ");return false;"">[prev]</a>" & vbCrLf
	Else
		vPageBody = vPageBody & "	<a href=""#"" title=""이전 페이지"" onclick=""return false;""><span style=""cursor:pointer;"">[prev]</span></a>" & vbCrLf
	End If
	...
	If intTotalPage > 1 Then
		For intLoop = intStartBlock To intEndBlock
			If intLoop > intTotalPage Then Exit For
			If Int(intLoop) = Int(intCurrentPage) Then
				vPageBody = vPageBody & "	<a href=""#"" title=""" & intLoop & " 페이지"" onclick=""" & strJsFuncName & "(" & intLoop & ");return false;""><span class='cRd1'>" & intLoop & "</span></a>" & vbCrLf
			Else
				vPageBody = vPageBody & "	<a href=""#"" title=""" & intLoop & " 페이지"" onclick=""" & strJsFuncName & "(" & intLoop & ");return false;"">" & intLoop & "</a>" & vbCrLf
			End If
		Next
	Else
		vPageBody = vPageBody & "	<a href=""#"" title=""1 페이지"" onclick=""" & strJsFuncName & "(1);return false;""><span class='cRd1'>1</span></a>" & vbCrLf
	End If

	'## 다음 페이지
	If Int(intEndBlock) < Int(intTotalPage) Then	'####### 다음페이지
		vPageBody = vPageBody & "	<a href=""#"" title=""다음 페이지"" onclick=""" & strJsFuncName & "(" & intEndBlock+1 & ");return false;"">[next]</a>" & vbCrLf
	Else
		vPageBody = vPageBody & "	<a href=""#"" title=""다음 페이지"" class=""next arrow"" onclick=""return false;"">[next]</a>" & vbCrLf
	End If
