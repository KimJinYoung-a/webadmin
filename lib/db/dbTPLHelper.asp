...
' ##########################################################################
Function dbtpl_fnExecSPReturnRSOutput(ByVal strSql, ByRef params)

	Dim cmd, i
    Set cmd = CreateObject("ADODB.Command")

    cmd.ActiveConnection = dbget_TPL
    cmd.CommandText = strSql
    cmd.CommandType = adCmdStoredProc
    Set cmd = dbtpl_collectParams(cmd, params)
    'cmd.Parameters.Refresh

    rsget_TPL.CursorLocation = adUseClient
    rsget_TPL.Open cmd, ,adOpenForwardOnly, adLockReadOnly
	...
	Set cmd = Nothing
    Set rsget_TPL.ActiveConnection = Nothing
...
' ##########################################################################
Function dbtpl_fnExecSPOutput(ByVal strSql,ByVal arrParm)
    ...
    With objCmd
      ...
      Set objCmd = dbtpl_collectParams(objcmd, arrParm)
      .Execute
...
'---------------------------------------------------------------------------
Function dbtpl_collectParams(objCmd,arrParm)
	Dim i,l,u,v
...
'rw v
	    ...
	    Set dbtpl_collectParams = objCmd
	    Exit Function
    Else
	    Set dbtpl_collectParams = objCmd
    End If
