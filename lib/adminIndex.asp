...
function chkForm() { 
	if(document.frmLogin.lgnMethod.value=="S") {
			...
			try { sn = MaGerAuth.GetSN();   }
				...
				alert('텐바이텐 인증용 ActiveX 컨트롤을 설치해주세요');
			...
			if (sn != '') {
			    ...
			    <% if (Not NoUsbValidIP) then %>
				alert('텐바이텐 웹어드민 인증용 USB키를 PC에 꼽고 다시 로그인해주세요.');
				return  ; 
				<% end if %>
			}
...
function PopChgHPNum() {
	alert("IP(or USB) 로그인 후 나의정보에서 휴대폰 본인확인 후 이용가능합니다.");
	return;
...
<body class="scmLogin <%if Application("scmBG") = "" then%>noImage<%end if%>" <%if Application("scmBG") <> "" then%>style="background-image:url(<%=Application("scmBG")%>);"<%end if%>>
	...
	<div class="loginBox">
		<div class="tabNav">
			<ul>
				<li class="tab1"><a href="#tab1"><%=CHKIIF(NoUsbValidIP,"IP","USB키")%></a></li>
				<li class="tab2"><a href="#tab2">SMS</a></li>
				<li class="tab3"><a href="#tab3">사번</a></li>
			</ul>
		</div>
	...
	<ul class="help">
		<li><a href="javascript:popSecLgnInfo('U')">USB키 인증안내</a></li>
