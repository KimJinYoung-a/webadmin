...
function DeliverDivTrace(byval divcd)
	if isNull(divcd) then
		...
		elseif CStr(divcd) = "9" then
			'(구)KGB
		    DeliverDivTrace =  "http://www.kgbls.co.kr/sub5/sub5_1.asp?f_slipno="
		elseif CStr(divcd) = "10" then
			'아주택배
		...
		elseif CStr(divcd) = "13" then
			'옐로우캡
		    DeliverDivTrace =  "https://www.yellowcap.co.kr/custom/custom03.asp?INVOICE_NO="
		elseif CStr(divcd) = "14" then
			'나이스택배
		    DeliverDivTrace =  ""
		elseif CStr(divcd) = "15" then
		...
		elseif CStr(divcd) = "28" then
			'동부택배
		    DeliverDivTrace =  "http://www.dongbups.com/newHtml/delivery/dvsearch.jsp?search_item_no="
		elseif CStr(divcd) = "35" then
			'CSVNet(편의점택배)
		    DeliverDivTrace =  "http://was.cvsnet.co.kr/_ver2/board/ctod_status.jsp?pageNum=3&subNum=1&ssNum=0&invoice_no="
		elseif CStr(divcd) = "99" then
		    DeliverDivTrace =  ""
		end if
...
%>
    <select name="<%= selBoxName %>" <%= chplg %>>
		...
		<option value='3' <% if cstr(selVal)=cstr("3") then response.write " selected" %> >주문접수</option>
...
Sub NewDrawSelectBoxDesignerwithNameAndUserDIV(selectBoxName, selectedId, userdiv)
    ...
    strRst = "<input type=""text"" class=""text"" name=""" & selectBoxName & """ value=""" & selectedId & """ size=""20"" >" & vbCrLf
...
Sub DrawYMSelBox(byval fy,fm,yyyy,mm)
	...
	buf = "<select class='select' name='" & fy & "'>"
    for i=2010 to (Year(now)+2)
    ...
    buf = buf + "<select class='select' name='" & fm & "' >"
...
Sub drawSelectBoxCoWorker_OnOff(byval selectBoxName, selectedId, onoff)
   ...
   query1 = query1 & " where  isusing= 1" & vbcrlf
	...
	' 퇴사예정자 처리	' 2018.10.16 한용민
	query1 = query1 & "	and (statediv ='Y' or (statediv ='N' and datediff(dd,retireday,getdate())<=0))" & vbcrlf
	query1 = query1 & "	and part_sn in(" + vOnOff + ") and userid <> '' "
    query1 = query1 & " order by username asc"
...
Sub DrawSelectExtSiteName(byval selectBoxName,extsitename)
	...
	sqlStr = " select top 100 (case when id in ('cjmall','interpark','lotteCom','lotteimall','gseshop','ezwel','Homeplus') then 1 else 100 end) as dispno, id from [db_partner].[dbo].tbl_partner "
	sqlStr = sqlStr + " where userdiv='999' "
	sqlStr = sqlStr + " and id<>'10x10' "
	sqlStr = sqlStr + " and isusing='Y' "
	sqlStr = sqlStr + " order by (case when id in ('cjmall','interpark','lotteCom','lotteimall','gseshop','ezwel','Homeplus') then 1 else 100 end), id "
	rsget.Open sqlStr,dbget,1
...
%>
    <select name="<%= selBoxName %>" <%= chplg %>>
		...
		<option value="ONLINE" <% if selVal="ONLINE" then response.write " selected" %>>ONLINE</option>
		<option value="OFFLINE" <% if selVal="OFFLINE" then response.write " selected" %>>OFFLINE</option>
	</select>
<%
end function
...
%>
	<select class="select" name="<%=selectBoxName%>" <%= chplg %>>
		...
		<option value='1UP20DOWN' <%if selectedId="1UP20DOWN" then response.write " selected"%>>1개 이상 20개이하</option>
...
%>
    <select name="<%=selectBoxName%>" <%= chplg %>>
        <option value='' <%if selectedId="" then response.write " selected"%>>선택</option>
<%
    query1 = " select notificationType, notificationName"
    ...
    if not rsget.EOF then
        ...
        response.write("<option value='"&rsget("notificationType")&"' "&tmp_str&">" + rsget("notificationName") + "</option>")
        tmp_str = ""
        rsget.MoveNext
        loop
    end if
