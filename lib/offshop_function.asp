...
Sub drawSelectBoxOffShopdiv_off(selectBoxName,selectedId,shopdiv,shopall,chplg)
	...
	<%
		...
		query1 = query1 & " where 1=1 "		'/and isusing='Y'	'/2017.05.10 한용민 회계팀에서 사용안함도 보이게 해달라고함.
...
'//오프라인매장 구분에 따라서 동적으로 가져 오기	'/정태훈 추가
...
'/주문상태에 따라서 수량 반환		'/2013.07.04 한용민 생성
...
'/주문상태에 따라서 공급가 반환			'/2013.07.04 한용민 생성
...
Sub drawSelectBoxShopBeasongDiv(selectBoxName,selectedId)
   ...
   %>
	<select class="select" name="<%=selectBoxName%>">
		<option value="">선택
		<option value="0" <% if (selectedId="0") then response.write "selected" %> >매장판매
...
'/할인유형구분	: 디테일 discountKind		'/2012.02.10 한용민 생성
...
function getdisp_price_currencyChar(price, currencyChar)
	...
	if len(bufCurrencyChar)<3 then
		...
		'2022-08-25 김진영 하단 주석처리
		' elseif (bufCurrencyChar="￥") then
		' 	tmpdisp = bufCurrencyChar & " " & FormatNumber(price ,0)	'/일본화폐 소수점 없음
		else
	    ...
	    ' 	tmpdisp = FormatNumber(price ,0) & " " & bufCurrencyChar	'/일본화폐 소수점 없음
	    else
...
function getdisp_price(price, currencyChar)
	...
	if len(bufCurrencyChar)<3 then
		...
		'2022-08-25 김진영 하단 주석처리
		' elseif (bufCurrencyChar="￥") then
		' 	tmpdisp = FormatNumber(price ,0)	'/일본화폐 소수점 없음
		else
