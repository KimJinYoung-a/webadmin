...
<%
'#############################################################
'	PageName 	: /lib/newSearchzip.asp
'	Description : 우편번호 찾기 - 우편번호 6자리를 하나로 전송 > 추후 5자리로 통합 예정
'	History		: 2015.11.23 허진원 생성
'#############################################################
%>
<HTML>
<HEAD>
...
.kindTab_on {width:50%;background-color:#DEF8D0;padding:5px 0 5px 0;float:left;font-weight:bold;cursor:pointer;}
.kindTab_off {width:50%;background-color:#E0E0E0;padding:5px 0 5px 0;float:left;font-weight:normal;cursor:pointer;}
</style>
...
<form action="/lib/newSearchzip.asp?" method="get" name="gil" onsubmit="SubmitForm(document.gil); return false;">
...
function CopyZip(t,zipcode,add,dong)
{
...
<form action="/lib/newSearchzip.asp?" method="get" name="gil2" onsubmit="SubmitForm(document.gil2); return false;">
<input type="hidden" name="target" value="<%=strTarget%>">
...
%>
				<tr bgcolor="#FFFFFF">
                  <td class="a" width="109" align="center" onclick="CopyZip('<%= strTarget %>','<%=rsget("ADDR_ZIP1") & "-" & rsget("ADDR_ZIP2")%>','<% = useraddr01 %>', '<% = useraddr02 %>')" style="cursor:hand">
						<input type="text" name="zipcode" size="7" value='<%=rsget("ADDR_zip1") & "-" & rsget("ADDR_zip2")%>' class="input" style="cursor:hand">
                  </td>
