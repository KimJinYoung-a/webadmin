...
Class CItemOptionMultiple
    ...
    public Sub GetOptionTypeInfo
        ...
        sqlstr = sqlstr + " 	from db_item.dbo.tbl_item_option_Multiple with (nolock)"
...
Class CItemOption
    ...
    public frectitemdivexists
    public frectitemdivNotexists
    ...
    public Sub GetItem_Option
        ...
        if frectitemdivexists<>"" then
            sqlstr = sqlstr & " and i.itemdiv in ("& frectitemdivexists &")"
        end if
    ...
    public Sub GetItemOptionInfo
		...
		sqlstr = sqlstr + " sm.lastupdate, IsNull(s.rackcodeByOption, o.optrackcode) as optrackcode, s.subRackcodeByOption as suboptrackcode, IsNull(o.optdanjongyn, 'N') as optdanjongyn "
		sqlstr = sqlstr + " from [db_item].[dbo].tbl_item_option o with (nolock) "
		...
		sqlstr = sqlstr + "     on s.itemgubun='10' and o.itemid=s.itemid and o.itemoption=s.itemoption"
		...
		if  not rsget.EOF  then
			...
			do until rsget.eof
                ...
                FItemList(i).Fsuboptrackcode = rsget("suboptrackcode")
    ...
    ' /common/pop_simpleitemedit.asp
    public Sub GetItemrackcodeInfo
		dim sqlstr,i, sqlsearch

        if frectitemgubun<>"" then
            sqlsearch = sqlsearch & " and h.itemgubun='"& frectitemgubun &"'"
        end if
        if frectitemid<>"" then
            sqlsearch = sqlsearch & " and h.itemid="& frectitemid &""
        end if

		sqlStr = "select top " & Cstr(FPageSize * FCurrPage) + vbcrlf
        sqlstr = sqlstr & " h.idx, h.itemgubun, h.itemid, h.itemoption, h.regadminid, h.regdate, h.rackcodeByOption, h.subRackcodeByOption, h.comment"
        sqlstr = sqlstr & " , o.optionname"
        sqlstr = sqlstr & " from db_log.dbo.tbl_iteminfo_option_history h with (nolock)"
        sqlstr = sqlstr & " left join db_item.dbo.tbl_item_option o with (nolock)"
        sqlstr = sqlstr & "     on h.itemid=o.itemid"
        sqlstr = sqlstr & "     and h.itemoption=o.itemoption"
...
public function fnGetOptAddPrice(ByVal itemid)
 if isNull(itemid) then exit Function
	...
	strSql = " SELECT isNull(sum(optaddprice),0) as OptAddPrice FROM  db_item.dbo.tbl_item_option with (nolock) where itemid = " & itemid
	rsget.Open strSql,dbget,1
		if Not rsget.Eof then
...
Class CItemAddImage
    ...
    public Sub GetOneItemAddImageList()
	    ...
	    sqlstr = "select top 100 * from [db_item].[dbo].tbl_item_addimage with (nolock)"
    ...
    public Function GetAddImageListIMGGUBUN1()
        dim sqlstr, i

        sqlstr = "select top 1 * from [db_item].[dbo].tbl_item_addimage with (nolock)"
        sqlstr = sqlstr & " where itemid=" & FRectItemID & " and GUBUN = 1 "
        sqlstr = sqlstr & " ORDER BY idx desc"
        rsget.Open sqlStr,dbget,1
        If Not rsget.Eof Then
            GetAddImageListIMGGUBUN1 = rsget.getrows()
        End If
        rsget.Close
    end Function

    public Function GetAddImageListIMGGUBUN2()
        dim sqlstr, i

        sqlstr = "select top 1 * from [db_item].[dbo].tbl_item_addimage with (nolock)"
        sqlstr = sqlstr & " where itemid=" & FRectItemID & " and GUBUN = 2 "
        sqlstr = sqlstr & " ORDER BY idx desc"
        rsget.Open sqlStr,dbget,1
        If Not rsget.Eof Then
            GetAddImageListIMGGUBUN2 = rsget.getrows()
        End If
        rsget.Close
    end Function
...
Class CItemColor
	...
	public function GetColorList()
        ...
        sqlStr = sqlStr & " from [db_item].[dbo].tbl_colorChips WITH(NOLOCK)"
	...
	public function GetColorItemList()
        ...
        sqlStr = sqlStr & " from [db_item].[dbo].tbl_colorChips as C WITH(NOLOCK) "
        sqlStr = sqlStr & " 	Join [db_item].[dbo].tbl_item_colorOption as O WITH(NOLOCK) "
...
Class CItem
    ...
    public FRectExceptNOTEpMapitem ''EpMap상품제외 2019/11/04
    ...
    public FRecItemManageType
    public FRectSizeYn
    public FRectdispcateviewyn
    public FRectGroupID
    public farrlist
    Public FRectResultCode
    Public FRectErrMsg
    Public FRectShowCopyitem
    public frectitemdivexists
    public frectitemdivNotexists
    public frectreserveItemTp
	...
	public Sub GetOneItem()
		...
		''sqlstr = sqlstr + " ,IsNull(sm.realstock,0) as realstock "
        sqlstr = sqlstr + " ,isNull((select sum(sm.realstock) from [db_summary].[dbo].tbl_current_logisstock_summary sm where sm.itemgubun='10' and i.itemid=sm.itemid),0) as realstock "
		sqlstr = sqlstr + " ,IsNull(sm.ipkumdiv2,0) as ipkumdiv2 "
		sqlstr = sqlstr + " ,IsNull(sm.ipkumdiv4,0) as ipkumdiv4 "
		sqlstr = sqlstr + " ,IsNull(sm.ipkumdiv5,0) as ipkumdiv5 "
		sqlstr = sqlstr + " ,IsNull(sm.offconfirmno,0) as offconfirmno "
        ...
        sqlstr = sqlstr + " , IsNull(s2.rackcodeByOption, i.itemrackcode) as itemrackcode "
        sqlstr = sqlstr + " , IsNull(s2.subRackcodeByOption, la.subitemrackcode) as subitemrackcode "
		sqlstr = sqlstr + " from [db_item].[dbo].tbl_item i with (nolock)"
		sqlstr = sqlstr + " left join [db_item].[dbo].tbl_item_Contents s with (nolock) on i.itemid=s.itemid"
		''카테고리관련
		sqlstr = sqlstr + " left join [db_item].[dbo].vw_category v with (nolock) "
        ...
        '// 랙코드(8자리)
		sqlstr = sqlstr + "     left join [db_item].[dbo].[tbl_item_option_stock] s2 with (nolock) "
		sqlstr = sqlstr + "     on s2.itemgubun='10' and i.itemid=s2.itemid and s2.itemoption='0000' "
        sqlstr = sqlstr + "     left join [db_item].[dbo].[tbl_item_logics_addinfo] la with (nolock) on i.itemid=la.itemid "
		''외국어관련
		sqlstr = sqlstr + " left join db_item.dbo.tbl_item_multiLang ml with (nolock)"
		sqlstr = sqlstr + "     on ml.countryCd='EN' and i.itemid=ml.itemid "
		''오픈예약 2014.02.20 정윤정 추가
		if FRectSellReserve <> "" then
		    sqlStr = sqlStr & " left join db_item.dbo.tbl_item_sellReserve as R with (nolock) "
		...
		sqlStr = sqlStr + " left join db_item.dbo.tbl_item_pack_Volumn vo with (nolock) "
		...
		if Not rsget.Eof then
            ...
            FOneItem.Fsubitemrackcode    = rsget("subitemrackcode")
			...
			FOneItem.FisCurrStockExists = (rsget("realstock")>0)
		...
		if FResultCount>0 then
			If FOneItem.FsafetyYn = "Y" Then
				sqlStr = "select tw.safetyDiv, tw.certNum from db_item.[dbo].[tbl_safetycert_tenReg] as tw with (nolock) "
				sqlStr = sqlStr & "left join db_item.[dbo].[tbl_safetycert_info] as iw with (nolock) on tw.itemid = iw.itemid and tw.certNum = iw.certNum "
	...
	public Sub GetItemContentsVideo()
		...
		sqlstr = sqlstr + " from [db_item].[dbo].tbl_item_videos with (nolock) "
		sqlstr = sqlstr + " where itemid=" + CStr(FRectItemID)
	...
	public function GetItemoptionList()
        ...
        if (FRectItemName <> "") then
            ...
            ELSEIF FRectSortDiv="cashH" and FRectMakerid<>"" Then
                sqlStr = sqlStr & " Order by ai.SellCash desc "
            ELSEIF FRectSortDiv="cashL" and FRectMakerid<>"" Then
                sqlStr = sqlStr & " Order by ai.SellCash"
            ELSEIF FRectSortDiv="best" and FRectMakerid<>"" Then
                sqlStr = sqlStr & " Order by ai.ItemScore desc "
            ELSE
                sqlStr = sqlStr & " Order by B.itemid desc "
            End IF
        ...
        sqlStr = sqlStr & " 	on i.itemid = o.itemid"
        sqlStr = sqlStr & " where i.itemid<>0" & addSql
		sqlStr = sqlStr & " order by i.itemid desc, o.itemoption desc" & vbcrlf
    ...
    ' 상품매입정보 이상구 생성
	public function GetBuyItemListOn()
		dim sqlStr, addSql, i
		...
		if (FRectItemOption <> "") and (FRectItemOption <> "0000") then
			addSql = addSql & " and IsNull(o.itemoption, '0000') = '" & FRectItemOption & "' "
		end if

		sqlStr = ""
		sqlStr = sqlStr & " select top " & FPageSize*FCurrPage
		sqlStr = sqlStr & "	'10' as itemgubun, i.itemid, IsNull(o.itemoption, '0000') as itemoption, s.barcode, s.upchemanagecode "
		sqlStr = sqlStr & "		, i.itemname, IsNull(o.optionname, '') as optionname "
		sqlStr = sqlStr & "		, b.makerid, b.buyitemname, b.buyitemoptionname, b.buyitemprice, b.currencyUnit "
		sqlStr = sqlStr & "		, ei.itemname as engItemname "
		sqlStr = sqlStr & "		, eo.optionname as engItemoptionname "
		sqlStr = sqlStr & "		, i.makerid as engMakerid "
		sqlStr = sqlStr & "	from "
		sqlStr = sqlStr & "		[db_item].[dbo].tbl_item i with (nolock)"
		sqlStr = sqlStr & "		left join [db_item].[dbo].[tbl_item_option] o with (nolock)"
		sqlStr = sqlStr & "		on "
		...
		sqlStr = sqlStr & "		    on b.itemgubun='10'"
		sqlStr = sqlStr & "			and b.buyitemid=i.itemid"
		sqlStr = sqlStr & "			and b.itemoption=IsNull(o.itemoption, '0000')"
		sqlStr = sqlStr & "		left join db_item.[dbo].[tbl_item_multiLang] ei with (nolock)"
		sqlStr = sqlStr & "		on "
		sqlStr = sqlStr & "			1 = 1 "
		sqlStr = sqlStr & "			and ei.itemid = i.itemid "
		sqlStr = sqlStr & "			and ei.countryCd = 'EN' "
		sqlStr = sqlStr & "			and ei.useyn = 'Y' "
		sqlStr = sqlStr & "		left join db_item.[dbo].[tbl_item_multiLang_option] eo with (nolock)"
		sqlStr = sqlStr & "		on "
		sqlStr = sqlStr & "			1 = 1 "
		sqlStr = sqlStr & "			and eo.itemid = i.itemid "
		sqlStr = sqlStr & "			and eo.itemoption = IsNull(o.itemoption, '0000') "
		sqlStr = sqlStr & "			and eo.countryCd = 'EN' "
		sqlStr = sqlStr & "			and eo.isusing = 'Y' "
		sqlStr = sqlStr & "	where "
		sqlStr = sqlStr & "		1 = 1 "
		...
		sqlStr = sqlStr & "		i.itemid desc, IsNull(o.itemoption, '0000') "
        ...
        if  not rsget.EOF  then
            rsget.absolutepage = FCurrPage
            do until rsget.EOF
                set FItemList(i) = new CBuyItemDetail
				...
				FItemList(i).FengItemname      	= rsget("engItemname")
				FItemList(i).FengItemoptionname = rsget("engItemoptionname")
				FItemList(i).FengMakerid      	= rsget("engMakerid")
    ...
    ' 이함수를 변경할경우 GetItemListOption_excel 의 조건문도 같이 수정해줘야 한다.
	public function GetItemList()
		...
		if FRectDispCate<>"" then
			...
			addSql = addSql + " and i.itemid in (select itemid from db_item.dbo.tbl_display_cate_item with (nolock) where catecode like '" + FRectDispCate + "%' and isDefault='y') "
		end if
        ...
        if FRectGroupID<>"" then
            ...
            addSql = addSql & " 	from db_partner.dbo.tbl_partner as sp with (nolock) "
            addSql = addSql & " 		join db_user.dbo.tbl_user_c as sc with (nolock) "
            ...
            addSql = addSql & " 		and sp.groupid='" & FRectGroupID & "' "
            addSql = addSql & " ) "
        end if
        If FRectreserveItemTp <> "" Then
        	addSql = addSql + " and i.reserveItemTp='1'"
        End If
        ...
        sqlStr = sqlStr & " left Join [db_item].[dbo].tbl_item_contents Ct with (nolock)"
        ...
        if FRectdispcateviewyn="Y" then
            sqlStr = sqlStr & " left join db_item.dbo.tbl_display_cate_item as ci with (nolock)" & vbcrlf
            sqlStr = sqlStr & "     on i.itemid = ci.itemid and ci.isDefault='Y'" & vbcrlf
        end if
        ...
        '// 본문 내용 접수
        sqlStr = "select top " + Cstr(FPageSize * FCurrPage)
        sqlStr = sqlStr & " i.itemid,i.makerid,i.cate_large,i.cate_mid,i.cate_small,i.itemdiv,i.itemgubun"
        sqlStr = sqlStr & " , replace(replace(replace( i.itemname ,char(9),''),char(10),''),char(13),'') as itemname"
        sqlStr = sqlStr & " ,i.itemname,i.sellcash,i.buycash,i.orgprice,i.orgsuplycash,i.sailprice,i.sailsuplycash"
        sqlStr = sqlStr & " ,i.mileage,i.regdate,i.lastupdate,i.sellEndDate,i.sellyn,i.limityn,i.danjongyn,i.sailyn"
        sqlStr = sqlStr & " ,i.isusing,i.isextusing,i.mwdiv,i.specialuseritem,i.vatinclude,i.deliverytype,i.availPayType"
        sqlStr = sqlStr & " ,i.deliverarea,i.deliverfixday,i.ismobileitem,i.pojangok,i.limitno,i.limitsold,i.evalcnt"
        sqlStr = sqlStr & " ,i.evalCnt_photo,i.optioncnt,i.itemrackcode,i.upchemanagecode,i.reipgodate,i.brandname,i.titleimage"
        sqlStr = sqlStr & " ,i.mainimage,i.smallimage,i.listimage,i.listimage120,i.basicimage,i.icon1image,i.icon2image,i.itemcouponyn"
        sqlStr = sqlStr & " ,i.curritemcouponidx,i.itemcoupontype,i.itemcouponvalue,i.itemscore,i.itemWeight,i.deliverOverseas"
        sqlStr = sqlStr & " ,i.tenOnlyYn,i.basicimage600,i.mainImage2,i.frontMakerid,i.reserveItemTp,i.mainimage3,i.basicimage1000"
        sqlStr = sqlStr & " ,i.maskimage,i.maskimage1000,i.orderMinNum,i.orderMaxNum,i.dispcate1,i.limitdispyn,i.sellSTDate"
        sqlStr = sqlStr & " ,i.tentenimage,i.tentenimage50,i.tentenimage200,i.tentenimage400,i.tentenimage600,i.tentenimage1000"
        sqlStr = sqlStr & " ,i.evaloffcnt,i.adultType"
        ...
        sqlStr = sqlStr & " , CASE itemcouponyn WHEN 'Y' THEN cd.couponbuyprice Else 0 END AS couponbuyprice "
        sqlStr = sqlStr & " , Ct.isbn13, ct.isbn10, ct.isbn_sub"
      ...
      '  sqlStr = sqlStr & " ,(select isnull(sum(realstock),0) from [db_summary].[dbo].tbl_current_logisstock_summary where itemid = i.itemid ) as realstock "
        ...
        if FRectdispcateviewyn="Y" then
            sqlStr = sqlStr & " , ci.catecode" & vbcrlf
        end if
        ...
        sqlStr = sqlStr & "     on i.itemid=Ct.itemid"
        IF (FRectInfodivYn<>"") or (FRectShowInfodiv<>"") or (FRectKeyword<>"") then
            sqlStr = sqlStr & " Left Join [db_item].dbo.tbl_item_infoDiv fd with (nolock)"
		...
		sqlStr = sqlStr & "     LEFT JOIN [db_item].[dbo].tbl_item_coupon_detail as cd WITH(noLock)"
		sqlStr = sqlStr & "     	on cd.itemcouponidx=i.curritemcouponidx"
		sqlStr = sqlStr & "     		and cd.itemid=i.itemid"
        ...
        if FRectdispcateviewyn="Y" then
            ...
            sqlStr = sqlStr & "     and ci.isDefault='Y'" & vbcrlf      ' 기본카테고리
        end if
       ...
       ' sqlStr = sqlStr & " order by i.itemid desc"
        ...
        if  not rsget.EOF  then
            ...
            do until rsget.EOF
                ...
                if(isNull(FItemList(i).FsellSTDate)) then FItemList(i).FsellSTDate = ""
                FItemList(i).FsellEndDate       = rsget("sellEndDate")
                FItemList(i).Fsellyn            = rsget("sellyn")
                FItemList(i).Flimityn           = rsget("limityn")
                FItemList(i).Fdanjongyn         = rsget("danjongyn")
                ...
                if ((Not IsNULL(rsget("tentenimage600"))) and (rsget("tentenimage600")<>"")) then
                FItemList(i).Ftentenimage600 = webImgUrl & "/image/tenten600/" + GetImageSubFolderByItemid(FItemList(i).FItemID) + "/"  + rsget("tentenimage600")
                end if
                ...
                FItemList(i).fisbn10	= rsget("isbn10")
                FItemList(i).fisbnsub	= rsget("isbn_sub")
    ...
    ' 이함수를 변경할경우 GetItemList 의 조건문도 같이 수정해줘야 한다.
    public function GetItemListOption_excel()
        dim sqlStr, addSql, i
        ...
        ' [ON]상품관리>>상품수정 에서 엑셀다운로드 버튼 클릭시 설정한 페이지의 상품을 그대로 쿼리해서 임시테이블에 담는다.
        sqlStr = "select"
        sqlStr = sqlStr & " i.itemid"
        sqlStr = sqlStr & " into #tmp_itemexists"
        sqlStr = sqlStr & " from [db_item].[dbo].tbl_item i with (readuncommitted)"
        sqlStr = sqlStr & " Left Join [db_item].[dbo].tbl_item_contents Ct with (readuncommitted)"
        sqlStr = sqlStr & "     on i.itemid=Ct.itemid"
        IF (FRectInfodivYn<>"") or (FRectShowInfodiv<>"") or (FRectKeyword<>"") then
            sqlStr = sqlStr & " Left Join [db_item].dbo.tbl_item_infoDiv fd with (readuncommitted)"
            sqlStr = sqlStr & " on Ct.infoDiv=fd.infoDiv"
        end if

        If FRectPurchasetype <> "" Then
            sqlStr = sqlStr & " LEFT JOIN db_partner.dbo.tbl_partner as p with (readuncommitted) on i.makerid = p.id"
        End If

        sqlStr = sqlStr & "     left join [db_item].[dbo].tbl_item_addimage A with (readuncommitted) on i.itemid=A.itemid and A.ImgType=1 and A.Gubun=1"
        sqlStr = sqlStr & "     left join [db_user].[dbo].tbl_user_c c with (readuncommitted) on i.makerid=c.userid"

		if frectcolorcode <> "" then
			sqlStr = sqlStr & " join db_item.dbo.tbl_item_colorOption co with (readuncommitted)"
			sqlStr = sqlStr & " 	on i.itemid = co.itemid"
    	end if

		if FRectSellReserve <> "" then
			sqlStr = sqlStr & " left join db_item.dbo.tbl_item_sellReserve as R with (readuncommitted)"
			sqlStr = sqlStr & " on i.itemid = R.itemid and R.sellstartdate is null and R.canceldate is null "
		end if
        ...
        sqlStr = sqlStr & " CREATE NONCLUSTERED INDEX IX_itemid ON #tmp_itemexists(itemid ASC)"
        ...
        '// 본문 내용 접수
        sqlStr = "select top " + Cstr(FPageSize * FCurrPage)
        sqlStr = sqlStr & " i.itemid,i.makerid,i.cate_large,i.cate_mid,i.cate_small,i.itemdiv,i.itemgubun, replace(replace(replace( i.itemname ,char(9),''),char(10),''),char(13),'') as itemname,i.itemname,i.sellcash,i.buycash" & vbcrlf
        sqlStr = sqlStr & " ,i.orgprice,i.orgsuplycash,i.sailprice,i.sailsuplycash,i.mileage,i.regdate,i.lastupdate,i.sellEndDate" & vbcrlf
        sqlStr = sqlStr & " ,Case When i.sellyn='Y' and isNull(o.optsellyn,'Y')='Y' and isNull(o.isusing,'Y')='Y' then 'Y' "
        sqlStr = sqlStr & " 	When i.sellyn='Y' and (isNull(o.optsellyn,'Y')='N' or isNull(o.isusing,'Y')='N') then 'N' "
        sqlStr = sqlStr & "     Else i.sellyn end as sellyn "
        sqlStr = sqlStr & " ,Case When i.isusing='Y' and isNull(o.isusing,'Y')='N' then 'N' Else i.isusing end as isusing "
        sqlStr = sqlStr & " ,i.limityn,i.danjongyn,i.sailyn,i.isextusing,i.mwdiv,i.specialuseritem,i.vatinclude,i.deliverytype" & vbcrlf
        sqlStr = sqlStr & " ,i.availPayType,i.deliverarea,i.deliverfixday,i.ismobileitem,i.pojangok,i.limitno,i.limitsold,i.evalcnt" & vbcrlf
        sqlStr = sqlStr & " ,i.evalCnt_photo,i.optioncnt,i.itemrackcode,i.reipgodate,i.brandname,i.titleimage,i.mainimage" & vbcrlf
        sqlStr = sqlStr & " ,i.smallimage,i.listimage,i.listimage120,i.basicimage,i.icon1image,i.icon2image,i.itemcouponyn,i.curritemcouponidx" & vbcrlf
        sqlStr = sqlStr & " ,i.itemcoupontype,i.itemcouponvalue,i.itemscore,i.itemWeight,i.deliverOverseas,i.tenOnlyYn,i.basicimage600" & vbcrlf
        sqlStr = sqlStr & " ,i.mainImage2,i.frontMakerid,i.reserveItemTp,i.mainimage3,i.basicimage1000,i.maskimage,i.maskimage1000" & vbcrlf
        sqlStr = sqlStr & " ,i.orderMinNum,i.orderMaxNum,i.dispcate1,i.limitdispyn,i.sellSTDate,i.tentenimage,i.tentenimage50,i.tentenimage200" & vbcrlf
        sqlStr = sqlStr & " ,i.tentenimage400,i.tentenimage600,i.tentenimage1000,i.evaloffcnt,i.adultType" & vbcrlf
        ...
        sqlStr = sqlStr & " , isnull(o.itemoption,'0000') as itemoption , replace(replace(replace( o.optionname ,char(9),''),char(10),''),char(13),'') as optionname , o.optiontypename"
        sqlStr = sqlStr & " , isnull(o.optlimitno,i.limitno) as optlimitno, isnull(o.optlimitsold,i.limitsold) as optlimitsold "
        sqlStr = sqlStr & " , isnull(o.optaddprice,0) as optaddprice, isnull(o.optaddbuyprice,0) as optaddbuyprice "
        sqlStr = sqlStr & " , s.barcode, s.upchemanagecode, b.buyItemName, b.buyItemOptionName, isNull(b.buyItemPrice,0) as buyItemPrice, isNull(b.currencyUnit,'') as currencyUnit "
        sqlStr = sqlStr & " , Ct.isbn13, ct.isbn10, ct.isbn_sub"
        IF (FRectInfodivYn<>"") or (FRectShowInfodiv<>"") then
            sqlStr = sqlStr & " , Ct.infodiv, fd.infoDivName, Ct.sellcount, Ct.recentsellcount"
        end if
      ...
      '  sqlStr = sqlStr & " ,(select isnull(sum(realstock),0) from [db_summary].[dbo].tbl_current_logisstock_summary where itemid = i.itemid ) as realstock "
        ...
        sqlStr = sqlStr & " from [db_item].[dbo].tbl_item i with (readuncommitted)"
        sqlStr = sqlStr & " join #tmp_itemexists t"
        sqlStr = sqlStr & "     on i.itemid = t.itemid"
        sqlStr = sqlStr & " left Join [db_item].[dbo].tbl_item_contents Ct with (nolock)"
        sqlStr = sqlStr & "     on i.itemid=Ct.itemid"
        sqlStr = sqlStr & "     left join [db_item].[dbo].tbl_item_addimage A with (readuncommitted) on i.itemid=A.itemid and A.ImgType=1 and A.Gubun=1"
        sqlStr = sqlStr & "     left join [db_user].[dbo].tbl_user_c c with (readuncommitted) on i.makerid=c.userid"
        sqlStr = sqlStr & "     left join [db_item].[dbo].tbl_item_option o with (readuncommitted) on o.itemid = i.itemid"
        sqlStr = sqlStr & "     left join [db_item].[dbo].tbl_item_option_stock s with (readuncommitted) on i.itemid = s.itemid and isNull(o.itemoption,'0000')=s.itemoption and i.itemgubun=s.itemgubun "
        sqlStr = sqlStr & "     left join [db_shop].[dbo].tbl_buy_item b with (readuncommitted) on i.itemid = b.buyitemid and isNull(o.itemoption,'0000')=b.itemoption and i.itemgubun=b.itemgubun and b.isusing='Y' "
        ...
        sqlStr = sqlStr & " 	and i.itemgubun=ls.itemgubun"
        sqlStr = sqlStr & " 	and i.itemid=ls.itemid"
        sqlStr = sqlStr & " 	and isNull(o.itemoption,'0000')=ls.itemoption"
       ...
       ' sqlStr = sqlStr & " order by i.itemid desc"
        ...
        if  not rsget.EOF  then
            ...
            do until rsget.EOF
               ...
               ' FitemList(i).Frealstock				= rsget("realstock")
				...
				FItemList(i).FbuyItemName       = rsget("buyItemName")
				FItemList(i).FbuyItemOptionName = rsget("buyItemOptionName")
				FItemList(i).FbuyItemPrice      = rsget("buyItemPrice")
				FItemList(i).FcurrencyUnit      = rsget("currencyUnit")
        ...
        if (FRectItemName <> "") then
            sqlStr = " drop table #TMPSearchItem"
			dbget.Execute sqlStr
            sqlStr = " drop table #tmp_itemexists"
			dbget.Execute sqlStr
        end if
    end Function

	public function GetItemCopyList()
        dim sqlStr, addSql, i
        ...
        sqlStr = sqlStr & " from [db_item].[dbo].tbl_item i with (nolock)"
        IF (FRectInfodivYn<>"") or (FRectShowInfodiv<>"") or (FRectKeyword<>"") then
            sqlStr = sqlStr & " Join [db_item].[dbo].tbl_item_contents Ct with (nolock)"
            sqlStr = sqlStr & " on i.itemid=Ct.itemid"
        end if
		if frectcolorcode <> "" then
			sqlStr = sqlStr & " join db_item.dbo.tbl_item_colorOption co with (nolock)"
        ...
        If FRectShowCopyitem <> "" Then
            sqlStr = sqlStr & " join db_item.[dbo].[tbl_api_item_copy] as cp with (nolock)" & vbcrlf
            sqlStr = sqlStr & "     on i.itemid = cp.copyitemid and cp.resultCode='OK' " & vbcrlf
        End If
        ...
        sqlStr = "select top " + Cstr(FPageSize * FCurrPage)
        sqlStr = sqlStr & " i.itemid,i.makerid,i.cate_large,i.cate_mid,i.cate_small,i.itemdiv,i.itemgubun, replace(replace(replace( i.itemname ,char(9),''),char(10),''),char(13),'') as itemname,i.itemname,i.sellcash,i.buycash" & vbcrlf
        sqlStr = sqlStr & " ,i.orgprice,i.orgsuplycash,i.sailprice,i.sailsuplycash,i.mileage,i.regdate,i.lastupdate,i.sellEndDate,i.sellyn" & vbcrlf
        sqlStr = sqlStr & " ,i.limityn,i.danjongyn,i.sailyn,i.isusing,i.isextusing,i.mwdiv,i.specialuseritem,i.vatinclude,i.deliverytype" & vbcrlf
        sqlStr = sqlStr & " ,i.availPayType,i.deliverarea,i.deliverfixday,i.ismobileitem,i.pojangok,i.limitno,i.limitsold,i.evalcnt" & vbcrlf
        sqlStr = sqlStr & " ,i.evalCnt_photo,i.optioncnt,i.itemrackcode,i.upchemanagecode,i.reipgodate,i.brandname,i.titleimage,i.mainimage" & vbcrlf
        sqlStr = sqlStr & " ,i.smallimage,i.listimage,i.listimage120,i.basicimage,i.icon1image,i.icon2image,i.itemcouponyn,i.curritemcouponidx" & vbcrlf
        sqlStr = sqlStr & " ,i.itemcoupontype,i.itemcouponvalue,i.itemscore,i.itemWeight,i.deliverOverseas,i.tenOnlyYn,i.basicimage600" & vbcrlf
        sqlStr = sqlStr & " ,i.mainImage2,i.frontMakerid,i.reserveItemTp,i.mainimage3,i.basicimage1000,i.maskimage,i.maskimage1000" & vbcrlf
        sqlStr = sqlStr & " ,i.orderMinNum,i.orderMaxNum,i.dispcate1,i.limitdispyn,i.sellSTDate,i.tentenimage,i.tentenimage50,i.tentenimage200" & vbcrlf
    ...
    '상품복제 이력 리스트
    Public Sub getItemCopyHistoryList
		Dim sqlStr, addSql, i
		...
		sqlStr = sqlStr & " FROM db_item.[dbo].[tbl_api_item_copy] as Q "
		sqlStr = sqlStr & " JOIN db_item.dbo.tbl_item as i on Q.itemid = i.itemid "
		sqlStr = sqlStr & " WHERE 1 = 1  "
		sqlStr = sqlStr & addSql
		rsget.CursorLocation = adUseClient
		rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
		...
		sqlStr = sqlStr & " Q.idx, Q.itemid, Q.regdate, Q.findate, Q.resultCode, Q.lastErrMsg, Q.lastUserid, Q.copyitemid "
		sqlStr = sqlStr & " FROM db_item.[dbo].[tbl_api_item_copy] as Q "
		sqlStr = sqlStr & " JOIN db_item.dbo.tbl_item as i on Q.itemid = i.itemid "
		sqlStr = sqlStr & " WHERE 1 = 1  "
		sqlStr = sqlStr & addSql
		sqlStr = sqlStr & " ORDER BY Q.findate DESC, Q.idx DESC "
		''rw sqlStr
		rsget.pagesize = FPageSize
		rsget.CursorLocation = adUseClient
		rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new CItemDetail
                    ...
                    FItemList(i).FCopyitemid	= rsget("copyitemid")
				i = i + 1
				rsget.moveNext
			Loop
		End If
    ...
    public function GetItemListWithOption()
        ...
        if FRectSellReserve <> "" then
        sqlStr = sqlStr & " ,R.sellreservedate "
      '  sqlStr = sqlStr & " ,(select isnull(sum(realstock),0) from [db_summary].[dbo].tbl_current_logisstock_summary with (nolock) where itemid = i.itemid ) as realstock "
      	end if
        ...
        If FRectPurchasetype <> "" Then
            sqlStr = sqlStr & " LEFT JOIN db_partner.dbo.tbl_partner as p on i.makerid = p.id with (nolock)"
        End If

        sqlStr = sqlStr & "     left join [db_item].[dbo].tbl_item_addimage A with (nolock) on i.itemid=A.itemid and A.ImgType=1 and A.Gubun=1"
        sqlStr = sqlStr & "     left join [db_user].[dbo].tbl_user_c c with (nolock) on i.makerid=c.userid"
        sqlStr = sqlStr & "     left join [db_item].[dbo].tbl_item_option o with (nolock) on o.itemid = i.itemid"
        'sqlStr = sqlStr & "    left join [db_item].[dbo].tbl_item_Contents s with (nolock)"
        'sqlStr = sqlStr & "    on i.itemid=s.itemid"
	...
	public function GetItemListNvCpn()
        ...
        if (FRectExceptNvEp<>"") then
            ...
            if (FRectExceptNOTEpMapitem="") then
...
'rw sqlStr
        ...
        sqlStr = sqlStr & " , IsNULL(defaultFreeBeasongLimit,0) as defaultFreeBeasongLimit, IsNULL(defaultDeliverPay,0) as defaultDeliverPay, IsNULL(defaultDeliveryType,'') as defaultDeliveryType"
        sqlStr = sqlStr & " , IsNULL(A.itemid,0) as infoimageExists"
        sqlStr = sqlStr & " , Case itemCouponyn When 'Y' then (Select top 1 couponbuyprice From [db_item].[dbo].tbl_item_coupon_detail with (nolock) Where itemcouponidx=i.curritemcouponidx and itemid=i.itemid) end as couponbuyprice "
        IF (FRectInfodivYn<>"") or (FRectShowInfodiv<>"") then
            sqlStr = sqlStr & " , Ct.infodiv, fd.infoDivName, Ct.sellcount, Ct.recentsellcount"
        end if
        if FRectSellReserve <> "" then
	...
	public function GetItemKeywordList()
		...
		if (FRectSearchKey <> "") and (CStr(FCurrPage) = "1") then
			...
			sqlStr = sqlStr + " from [DBAPPWISH].db_AppWish.dbo.tbl_item_SearchBase with (nolock) "
		...
		addSql = ""
		addSql = addSql & " from "
		addSql = addSql & " 	db_item.dbo.tbl_item i with (nolock) "
		addSql = addSql & " 	join db_item.dbo.tbl_item_Contents c with (nolock) "
		addSql = addSql & " 	on "
		addSql = addSql & " 		i.itemid = c.itemid "

		if (FRectSearchKey <> "") then
			addSql = addSql & " 	join [db_temp].[dbo].[tbl_item_searchKeyword] T with (nolock) "
			addSql = addSql & " 	on "
	...
	public function GetItem_Evaluate_exclude()
		...
		sqlStr = sqlStr & " left join db_board.dbo.tbl_Item_Evaluate_exclude ee with (nolock)"
	...
	public function GetItemListByOnlineBrand()
        ...
        sqlStr = sqlStr & "     left join [db_item].dbo.tbl_item_option_stock b with (nolock) on b.itemgubun = '10' and i.itemid = b.itemid and IsNull(o.itemoption, '0000') = b.itemoption "
	...
	public function GetItemListByOfflineBrand()
        ...
        sqlStr = sqlStr & "     left join [db_item].dbo.tbl_item_option_stock b with (nolock) on b.itemgubun = i.itemgubun and i.shopitemid = b.itemid and i.itemoption = b.itemoption "
	...
	public function GetItemAboardList()
		...
		if FRecItemManageType <> "" then
			if FRecItemManageType = "O" then
				addSql = addSql + " and IsNull(ai.itemManageType, 'I') = 'O' "

			else
				addSql = addSql + " and IsNull(ai.itemManageType, 'I') = 'I' "
			end if
		end if

		if FRectSizeYn <> "" then
        	if FRectSizeYn="Y" then
	        	addSql = addSql + " and IsNull(vo.volX,0) > 0 "
	        else
				addSql = addSql + " and IsNull(vo.volX,0) <= 0 "
			end if
		end if
        ...
        if frectitemdivNotexists<>"" then
            addSql = addSql & " and i.itemdiv not in ("& frectitemdivNotexists &")"
        end if
		...
		If (FRectStockType <> "") And (FRectlimitrealstock <> "") Then
			...
			joinSqlStr = joinSqlStr + "			from [db_summary].[dbo].tbl_current_logisstock_summary s with (nolock) "
        ...
        sqlStr = sqlStr & " left join [db_item].[dbo].tbl_item_addimage A with (nolock) on i.itemid=A.itemid and A.ImgType=1 and A.Gubun=1"
        sqlStr = sqlStr & " left join [db_user].[dbo].tbl_user_c c with (nolock) on i.makerid=c.userid"
        'sqlStr = sqlStr & " left join [db_item].[dbo].tbl_item_Contents s with (nolock)" & vbcrlf
        'sqlStr = sqlStr & "    on i.itemid=s.itemid" & vbcrlf
		sqlStr = sqlStr + " left join db_item.dbo.tbl_item_pack_Volumn vo with (nolock)" & vbcrlf
		sqlStr = sqlStr + "     on i.itemid=vo.itemid" & vbcrlf
		if FRecItemManageType = "O" then
			sqlstr = sqlstr + " join [db_item].[dbo].[tbl_item_logics_addinfo] ai with (nolock) "
			sqlstr = sqlstr + " on i.itemid=ai.itemid and ai.itemManageType = 'O' "
		else
			sqlstr = sqlstr + " left join [db_item].[dbo].[tbl_item_logics_addinfo] ai with (nolock) "
			sqlstr = sqlstr + " on i.itemid=ai.itemid "
		end if
        sqlStr = sqlStr & " where i.itemid<>0" & addSql
        ...
        '// 본문 내용 접수
        sqlStr = "select top " + Cstr(FPageSize * FCurrPage)
        sqlStr = sqlStr & " i.itemid, i.makerid, i.cate_large, i.cate_mid, i.cate_small, i.itemdiv, i.itemgubun, i.itemname, i.sellcash, i.buycash, i.orgprice"	' 0~10
        sqlStr = sqlStr & " , i.orgsuplycash, i.sailprice, i.sailsuplycash, i.mileage, i.regdate, i.lastupdate, i.sellEndDate, i.sellyn, i.limityn, i.danjongyn"		'11~20
        sqlStr = sqlStr & " , i.sailyn, i.isusing, i.isextusing, i.mwdiv, i.specialuseritem, i.vatinclude, i.deliverytype, i.availPayType, i.deliverarea, i.deliverfixday"	' 21~30
        sqlStr = sqlStr & " , i.ismobileitem, i.pojangok, i.limitno, i.limitsold, i.evalcnt, i.evalCnt_photo, i.optioncnt, i.itemrackcode, i.upchemanagecode, i.reipgodate"	' 31~40
        sqlStr = sqlStr & " , i.brandname, i.titleimage, i.mainimage, i.smallimage, i.listimage, i.listimage120, i.basicimage, i.icon1image, i.icon2image, i.itemcouponyn"	' 41~50
        sqlStr = sqlStr & " , i.curritemcouponidx, i.itemcoupontype, i.itemcouponvalue, i.itemscore, i.itemWeight, i.deliverOverseas, i.tenOnlyYn, i.basicimage600, i.mainImage2, i.frontMakerid"	' 51~60
        sqlStr = sqlStr & " , i.reserveItemTp, i.mainimage3, i.basicimage1000, i.maskimage, i.maskimage1000, i.orderMinNum, i.orderMaxNum, i.dispcate1, i.limitdispyn, i.sellSTDate"		' 61~70
        sqlStr = sqlStr & " , i.tentenimage, i.tentenimage50, i.tentenimage200, i.tentenimage400, i.tentenimage600, i.tentenimage1000, i.evaloffcnt, i.adultType"	' 71~
        sqlStr = sqlStr & " , IsNULL(defaultFreeBeasongLimit,0) as defaultFreeBeasongLimit, IsNULL(defaultDeliverPay,0) as defaultDeliverPay, IsNULL(defaultDeliveryType,'') as defaultDeliveryType"
        sqlStr = sqlStr & " , IsNULL(A.itemid,0) as infoimageExists"
        sqlStr = sqlStr & " , Case itemCouponyn When 'Y' then (Select top 1 couponbuyprice From [db_item].[dbo].tbl_item_coupon_detail with (nolock) Where itemcouponidx=i.curritemcouponidx and itemid=i.itemid) end as couponbuyprice "
        sqlstr = sqlstr & " , IsNull(vo.volX,0) as volX, IsNull(vo.volY,0) as volY, IsNull(vo.volZ,0) as volZ, IsNull(ai.itemManageType, 'I') as itemManageType" & vbcrlf
        sqlStr = sqlStr & " from [db_item].[dbo].tbl_item i with (nolock) "
		sqlStr = sqlStr & joinSqlStr
        sqlStr = sqlStr & " left join [db_item].[dbo].tbl_item_addimage A with (nolock) on i.itemid=A.itemid and A.ImgType=1 and A.Gubun=1"
        sqlStr = sqlStr & " left join [db_user].[dbo].tbl_user_c c with (nolock) on i.makerid=c.userid"
	...
	'// 해외배송 상품 목록
    ' /admin/itemmaster/itemAboard_list.asp
	public function GetItemAboardList_notpaging()
        dim sqlStr, addSql, i
		Dim joinSqlStr, stockFieldName
        ...
        sqlStr = sqlStr & " , i.curritemcouponidx, i.itemcoupontype, i.itemcouponvalue, i.itemscore, i.itemWeight, i.deliverOverseas, i.tenOnlyYn, i.basicimage600, i.mainImage2, i.frontMakerid"	' 51~60
        sqlStr = sqlStr & " , i.reserveItemTp, i.mainimage3, i.basicimage1000, i.maskimage, i.maskimage1000, i.orderMinNum, i.orderMaxNum, i.dispcate1, i.limitdispyn, i.sellSTDate"		' 61~70
        sqlStr = sqlStr & " , i.tentenimage, i.tentenimage50, i.tentenimage200, i.tentenimage400, i.tentenimage600, i.tentenimage1000, i.evaloffcnt, i.adultType"	' 71~78
        sqlStr = sqlStr & " , IsNULL(defaultFreeBeasongLimit,0) as defaultFreeBeasongLimit, IsNULL(defaultDeliverPay,0) as defaultDeliverPay, IsNULL(defaultDeliveryType,'') as defaultDeliveryType"
        sqlStr = sqlStr & " , IsNULL(A.itemid,0) as infoimageExists"
        sqlStr = sqlStr & " , Case itemCouponyn When 'Y' then (Select top 1 couponbuyprice From [db_item].[dbo].tbl_item_coupon_detail with (nolock) Where itemcouponidx=i.curritemcouponidx and itemid=i.itemid) end as couponbuyprice "
        sqlstr = sqlstr & " , IsNull(vo.volX,0) as volX, IsNull(vo.volY,0) as volY, IsNull(vo.volZ,0) as volZ, IsNull(ai.itemManageType, 'I') as itemManageType" & vbcrlf
        ...
        if (FResultCount<1) then FResultCount=0
        if (FTotalCount<1) then FTotalCount=0
	...
	Function GetItemNotAddexplainList()
		...
		sqlStr = sqlStr & "			from "
		sqlStr = sqlStr & "			db_item.dbo.tbl_item i with (nolock) "
		sqlStr = sqlStr & "			Inner Join db_item.dbo.tbl_item_Contents C with (nolock) "
		sqlStr = sqlStr & "			on i.itemid =c.itemid "
		if (FRectMduserid <> "") Then
		sqlStr = sqlStr & "			left outer join  db_user.dbo.tbl_user_c as uc with (nolock) on i.makerid = uc.userid "
		End if
		sqlStr = sqlStr & "			where 1= 1 " & addSql
		...
		sqlStr = sqlStr & "	, (select company_name from [db_partner].[dbo].tbl_partner with (nolock) where id = uc.mduserid ) as mdname "
		sqlStr = sqlStr & "			from "
	...
	Function GetItemNotAddexplain_FieldBrand()
		...
		sqlStr = sqlStr & "			inner Join db_item.dbo.tbl_item_Contents C with (nolock) "
		sqlStr = sqlStr & "			on i.itemid =c.itemid	 "
		sqlStr = sqlStr & "			inner join db_item.dbo.tbl_item_infoDiv ic with (nolock) "
		sqlStr = sqlStr & "			on c.infodiv =ic.infodiv "
		sqlStr = sqlStr & "			left outer Join db_item.dbo.tbl_item_infoCont Fc with (nolock) "
		sqlStr = sqlStr & "			on i.itemid =Fc.itemid	"
		sqlStr = sqlStr & "			and fc.infocd <>''	"
		sqlStr = sqlStr & "			left outer join db_user.dbo.tbl_user_c as uc on i.makerid = uc.userid with (nolock) "
		sqlStr = sqlStr & "			where 1=1  "  & addSql
...
'		rsget.Close
		...
		sqlStr = sqlStr & "			having sum( CASE WHEN Fc.infoContent <> '' or Fc.infocd='02004' then 1 ELSE 0 END)<>isNULL(ic.infovalidCNT,0)	"
		sqlStr = sqlStr & "		) T	"
		sqlStr = sqlStr & "     left join [db_partner].[dbo].tbl_partner p with (nolock) on T.mduserid=p.id"
	...
	public function addExplainGetItemList()
        ...
        sqlStr = sqlStr & " and i.itemid in ("
            sqlStr = sqlStr & " select i.itemid from [db_item].[dbo].tbl_item i with (nolock)"
            sqlStr = sqlStr & " Join [db_item].[dbo].tbl_item_contents Ct with (nolock)"
            sqlStr = sqlStr & " on i.itemid=Ct.itemid"
            sqlStr = sqlStr & " inner join 	db_item.dbo.tbl_item_infoDiv ic with (nolock) on Ct.infodiv =ic.infodiv "
			sqlStr = sqlStr & " left outer Join db_item.dbo.tbl_item_infoCont Fc with (nolock) on i.itemid =Fc.itemid and fc.infocd <>''"
			sqlStr = sqlStr & " where 1=1 " & addSql
...
public function getCategoryInfo(iid)
	...
	SQL =	"select c1.code_nm, c2.code_nm, c3.code_nm, ic.code_large, ic.code_mid, ic.code_small, ic.code_div " &_
			"from db_item.dbo.tbl_Item_category as ic with (nolock) " &_
			"	join db_item.dbo.tbl_Cate_large as c1 with (nolock) " &_
			"		on ic.code_large=c1.code_large " &_
			"	join db_item.dbo.tbl_Cate_mid as c2 with (nolock) " &_
			"		on ic.code_mid=c2.code_mid " &_
			"			and c1.code_large=c2.code_large " &_
			"	join db_item.dbo.tbl_Cate_small as c3 with (nolock) " &_
...
public function getDispCategory(iid)
	...
	SQL = "select d.catecode, i.isDefault, i.depth " &_
		...
		"from db_item.dbo.tbl_display_cate as d with (nolock) " &_
		"	join db_item.dbo.tbl_display_cate_item as i with (nolock) " &_
...
public function GetItemRelationStr(itemid)
    ...
    sqlStr = "Select subItemid From db_item.dbo.tbl_item_relation with (nolock) Where mainItemid='" & itemid & "'"
    rsget.Open sqlStr,dbget,1
