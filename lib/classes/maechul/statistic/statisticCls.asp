...
'rw 	sql
	...
	public function fStatistic_sitename_realtime			'매출집계-사이트별
	dim i , sql

	if FRectDateGijun="beasongdate" or FRectDateGijun="jungsanfixdate" then
	...
	sql = sql & " AND m.ipkumdiv>3 AND m.cancelyn='N' AND d.cancelyn<>'Y' "

	if FRectIncSubCost<>"" then
		sql = sql & " AND d.itemid<>0 AND d.itemid<>100 "
	end if
	...
	sql = sql & "	GROUP BY m.sitename "
	sql = sql & "	ORDER BY m.sitename "
	...
	Select Case FRectIsPlusSaleItem
		Case "P"
			sql = sql & " and d.plus_sale_item_idx is not null "
		Case "N"
			sql = sql & " and d.plus_sale_item_idx is null "
	end Select
	...
	sql = "SELECT "
	sql = sql & "		d.makerid, p.purchasetype, pc.pcomm_name as purchasetypename"
	...
	End If 
	sql = sql & "	FROM [db_order].[dbo].[tbl_order_master] as m with (nolock)"
	if (FRectBizSectionCd<>"") then
	    sql = sql & " Join db_partner.dbo.tbl_partner p3 with (nolock)"
	    sql = sql & " on m.sitename=p3.id"
	    sql = sql & " and isNULL(p3.sellbizcd,'0000000101')='"&FRectBizSectionCd&"'"
	end if
	sql = sql & "		INNER JOIN [db_order].[dbo].[tbl_order_detail] as d with (nolock) ON m.orderserial = d.orderserial "
	If FRectCateL <> "" Then
	    sql = sql & "		INNER JOIN [db_item].[dbo].[tbl_item] as i with (nolock) ON d.itemid = i.itemid "
	end if
		 
		sql = sql & " INNER JOIN [db_partner].[dbo].[tbl_partner] as p with (nolock) on d.makerid = p.id "
		sql = sql & " LEFT JOIN [db_partner].[dbo].tbl_partner_comm_code as pc with (nolock)"
		...
		sql = sql & " INNER JOIN db_item.dbo.tbl_display_cate_item as dc with (nolock) on d.itemid = dc.itemid and dc.catecode like '" + FRectDispCate + "%' and dc.isDefault='y'"
		END IF
	sql = sql & "	WHERE m." & FRectDateGijun & " >= '" & FRectStartdate & "' and m." & FRectDateGijun & "<'" & DateAdd("d",1,FRectEndDate) & "' AND m.ipkumdiv>3 AND m.cancelyn='N' AND d.cancelyn<>'Y' AND d.itemid<>0 AND d.itemid<>100"
...
if (FRectSellChannelDiv<>"") then
   ...
   If FRectMakerid <> "" Then
	...
	if (TRUE) then
    	...
    	If Not rsget.Eof Then
    		Do Until rsget.Eof
    			set FList(i) = new cStaticTotalClass_oneitem
					FList(i).fpurchasetypename					= rsget("purchasetypename")
  ...
  'rw   sql 
	...
	'카테고리 상품 목록
    public function FStatisticCategoryItems
        dim i , sql, vDB
        dim DispCateCode : DispCateCode = FRectCateL & FRectCateM & FRectCateS  ''기존 포멧과 맞춤
        dim grpLen : grpLen = 3+Len(DispCateCode)
        dim icateCode
    	...
    	sql = sql & "  d.itemid, ti.itemname, ti.makerid, ti.sellyn, ti.isusing, "
    	sql = sql & "		isNull(sum(d.itemno),0) AS itemno, "
    	sql = sql & "		isNull(sum(d.itemcost*d.itemno),0) AS itemcost, "
    	sql = sql & "		isNull(sum(d.buycash*d.itemno),0) as buycash, "
    	sql = sql & "		isNull(sum(d.reducedPrice*d.itemno),0) as reducedprice "
    	sql = sql & "	FROM " & vDB & " "
    	sql = sql & "       join db_item.dbo.tbl_item ti ON d.itemid=ti.itemid "
		sql = sql & "       left join db_partner.dbo.tbl_partner p2 ON m.sitename=p2.id "
    	sql = sql & "		LEFT JOIN db_item.[dbo].tbl_display_cate_item as i ON d.itemid = i.itemid AND i.isDefault='y' "
    	sql = sql & "       LEFT JOIN db_item.[dbo].tbl_display_cate as l ON Left(i.catecode,"&grpLen&")=l.catecode"
    	...
    	if (left(DispCateCode,3)<>"999") then
            sql = sql & " and Left(l.catecode,"&Len(DispCateCode)&")='"&DispCateCode&"'"
        else
			sql = sql & " and l.catecode is null "
		end if
        ...
        sql = sql & " GROUP BY  d.itemid, ti.itemname, ti.makerid, ti.sellyn, ti.isusing ORDER BY d.itemid"
