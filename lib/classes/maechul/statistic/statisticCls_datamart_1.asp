...
'rw 	sql
	...
	sql = sql & "	GROUP BY isNULL('10x10::'+ case when left(m.rdsite,6) = 'nvshop' then 'nvshop' when left(m.rdsite,13) = 'mobile_nvshop' then 'mobile_nvshop' else m.rdsite end, m.sitename) "
...
'	sql = sql & "	ORDER BY isNULL('10x10::'+m.rdsite,m.sitename) ASC "
	...
	sql = sql & "	AND m.ipkumdiv>3 AND m.cancelyn='N' AND d.cancelyn<>'Y' AND d.itemid<>0 "
...
'	end if
	...
	if FRectChkchannel = "1" then
            ...
            sql = "SELECT "
            sql = sql & " makerid, sum(ordercnt) as ordercnt  "
 ...
 public function fStatistic_DispCategory  '전시 카테고리별매출
        ...
        if FRectChkchannel = "1" then
            	...
            	sql = sql & "	WHERE " & FRectDateGijun & " BETWEEN '" & FRectStartdate & "' and '" & DateAdd("d",1,GetValidDate(FRectEndDate)) & "' AND m.ipkumdiv>3 AND m.cancelyn='N' AND d.cancelyn<>'Y' AND d.itemid<>0 "
...
'rw sql
	...
	db3_rsget.close
	 ...
	 if FRectChkchannel = "1" then
        	...
        	sql = sql & "   ,www_itemno, www_orgitemcost, www_itemcostCouponNotApplied,www_itemcost,www_buycash,www_reducedprice "
        	sql = sql & "   ,ma_itemno, ma_orgitemcost, ma_itemcostCouponNotApplied,ma_itemcost,ma_buycash,ma_reducedprice "
        	sql = sql & " FROM ( "
            ...
            sql = sql & "   select d.itemid, i.smallimage,  d.makerid "
            sql = sql & "		isNull(sum(d.itemno),0) AS itemno, "
        	sql = sql & "		isNull(sum(d.orgitemcost*d.itemno),0) AS orgitemcost, "
        	sql = sql & "		isNull(sum(d.itemcostCouponNotApplied*d.itemno),0) AS itemcostCouponNotApplied, "
        	sql = sql & "		isNull(sum(d.itemcost*d.itemno),0) AS itemcost, "
	...
	sql = sql & " AND m.ipkumdiv>3 AND m.cancelyn='N' AND d.cancelyn<>'Y' AND d.itemid<>0 "
 	...
 	if FRectChkchannel = "1" then
 	sql = sql & "	GROUP BY d.itemid,i.smallimage, d.makerid, m.beadaldiv "    
 	sql = sql & " ) as T Group by itemid ,smallimage, makerid "
    else    
