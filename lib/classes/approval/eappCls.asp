...
public Fcurrencyprice
	...
	'//보낸 결재함 리스트
	' /admin/approval/eapp/eappsend.asp
	public Function fnGetEAppSendList
		Dim strSql	 
		IF FreportState = ""  THEN FreportState = -1
		strSql ="exec [db_partner].[dbo].[sp_Ten_eAppReport_getSendListCnt] '"&FadminId&"',"&FreportState&", '"& freportname &"', '"& freportcontents &"', '"& freportprice &"', '"& fregdate &"', '"& fusername &"'"  
		...
		strSql ="exec [db_partner].[dbo].sp_Ten_eAppReport_getSendList '"&FadminId&"',"&FreportState&","&FSPageNo&","&FEPageNo&", '"& freportname &"', '"& freportcontents &"', '"& freportprice &"', '"& fregdate &"', '"& fusername &"'"	 
	...
	'//받은결재함 리스트
	' /admin/approval/eapp/eappreceive.asp
	public Function fnGetEAppReceiveList
		Dim strSql	 

		IF FAuthState = "" THEN FAuthState = 0
		strSql ="exec [db_partner].[dbo].[sp_Ten_eAppReport_getReceiveListCnt] '"&FadminId&"',"&FAuthState&","&FreportState&", '"& freportname &"', '"& freportcontents &"', '"& freportprice &"', '"& fregdate &"', '"& fusername &"'"
		...
		strSql ="exec [db_partner].[dbo].sp_Ten_eAppReport_getReceiveList '"&FadminId&"',"&FAuthState&","&FreportState&","&FSPageNo&","&FEPageNo&", '"& freportname &"', '"& freportcontents &"', '"& freportprice &"', '"& fregdate &"', '"& fusername &"'"
	...
	public Function fnGetEAppReferList
		Dim strSql	 
		strSql ="[db_partner].[dbo].[sp_Ten_eAppReport_getReferListCnt]('"&FadminId&"', '"& freportname &"', '"& freportcontents &"', '"& freportprice &"', '"& fregdate &"', '"& fusername &"')"	 
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly, adCmdStoredProc
		IF Not (rsget.EOF OR rsget.BOF) THEN
			FTotCnt = rsget(0)
		END IF
		...
		IF FTotCnt > 0 THEN
			...
			strSql ="[db_partner].[dbo].sp_Ten_eAppReport_getReferList('"&FadminId&"',"&FSPageNo&","&FEPageNo&", '"& freportname &"', '"& freportcontents &"', '"& freportprice &"', '"& fregdate &"', '"& fusername &"')"	 
			rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly, adCmdStoredProc
			IF Not (rsget.EOF OR rsget.BOF) THEN
...
Function getEdmsFileName(ByVal edmsCode, ByVal edmsName, ByVal edmsFile, ByRef physicalFileName)
	if edmsFile <> "" Then
 		getEdmsFileName = edmsCode&"_"&edmsName&"."&split(edmsFile,".")(ubound(split(edmsFile,".")))
		physicalFileName = split(edmsFile,"/")(ubound(split(edmsFile,"/")))
	end if
end function
