...
'' [db_contents].[dbo].tbl_main_contents_poscode
...
function DrawMainPosCodeCombo(selectBoxName,selectedId,changeFlag)
   ...
   <%
   query1 = " select top 100 * from [db_contents].[dbo].tbl_othermall_main_contents_poscode where 1=1 and left(posVarname,5) = 'other' and isusing='Y' order by poscode"
...
function DrawFixTypeCombo(selectBoxName,selectedId,changeFlag)
    dim bufStr, tmp_str
    bufStr = "<select name='" + selectBoxName + "' " + changeFlag + " >" + VbCrlf
...
function DrawLinktypeCombo(selectBoxName,selectedId,changeFlag)
    dim bufStr, tmp_str
    bufStr = "<select name='linktype' " + changeFlag + " >" + VbCrlf
    bufStr = bufStr + " <option value='' > 선택" + VbCrlf
...
Class CMainContentsCode
    ...
    public Sub GetOneContentsCode()
        dim SqlStr
        SqlStr = "select top 1 * "
        SqlStr = SqlStr + " from [db_contents].[dbo].tbl_othermall_main_contents_poscode"
    ...
    public Sub GetposcodeList()
        dim sqlStr
        sqlStr = "select count(poscode) as cnt from [db_contents].[dbo].tbl_othermall_main_contents_poscode"
        rsget.Open sqlStr, dbget, 1
		FTotalCount = rsget("cnt")
		rsget.close
        
        sqlStr = "select top " + CStr(FPageSize * FCurrPage) + " * from [db_contents].[dbo].tbl_othermall_main_contents_poscode "
...
Class CMainContentsItem
    ...
    public function GetImageUrl()
        if (IsNULL(Fimageurl) or (Fimageurl="")) then
            ...
            GetImageUrl = "http://imgstatic.10x10.co.kr/main/" + Fimageurl
        end if
    end function
...
Class CMainContents
    ...
    public Sub GetOneMainContents()
        ...
        sqlStr = sqlStr + " from [db_contents].[dbo].tbl_othermall_main_contents c"
        sqlStr = sqlStr + " left join [db_contents].[dbo].tbl_othermall_main_contents_poscode p"
    ...
    public Sub GetMainContentsList()
        ...
        sqlStr = " select count(idx) as cnt from [db_contents].[dbo].tbl_othermall_main_contents"
