...
Class CwishBest
    ...
    public Sub GetContentsList()
		...
		sqlStr = " select count(idx) as cnt from db_sitemaster.dbo.tbl_pc_main_wishbest_list with (nolock)"
        ...
        sqlStr = sqlStr + " from db_sitemaster.dbo.tbl_pc_main_wishbest_list as a with (nolock)"
		...
		if Fsdt<>"" then sqlStr = sqlStr & " and '"&Fsdt&"' >= a.StartDate and '" & Fsdt & " 23:59:59' <= a.Enddate "
		...
		if  not rsget.EOF  then
			...
			do until rsget.eof
				set FItemList(i) = new CwishbestItem
				...
				FItemList(i).Fmaincopy1		= rsget("maincopy1")
				FItemList(i).Fmaincopy2		= rsget("maincopy2")
	...
	public Sub GetOneSubItem()
        ...
        sqlStr = sqlStr & "From [db_sitemaster].[dbo].tbl_pc_main_wishbest_item as s "
        ...
        set FOneItem = new CwishbestItem
        if Not rsget.Eof then
    ...
    public Sub GetOneContents()
        dim sqlStr
        sqlStr = "select top 1 * "
        sqlStr = sqlStr + " from db_sitemaster.dbo.tbl_pc_main_wishbest_list with (nolock)"
    ...
    public Sub GetContentsItemList()
       dim sqlStr, addSql, i
		...
		sqlStr = " select count(listidx) as cnt from db_sitemaster.dbo.tbl_pc_main_wishbest_item "
		sqlStr = sqlStr + " where 1=1"
        ...
        sqlStr = "Select top " + CStr(FPageSize * FCurrPage) + " s.subidx , s.listidx , s.itemid , s.isusing as itemusing , s.sortnum, isnull(s.wishitemname, i.itemname) as itemname, i.smallImage , s.label , s.ldv, i.itemdiv "
        sqlStr = sqlStr & "From [db_sitemaster].[dbo].tbl_pc_main_wishbest_item as s "
        sqlStr = sqlStr & "	left join db_item.dbo.tbl_item as i "
        sqlStr = sqlStr & "		on s.itemid=i.itemid "
        sqlStr = sqlStr & "			and i.itemid<>0 "
