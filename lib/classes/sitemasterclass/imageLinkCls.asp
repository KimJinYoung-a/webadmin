...
Class CimageLinkItem
	...
	Public FYValue
	Public FIconType
	Public FRegUserImage
	Public FRegUserFrontName
...
Class CimageLink
    ...
    public Sub GetContentsList()
		...
		sqlStr = " select count(idx) as cnt from db_sitemaster.dbo.tbl_ImageLink_Master with (nolock)"
        ...
        sqlStr = sqlStr + " from db_sitemaster.dbo.tbl_ImageLink_Master as a with (nolock)"
        sqlStr = sqlStr + " left join db_partner.dbo.tbl_user_tenbyten as u with (nolock) on a.reguser = u.userid "
        sqlStr = sqlStr + " left join db_partner.dbo.tbl_user_tenbyten as u2 with (nolock) on a.modifyuser = u2.userid "
        sqlStr = sqlStr + " where 1=1"
		...
		if  not rsget.EOF  then
			...
			do until rsget.eof
				...
				FItemList(i).FRegUserImage		= rsget("reguserimage")
				FItemList(i).FRegUserFrontName	= rsget("reguserfrontname")
    ...
    public Sub GetOneDetailContents()
        dim sqlStr
        sqlStr = "select top 1 * "
        sqlStr = sqlStr + " from db_sitemaster.dbo.tbl_ImageLink_Detail "
        ...
        if Not rsget.Eof Then
			...
			FOneItem.FIconType		= rsget("IconType")
    ...
    public Sub GetOneContents()
        ...
        if Not rsget.Eof Then
			...
			FOneItem.Fadminid		= rsget("reguser")
			FOneItem.Flastadminid	= rsget("modifyuser")
			FOneItem.FRegUserImage		= rsget("RegUserImage")
			FOneItem.FRegUserFrontName	= rsget("reguserfrontname")
    ...
    public Sub GetLinkListContents()
       dim sqlStr, addSql, i
		...
		sqlStr = " select count(*) as cnt from db_sitemaster.dbo.tbl_ImageLink_Detail "
		sqlStr = sqlStr + " where 1=1"
        ...
        sqlStr = "Select top 1000 s.idx , s.MasterIdx , s.XValue , s.YValue, s.ItemID, s.IconType, s.IsUsing, s.RegUser, s.ModifyUser, s.RegDate, s.LastUpDate"
		sqlStr = sqlStr + " ,u.username, u2.username as username2 "
        sqlStr = sqlStr & "From db_sitemaster.dbo.tbl_ImageLink_Detail as s "
		...
		if  not rsget.EOF  then
			...
			do until rsget.eof
	            ...
	            FItemList(i).FIsUsing				= rsget("IsUsing")
	            FItemList(i).FIconType				= rsget("IconType")
				FItemList(i).Fadminid				= rsget("reguser")
				FItemList(i).Flastadminid			= rsget("modifyuser")
				FItemList(i).Fregdate				= rsget("regdate")
				FItemList(i).Flastupdate			= rsget("lastupdate")
