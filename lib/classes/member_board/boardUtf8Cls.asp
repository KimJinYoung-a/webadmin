...
Class board
	...
	Public Function fnBoardview
		Dim strSql, i
		strSql = ""
		strSql = strSql & " select Top 1 B.brd_sn, B.id, B.brd_subject, B.brd_content, B.brd_hit, B.brd_regdate, B.brd_fixed, B.brd_team, B.brd_type, P.part_sn,p.department_id, T.username, J.job_name, F.posit_name " & vbcrlf
		strSql = strSql & " from db_partner.dbo.tbl_cooperate_board as B " & vbcrlf
	...
	Public Function fnBoardmodify
		Dim strSql, i
		strSql = ""
		strSql = strSql & " select top 1 B.id, T.username, B.brd_subject, B.brd_content, B.brd_hit, B.brd_regdate, B.brd_fixed, B.brd_team, B.brd_type, P.level_sn, P.posit_sn, P.job_sn, B.brd_isusing, P.department_id " & vbcrlf
		strSql = strSql & " from db_partner.dbo.tbl_cooperate_board as B " & vbcrlf
		strSql = strSql & " Inner Join db_partner.dbo.tbl_cooperate_board_part as P on B.brd_sn = P.brd_sn  " & vbcrlf
		strSql = strSql & " Inner Join db_partner.dbo.tbl_user_tenbyten as T on B.id = T.userid " & vbcrlf
		strSql = strSql & " where B.brd_sn = '"& Fbrd_sn &"'"& vbcrlf
...
<%
	...
	Function fnBrdType(page, isAll, value, onchange)
		...
		If page = "w" Then	'### write
			vBody = "<select name=""brd_type"" class=""a"" " & onchange & ">"
			If isAll = "Y" Then
				vBody = vBody & "	<option value="""">-선택-</option>"
			End If
			vBody = vBody & "	<option value=""0"" " & CHKIIF(value="0","selected","") & ">일반 안내 공지</option>"
			vBody = vBody & "	<option value=""5"" " & CHKIIF(value="5","selected","") & ">업무상 관련공지</option>"
			vBody = vBody & "	<option value=""1"" " & CHKIIF(value="1","selected","") & ">인사 관련공지</option>"
			vBody = vBody & "	<option value=""2"" " & CHKIIF(value="2","selected","") & ">경영제도 관련공지</option>"
			vBody = vBody & "	<option value=""3"" " & CHKIIF(value="3","selected","") & ">회사내규 관련공지</option>"
			vBody = vBody & "	<option value=""6"" " & CHKIIF(value="6","selected","") & ">보안 관련 공지</option>"
			vBody = vBody & "	<option value=""4"" " & CHKIIF(value="4","selected","") & ">경조사 관련공지</option>"
			vBody = vBody & "</select>"
		ElseIf page = "v" Then	'### view
			Select Case value
				Case "1" : vBody = "인사 관련공지"
				Case "2" : vBody = "경영제도 관련공지"
				Case "3" : vBody = "회사내규 관련공지"
				Case "4" : vBody = "경조사 관련공지"
				Case "5" : vBody = "업무상 관련공지"
				Case "0" : vBody = "일반 안내 공지"
				Case "6" : vBody = "보안 관련 공지"
				Case Else : vBody = ""
			End Select
		End IF
		fnBrdType = vBody
