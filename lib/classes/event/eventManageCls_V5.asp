...
'## fnGetEventMDThemeInfo :이벤트 엠디 등록 이벤트 테마 정보 ##
	...
	'// 메일진 표시용 기획전목록 정보	' 이상구 생성
	public Function fnGetMailzineEventListData
		Dim strSql, i, strEvtCodeList, arrEvtCodeList

		strEvtCodeList = Replace(FECodeArr, vbCrLf, ",")
		arrEvtCodeList = Split(FECodeArr, vbCrLf)

		strSql = " select"
		strSql = strSql & " e.evt_code, d.etc_itemimg, d.evt_mo_listbanner"
		strSql = strSql & " , replace(isnull(e.evt_name,''), '|~' + convert(varchar,isnull(d.salePer,'')) + '%', '') as evt_name"
		strSql = strSql & " , e.evt_subcopyK, (case when d.issale = 1 then d.salePer else '' end), (case when d.iscoupon = 1 then d.saleCPer else '' end)  "
		strSql = strSql & " from [db_event].[dbo].[tbl_event] e with (nolock)"
		strSql = strSql & " left join [db_event].[dbo].[tbl_event_display] d with (nolock)"
		strSql = strSql + " 	on e.evt_code = d.evt_code"
		strSql = strSql + " where "
		strSql = strSql + " 	1 = 1 "
	...
	public Function fnGetMailzineMDPickData
		...
		strSql = strSql + " 	f.linkitemid, 'http://imgstatic.10x10.co.kr/contents/maincontents/' + f.photoimg as photoimg, i.icon1image " & vbcrlf
 		...
 		strSql = strSql + " from [db_sitemaster].[dbo].tbl_main_mdchoice_flash f with (nolock)" & vbcrlf
 		strSql = strSql + " left join [db_item].[dbo].tbl_item i with (nolock) on f.linkitemid=i.itemid " & vbcrlf
 		...
 		strSql = strSql + " 		[db_item].[dbo].[tbl_item_coupon_master] m with (nolock)" & vbcrlf
 		strSql = strSql + " 		join [db_item].[dbo].[tbl_item_coupon_detail] d with (nolock) on m.itemcouponidx = d.itemcouponidx " & vbcrlf
...
''		response.Write strSql
	...
	'## fnGetItemInfoLinkEvent : 관련 이벤트 정보 가져오기 ##
	public Function fnGetItemInfoLinkEvent
		Dim strSql
		strSql = " SELECT top 1 eventType, itemArray" + vbcrlf
		strSql = strSql + " FROM [db_event].[dbo].[tbl_event_item_infoView]" + vbcrlf
		strSql = strSql + " WHERE eventCode=" & FECode
		strSql = strSql + " order by idx desc"
		rsget.Open strSql,dbget
		IF not rsget.EOF THEN
			fnGetItemInfoLinkEvent = rsget.getRows()
		End IF
		rsget.Close
	End Function
