...
Class ClsEvent
	...
	IF FECode = "" THEN Exit Function
		strSql = " SELECT  evt_category, evt_cateMid, issale, isgift, iscoupon,iscomment,isbbs,isitemps, isapply, evt_bannerimg, evt_template,"&_
				...
				"	, mdtheme, mdthememo, themecolor, themecolormo, textbgcolor, textbgcolormo, mdbntype, mdbntypemo, salePer, saleCPer, endlessView, eventtype_pc, eventtype_mo, bannerTypeDiv, bannerCouponTxt, bannerGubun, videoLink, videoType"&_
...
'## fnGetEventMDThemeInfo :이벤트 엠디 등록 이벤트 테마 정보 ##
	...
	public Function fnGetMailzineEventListData
		...
		strSql = " select e.evt_code, d.etc_itemimg, d.evt_mo_listbanner, replace(e.evt_name, '|~' + convert(varchar,isNull(d.salePer,'')) + '%', '') as evt_name, e.evt_subcopyK, (case when d.issale = 1 then isNull(d.salePer,'') else '' end), (case when d.iscoupon = 1 then isNull(d.saleCPer,'') else '' end)  "
		strSql = strSql + " from "
	...
	public Function fnGetMailzineMDPickData
		...
		strSql = " select top 12 "
		strSql = strSql + " 	f.linkitemid, 'http://imgstatic.10x10.co.kr/contents/maincontents/' + f.photoimg as photoimg, i.icon1image "
 		strSql = strSql + " 	, 'http://www.10x10.co.kr' + f.linkinfo as linkinfo, f.textinfo "
 		strSql = strSql + " 	, i.orgprice "
 		strSql = strSql + " 	, (case "
 		strSql = strSql + " 			when ss.saleprice is NULL and se.itemid is NULL and i.sailyn = 'Y' then i.sellcash "
 		strSql = strSql + " 			when ss.saleprice is not NULL and i.orgprice > ss.saleprice then ss.saleprice "
...
'신상품순 -1, 저가격순-2,지정번호순-3, 베스트셀러순-4,고가격순-5, 할인율순 - 6, 후기순후기순(별점3개이상) -7,위시순-8, (그룹순-9, 브랜드순-10)
	...
	ELSEIF FESSort = "3" THEN
	    if FEChannel ="P" THEN
		strSort = "  ,evtitem_imgsize desc, evtitem_sort ,A.itemid desc"
        else
        strSort = "  ,evtitem_imgsize desc, evtitem_sort_mo ,A.itemid desc"
        end if
...
'				"		WHERE evt_code = " &FECode &addSql& strGroup & striSort & " ) " & strGroup&addSql& strSort
	...
	public function fnGetWorkerNameToID(WorkerName)
		dim strSql
		'ID 검색
		strSql = "select userid from db_partner.dbo.tbl_user_tenbyten where username='"&WorkerName&"'"
...
'	END IF
	...
	IF FTotCnt >0 THEN
		...
		strSql = " SELECT evt_code,evt_kind,evt_manager,evt_scope,evt_name,evt_startdate,evt_enddate,evt_level,evt_state,evt_regdate,evt_bannerimg "&_
				...
				"       ,designerid2, designername2, dsn_state1, dsn_state2, eventtype_pc, eventtype_mo, iscomment, isbbs, isitemps, isGetBlogURL "&_
				" FROM"&_
				" ( "&_
