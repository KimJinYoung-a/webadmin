...
Class ClsEvent
	...
	IF FECode = "" THEN Exit Function
		strSql = " SELECT  evt_category, evt_cateMid, issale, isgift, iscoupon,iscomment,isbbs,isitemps, isapply, evt_bannerimg, evt_template,"&_
				...
				"	, mdtheme, mdthememo, themecolor, themecolormo, textbgcolor, textbgcolormo, mdbntype, mdbntypemo, salePer, saleCPer, endlessView, eventtype_pc, eventtype_mo, bannerTypeDiv, bannerCouponTxt, bannerGubun, videoLink"&_
...
'## fnGetEventMDThemeInfo :이벤트 엠디 등록 이벤트 테마 정보 ##
	...
	IF FECode = "" THEN Exit Function
		strSql = " SELECT  comm_isusing, comm_text, freebie_img, comm_start, comm_end, gift_isusing, gift_text1, gift_img1"&_
				...
				" 	, using_contents2, using_text3, using_contents3, nocate, title_pc, title_mo, eval_isusing ,eval_text ,eval_freebie_img ,eval_start ,eval_end" &_
...
'	END IF
	...
	IF FSednm <> "" THEN
		...
		''느려서 방식 수정 2018/05/30 eastone
		strSearch  = strSearch &  "and Exists(select 1 from db_partner.dbo.tbl_user_tenbyten p where username='"&FSednm&"' and (p.userid=B.designerid or p.userid=B.designerid2))"
