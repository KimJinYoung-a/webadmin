...
Class CMakeGlobItem
	...
	public function GetMakeGlobItemWaitingList()
        ...
        if (FRectBrandName <> "") Then
			addSql = addSql & " And c.userid = '"&FRectBrandName&"' "
		End If
		...
		strsql = " Select count(i.itemid) "
		strsql = strsql & " From db_item.dbo.tbl_item i "
		strsql = strsql & " inner join db_item.dbo.tbl_item_contents ic on i.itemid = ic.itemid "
		strsql = strsql & " inner join db_item.dbo.tbl_display_cate_item ci on i.itemid = ci.itemid And ci.isDefault='y' "
		strsql = strsql & " inner join db_user.dbo.tbl_user_c c on i.makerid = c.userid "
...
'		strsql = strsql & " Where  i.deliverOverseas='Y' And i.itemweight<>0 And i.mwdiv in ('m','w') "&addSql '// 텐배 상품만 표시
		strsql = strsql & " Where  i.deliverOverseas='Y' And i.itemweight<>0 "
