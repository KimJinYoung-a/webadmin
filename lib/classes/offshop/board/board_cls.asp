	...
	public sub fnGetboardList()
		...
		if frectdispshop <> "99" then
			...
			if frectshopid <> "" then
				...
				strSubSql = strSubSql & " 		from db_shop.dbo.tbl_offshop_board_shop with (nolock)"
		...
		sqlStr = sqlStr & " FROM db_shop.dbo.tbl_offshop_board_document AS A with (nolock)" + vbcrlf
		sqlStr = sqlStr & " join db_partner.dbo.tbl_partner c with (nolock)"
		...
		sqlStr = sqlStr & " 	from db_shop.dbo.tbl_offshop_commoncode with (nolock)"
		...
		sqlStr = sqlStr & " 	from db_shop.dbo.tbl_offshop_board_ans with (nolock)"
		sqlStr = sqlStr & " 	where doc_idx = a.doc_idx and ans_useyn='Y') as ans_count"
		sqlStr = sqlStr & " ,(select count(*)"
		sqlStr = sqlStr & " 	from db_shop.dbo.tbl_offshop_board_read with (nolock)"
		sqlStr = sqlStr & " 	where doc_idx = a.doc_idx) as read_count"
		sqlStr = sqlStr & " ,(select count(*)"
		sqlStr = sqlStr & " 	from db_shop.dbo.tbl_offshop_board_shop with (nolock)"
		...
		sqlStr = sqlStr & " FROM db_shop.dbo.tbl_offshop_board_document AS A with (nolock)"
		sqlStr = sqlStr & " join db_partner.dbo.tbl_partner c with (nolock)"
		sqlStr = sqlStr & "		ON A.id = C.id"
		'sqlStr = sqlStr & "		and c.isusing='Y'"
		sqlStr = sqlStr & " left join db_partner.dbo.tbl_user_tenbyten ut with (nolock)"
