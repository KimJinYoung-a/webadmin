...
Class cshortagestock_item
	...
	public function GetFranchiseBuycash()
		...
		if Fshopsuplycash<>0 then
			...
			''온라인만 세일 하는 경우등 // 특정->매입출고 인 경우 // 이 조건 제외 (2012-02-16)
    ...
    public function GetBasicImage()
        if Fitemgubun="10" then
			GetBasicImage = FimageBasic
		else
			GetBasicImage = foffimgmain
		end if
    end function

	public function GetSellYNstr()
		if Fsellyn = "Y" then
			GetSellYNstr = "판매함"
		else
			GetSellYNstr = "판매안함"
	...
	public sub fshortagestock_list()
		...
		if FRectIncludePreOrder = "on" then
	        if Frectshortagetype="3" then
				sqlsearch = sqlsearch + " and (s.sell3days*1) - (db_summary.[dbo].[uf_replacezero](s.realstockNo+s.errsampleitemno+s.errbaditemno)+(case when s.preordernofix < 0 then 0 else s.preordernofix end)) > 0"
	    	elseif Frectshortagetype="7" then
				sqlsearch = sqlsearch + " and (s.sell7days*1) - (db_summary.[dbo].[uf_replacezero](s.realstockNo+s.errsampleitemno+s.errbaditemno)+(case when s.preordernofix < 0 then 0 else s.preordernofix end)) > 0"
		elseif Frectshortagetype="14" then
			sqlsearch = sqlsearch + " and (s.sell7days*2) - (db_summary.[dbo].[uf_replacezero](s.realstockNo+s.errsampleitemno+s.errbaditemno)+(case when s.preordernofix < 0 then 0 else s.preordernofix end)) > 0"
			elseif Frectshortagetype="28" then
				sqlsearch = sqlsearch + " and (s.sell7days*4) - (db_summary.[dbo].[uf_replacezero](s.realstockNo+s.errsampleitemno+s.errbaditemno)+(case when s.preordernofix < 0 then 0 else s.preordernofix end)) > 0"
			else
		...
		if FRectOnlineMWdiv <> "" then
		   sqlsearch = sqlsearch + " and ii.mwdiv = '" & FRectOnlineMWdiv & "' "
		end if

        if (FRectSellYN="YS") then
            sqlsearch = sqlsearch & " and ii.sellyn<>'N'"
        elseif (FRectSellYN <> "") then
            sqlsearch = sqlsearch & " and ii.sellyn='" + FRectSellYN + "'"
        end if
		...
		sqlStr = sqlStr & " left join db_item.dbo.tbl_item_option io"
		sqlStr = sqlStr & " 	on i.shopitemid = io.itemid"
		sqlStr = sqlStr & " 	and i.itemoption=io.itemoption"
		sqlStr = sqlStr & " 	and i.itemgubun = '10'"
		...
		sqlStr = sqlStr & " ,ii.smallimage, ii.sellyn, ii.limityn, ii.limitno, ii.limitsold"
		...
		sqlStr = sqlStr & " , IsNULL(io.optlimitno,0) as optlimitno, IsNULL(io.optlimitsold,0) as optlimitsold, IsNULL(io.optsellyn,'Y') optsellyn"
		...
		''Else
			sqlStr = sqlStr & " ,( (s.sell3days*1) - (db_summary.[dbo].[uf_replacezero](s.realstockNo+s.errsampleitemno+s.errbaditemno)) ) as require3daystock"
			sqlStr = sqlStr & " ,( (s.sell7days*1) - (db_summary.[dbo].[uf_replacezero](s.realstockNo+s.errsampleitemno+s.errbaditemno)) ) as require7daystock"
			sqlStr = sqlStr & " ,( (s.sell7days*2) - (db_summary.[dbo].[uf_replacezero](s.realstockNo+s.errsampleitemno+s.errbaditemno)) ) as require14daystock"
			'sqlStr = sqlStr & " ,( (s.sell7days*4) - (db_summary.[dbo].[uf_replacezero](s.realstockNo+s.errsampleitemno+s.errbaditemno)) ) as require28daystock"
		''End If


		sqlStr = sqlStr & " ,d.chargediv ,d.comm_cd ,d.defaultmargin ,d.defaultsuplymargin, ii.mwdiv as onlinemwdiv "
		sqlStr = sqlStr & " from db_summary.dbo.tbl_current_shopstock_summary s"
		sqlStr = sqlStr & " join db_shop.dbo.tbl_shop_item i"
		sqlStr = sqlStr & " 	on s.itemgubun = i.itemgubun"
		sqlStr = sqlStr & " 	and s.itemid = i.shopitemid"
		...
		Select Case FRectOrder
		   Case "BI"
			  sqlStr = sqlStr & " order by i.makerid asc, i.itemgubun asc, i.shopitemid desc, i.itemoption asc"
		   Case "I"
...
'end if
		...
		if  not rsget.EOF  then
			...
			do until rsget.EOF
                ...
                '' 위치변경 2016/06/13 eastone if FItemList(i).Fitemoption="0000" then 맨위에 있었음
	...
	public sub fnewitemstock_list_nostock()
		dim sqlStr,i ,sqlsearch , iStartDate
...
'		sqlStr = sqlStr & "		and d.shopid = s.shopid"
		...
		sqlStr = sqlStr & " 	ELSE i.shopbuyprice END) as shopbuyprice"
		sqlStr = sqlStr & " ,ii.smallimage,IsNULL(ii.sellcash,0) as sellcash, IsNULL(ii.buycash,0) as buycash,IsNULL(ii.orgprice,0) as orgprice"
		sqlStr = sqlStr & " , 0 as logicsipgono, 0 as logicsreipgono"
		sqlStr = sqlStr & " , 0 as brandipgono, 0 as brandreipgono"
		sqlStr = sqlStr & " , 0 as sellno, 0 as resellno,0 as errsampleitemno"
		sqlStr = sqlStr & " , 0 as errbaditemno, 0 as errrealcheckno"
		sqlStr = sqlStr & " , 0 as sysstockno, 0 as realstockno, 0 as requiredStock"
		sqlStr = sqlStr & " , 0 as sell7days, 0 as sell3days ,NULL as lastupdate"
		sqlStr = sqlStr & " , 0 preorderno ,0 preordernofix"
		sqlStr = sqlStr & " ,( 0 ) as require3daystock"
		sqlStr = sqlStr & " ,( 0 ) as require7daystock"
		sqlStr = sqlStr & " ,( 0 ) as require14daystock"
		sqlStr = sqlStr & " ,d.chargediv ,d.comm_cd ,d.defaultmargin ,d.defaultsuplymargin,d.shopid"
		sqlStr = sqlStr & " ,IsNULL(o.optaddprice,0) as optaddprice, IsNULL(o.optaddbuyprice,0) as optaddbuyprice"
		sqlStr = sqlStr & " from db_shop.dbo.tbl_shop_item i"
		sqlStr = sqlStr & " join [db_shop].[dbo].tbl_shop_designer d"
...
'		sqlStr = sqlStr & "		and d.shopid = s.shopid"
	...
	public sub fnewitemstock_list_datamart2()
		...
		sqlStr = "exec [dbo].[sp_Ten_API_get_ShopItemInfo_Summary_CNT] '"&Frectshopid&"'"
		...
		sqlStr = "exec [dbo].[sp_Ten_API_get_ShopItemInfo_Summary_LIST] "&FPageSize&","&FCurrPage&",'"&Frectshopid&"'"
		...
		if  not db3_rsget.EOF  then
			do until db3_rsget.EOF
				...
				FItemList(i).Fsellyn = db3_rsget("sellyn")
				...
				FItemList(i).FitemWeight = db3_rsget("itemWeight")
				FItemList(i).Fsourcearea = db2html(db3_rsget("sourcearea"))
				FItemList(i).Fitemsource = db2html(db3_rsget("itemsource"))
				FItemList(i).Fitemsize = db2html(db3_rsget("itemsize"))

				if FItemList(i).FimageSmall<>"" then FItemList(i).FimageSmall     = "http://webimage.10x10.co.kr/image/small/" + GetImageSubFolderByItemid(FItemList(i).Fitemid) + "/" + FItemList(i).FimageSmall
				if FItemList(i).FOffimgSmall<>"" then FItemList(i).FOffimgSmall = "http://webimage.10x10.co.kr/offimage/offsmall/i" + FItemList(i).Fitemgubun + "/" + GetImageSubFolderByItemid(FItemList(i).Fitemid) + "/" + FItemList(i).FOffimgSmall

                FItemList(i).FimageBasic = db2html(db3_rsget("basicimage"))
				...
				FItemList(i).Foptisusing = db3_rsget("optisusing")
				FItemList(i).Foptsellyn = db3_rsget("optsellyn")
				FItemList(i).Foptlimityn = db3_rsget("optlimityn")
				FItemList(i).Foptlimitno = db3_rsget("optlimitno")
				FItemList(i).Foptlimitsold = db3_rsget("optlimitsold")

				FItemList(i).Fdispcatecode = db3_rsget("dispcatecode")
	...
	public sub fnewitemstock_list_datamart()
		dim sqlStr,i ,sqlsearch , iStartDate
		...
		sqlStr = sqlStr & " join [db_shop].[dbo].tbl_shop_designer d"
		sqlStr = sqlStr & "		on i.makerid = d.makerid"
		sqlStr = sqlStr & " left join db_datamart.dbo.tbl_item ii"
		sqlStr = sqlStr & " 	on i.shopitemid = ii.itemid"
		sqlStr = sqlStr & " 	and i.itemgubun = '10'"
		sqlStr = sqlStr + " left join [db_datamart].[dbo].tbl_item_option o "
		sqlStr = sqlStr + "     on i.itemgubun='10'"
		sqlStr = sqlStr + "     and i.shopitemid=o.itemid"
		sqlStr = sqlStr + "     and i.itemoption=o.itemoption"
		sqlStr = sqlStr & " where i.shopitemid <> 0 and i.itemgubun <>'70' " & sqlsearch	'and (IsNULL(ii.sellyn,'')<>'N')
		...
		sqlStr = sqlStr & " , 0 as sysstockno, 0 as realstockno, 0 as requiredStock"
		sqlStr = sqlStr & " , 0 as sell7days, 0 as sell3days ,NULL as lastupdate"
		sqlStr = sqlStr & " , 0 as preorderno ,0 as preordernofix"
		sqlStr = sqlStr & " ,0 as require3daystock"
		sqlStr = sqlStr & " ,0 as require7daystock"
		sqlStr = sqlStr & " ,0 as require14daystock"
		'sqlStr = sqlStr & " ,( (s.sell7days*4) - (db_summary.[dbo].[uf_replacezero](s.realstockNo+s.errsampleitemno+s.errbaditemno)+s.preordernofix) ) as require28daystock"
