<%
'###########################################################
' Description :  cs 메모 클래스
' History : 2007.10.26 이상구 생성
'           2016.12.07 한용민 수정
'###########################################################

'/사용안함. 디비화 시킴. 2016.12.07 한용민
    ...
    public Sub GetCSMemoList()
		...
		if (FRectContents <> "") then
			...
			if (FRectUserID = "") and (FRectOrderserial = "") and (FRectPhoneNumber = "") then
				tmpSql = "select (max(id) - 80000) as minIdx from [db_cs].[dbo].tbl_cs_memo with (nolock) "	' 너무 짤려서 나온다고 해서 1만개->8만개로 늘림 2018.03.13 한용민
		...
		if (FRectMMGubunExvlude <> "") and (FRectContents = "") and (FRectPhoneNumber = "") then
			'// 전화번호 또는 내용검색시 SMS/이메일 안내 메모 제외 안함
