...
function GetReceiptMinusOrderSUM(orderserial)
    ...
    sqlStr = " select isNULL(accountDiv,'') as accountDiv from [db_order].[dbo].tbl_order_master where orderserial='"&orderserial&"'"
    rsget.CursorLocation = adUseClient
    rsget.Open sqlStr,dbget,adOpenForwardOnly, adLockReadOnly
    if NOT rsget.Eof then
		orgAccountDiv = rsget("accountDiv")
    end if
	rsget.close

	orgAccountDiv=Trim(orgAccountDiv)

	if (orgAccountDiv="7" or orgAccountDiv="20") then
        ...
        sqlStr = " select IsNull(sum(sumpaymentEtc),0) as subtotalprice " &VbCRLF
    	sqlStr = sqlStr + " from [db_order].[dbo].tbl_order_master " &VbCRLF
    	sqlStr = sqlStr + " where orderserial in ( " &VbCRLF
    	sqlStr = sqlStr + " 	select refminusorderserial " &VbCRLF
    	sqlStr = sqlStr + " 	from [db_cs].[dbo].[tbl_new_as_list] " &VbCRLF
...
function fnGetKakaoPaySpendPointSUM(orderserial)
    dim sqlStr
    fnGetKakaoPaySpendPointSUM=0
    sqlStr = " select top 1 realPayedsum " &VbCRLF
    sqlStr = sqlStr + " from db_order.dbo.tbl_order_PaymentEtc " &VbCRLF
    sqlStr = sqlStr + " where orderserial='"&orderserial&"'" &VbCRLF
    sqlStr = sqlStr + " and acctdiv='20'" &VbCRLF
    rsget.CursorLocation = adUseClient
    rsget.Open sqlStr,dbget,adOpenForwardOnly, adLockReadOnly
    if NOT rsget.Eof then
        fnGetKakaoPaySpendPointSUM = rsget("realPayedsum")
    end if
    rsget.close
end function
...
class CCashReceipt
    ...
    public sub GetMinusReceiptList()
        'select top 100 m1.orderserial,m1.linkorderserial ,m1.subtotalprice,
        'm2.cashreceiptreq
        'from db_order.dbo.tbl_order_master m1
       ' 	left join db_order.dbo.tbl_order_master m2
       ' 	on m1.linkorderserial=m2.orderserial
       ' where m1.regdate>'2009-01-01'
       ' and m1.jumundiv=9
       ' and m1.accountdiv=7
       ' and m1.cancelyn='N'
       ' and m1.subtotalprice<0
       ' and m2.cashreceiptreq is Not NULL
    end sub
    ...
    public Sub getCancelRequireList()
        ...
        sqlStr = sqlStr & " c.*, m.ipkumdiv, m.cancelyn as OrderCancelYn from [db_log].[dbo].tbl_cash_receipt c"
...
'rw   sqlStr
    ...
    ' /cscenter/taxsheet/popCashReceipt.asp	' 2016/08/19 이상구 생성
    public sub GetReceiptLogList()
        ...
        sqlStr = " select count(*) as cnt from [db_log].[dbo].tbl_cash_receipt c with (nolock)"
        ...
        sqlStr = " select top " & CStr(FPageSize*FCurrPage)
        sqlStr = sqlStr & " c.* from [db_log].[dbo].tbl_cash_receipt c with (nolock)"
        sqlStr = sqlStr & " where c.orderserial='"&FRectorderSerial&"'"
        sqlStr = sqlStr & AddSQL
...
''response.write     sqlStr
        sqlStr = " select top " & CStr(FPageSize*FCurrPage)
        sqlStr = sqlStr & " c.*, m.ipkumdiv, m.cancelyn as OrderCancelYn,m.subtotalprice  from [db_log].[dbo].tbl_cash_receipt c with (noLock) "
        sqlStr = sqlStr & "     join [db_order].[dbo].tbl_order_master m with (noLock) "
        sqlStr = sqlStr & "     on c.orderserial=m.orderserial"
        sqlStr = sqlStr & "     and m.cancelyn='N'"
        sqlStr = sqlStr & "     and m.ipkumdiv>'4'"
...
''rw sqlStr
	...
	' /cscenter/taxsheet/popCashReceipt.asp
	public sub GetOneCashReceipt()
		dim sqlStr,i

		sqlStr = "select top 1 * from [db_log].[dbo].tbl_cash_receipt with (nolock)"
	...
	public sub GetReceiptByOrderSerial()
        ...
        if FRectExcFailData <> "" then
            sqlStr = sqlStr + " and resultcode is not NULL "
        end if
	...
	public sub GetReceiptByOrderSerial_OLD()
		dim sqlStr,i
		sqlStr = "select top 1 * from [db_log].[dbo].tbl_cash_receipt_OLD with (nolock)"
		sqlStr = sqlStr + " where orderserial='" + FRectOrderserial + "'"
...
Class CMyOrder
	...
	public Sub GetOneReceiptOrder()
        ...
        IF (FRectOldOrder="on") then
            ...
            sqlStr = sqlStr + " left join [db_log].[dbo].tbl_old_order_detail_2003 d on m.orderserial=d.orderserial and d.itemid=0 and d.cancelyn<>'Y'"
        ELSE
		...
		if (FOrderExist) then
			...
			IF (FRectOldOrder="on") then
			    sqlStr = sqlStr + " from [db_log].[dbo].tbl_old_order_detail_2003 d, [db_item].[dbo].tbl_item i"
			ELSE
