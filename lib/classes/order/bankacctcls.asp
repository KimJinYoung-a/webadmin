...
class CBankMileItem
	...
	public function DelMilegelog()
		...
		sqlStr = sqlStr + " and jukyocd in ('2','3')" + VbCrlf						'jukyocd : 2 상품구매, 3 : 부분취소시 환원마일리지
	...
	public function DelGiftCardLog()
		dim sqlStr
...
class CBankAcct
	...
	'// 참고 : db_order.dbo.tbl_account_div
	public Function GetDepositSpendList(byval orderidx)
		dim i,sqlStr
		''예치금 사용 체크.
		sqlStr = "select m.idx, m.orderserial, m.userid "
		sqlStr = sqlStr + " from [db_order].[dbo].tbl_order_master m "
		...
		sqlStr = sqlStr + " and IsNull(m.sumPaymentEtc, 0) > 0 "
		sqlStr = sqlStr + " and e.acctdiv in ('200') "
	...
	'// 참고 : db_order.dbo.tbl_account_div
	public Function GetGiftCardSpendList(byval orderidx)
		dim i,sqlStr
		''기프트카드 사용 체크.
		sqlStr = "select m.idx, m.orderserial, m.userid "
		sqlStr = sqlStr + " from [db_order].[dbo].tbl_order_master m "
		...
		sqlStr = sqlStr + " and m.cancelyn='Y'"
		sqlStr = sqlStr + " and IsNull(m.sumPaymentEtc, 0) > 0 "
		sqlStr = sqlStr + " and e.acctdiv in ('900') "
	...
	public Function GetMiipkummailingList()
		...
		sqlStr = " select count(idx) as cnt from [db_order].[dbo].tbl_order_master"
...
'        sqlStr = sqlStr + " )"
		...
		sqlStr = sqlStr + " reqhp,reqzipcode,reqaddress,reqzipaddr,subtotalprice,miletotalprice,sitename,accountno,accountdiv,regdate, SumPaymentEtc"
		sqlStr = sqlStr + " from [db_order].[dbo].tbl_order_master"
		sqlStr = sqlStr + " where regdate>'2004-01-01'"
		sqlStr = sqlStr + " and datediff(day,regdate,getdate())>" + CStr(FRectDayDiffStart)
		sqlStr = sqlStr + " and datediff(day,regdate,getdate())<=" + CStr(FRectDayDiff)
...
'        sqlStr = sqlStr + " )"
	...
	public Function GetOldMiipkumList()
        ...
        '티켓주문 : 주문 다음날 밤 12시까지 입금되지 않으면 취소된다. (2->1)
        searchEnddateTicket = CStr(dateAdd("d",1*-1,now()))
		...
		'티켓주문 : (m.jumundiv =  '4')
		sqlStr = " select count(m.idx) as cnt from [db_order].[dbo].tbl_order_master m"
		sqlStr = sqlStr + " where "
		sqlStr = sqlStr + " 	1 = 1 "
		sqlStr = sqlStr + " 	and m.cancelyn='N'"
		sqlStr = sqlStr + " 	and m.accountdiv='7'"
		sqlStr = sqlStr + " 	and m.ipkumdiv='2'"
'''		sqlStr = sqlStr + " 	and m.jumundiv='7'" ''현장수령
		sqlStr = sqlStr + " 	and "
		sqlStr = sqlStr + " 		( "
		...
		sqlStr = sqlStr + " 		) "
        sqlStr = sqlStr + "     and m.orderserial not in ("  '' db_order.dbo.tbl_order_CyberAccountLog ''기간 연장의 경우 재껴야 할듯
        ...
        sqlStr = sqlStr + "         and differencekey>0 "
        sqlStr = sqlStr + "     ) "
...
'rw sqlStr        
		...
		sqlStr = sqlStr + " m.reqhp,m.reqzipcode,m.reqaddress,m.reqzipaddr,m.subtotalprice,m.miletotalprice,m.sitename,m.regdate, m.tencardspend, m.SumPaymentEtc, s.senddate as sregdate"
		sqlStr = sqlStr + " from [db_order].[dbo].tbl_order_master m"
		sqlStr = sqlStr + "     left join [db_temp].[dbo].tbl_bankmail_sendlist s on m.orderserial=s.orderserial"
		sqlStr = sqlStr + " where "
		sqlStr = sqlStr + " 	1 = 1 "
		sqlStr = sqlStr + " 	and m.cancelyn='N'"
		sqlStr = sqlStr + " 	and m.accountdiv='7'"
