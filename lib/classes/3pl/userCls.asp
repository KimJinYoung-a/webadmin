...
Class CTPLUser
	...
	public Sub GetTPLUserList()
		dim i,sqlStr, addSql

		addSql = ""
		if (FRectCompanyID <> "") then
			addSql = addSql & " and u.companyid = '" & FRectCompanyID & "'" & vbcrlf
		end if

		if (FRectUseYN <> "") then
			addSql = addSql & " and u.useyn like '" & FRectUseYN & "'" & vbcrlf
        ...
        sqlStr = sqlStr & " from [db_threepl].[dbo].[tbl_user] u" & vbcrlf
		sqlStr = sqlStr & " join [db_threepl].[dbo].[tbl_company] c on u.companyid = c.companyid " & vbcrlf
        ...
        sqlStr = sqlStr & " u.*, c.company_name, c.useyn as companyuseyn " & vbcrlf
        sqlStr = sqlStr & " from [db_threepl].[dbo].[tbl_user] u" & vbcrlf
		sqlStr = sqlStr & " join [db_threepl].[dbo].[tbl_company] c on u.companyid = c.companyid " & vbcrlf
        sqlStr = sqlStr & " where 1 = 1 " & vbcrlf
		sqlStr = sqlStr & addSql
        sqlStr = sqlStr & " order by u.regdate desc " & vbcrlf
		'response.write sqlStr & "<br>"
		'response.end
		...
		If not rsget_TPL.EOF Then
			...
			Do until rsget_TPL.EOF
				Set FItemList(i) = new CTPLUserItem
					FItemList(i).Fuserid		= rsget_TPL("userid")
					FItemList(i).Fcompanyid		= rsget_TPL("companyid")
					FItemList(i).Fusername   	= db2html(rsget_TPL("username"))
	...
	public Sub GetTPLUserOne()
		...
		set FOneItem = new CTPLUserItem
		If not rsget_TPL.EOF Then
			FOneItem.Fuserid		= rsget_TPL("userid")
			FOneItem.Fcompanyid		= rsget_TPL("companyid")
			FOneItem.Fusername   	= db2html(rsget_TPL("username"))
