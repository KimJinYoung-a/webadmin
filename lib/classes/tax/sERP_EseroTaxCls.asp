...
function fnGetOrMakeCUST_sERP(iCorpNo,iTaxKey,byRef icustcd)
    dim retVal : retVal=-9
    dim isGroupPartner : isGroupPartner = false
    
    retval = fnGetCustCDByCorpNo_sERP(iCorpNo,icustcd)

    if (retval=1) or (retval=-1) then
        fnGetOrMakeCUST_sERP = retVal
        exit function
    end if
    
    ''2017/01/06 추가 partner에 등록된 거래처는 자동생성안함.
    if (retval=-9) then
    ...
    if (LEN(replace(iCorpNo,"-",""))=13) then
        fnGetOrMakeCUST_sERP = -9
        exit function
    end if
    
    retVal = fnMakeCustByTaxKey_sERP(iTaxKey, icustcd)
    if (retVal=1) then
        retVal = fnGetCustCDByCorpNo_sERP(iCorpNo,icustcd)
    end if
    fnGetOrMakeCUST_sERP = retVal
...
function fnGetCustCDByCorpNo_sERP(iCorpNo,byRef icustcd)
    Dim sqlStr, retArr
    ...
    IF isArray(retArr) then
        if (UBound(retArr,2)+1)>1 then
            fnGetCustCDByCorpNo_sERP = -1        '''이미 2건 이상 존재 할 경우.
        else
            icustcd = retArr(0,0)
            fnGetCustCDByCorpNo_sERP=1
        end if
    ELSE
        fnGetCustCDByCorpNo_sERP = -9        '' 사업자로 된 거래처 없음.
    end if
end function

function fnMakeCustByTaxKey_sERP(iTaxKey,byRef icustcd)
    fnMakeCustByTaxKey_sERP = -9
''exit function ''개발 임시
    Dim clsEsero, ArrVal
...
rw "목록수신"
		...
		With objCmd
			...
			End With
	    Set objCmd = nothing
	    fnMakeCustByTaxKey_sERP = returnValue
    end if
end function
