...
function fnGetOrMakeCUST(iCorpNo,iTaxKey,byRef icustcd)
    '' 그룹코드 있는것은(매입처) 자동 등록 못하게 (그룹코드로 등록되어야 함)
    
    dim retVal : retVal=-9
    dim isGroupPartner : isGroupPartner = false
    ...
    ''2015/10/21 추가 partner에 등록된 거래처는 자동생성안함.
    if (retval=-9) then
        sqlStr = "select top 1 * from db_partner.dbo.tbl_partner_group "
        sqlStr = sqlStr & "  where replace(company_no,'-','')='"&replace(iCorpNo,"-","")&"'"
    ...
    ''2017/01/06 추가
    if (LEN(replace(iCorpNo,"-",""))=13) then
        fnGetOrMakeCUST = -9
        exit function
    end if
...
function fnGetCustCDByCorpNo(iCorpNo,byRef icustcd)
    ...
    sqlStr = sqlStr & " and USE_YN='Y'" ''' 이조건을 빼야 할듯, (사용하든 안하든 1사업자번호 1거래처가 원칙)'' 사용하는것 먼저, 
...
function IsERPSendAvail(imatchState, imatchType, ierpLinkType, ierpLinkKey, bizSecCD, iTargetState, iarap_cd, byREF inValidStr)
    ...
    if (imatchType="1") and (iarap_cd<>"912") and (iarap_cd<>"940") then  '' and (iarap_cd<>"912") 추가 2016/07/11, and (iarap_cd<>"940") 추가 2016/12/08
...
function getMatchTypeName(imatchType)
    ...
    SELECT CASE imatchType
        ...
        CASE 23 : getMatchTypeName = "온라인기타정산"
        CASE ELSE : getMatchTypeName = imatchType
    ENd SELECT
end function

' 매칭구분      ' 2022.09.29 한용민 생성
Function drawSelectBoxMatchTypeName(selectBoxName, selectedId, chplg)
%>
    <select class="select" name="<%= selectBoxName %>" <%= chplg %>>
        <option value="">선택</option>
        <option value="0" <% if selectedId="0" then response.write "selected" %> >수기 매칭</option>
        <option value="1" <% if selectedId="1" then response.write "selected" %> >온라인매입</option>
        <option value="2" <% if selectedId="2" then response.write "selected" %> >오프라인매입</option>
        <option value="3" <% if selectedId="3" then response.write "selected" %> >아이띵소매입</option>
        <option value="4" <% if selectedId="4" then response.write "selected" %> >원가</option>
        <option value="9" <% if selectedId="9" then response.write "selected" %> >기타매입</option>
        <option value="11" <% if selectedId="11" then response.write "selected" %> >매출</option>
        <option value="19" <% if selectedId="19" then response.write "selected" %> >기타매출(B2B)</option>
        <option value="21" <% if selectedId="21" then response.write "selected" %> >수수료매출(온라인)</option>
        <option value="22" <% if selectedId="22" then response.write "selected" %> >수수료매출(오프라인)</option>
        <option value="23" <% if selectedId="23" then response.write "selected" %> >기타정산</option>
        <option value="999" <% if selectedId="999" then response.write "selected" %> >수정계산서</option>
        <option value="900" <% if selectedId="900" then response.write "selected" %> >자동이체</option>
        <option value="910" <% if selectedId="910" then response.write "selected" %> >기타등록 매칭</option>
    </select>
<%
End Function
...
function getbizSecCDName(iBizCd)
    ...
    SELECT CASE iBizCd
        CASE "0000000100" : getbizSecCDName = "온라인사업부"
        CASE "0000000101" : getbizSecCDName = "온라인(공통)"
        
        CASE "F9660E7B5F" : getbizSecCDName = "디자인문구"
        CASE "FB6040EF1B" : getbizSecCDName = "디지털/핸드폰"
        CASE "FAEF3689C1" : getbizSecCDName = "캠핑/트래블"
        CASE "FC6557E898" : getbizSecCDName = "토이"
        CASE "FA22F89453" : getbizSecCDName = "디자인가전"
        CASE "FC993B59CF" : getbizSecCDName = "가구/수납"
        CASE "F256CB8DF2" : getbizSecCDName = "데코/조명"
        CASE "FD1C0D7F03" : getbizSecCDName = "패브릭/생활"
        CASE "FB839A95DC" : getbizSecCDName = "키친"
        CASE "F85F1BA354" : getbizSecCDName = "푸드"
        CASE "F99F31EFC2" : getbizSecCDName = "패션의류"
        CASE "FAE7E081C6" : getbizSecCDName = "패션잡화"
        CASE "F4EDBB9DA8" : getbizSecCDName = "주얼리/시계"
        CASE "FB78DA7BC1" : getbizSecCDName = "뷰티"
        CASE "FCB4DCEF47" : getbizSecCDName = "베이비/키즈"
        CASE "F85AA8DDBD" : getbizSecCDName = "마케팅"
        CASE "F839B0182D" : getbizSecCDName = "기획/디자인"
        CASE "F3BACA4296" : getbizSecCDName = "컨텐츠"
        CASE "FB8D33268D" : getbizSecCDName = "전략상품"
        CASE "F45FDADA25" : getbizSecCDName = "WD"
        CASE "F2E4E8AB21" : getbizSecCDName = "채널운영"
        CASE "F026152B4A" : getbizSecCDName = "채널제휴"
        CASE "FDBCCEFBFF" : getbizSecCDName = "PB"
        CASE "FC7CF02FC5" : getbizSecCDName = "Cat/Dog"
	
        CASE "0000000200" : getbizSecCDName = "오프라인사업부"
        ...
        CASE "0000000212" : getbizSecCDName = "일산벨라시타점"
        CASE "0000000214" : getbizSecCDName = "건대커먼그라운드"
        CASE "0000000217" : getbizSecCDName = "고양스타필드점"
        CASE "0000000218" : getbizSecCDName = "N서울타워점"
        CASE "0000000219" : getbizSecCDName = "원더플레이스대구점"
        CASE "0000000220" : getbizSecCDName = "원더플레이스홍대점"
        CASE "0000000210" : getbizSecCDName = "텐텐문방구"
        CASE "0000000221" : getbizSecCDName = "원더플레이스대전점"
 
        CASE "0000000300" : getbizSecCDName = "아이띵소사업부"
        CASE "0000000301" : getbizSecCDName = "아이띵소본사"
        CASE "0000000302" : getbizSecCDName = "아이띵소홍대매장"
        CASE "0000000303" : getbizSecCDName = "아이띵소종로영풍"

        
        CASE "0000000400" : getbizSecCDName = "핑거스사업부"
        CASE "0000000401" : getbizSecCDName = "더핑거스"
        CASE "0000000402" : getbizSecCDName = "카페1010"

        CASE "0000000500" : getbizSecCDName = "지원부서"
        ...
        CASE "0000000600" : getbizSecCDName = "상품소싱사업부"

        CASE "0000000800" : getbizSecCDName = "기타"
        CASE "0000000810" : getbizSecCDName = "기타"

        CASE "FA3A72F1F9" : getbizSecCDName = "29CM"
        CASE "0000000910" : getbizSecCDName = "29cm"
        CASE "F1ACBF3221" : getbizSecCDName = "3PL(아이코닉)"

        CASE "0000009000" : getbizSecCDName = "공통안분"
