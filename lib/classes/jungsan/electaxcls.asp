...
class CElecTaxRegItem
	...
	public Fcur_addr '서울시 종로구 대학로 57 홍익대학교 대학로캠퍼스 교육동 14층 텐바이텐
    ...
    public Ffinal_status
    public FRectBillSite
    public FJGubun
	...
	Private Sub Class_Initialize()
		...
		Ffinal_status = "12" ''바로 승인(12) , 01 :일반.
	end sub
...
class CElecTaxReg
    ...
    ''bill36524 발행 : flexApi로 발행함..
    public sub ExecReverseTaxBill36524
        exit sub
    end sub
	...
	public sub ExecDTIXmlDom()
		...
		reqParam = reqParam + "&final_status=" + FRectOneRegitem.Ffinal_status
...
'dbget.close()	:	response.End
		...
		objXMLHTTP.Open "POST",	"http://web1.neoport.net:8383/tx_create.req", False		' 리얼
'		objXMLHTTP.Open "POST",	"http://ifs.neoport.net:8383/tx_create.req", False		' 테스트
		...
		If FResultMsgALL <> "" Then
			...
			if Fresultmsg="OK" then
			    ...
			    elseif (FRectOneRegitem.Fjungsangubun="OFFSHOP") then	' 오프 가맹점
					sqlStr = " update db_shop.dbo.tbl_fran_meachuljungsan_master" + vbCrlf
					sqlStr = sqlStr + " set taxlinkidx=" + CStr(FRectOneRegitem.Fidx) + vbCrlf
					sqlStr = sqlStr + " ,neotaxno='" + CStr(Ftax_no) + "'" + vbCrlf
					sqlStr = sqlStr + " ,stateCd='4'"  + vbCrlf		' 계산서발행완료
					sqlStr = sqlStr + " ,taxdate = '" + FRectOneRegitem.Fwrite_date + "'"  + vbCrlf
					sqlStr = sqlStr + " ,taxregdate = getdate() " + vbCrlf
					sqlStr = sqlStr + " where idx=" + CStr(FRectOneRegitem.Fjungsanid)
					...
					sqlStr = sqlStr + " SET a.segumDate = '" + FRectOneRegitem.Fwrite_date + "'"  + vbCrlf
					...
					sqlStr = sqlStr + " WHERE b.masterIdx = " + CStr(FRectOneRegitem.Fjungsanid)
	...
	public sub SavePreData()
        ...
        rsget("billsite") = FRectOneRegitem.FRectBillSite   ''200912추가
        rsget("jgubun") = FRectOneRegitem.FJGubun
		rsget.update
			iid = rsget("idx")
		rsget.close
	...
	Public Function fnTaxdateList
		Dim sqlStr, addSql
		
		If FRectyyyy <> "" Then
			addSql = addSql & " and yyyy = '"&FRectyyyy&"' "
		End If

		sqlStr = ""
		sqlStr = sqlStr & " SELECT TOP 12 idx, yyyymm, taxdate, regUserid, lastupdate "
		sqlStr = sqlStr & " FROM [db_sitemaster].[dbo].[tbl_taxdate_manage]  "
		sqlStr = sqlStr & " WHERE 1=1 "
		sqlStr = sqlStr & addSql
		sqlStr = sqlStr & " ORDER BY yyyymm ASC "
	    rsget.Open sqlStr,dbget,1
	    If not rsget.EOF Then
	        fnTaxdateList = rsget.getRows()
	    End If
	    rsget.Close
	End Function
