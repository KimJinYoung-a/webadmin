...
class UserJoinNaiMasterItem
	...
	Private Sub Class_Initialize()
		...
		FItemList(0).FNaiStr = "10대"
		FItemList(0).FNaiStart = 0
		FItemList(0).FNaiEnd = 20

		FItemList(1).FNaiStr = "20~23세"
		FItemList(1).FNaiStart = 20
		FItemList(1).FNaiEnd = 23

		FItemList(2).FNaiStr = "24~27세"
		FItemList(2).FNaiStart = 23
		FItemList(2).FNaiEnd = 27

		FItemList(3).FNaiStr = "28~30세"
		FItemList(3).FNaiStart = 27
		FItemList(3).FNaiEnd = 30

		FItemList(4).FNaiStr = "31~33세"
		FItemList(4).FNaiStart = 30
		FItemList(4).FNaiEnd = 33

		FItemList(5).FNaiStr = "34세이상"
		FItemList(5).FNaiStart = 33
		FItemList(5).FNaiEnd = 100
...
class UserJoinClass
	...
	if FRectBeasongArea = "1" then
		sqlStr = "select count(m.orderserial) as cnt, Left(n.zipcode,1) as area" + vbcrlf
		...
		sqlStr = sqlStr + " group by Left(n.zipcode,1)" + vbcrlf
		sqlStr = sqlStr + " order by area"
	else
		sqlStr = "select count(orderserial) as cnt, Left(reqzipcode,1) as area" + vbcrlf
		...
		sqlStr = sqlStr + " group by Left(reqzipcode,1)" + vbcrlf
		sqlStr = sqlStr + " order by area"
	end if
	...
	public sub GetUserJoinByNai()
		...
		sqlStr = "select sum((i.sellcash - i.buycash) * d.itemno) as cnt,(year(getdate())-Left(n.juminno,2)-1900) as nai," + vbcrlf
		...
		sqlStr = sqlStr + " and m.regdate >='" + FRectStart + "'" + vbcrlf
		sqlStr = sqlStr + " and m.regdate <'" + FRectEnd + "'" + vbcrlf
		sqlStr = sqlStr + " and i.cate_large = '" + FRectGubun + "'" + vbcrlf
		sqlStr = sqlStr + " group by (year(getdate())-Left(n.juminno,2)-1900), Left(Right(n.juminno,7),1)" + vbcrlf
		sqlStr = sqlStr + " order by (year(getdate())-Left(n.juminno,2)-1900) asc"
	...
	public sub GetUserJoinByNai2()
		dim sqlStr
		dim i

		sqlStr = "select sum(d.itemcost * d.itemno) as cnt,(year(getdate())-Left(n.juminno,2)-1900) as nai," + vbcrlf
		sqlStr = sqlStr + " Left(Right(n.juminno,7),1) as sex" + vbcrlf
