<%
'###############################################
' PageName :exhibitionCls
' Discription : 사이트 메인 exhibition
' History : 2016.04.07 이종화 생성
'###############################################

Class CexhibitionItem
	public fidx
	Public fexhibitiontitle
	Public Fstartdate
	Public Fenddate 
	Public Fadminid
	Public Flastadminid
...
Class Cexhibition
    ...
    public Sub GetOneContents()
        dim sqlStr
        sqlStr = "select top 1 * "
        sqlStr = sqlStr + " from db_sitemaster.dbo.tbl_mobile_main_exhibition_list "
        ...
        set FOneItem = new CexhibitionItem
        
        if Not rsget.Eof then
    		FOneItem.fidx				= rsget("idx")
    		FOneItem.fexhibitiontitle	= rsget("exhibitiontitle")
    ...
    public Sub GetContentsList()
		...
		sqlStr = " select count(idx) as cnt from db_sitemaster.dbo.tbl_mobile_main_exhibition_list "
		...
		if  not rsget.EOF  then
			...
			do until rsget.eof
				set FItemList(i) = new CexhibitionItem
				
				FItemList(i).fidx					= rsget("idx")
				FItemList(i).fexhibitiontitle		= rsget("exhibitiontitle")
				...
				FItemList(i).Ftopview				= rsget("topview")
    ...
    public Sub GetContentsItemList()
       dim sqlStr, addSql, i
		...
		sqlStr = " select count(listidx) as cnt from db_sitemaster.dbo.tbl_mobile_main_exhibition_item "
		sqlStr = sqlStr + " where 1=1"
