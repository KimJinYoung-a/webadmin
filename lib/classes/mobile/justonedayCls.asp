...
Class Cjustoneday
    ...
    public Sub GetOneContents()
        dim sqlStr
        sqlStr = "select top 1 * "
        sqlStr = sqlStr + " from db_sitemaster.dbo.tbl_mobile_main_justoneday_list "
        ...
        set FOneItem = new CjustonedayItem
	...
	public Sub GetOneSubItem()
        ...
        if Not rsget.Eof then
			...
			FOneItem.Fextraimg		= staticImgUrl & "/mobile/justoneday" & rsget("extraimg")
    ...
    public Sub GetContentsList()
		...
		sqlStr = " select count(idx) as cnt from db_sitemaster.dbo.tbl_mobile_main_justoneday_list "
    ...
    public Sub GetContentsItemList()
       dim sqlStr, addSql, i
		...
		sqlStr = " select count(listidx) as cnt from db_sitemaster.dbo.tbl_mobile_main_justoneday_item "
		sqlStr = sqlStr + " where 1=1"
        ...
        sqlStr = "Select top " + CStr(FPageSize * FCurrPage) + " s.subidx , s.listidx , s.itemid , s.isusing as itemusing , s.sortnum , s.extraimg , s.extraurl , i.itemname, i.smallImage "
        sqlStr = sqlStr & "From [db_sitemaster].[dbo].tbl_mobile_main_justoneday_item as s "
        sqlStr = sqlStr & "	left join db_item.dbo.tbl_item as i "
        sqlStr = sqlStr & "		on s.itemid=i.itemid "
        sqlStr = sqlStr & "			and i.itemid<>0 "
		...
		if  not rsget.EOF  then
			...
			do until rsget.eof
				...
				FItemList(i).Fextraimg			= staticImgUrl & "/mobile/justoneday" & rsget("extraimg")
				FItemList(i).Fextraurl				= rsget("extraurl")
