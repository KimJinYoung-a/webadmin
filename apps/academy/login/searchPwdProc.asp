...
CASE "A" '비밀번호 인증
	...
	sAuthno = requestCheckVar(trim(request.Form("sAuthno")),6)
		skey    = requestCheckVar(request.Form("skey"),32)
		 ...
		 if dbAuthno <> sAuthno then
		 	response.write("<script>alert('입력하신 인증번호가 일치하지 않습니다.\n인증번호를 정확히 입력해주세요.') ;parent.document.frmAuth.sAuthNo.value='';</script>") 
		   response.End
		 end if
	...
	response.write("<script>top.location.href='/apps/academy/login/resetPwd.asp';</script>")  
	response.end
CASE "C" '비밀번호 변경
...
Enc_2userpass64= SHA256(MD5(userpassSec1))
  ...
  dbget.Execute sql
	...
	response.write "<script language='javascript'>" &vbCrLf &_
						"	alert('변경되었습니다.');fnAPPclosePopup();" &vbCrLf &_
						"</script>"
		dbget.close()	:	response.End
