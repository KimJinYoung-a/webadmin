	...
	public function GetDiyQnaList()
		...
		If FRectSearchDIV<>"0" Then
			If FRectSearchDIV="1" Then
				sqlsearch = sqlsearch & " and q.itemid like '%"&FRectSearchTXT&"%'"
			ElseIf FRectSearchDIV="2" Then
				sqlsearch = sqlsearch & " and i.itemname like '%"&FRectSearchTXT&"%'"
			ElseIf FRectSearchDIV="3" Then
				sqlsearch = sqlsearch & " and q.userid like '%"&FRectSearchTXT&"%'"
			ElseIf FRectSearchDIV="4" Then
				sqlsearch = sqlsearch & " and q.comment like '%"&FRectSearchTXT&"%'"
			End If
		End If
		...
		if FRectStateDIV <> "0" then		''상품상세 qna
			sqlsearch = sqlsearch & " and q.answerYN = '"&FRectStateDIV&"'"
		end If
		'// 총 카운트
		sqlStr = "select count(*) as cnt"
...
'		response.end
		...
		sqlStr = "select top " + Cstr(FPageSize * FCurrPage)
		sqlStr = sqlStr & " q.idx, q.itemid, q.reply_group_idx, q.reply_depth, q.reply_num, q.userid, q.userlevel, q.replyuserid, q.answerYN, q.qna, q.title, q.comment, q.device, q.isusing, q.regdate, i.makerid, i.itemname, i.listimage"
		sqlStr = sqlStr & " from [db_academy].[dbo].[tbl_academy_qna_new] as q "
		sqlStr = sqlStr & " JOIN db_academy.dbo.tbl_diy_item as i "
		sqlStr = sqlStr & "		on q.itemid=i.itemid "
		sqlStr = sqlStr & " where q.isusing='Y' " + sqlsearch
        ...
        if  not rsACADEMYget.EOF  then
            ...
            do until rsACADEMYget.EOF
				set FItemList(i) = new CdiyQnaItem
					...
					FItemList(i).FListimage			= rsACADEMYget("listimage")
				if ((Not IsNULL(FItemList(i).FListimage)) and (FItemList(i).FListimage<>"")) then FItemList(i).FListimage = imgFingers & "/diyItem/webimage/list/" + GetImageSubFolderByItemid(FItemList(i).FItemID) + "/"  + FItemList(i).FListimage
                rsACADEMYget.movenext
                i=i+1
            loop
        end if
