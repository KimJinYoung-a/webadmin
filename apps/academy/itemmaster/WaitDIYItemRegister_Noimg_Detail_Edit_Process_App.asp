...
If Request.Form("currstate") = 1 Then
'###########################################################################
'앱 등록 로그 남기기
'###########################################################################
sqlStr = "insert into db_academy.dbo.tbl_diy_wait_item_log(itemid, currstate, logmsgcd,logmsg, regdate, adminid)"
sqlStr = sqlStr & " values ('" & waititemid & "','" & Request.Form("currstate") & "','app','" + flgDevice + "',getdate(),'" + CStr(DesignerID) + "')"  
...
If Trim(Request.Form("itemvideo")) <> "" Then
	...
	'' 유튜브 입력 방식은 아래 두가지임 2번째 타입 작업 요망
	''1. <iframe width="560" height="315" src="https://www.youtube.com/embed/Jb7ambXUZMA" frameborder="0" allowfullscreen></iframe>
	''2. https://youtu.be/Jb7ambXUZMA
