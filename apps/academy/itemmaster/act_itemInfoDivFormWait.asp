...
<%
	...
	Dim ArrInfo, ArrCnt, ArrinfoDiv, ArrinfoCont, ArrinfoChk
	ArrCnt=0
	If param <> "" Then
	ArrInfo = Split(param,"!")
	ArrinfoDiv = ArrInfo(0)
	ArrinfoCont = Split(ArrInfo(1),",")
	ArrinfoChk = Split(ArrInfo(2),",")
	ArrCnt = ubound(ArrinfoCont)
	End If
	...
	function getFormInfoType(icd,inm,idc,itp,irq,cdv,ctn,lnum)
		...
		Select Case itp
			...
			Case "I" '단어
				strRst = strRst & "<dd><input type='text' name='infoCont' " & chkIIF(irq="Y","id='[on,off,off,off]["&inm&"]'","") & " value='" & ctn & "' /><input type='hidden' name='infoCd' value='" & icd & "' /><input type='hidden' name='infoChk' value='N' /></dd>" & vbCrLf
				if Not(isNull(idc) or idc="") Then
				strRst = strRst & "<dd class='addition'>" & idc & "</dd>" & vbCrLf
				End If
			Case "T" '문장
				strRst = strRst & "<dd><textarea name='infoCont' " & chkIIF(irq="Y","id='[on,off,off,off]["&inm&"]'","") & " rows='3'>" & ctn & "</textarea><input type='hidden' name='infoCd' value='" & icd & "' /><input type='hidden' name='infoChk' value='N' /></dd>" & vbCrLf
				if Not(isNull(idc) or idc="") Then
				strRst = strRst & "<dd class='addition'>" & idc & "</dd>" & vbCrLf
				End If
			Case "C" '여부+단어
				strRst = strRst & "<dd class='sltYN selectBtn'>" & vbCrLf
				If cdv ="Y" Then
			...
			Case "K" '여부+선택문장
				...
				End If
			Case "M" '날짜(년월)
				strRst = strRst & "<dd><input type='text' name='infoCont' " & chkIIF(irq="Y","id='[on,off,off,off]["&inm&"]'","") & " maxlength='7' value='" & ctn & "' /><input type='hidden' name='infoCd' value='" & icd & "' /><input type='hidden' name='infoChk' value='N' /></dd>" & vbCrLf
				if Not(isNull(idc) or idc="") Then
				strRst = strRst & "<dd class='addition'>" & idc & "</dd>" & vbCrLf
				End If
			Case "D" '날자(년월일)
				strRst = strRst & "<dd><input name='infoCont' id='[on,off,off,off]["&inm&"]' value='" & ctn & "' class='text' size='10' maxlength='10' /><input type='hidden' name='infoCd' value='" & icd & "' /><input type='hidden' name='infoChk' value='N' /></dd>" & vbCrLf
				if Not(isNull(idc) or idc="") Then
				strRst = strRst & "<dd class='addition'>" & idc & "</dd>" & vbCrLf
				End If
			Case "P" 'DESC표시
				strRst = strRst & "<dd><input type='text' name='infoCont' class='text_ro' readonly style='width:80%;' value='" & idc & "' /><input type='hidden' name='infoCd' value='" & icd & "' /><input type='hidden' name='infoChk' value='N' /></dd>" & vbCrLf
				if Not(isNull(idc) or idc="") Then
				strRst = strRst & "<dd class='addition'>" & idc & "</dd>" & vbCrLf
				End If
			Case "N" '입력받지 않음
	...
	'// 해당 품목내 항목 접수
	strSql = "Select c.infoCd, c.infoItemName, isnull(c.infoDesc,'') as infoDesc, c.infoType, c.infoReq "
	...
	<li id="info_<%=i%>">
		<dl class="infoUnit">
			...
			<% if ArrinfoDiv<>infoDiv Then %>
			<%=getFormInfoType(rsACADEMYget(0),rsACADEMYget(1),replace(rsACADEMYget(2),"'","&#39;"),rsACADEMYget(3),rsACADEMYget(4),"","",i)%>
			<% else %>
			<%=getFormInfoType(rsACADEMYget(0),rsACADEMYget(1),replace(rsACADEMYget(2),"'","&#39;"),rsACADEMYget(3),rsACADEMYget(4),ArrinfoChk(i-1),replace(ArrinfoCont(i-1),"'","&#39;"),i)%>
			<% end if %>
		</dl>
	</li>
	<%
