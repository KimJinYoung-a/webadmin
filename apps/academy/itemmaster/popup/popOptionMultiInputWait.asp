...
<%
Dim ItemDefaultMargin, SellCash, BuyCash, DefaultMargin, thisUrl, OptionDiv
...
Dim ArrCnt, OptionTypeName, ix
'Response.write param
'Response.end
dim jsonParse
set jsonParse = JSON.parse(param)
redim ArroptionName(jsonParse.optionname.length-1)
redim ArrOptionPrice(jsonParse.optionname.length-1)
redim ArrOptionBuyPrice(jsonParse.optionname.length-1)
OptionTypeName = jsonParse.opttypename
ArrCnt = jsonParse.optionname.length-1
For ix = 0 To ArrCnt
	ArrOptionName(ix) = jsonParse.optionname.get(ix)
	ArrOptionPrice(ix) = jsonParse.optionaddprice.get(ix)
	ArrOptionBuyPrice(ix) = jsonParse.optionbuyprice.get(ix)
Next
...
function fnOptionEachDisable(num){
	...
	$("#optdiv"+num).remove();
	$("#Opt #addbtn0").attr("disabled",false);//버튼 활성화
}
</script>
</head>
<body>
<div class="wrap bgGry">
	<div class="container">
		...
		<input type="hidden" name="checkopt" id="checkopt" value="<% If ArrCnt>=0 Then Response.write ArrCnt Else Response.write "0" End If %>">
		<input type='hidden' name='checkcnt' id='checkcnt' value='<% If ArrCnt>=0 Then Response.write ArrCnt Else Response.write "0" End If %>' />
						...
						<dl class="optionUnit">
							...
							<dd id="optdiv<%=ix%>">
								<ul>
									<li>
										<div>
											<span><input type="text" name="optionName" value="<%= ArrOptionName(ix) %>" id="optionName" placeholder="옵션 내 항목" style="width:100%;" /></span>
										</div>
									</li>
									<li>
										<div>
											<span><input type="number" name="optaddprice" id="optaddprice" value="<%= ArrOptionPrice(ix) %>" placeholder="추가금액" style="width:100%;" onKeyUp="AutoCalcuBuyPrice(<%=ix%>);" /></span>
											<span>원</span>
										</div>
									</li>
									<li>
										<div>
											<span><input type="number" name="optaddbuyprice" id="optaddbuyprice" value="<%= ArrOptionBuyPrice(ix) %>" placeholder="공급가" style="width:100%;" readonly /></span>
											<span>원</span>
										</div>
									</li>
								</ul>
								<button type="button" class="btnM1 btnGry tMar1r" onclick="fnOptionEachDisable('<%=ix%>')"<% If ArrCnt<1 Then %> style="display:none"<% End If %>>옵션삭제</button>
							</dd>
							<% Next %>
							<% Else %>
							<dd id="optdiv0">
