...
function addDispCateItem(dcd,cnm,div,dpt) {
	...
	if(CheckOverlap){
		...
		$("input[name='checkcate']").val($("input[name='checkcate']").val()+1);
	}
}
...
function fnAppCallWinConfirm(){
	...
	if($("input[name='checkcate']").val()<1){
		...
		document.cate.action="/apps/academy/itemmaster/popup/DIYItemPopupDetailinfoEdit_Process.asp";
	...
	<div class="container">
		...
		<div class="content bgGry">
			...
			<div class="ctgySetting">
				...
				<div class="addBtn">
					<button type="button" class="btnB1 btnDkGry" disabled="disabled" onclick="sendDispCateItem();chgodr('HelpInfo',1,'','');"><span class="itemAdd">추가</span></button>
				</div>
				<div class="addCtgyList">
					<ul id="tbl_DispCate"><% If itemid<>"" Then %><% =getDispCategory(itemid) %><% End If %></ul>
				</div>
				<div ></div>
				<div class="linkNotice" id="HelpInfo" style="display:<% If itemid<>"" Then %><% If getDispCategoryCount(itemid) > 0 Then %>none<% End If %><% End If %>">
