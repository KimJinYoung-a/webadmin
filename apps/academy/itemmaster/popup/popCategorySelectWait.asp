...
Dim waititemid, param
waititemid = requestCheckVar(request("waititemid"),10)
param = URLDecode(requestCheckVar(request("param"),256))

Dim ArrCate,ArrCnt, ArrCode, ArrDepth, ArrDefault, ArrCateName, ix
ArrCnt=0
If param <> "" Then
ArrCate = Split(param,"!")
ArrCode = Split(ArrCate(0),",")
ArrDepth = Split(ArrCate(1),",")
ArrDefault = Split(ArrCate(2),",")
ArrCateName = Split(ArrCate(3),",")
ArrCnt = ubound(ArrCode)
End If
%>
	...
	if($("input[name='checkcate']").val()<1){
		...
		$("input[name=isDefault]").each(function(idx){
			...
			if(idx<1){
				...
				arrcatename = $("input[name=arrdepthname]:eq(" + idx + ")").val()
			}else{
				arrcode += "," + $("input[name=catecode]:eq(" + idx + ")").val();
				arrdepth += "," + $("input[name=catedepth]:eq(" + idx + ")").val();
				arrdefault += "," + $("input[name=isDefault]:eq(" + idx + ")").val();
		...
		jsontxt = catename + "!" + arrcode + "!" + arrdepth + "!" + arrdefault + "!" + arrcatename;
...
<div class="wrap bgGry">
	<div class="container">
		...
		<input type="hidden" name="checkcate" value="<%=ArrCnt+1%>">
		<div class="content bgGry">
			...
			<div class="ctgySetting">
				...
				<div class="addCtgyList">
					...
					<% If ArrCnt>=0 Then %>
						<% For ix = 0 To ArrCnt	%>
						<li><div><span><%=ArrCateName(ix)%></span>
						<input type='hidden' name='isDefault' value="<%=ArrDefault(ix)%>">
						<input type='hidden' name='catecode' value="<%=ArrCode(ix)%>">
						<input type='hidden' name='catedepth' value="<%=ArrDepth(ix)%>">
						<input type='hidden' name='arrdepthname' value="<%=ArrCateName(ix)%>">
						<button type='button' class='btnListDel' onClick="delDispCateItem('<%=ArrDepth(ix)%>')">삭제</button></div></li>
						<% Next %>
					<% End If %>
					</ul>
				</div>
				<div ></div>
				<div class="linkNotice" id="HelpInfo" style="<%=chkIIF(ArrCnt>1,"display:none;","")%>">
