...
function chgInfoDiv(v) {
	...
	if(v=="") {
		...
		var str = $.ajax({
			type: "POST",
			url: "/apps/academy/itemmaster/act_itemInfoDivForm.asp",
			data: "itemid=<%=waititemid%>&ifdv="+v+"&fingerson=on",
			dataType: "html",
			async: false
		}).responseText;
...
function chgInfoChk(fm,cnt,yn) {
	...
	$("#info_"+cnt+" input[name=infoCont]").val();
}
...
function fnAppCallWinConfirm(){
	...
	if($("#infoDiv option:selected").val()==""){
		...
		document.iteminfo.action="/apps/academy/itemmaster/popup/WaitDIYItemPopupDetailinfoEdit_Process.asp";
		document.iteminfo.target="FrameCKP";
		document.iteminfo.submit();
	}
}

function fnDetailInfoEnd(waititemid){
	
	jsontxt = $("#infoDiv").val() + "!" + $("#infoDiv option:selected").text() + "!" + waititemid;
		//alert("reg:"+waititemid +  " / " + jsontxt);
	fnAPPopenerJsCallClose("fnWaitItemInfoDivSet(\"" + jsontxt + "\")");
...
<div class="wrap bgGry">
	<div class="container">
		...
		<input type="hidden" name="mode" value="iteminfofirst">
