...
<%
Dim ItemDefaultMargin, SellCash, BuyCash, DefaultMargin, thisUrl, limityn, waititemid
...
dim oitemoption, optionTypename, ix
...
function fnDetailInfoEnd(){
	...
	jSonTXT = JSON.stringify({"mode":$("#mode").val(), "optiontypename":$("input[name=optionTypename]").val(), "optionname":arrOptName, "optionaddprice":arrOptAddPrice, "optionbuyprice":arrOptBuyPrice,"optioncode":$("#optioncode").val(),"waititemid":<%=waititemid%>});
	jSonTXT = Base64.encode(jSonTXT);
	fnAPPopenerJsCallClose("fnOptionSet(\"" + jSonTXT + "\")");
}

function fnDetailInfoEnd3(waititemid){
	var jSonTXT;
	var arrOptName, arrOptAddPrice, arrOptBuyPrice;
	arrOptName = new Array();
	arrOptAddPrice = new Array();
				...
				<div class="optSetListWrap" id="Opt">
					<div class="setList">
						<dl class="optionUnit">
							...
							<% If oitemoption.FResultCount>0 Then %>
							<% For ix=0 To oitemoption.FResultCount - 1 %>
							<dd id="optdiv<%=ix%>">
								<ul>
									<li>
										<div>
											<span><input type="text" name="optionName" value="<%= oitemoption.FITemList(ix).FOptionName %>" id="optionName" placeholder="옵션 내 항목" style="width:100%;" /></span>
										</div>
									</li>
									<li>
										<div>
											<span><input type="number" name="optaddprice" id="optaddprice" value="<%= oitemoption.FITemList(ix).Foptaddprice %>" placeholder="추가금액" style="width:100%;" onKeyUp="AutoCalcuBuyPrice(<%=ix%>);" /></span>
											<span>원</span>
										</div>
									</li>
									<li>
										<div>
											<span><input type="number" name="optaddbuyprice" id="optaddbuyprice" value="<%= oitemoption.FITemList(ix).Foptaddbuyprice %>" placeholder="공급가" style="width:100%;" readonly /></span>
											<span>원</span>
										</div>
									</li>
								</ul>
								<button type="button" class="btnM1 btnGry tMar1r" onclick="fnOptionEachDisable('<%=ix%>')"<% If oitemoption.FResultCount<2 Then %> style="display:none"<% End If %>>옵션삭제</button>
							</dd>
							<% Next %>
							<% Else %>
							<dd id="optdiv0">
