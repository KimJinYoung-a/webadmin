...
<%
dim MakerID, OrderSerial, ojumun, oitem, ix, odiv, CheckOrderConfirmYN, CSID
...
function fnSongjangInputCheck(){
	var checkedcnt=0;
	var arrdetailidx='';
	$(".artList input[name=ordercheck]").each(function(i){
		if($(".artList input[name=ordercheck]:eq(" + i + ")").is(":checked")==true){
			checkedcnt++;
			if(i<1){
				arrdetailidx += $(".artList input[name=ordercheck]:eq(" + i + ")").val();
			}else{
				arrdetailidx += "," + $(".artList input[name=ordercheck]:eq(" + i + ")").val();
			}
		}
...
function fnSongjangAllCheck(){
	var arrdetailidx='';
	$(".artList input").prop('checked', true);
	$(".artList input[name=ordercheck]").each(function(i){
		if(i<1){
			arrdetailidx += $(".artList input[name=ordercheck]:eq(" + i + ")").val();
		}else{
	...
	<div class="container">
		...
		<div class="content bgGry">
			...
			<div class="orderManage">
				<ul class="listTab">
					<li onclick="fnGoTapPage('C')"><div>CS정보</div></li>
					<li class="current" onclick="fnGoTapPage('B')"><div>주문상세</div></li>
				</ul>
