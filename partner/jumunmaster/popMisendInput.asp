...
<%
'###########################################################
' Description : [CS]배송관리>>미출고리스트 NEW
' History : 이상구 생성
'			2017.12.21 한용민 수정
'###########################################################
%>
...
dim i, tmpStr, michulgoidx, sqlStr
...
function ShowHideObject(comp) {
	...
	var Isipgodate = ((comp.value == '05') || (comp.value == '66') || (comp.value == '06'));
	...
	<% if (C_ADMIN_USER) then %>
		if ((comp.value == '05') || (comp.value == '66') || (comp.value == '06')) {
...
function MisendInput(){
    ...
    // 품절출고불가(05), 가격오류(66)
    if ((frm.MisendReason.value != "05") && (frm.MisendReason.value != "66") && (frm.MisendReason.value != "06")) {
    ...
    if (confirm('미출고 사유를 저장 하시겠습니까?')){
	    frm.action = "/partner/jumunmaster/upchebeasong_Process.asp";
	    frm.submit();
	}
}
...
<form name="frmMisend" method="post" action="/partner/jumunmaster/upchebeasong_Process.asp" onsubmit="return false;" style="margin:0px;">
...
<table width="610" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
	...
	<tr height="25" bgcolor="#FFFFFF">
	    <td>부족수량(품절시)</td>
	    <td>
			<input type="text" class="text" name="itemlackno" size="2" value="<%= omisend.FOneItem.Fitemlackno %>"> 개
		</td>
	</tr>
	<tr height="25" bgcolor="#FFFFFF">
	    <td>미출고사유</td>
	    <td>
	        <% if (Not C_ADMIN_USER) and (Not C_CSUser) and omisend.FOneItem.isMisendAlreadyInputed then %>
	        ...
	        <% else %>
				<select name="MisendReason" id="MisendReason" class="select" onChange="ShowHideObject(this);">
					...
					<% if C_ADMIN_USER then '// 고객센터에서만 입력가능 %>
					<option value="">---------</option>
					<option value="66" <%= ChkIIF(omisend.FOneItem.FMisendReason="66","selected"," ") %> >가격오류</option>
					<% end if %>
					<option value="">---------</option>
					<option value="05" <%= ChkIIF(omisend.FOneItem.FMisendReason="05","selected"," ") %> >품절출고불가</option>
					<option value="06" <%= ChkIIF(omisend.FOneItem.FMisendReason="06","selected"," ") %> >택배파업(취소)</option>
					<option value="">---------</option>
				</select>
			<% end if %>
			<span id="itemSoldOutFlag" name="itemSoldOutFlag" style="display=none" align="right" >
			<input type="radio" name="itemSoldOut" value="S" checked>상품 일시품절처리
			</span>
		</td>
	</tr>
	<tr height="25" bgcolor="#FFFFFF">
	    ...
	    <td>
			...
			<textarea class="textarea" name="reqaddstr" cols="65" rows="9" <% if (Not C_ADMIN_USER) and omisend.FOneItem.isMisendAlreadyInputed then %>readonly<% end if %> ><% if (omisend.FOneItem.FupcheRequestString = "") then %>고객통화여부 : N
변경가능 옵션 :
기타 전달사항 :
...
<table width="610" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
	...
	<tr bgcolor="#FFFFFF" id="SMSContentAll">
	    <td>
			...
			sqlStr = sqlStr & " where l.prevcode = '05'"	' 품절출고불가
			sqlStr = sqlStr & " and l.code = '03'"	' 출고지연
			sqlStr = sqlStr & " and l.ipgodate is not null"
			'sqlStr = sqlStr & " and IsNull(l.finishstr, '') <> '품절상품 출고지연전환 알림톡 발송완료'"
			sqlStr = sqlStr & " and l.detailidx="& idx &""

			'response.write sqlStr & "<br>"
			rsget.CursorLocation = adUseClient
			rsget.Open sqlStr, dbget, adOpenForwardOnly
			if Not rsget.Eof then
				michulgoidx = rsget("idx")
			end if
			rsget.close

			' 품절출고불가 상품 출고지연전환 알림톡
			if michulgoidx<>"" then
				...
				fullText = fullText & "■ 주문번호 : "& omisend.FOneItem.Forderserial &"" & vbCrLf
				fullText = fullText & "■ 상품명 : "& omisend.FOneItem.FItemName &"" & vbCrLf
				fullText = fullText & "■ 출발예정일 : #{YYYY-MM-DD}" & vbCrLf & vbCrLf
