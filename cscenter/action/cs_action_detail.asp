	...
	<tr valign="top" height="600">
		<td>
			...
			<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
			        ...
			        <td colspan="4">
			            <table width="100%" align="center" border="0" cellpadding="0" cellspacing="0" class="a" >
			            	<tr>
				    		    ...
				    		    <td align="right" >
									<input type="button" class="button" value="고객파일전송관리" class="csbutton" onclick="PopCSfileSend('','','','<%= id %>');" style="width:120px;">
									...
									<% if (IsNumeric(ocsaslist.FOneItem.Forderserial)) then %>
				    		        	<input class="button" type="button" value="정보수정" onclick="javascript:PopCSActionEdit('<%= id %>','editreginfo');" >
				    		        <%
									' Gift카드사용 금액이 없을경우에도 일반 cs수정 팝업으로		' 2018.12.04 한용민
									elseif False and FormatNumber(orefund.FOneItem.Forggiftcardsum*-1,0) < 1 then
									%>
										<input class="button" type="button" value="정보수정" onclick="javascript:PopCSActionEdit('<%= id %>','editrefundinfo');" >
				    		        <% else %>
				    		        	<input class="button" type="button" value="정보수정" onclick="javascript:PopCSGiftCardActionEdit('<%= id %>','editreginfo');" >
				    		    	<% end if %>
				    		    </td>
				    		 </tr>
		    		    </table>
				...
				<% if ccsfile.FresultCount>0 then %>
					<tr bgcolor="#F4F4F4">
						<td bgcolor="<%= adminColor("topbar") %>">첨부파일</td>
						<td colspan="3" bgcolor="#FFFFFF">
		...
		<td width="30%">
			            	...
			            	<tr>
				    		    ...
				    		    <td align="right" >
									...
									%>
										<input class="button" type="button" value="정보수정" onclick="javascript:PopCSActionEdit('<%= id %>','editrefundinfo');" >
									<% else %>
				    		        	<input class="button" type="button" value="정보수정" onclick="javascript:PopCSGiftCardActionEdit('<%= id %>','editrefundinfo');" >
				    		    	<% end if %>
				    		    </td>
				    		</tr>
		    		    </table>
			...
			<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
			    <tr height="25" bgcolor="<%= adminColor("topbar") %>">
			        <td colspan="2">
			            <table width="100%" align="center" border="0" cellpadding="0" cellspacing="0" class="a" >
			            	<tr>
				    		    <td>
				    		    	<img src="/images/icon_star.gif" align="absbottom">&nbsp;<b>고객추가결제 정보</b>
				    		    </td>
				    		    <td align="right" >
									<% if (IsNumeric(ocsaslist.FOneItem.Forderserial)) then %>
				    		        	<input class="button" type="button" value="정보수정" onclick="javascript:PopCSActionEdit('<%= id %>','editrefundinfo');" >
				...
				<tr height="25">
				    <td width="80" bgcolor="<%= adminColor("topbar") %>">관련주문번호</td>
				    <td bgcolor="#FFFFFF">
						<% if (payorderserial <> "") then %>
							<%= payorderserial %>
				...
				<tr height="25">
				    <td width="80" bgcolor="<%= adminColor("topbar") %>">가상계좌</td>
				    <td bgcolor="#FFFFFF">
						<% if (payorderserial <> "") then %>
				...
				<% else %>
				<tr height="25" >
				    <td colspan="2" align="center" bgcolor="#FFFFFF">[고객추가결제 정보가 없습니다.]</td>
				</tr>
				<% end if %>
			</table>
			<!-- 계좌정보 끝-->
			        ...
			        <td colspan="5">
			            <table width="100%" align="center" border="0" cellpadding="0" cellspacing="0" class="a" >
			            	<tr>
				    		    ...
				    		    <td align="right" >
                                    <% if IsExtSendOK then %>
                                    <input class="button" type="button" value="제휴 <%= CHKIIF(IsExtAllFinished, "완료됨", "완료처리") %>" onclick="PopExtCSActionFinish('<%= ocsaslist.FOneItem.Fextsitename %>', '<%= ocsaslist.FOneItem.Forderserial %>', '<%= oExtDivCD %>', '<%= id %>');" <% if (IsExtAllFinished = True) then %>disabled<% end if %> >
                                    <% end if %>
									<% if (IsNumeric(ocsaslist.FOneItem.Forderserial)) then %>
										<% if (divcd = "A004") then %>
											<input class="button" type="button" value="즉시완료" onclick="ifrCSActionFinishDirect('<%= id %>','finishreginfo');" <% if (disableFinishButton = True) then %>disabled<% end if %> >
										<% end if %>
										<input class="button" type="button" value="완료처리" onclick="PopCSActionFinish('<%= id %>','finishreginfo');" <% if (disableFinishButton = True) then %>disabled<% end if %> >
				    		        <%
									' Gift카드사용 금액이 없을경우에도 일반 cs수정 팝업으로		' 2018.12.04 한용민
									elseif False and FormatNumber(orefund.FOneItem.Forggiftcardsum*-1,0) < 1 then
									%>
										...
										<input class="button" type="button" value="완료처리" onclick="PopCSActionFinish('<%= id %>','finishreginfo');" <% if (disableFinishButton = True) then %>disabled<% end if %> >
				    		        <% else %>
				    		        	<input class="button" type="button" value="완료처리" onclick="PopCSGiftCardActionFinish('<%= id %>','finishreginfo');" >
				    		    	<% end if %>
				    		    </td>
				    		 </tr>
		    			</table>
