...
'' IniPay 만 취소만 가능
if (pggubun <> "TS") and (pggubun <> "CH") and ((MID(orefund.FOneItem.FpaygateTid,9,2) <> "NP") and Not(Left(orefund.FOneItem.FpaygateTid,1)="T" and Len(orefund.FOneItem.FpaygateTid)>=20) and Left(orefund.FOneItem.FpaygateTid,3)<>"cns") and (Left(orefund.FOneItem.FpaygateTid,5)<>"KCTEN") and (Left(orefund.FOneItem.FpaygateTid,10)<>"IniTechPG_") AND (Left(orefund.FOneItem.FpaygateTid,10)<>"INIMX_CARD") AND (Left(orefund.FOneItem.FpaygateTid,10)<>"INIMX_ISP_") AND (Left(orefund.FOneItem.FpaygateTid,6)<>"Stdpay") AND orefund.FOneItem.Freturnmethod<>"R400" AND orefund.FOneItem.Freturnmethod<>"R420" and Not IsNumeric(orefund.FOneItem.FpaygateTid) then
    if (IsAutoScript) then
...
if (ResultCode="00") or (ResultCode="0000") or (IsKakaoPay and (resultCode="2001")) then
    ...
    sqlStr = sqlStr & " WHERE ad.orderserial = '" & iorderserial & "'"
    sqlStr = sqlStr & " and masterid="& id &""

    'response.write sqlStr & "<Br>"
    dbget.CursorLocation = adUseClient
    dbget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
    ...
    if (iorderserial<>"") and (ibuyhp<>"") then
        ...
        fullText = fullText & "■ 취소금액 : "& FormatNumber(CancelPrice,0) &""
        failText = "[텐바이텐]주문취소가 완료되었습니다.주문번호 : "& iorderserial &""
        Call SendKakaoCSMsg_LINK("", ibuyhp,"1644-6030","KC-0021",fullText,"SMS","",failText,"","","")
    end if
