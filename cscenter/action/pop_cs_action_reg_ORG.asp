...
FinishActType('<%= request("finishtype") %>');
<% end if %>
...
function selectGubun(value_gubun01, value_gubun02, value_gubun01name, value_gubun02name, name_gubun01, name_gubun02, name_gubun01name, name_gubun02name ,name_frm, targetDiv){
	...
	// 사유에 따라 배송비가 달라질 수 있다.(상품불량X, 단순변심O)
    // 완료시에는 금액 수정 불가
...
function DispCheckedUpcheID(frm) {
    ...
    <% if (divcd="A004") or (divcd="A000") then %> // 반품접수(업체배송), 맞교환출고
...
function CheckReturnMethod(frm){
    ...
    <% if oordermaster.FOneItem.FSiteName<>"10x10" then %>
		...
		<% for i=0 to ocsOrderDetail.FResultCount-1 %>
			...
			<% else %>
				...
				<% if (IsStateFinished) and ((divcd="A010") or (divcd="A011")) and ((ocsOrderDetail.FItemList(i).Fgubun02="CE01") or (ocsOrderDetail.FItemList(i).Fgubun02="CF02")) then %>
						<br><input type="button" class="button" value="불량등록" onClick="popBadItemReg('10<%= Format00(6,ocsOrderDetail.FItemList(i).FItemid) %><%= ocsOrderDetail.FItemList(i).FItemOption %>','<%= ocsOrderDetail.FItemList(i).GetDefaultRegNo(IsRegState) %>');">
        ...
        <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="BABABA">
	    	...
	    	<tr bgcolor="FFFFFF">
	    	    <td colspan="2" height="50">
	    	    	...
	    	    	*마일리지 일부환불이 필요한 경우, 마일리지환원 체크를 해제하고, 마일리지 환불하시면 됩니다.
	    	    </td>
	    	</tr>
	    	<% end if %>
