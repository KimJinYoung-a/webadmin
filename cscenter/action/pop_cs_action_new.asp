...
'			2023.10.23 한용민 수정(상품쿠폰 환원 로직 추가)
'###########################################################
%>
...
dim DepositLogCount
DepositLogCount = 0
if (id <> "") and (mode = "editreginfo") and (ocsaslist.FOneItem.Fdivcd = "A003") then
	DepositLogCount = GetDepositLogCountByAsid(orderserial, id)
end if
...
if (Left(orderserial,1)<>"A") and (oordermaster.FResultCount<1) then
	...
	response.write "	alert('과거 주문내역이거나 해당되는 주문 내역이 없습니다.\n기프트카드 주문내역은 이곳에서 보실수 없습니다.');"
...
' 상품쿠폰
dim oCsItemCoupon
set oCsItemCoupon = New CCSCenterItemCoupon
' 상품쿠폰환급여부
if itemCouponRefundYN="Y" and oordermaster.FOneItem.FUserID <> "" then
	oCsItemCoupon.FPageSize=500
	oCsItemCoupon.FCurrPage=1
	oCsItemCoupon.FRectorderserial = orderserial
	oCsItemCoupon.FRectuserid = oordermaster.FOneItem.FUserID
	oCsItemCoupon.frectcouponGubun = "P"	' 지정인발급
	oCsItemCoupon.GetCSCenterorderItemCoupon
end if
...
if (IsStatusRegister) then
	...
	'// 기본문구 설정
	if InStr("A004,A001,A002,A200,A009,A006,A060,A700,A000,A999", divcd) then
...
set ocsOrderDetail = new CCSASList
	...
	ocsOrderDetail.GetOrderDetailByCsDetailNew
...
if (orderserial<>"") then
	omainpayment.FRectOldOrder = oordermaster.FRectOldOrder
...
IsUpcheConfirmState = (ocsaslist.FOneItem.FCurrState="B006" or ocsaslist.FOneItem.FCurrState="B005")
...
dim oRefCSASList, csorderserial
...
if IsStatusFinished and (ocsaslist.FOneITem.FDeleteyn="N") then
	if (divcd="A004") or (divcd="A010") or (divcd="A008") then
        ...
        if (divcd="A010") and (ocsaslist.FOneItem.Fextsitename = "10x10_cs") then
            ...
            DelFinishedCSInValidMsg = "CS주문[" & csorderserial & "]에서 취소 또는 반품을 잡으세요."
		elseif (oRefCSASList.FResultCount > 0) then
...
dim IsPayOrderCancelYN, PayOrderMasterState
...
if Not IsNull(ocsaslist.FOneItem.Fpayorderserial) and ocsaslist.FOneItem.Fpayorderserial <> "" then
	...
	IsPayOrderCancelYN  = opayordermaster.FOneItem.Fcancelyn
	PayOrderMasterState = opayordermaster.FOneItem.FIpkumDiv
end if

'==============================================================================
dim IsDelNotFinishedCSAvail : IsDelNotFinishedCSAvail = True
dim DelNotFinishedCSInValidMsg : DelNotFinishedCSInValidMsg = ""

if IsStatusEdit and (divcd = "A999") and (IsPayOrderCancelYN = "N") then
	IsDelNotFinishedCSAvail = False
	DelNotFinishedCSInValidMsg = "삭제불가\n\n추가결제주문이 취소이전 상태입니다.\n\n내역을 삭제하려면 \n먼저 관련주문(" & payorderserial & ")을 취소하세요."
end if

dim IsLastReturnBrand : IsLastReturnBrand = False
if IsStatusRegister and ((divcd="A004") or (divcd="A010")) then
    '// 전체반품(반품안된 브랜드가 1개인 상태)인지 체크
    IsLastReturnBrand = GetIsLastReturnBrand(orderserial)
end if
...
var IsPayOrderCancelYN			= "<%= IsPayOrderCancelYN %>";
var PayOrderMasterState			= "<%= PayOrderMasterState %>";
...
var IsLastReturnBrand           = <%= LCase(IsLastReturnBrand) %>;
...
var IsAllCancelProcess			= <%= LCase(ckAll = "on") %>;
...
// 한개의 브랜드만 선택가능한가
// 반품접수(업배), 맞교환출고, 누락재발송, 서비스발송, 기타회수, 출고시유의사항, 업체긴급문의, 회수신청(텐바이텐배송), 업체기타정산, 고객추가결제
var IsOnlyOneBrandAvailable		= <%= LCase(InStr("A004,A000,A001,A002,A200,A006,A060,A010,A700,A999", divcd) > 0) %>;
...
var DepositLogCount			= <%= DepositLogCount %>;

var NotFinisherCancelCSMakeridList = '<%= LCase(NotFinisherCancelCSMakeridList) %>';

var UserLevel = '<%= getUserLevelStrByDate(oordermaster.FOneItem.fUserLevel, left(oordermaster.FOneItem.Fregdate,10)) %>';
...
<script language="javascript" SRC="/cscenter/js/newcsas.js?v=2.4"></script>
...
<!-- 1. 이전 CS 내역                                                        -->
<!-- ====================================================================== -->
<!-- #include virtual="/cscenter/action/include/inc_cs_action_prev_cslist.asp" -->

<!-- ====================================================================== -->
<!-- 2. CS 마스터 정보                                                      -->
<!-- ====================================================================== -->
<!-- #include virtual="/cscenter/action/include/inc_cs_action_master_info.asp" -->

<!-- ====================================================================== -->
<!-- 3. 상품정보                                                            -->
<!-- ====================================================================== -->
<!-- #include virtual="/cscenter/action/include/inc_cs_action_item_list.asp" -->

<!-- ====================================================================== -->
<!-- 4. 다른상품 맞교환 출고 상품정보                                       -->
<!-- ====================================================================== -->
<!-- #include virtual="/cscenter/action/include/inc_cs_action_change_item_list.asp" -->

<!-- ====================================================================== -->
<!-- 3-1. 사은품                                                            -->
<!-- ====================================================================== -->
<!-- #include virtual="/cscenter/action/include/inc_cs_action_gift_info.asp" -->
...
<tr>
    <td bgcolor="#FFFFFF" width="50%" valign="top">
        ...
        <tr height="25">
            <td colspan="6" bgcolor="<%= adminColor("topbar") %>">
            	<img src="/images/icon_star.gif" align="absbottom">
            	&nbsp;<b>취소관련 정보</b>
            </td>
        </tr>
		<!-- #include virtual="/cscenter/action/include/inc_cs_action_cancel_info.asp" -->
		</table>

    	<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="BABABA" style="margin-top:5px;">
    		<tr height="25">
	        ...
	        <!-- #include virtual="/cscenter/action/include/inc_cs_action_add_payment_info.asp" -->
    	</table>
    </td>
    <td bgcolor="#FFFFFF" valign="top" align="left">
        ...
        <!-- #include virtual="/cscenter/action/include/inc_cs_action_refund_info.asp" -->
        </table>
    	...
    	<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="BABABA" style="margin-top:5px;">
	        ...
	        <!-- #include virtual="/cscenter/action/include/inc_cs_action_bonuscoupon_info.asp" -->
    	</table>
		...
		%>
			<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="BABABA" style="margin-top:5px;">
				<tr height="25">
					<td colspan="4" bgcolor="<%= adminColor("topbar") %>">
						<img src="/images/icon_star.gif" align="absbottom">
						&nbsp;<b>상품쿠폰(지정인발급만)관련 정보</b>
					</td>
				</tr>
				<!-- #include virtual="/cscenter/action/include/inc_cs_action_itemcoupon_info.asp" -->
			</table>
		<% end if %>
    	...
    	<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="BABABA" style="margin-top:5px;">
	        ...
	        <!-- #include virtual="/cscenter/action/include/inc_cs_action_upchejungsan_info.asp" -->
    	</table>
    	...
    	<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="BABABA" style="margin-top:5px;">
	        ...
	        <!-- #include virtual="/cscenter/action/include/inc_cs_action_customeraddbeasongpay_info.asp" -->
    	</table>
...
<!-- 6. 버튼                                                                -->
<!-- ====================================================================== -->
<!-- #include virtual="/cscenter/action/include/inc_cs_action_button.asp"   -->
<!-- ====================================================================== -->
<!-- 6. 버튼                                                                -->
<!-- ====================================================================== -->
