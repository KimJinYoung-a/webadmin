...
dim ExistsRegedCSCount, ExistsCustomerAddPayRegedCSCount
ExistsRegedCSCount = oOldcsaslist.FResultCount
ExistsCustomerAddPayRegedCSCount = 0
    ...
    <td>
        ...
        for i = 0 to (oOldcsaslist.FResultCount - 1)
            if (oOldcsaslist.FItemList(i).Fdeleteyn <> "Y") and (oOldcsaslist.FItemList(i).Fdivcd = "A999") then
                '// 고객추가결제 건수
                ExistsCustomerAddPayRegedCSCount = ExistsCustomerAddPayRegedCSCount + 1
            end if
        %>
