...
<tr >
    <td >
		...
		<input type="hidden" name="itemcostCouponNotApplied">
		...
		<input type="hidden" name="buycash">
	...
	<% for i = 0 to ocsOrderDetail.FResultCount - 1 %>
		...
		if (IsItemCanceled or IsOrderCanceled) and (Trim(ocsOrderDetail.FItemList(i).Fgubun01name & "") = "") then
			'취소
			...
			if ((divcd = "A060") or (divcd = "A999")) and (Not IsBeasongPay) then
				'// 취소 상품에 업체긴급문의,고객추가결제 등록 가능
				strhtmldisabled 	= ""
                strhtmlmodify 		= ""
			end if
		...
		%>
			...
			<% else %>
                        ...
                        <% elseif (IsStatusRegister and Not IsItemCanceled and IsCSReturnProcess(divcd)) then %>
                        <input type="button" class="button" value="배송비반품" onClick="CsRegReturnBeasongPayProc(frmaction, <%= ocsOrderDetail.FItemList(i).Forderdetailidx %>);" <% if (ocsOrderDetail.FItemList(i).GetBonusCouponPrice = 0) or (ocsOrderDetail.FItemList(i).Fprevcsreturnfinishno <> 0) or (OrderDetailState <> "7") then %>disabled<% end if %> >
						<% end if %>
			<% end if %>
			...
			<% end if %>
					...
					<td align="right">
						<input type="hidden" name="itemcostCouponNotApplied" value="<%= ocsOrderDetail.FItemList(i).FitemcostCouponNotApplied %>">
					...
					<td align="right">
						...
						<input type="hidden" name="reducedPrice" value="<%= ocsOrderDetail.FItemList(i).FreducedPrice %>">
						<input type="hidden" name="buycash" value="<%= ocsOrderDetail.FItemList(i).Fbuycash %>">
			...
			<% end if %>
						...
						<input type="hidden" id="odlvtype_<%= i%>" name="odlvtype" value="<%= ocsOrderDetail.FItemList(i).Fodlvtype %>">
