...
<% if (IsDisplayItemList and (ocsOrderDetail.FResultCount > 0)) then %>
	...
	<% for i = 0 to ocsOrderDetail.FResultCount - 1 %>
    	...
    	<tr bgcolor="FFFFFF" height="25">
    		...
    		<td>배송비(<% if (IsUpcheBeasong) then %><%= ocsOrderDetail.FItemList(i).FMakerId %><% else %>텐배<% end if %>)<%= ocsOrderDetail.FItemList(i).Fisupchebeasong %></td>
    		<td>
    		...
    		<td align="right">
    		    <input class="text_ro" type="text" name="refundbrandbeasongpay" value="<% if (Not IsStatusRegister) and (ocsOrderDetail.FItemList(i).GetDefaultRegNo(IsStatusRegister) > 0) then %><%= ocsOrderDetail.FItemList(i).GetItemCouponPrice %><% else %>0<% end if %>" size="8" style="text-align:right;background-color:#DDDDFF" readonly>
    		</td>
    		<td align="right">
    		    <input class="text_ro" type="text" name="remainbrandbeasongpay" value="<%= ocsOrderDetail.FItemList(i).GetItemCouponPrice %>" size="8" style="text-align:right" readonly>
    		</td>
    	</tr>
...
<% end if %>
		...
		<input type="hidden" name="refundbeasongpay" value="0">
		<input type="hidden" name="remainbeasongpay" value="0">
    	...
    	<tr bgcolor="FFFFFF" height="25" <% if (Not IsCSReturnProcess(divcd)) then %>style='display:none'<% end if %>>
    		...
    		<td>
        		<input <% if (Not IsPossibleModifyRefundInfo) or (IsStatusRegister and Not IsJupsuProcessAvail) then response.write "disabled" %>  type="checkbox" name="ckreturnpay" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)" <% if (orefund.FOneItem.Frefunddeliverypay <= -4000) then response.write "checked" %> >
        		업체왕복배송비
        		<!-- 추후 출고 배송비 차감으로 변경 -->
        		<br>
        		<input <% if (Not IsPossibleModifyRefundInfo) or (IsStatusRegister and Not IsJupsuProcessAvail) then response.write "disabled" %>  type="checkbox" name="ckreturnpayHalf" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)"  <% if (orefund.FOneItem.Frefunddeliverypay < 0) and (orefund.FOneItem.Frefunddeliverypay > -4000) then response.write "checked" %> >
        		회수배송비
        		<br>
        		<input <% if (Not IsPossibleModifyRefundInfo) or (IsStatusRegister and Not IsJupsuProcessAvail) then response.write "disabled" %>  type="checkbox" name="ckreturnpayZero" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)">
        		차감안함
    		</td>
    		<td align="center"></td>
    		<td align="right"><input class="text_ro" type="text" name="refunddeliverypay" value="<%= orefund.FOneItem.Frefunddeliverypay %>" size="8" style="text-align:right" style="text-align:right" ></td>
    	    <td></td>
    	</tr>
    	<tr bgcolor="FFFFFF" height="25" <% if (Not IsCSReturnProcess(divcd)) then %>style='display:none'<% end if %>>
