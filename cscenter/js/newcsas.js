...
function CheckForItemChanged() {
	...
	var i, remainitemcostsum, addbeasongpaylimit, addbeasongpay, addbeasongpayexists, addbeasongpaybrand;
	var checkedbrand;
	...
	// 취소시 체크
	// ========================================================================
	// if ((IsCSCancelProcess == true) && (frm.orderserial.value == "19042454586")) {
	if (IsCSCancelProcess == true) {
		// 상품취소로 인한 배송비 추가
		if (IsStatusRegister == true) {
			addbeasongpayexists = frm.addbeasongpay.value*1 != 0 ? true : false;
			frm.ckreturnpayHalf.checked = false;
			frm.addbeasongpay.value = 0;
			frm.refunddeliverypay.value = 0;
			frm.requireupche.value = "";
			frm.requiremakerid.value = "";
			frm.csmailsend.disabled = false;
			frm.csmailsend.checked = true;
			addbeasongpaybrand = [];			// 추가배송비 붙는 브랜드
			checkedbrand = [];					// 체크한 브랜드 전체
			if (IsCheckedItemExists(frm, "") == true) {
				// 텐배
				checkedbrand.push('');
				if (IsAddBrandBeasongPayNeed(frm, "") == true) {
					remainitemcostsum = GetOneBrandNotSelectedItemcost(frm, "");
					addbeasongpaylimit = GetUpcheFreeBeasongLimit("");
					addbeasongpay = GetUpcheDeliverPay("");

					if (addbeasongpaybrand.length == 0) {
						if (addbeasongpayexists == false) {
							alert("텐바이텐 배송 상품중 취소 후 나머지상품의 금액이\n" + addbeasongpaylimit + "원 미만(" + remainitemcostsum + "원)이므로\n배송비 " + addbeasongpay + "원이 추가됩니다.\n\n(브랜드별로만 취소 가능합니다.)");
						}
						addbeasongpaybrand.push('');

						// 나머지 브랜드 체크해제
						if (UncheckCheckedItemExceptBrand(frm, "") > 0) {
							if (addbeasongpaybrand[0] == '') {
								alert('현재 텐바이텐 배송 상품의 취소로 추가배송비가 발생하여 다른 브랜드 상품을 선택할 수 없습니다.');
							} else {
								alert('현재 ' + addbeasongpaybrand[0] + ' 배송 상품의 취소로 추가배송비가 발생하여 다른 브랜드 상품을 선택할 수 없습니다.');
							}
						}

						frm.ckreturnpayHalf.checked = true;
						frm.addbeasongpay.value = addbeasongpay*-1;
						frm.refunddeliverypay.value = addbeasongpay*-1;

						// 추가배송비 발생시 무료배송비 취소
						CheckUpcheDeliverPayIfAddBeasongPayExists(frm, "", true);
					}
				} else if ((IsAllCancelProcess == false) && (IsUpcheConfirmItemExist(frm, "") == true)) {
					// 나머지 브랜드 체크해제
					if (UncheckCheckedItemExceptBrand(frm, "") > 0) {
						alert('업체확인중인 상품은 브랜드별로만 취소 가능합니다.');
					}
				}
			}

			for (i = 0; i < arrmakerid.length; i++) {
				// 업배
				if (IsCheckedItemExists(frm, arrmakerid[i]) == true) {
					if (NotFinisherCancelCSMakeridList.indexOf('|' + arrmakerid[i].toLowerCase() + '|') >= 0) {
						alert('완료되지 않은 주문취소 접수건이 있습니다.\n먼저 접수건을 완료처리 하세요.\n\n브랜드 : ' + arrmakerid[i]);
						UncheckCheckedItemOneBrand(frm, arrmakerid[i]);
						continue;
					}

					checkedbrand.push(arrmakerid[i]);
					if (IsAddBrandBeasongPayNeed(frm, arrmakerid[i]) == true) {
						remainitemcostsum = GetOneBrandNotSelectedItemcost(frm, arrmakerid[i]);
						addbeasongpaylimit = GetUpcheFreeBeasongLimit(arrmakerid[i]);
						addbeasongpay = GetUpcheDeliverPay(arrmakerid[i]);

						if (addbeasongpaybrand.length == 0) {
							if (addbeasongpayexists == false) {
								alert(arrmakerid[i] + " 배송 상품중 취소 후 나머지상품의 금액이\n" + addbeasongpaylimit + "원 미만(" + remainitemcostsum + "원)이므로\n배송비 " + addbeasongpay + "원이 추가됩니다.\n\n(브랜드별로만 취소 가능합니다.)");
							}
							addbeasongpaybrand.push(arrmakerid[i]);
							frm.requireupche.value = "Y";
							frm.requiremakerid.value = arrmakerid[i];

							// 나머지 브랜드 체크해제
							if (UncheckCheckedItemExceptBrand(frm, arrmakerid[i]) > 0) {
								if (addbeasongpaybrand[0] == '') {
									alert('현재 텐바이텐 배송 상품의 취소로 추가배송비가 발생하여 다른 브랜드 상품을 선택할 수 없습니다.');
								} else {
									alert('현재 ' + addbeasongpaybrand[0] + ' 배송 상품의 취소로 추가배송비가 발생하여 다른 브랜드 상품을 선택할 수 없습니다.');
							...
							// 추가배송비 발생시 무료배송비 취소
							CheckUpcheDeliverPayIfAddBeasongPayExists(frm, arrmakerid[i], true);
						}
					} else if ((IsAllCancelProcess == false) && (IsUpcheConfirmItemExist(frm, arrmakerid[i]) == true)) {
						frm.requireupche.value = "Y";
						frm.requiremakerid.value = arrmakerid[i];
					...
					if ((frm.requireupche.value == "Y") && (IsUpcheConfirmItemExist(frm, arrmakerid[i]) == true)) {
						// 업체확인중인 경우, 취소접수상태로 남기때문에 안내메일 발송안함
						frm.csmailsend.disabled = true;
						frm.csmailsend.checked = false;
					}
				}
			}

			if ((frm.requireupche.value == "") && (checkedbrand.length == 1)) {
				// 취소되는 브랜드가 한개일 경우, makerid 세팅, 업체 어드민에 오픈
				frm.requireupche.value = "Y";
				frm.requiremakerid.value = checkedbrand[0];
			}
		}
	}
...
//  - 전체취소 : 자동으로 전체선택
//
//  - 부분취소 : 배송비 매출이 있는 배송비 선택되어 있으면 10x10logistics 배송비 자동선택, 아니면 해제
//
// 반품
//
//  - 일부반품 : 배송비 매출이 있는 배송비 선택되어 있으면 10x10logistics 배송비 자동선택, 아니면 해제
//
//  - 전체반품 : 마지막 브랜드이고, 잔여상품 전체선택이고, 10x10logistics 배송비 반품접수 안되어 있으면 자동선택
//
// ============================================================================
function CheckUpcheDeliverPay(frm) {
    ...
    var objdeliver, objitem;
	var obj10x10logistics = undefined;
	var needChk10x10logistics = false;
	var beasongPayExists = false;
    ...
    for (var i = 0; i < frm.orderdetailidx.length; i++) {
		if ((frm.itemid[i].value != "") && (frm.itemid[i].value*1 == 0)) {
			...
			upbeaMakerid = frm.makerid[i].value;
			cancelyn = frm.cancelyn[i].value;
			reducedPrice = frm.reducedPrice[i].value;

			if (cancelyn != "Y") {
				if (upbeaMakerid == '10x10logistics') {
					if (frm.prevcsreturnfinishno[i].value*1 == 0) {
						// 기접수 없으면
						obj10x10logistics = frm.orderdetailidx[i];
					}
				} else {
					beasongPayExists = true;
					if (IsOneBrandAllSelected(frm, upbeaMakerid) == true) {
						objdeliver.checked = true;
						if (reducedPrice*1 > 0) {
							needChk10x10logistics = true;
						} else if (IsLastReturnBrand == true) {
							needChk10x10logistics = true;
						}
					} else {
						objdeliver.checked = false;
	...
	if (obj10x10logistics != undefined) {
		if (beasongPayExists != true) {
			for (var i = 0; i < frm.orderdetailidx.length; i++) {
				if ((frm.itemid[i].value != "") && (frm.itemid[i].value*1 != 0)) {
		...
		obj10x10logistics.checked = needChk10x10logistics;

		if (obj10x10logistics.checked){
			// CS 사유구분 복사
			eval("frm.gubun01_" + obj10x10logistics.value).value = frm.gubun01.value;
			eval("frm.gubun02_" + obj10x10logistics.value).value = frm.gubun02.value;
			eval("frm.gubun01name_" + obj10x10logistics.value).value = frm.gubun01name.value;
			eval("frm.gubun02name_" + obj10x10logistics.value).value = frm.gubun02name.value;
		}else{
			delGubun("gubun01_" + obj10x10logistics.value,"gubun02_" + obj10x10logistics.value,"gubun01name_" + obj10x10logistics.value,"gubun02name_" + obj10x10logistics.value,frm.name,causepop);
		}

		AnCheckClick(obj10x10logistics);
	}
}

// 추가배송비 발생시 무료배송비 취소
function CheckUpcheDeliverPayIfAddBeasongPayExists(frm, makerid, forceCheck) {
    var upbeaMakerid;
	var cancelyn;
    ...
    if (!frm.orderdetailidx) return;
    if (IsCSCancelProcess != true) return;
    ...
    for (var i = 0; i < frm.orderdetailidx.length; i++) {
		if ((frm.itemid[i].value != "") && (frm.itemid[i].value*1 == 0)) {
			...
			if ((cancelyn != "Y") && (makerid == upbeaMakerid)) {
				objdeliver.checked = forceCheck;
...
// ============================================================================
// 업체무료배송, 텐배무료배송상품, 착불배송상품 있는지
// ============================================================================
function IsExistsFreeDeliveryItemOrReceivePayItem(frm, currmakerid) {
	var orderdetailidx, makerid, cancelyn, itemid, odlvtype;
    ...
    for (var i = 0; ; i++) {
		...
		odlvtype 		= document.getElementById("odlvtype_" + i);
		...
		if ((odlvtype.value != "2") && (odlvtype.value != "4") && (odlvtype.value != "7")) { continue; }
		if (cancelyn.value === "Y") { continue; }
		if (parseInt(itemid.value,10) === 0) { continue; }
		if (currmakerid == "") {
			if (odlvtype.value != "4") { continue; }
		} else {
			if (makerid.value != currmakerid) { continue; }
		}
		if ((orderdetailidx.checked === true) && (parseInt(itemno.value) === parseInt(regitemno.value))) { continue; }
...
function CheckBeasongPayCutItemChange(frm) {
	...
	if (frm.add_customeraddmethod == undefined) {
		return;
	}
...
function IsCheckedItemExists(frm, currmakerid) {
	var orderdetailidx, cancelyn, makerid, itemid;
	var odlvtype, isupchebeasong, regitemno;
...
// 현재 브랜드 제외하고 나머지 브랜드 체크해제
function UncheckCheckedItemExceptBrand(frm, currmakerid) {
	var orderdetailidx, cancelyn, makerid, itemid;
	var odlvtype, isupchebeasong, regitemno;
	var uncheckcount = 0;
    ...
    for (var i = 0; ; i++) {
		...
		if (orderdetailidx.checked != true) { continue; }

		if (currmakerid == "") {
			if (parseInt(itemid.value,10) === 0) {
				if (makerid.value == currmakerid) { continue; }
			}
			if (isupchebeasong.value != "Y") { continue; }
		} else {
		...
		orderdetailidx.checked = false;
		AnCheckClick(orderdetailidx);
...
function UncheckCheckedItemOneBrand(frm, currmakerid) {
	var orderdetailidx, cancelyn, makerid, itemid;
	var odlvtype, isupchebeasong, regitemno;
	var uncheckcount = 0;
...
function IsOneBrandAllSelected(frm, targetmakerid) {
    ...
    for (var i = 0; ; i++) {
		...
		if (IsCSReturnProcess) {
			var value_gubun02 = frm.gubun02.value;
			if ((value_gubun02 == "CD01") || (value_gubun02 == "CD06")) {
				// 변심반품인 경우, 포장비환불 없이 왕복배송비 차감
				if (parseInt(itemid.value,10) === 100) { continue; }
			}
		}
...
function GetOneBrandNotSelectedItemcost(frm, makerid) {
    ...
    for (var i = 0; i < frm.orderdetailidx.length; i++) {
    	...
    	if (makerid == "") {
    		if (frm.isupchebeasong[i].value != "Y") {
    			if (e.checked != true) {
    				result = result + (frm.itemcostCouponNotApplied[i].value * frm.itemno[i].value);
    				continue;
    			}

	            if (frm.regitemno[i].value != frm.itemno[i].value) {
    				result = result + (frm.itemcostCouponNotApplied[i].value * (frm.itemno[i].value - frm.regitemno[i].value));
    				continue;
	            }
    		}
    	} else {
...
function CalculateGiftCard(frm) {
	...
	if ((IsStatusFinished != true) && (IsStatusFinishing != true)) {
		if (frm.forcegiftcardreturn.checked) {
	        ...
	        if (Math.abs(prevrefundsubtotalprice) < Math.abs(frm.orggiftcardsum.value*-1)) {
	        	refundgiftcardsum = Math.abs(prevrefundsubtotalprice)*-1;
	        } else {
...
function CalculateDeposit(frm) {
	...
	if ((IsStatusFinished != true) && (IsStatusFinishing != true)) {
		if (frm.forcedepositreturn.checked) {
			// 환원 예치금이 취소금액보다 큰경우
	        if (Math.abs(prevrefundsubtotalprice) < Math.abs(orgdepositsum*-1)) {
	        	refunddepositsum = Math.abs(prevrefundsubtotalprice)*-1;
	        } else {
...
function CalculateSelectedReducedPriceSum(frm) {
	var objdeliver;
	var reducedPriceSUM = 0;
	var reducedPrice = 0;
	var regitemno = 0;
	var itemid;
	...
	for (var i = 0; i < frm.orderdetailidx.length; i++) {
		objdeliver = frm.orderdetailidx[i];
		itemid = frm.itemid[i];
		reducedPrice = frm.reducedPrice[i];
		...
		reducedPriceSUM = reducedPriceSUM + ((reducedPrice.value*1) * (regitemno.value*1));
	}

	return reducedPriceSUM;
}

function CalculateSelectedBuycashSum(frm) {
	var objdeliver;
	var buycashSUM = 0;
	var buycash = 0;
	var regitemno = 0;
	...
	for (var i = 0; i < frm.orderdetailidx.length; i++) {
		...
		buycashSUM = buycashSUM + ((buycash.value*1) * (regitemno.value*1));
	}
...
function CalculateMileage(frm) {
	...
	if ((IsStatusFinished != true) && (IsStatusFinishing != true)) {
		if (frm.forcemileagereturn.checked) {
			// 환원마일리지가 취소금액보다 큰경우
			if (Math.abs(prevrefundsubtotalprice) < Math.abs(frm.orgmileagesum.value*-1)) {
	        	refundmileagesum = Math.abs(prevrefundsubtotalprice)*-1;
			} else {
...
function CalculateUpcheAddPayment(frm) {
    var additempay		= 0;
	var additembuypay	= 0;
	var addbeasongpay   = 0;
	var makerid			= "";
	...
	if ($('#add_customeradditempay').val() == '') {
		$('#add_customeradditempay').val(0);
	}

	if ($('#add_customeradditembuypay').val() == '') {
		$('#add_customeradditembuypay').val(0);
	}

	if ($('#add_customeraddbeasongpay').val() == '') {
		$('#add_customeraddbeasongpay').val(0);
	}

	if (frm.customerpayordertype.length == undefined) {
		gubun = frm.customerpayordertype.value;
	} else {
		if (frm.customerpayordertype[0].checked == true) {
			gubun = 'B';
		} else {
			gubun = 'A';
		}
	...
	var reducedPriceSUM = CalculateSelectedReducedPriceSum(frm);
	var buycashSUM = CalculateSelectedBuycashSum(frm);
	...
	if (gubun == 'B') {
		...
		// 기출고결제
		$('#add_customeradditempay').val(reducedPriceSUM);
		$('#add_customeradditembuypay').val(buycashSUM);
	} else {
		// 에러
	}

	additempay = $('#add_customeradditempay').val()*1;
	additembuypay = $('#add_customeradditembuypay').val()*1;
	addbeasongpay = $('#add_customeraddbeasongpay').val()*1;

	$('#add_customeraddpay_sum').val(additempay + addbeasongpay);

    if (frm.buf_refunddeliverypay!=undefined){
    	if (makerid != "10x10logistics") {
	        frm.buf_refunddeliverypay.value = (additembuypay + addbeasongpay)*1;
    	} else {
	        frm.buf_refunddeliverypay.value = 0;
    	}
    	frm.buf_totupchejungsandeliverypay.value = frm.buf_refunddeliverypay.value*1 + frm.add_upchejungsandeliverypay.value*1;
...
function CheckSetAddBeasongPayment(frm, gubun) {
	if (SaveCheckedItemList(frm) != true) {
		return;
	}

	var makerid = frm.buf_requiremakerid.value;
	var beasongpay = GetUpcheDeliverPayItemChange(makerid);
	var obj = $('#add_customeraddbeasongpay');

	if (gubun == '2') {
		// 왕복배송비
		beasongpay = beasongpay * 2;
	}

	if (gubun == 'E') {
		// 기타
		obj.addClass('text').removeClass('text_ro').removeAttr("readonly").focus().select();
	} else {
		obj.addClass('text_ro').removeClass('text').attr("readonly",true).val(beasongpay);
	}

	CalculateUpcheAddPayment(frm);
}

function CheckSetAddItemPayment(frm) {
	if (frm.customerpayordertype == undefined) { return; }

	if (SaveCheckedItemList(frm) != true) {
		$('#add_customeradditempay').val(0);
		$('#add_customeradditembuypay').val(0);
		frm.customerpayordertype[0].checked = true;
	}
...
function GetNotCheckedUpcheBeasongPayByBrand(frm, makerid) {
	...
	if ((IsCSReturnProcess != true) && (IsCSCancelProcess != true)) {
		return 0;
	}
...
function InsertCheckedUpcheID(frm) {
	...
	if ((IsUpcheReturnState(frm) == true) || (divcd == "A000") || (divcd == "A100") || (divcd == "A700") || (divcd == "A001") || (divcd == "A002") || (divcd == "A200") || (divcd == "A999")) {
		IsUpcheReturn = true;
	} else {
		IsUpcheReturn = false;
	}
...
function IsAddBrandBeasongPayNeed(frm, makerid) {
	...
	// 스탭은 배송비 판매가 0원 매입가는 부과
	// if (UserLevel == 'STAFF') { return false; }
	...
	// 업체무료배송상품 텐배무료배송상품 또는 착불 상품이 있는지
	if (IsExistsFreeDeliveryItemOrReceivePayItem(frm, makerid) == true) {
		return false;
	}

	// 선택안된상품이 30000만원 미만인 경우
	if (GetOneBrandNotSelectedItemcost(frm, makerid) < GetUpcheFreeBeasongLimit(makerid)) {
		return true;
	}
...
// ============================================================================
// 업체확인중인 상품이 있는지 : 있는 경우 브랜드별로만 취소 가능
// ============================================================================
function IsUpcheConfirmItemExist(frm, makerid) {
    if (!frm.orderdetailidx) return false;
    ...
    for (var i = 0; i < frm.orderdetailidx.length; i++) {
		...
		if (frm.orderdetailcurrstate[i].value == '') {
			continue;
		}

		if (frm.orderdetailcurrstate[i].value*1 < 3) {
			continue;
		}
		...
		if ((makerid == '') && (frm.isupchebeasong[i].value == 'N')) { true; }
...
function CsRegProc(frm) {
    if ((IsTicketOrder==true)&&(ticketCancelDisabled==true)){
        ...
        //권한 있는지 check
        if ((IsCsPowerUser != true) && (IsC_ADMIN_AUTH != true)) {
	...
	if (divcd == "A003") {
		...
		if ((frm.returnmethod.value != "R007") && (frm.returnmethod.value != "R910")) {
			alert('무통장 또는 예치금환불만 선택가능합니다.');
			return;
		}
	}
	...
	if (divcd == "A999") {
		if (frm.acctno) {
			if ((frm.add_customeraddpay_sum.value*0 != 0) || (frm.add_customeraddpay_sum.value*1 <= 50)) {
				alert('추가결제 금액을 확인하세요.');
				frm.add_customeraddpay_sum.focus();
				return;
			}

			if (frm.acctno.selectedIndex == 0) {
				alert('입금은행을 선택하세요.');
				frm.acctno.focus();
				return;
			}
		...
		if ((frm.gubun01.value == "C004") && (frm.gubun02.value == "CD01")) {
			...
			if (frm.tmpcopycouponinfo) {
				if (frm.tmpcopycouponinfo.checked == false) {
					if (confirm("\n변심반품이 아닌 경우 보너스쿠폰을 재발급해 드릴 수 있습니다.\n(한장만 재발급 가능합니다.)\n\n쿠폰 재발급하시겠습니까?\n\n\n") == true) {
    ...
    if (frm.add_upchejungsandeliverypay){
        ...
        if (frm.add_upchejungsandeliverypay.value*1!=0){
			...
			frm.requiremakerid.value = frm.buf_requiremakerid.value;
...
function IsRefundInfoOK(frm) {
	...
	/* 환불이 없더라도 체크한다. 2019-04-19, skyer9
	if (frm.returnmethod) {
	...
	*/
	...
	if (frm.orgpercentcouponsum) {
		...
		if ((frm.orgallatsubtractsum.value*1 == 0) && (frm.refundallatsubtractsum.value*1 != 0)) {
			frm.orgallatsubtractsum.value = frm.refundallatsubtractsum.value * 1;
			frm.allatdiscountprice.value = frm.refundallatsubtractsum.value * -1;
			// 기타할인 생겨난 경우 기타할인 입력, 2021-11-04, skyer9
...
function CheckReturnMethod(frm) {
	...
	if (frm.returnmethod.value == "R000") {
		...
		if (frm.refundsubtotalprice.value*1 > 0) {
			if (IsCsPowerUser == true) {
				if (confirm('[관리자권한] 환불금액이 있습니다. 환불없음으로 진행하시겠습니까?') != true) {
    ...
    if (frm.returnmethod.value=="R022") {
        if (pggubun=="NP" || pggubun=="KK" || pggubun=="TS" || pggubun=="CH" || pggubun=="PY") {
            return true;
		} else {
			if (pggubun == '') {
				return true;
	...
	if ((sitename != "10x10") && (sitename != "10x10_cs")) {
	    //외부몰인 경우 외부몰 환불접수만 가능..
	    if ((frm.returnmethod.value != "R050") && (frm.returnmethod.value != "R007")) {
	        alert('외부몰인 경우 외부몰 환불 또는 무통장환불을 선택하세요. \n\n제휴 담당자를 통해 제휴몰에서 취소 환불 처리 합니다.');
	        frm.returnmethod.focus();
	        return false;
	    }
	}
...
function CheckReturnForm(frm) {
	...
	*/
	...
	} else if (frm.returnmethod.value=="R910") {
		if (DepositLogCount > 0) {
            alert('======================================\n예치금은 예치금으로 환불할 수 없습니다.\n======================================\n\n접수내역을 삭제하세요.');
            return false;
		}
	}
...
function CsRegCancelFinishedProc(frm) {
	if (confirm('\n\n완료CS내역(취소,반품,교환)을 삭제하시겠습니까?\n\n')) {
        frm.mode.value = "delfinishedcsas";
        frm.submit();
    }
}

function CsRealDelProc(frm) {
	if (confirm('\n\n\n============ 경고!!!============\n\n데이타베이스에서 실제로 DELETE 합니다.\n삭제하시겠습니까?\n\n')) {
        frm.mode.value = "realdelcsas";
        frm.submit();
    }
}

function CsFinishToJupsu(frm) {
	if (confirm('\n\n완료CS내역 완료이전 전환 하시겠습니까?\n\n')) {
...
function CsRegFinishProc(frm) {
	...
	if (IsStatusFinishing && (divcd == "A007" || ((divcd == "A003") && (frm.returnmethod.value=="R007")))) {
		if (IsAdminLogin || IsC_ADMIN_AUTH) {
...
// 업체처리완료=>업체재확인요청 변경
// ============================================================================
function CsUpcheConfirm2ReConfirmProc(frm) {
    if (confirm('업체재확인요청 상태로 변경 하시겠습니까?')){
        frm.mode.value = "upcheconfirm2reconfirm";
        frm.submit();
    }
}
...
// 삭제CS(완료이전) 복구
// ============================================================================
function CsRestoreDelProc(frm) {
    if (confirm('삭제CS(완료이전) 복구 하시겠습니까?')){
        frm.mode.value = "restoredel";
        frm.submit();
    }
}
...
// 배송비반품
function CsRegReturnBeasongPayProc(frm, orderdetailidx) {
	if ((IsC_ADMIN_AUTH != true) && (IsCsPowerUser != true)) {
		alert('\n\n\n============== 관리자 전용 ==============\n\n\n');
		return;
	}

	var upbeaMakerid, chkidx;

	if (CheckedProductExist(frm, orderdetailidx) == true) {
		alert("배송비반품 불가\n\n체크된 상품이 있는 경우 배송비를 선택반품할 수 없습니다.");
		return;
	}
	...
	for (var i = 0; i < frm.length; i++) {
		...
		if (e.name == "orderdetailidx") {
			if (e.value*1 == orderdetailidx) {
				...
				upbeaMakerid = GetCheckedBeasongpayMaker(frm);
				frm.buf_requiremakerid.value = upbeaMakerid;
				frm.requiremakerid.value = upbeaMakerid;
				frm.requireupche.value = upbeaMakerid == "" ? "N" : "Y";
...
// 선택된 배송비의 브랜드 가져오기
// ============================================================================
function GetCheckedBeasongpayMaker(frm) {
    var objitem;
    var upbeaMakerid;
