...
elseif (mode="ChangeEditItemNoPart") then
	...
	tenbeasongpay = getDefaultBeasongPayByDate(Left(Now, 10))			'// 배송비
	upchebeasongpay = 0
...
elseif (mode="itemChange") then
    ...
    if (errExists) then
	    response.write "<script language='javascript'>alert('에러 : 취소추가상품의 배송상태가 서로 다릅니다.');history.back();</script>"
	    dbget.close()	:	response.End
    end if
	...
	if (copysaleinfo = "Y") then
		...
		'// 판매가(할인가) 동일하면 구매마일리지 복사(2014-04-29, skyer9)
	...
	contents_itemlist	= contents_itemlist & "[" & html2db(itemname) & "] " & html2db(itemoptionname) & " " & CStr(itemnoadd) & "개 추가" & vbCrLf
	...
	contents_itemlist	= contents_itemlist & "[" & html2db(itemname) & "] " & html2db(itemoptionname) & " " & CStr(itemnocancel) & "개 취소" & vbCrLf
...
elseif (mode="orderChange") then
	...
	if (toItemOption = "0000") then
		contents_itemlist	= contents_itemlist & "[" & toItemId & "-" & toItemOption & "] " & toItemName & " " & itemnoadd & "개 출고" & vbCrLf
	else
		contents_itemlist	= contents_itemlist & "[" & toItemId & "-" & toItemOption & "] " & toItemName & "[" & toItemOptionName & "] " & itemnoadd & "개 출고" & vbCrLf
	end if
	...
	if (fromItemOption = "0000") then
		contents_itemlist	= contents_itemlist & "[" & fromItemId & "-" & fromItemOption & "] " & fromItemName & " " & itemnocancel & "개 회수" & vbCrLf
	else
		contents_itemlist	= contents_itemlist & "[" & fromItemId & "-" & fromItemOption & "] " & fromItemName & "[" & fromItemOptionName & "] " & itemnocancel & "개 회수" & vbCrLf
	end If
    ...
    if (isupchebeasong = "Y") then
		...
		response.write "<script>alert('교환출고(상품변경) 접수완료 - 업체배송');</script>"
        ...
        response.write "<script>alert('교환출고(상품변경) 접수 및 회수접수완료 - 텐바이텐 배송');</script>"
    end if
...
elseif (mode="modistate2") then
	...
	call AddCsMemo(orderserial,"1", "", session("ssBctId"), "고객의 요청으로 주문통보 전환")

	response.write "<script>alert('주문통보 전환되었습니다.'); history.back();</script>"
