...
dim ojumun, IsAppExists, IsTempOrderExists, ErrStr, isfailorder
...
elseif (mode="ipkumdivnextstep") then
	...
	elseif (ipkumdiv="1") or (ipkumdiv="0") then
        ...
        IF not(acctdiv="100" or acctdiv="20" or acctdiv="400") THEN
            response.write "<script>alert('결제방법은 신용카드,실시간이체,핸드폰결제 만 선택가능 합니다.');</script>"
            response.write "<script>history.back()</script>"
            dbget.close()	:	response.End
        end if
...
elseif (mode="edithandmadereq") then    
    ...
    If Err.Number = 0 Then
        ...
        sqlStr = sqlStr & " set requiredetail='" & trim(html2db(requiredetail)) & "' where" + VbCrlf    ' 차후 해당필드 사용안함
        sqlStr = sqlStr & " idx=" + CStr(detailidx)
