...
<tr height="25" bgcolor="<%= adminColor("tabletop") %>">
	<td>
		...
		<!-- 정직원 이상 -->
	    <input type="button" class="button" value="수정" onclick="updateqadiv();" <% if Not (session("ssAdminLsn") <= "4") then %>disabled<% end if %>>
	    <% if Not (session("ssAdminLsn") <= "4") then %><font color=gray>(수정불가 : 정직원문의)</font><% end if %>
	</td>
</tr>
</table>
</form>
...
<tr>
	...
	<td bgcolor="#FFFFFF">
        ...
        &nbsp;
        <input type="checkbox" name="delupcheans" value="Y"> 상품정보/업체답변 삭제
	</td>
</tr>
<tr height="25">
	...
	<td bgcolor="#FFFFFF">
		...
		<% if boardqna.results(0).Fmakerid<>"" and not(isnull(boardqna.results(0).Fmakerid)) then %>
			...
			<a href="#" onclick="popSimpleBrandInfo('<%= boardqna.results(0).Fmakerid %>');" onfocus="this.blur();">
				<%= boardqna.results(0).Fmakerid %><%= CHKIIF(boardqna.results(0).Fisupchebeasong = "Y", "<font color='red'>(업배)</font>", "")%>
			</a>
		<% end if %>
	</td>
</tr>
...
<tr>
    <td width="90" align="center" bgcolor="#FFFFFF" height="30">담당자</td>
	<td colspan="3" bgcolor="#FFFFFF">
        ...
        <input type="button" class="button" value="변경" onClick="jsChangeChargeID()">
	</td>
</tr>
