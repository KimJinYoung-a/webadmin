...
<!-- #include virtual="/lib/classes/board/_myqnacls.asp" -->
<%
...
function popMyQNA_IMAGE(idx) {
    var window_width = 1200;
    var window_height = 800;
	var popwin = window.open("popMyQNA_IMAGE.asp?idx=" + idx,"popMyQNA_IMAGE","width=" + window_width + " height=" + window_height + " left=50 top=50 scrollbars=yes resizable=yes status=yes");
	popwin.focus();
}
...
<tr>
	...
	<td width="570" bgcolor="#FFFFFF">
	    ...
	    &nbsp;&nbsp;
	    [ <font color="<%= getUserLevelColor(boardqna.results(i).fUserLevel) %>"><b><%= getUserLevelStr(boardqna.results(i).fUserLevel) %></b></font> ]
	    <%
...
<tr>
	...
	<td colspan="3" bgcolor="#FFFFFF" height="25">
		<% if boardqna.results(0).FattachFile <> "" then %>
			<a href="javascript:popMyQNA_IMAGE(<%= boardqna.results(0).id %>)"><img id="idAttachFile" src="<%= uploadUrl %><%= boardqna.results(0).FattachFile %>" border="0"></a>
		<% end if %>
		<% if boardqna.results(0).FattachFile2 <> "" then %>
			<a href="javascript:popMyQNA_IMAGE(<%= boardqna.results(0).id %>)"><img id="idAttachFile2" src="<%= uploadUrl %><%= boardqna.results(0).FattachFile2 %>" border="0"></a>
		<% end if %>
	</td>
</tr>
<tr>
