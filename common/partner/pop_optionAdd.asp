...
function saveItemOptionAdd(){
    ...
    if (GetByteLength(frm.optionTypename.value)>32){
       alert('옵션구분명은 32byte (한글 16자, 영문 32자) 이내로 입력해주세요'+frm.optionTypename.value); 
       frm.optionTypename.focus();
       return;
     } 
...
<% end if %> 
 			<div class="tPad15 ct"> 
          <% if optAddType<>"D" then %>
          <input type="button" value="추가한 옵션 저장" name="btnoptsave" onclick="saveItemOptionAdd();" class="btn3 btnRd"/>
          <% else %>
          <input type="button" value="추가한 옵션 저장" name="btnoptsave" onclick="saveDoubleOptionAdd();" class="btn3 btnRd"/>
          <% end if %> 
			</div>  
