...
if itemid = "" then
	response.write "<script type='text/javascript'>alert('상품코드가 없습니다'); history.back();</script>"
	dbget.close()	:	response.End
end if

''2014/03/18 추가
if (limityn <> "Y") and (limityn <> "N") then
	response.write "<script type='text/javascript'>alert('파라메터 오류 limityn'); history.back();</script>"
	dbget.close()	:	response.End
end if

if (sellyn <> "Y") and (sellyn <> "N") and (sellyn <> "S") then
	response.write "<script type='text/javascript'>alert('파라메터 오류 sellyn'); history.back();</script>"
	dbget.close()	:	response.End
end if

if (isusing <> "Y") and (isusing <> "N") then
	response.write "<script type='text/javascript'>alert('파라메터 오류 isusing'); history.back();</script>"
	dbget.close()	:	response.End
end if
''2014/03/18
...
if (orgisusing="N") then
    response.write "<script type='text/javascript'>alert('판매 중지된 상품입니다. 수정불가'); history.back();</script>"
    dbget.close()	:	response.End
end if
