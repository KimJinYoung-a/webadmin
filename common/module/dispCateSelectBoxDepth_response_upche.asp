...
<%
	...
	for i=1 to depth
		...
		sqlStr = sqlStr & " WHERE depth = '" & i & "'  and catecode <> 123 and useyn ='Y' "
		if i>1 then
		...
		if Not(rsget.EOF or rsget.BOF) then
           vBody = vBody & "<span class=""rMar10"">"
			vBody = vBody & "<select name=""disp" & i & """ class=""formSlt"" onchange=""chgDispCate(this.value,"&maxdepth&")"" title="""&i&"차 카테고리 선택"">" & vbCrLf
