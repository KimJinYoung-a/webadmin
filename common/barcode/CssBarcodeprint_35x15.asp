...
<%
'####################################################
' Description : 바코드 출력 35X15
' History : 2023.09.08 한용민 생성
'####################################################
%>
			...
			%>
						<div class="labelItem">
							<div class="barcodeDesc">
								...
								<div class="prdoptionname">
									<% if isforeignprint="Y" then %>
										<%= chkIIF(prdoptionname_foreign(cksel(iloop))<>"","["&prdoptionname_foreign(cksel(iloop))&"]","&nbsp;") %>
									<% else %>
										<%= chkIIF(prdoptionname(cksel(iloop))<>"","["&prdoptionname(cksel(iloop))&"]","&nbsp;") %>
									<% end if %>
								</div>
								<div class="itemPrice">
									<% if isforeignprint="Y" then %>
							...
							<script type="text/javascript">
								...
								if barcodetype="T" then
								%>
									$("#label<%=chkCount%>").barcode("<%= prdcode(cksel(iloop)) %>","code128",{barWidth:1,barHeight:10,showHRI:false,output:"svg"});
								<% else %>
									$("#label<%=chkCount%>").barcode("<%= generalbarcode(cksel(iloop)) %>","code128",{barWidth:1,barHeight:10,showHRI:false,output:"svg"});
								<% end if %>
							</script>
						</div>
			<%
