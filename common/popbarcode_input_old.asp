...
if Len(itembarcode)=12 and left(itembarcode,2)="10" then
...
function InputBarcode(comp){
	...
	if ((frm.itemgubun.value!="10")&&(frm.itemgubun.value!="90")){
...
function GetOnLoad(){
	...
	if (document.frmbar.publicbar_<%= itemoption %>.value.length>1){
		...
		<td colspan="3">
			<table width="100%" align="center" cellpadding="0" cellspacing="0" class="a">
				<tr>
				    ...
				    <td align="right">
						상품코드: <input type="text" class="text" name="itembarcode" value="<%= itembarcode %>" size=14 maxlength=14 AUTOCOMPLETE="off" onKeyPress="if (event.keyCode == 13){ Research(frmbar); return false;}">
...
<% if oitembar.FResultCount>0 then %>
    <tr bgcolor="#FFFFFF">
    	...
    	<td colspan="2"><%= oitembar.FItemList(0).Fmakerid %></td>
    </tr>
    <tr bgcolor="#FFFFFF">
	...
	<form name="frmitemrackcode" method=post  action="barcode_input_process.asp">
    ...
    <tr bgcolor="#FFFFFF">
		<td bgcolor="<%= adminColor("tabletop") %>">상품랙코드(4자리)</td>
    	<td colspan="2">
    ...
    <tr bgcolor="#FFFFFF">
    	...
    	<td colspan="2"><img src="<%= oitembar.FItemList(0).FImageList %>" width="100" height="100"></td>
    </tr>
...
<% else %>
	<tr bgcolor="#FFFFFF">
    	<td colspan="3" align="center">
    		검색결과가 없습니다<br>
    		현재 10코드(온라인등록상품)만 등록이 가능합니다.<br>
    		90코드의 경우 오프상품관리를 이용하세요.
    	</td>
    </tr>
<% end if %>
...
<input type="hidden" name="confirmbarcode" value="">
</form>
