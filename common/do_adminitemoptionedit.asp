...
' Description : 상품수정
' Hieditor : 서동석 생성
'            2023.03.10 한용민 수정(옵션 삭제시 주문서 체크로직 누락으로 인한 추가)
'###########################################################
%>
...
dim TypeCnt, OptCnt, ordersheetExistsCount
 dim sRetValue
...
if (mode="editOptionMultiple") then
    ...
    for i=1 to OptCnt
        ...
        sqlStr = sqlStr + " ,optaddprice=round("& optaddprice &",0)"
        sqlStr = sqlStr + " ,optaddBuyprice=round("& optaddBuyprice &",0)"
        sqlStr = sqlStr + " where itemid=" & CStr(itemid) &VbCRLF
        sqlStr = sqlStr + " and TypeSeq=" & CStr(TypeSeq) &VbCRLF
        sqlStr = sqlStr + " and KindSeq='" & CStr(KindSeq) & "'" &VbCRLF
        sqlStr = sqlStr + " and (" &VbCRLF
        sqlStr = sqlStr + "     (optionKindName<>'" + html2Db(optionName) + "')" &VbCRLF
        sqlStr = sqlStr + "     or (optaddprice<>round(" & optaddprice & ",0))" &VbCRLF
        sqlStr = sqlStr + "     or (optaddBuyprice<>round(" & optaddBuyprice & ",0))" &VbCRLF
        sqlStr = sqlStr + " )"
...
if (mode = "deleteoption") then
	...
	if (Not IsUpchebeasong) then
    	...
    	if ErrStr="" then
    		sqlStr = "select top 1 * from "
    		sqlStr = sqlStr + " [db_log].[dbo].tbl_old_order_detail_2003 d with (nolock)"
    		sqlStr = sqlStr + " where d.itemid=" + CStr(itemid)
    		sqlStr = sqlStr + " and d.itemoption='" + Trim(itemoption) + "'"
        ...
        ordersheetExistsCount=0
    	if ErrStr="" then
            sqlStr = "select count(m.idx) as ordersheetExistsCount"
            ...
            sqlStr = sqlStr & " where m.deldt is null and d.deldt is null"
            sqlStr = sqlStr & " and d.itemgubun='10'"
            sqlStr = sqlStr & " and d.itemid="& trim(itemid) &""
            sqlStr = sqlStr & " and d.itemoption='"& trim(itemoption) &"'"

            'response.write sqlStr &"<br>"
    		rsget.Open sqlStr, dbget, 1
    		if Not rsget.Eof then
                ordersheetExistsCount = rsget(0)
    		end if
    		rsget.close

            if ordersheetExistsCount>0 then
    			ErrStr = "삭제하려는 옵션으로 주문서 입출 내역이 있습니다. 삭제하실 수 없습니다. - 관리자 문의요망"
            end if
    	end if
	end if
    ...
    if (sRetValue="Y") then
        ...
        sqlStr = sqlStr & " select s.itemid from db_summary.dbo.tbl_current_shopstock_summary s "&vbCRLF
        sqlStr = sqlStr & " where s.itemgubun='10' and s.itemid = "& CStr(itemid) &" and s.itemoption ='" + Trim(itemoption) + "' "&vbCRLF
        sqlStr = sqlStr & " and s.shopid in (select u.userid from db_shop.dbo.tbl_shop_user u where u.shopdiv in (1,3) and u.isusing='Y' and LEFT(u.userid,11)<>'streetshop9')"&vbCRLF  '' 직영/사맹,사용,마케팅용 제외매장
        sqlStr = sqlStr & " and s.realstockno<>0"&vbCRLF                      ''실사재고<>0
        sqlStr = sqlStr & " and s.lastupdate>dateadd(m,-6,getdate())"&vbCRLF  ''6개월이상 변동이 없으면 재낌.
        sqlStr = sqlStr & " )  select 'Y'   "
        sqlStr = sqlStr & " else select 'N' "
	...
	if (ErrStr<>"") then
		...
		sqlStr = "delete from [db_item].[dbo].tbl_item_option_stock where" & VbCrlf
		sqlStr = sqlStr & " itemgubun='10'" & VbCrlf
...
if (mode = "deleteMultipleOption") then
 ...
 ''오푸상품등록 내역(업체+텐배 포함) //2016.05.19 정윤정 추가(이문재 이사님 요청) 
    ...
    if (sRetValue="Y") then
        ''재고에 내역이 있는경우로 함더 체크
        sRetValue = ""
        sqlStr = " if exists(select itemid from db_summary.dbo.tbl_current_shopstock_summary s where s.itemgubun='10' and s.itemid = "& CStr(itemid) 
            if (TypeSeq=1) then
                sqlStr = sqlStr & " and LEFT(s.itemoption,2)='Z" & CStr(KindSeq) & "'"
            elseif (TypeSeq=2) then
            	sqlStr = sqlStr & " and LEFT(s.itemoption,1)='Z'"
            	sqlStr = sqlStr & " and LEFT(RIGHT(s.itemoption,3),1)='" & CStr(KindSeq) & "'"
            elseif (TypeSeq=3) then
            	    sqlStr = sqlStr & " and LEFT(is.temoption,1)='Z'"
            	    sqlStr = sqlStr & " and RIGHT(s.itemoption,1)='" & CStr(KindSeq) & "'"
           end if
    ...
    for i = 0 to ubound(arritemoption)
        if (Trim(arritemoption(i)) <> "") then
            ...
            if (found = false) then
                ...
                sqlStr = sqlStr + " values(" + CStr(itemid) + ", '" + CStr(arritemoption(i)) + "', '" + html2db(optionTypename) + "', '" + CStr(html2db(arritemoptionname(i))) + "', 'Y', 'Y', '" + itemLimitYn + "', 0, 0) "
...
function checkIsUpcheItemEditValid(itemid)
    ...
    if (C_IS_Maker_Upche) and (session("ssBctId")<>"") then
        sqlStr = " select top 1 itemid from db_item.dbo.tbl_item with (nolock)" &VBCRLF
