...
<table width="100%" border="0" cellspacing="1" cellpadding="3" bgcolor="#CCCCCC" class="a">
	...
	<tr bgcolor="#FFFFFF" height="25">
		...
		<td>
		    ...
		    <input type="button" class="button" value="판매가수정" onclick="javascript:EditDetail(<%= ojumunDetail.FJumunDetail.Fdetailidx %>,'itemcost',frm.itemcost)">
		    <% end if %>
        </td>
	</tr>
	<tr bgcolor="#FFFFFF" height="25">
	...
	<tr bgcolor="#FFFFFF" height="25">
		...
		<td>
			...
			<% if (FALSE) then ''사용중지 %>
			<input type="checkbox" name="applyallitem" value="Y"> 전상품적용
			<% end if %>
		</td>
