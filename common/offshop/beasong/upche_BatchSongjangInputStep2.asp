...
''rw ioneLine
    ...
    dim iDetailidx, iorderno, iSongjangDiv, iSongjangNo
    ...
    ''길이 Check
    if Len(iDetailidx)<2 then Exit Function
    if Len(iorderno)<>16 then Exit Function
    ...
    if Not IsNumeric(iorderno) then Exit Function
    ...
    iValidArr(oArrLen+1).Forderno = iorderno

    ''코코로박스 자꾸 이노지스택배로 등록함.. 강제로 CJ로 변경.
    if (session("ssBctId")="COCOROBOX") or (session("ssBctId")="COCOROBOXdeco") then
        if (iSongjangDiv="30") then iSongjangDiv="4"
    end if
...
<form name="frmSv" method="post" action="/common/offshop/beasong/upche_beasong_Process.asp">
...
<% for i=0 to UBound(ValidArr) %>
    ...
    <input type="hidden" name="ordernoArr" value="<%= ValidArr(i).Forderno %>">
...
<tr bgcolor="#FFFFFF">
    <td align="center"><input type="button" value="송장입력진행" onClick="regSongJangProc(frmSv)" class="button"></td>
</tr>
</table>
<% end if %>
