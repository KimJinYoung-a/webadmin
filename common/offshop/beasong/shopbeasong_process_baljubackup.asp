...
dim i , orderno , itemgubunarr ,itemoptionarr, itemidarr, mode , sql , shopidarr, songjangdiv, workgroup
dim buyname , buyphone1 ,buyphone2 ,buyphone3 ,buyhp1 ,buyhp2 ,buyhp3 ,reqname, ReBaljuOrderNoArr
...
dim TotAssignedRow, AssignedRow, FailRow ,ordernoArr, oedit, dummiordernoarr, iid, obaljudate
...
elseif mode="beasonginput" then
	...
	dummiordernoarr = replace(left(trim(ordernoarr),len(trim(ordernoarr))-1),",","','")
    ...
    sql = sql & " and d.currstate<2" & vbcrlf
    sql = sql & " and m.orderno in ('"& dummiordernoarr &"')" & vbcrlf
    sql = sql & " and d.itemno < 0 " & vbcrlf
	...
	sql = " select orderno from [db_shop].[dbo].tbl_offline_customer_baljudetail" & vbcrlf
	sql = sql & " where orderno in ('"& dummiordernoarr &"')" & vbcrlf

	'response.write sql & "<br>"
	rsget.Open sql,dbget,1
	if Not rsget.Eof then
		do until rsget.eof
			if (ReBaljuOrderNoArr = "") then
				ReBaljuOrderNoArr = rsget("orderno")
			else
				ReBaljuOrderNoArr = ReBaljuOrderNoArr + "," + rsget("orderno")
			end if
			rsget.movenext
		loop
	end if
	...
	if (ReBaljuOrderNoArr <> "") then
		response.write "<script type='text/javascript'>"
		response.write "	alert('이미 출고지시된 주문입니다. \n\n이미 출고지시한 주문(" & ReBaljuOrderNoArr & ")건은 중복 출고지시 할 수 없습니다.');"
		response.write "	history.back();"
		response.write "</script>"
		response.End
	end if

	sql = " select (count(id) + 1) as differencekey" & vbcrlf
	sql = sql & " from [db_shop].[dbo].tbl_offline_customer_baljumaster" & vbcrlf
	sql = sql & " where convert(varchar(10),baljudate,21)=convert(varchar(10),getdate(),21)" & vbcrlf
	...
	' 출고지시 마스터
	sql = "insert into [db_shop].[dbo].tbl_offline_customer_baljumaster(" & vbcrlf
	sql = sql & " baljudate,differencekey,workgroup, songjangdiv, baljutype, extSiteName, isGiftPojang) values (" & vbcrlf
	sql = sql & " getdate()," & CStr(differencekey) & ",'" & workgroup & "','" & songjangdiv & "','', '10X10SHOP', '')" & vbcrlf

	'response.write sql &"<br>"
	dbget.execute sql

	sql = "select top 1 id, convert(varchar(19),baljudate,21) as baljudate from [db_shop].[dbo].tbl_offline_customer_baljumaster order by id desc" & vbcrlf
	...
	for i = 0 to ubound(ordernoarr) - 1
		...
		sql = "insert into [db_shop].[dbo].tbl_offline_customer_baljudetail(" & vbcrlf
		sql = sql & " baljuid,orderno,sitename) values (" & vbcrlf
		sql = sql & " " & CStr(iid) & "" & vbcrlf
		sql = sql & " ,'" & trim(ordernoarr(i)) & "'" & vbcrlf
		sql = sql & " ,'10X10SHOP'" & vbcrlf
		sql = sql & " )" & vbcrlf
	...
	sql = "update bd set bd.IsOnlyTenBeaSong = 'Y'" & vbcrlf
	sql = sql & " from [db_shop].[dbo].tbl_offline_customer_baljudetail bd" & vbcrlf
	sql = sql & " join db_shop.dbo.tbl_shopbeasong_order_master m" & vbcrlf
	sql = sql & " 	on bd.orderno = m.orderno" & vbcrlf
	sql = sql & " 	and m.cancelyn='N'" & vbcrlf
	sql = sql & " join db_shop.dbo.tbl_shopbeasong_order_detail d" & vbcrlf
	sql = sql & " 	on m.masteridx = d.masteridx" & vbcrlf
	...
	sql = sql & " where bd.baljuid = "& iid &"" & vbcrlf

	'response.write sql &"<br>"
	dbget.execute sql

    sql = "update [db_shop].[dbo].tbl_offline_customer_baljumaster" & vbcrlf
    sql = sql & " set songjanginputed='Y'" & vbcrlf
    sql = sql & " where id=" + CStr(iid)

	'response.write sql &"<br>"
	dbget.execute sql

	sql = "update d set d.songjangdiv = '"& songjangdiv &"'" & vbcrlf
	sql = sql & " from [db_shop].[dbo].tbl_offline_customer_baljudetail bd" & vbcrlf
	sql = sql & " join db_shop.dbo.tbl_shopbeasong_order_master m" & vbcrlf
	sql = sql & " 	on bd.orderno = m.orderno" & vbcrlf
	sql = sql & " 	and m.cancelyn='N'" & vbcrlf
	...
	''출고지시 아이템 목록
	sql = " insert into [db_temp].[dbo].tbl_baljuitem_offline_customer(" & vbcrlf
	sql = sql & " baljuid,itemgubun,itemid,itemoption,rackcode,makerid,itemname,itemoptionname" & vbcrlf
	sql = sql & " ,orgsellprice,baljuno,ipgono,smallimage,listimage,itemrackcode" & vbcrlf
	sql = sql & " )" & vbcrlf
	sql = sql & " 	select " + CStr(iid) + ",bd.itemgubun,bd.itemid,bd.itemoption,0,bd.makerid,d.itemname, d.itemoptionname" & vbcrlf
	sql = sql & " 	,0, sum(bd.itemno),0,'','',''" & vbcrlf
	sql = sql & " 	from [db_shop].[dbo].tbl_offline_customer_baljudetail cd" & vbcrlf
	sql = sql & " 	join db_shop.dbo.tbl_shopbeasong_order_master bm" & vbcrlf
	sql = sql & " 		on cd.orderno = bm.orderno" & vbcrlf
	sql = sql & " 		and bm.cancelyn='N'" & vbcrlf
	sql = sql & " 	join db_shop.dbo.tbl_shopbeasong_order_detail bd" & vbcrlf
	sql = sql & " 		on bm.masteridx = bd.masteridx" & vbcrlf
	sql = sql & " 		and bd.odlvType = '1'" & vbcrlf
	sql = sql & " 		and bd.isupchebeasong='N'" & vbcrlf
	sql = sql & " 		and bd.cancelyn='N'" & vbcrlf
	sql = sql & " 	left join db_shop.dbo.tbl_shopjumun_detail d" & vbcrlf
	sql = sql & " 		on bd.orderno = d.orderno" & vbcrlf
	sql = sql & " 		and d.cancelyn='N'" & vbcrlf
	sql = sql & " 		and bd.itemgubun = d.itemgubun" & vbcrlf
	sql = sql & " 		and bd.itemid = d.itemid" & vbcrlf
	sql = sql & " 		and bd.itemoption = d.itemoption" & vbcrlf
	sql = sql & " 	where cd.baljuid = "& iid &"" & vbcrlf
	sql = sql & " 	group by bd.itemgubun,bd.itemid,bd.itemoption,bd.makerid,d.itemname, d.itemoptionname" & vbcrlf

	'response.write sql &"<br>"
	dbget.execute sql

	sql = "update ti" & vbcrlf
	sql = sql & " set ti.rackcode = IsNULL(c.prtidx,0)" & vbcrlf
	sql = sql & " ,ti.orgsellprice = ii.orgsellprice" & vbcrlf
	sql = sql & " ,ti.itemrackcode = replace(isnull(ii.offitemrackcode,''),'','9999')" & vbcrlf
	sql = sql & " ,ti.smallimage = i.smallimage" & vbcrlf
	sql = sql & " ,ti.listimage = i.listimage" & vbcrlf
	sql = sql & " from [db_temp].[dbo].tbl_baljuitem_offline_customer ti" & vbcrlf
	sql = sql & " join db_shop.dbo.tbl_shop_item ii" & vbcrlf
	sql = sql & " 	on ti.itemgubun = ii.itemgubun" & vbcrlf
	sql = sql & " 	and ti.itemid = ii.shopitemid" & vbcrlf
	sql = sql & " 	and ti.itemoption = ii.itemoption" & vbcrlf
	sql = sql & " left join db_item.dbo.tbl_item i" & vbcrlf
	sql = sql & " 	on ti.itemgubun='10'" & vbcrlf
	sql = sql & " 	and ti.itemid = i.itemid" & vbcrlf
	sql = sql & " left join db_user.dbo.tbl_user_c c" & vbcrlf
	sql = sql & " 	on ti.makerid = c.userid" & vbcrlf
	sql = sql & " where ti.baljuid = "& iid &"" & vbcrlf
	...
	sql = sql & " and orderno in ('"& dummiordernoarr &"')"
	...
	sql = sql & " 	and dd.orderno in ('"& dummiordernoarr &"')" & vbcrlf
	sql = sql & " 	group by dd.masteridx" & vbcrlf
	sql = sql & " ) as t" & vbcrlf
	sql = sql & " 	on m.masteridx = t.masteridx" & vbcrlf
	sql = sql & " where m.cancelyn='N'" & vbcrlf
	sql = sql & " and m.ipkumdiv < 5" & vbcrlf
	sql = sql & " and m.orderno in ('"& dummiordernoarr &"')"
