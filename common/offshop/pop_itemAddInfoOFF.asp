...
dim i, PriceDiffExists , actionURL , saleflg
	...
	if shopid = "" then
		response.write "<script type='text/javascript'>alert('샵ID 가 없습니다'); self.close();</script>"
		response.end
	end if
...
<tr height="40" valign="bottom">
    <td align="left">
    	※샵(<%=shopid%>)과 계약된 상품만 노출 되며, 매입가 & 샵공급가 설정이 없는경우, 해당 샵의 계약 기본마진 으로 계산됩니다.
    </td>
    <td align="right">
...
<% if oitem.FresultCount > 0 then %>
	...
	<td>
		...
		<input type="hidden" name="chkshopsuplycash" value="-1">
