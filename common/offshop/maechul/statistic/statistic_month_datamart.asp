...
Set cStatistic = New cStaticdatamart_list
	...
	cStatistic.fStatistic_monthlist_datamart()
...
<tr bgcolor="#FFFFFF">
	...
	<td align="center" >
		[<a href="javascript:detailStatistic('<%=Year(cStatistic.fitemlist(i).FMinDate)%>','<%=Month(cStatistic.fitemlist(i).FMinDate)%>','<%=Day(cStatistic.fitemlist(i).FMinDate)%>','<%=Year(cStatistic.fitemlist(i).FMaxDate)%>','<%=Month(cStatistic.fitemlist(i).FMaxDate)%>','<%=Day(cStatistic.fitemlist(i).FMaxDate)%>','<%= shopid %>','<%= datefg %>','<%= offgubun %>','<%= oldlist %>')">상세</a>]
	</td>
</tr>
<%
