...
<%
'###########################################################
' Description :  오프라인 매출통계 주별 실시간
' History : 2012.11.02 한용민 생성
'###########################################################
%>
...
<tr bgcolor="#FFFFFF">
	...
	<td align="center"><%= Year(cStatistic.fitemlist(i).FMinDate) %> - <%= cStatistic.fitemlist(i).FWeek %>주</td>
	...
	<td align="center" >
		[<a href="javascript:detailStatistic('<%=Year(cStatistic.fitemlist(i).FMinDate)%>','<%=Month(cStatistic.fitemlist(i).FMinDate)%>','<%=Day(cStatistic.fitemlist(i).FMinDate)%>','<%=Year(cStatistic.fitemlist(i).FMaxDate)%>','<%=Month(cStatistic.fitemlist(i).FMaxDate)%>','<%=Day(cStatistic.fitemlist(i).FMaxDate)%>','<%= shopid %>','<%= datefg %>','<%= offgubun %>')">
		상세</a>]
	</td>
</tr>
<%
