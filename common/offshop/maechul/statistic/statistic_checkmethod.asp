...
<%
'###########################################################
' Description :  오프라인 매출통계 매출집계-결제방식별 실시간
...
dim vTot_spendmilecnt, vTot_TenGiftCardPaycount, vTot_giftcardPaycnt, vTot_cardcnt, vTot_cashcnt, inc3pl
dim vTot_extPaySum, vTot_extPayCnt
...
%>
	...
	<td><%= FormatNumber(cStatistic.FItemList(i).fextPaycnt,0) %></td>
...
<%
	...
	vTot_extPaycnt		= vTot_extPaycnt + CLng(cStatistic.FItemList(i).fextPaycnt)
Next
%>
	<tr bgcolor="<%= adminColor("tabletop") %>" align="center">
	<td colspan="2">합계</td>
	...
	<td style="padding-right:5px;"><%= FormatNumber(vTot_extPaycnt,0) %></td>
