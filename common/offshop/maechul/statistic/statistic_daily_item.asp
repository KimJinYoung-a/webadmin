...
<%
'###########################################################
' Description :  오프라인 매출통계 [OFF]매출통계-실시간>>상품별매출-일별
...
dim vTot_itemno, vTot_Iorgsellprice, vTot_sellprice, vTot_realsellprice, vTot_suplyprice, vTot_MaechulProfit
dim vTot_MaechulProfitPer, offgubun, reload, inc3pl
...
function popitemdetail(yyyy1,mm1,dd1,shopid, purchasetype, makerid, datefg, offgubun){
	var popitemdetail = window.open('/admin/offshop/todayselldetail.asp?yyyy1='+yyyy1+'&mm1='+mm1+'&dd1='+dd1+'&yyyy2='+yyyy1+'&mm2='+mm1+'&dd2='+dd1+'&shopid='+shopid+'&purchasetype='+purchasetype+'&makerid='+makerid+'&datefg='+datefg+'&offgubun='+offgubun+'&inc3pl=<%=inc3pl%>&menupos=<%= menupos %>','popitemdetail','width=1024,height=768,scrollbars=yes,resizable=yes');
	popitemdetail.focus();
}
...
<tr bgcolor="#FFFFFF">
	...
	<td align="center" >
		[<a href="javascript:popitemdetail('<%= left(cStatistic.FItemList(i).FRegdate,4) %>','<%= mid(cStatistic.FItemList(i).FRegdate,6,2) %>','<%= right(cStatistic.FItemList(i).FRegdate,2) %>','<%= Shopid %>','<%= vPurchaseType %>','<%= makerid %>','<%= datefg %>','<%= offgubun %>');">상세</a>]
	</td>
</tr>
<%
