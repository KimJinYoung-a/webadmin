...
<%
Dim i, cStatistic, shopid, datefg,yyyy1,mm1,dd1,yyyy2,mm2,dd2 ,BanPum ,fromDate ,toDate, offgubun, reload,oldlist
dim vTot_ordercnt, vTot_orgsellprice, vTot_totsale, vTot_possale, vTot_realSum, vTot_bonuscouponprice, vTot_TotalSum, vTot_Maechul,vTot_profit
dim inc3pl
...
Set cStatistic = New cStaticdatamart_list
	...
	cStatistic.fStatistic_shop_datamart()
...
{
	var detailStatistic = window.open("/common/offshop/maechul/statistic/statistic_daily_datamart.asp?yyyy1="+yyyy1+"&mm1="+mm1+"&dd1="+dd1+"&yyyy2="+yyyy2+"&mm2="+mm2+"&dd2="+dd2+"&shopid="+shopid+"&datefg="+datefg+"&offgubun="+offgubun+"&oldlist="+oldlist+"&inc3pl=<%=inc3pl%>&menupos=<%=menupos%>","detailStatistic","width=1000,height=780,scrollbars=yes,resizable=yes");
...
<tr bgcolor="#FFFFFF">
	...
	<td align="right" style="padding-right:5px;"  ><b><%= FormatNumber(cStatistic.FItemList(i).fprofit,0) %></b></td>
	<td align="right" style="padding-right:5px;"  >
		<% if cStatistic.FItemList(i).FMaechul > 0 and cStatistic.FItemList(i).fprofit > 0 then %>
				<%= FormatNumber(cStatistic.FItemList(i).fprofit/cStatistic.FItemList(i).FMaechul*100,2) %>%
			<% else %>
				0%
			<% end if %>    
	</td>
	<td align="center" >
		[<a href="javascript:detailStatistic('<%=yyyy1%>','<%=mm1%>','<%=dd1%>','<%=yyyy2%>','<%=mm2%>','<%=dd2%>','<%= cStatistic.FItemList(i).fshopid %>','<%= datefg %>','<%= offgubun %>','<%= oldlist %>')">일별</a>]
	</td>
</tr>
<%
...
vTot_profit  = vTot_profit + cStatistic.FItemList(i).fprofit
Next
%>
<tr bgcolor="<%= adminColor("tabletop") %>">
	...
	<td  align="right" style="padding-right:5px;"><%=FormatNumber(vTot_profit,0)%></td>
	<td align="center"></td>
	<td align="center"></td>
</tr>
