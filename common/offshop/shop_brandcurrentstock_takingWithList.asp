...
function setSvrSt(idx,svrSt){
    var frm = document.frmArr;
    if (idx==-1){
        frm.prechkNo.value=svrSt;
        if (svrSt!=frm.Arrrealstock.value) alert(svrSt + '.' + frm.Arrrealstock.value)
    }else if (idx!=""){
        frm.prechkNo[idx].value=svrSt;
        if (svrSt!=frm.Arrrealstock[idx].value) alert(svrSt + '.' + frm.Arrrealstock[idx].value)
    }
...
function fnFindLocal(shopid,makerid,ibarCode){
    var frm = document.frmArr;
    var i = 0;
    
    if (frm){
        if (frm.Arrrealstock.length){
            for (i=0;i<frm.Arrrealstock.length;i++){
                if ((frm.tenBarcoed[i].value==ibarCode)||(frm.publicBarcoed[i].value==ibarCode)){
                    //frm.Arrrealstock[i].value= frm.Arrrealstock[i].value*1+1;
                    //focusRow(frm.Arrrealstock[i]);
                    actSave(shopid,makerid,ibarCode,"AddRowItem(frmArr.Arrrealstock["+i+"])",i);
                    return true;
                }
            }
        }else{
            if ((frm.tenBarcoed.value==ibarCode)||(frm.publicBarcoed.value==ibarCode)){
                //frm.Arrrealstock.value= frm.Arrrealstock.value*1+1;
                //focusRow(frm.Arrrealstock);
                actSave(shopid,makerid,ibarCode,"AddRowItem(frmArr.Arrrealstock)","");
                return true;
            }
        }
    }
...
function showDoing(){
    var odiv = document.all["divLoading"];
    
    var bufX = (document.body.scrollWidth-odiv.offsetWidth)/2; 
    var bufY = (document.body.scrollHeight-odiv.offsetHeight)/2;
   
  
    odiv.style.left = bufX;
    odiv.style.top = bufY+150;
...
function actSave(shopid,makerid,itembarcode,actStr,vidx){
    $.ajax({
		type: "POST",
		url: "/common/offshop/getBarcodeStockTakingWithList.asp",
		data: "mode=actSave&shopid="+shopid+"&makerid="+makerid+"&itembarcode="+itembarcode+"&stTakingIdx=<%= stTakingIdx %>&actStr="+actStr+"&vidx="+vidx,
		dataType: "text",
		//timeout : 1000,
		error: function(){
			html = "/common/offshop/getBarcodeStockTakingWithList.asp?mode=actSave&shopid="+shopid+"&makerid="+makerid+"&itembarcode="+itembarcode+"&stTakingIdx=<%= stTakingIdx %>&actStr="+actStr+"&vidx="+vidx;
			$("#divDumi").html(html);
		},
	...
	//alert("mode=actSave&shopid="+shopid+"&makerid="+makerid+"&itembarcode="+itembarcode+"&stTakingIdx=<%= stTakingIdx %>&actStr="+actStr+"&vidx="+vidx);
}

function jsView(shopid,makerid,itembarcode){
    if ((itembarcode!="")&&(fnFindLocal(shopid,makerid,itembarcode))) return;
    <% if (Not isWorkingState) then %>
	...
	$.ajax({
		...
		//timeout : 1000,
		error: function(){
			html = "/common/offshop/getBarcodeStockTakingWithList.asp?shopid="+shopid+"&makerid="+makerid+"&itembarcode="+itembarcode+"&stTakingIdx=<%= stTakingIdx %>";
			$("#divView").html(html);
			hideDoing();
		},
		success: function(html){
...
function checkPreNHandDiff(){
    ...
    if (frm.cksel.length){
        for (i=0;i<frm.cksel.length;i++){
            if (frm.prechkNo[i].value!=frm.Arrrealstock[i].value){
                alert('수기 입력수량 ' + frm.Arrrealstock[i].value + ' 과 저장 된 수량 ' + frm.prechkNo[i].value + ' 이 일치 하지 않습니다.\n\n재고 파악 내용 수정(저장) 후 사용하세요.' );
        ...
        if (frm.prechkNo.value!=frm.Arrrealstock.value){
            alert('수기 입력수량 ' + frm.Arrrealstock.value + ' 과 저장 된 수량 ' + frm.prechkNo.value + ' 이 일치 하지 않습니다.\n\n재고 파악 내용 수정(저장) 후 사용하세요.' );
...
function nextStockStep(nVal){
    ...
    frm.mode.value="stockTakingNext";
    
    if (nVal==3){
        if (!checkPreNHandDiff()) return;
...
function delCurrTempStock(idx){
    var frm = document.frmup;
    
    if (idx>0){
        if (confirm('현재 재고파악 작업을 삭제 하시겠습니까?')){
            frm.mode.value="stockTakingDel";
            frm.submit();
	...
	<tr align="center" bgcolor="#FFFFFF" >
		...
		<% else %>
		    <span id="jobText">
		        ...
		        &nbsp;&nbsp; <input type="button" class="button" value="작업삭제" onClick="delCurrTempStock(<%= oOffStockTaking.FOneItem.FstTakingIdx %>)" style="color:red">
		        <% end if %>
		    </span>
...
<div id="divLoading" style='position:absolute; left:400px; top:240px; width:100; height:100; z-index:2; visibility: hidden'><img src="/images/loading.gif" width="100" height="100"></div>
<div id="divDumi"></div>
