...
dim ipgo, sell7days, includepreorder, shortagetype, onlinemwdiv, ordby
dim cp_idx
...
<tr align="center" bgcolor="<%= adminColor("tabletop") %>">
	...
	<% if not(C_IS_Maker_Upche) and not(C_IS_SHOP) then %>
		<td width="50"><%= CTX_Purchase_price %></td>
	<% end if %>
...
<tr align="center" bgcolor="#FFFFFF">
	...
	<td align="left">
		...
		<br>해외판매가:<%= ioffitem.FItemList(i).Fforeign_sellprice %>
    </td>
	<td align="left">
	...
	<td <%= ChkIIF(C_STOCK_DISP,"rowspan=2","") %> >
		...
		<% if ioffitem.FItemList(i).Flimityn="Y" then %>
			<font color="blue"><%= CTX_Limit %>(<%= ioffitem.FItemList(i).getLimitNo %>)</font>
		<% end if %>
	</td>
</tr>
