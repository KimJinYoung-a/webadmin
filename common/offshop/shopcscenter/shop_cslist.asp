...
set ioneas = new corder
	...
	ioneas.frectshopid = loginidshopormaker	      
	ioneas.FRectCurrstate  = currstate
	ioneas.FRectSearchType = "shopview"	
...
function regdetail(csmasteridx,menupos){
	location.href='/common/offshop/shopcscenter/shop_csdetail.asp?csmasteridx=' + csmasteridx + '&menupos=' + menupos
}
...
<tr align="center" bgcolor="<%= adminColor("topbar") %>" >
	...
	<td align="left">
		...
		<% else %>
			<%= loginidshopormaker %>
		<% end if %>	
		상태: <% drawcurrstate "currstate" ,currstate ,"" %>
		&nbsp;
		기타검색:
...
<tr align="center" bgcolor="#FFFFFF">
	...
	<td>
		<% if ioneas.FItemList(i).Frequireupche = "Y" then %>
			<font color="red">업체배송</font>
		<% end if %>
		<% if ioneas.FItemList(i).frequiremaejang = "Y" then %>
			<font color="blue">매장배송</font>
		<% end if %>		
	</td>
