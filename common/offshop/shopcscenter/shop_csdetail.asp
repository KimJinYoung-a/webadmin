...
<tr height="25" bgcolor="FFFFFF">
	<td colspan="15" align="center">
		...
		if ioneas.FOneItem.Fcurrstate <> "B006" and ioneas.FOneItem.Fcurrstate <> "B007" and ioneas.FOneItem.Fcurrstate <> "B008" then
			...
			if (ioneas.FOneItem.Fdivcd="A030" and C_IS_Maker_Upche) or (ioneas.FOneItem.Fdivcd="A031" and C_IS_SHOP) then
		%>
				<input type="button" class="button" value="매장처리완료" onclick="javascript:SaveFin(frm);">
		<%
			'/관리자나 , 오프라인관리자 일경우 수정 가능
			elseif C_ADMIN_AUTH or C_OFF_AUTH then
		%>
			 	<input type="button" class="button" value="매장처리완료(관리자모드)" onclick="javascript:SaveFin(frm);">
			<% else %>
				<input class="button" type="button" value="완료처리불가" onclick="alert('업체 에서만 변경가능 합니다.');" >
			<% end if %>
		<% end if %>

		<input type="button" class="button" value="목록보기" onClick="location.href='/common/offshop/shopcscenter/shop_cslist.asp';">
	</td>
</tr>
</form>
