...
// 기본 계약상의 주휴시간
// 유급 또는 공휴일이면 근무시간을 기본 계약상의 주휴시간으로 설정한다.
var holidaywdtime = 0;
...
function jsSetTotTime(iWd){  
	...
	if (eval("document.frmPay.selWH"+iWd).value =="4" && iduty>0) {
		var arrdft = eval("document.frmPay.dfT"+iWd).value.split(":");
		var idft =  parseInt(arrdft[0],10)*60+parseInt(arrdft[1],10);
		var arrivt = eval("document.frmPay.iVT"+iWd).value.split(":");
		var ivt =  parseInt(arrivt[0],10)*60+parseInt(arrivt[1],10); 
		 
		 if( (iduty - (idft -ivt)) > 0 ){
		 iextendtime = iextendtime + (iduty - (idft -ivt)); 
		 iduty = (idft-ivt);
		}
	}
	//
	...
	//휴일근무수당
	if(eval("document.frmPay.selWH"+iWd).value =="3" || eval("document.frmPay.selWH"+iWd).value =="5"|| eval("document.frmPay.selWH"+iWd).value =="2"){ 
		if(iduty>0){
...
// alert( iSWday+"-"+iEWday);
	...
	// 	}
		...
		//유상 주휴일일 경우 계약상 주유수당을 강제 지급
		if( eval("document.frmPay.selWH"+i).value=="7" ){
			eval("document.frmPay.iwhWT"+i).value = jsTimeForm(holidaywdtime);
		}

//		//기본근무시간+연장근무시간
//		totWorkDuty = totWorkDuty + iValue; //기본근무시간만
 
		//연장근무수당 2015.01.26 정윤정 주석처리.. 연장근무는 일단위로만 계산
...
function jsSetTotTimeSum(){
	var arrValue1, arrValue2, arrValue3, arrValue4, arrValue5,arrValue6;
	var iwt =0;
	var iewt =0;
	var inwt =0;
	var ihwt =0;
	...
	var ipwhwt =0;
	var ipvt = 0;
 var iSDay = document.frmPay.hidSday.value
	...
	for(i=0;i<= document.frmPay.hidEday.value;i++){
		...
		arrValue5 = eval("document.frmPay.iwhWT"+i).value.split(":");
		arrValue6 = eval("document.frmPay.iVT"+i).value.split(":");
		if (i<iSDay){
			ipwt = ipwt + parseInt(arrValue1[0],10)*60+parseInt(arrValue1[1],10);
			ipewt = ipewt + parseInt(arrValue2[0],10)*60+parseInt(arrValue2[1],10);
			ipnwt = ipnwt + parseInt(arrValue3[0],10)*60+parseInt(arrValue3[1],10);
			iphwt = iphwt + parseInt(arrValue4[0],10)*60+parseInt(arrValue4[1],10);
			ipwhwt = ipwhwt + parseInt(arrValue5[0],10)*60+parseInt(arrValue5[1],10);
			ipvt = ipvt + parseInt(arrValue6[0],10)*60+parseInt(arrValue6[1],10);
		}else{
			...
			ivt   = ivt +  parseInt(arrValue6[0],10)*60+parseInt(arrValue6[1],10);
		}	
			
	}


	if(typeof(document.getElementById("iwhWT40")) !="undefined"){
		if(document.getElementById("iwhWT40").value !="0"){
			jsAddWH();
			arrValue5 =  document.getElementById("iwhWT40").value.split(":");
			iwhwt=iwhwt+ parseInt(arrValue5[0],10)*60+parseInt(arrValue5[1],10);
		}
	}
		...
		if (typeof(document.frmPay.totPWT)!="undefined"){  	
			
		document.frmPay.totRWT.value = jsTimeForm(ipwt);	//'이전달 26일부터 말일까지 실근무시간 계산
		document.frmPay.totReWT.value = jsTimeForm(ipewt);
		document.frmPay.totRnWT.value = jsTimeForm(ipnwt);
		document.frmPay.totRhWT.value = jsTimeForm(iphwt);
		document.frmPay.totRwhWT.value = jsTimeForm(ipwhwt);
		document.frmPay.totRVT.value = jsTimeForm(ipvt);
		
			arrValue1 = document.frmPay.totPWT.value.split(":");
			arrValue2 = document.frmPay.totPeWT.value.split(":");
			arrValue3 = document.frmPay.totPnWT.value.split(":");
			arrValue4 = document.frmPay.totPhWT.value.split(":");
			arrValue5 = document.frmPay.totPwhWT.value.split(":");
			arrValue6 = document.frmPay.totPVT.value.split(":");
		 
			var isumwt = ipwt-(parseInt(arrValue1[0],10)*60+parseInt(arrValue1[1],10));
			var isumewt = ipewt- (parseInt(arrValue2[0],10)*60+parseInt(arrValue2[1],10));
			var isumnwt = ipnwt-(parseInt(arrValue3[0],10)*60+parseInt(arrValue3[1],10));
			var isumhwt = iphwt-(parseInt(arrValue4[0],10)*60+parseInt(arrValue4[1],10));
			var isumwhwt = ipwhwt-(parseInt(arrValue5[0],10)*60+parseInt(arrValue5[1],10));
			var isumpvt = ipvt-(parseInt(arrValue6[0],10)*60+parseInt(arrValue6[1],10));
		 
		document.frmPay.totSumWT.value =  jsTimeForm(isumwt);// '이전달 마감시 근무시간 - 실근무시간 계산 차액 
		document.frmPay.totSumeWT.value =  jsTimeForm(isumewt);
		document.frmPay.totSumnWT.value = jsTimeForm(isumnwt);
		document.frmPay.totSumhWT.value =  jsTimeForm(isumhwt);
		document.frmPay.totSumwhWT.value =  jsTimeForm(isumwhwt);
		document.frmPay.totSumVT.value =  jsTimeForm(isumpvt);
		
		document.frmPay.totWT.value = jsTimeForm(iwt);     //'현재달 1~말일까지 근무시간계산 
		...
		document.frmPay.totVT.value = jsTimeForm(ivt);
		
		document.frmPay.totSWT.value = jsTimeForm(iwt+isumwt);	
		document.frmPay.totSeWT.value = jsTimeForm(iewt+isumewt);
		document.frmPay.totSnWT.value = jsTimeForm(inwt+isumnwt);
		document.frmPay.totShWT.value = jsTimeForm(ihwt+isumhwt);
		document.frmPay.totSwhWT.value = jsTimeForm(iwhwt+isumwhwt);
		document.frmPay.totSVT.value = jsTimeForm(ivt+isumpvt);
	}else{
