...
//평균주수
	...
	function jsSetWeekHoliday(){
		...
		if (totDutyH>40){	WHTime=	(8*(40/40))*60};
...
//		 	} 
	...
	function jsChangeWeekHoliday(iWeekday,iWorkday){
		...
		var iEWday = iSWday+6; 
	 	...
	 	if(iSWday <= 0){
	 		iSWday = parseInt(document.frmPay.hidPED.value,10)+iSWday;
	 		if (iSWday <= 0){ iSWday = 1}
	 		if(iEWday<=0){
	   ...
	   //다시 재 설정	 - 현재 달 계산  
        ...
        if (pWDay<0){
            //이전달근무 계산
            iPSWday = parseInt(document.frmPay.hidPED.value,10)+pWDay;
	 		if(iEWday<=0){
	 			iPEWday = parseInt(document.frmPay.hidPED.value,10)+iEWday;
	 		}else{
		...
		for(i=iSWday;i<=iEWday;i++){ //수정된 날짜가 포함된 일주일
			...
			if(i ==1 &&  document.frmPay.hidWD1.value==1){ //이전달 계산없이 주휴일로 데이터 시작할 경우만 이전달값 합해준다
