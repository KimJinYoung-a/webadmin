...
<table width="840" border="0" cellpadding="5" cellspacing="0" bgcolor="#CCCCCC">
	...
	<tr>
		<td class="a" >
			...
			<input type="radio" name="mode" value="byevent" <% if mode="byevent" then response.write "checked" %> >행사 상품
			<input type="radio" name="mode" value="byetc" <% if mode="byetc" then response.write "checked" %> >기타소모품

			<br>
			브랜드 :<% drawSelectBoxShopjumunDesigner "designer",designer,shopid,suplyer %>
			<br>
...
<table width="840" cellspacing="1" class="a" bgcolor=#3d3d3d>
	...
	<% end if %>
		...
		<% if ioffitem.FItemList(i).IsUpchebeasongItem then %>
		<font color="#33CC33">[개별발송]</font><br>
		<% end if %>
		</td>
	</tr>
	</form>
