...
//	}
	...
	if(frm.justDesc.value.length<=0||frm.justDesc.value.length>=240) {
		alert("상품의 Just1Day 설명을 240자이내(4줄 이내)로 작성해주세요.\n\n");
		frm.justDesc.focus();
		return;
	}
...
<tr height="30">
	<td colspan="2" bgcolor="#FFFFFF">
		...
		<font color="red"><b>핑거스 저스트 원데이 등록/수정</b></font>
	</td>
</tr>
<% if mode="add" then %>
<tr>
...
<tr>
	...
	<td bgcolor="#FFFFFF">
		<input type="radio" name="targetinput" value="1"<% If targetinput=1 Then Response.write " checked" %>>할인율 <input type="radio" name="targetinput" value="2"<% If targetinput=2 Then Response.write " checked" %>>할인금액<br>
		할인금액 <input type="text" name="salePrice" value="" size="10" style="text-align:right" onkeyup="putPercent()"<% If targetinput=2 Then %>class="text" <% Else %>class="text_ro" readonly<% End If %>>원
		/ 판매가 <input type="text" class="text_ro" name="orgPrice" value="0" size="8" readonly style="text-align:right">원,
		할인율 <input type="text" name="saleRate" value="0" size="4" style="text-align:center" onkeyup="putPercent2()"<% If targetinput=1 Then %>class="text"<% Else %>class="text_ro" readonly<% End If %>>%
		<br>매입금액 <input type="text" class="text" name="saleSuplyCash" value="" size="8" style="text-align:right" onkeyup="fnCalcMargin('C')">원
		(할인마진 <input type="text" class="text" name="saleMar" value="0" size="5" style="text-align:right" onkeyup="fnCalcMargin('M')">%)
...
<tr>
	<td align="center" bgcolor="<%= adminColor("tabletop") %>">Just1Day 설명</td>
	<td bgcolor="#FFFFFF">
...
<tr align="center" bgcolor="<%= adminColor("tabletop") %>" height="30">
	...
	<td bgcolor="#FFFFFF" align="left">
		&nbsp;
<!--	<input type="button" class="button" size="30" value="이미지 넣기" onclick="jsImgInput('image3div','image3','i3','600','450','true','');"/>	-->		
...
<tr align="center" bgcolor="<%= adminColor("tabletop") %>" height="30">
	...
	<td bgcolor="#FFFFFF" align="left">
		&nbsp;
<!--	<input type="button" class="button" size="30" value="이미지 넣기" onclick="jsImgInput('image4div','image4','i4','600','450','true','');"/>	-->	
...
<tr>
	...
	<td bgcolor="#FFFFFF">
		<input type="radio" name="targetinput" value="1"<% If targetinput=1 Then Response.write " checked" %>>할인율 <input type="radio" name="targetinput" value="2"<% If targetinput=2 Then Response.write " checked" %>>할인금액<br>
		할인금액 <input type="text" name="salePrice" value="<%= fmainitem.FItemList(0).FjustSalePrice %>" size="10" style="text-align:right" onkeyup="putPercent()"<% If targetinput=2 Then %>class="text" <% Else %>class="text_ro" readonly<% End If %>>원
		...
		할인율 <input type="text" name="saleRate" value="<%= FormatPercent(1-(fmainitem.FItemList(0).FjustSalePrice/fmainitem.FItemList(0).ForgPrice),2) %>" size="5" style="text-align:center" onkeyup="putPercent2()"<% If targetinput=1 Then %>class="text"<% Else %>class="text_ro" readonly<% End If %>>
		<br>매입급액 <input type="text" class="text" name="saleSuplyCash" value="<%= fmainitem.FItemList(0).FsaleSuplyCash %>" size="8" style="text-align:right" onkeyup="fnCalcMargin('C')">원
		(할인마진 <input type="text" class="text" name="saleMar" value="<%= formatNumber(100*(1-(fmainitem.FItemList(0).FsaleSuplyCash/fmainitem.FItemList(0).FjustSalePrice)),2) %>" size="5" style="text-align:right" onkeyup="fnCalcMargin('M')">%)
		<br>(할인 안할경우 할인금액0, 매입금액 0 입력)
	</td>
</tr>
<tr>
...
<tr align="center" bgcolor="<%= adminColor("tabletop") %>" height="30">
	...
	<td bgcolor="#FFFFFF" align="left">
		&nbsp;
<!--	<input type="button" class="button" size="30" value="이미지 넣기" onclick="jsImgInput('image3div','image3','i3','600','450','true','<%= fmainitem.FItemList(0).Fimg3 %>');"/>	-->
...
<tr align="center" bgcolor="<%= adminColor("tabletop") %>" height="30">
	...
	<td bgcolor="#FFFFFF" align="left">
		&nbsp;
<!--	<input type="button" class="button" size="30" value="이미지 넣기" onclick="jsImgInput('image4div','image4','i4','600','450','true','<%= fmainitem.FItemList(0).Fimg4 %>');"/>	-->
