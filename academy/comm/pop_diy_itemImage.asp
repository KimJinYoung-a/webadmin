...
function remakeThumb(){
    ...
    if (confirm('썸네일 이미지 재생성 하시겠습니까?')){
        var popwin = window.open('<%= UploadImgFingers %>/linkweb/items/remakeThumb.asp?itemid='+iitemid,'remakeThumb','scrollbars=yes,resizable=yes,width=400,height=400');
        popwin.focus();
    }
}
...
<% end if %>
  ...
  <tr align="left">
  	<td height="30" width="15%" bgcolor="#DDDDFF">아이콘이미지<br>(자동생성) :<% if (C_ADMIN_USER) then %><br><input type="button" value="재생성" onClick="remakeThumb()"><% end if %></td>
  	<td bgcolor="#FFFFFF" colspan="3">
<% if (oitem.FOneItem.Ficon1image <> "") then %>
      <img src="<%= oitem.FOneItem.Ficon1image %>" > icon1(720x480) <br>
<% end if %>
<% if (oitem.FOneItem.Ficon2image <> "") then %>
      <img src="<%= oitem.FOneItem.Ficon2image %>"> icon2(390x260) <br>
<% end if %>
<% if (oitem.FOneItem.Flistimage120 <> "") then %>
      <img src="<%= oitem.FOneItem.Flistimage120 %>"> list120(300x200) <br>
<% end if %>
<% if (oitem.FOneItem.Flistimage <> "") then %>
      <img src="<%= oitem.FOneItem.Flistimage %>"> list(150x100) <br>
<% end if %>
<% if (oitem.FOneItem.Fsmallimage <> "") then %>
      <img src="<%= oitem.FOneItem.Fsmallimage %>"> small(60x40) <br>
<% end if %>
  	</td>
  </tr>
  <tr align="left">
...
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>" id="imgIn">
	<% If isArray(vArr) Then
			...
			For k = 1 To vArr(3,UBound(vArr,2))
				'If vArr(4,k-1) <> "" Then '//이미지 없을때 노출 안함
	%>
			  <tr align="left">
