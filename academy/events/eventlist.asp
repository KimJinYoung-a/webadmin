...
evt_startdate	= RequestCheckvar(request("evt_startdate"),32)
evt_enddate		= RequestCheckvar(request("evt_enddate"),32)
...
%>
<script>
function goRegEvent(v){
	location.href='/academy/events/event_regist.asp?idx='+v+'&menupos=<%=menupos%>'
}
function goPage(pg){
...
<tr align="center" bgcolor="#FFFFFF" >
	...
	<td align="left">
		...
		<select name="searchKey" class="select">
			...
			<option value="actid" <%= chkiif(searchKey="actid", "selected", "") %>>작가/강사 ID</option>
			<option value="company_name" <%= chkiif(searchKey="company_name", "selected", "") %>>작가/강사 이름</option>
		</select>
...
<tr align="center" bgcolor="#FFFFFF" >
	<td align="left">
		...
		<select name="gubun" class="select">
			<option value="">전체
			<option value="D" <%= chkiif(gubun="D", "selected", "") %> >작가 프로필(작품)
			<option value="L" <%= chkiif(gubun="L", "selected", "") %> >강사 프로필(강좌)
		</select>
		&nbsp;&nbsp;&nbsp;
		사용여부
...
<tr height="25" bgcolor="FFFFFF">
	...
	<td align="center">
		<input type="button" class="button" value="신규등록" onclick="goRegEvent('');">
	</td>
</tr>
<tr height="35" align="center" bgcolor="<%= adminColor("tabletop") %>">
	<td width="80">번호</td>
	<td width="150">등록작가/강사</td>
	<td width="350">기간</td>
...
<tr height="35" align="center" bgcolor="#FFFFFF" style="cursor:pointer;" onclick="goRegEvent('<%= oEvent.FItemList(i).FIdx %>')" onmouseover=this.style.background="f1f1f1"; onmouseout=this.style.background='ffffff';>
	<td><%= oEvent.FItemList(i).FIdx %></td>
	<td><%= oEvent.FItemList(i).FActid %></td>
	<td>
	<%
