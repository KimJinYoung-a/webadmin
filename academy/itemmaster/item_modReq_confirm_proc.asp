...
chkReturnCount = 0
	...
	For i=0 To UBound(itemid)
		...
		Set objCmd = Server.CreateObject("ADODB.COMMAND")
			With objCmd
				...
				.CommandText = "{?= call db_academy.[dbo].[sp_Fingers_item_UpcheReq_update]("&itemid(i)&", "&editidx(i)&", '"&adminid&"' ,'"&edittype(i)&"')}"
		...
		''2017/02/01 추가 corpse2 상품가격 변경 결과 push
		sqlStr = "exec db_academy.[dbo].[sp_ACA_sendPushMsgItemCoastEditConfirm_Artist] " & Cstr(itemid(i)) & ",'A'"
 ...
 strResultMsg = "" 
	...
	Call Alert_move(strResultMsg, "/academy/itemmaster/item_modReq_confirm.asp?menupos="&menupos&"&sS="&rectSort&"&makerid="&rectmakerid&"&itemname="&rectitemname&"&itemid="&rectitemid&"&disp="&rectdispCate&"&dSD="&rectstartdate&"&dED="&rectenddate&"&selRT="&rectreqtype)
...
chkReturnCount = 0
	...
	Set objCmd = Server.CreateObject("ADODB.COMMAND")
		With objCmd
			.ActiveConnection = dbACADEMYget
			.CommandType = adCmdText
			.CommandText = "{?= call db_academy.[dbo].[sp_Fingers_item_UpcheReq_Return]("&itemid(i)&", "&editidx(i)&",'"&rejectstr&"', '"&adminid&"' )}"
			.Parameters.Append .CreateParameter("RETURN_VALUE", adInteger, adParamReturnValue)
			.Execute, , adExecuteNoRecords
			End With
		    returnValue = objCmd(0).Value
...
CASE "C" '//대기승인으로 변경
	Set objCmd = Server.CreateObject("ADODB.COMMAND")
		With objCmd
			.ActiveConnection = dbACADEMYget
			.CommandType = adCmdText
			.CommandText = "{?= call db_academy.[dbo].[sp_Fingers_item_UpcheReq_Change]("&itemidarr&","&editidxarr&",'"&adminid&"' )}"
			.Parameters.Append .CreateParameter("RETURN_VALUE", adInteger, adParamReturnValue)
			.Execute, , adExecuteNoRecords
			End With
		    returnValue = objCmd(0).Value
	...
	ELSE	
			Call Alert_move("승인대기 상태로 변경되었습니다.", "/academy/itemmaster/item_modReq_confirm.asp?menupos="&menupos&"&sS="&rectSort&"&makerid="&rectmakerid&"&itemname="&rectitemname&"&itemid="&rectitemid&"&disp="&rectdispCate&"&dSD="&rectstartdate&"&dED="&rectenddate&"&selRT="&rectreqtype)
	END IF
response.end
