...
<!--
	...
	function GotoLectureDel(){
		if (confirm('삭제 하시겠습니까?')){
			document.frm_write.mode.value="DelField";
...
<input type="hidden" name="mode" value="AnsField">
...
<tr align="center" bgcolor="#F0F0FD">
	<td height="26" align="left" colspan="4"><b>현장강좌 문의 상세 내용 / 답변 작성</b></td>
</tr>
<tr>
	<td align="center" width="120" bgcolor="#DDDDFF">공예분야</td>
...
<tr>
	<td align="center" width="120" bgcolor="#DDDDFF">강좌 개요</td>
	<td bgcolor="#FDFDFD" colspan="3"><%=oLecture.FItemList(0).Flectitle%></td>
</tr>
<tr>
...
<tr>
	...
	<%
		if oLecture.FItemList(0).Flecurl<>"" then
			Response.Write "<a href='http://"& Replace(oLecture.FItemList(0).Flecurl,"http://","") & "' target='_blank'>" & oLecture.FItemList(0).Flecurl & "</a>"
		end if
	%>
	</td>
</tr>
<tr>
	<td align="center" width="120" bgcolor="#DDDDFF">공방약도</td>
	<td bgcolor="#FDFDFD" colspan="3">
	<%
		if oLecture.FItemList(0).Flecmap<>"" then
			'파일종류에 따른 보기 옵션 추가
			Select Case getFileExtention(oLecture.FItemList(0).Flecmap)
				Case "jpg", "gif", "png"
					Response.Write "<span onClick=""NewWindow('" & Server.URLencode(imgFingers & oLecture.upfolder & "field/" & oLecture.FItemList(0).Flecmap) & "')"" style='cursor:pointer'>" & oLecture.FItemList(0).Flecmap & "</span>"
				Case Else
					Response.Write "<a href='" & imgFingers & "/linkweb/download.asp?filepath=" & Server.URLencode(oLecture.upfolder & "field/") & "&filename=" & Server.URLencode(oLecture.FItemList(0).Flecmap) & "'>" & oLecture.FItemList(0).Flecmap & "</a>"
			end Select
		end if
	%>
	</td>
...
<tr>
	<td align="center" width="120" bgcolor="#DDDDFF">공방주소</td>
	<td bgcolor="#FDFDFD" colspan="3"><%=oLecture.FItemList(0).Flecmapaddress%></td>
</tr>
<tr>
	<td align="center" width="120" bgcolor="#DDDDFF">작품 브랜드</td>
	...
	<%
		if oLecture.FItemList(0).Flecfile<>"" then
			...
			Select Case getFileExtention(oLecture.FItemList(0).Flecfile)
				Case "jpg", "gif", "png"
					Response.Write "<span onClick=""NewWindow('" & Server.URLencode(imgFingers & oLecture.upfolder & "field/" & oLecture.FItemList(0).Flecfile) & "')"" style='cursor:pointer'>" & oLecture.FItemList(0).Flecfile & "</span>"
				Case Else
					Response.Write "<a href='" & imgFingers & "/linkweb/download.asp?filepath=" & Server.URLencode(oLecture.upfolder & "field/") & "&filename=" & Server.URLencode(oLecture.FItemList(0).Flecfile) & "'>" & oLecture.FItemList(0).Flecfile & "</a>"
			end Select
		end if
	%>
	</td>
...
<tr>
	<td colspan="4" height="32" bgcolor="#FAFAFA" align="center">
		...
		<img src="/images/icon_cancel.gif" onClick="self.location='partnerFieldLecture_List.asp?menupos=<%=menupos & param%>'" style="cursor:pointer" align="absmiddle">
	</td>
</tr>
</form>
</table>
