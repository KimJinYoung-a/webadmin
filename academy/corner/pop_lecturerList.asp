...
<%
Dim oPartner, i, page, charcd, searchKey, searchString
...
function openerRegID(iid, icompany_name, isocname, isocname_kor){
	opener.document.frmcontents.lecturer_id.value = iid;
	opener.document.frmcontents.lecturer_name.value = icompany_name;
	opener.document.frmcontents.socname.value = isocname;
	opener.document.frmcontents.socname_kor.value = isocname_kor;
	window.close();
}
</script>
</head>
...
<tr height="30" bgcolor="FFFFFF" align="center" onclick="openerRegID('<%= oPartner.FItemList(i).FId %>', '<%= oPartner.FItemList(i).FCompany_name %>', '<%= oPartner.FItemList(i).FSocname %>', '<%= oPartner.FItemList(i).FSocname_kor %>');" style="cursor:pointer;" onmouseover=this.style.background="f1f1f1"; onmouseout=this.style.background='ffffff';>
