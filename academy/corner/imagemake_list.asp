...
if (!CheckSelected()){
	...
	var frm;
		for (var i=0;i<document.forms.length;i++){
			...
			upfrm.fidx.value = ""
		var AssignFlashReal;
		AssignFlashReal = window.open("<%=wwwFingers%>/chtml/corner_make_flash.asp?idx=" +tot + '&poscode='+poscode+'&imagecount='+imagecount, "AssignFlashReal","width=800,height=600,scrollbars=yes,resizable=yes");
...
if (!CheckSelected()){
	...
	var frm;
		for (var i=0;i<document.forms.length;i++){
			...
			upfrm.fidx.value = ""
		var AssignimageReal;
		AssignimageReal = window.open("<%=wwwFingers%>/chtml/corner_make_image.asp?idx=" +tot + '&poscode='+poscode+'&imagecount='+imagecount, "AssignimageReal","width=800,height=600,scrollbars=yes,resizable=yes");
		AssignimageReal.focus();
}

//포스 코드 등록 & 수정
function popPosCodeManage(){
    var popPosCodeManage = window.open('/academy/corner/imagemake_poscode.asp','popPosCodeManage','width=800,height=600,scrollbars=yes,resizable=yes');
    popPosCodeManage.focus();
}

//이미지신규등록 & 수정
function AddNewMainContents(idx){
    var AddNewMainContents = window.open('/academy/corner/imagemake_contents.asp?idx='+ idx,'AddNewMainContents','width=800,height=600,scrollbars=yes,resizable=yes');
...
<% if oMainContents.FResultCount > 0 then %> 
	...
	<form action="" name="frmBuyPrc<%=i%>" method="get">			 		
	    ...
	    <td align="center">
	    	...
	    	<img width=40 height=40 src="<%=imgFingers%>/corner/main/<%= oMainContents.FItemList(i).fimagepath %>" border="0">
	    	</a>
	    </td>
	    <td align="center">
	    	<a href="javascript:AddNewMainContents('<%= oMainContents.FItemList(i).Fidx %>');">
