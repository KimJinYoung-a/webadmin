	...
	public Function fnGetSaleList
		...
		IF FTotCnt >0 THEN
			...
			strSql = " SELECT TOP "&FPSize&"  [sale_code], [sale_name], [sale_rate], [sale_margin], [evt_code], [evtgroup_code]"&_
					...
					", (select top 1 c.socname_kor FROM [db_academy].[dbo].[tbl_saleItem] s left join [db_academy].[dbo].[tbl_diy_item] i on i.itemid=s.itemid left join [TENDB].[db_user].[dbo].tbl_user_c c on c.userid=i.makerid where s.sale_code=A.sale_code) as brandname"&_
