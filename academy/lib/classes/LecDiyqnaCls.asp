...
Class CQna
	...
	Public Sub getMyqnaList()
		...
		If FRectsearchgubun <> "" Then
			addSql = addSql & " and pagegubun='" & FRectsearchgubun & "'"
		else
			addSql = addSql & " and pagegubun in ('D','L') "'D : 상품 , L : 강좌
		End If
		...
		If FRectsearchKey <> "" Then
			If FRectsearchString <> "" Then
				...
				ElseIf FRectsearchKey = "regname" Then
					addSql = addSql & " and username = '" & FRectsearchString & "'"
				...
				ElseIf FRectsearchKey = "regmakername" Then
					addSql = addSql & " and (lecturer_name like '%" & FRectsearchString & "%' OR brandname like '%" & FRectsearchString & "%') "
				End If
			End If
		End If
		...
		If  not rsACADEMYget.EOF  then
			Set FOneItem = new CQnaItem
				...
				elseif FRectsearchDiv = "L" Then
					...
					FOneItem.Flistimage     = imgFingers & "/lectureitem/list/"& GetImageSubFolderByItemid(FOneItem.Flec_idx)&"/"&rsACADEMYget("listimage")
				end if
