...
class UserJoinClass
	...
	if FRectBeasongArea = "1" then
		...
		sqlStr = sqlStr + " from [db_academy].[dbo].tbl_academy_order_master m, [DBDATAMART].[db_user].[dbo].tbl_user_n n" + vbcrlf
		sqlStr = sqlStr + " where m.userid = n.userid" + vbcrlf
		sqlStr = sqlStr + " and m.ipkumdiv >= 4" + vbcrlf
		sqlStr = sqlStr + " and Len(n.zipcode) >= 7" + vbcrlf
		...
		do until rsACADEMYget.Eof
			...
			FItemList(i).FArea =  rsACADEMYget("area")
	...
	public sub GetUserJoinByArea2()
		dim sqlStr
		dim i
	if FRectBeasongArea = "1" then
		sqlStr = "select count(m.orderserial) as cnt, Left(n.zipcode,2) as area" + vbcrlf
		sqlStr = sqlStr + " from [db_academy].[dbo].tbl_academy_order_master m, [DBDATAMART].[db_user].[dbo].tbl_user_n n" + vbcrlf
		sqlStr = sqlStr + " where m.userid = n.userid" + vbcrlf
		sqlStr = sqlStr + " and m.ipkumdiv >= 4" + vbcrlf
		sqlStr = sqlStr + " and Len(n.zipcode) <= 6" + vbcrlf
		sqlStr = sqlStr + " and m.cancelyn='N'" + vbcrlf
		sqlStr = sqlStr + " and m.regdate >='" + FRectStart + "'" + vbcrlf
		sqlStr = sqlStr + " and m.regdate <'" + FRectEnd + "'" + vbcrlf
		If FRectSiteName <> "" Then
		    sqlStr = sqlStr & " and m.sitename = '" & FRectSiteName & "'"
		End If
		sqlStr = sqlStr + " group by Left(n.zipcode,2)" + vbcrlf
		sqlStr = sqlStr + " order by area"
	else
		sqlStr = "select count(orderserial) as cnt, Left(reqzipcode,2) as area" + vbcrlf
		sqlStr = sqlStr + " from [db_academy].[dbo].tbl_academy_order_master" + vbcrlf
		sqlStr = sqlStr + " where ipkumdiv >= 4" + vbcrlf
		sqlStr = sqlStr + " and Len(n.zipcode) <= 6" + vbcrlf
		sqlStr = sqlStr + " and cancelyn='N'" + vbcrlf
		...
		If FRectSiteName <> "" Then
		    sqlStr = sqlStr & " and sitename = '" & FRectSiteName & "'"
		End If
		sqlStr = sqlStr + " group by Left(reqzipcode,2)" + vbcrlf
		sqlStr = sqlStr + " order by area"
	end if
	...
	public sub GetUserJoinByNai()
		...
		do until rsACADEMYget.eof
			FNaiMaster.AddData rsACADEMYget("cnt"), rsACADEMYget("nai"), rsACADEMYget("sex")
			i=i+1
			rsACADEMYget.moveNext
		loop
	...
	public sub GetUserJoinBySex()
		...
		sqlStr = sqlStr + " from [db_academy].[dbo].tbl_academy_order_master m, [DBDATAMART].[db_user].[dbo].tbl_user_n n" + vbcrlf
		'sqlStr = sqlStr + " from [db_academy].[dbo].tbl_academy_order_master m,[db_user].[dbo].tbl_user_n n" + vbcrlf
		sqlStr = sqlStr + " where m.userid = n.userid" + vbcrlf
		sqlStr = sqlStr + " and m.ipkumdiv >= 4" + vbcrlf
		sqlStr = sqlStr + " and m.cancelyn='N'" + vbcrlf
		sqlStr = sqlStr + " and m.regdate >='" + FRectStart + "'" + vbcrlf
		...
		'response.write sqlSTr
		do until rsACADEMYget.eof
			if rsACADEMYget("sex")="1" then
				FManNo = rsACADEMYget("cnt")
			elseif rsACADEMYget("sex")="2" then
				FWoManNo = rsACADEMYget("cnt")
			end if
			i=i+1
			rsACADEMYget.moveNext
		loop
