...
Class CWaitItemAddImage
	...
	Public Sub getAddImage(byval itemid)
		...
		redim  FItemList(FResultCount)
			...
			For i=0 to FResultCount-1
				...
				FItemList(i).FAddimgText		= ArrRows(3,i)
				IF ArrRows(1,i)="1" Or ArrRows(1,i)="2" Then
					FItemList(i).FAddimage 		= fingersImgUrl & "/diyitem/waitcontentsimage/" & GetImageSubFolderByItemid(itemid) & "/" & ArrRows(2,i)
				Else
					FItemList(i).FAddimage 		= fingersImgUrl & "/diyItem/waitimage/add" & Cstr(FItemList(i).FAddimageGubun) & "/" & GetImageSubFolderByItemid(itemid) & "/" & ArrRows(2,i)
					FItemList(i).FAddimageSmall	= fingersImgUrl & "/diyItem/waitimage/add" & Cstr(FItemList(i).FAddimageGubun) & "icon/" & GetImageSubFolderByItemid(itemid) & "/C" & ArrRows(2,i)
				End IF
    ...
    public Sub GetOneItemAddImageList()
        ...
        for i=0 to bufimgaddCnt-1
            ...
            if ((Not IsNULL(FItemList(i).FADDIMAGE_400)) and (FItemList(i).FADDIMAGE_400<>"")) then FItemList(i).FADDIMAGE_400 = imgFingers & "/diyItem/waitimage/add" & CStr(i+1) & "/" & GetImageSubFolderByItemid(FItemList(i).FItemID) + "/"  + FItemList(i).FADDIMAGE_400
        next
...
Class CWaitItem
	...
	public Sub GetOneItem()
		...
		sqlStr = sqlStr & " ,i.keywords, i.mwdiv, I.vatYn, i.ordercomment, i.mwdiv, i.optioncnt, i.currstate, "
		...
		if Not rsACADEMYget.Eof then
            ...
            FOneItem.FMainImage 		= imgFingers & "/diyItem/waitimage/main/" & GetImageSubFolderByItemid(FOneItem.FWaitItemid) + "/" + rsACADEMYget("mainimage")
			FOneItem.FListImage 		= imgFingers & "/diyItem/waitimage/list/" & GetImageSubFolderByItemid(FOneItem.FWaitItemid) + "/" + rsACADEMYget("listimage")
			FOneItem.FSmallImage 		= imgFingers & "/diyItem/waitimage/small/" & GetImageSubFolderByItemid(FOneItem.FWaitItemid) + "/" + rsACADEMYget("smallimage")
			
            FOneItem.FBasicImage      = imgFingers & "/diyItem/waitimage/basic/" & GetImageSubFolderByItemid(FOneItem.FWaitItemid) + "/" + rsACADEMYget("basicimage")
            FOneItem.FBasicImageIcon  = imgFingers & "/diyItem/waitimage/basicicon/" & GetImageSubFolderByItemid(FOneItem.FWaitItemid) + "/C" + rsACADEMYget("basicimage")
...
Class CItemListItems
	...
	public function GetCurrStateColor()
		...
		elseif FCurrState="3" or FCurrState="8" then
			GetCurrStateColor = "#888888"
		else
			GetCurrStateColor = "#000000"
		end if
	end function
...
class CWaitItemlist
	...
	public sub getWaitProductListByBrand()
		...
		elseif FRectCurrState="WR" then
			...
			sqlsearch = sqlsearch + " and currstate not in ('3','8')"
		end if
