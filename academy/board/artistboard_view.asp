...
<!-- 보기 화면 시작 -->
<form name="frmDel" method="post" action="<%=imgFingers%>/linkweb/artist/procboard.asp" enctype="multipart/form-data">
...
<tr>
	<td align="center" width="15%" bgcolor="#DDDDFF">강사</td>
	<td bgcolor="#FFFFFF"><%=lecuserid%></td>
</tr>
<tr>
...
<tr>
	...
	<!--%IF Cstr(userid) = Cstr(session("ssBctId")) THEN%-->
		<img src="/images/icon_modify.jpg" onClick="self.location='artistboard_modi.asp?menupos=<%=menupos%>&idx=<%=idx%>&lecuserid=<%=lecuserid%>'" style="cursor:pointer" align="absmiddle"> &nbsp;
		<img src="/images/icon_delete.gif" onClick="jsDel()" style="cursor:pointer" align="absmiddle"> &nbsp;
	<!--%END IF%-->	
	
		<img src="/images/icon_reply.gif" onClick="self.location='artistboard_write.asp?menupos=<%=menupos %>&idx=<%=idx%>&lecuserid=<%=lecuserid%>'" style="cursor:pointer" align="absmiddle">
