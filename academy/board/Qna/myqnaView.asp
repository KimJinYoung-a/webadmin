...
SET oMyqna = new CQna
	...
	If oMyqna.FResultCount < 1 Then
		response.write "<script>alert('오류가 발생했습니다.');location.replace('/academy/board/Qna/myqnaList.asp?menupos="&menupos&"');</script>"
		response.end
	End If
...
<!--		<select class="select" name="gubunVal">
			<option value="1" <%= Chkiif(masterQGubun = "1", "selected", "") %> >강좌문의</option>
			<option value="2" <%= Chkiif(masterQGubun = "2", "selected", "") %>>재료문의</option>
			<option value="3" <%= Chkiif(masterQGubun = "3", "selected", "") %>>신설 강좌요청</option>
			<option value="4" <%= Chkiif(masterQGubun = "4", "selected", "") %>>수강료,재료비 문의</option>
			<option value="5" <%= Chkiif(masterQGubun = "5", "selected", "") %>>입금확인</option>
			<option value="6" <%= Chkiif(masterQGubun = "6", "selected", "") %>>강좌취소</option>
			<option value="7" <%= Chkiif(masterQGubun = "7", "selected", "") %>>강좌대기문의</option>
			<option value="8" <%= Chkiif(masterQGubun = "8", "selected", "") %>>DIY 주문문의</option>
			<option value="9" <%= Chkiif(masterQGubun = "9", "selected", "") %>>DIY 주문취소문의</option>
			<option value="10" <%= Chkiif(masterQGubun = "10", "selected", "") %>>DIY 상품문의</option>
			<option value="11" <%= Chkiif(masterQGubun = "11", "selected", "") %>>기타 문의</option>
		</select>
