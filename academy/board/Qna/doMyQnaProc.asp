...
Select Case mode
	Case "C"				'문의분야 변경
		...
		returnurl = "/academy/board/Qna/myqnaView.asp?menupos="&menupos&"&idx="&idx&"&gridx="&gridx
		msg = "수정하였습니다."
	Case "D"				'문의글 삭제
		sqlStr = ""
		sqlStr = sqlStr & " UPDATE db_academy.[dbo].[tbl_academy_qna_NEW] SET "
		...
		dbACADEMYget.execute(sqlStr)
		returnurl = "/academy/board/Qna/myqnaList.asp?menupos="&menupos
		msg = "삭제하였습니다."
...
'			mailcontent = Replace(mailcontent,"#qstTitle#",masterQTitle)           
			...
			Call sendmail("customer@thefingers.co.kr", usermail, "[더핑거스] 문의하신 내용에 대한 답변이 등록되었습니다", mailcontent)
		End If

		'SMS체크했다면 SMS전송
		If (Len(lastsmsok) > 0 AND lastsmsok = "Y") AND (Len(lastSmsNum) > 0) Then
			sqlStr = " exec [db_sms].[dbo].[usp_SendSMS] '"&lastSmsNum&"','1644-1557','[더핑거스] 문의하신 내용에 대한 답변이 등록되었습니다.'"
