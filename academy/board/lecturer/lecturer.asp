...
<tr align="center" bgcolor="#FFFFFF" >
	...
	<td align="left">
		...
		<%=CommonCode("w","G000",sDoc_Type)%>
     	...
     	&nbsp;&nbsp;
     	<input type="checkbox" name="incnousing" <%=CHKIIF(incnousing="on","checked","") %> >미사용 게시글 표시
...
<tr>
	...
	<td align="left" width="20%">
		<a href="?menupos=<%=g_MenuPos%>&s_type=G010">공지사항</a> | <a href="?menupos=<%=g_MenuPos%>&s_type=G020">문의게시판</a>
	</td>
	<td align="right" width="70%">
		<!--<input type="button" class="button" value="예전강사게시판보기" onClick="location.href='/academy/board/lec_board_list.asp?menupos=<%=g_MenuPos%>'">-->
		<input type="button" class="button" value="예전공지사항보기" onClick="location.href='/academy/board/notice_list.asp?menupos=<%=g_MenuPos%>'">
	</td>
</tr>
</table>
<!-- 액션 끝 -->
...
<tr align="center" bgcolor="<%=CHKIIF(olect.FItemList(i).fadmin_usingyn="N","#CCCCCC","#FFFFFF")%>" height="30" onmouseout="this.style.backgroundColor='<%=CHKIIF(olect.FItemList(i).fadmin_usingyn="N","#CCCCCC","#FFFFFF")%>'" onmouseover="this.style.backgroundColor='#F1F1F1'" style="cursor:pointer">
	...
	<td>
	    <% if (olect.FItemList(i).IsPushSendReqNotice) then %>
    	    ...
    	    <% else %>
    	        <input type="button" value="발송" onClick="popSendNoticePush('<%=olect.FItemList(i).fdoc_idx%>')">
    	    <% end if %>
    	<% else %>
    	-
        <% end if %>
