...
dim sDoc_WorkerView , i , tContents , olect, lectFile , arrFileList ,g_MenuPos
	...
	Else
		...
		tContents	= replace(replace(olect.FOneItem.FDoc_Content,vbcrlf,"<br>"),"'","")	
...
function checkform(frm){
	//if (frm.G000.value == "") //핑거스 공지만 가능 2016/07/12 (권한관련 구분)
	if (frm.G000.value != "G010"){
		alert("업무 구분을 선택해 주세요! (핑거스 공지만 가능)");
		frm.G000.focus();
		return;
	}
