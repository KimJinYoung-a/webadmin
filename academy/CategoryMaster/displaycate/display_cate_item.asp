...
<%
	Dim cDisp, i, vDepth, vCateCode, vCurrpage, vPageSize, vIsThisCate, vParam, vSearch, vNotCateReg, dispCate
	vCurrPage	= NullFillWith(Request("cpg"), "1")
	vDepth 		= NullFillWith(Request("depth_s"), "1")
	vCateCode 	= RequestCheckvar(Request("catecode_s"),10)
	vIsThisCate	= RequestCheckvar(Request("isthiscate"),1)
	vPageSize	= NullFillWith(RequestCheckvar(Request("pagesize"),10), 100)
	vSearch		= RequestCheckvar(Request("search"),5)
	vNotCateReg	= RequestCheckvar(Request("notcatereg"),10)
	dispCate	= RequestCheckvar(Request("disp"),10)
	If dispCate="" Then dispCate=vCateCode

	Dim makerid, cdl, cdm, cds, itemid_s, itemname, keyword, sellyn, usingyn, danjongyn, limityn, sailyn, deliverytype, sortDiv
	...
	keyword		= RequestCheckvar(request("keyword"),64)
...
function jsSearchBrandID2(frmName,compName){
    ...
    var popwin = window.open("/admin/member/popBrandSearch.asp?frmName=" + frmName + "&compName=" + compName + "&rect=" + compVal + "&sitediv=f","popBrandSearch","width=800 height=400 scrollbars=yes resizable=yes");
	...
	<td width="75%">
			...
			<td bgcolor="#FFFFFF">
				...
				<tr>
					<td bgcolor="#FFFFFF">
						<table class="a">
							<tr>
								<td>브랜드 : <%	NewDrawSelectBoxDesignerwithNameFingers "makerid", makerid %></td> 
				...
				<tr>
					<td bgcolor="#FFFFFF">
						<!-- #include virtual="/academy/CategoryMaster/displaycate/categoryselectbox.asp"-->
					</td>
				</tr>
				<tr>
					<td bgcolor="#FFFFFF">
		...
		<tr>
			<td colspan="2" bgcolor="#FFFFFF">
				<input type="button" value="선택한 상품에 등록된 상단 선택 카테고리 삭제" onClick="delCateItem()">
...
<% If vSearch = "o" Then %>
	parent.jsEditLink();
<% End If %>
function fnDispCateAddEnd(){
	document.frmitem.itemid_s.value="";
	searchFrm();
}
</script>
