...
<%
	...
	vCateName	= RequestCheckvar(Request("catename_s"),32)
	vUseYN 		= RequestCheckvar(Request("useyn_s"),10)
	vSortNo 	= RequestCheckvar(Request("sortno_s"),10)
	...
	cdl 		= RequestCheckvar(request("cdl"),8)
	cdm 		= RequestCheckvar(request("cdm"),8)
	cds 		= RequestCheckvar(request("cds"),8)
	itemid_s	= RequestCheckvar(request("itemid_s"),10)
...
function jsRealServerReg(){
	if(confirm("더핑거스 GNB의 카테고리 메뉴를 생성하시겠습니까?") == true){
	    var popCreateTemp = window.open("http://<%=CHKIIF(application("Svr_Info")="Dev","2016www","www1")%>.thefingers.co.kr/chtml/dispcate/menu_make_html.asp","popCreateTemp","width=1200 height=930 scrollbars=yes resizable=yes");
		popCreateTemp.focus();
	}
}
</script>
...
<div class="box2">
	...
	<tr id="lyrSbmBtn1">
		<td>
			...
			&nbsp;&nbsp;&nbsp;
			<input type="button" value="카테고리내 전 상품 이동" onClick="jsItemAllMove();">
			&nbsp;&nbsp;&nbsp;
			<input type="button" value="선택한 카테고리 상품보기(하위뎁스포함)" onClick="jsSelctCateShowItem();">
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="button" value="더핑거스 사이트 주 메뉴생성" onClick="jsRealServerReg()">&nbsp;&nbsp;&nbsp;
		</td>
		<td align="right">
...
<iframe src="" id="cateproc" name="cateproc" width="100" height="100" frameborder="0"></iframe>
<% SET cDisp = Nothing %>
