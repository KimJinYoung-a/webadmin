...
<%
	...
	if page="" then page=1
	if CateDiv="" then CateDiv="CateCD1"
	if searchKey="" then searchKey=CateDiv & "_Name"
...
<form name="frm_search" method="GET" action="categoryList.asp" onSubmit="return chk_form(this)">
...
<tr height="30">
	...
	<td valign="top" align="right">
		...
		<select name="CateDiv" onChange="goPage(frm_search.page.value)">
			<option value="CateCD1">클래스구분</option>
			<option value="CateCD2">강좌분야</option>
			<option value="CateCD3">장소구분</option>
		</select>
		/ 상태
		...
		<select name="searchKey">
			<option value="<%=CateDiv%>">카테고리코드</option>
			<option value="<%=CateDiv%>_Name">코드명</option>
		</select>
		<script language="javascript">
...
<table width="750" border="0" class="a" cellpadding="2" cellspacing="1" bgcolor="#BABABA">
	<tr align="center" bgcolor="#F0F0FD">
		<td colspan="6" align="left">검색건수 : <%= oCate.FTotalCount %> 건 Page : <%= page %>/<%= oCate.FTotalPage %></td>
	</tr>
	<tr align="center" bgcolor="#DDDDFF">
		<td align="center" width="40">번호</td>
		<td align="center" width="80">코드</td>
		<td align="center">코드명</td>
		<% if CateDiv="CateCD2" then %><td align="center">영문명</td><% end if %>
	...
	<tr align="center" bgcolor="<%=bgcolor%>">
		...
		<td><a href="categoryModi.asp?CateCD=<%= oCate.FCateList(lp).FCateCD %>&page=<%=page & param%>"><%= oCate.FCateList(lp).FCateCD %></a></td>
		<td align="left"><a href="categoryModi.asp?CateCD=<%= oCate.FCateList(lp).FCateCD %>&page=<%=page & param%>"><%= db2html(oCate.FCateList(lp).FCateCD_Name) %></a></td>
		<% if CateDiv="CateCD2" then %><td align="left"><%= db2html(oCate.FCateList(lp).FCateCD_NameEng) %></td><% end if %>
		<% if CateDiv<>"CateCD1" then %>
	...
	<tr bgcolor="#FFFFFF">
		<td colspan="6" height="30" align="center">
			...
			<tr>
				...
				<td width="77" align="right"><a href="categoryWrite.asp?page=<%=param%>"><img src="/images/icon_new_registration.gif" width="75" height="20" border="0"></a></td>
			</tr>
			</table>
		</td>
	</tr>
