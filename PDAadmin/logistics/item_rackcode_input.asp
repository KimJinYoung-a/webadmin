...
function SaveRackcode(frm){
	...
	if (confirm('상품 랙코드를 저장하시겠습니까?')){
		frm.method="post";
		frm.action="/warehouse/itemrackcode_process.asp"
		frm.submit();
	}
}
...
<table width="100%" border="0" align="center" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	...
	<!-- 상단바 끝 -->
	<tr bgcolor="#FFFFFF">
        <td width="60" align="center" bgcolor="<%= adminColor("tabletop") %>">랙코드</td>
        <td><input type="text" name="itemrackcode" value="<%= itemrackcode %>" size=4 maxlength=4 AUTOCOMPLETE="off"></td>
    </tr>
    <tr bgcolor="#FFFFFF">
        <td align="center" bgcolor="<%= adminColor("tabletop") %>">상품코드</td>
        <td><input type="text" name="itembarcode" value="<%= itembarcode %>" size=14 maxlength=14 AUTOCOMPLETE="off" onKeyPress="if (event.keyCode == 13){ research(frmrackcodeinput); return false;}"><input type=button value="저장" onclick="research(frmrackcodeinput)"></td>
    </tr>
    <% if oitembar.FResultCount>0 then %>
    <tr bgcolor="#FFFFFF">
