...
set oip = new cevent_list
	...
	if evt_code <> "" then
		...
		m_main_content	= oip.FItemList(0).fm_main_content
		m_cmt_desc		= oip.FItemList(0).fm_cmt_desc
...
<!----- ## 모바일 폼 시작 --------------------------------->
	...
	<tr bgcolor="#FFFFFF" id="trMobile5" <%=chkIIF(m_isusing="Y","","style='display:none;'")%>>
		<td align="center" colspan="2">
			<textarea rows="10" name="m_main_content" style="width:100%" class="textarea"><%=chkIIF(evt_code<>"" and Not(m_main_content="" or isnull(m_main_content)),m_main_content,"" & vbCrLf) %></textarea>
		</td>
	</tr>
	...
	<tr bgcolor="#FFFFFF" id="trMobile7" <%=chkIIF(m_isusing="Y","","style='display:none;'")%>>
		<td align="center" colspan="2">
			<textarea rows="8" name="m_cmt_desc" style="width:100%" class="textarea"><%=chkIIF(evt_code<>"" and not(m_cmt_desc="" or isnull(m_cmt_desc)),m_cmt_desc,"" & vbCrLf) %></textarea>
		</td>
	</tr>
<!----- ## 모바일 폼 끝 --------------------------------->	
