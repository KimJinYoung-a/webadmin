...
function event_contents_delete(evtcode){
	...
	if (str  == "ok"){
		alert("선택하신 이벤트의 컨텐츠가 삭제 되었습니다.");
	}else{
		alert("컨텐츠 삭제에 문제가 있습니다. 문의 바랍니다.");
	}
}
	...
	<tr align="center" bgcolor="#FFFFFF" >
		<td align="left">
			...
			<select name="evt_mobile_yn" value="<%=evt_mobile_yn%>" class="select">
				<option value="" <% if evt_mobile_yn = "" then response.write " selected" %>>전체</option>
	...
	<tr align="center" bgcolor="#FFFFFF" >
		...
		<td align="left">
			...
			<select name="ses" onChange="TnSearchEvtSelect(this.value)">
				<option value="evt_code_search" <%=chkIIF(evt_code_search<>"","selected","")%>>이벤트코드</option>
				<option value="evt_name_search" <%=chkIIF(evt_name_search<>"","selected","")%>>이벤트명</option>
				<option value="evt_partner_search" <%=chkIIF(evt_partner_search<>"","selected","")%>>진행업체</option>
			</select>
...
<tr>
	<td align="left">
		<img src="/images/icon_reload.gif" onClick="javascript:RefreshMainCorItemRec(frm);" style="cursor:pointer" align="absmiddle" alt="XML만들기">프론트에 적용
	</td>
</tr>
<!-- <tr>
	<td align="left">
		<img src="/images/icon_reload.gif" onClick="javascript:MORefreshMainCorItemRec(frm);" style="cursor:pointer" align="absmiddle" alt="XML만들기">모바일 프론트에 적용(2개선택)
	</td>
</tr> -->
</table>
<!-- 액션 시작 -->
	...
	<tr>
		<td align="left">
			<% if evt_type_search <> "" and evt_code_search = "" then %>				
				&nbsp;&nbsp;<a href="javascript:AssignReal(frm,<%= evt_type_search %>);"><img src="/images/refreshcpage.gif" border="0">SubPage Category생성</a>		
			<% end if %>
		</td>
		<td align="right">
			<% if evt_mobile_yn="Y" then %>	<input type="button" class="button" value="Mobile정렬순서 저장" onclick="save_mSortNo();"> /<% end if %>
			<% if (evt_type_search<>"" or srchStat="Y") and evt_code_search="" and evt_mobile_yn<>"Y" then %><input type="button" class="button" value="Web정렬순서 저장" onclick="save_webSortNo();"> /<% end if %>
			...
			<input type="button" class="button" value="Event등록" onclick="event_edit('');">
		</td>
	</tr>
</table>
<!-- 액션 끝 -->
...
<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
	...
	<tr align="center" bgcolor="<%= adminColor("tabletop") %>">
		...
		<td align="center">응모수</td>
		<!-- <td align="center" title="웹 사이트 목록에 표시될 정렬 순서">Web정렬</td>
		<td align="center" title="모바일 사이트 목록에 표시될 정렬 순서">Mobile정렬</td> -->
    ...
    <tr align="center" bgcolor="<%=rowColor%>">
			...
			<td align="center">
				...
				<%= oip.FItemList(i).fevt_code %><br>[보기]</a>
			</td>		
			<td align="center">
			...
			<td align="center"><%= "<span title='이벤트 사용여부'>" & oip.FItemList(i).fisusing & "</span> <font color='darkgray' title='모바일 사용여부'>(" & oip.FItemList(i).fm_isusing & ")</font>" %></td>
			<td align="center">
			...
			<%
				If oip.FItemList(i).fsubcount > 0 Then
					Response.Write "<br><a href=""javascript:"" onClick=""window.open('pop_event_votelist_xls.asp?eC=" & oip.FItemList(i).fevt_code & "','voteXls','width=400,height=150');"">[xls다운]</a>"
				End If
			%>
			</td>
			...
			<td align="center">
				<input type="button" onclick="event_edit('<%= oip.FItemList(i).fevt_code %>');" value="수정" class="button">&nbsp;<input type="button" onclick="event_contents_delete('<%= oip.FItemList(i).fevt_code %>');" value="초기화" class="button">
				<input type="button" class="button" value="당첨자등록 (<%= oip.FItemList(i).fprizeyn %>)" onclick="prize(<%= oip.FItemList(i).fevt_code %>);">
				<br><a href="javascript:maincategoryimage(<%= oip.FItemList(i).fevt_code %>,<%= oip.FItemList(i).fevt_type %>);">
				<img src="/images/refreshcpage.gif" border="0">MainCate Image생성</a>
			</td>
    </tr>   
	<% next %>
	<% else %>
