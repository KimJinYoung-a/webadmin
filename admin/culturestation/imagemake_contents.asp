...
<%
'###########################################################
' Description : Culture Station 이미지 와 내용 등록  
...
<form name="frmcontents" method="post" action="<%=staticimgurl%>/linkweb/culturestation/image_proc.asp" onsubmit="return false;" enctype="multipart/form-data">
...
<!--<input type="hidden" name="ckUserId" value="<%=request.Cookies("partner")("userid")%>">-->
	...
	<tr bgcolor="#FFFFFF">
		...
		<% if oMainContents.FOneItem.Fidx<>"" then %>
		<br><img src="<%=uploadUrl%>/culturestation/main/<%= oMainContents.FOneItem.fimagepath %>" border="0"> 
		<br><%=uploadUrl%>/culturestation/main/<%= oMainContents.FOneItem.fimagepath %>
		<% end if %>
		</td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td width="150" align="center">서브이미지 :</td>
		<td><input type="file" name="file2" value="" size="32" maxlength="32" class="file"><br>※필요한경우에만등록(2011부터 메인_멀티xml용 <b>ON</b> 이미지)
		<% if oMainContents.FOneItem.Fidx<>"" then %>
		<br><img src="<%=uploadUrl%>/culturestation/main/<%= oMainContents.FOneItem.fimagepath2 %>" border="0"> 
		<br><%=uploadUrl%>/culturestation/main/<%= oMainContents.FOneItem.fimagepath2 %>
		<% end if %>
		</td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td width="150" align="center">서브이미지2 :</td>
		<td><input type="file" name="file3" value="" size="32" maxlength="32" class="file"><br>※필요한경우에만등록(2011부터 메인_멀티xml용 <b>OFF</b> 이미지)
		<% if oMainContents.FOneItem.Fidx<>"" then %>
		<br><img src="<%=uploadUrl%>/culturestation/main/<%= oMainContents.FOneItem.fimagepath3 %>" border="0"> 
		<br><%=uploadUrl%>/culturestation/main/<%= oMainContents.FOneItem.fimagepath3 %>
		<% end if %>
		</td>
	</tr>	
	<tr bgcolor="#FFFFFF">
	...
	<%
	'// 포스코드200(컬쳐메인하단배너3개) 일경우 이벤트코드 입력 받음
	if poscode = "200" or oMainContents.FOneItem.Fposcode = "200" then
	%>
	<tr bgcolor="#FFFFFF">
	    <td width="150" align="center">이벤트코드 :</td>
	    <td>
	        <% 
	        '// 수정
	        if oMainContents.FOneItem.Fidx<>"" then 
	        %>
	            <input type="text" name="evt_code" value="<%= oMainContents.FOneItem.fevt_code %>"  size="10">
	        <% 
	        '//신규
	        else 
	        %>
	            <% if poscode<>"" then %>
	                    <input type="text" name="evt_code" value=""  size="10">
	            <% else %>
	            <font color="red">구분을 먼저 선택하세요</font>
	            <% end if %>
	        <% end if %>
