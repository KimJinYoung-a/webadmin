...
set oip = new cevent_list
	...
	if evt_code <> "" then
		...
		startdate = left(oip.FItemList(0).fstartdate,10)
		enddate = left(oip.FItemList(0).fenddate,10)
		eventdate = left(oip.FItemList(0).feventdate,10)
		...
		m_main_content	= nl2blank(db2html(oip.FItemList(0).fm_main_content))
	...
	$(document).ready(function(){
		$('#evt_type').change(function(){
			if($('#evt_type').val()=="0"){
				...
				$("#evt_kind").append("<option value='0'>영화</option>");
				$("#evt_kind").append("<option value='1'>연극</option>");
				$("#evt_kind").append("<option value='2'>공연</option>");
				$("#evt_kind").append("<option value='3'>뮤지컬</option>");
				$("#evt_kind").append("<option value='5'>전시</option>");
				$("#evt_kind").append("<option value='6'>공모전</option>");
			}else{
				$("#evt_kind option").remove();
				$("#evt_kind").append("<option value='4'>도서</option>");
			}
		});
	});
</script>
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="#BABABA">
	...
	<tr bgcolor="#FFFFFF">
		...
		<td><select name="evt_kind" id="evt_kind" class="select">
				...
				<option value="5" <% if evt_kind = "5" then response.write " selected" %>>전시</option>
				<option value="6" <% if evt_kind = "6" then response.write " selected" %>>공모전</option>
	...
	<tr bgcolor="#FFFFFF">
		...
		<td>
			<input type="text" name="startdate" id="termSdt" size=10 value="<%= startdate %>" class="text">
			<img src="/images/calicon.gif" width="21" border="0" align="middle" id="ChkStart_trigger" onclick="return false;"> -
			<input type="text" name="enddate" id="termEdt" size=10  value="<%= left(enddate,10) %>" class="text">
			<img src="/images/calicon.gif" width="21" border="0" align="middle" id="ChkEnd_trigger" onclick="return false;">	
			<script type="text/javascript">
				...
				var CAL_End = new Calendar({
					...
					onSelect: function() {
						...
						CAL_Start.redraw();
						this.hide();
					}, bottomBar: true, dateFormat: "%Y-%m-%d" <%=chkIIF(enddate<>"",", min: " & replace(startdate,"-",""),"")%>
				});
			</script>
		</td>
	</tr>
	<tr bgcolor="#FFFFFF">
		...
		<td>
			<input type="text" name="eventdate" id="termPrizeEdt" size=10 value="<%= eventdate %>" class="text">
			<img src="/images/calicon.gif" width="21" border="0" align="middle" id="ChkPrizeEnd_trigger" onclick="return false;">
			<script type="text/javascript">
				var CAL_Start = new Calendar({
					inputField : "termPrizeEdt", trigger    : "ChkPrizeEnd_trigger",
					onSelect: function() {
						var date = Calendar.intToDate(this.selection.get());
						CAL_End.args.min = date;
						CAL_End.redraw();
	...
	<tr bgcolor="#FFFFFF">
		...
		<td>
			<%sbGetMKTid "selMId",emid,""%>
		</td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td align="center" bgcolor="<%=adminColor("sky")%>"><b>기본 이미지</b></td>
		<td>
			<input type="button" value="이미지 넣기" onclick="jsImgInput('listdiv','listimgName','list','50000 ','9000','9000','true');"  class="button"> 
