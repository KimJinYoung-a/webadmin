...
//신규추가
function popnew(){
	var popnew = window.open('/admin/organizer/event_new.asp','popnew','width=600,height=200,resizable=yes,scrollbars=yes')
	popnew.focus();
}

//수정
function popedit(idx){
	var popedit = window.open('/admin/organizer/event_edit.asp?idx='+idx,'popedit','width=600,height=200,resizable=yes,scrollbars=yes')
	popedit.focus();
}
...
<% End IF %>
	<tr bgcolor="#FFFFFF">
		...
		<!-- 페이지 시작 -->
	    	<a href="?page=1" onfocus="this.blur();"><img src="http://fiximage.10x10.co.kr/web2007/common/pprev_btn.gif" width="10" height="10" border="0"></a>
			<% if oDiary.HasPreScroll then %>
				<span class="list_link"><a href="?page=<%= oDiary.StartScrollPage-1 %>&flagdatebox=<%=flagdate%>">&nbsp;<img src="http://fiximage.10x10.co.kr/web2007/common/prev_btn.gif" width="10" height="10" border="0">&nbsp;</a></span>
			<% else %>
			...
			<% for i = 0 + oDiary.StartScrollPage to oDiary.StartScrollPage + oDiary.FScrollCount - 1 %>
				...
				<% else %>
				<a href="?page=<%= i %>&flagdatebox=<%=flagdate%>" class="list_link"><font color="#000000"><%= i %>&nbsp;&nbsp;</font></a>
				<% end if %>
			<% next %>
			<% if oDiary.HasNextScroll then %>
				<span class="list_link"><a href="?page=<%= i %>&flagdatebox=<%=flagdate%>">&nbsp;<img src="http://fiximage.10x10.co.kr/web2007/common/next_btn.gif" width="10" height="10" border="0">&nbsp;</a></span>
			<% else %>
			&nbsp;<img src="http://fiximage.10x10.co.kr/web2007/common/next_btn.gif" width="10" height="10" border="0">&nbsp;
			<% end if %>
			<a href="?page=<%= oDiary.FTotalpage %>&flagdatebox=<%=flagdate%>" onfocus="this.blur();"><img src="http://fiximage.10x10.co.kr/web2007/common/nnext_btn.gif" width="10" height="10" border="0"></a>
		<!-- 페이지 끝 -->
