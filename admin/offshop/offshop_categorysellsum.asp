...
'		[OFF]오프_통계관리>>브랜드별매출통계 /admin/offshop/sellreportbrand.asp 에서 사용
'		[OFF]오프_통계관리>>요일별매출분석 /admin/offshop/weeklysellreport.asp 에서 사용
...
<tr align="center" bgcolor="#FFFFFF" >
	...
	<td align="left">
		* 기간 :
		...
		<!--<input type="checkbox" name="oldlist" <%' if oldlist="on" then response.write "checked" %> >2009년이전내역조회-->
		&nbsp;&nbsp;&nbsp;
		<%
		'직영/가맹점
		if (C_IS_SHOP) then
		...
		* 요일:<% drawweekday_select "weekdate" , weekdate ," onchange='ReSearch("""",""RESETOLDDATAY"");'" %>
		&nbsp;&nbsp;
		* 국적구분: <% drawoffshop_commoncode "buyergubun", buyergubun, "buyergubun", "MAIN", "", " onchange='ReSearch("""",""RESETOLDDATAY"");'" %>
	</td>
...
<tr valign="bottom">
    <td align="left">
    	※ 정산은 주문일 기준으로 정산 되며, 실시간 데이터는 최근 3년 데이터만 검색 가능합니다.
    </td>
    <td align="right">
