...
Function ExcelPasing(ByVal TotalCount, PageSize)
	Dim totalPage
	totalPage =  CInt(TotalCount\PageSize)
	if  (TotalCount\PageSize)<>(TotalCount/PageSize) then
		totalPage = totalPage +1
	end If
	ExcelPasing = totalPage
End Function
%>
...
function pop_itemedit_off_edit(ibarcode){
    <% if C_IS_Maker_Upche and Not(IsDirectIpchulContractExistsBrand) and Not isupcheitemreg then %>
    	...
    	return;
	<% else %>
		var pop_itemedit_off_edit = window.open('/common/offshop/item/pop_itemedit_off_edit.asp?barcode=' + ibarcode,'pop_itemedit_off_edit','width=1400,height=800,resizable=yes,scrollbars=yes');
...
// ON/OFF가격연동 일괄수정
function chcheckallonofflinkyn(){
    var frmlist;
    var pass = false;

	if (frm.checkallonofflinkyn.value=="") {
	    alert('일괄적용하실 기준 ON/OFF가격연동을 선택하세요.');
	    frm.checkallonofflinkyn.focus();
	    return false;
	}
	...
	for (var i=0;i<document.forms.length;i++){
		...
		if (frmlist.name.substr(0,9)=="frmBuyPrc") {
			if (frmlist.cksel.checked){
				pass = true;
				if (frm.checkallonofflinkyn.value=="Y") {
					frmlist.onofflinkyn[0].checked=true;
				}else{
					frmlist.onofflinkyn[1].checked=true;
				}
			}
		}
	}
...
function downloadexcel(page){
	if(page < document.frm.excelsize.value)
	{
		alert("추출 가능한 페이지 수가 아닙니다.\n" +document.frm.excelsize.value+" 페이지 이하로 추출가능합니다.");
	}
	else
	{
...
function uploadexcel(){
	...
	var popwin = window.open('/admin/offshop/pop_shopitemlist_excel_upload_edit.asp','addreg','width=1280,height=400,scrollbars=yes,resizable=yes');
	popwin.focus();
}
</script>
...
<tr>
	...
	<td align="right">
		( <input type="text" name="excelsize" size="2">/<%=ExcelPasing(ioffitem.FTotalcount,3500) %> <input type="button" onclick="downloadexcel(<%=ExcelPasing(ioffitem.FTotalcount,3500) %>);" value="엑셀다운로드" class="button"> )
		<input type="button" class="button" value="엑셀업로드일괄수정" onclick="uploadexcel()">
...
<tr align="center" bgcolor="<%= adminColor("tabletop") %>">
	...
	<% if C_ADMIN_USER then %>
		<td width="70">
			ON/OFF<br>가격연동
			<br>
			<% drawSelectBoxisusingYN "checkallonofflinkyn","","" %>
	        <br><input class="button" type="button" value="선택적용" onClick="chcheckallonofflinkyn();">
		</td>
	<% end if %>
...
<% IF application("Svr_Info")="Dev" THEN %>
	<iframe id="view" name="view" src="" width=800 height=300 frameborder="0" scrolling="no"></iframe>
<% else %>
