...
function AddItems(frm){
	...
	popwin = window.open('popoffjumunitem.asp?suplyer=' + suplyer + '&idx=' + frm.masteridx.value ,'offjumuninputadd','width=880,height=600,scrollbars=yes,resizable=yes');
	popwin.focus();
}
...
<form name="frmMaster" method="post" action="doshopjumun.asp">
...
<table width="760" cellspacing="1" class="a" bgcolor=#3d3d3d>
	...
	%>
	<form name="frmBuyPrc_<%= i %>" method="post" action="doshopjumun.asp">
	<input type="hidden" name="mode" value="">
	<input type="hidden" name="masteridx" value="<%= idx %>">
...
%>
<form name="frmadd" method=post action="doshopjumun.asp">
...
<script language='javascript'>
if (jumunwait!=true) alert('주문접수 상태가 아니면 수정하실 수 없습니다.');
</script>
