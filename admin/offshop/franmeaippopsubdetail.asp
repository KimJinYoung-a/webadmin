		...
		if ((e.type=="checkbox")) {
			...
			if (e.checked){
				if (frm.elements[i+1].type="text"){
					...
					frm.buycasharr.value	= frm.buycasharr.value + frm.elements[i+4].value + ",";
					frm.suplycasharr.value = frm.suplycasharr.value + frm.elements[i+5].value + ",";
				}
			}
		}
	}
...
function AddNewDetail(idx,topidx,makerid){
	...
	var popwin = window.open('franetcjungsandetailadd.asp?idx=' + idx + '&topidx=' + topidx + '&makerid=' + makerid +'&shopid=<%= ofranchulgodetail.FOneItem.Fshopid %>','franetcjungsandetailadd','width=400, height=350, scrollbars=yes, resizable=yes');
	popwin.focus();
}
</script>
...
<form name=frmarr method=post action="meaipchulgojungsan_process.asp">
