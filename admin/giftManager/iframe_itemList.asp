...
if objView.ListType ="wish" or Div<>"" then
...
function popItemWindow(iid,v){
	...
	if (frm.cdL.value!=''&&frm.cdM.value!=''&&frm.cdS.value!=''){
		window.open("Pop_ViewitemList.asp?cd1=&target=" + frm, "addpop", "width=800,height=500,scrollbars=yes,status=no,resizable=yes");
	} else {
		alert('카테고리를 선택해 주세요');
	}
}
...
function AddItems(){
	...
	window.open("Item_Process.asp", "popup_item", "width=100,height=100,scrollbars=no,status=no,resizable=no");
...
<table width="780" border="0" cellpadding="1" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
	...
	<tr>
		<td colspan="11" bgcolor="#FFFFFF">
			<input type="radio" name="div" value="" <% if DIV="" then response.write "checked" %> onclick="frmsub();">일반상품
