	...
	If intTotalPage > 1 Then
		For intLoop = intStartBlock To intEndBlock
			...
			If Int(intLoop) = Int(intCurrentPage) Then		'현재 페이지
				Response.Write "&nbsp;<span class='menu_link3'><strong>" & intLoop & "</strong></span>&nbsp;"
			Else															'그 외 페이지
				Response.Write "&nbsp;<a href='javascript:document.frmPaging." & strCurrentPage & ".value=" & intLoop & ";document.frmPaging.submit();'>" & intLoop & "</font></a>&nbsp;"
			End If
		
		Next
	Else		'한 페이지만 존재 할때
		Response.Write "&nbsp;<span class='menu_link3'><strong>1</strong></span>&nbsp;"
	End If
