...
'// 리뉴얼 페이지
Dim isRenewalPath, renewalPaths, renewalPath
renewalPaths = Array( _
	"/admin/sitemaster/paidGift/" _
)
isRenewalPath = false
For Each renewalPath In renewalPaths
	If InStr(Request.ServerVariables("url"), renewalPath) > 0 Then
		isRenewalPath = true
		Exit For
	End If
Next
...
<% If isRenewalPath Then %>
	<link rel="stylesheet" href="/css/commonV20.css" type="text/css">
<% Else %>
...
<% if False and session("sslgnMethod")<>"S" then '// 2018-05-28, USB체크 안함, skyer9 %>
...
<body bgcolor="#F4F4F4" <% if False and session("sslgnMethod")<>"S" then %>onload="checkUSBKey()"<% end if %>>
