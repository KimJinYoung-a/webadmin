...
Class giftManagerMenu
	...
	Public Sub getMenuListLarge()
			...
			strSQL =" SELECT  LCode , LCodeNm ,OrderNo,isUsing" &_
					" FROM db_redribbon.dbo.tbl_redR_LMenu " &_
	...
	Public Sub getMenuListMid()
			...
			strSQL = " SELECT  LCode , MCode , MCodeNm , OrderNo,isUsing " &_
					" FROM db_redribbon.dbo.tbl_redR_MMenu " &_
	...
	Public Sub getMenuListSmall()
			...
			strSQL =" SELECT  LCode , MCode , SCode, SCodeNm , OrderNo,isUsing " &_
					" FROM db_redribbon.dbo.tbl_redR_SMenu " &_
					" WHERE LCode ='" & FRectCDL & "'" &_
...
CLASS giftManagerView
	...
	Public function getMenuView(byval cdL, byval cdM ,byval cdS)
			...
			dim strSQL,i
			strSQL =" [db_redribbon].dbo.ten_gift_MenuView "&_
					"  @cdL='" & cdL & "'" &_
					" ,@cdM='" & cdM & "'" &_
					" ,@cdS='" & cdS & "'"
...
CLASS giftManagerCls
	...
	Public Sub getGiftItemList()
		...
		dim strSQL 
			...
			strSQL =" [db_redribbon].[dbo].[ten_gift_ItemList_Tcnt] "&_
					" @cdL = '" & FRectCDL & "'" &_
					" ,@cdM = '" & FRectCDM & "'" &_
					" ,@cdS = '" & FRectCDS & "'" &_
			...
			strSQL =" [db_redribbon].[dbo].[ten_gift_ItemList] "&_
					" @cdL = '" & FRectCDL & "'" &_
					" ,@cdM = '" & FRectCDM & "'" &_
					" ,@cdS = '" & FRectCDS & "'" &_
					" ,@SortMethod ='" & FRectSort & "'" &_
	...
	public Sub getBestItemList()
		...
		strSQL =" [db_redribbon].[dbo].[ten_gift_BestItem_Tcnt] "&_
					" @Div = '" & FRectDiv & "'" &_
					" ,@cdL = '" & FRectCDL & "'" &_
					" ,@cdM = '" & FRectCDM & "'" &_
					" ,@cdS = '" & FRectCDS & "'" &_
		...
		strSQL =" [db_redribbon].dbo.ten_gift_BestItem "&_
					" @Div = '" & FRectDiv & "'" &_
					" ,@cdL = '" & FRectCDL & "'" &_
					" ,@cdM = '" & FRectCDM & "'" &_
					" ,@cdS = '" & FRectCDS & "'" &_
	...
	Public Function getGiftServiceMainList
	dim strSQL ,i
		If FUseYN = True Then
			FUseYN = "1"
		ElseIf FUseYN = False Then
			FUseYN = "0"
		End If
		If FGubun = "L" Then
			strSQL =" [db_redribbon].dbo.ten_GiftServiceMainList_Tcnt '" & FMstNo & "', '" & FUseYN & "' "
			'response.write strSQL
				rsget.CursorLocation = adUseClient
				rsget.CursorType = adOpenForwardOnly
				rsget.LockType = adLockReadOnly		
		...
		IF FTotCnt > 0 THEN
		strSQL =" [db_redribbon].dbo.ten_GiftServiceMainList '" & FGubun & "','" & FPageSize & "', '" & FMstNo & "', '" & FUseYN & "', '" & FCurrPage & "', '" & FAnniv & "', '" & FContent & "' "
		'dbget.close()	:	response.End
			...
			IF not rsget.EOF THEN
				getGiftServiceMainList = rsget.getRows()
			End IF
			rsget.Close
		End IF
	End Function
