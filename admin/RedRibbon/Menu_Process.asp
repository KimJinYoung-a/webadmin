...
dim LCode , MCode , SCode , CodeNm, OrderNo , SortMethod ,ListType,isUsing,guideoffimg
...
'// 메뉴 수정
		...
		SELECT CASE Depth
			CASE "L"
				strSQL =" UPDATE [db_redribbon].[dbo].[tbl_redR_LMenu] " &_
						" SET LCodeNm='" & CodeNm &"'" &_
						" , OrderNo = '" & OrderNo & "'" &_
						" , isUsing='" & isUsing &"'" &_
						" WHERE LCode='" & LCode &"'"
			CASE "M"
				strSQL =" UPDATE [db_redribbon].[dbo].[tbl_redR_MMenu] " &_
						...
						" and MCode ='" & MCode &"'"
			CASE "S"
				strSQL =" UPDATE [db_redribbon].[dbo].[tbl_redR_SMenu] " &_
		...
		END SELECT
				'// 카테고리 코드별 수정
				strSQL = strSQL &_
						" UPDATE [db_redribbon].[dbo].[tbl_redR_ViewMenu] " &_
...
'// 메뉴삭제
		...
		strSQL =" SELECT count(*) as count FROM [db_redribbon].[dbo].[tbl_redR_item] " &_
				" WHERE LCode='" & LCode &"'"
			...
			if cnt >0 then
				...
				SELECT CASE  Depth
					CASE "L"
						strSQL = strSQL & _
								" UPDATE [db_redribbon].[dbo].[tbl_redR_LMenu] " &_
								" SET IsUsing='N' " &_
								" WHERE LCode='" & LCode &"'"
						
					CASE "M"
						strSQL = strSQL & _
						 		" UPDATE [db_redribbon].[dbo].[tbl_redR_MMenu] " &_
						 		" SET IsUsing='N' " &_
								" WHERE LCode='" & LCode &"'"
					...
					CASE "S"
						...
						strSQL = strSQL & _
								" UPDATE [db_redribbon].[dbo].[tbl_redR_SMenu] " &_
								" SET IsUsing='N' " &_
								" WHERE LCode='" & LCode &"'" 
								IF MCode<>"" THEN
									strSQL=strSQL & " and MCode ='" & MCode & "' " 
...
elseif  mode="cashedit" then
		...
		if minCnt <> maxCnt then
			...
			strSQL =" DELETE [db_redribbon].[dbo].[tbl_redR_CashMenu]" &_
					" WHERE LCode ='" & LCode & "' " 
				...
				for i = 0 to minCnt  			
					strSQL = strSQL &_
							" INSERT INTO [db_redribbon].[dbo].[tbl_redR_CashMenu](LCode ,MCode ,SCode,MinCash ,MaxCash) " &_
							" VALUES ("
							IF LCode<>"" then
								strSQL = strSQL & "'" & LCode & "'"
							else
...
'// 메뉴 추가
		...
		SELECT CASE Depth
			CASE "L"
				strSQL =	" SELECT count(*) as count FROM [db_redribbon].[dbo].[tbl_redR_LMenu] " &_
							" WHERE LCode='" & LCode &"'"
			CASE "M"
				strSQL =	" SELECT count(*) as count FROM [db_redribbon].[dbo].[tbl_redR_MMenu] " &_
							" WHERE LCode='" & LCode &"'" &_
							" and MCode ='" & MCode &"'"
			CASE "S"
				strSQL =	" SELECT count(*) as count FROM [db_redribbon].[dbo].[tbl_redR_SMenu] " &_
							" WHERE LCode='" & LCode &"'" &_
							" and MCode ='" & MCode &"'" &_
							" and SCode ='" & SCode &"'"
		END SELECT
		...
		SELECT CASE Depth
			...
			CASE "L"
				strSQL ="INSERT INTO [db_redribbon].[dbo].[tbl_redR_LMenu] (LCode,LCodeNm,OrderNo)" &_
						" VALUES ('" & LCode &"','" & CodeNm &"','" & OrderNo &"') " &_
						
						"INSERT INTO  [db_redribbon].[dbo].[tbl_redR_ViewMenu] (LCode,LCodeNm,LCodeImgOn,LCodeImgOFF,ListType,SortMethod,OrderNo)" &_
						"VALUES ('" & LCode & "','" & CodeNm & "','" & LCodeImgOn & "','" & LCodeImgOFF & "','" & ListType & "','" & SortMethod & "','" & OrderNo & "')" &_
			...
			CASE "M"
				strSQL =" INSERT INTO [db_redribbon].[dbo].[tbl_redR_MMenu] (LCode,MCode,MCodeNm,OrderNo)" &_
						" VALUES ('" & LCode &"','" & MCode &"','" & CodeNm &"','" & OrderNo &"') " &_
						
						" INSERT INTO  [db_redribbon].[dbo].[tbl_redR_ViewMenu] (LCode,MCode,LCodeNm,MCodeNm,MCodeTopImg,ListType,SortMethod,OrderNo)" &_
						" SELECT top 1 LCode,'" & MCode &"',LCodeNm,'" & CodeNm & "','" & MCodeTopImg & "',ListType,'" & SortMethod & "','" & OrderNo & "'" &_
						" FROM db_redribbon.dbo.tbl_redR_ViewMenu  " &_
						" WHERE Lcode='" & LCode & "' "&_
			...
			CASE "S"
				strSQL ="INSERT INTO [db_redribbon].[dbo].[tbl_redR_SMenu] (LCode,MCode,SCode,SCodeNm,OrderNo)" &_
						" VALUES ('" & LCode &"','" & MCode &"','" & SCode & "','" & CodeNm &"','" & OrderNo &"') " &_
						
						" INSERT INTO  [db_redribbon].[dbo].[tbl_redR_ViewMenu] (LCode,MCode,SCode,LCodeNm,MCodeNm,SCodeNm,GuideListImg,GuideTopImg,ListType,SortMethod,OrderNo)" &_
