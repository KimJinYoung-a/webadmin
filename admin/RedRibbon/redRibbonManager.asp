...
function PopEditMenu(dep,cdl,cdm,cds){
	window.open('Pop_Menu_Edit.asp?Depth=' + dep + '&cdl=' + cdl + '&cdm=' + cdm + '&cds=' +cds,'pop','width=500,height=300,resizable=yes');
}
...
function CreateMenu(){
	window.open('<%= wwwUrl %>/chtml/make_RedRibbon_LeftMenu.asp','pop','width=500,height=300');
}
function fnDelDisp(bl){
	document.location="?cdL=<%= cdL %>&cdM=<%= cdM %>&cdS=<%= cdS %>&deldisp="+ bl;
}
function GiftManager(){
	window.open('Pop_Gift_Mng.asp','gift_manager','width=500,height=600');
}
</script>
	...
	<td valign="top" width="220">
		<table border="0" cellpadding="2" cellspacing="1" class="a">
			<tr>
				...
				<td><input type="checkbox" name="deldisp" <% IF deldisp="true" then response.write "checked" %> onclick="fnDelDisp(this.checked);">삭제메뉴보기</td>
			</tr>
		</table>
		<table border="0" cellpadding="2" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
			...
			<tr bgcolor="<%= defbgColor(cdL,objMnL.FItemList(i).LCode,objMnL.FItemList(i).isUsing) %>">
				...
				<td align="center"><a href="?cdL=<%= objMnL.FItemList(i).LCode %>&deldisp=<%= deldisp %>"><%= objMnL.FItemList(i).LCodeNm %></a></td>
				<td align="center">
		...
		<% end if %>
		<br><br>
		<center><hr><input type="button" class="button" value="Gift Service 관리" onclick="GiftManager();"><hr></center>
	</td>
