...
<%
Dim ocontents,i,page , keyid , keyword , isusing
	menupos = request("menupos")
	page = request("page")
	keyid = request("keyidsearch")
...
set ocontents = new cword_list
	...
	ocontents.frectkeyword = keyword
	ocontents.frectisusing = isusing			
	ocontents.fword_list()
%>
...
//신규등록 & 수정
function reg(keyid){
	var reg = window.open('/admin/momo/word/word_reg.asp?keyid='+keyid,'reg','width=600,height=600,scrollbars=yes,resizable=yes');
	reg.focus();
}


//코맨트보기
function regcomment(keyid){
	var regcomment = window.open('/admin/momo/word/word_comment_list.asp?keyid='+keyid,'regcomment','width=1024,height=768,scrollbars=yes,resizable=yes');
	regcomment.focus();
}
		...
		for (var i=0;i<document.forms.length;i++){
			...
			if (frm.name.substr(0,9)=="frmBuyPrc") {
				if (frm.cksel.checked){
					upfrm.keyid.value = upfrm.keyid.value + frm.keyid.value + "," ;
			...
			var tot;
			tot = upfrm.keyid.value;
			upfrm.keyid.value = ""
		var changestats;

		changestats = window.open("/admin/momo/word/word_process.asp?keyid=" +tot + "&mode=ing" , "changestats","width=400,height=300,scrollbars=yes,resizable=yes");
		changestats.focus();
}
...
<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
	...
	<tr align="center" bgcolor="#FFFFFF" >
		...
		<td align="left">
			keyid:<input type="text" name="keyidsearch" value="<%=keyid%>" size=10>
			&nbsp; Keyword:<input type="text" name="keyword" value="<%=keyword%>" size=20>
			&nbsp; 사용여부:
...
<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
	...
	<% end if %>
		...
		<td align="center">
			<%= ocontents.FItemList(i).fkeyid %><input type="hidden" name="keyid" value="<%= ocontents.FItemList(i).fkeyid %>">
		</td>
		<td align="center">
			<%= ocontents.FItemList(i).fkeyword %>
		</td>
			
		<td align="center">
			<img src="<%=webImgUrl%>/momo/word/main/<%= ocontents.FItemList(i).fmainimage %>" width=50 height=50>
		</td>
		...
		<td align="center">
			...
			<a href="javascript:regcomment(<%= ocontents.FItemList(i).fkeyid %>)" onfocus="this.blur();"><%= ocontents.FItemList(i).fcommentcount %></a>
			<% else %>
			<%= ocontents.FItemList(i).fcommentcount %>
			<% end if %>
		</td>			
		<td align="center"><input type="button" onclick="reg(<%= ocontents.FItemList(i).fkeyid %>);" class="button" value="수정"></td>
    </tr>   
	</form>
	<% next %>
	<% else %>
