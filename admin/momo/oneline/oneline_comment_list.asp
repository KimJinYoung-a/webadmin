...
dim ocomment , i
set ocomment = new coneline_list
	ocomment.FPageSize = 20
	ocomment.FCurrPage = page
	ocomment.frectonelineid = onelineid
	ocomment.fonelinecomment_list()
%>

<script language="javascript">

	function comment_delete(idx){
		var comment_delete = window.open('oneline_comment_process.asp?idx='+idx,'comment_delete','width=800,height=600,scrollbars=yes,resizable=yes');
...
<% if ocomment.fresultcount > 0 then %>
	...
	<tr bgcolor="FFFFFF">
		<td align="center">
			<%= ocomment.fitemlist(i).fonelineid %>
		</td>	
		<td align="center">
    ...
    <tr height="25" bgcolor="FFFFFF">
		<td colspan="15" align="center">
	       	<% if ocomment.HasPreScroll then %>
				<span class="list_link"><a href="?page=<%= ocomment.StartScrollPage-1 %>&onelineid=<%=onelineid%>">[pre]</a></span>
			<% else %>
			[pre]
			<% end if %>
			<% for i = 0 + ocomment.StartScrollPage to ocomment.StartScrollPage + ocomment.FScrollCount - 1 %>
				...
				<% else %>
				<a href="?page=<%= i %>&onelineid=<%=onelineid%>" class="list_link"><font color="#000000"><%= i %></font></a>
				<% end if %>
			<% next %>
			<% if ocomment.HasNextScroll then %>
				<span class="list_link"><a href="?page=<%= i %>&onelineid=<%=onelineid%>">[next]</a></span>
			<% else %>
			[next]
			<% end if %>
		</td>
