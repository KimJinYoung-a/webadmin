...
<%
dim menupos, page, mode, designer,suplyer,shopid,itemid
dim idx
...
if suplyer="10x10" then
	...
	'ioffitem.FRectDesignerjungsangubun = "'2','4'"
else
	if shopid="10x10" then
...
function search(frm){
	if ((frm.itemid.value.length<1)&&(frm.mode[0].checked)&&(frm.designer.value.length<1)){
		alert('브랜드를 선택 하세요.');
		frm.designer.focus();
		return;
	}
