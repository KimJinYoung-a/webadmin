...
if (application("Svr_Info")	= "Dev") then
  getPdfDownLinkUrlAdm = "/pdf/dncommercialinvoicePdf.asp?"&addparam
else
  getPdfDownLinkUrlAdm = "http://apps.10x10.co.kr/pdf/dncommercialinvoicePdf.asp?"&addparam
end if
'---------------------------------------------------------
%>
	<style type="text/css">
...
function jsPrint() {
	if (confirm("프린트하시겠습니까?") != true) {
		return;
	}

	document.getElementById("idButtonArea").style.display = "none";
	window.print();
}

function jsGoEXCEL(){
	  var popwin = window.open('/admin/fran/viewcommercialinvoice_excel.asp?<%= addparam %>','dnPdf','width=1024,height=768,scrollbars=yes,resizable=yes');
}
...
<div class="wrapper">
		...
		<div class="container">
			...
			<div class="vPad10">
				<table>
					<colgroup>
						<col width="80px" /><col width="" /><col width="" /><col width="70px" /><col width="60px" /><col width="70px" />
					</colgroup>
					<thead>
						<tr>
							<th>Item Code</th>
					...
					<tbody>
						...
						<tr>
							...
							<td class="lt"><%=arrList(6,intLoop)%></td>
							<td class="rt"><%'--shop기준 화폐단위가 won 이 아닐땐 해외값으로 계산 %>
							    ...
							    <% if (FALSE) then %>
    								...
    								<%else%>
    								<%=FormatNumber(arrList(13,intLoop),0)%><%=currencyChar%> <!-- arrList(8,intLoop)-->
    								<%end if%>
								<%end if%>
							</td>
							...
							<td class="rt">
							    ...
							    <% if (FALSE) then %>
    								<%IF  currencyunit <> "WON" THEN '상품 detail 리스트 합계처리
    									subdamount = subdamount + (arrList(13,intLoop)*arrList(11,intLoop))
    								%>
    								...
    								<%ELSE
    									 subdamount = subdamount + (arrList(13,intLoop)*arrList(11,intLoop))
    								 %>
    									<%=FormatNumber(arrList(13,intLoop)*arrList(11,intLoop),0)%> <%=currencyChar%>
    								<%END IF%>
								<%END IF%>
							</td>
						</tr>
                        ...
                        if (FALSE) then
    						if currencyunit <> "WON" THEN
    							...
    							totaldamount  = FormatNumber(subdamount+totalDeliverPriceForeign,0)&currencychar
	...
	<div class="btnArea tMar30 ct" id="idButtonArea">
		<button type="button" class="btn btnB1 btnWhite btnW185 lMar10" onClick="jsPrint();">인쇄하기</button>
