...
'--- pdf 전환처리----------------------------------------
addparam = "idx="&idx&"&ls="&loginsite&"&boxidx="&boxidx&"&cunit="&currencyunit&"&tpl="&tplcompanyid&"&ekey="&md5(idx&loginsite&boxidx)&"&jungsanidx=" & jungsanidx & "&workidx=" & workidx

if (application("Svr_Info")	= "Dev") then
  getPdfDownLinkUrlAdm = "/pdf/dnpacklinglistPdf.asp?"&addparam
else
  getPdfDownLinkUrlAdm = "http://apps.10x10.co.kr/pdf/dnpacklinglistPdf.asp?"&addparam
end if
...
function jsGoEXCEL(){
	  var popwin = window.open('/admin/fran/viewpacklinglist_excel.asp?<%= addparam %>','dnPdf','width=1024,height=768,scrollbars=yes,resizable=yes');
}
