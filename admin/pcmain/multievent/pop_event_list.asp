...
<!--
	...
	function jsSetEvtCont(ieC){
	  if(typeof(opener.document) == "object"){
	     ...
	     opener.location.href = "<%= request("pTarget") %>&eC="+ieC+"&dispOption=<%=dispOption%>";
		 <% else %>		 
		 opener.location.href = "event_insert.asp?menupos=<%=menupos%>&sDt=<%=sDt%>&eDt=<%=eDt%>&linktype=1&idx=<%=idx%>&eC="+ieC+"&dispOption=<%=dispOption%>";
		 <% end if %>
		 window.close();
	  }
	}
...
<input type="hidden" name="dispOption" value="<%=request("dispOption")%>">
