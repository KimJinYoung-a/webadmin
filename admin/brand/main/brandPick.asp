...
function AssignXmlReal(upfrm,imagecount){
	...
	AssignimageReal.location.href="<%=wwwUrl%>/chtml/street/Main_BrandPickJS.asp?idx=" +tot + '&imagecount='+imagecount;
...
function jsSort() {
	...
	document.frm.mode.value = 'brandpick';
	document.frm.action = '/admin/brand/main/mainSortnoProcess.asp';
	document.frm.submit();
}
//이미지신규등록 & 수정
function AddNewMainContents(idx){
	var AddNewMainContents = window.open('/admin/brand/main/brandPick_imagemake_contents.asp?idx='+ idx,'AddNewMainContents','width=1200,height=600,scrollbars=yes,resizable=yes');
...
<tr>
	<td align="left">
		...
		&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="javascript:AssignXmlReal(frm,4);"><img src="/images/refreshcpage.gif" border="0"> Real 적용</a>
		&nbsp;&nbsp;&nbsp;&nbsp;
...
<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
	<tr height="25" bgcolor="FFFFFF">
		<td colspan="15">검색결과 : <b><%=lbrand.FTotalCount%></b></td>
	</tr>
	...
	<input type="hidden" name="idx" value="<%= lbrand.FItemList(i).Fidx %>">
	    ...
	    <td align="center">
			...
			<% if instr(lbrand.FItemlist(i).FImagepath,"http://")>0 then %>
			<img width=40 height=40 src="<%= lbrand.FItemlist(i).FImagepath %>" border="0">
			<% else %>
