...
function OrderNotItemEvaluatedownloadexcel(){
	document.frm.target = "view";
	document.frm.action = "/admin/crm/OrderNotItemEvaluate_excel.asp";
	document.frm.submit();
	document.frm.target = "";
	document.frm.action = "";
}
...
<tr>
    ...
    <td align="right">
		<input type="button" onclick="OrderNotItemEvaluatedownloadexcel();" value="엑셀다운로드" class="button">
    </td>
</tr>
</table>
...
<tr bgcolor="FFFFFF">
	<td colspan="8">
		...
		&nbsp;
		페이지 : <b><%= page %>/ <%= oEvaluate.FTotalPage %></b>
	</td>
</tr>
...
<% if oEvaluate.FResultCount>0 then %>
<% for i=0 to oEvaluate.FResultCount-1 %>
<tr bgcolor="#FFFFFF" align="center">
    <td>
        <% if C_CriticInfoUserLV1 then %>
            <%= oEvaluate.FItemList(i).fuserid %>
        <% else %>
            <%= printUserId(oEvaluate.FItemList(i).fuserid,2,"*") %>
        <% end if %>
    </td>
    <td>
        <% if C_CriticInfoUserLV1 then %>
            <%= oEvaluate.FItemList(i).fusername %>
        <% else %>
            <%= printUserId(oEvaluate.FItemList(i).fusername,2,"*") %>
        <% end if %>
    </td>
    <td><%= oEvaluate.FItemList(i).fuserlevel %></td>
    <td><%= oEvaluate.FItemList(i).fpushYn %></td>
    <td><%= oEvaluate.FItemList(i).fsmsok %></td>
    <td><%= oEvaluate.FItemList(i).femailok %></td>
    <td><%= oEvaluate.FItemList(i).flastlogin %></td>
    <td></td>
</tr>
<% next %>
...
<tr bgcolor="FFFFFF">
	<td colspan="8" align="center">
		<% if oEvaluate.HasPreScroll then %>
		<a href="javascript:NextPage('<%= oEvaluate.StartScrollPage-1 %>')">[pre]</a>
		<% else %>
			[pre]
		<% end if %>

		<% for i=0 + oEvaluate.StartScrollPage to oEvaluate.FScrollCount + oEvaluate.StartScrollPage - 1 %>
			<% if i>oEvaluate.FTotalpage then Exit for %>
		...
		<% if oEvaluate.HasNextScroll then %>
