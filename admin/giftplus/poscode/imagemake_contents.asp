...
<form name="frmcontents" method="post" action="<%=uploadUrl%>/linkweb/giftplus/image_proc.asp" onsubmit="return false;" enctype="multipart/form-data">
...
<!--<input type="hidden" name="ckUserId" value="<%=request.Cookies("partner")("userid")%>">-->
	...
	<tr bgcolor="#FFFFFF">
	  ...
	  <% if oMainContents.FOneItem.Fidx<>"" then %>
	  <br><img src="<%=uploadUrl%>/giftplus/main/<%= oMainContents.FOneItem.fimagepath %>" border="0"> 
	  <br><%=staticImgUrl%>/giftplus/main/<%= oMainContents.FOneItem.fimagepath %>
	  <% end if %>
	  </td>
	</tr>
	<tr bgcolor="#FFFFFF">
	        ...
	        <% else %>
	            <% if poscode<>"" then %>
	                <% if oposcode.FOneItem.fimagetype="map" then %>
	                    <textarea name="linkpath" cols="60" rows="6"><map name="map<%=oposcode.FOneItem.fposcode%>"></map></textarea>
	                    <br>(이미지맵 변수값 변경 금지)
	                <% else %>
