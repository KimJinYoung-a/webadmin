...
<%
dim cdL, cdM, cdS, PageCount, Page,DIV , objT, objView,i , SortMethod
	cdL= request("cdL")
	cdM= request("cdM")
	cdS= request("cdS")
...
function popItemWindow(iid,v){
	...
	<% if objView.ListType = "menu" then %>
		if (frm.cdL.value!=''&&frm.cdM.value!=''&&frm.cdS.value!=''){
			...
			alert('대 중 소 카테고리를 모두 선택해 주세요');
		}
	<% else %>
		if (frm.cdL.value!=''&&frm.cdM.value!=''){
			window.open("pop_itemAddInfo.asp?cd1=&target=" + frm, "addpop", "width=800,height=500,scrollbars=yes,status=no,resizable=yes");
		} else {
			alert('대 중 카테고리를 선택해 주세요');
		}	
	<% end if %>
