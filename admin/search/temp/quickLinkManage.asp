...
function jsUploadImg(a,b){
	document.domain ="10x10.co.kr";
	var popupl;
	popupl = window.open('/admin/search/temp/pop_uploadimg.asp?folder=quick&span='+b+'&sname='+a+'','popupl','width=370,height=150');
	popupl.focus();
}
		...
		<div class="searchWrap inputWrap">
			...
			<table class="writeTb tMar10">
				...
				<tr>
					...
					<td>
						<% If vIdx <> "" Then
							Response.Write "<input type=hidden name=quicktype value=" & vQuickType & ">" & fnQuickLinkTypeName(vQuickType)
						Else %>
						<span class="rMar10"><input type="radio" id="attr1" name="quicktype" value="txt" onClick="goTypeChange('txt');" <%=CHKIIF(vQuickType="txt","checked","")%> /> <label for="attr1">텍스트형</label></span>
						<span class="rMar10"><input type="radio" id="attr2" name="quicktype" value="nor" onClick="goTypeChange('nor');" <%=CHKIIF(vQuickType="nor","checked","")%> /> <label for="attr2">기본형</label></span>
						<span class="rMar10"><input type="radio" id="attr3" name="quicktype" value="set" onClick="goTypeChange('set');" <%=CHKIIF(vQuickType="set","checked","")%> /> <label for="attr3">설정형</label></span>
						<span class="rMar10"><input type="radio" id="attr4" name="quicktype" value="brd" onClick="goTypeChange('brd');" <%=CHKIIF(vQuickType="brd","checked","")%> /> <label for="attr4">브랜드형</label></span>
						<span class="rMar10"><input type="radio" id="attr5" name="quicktype" value="cus" onClick="goTypeChange('cus');" <%=CHKIIF(vQuickType="cus","checked","")%> /> <label for="attr5">커스텀형</label></span>
						<% End If %>
					</td>
				</tr>
				<tr>
					...
					<td><input type="text" class="formTxt" value="<%=vName%>" id="quickname" name="quickname" placeholder="퀵링크명을 입력해주세요." style="width:45%" maxlength="20" />
						<% If vQuickType = "brd" Then %>
						&nbsp;<input type="button" class="btn3" value="브랜드 검색" onClick="jsSearchBrandID1(this.form.name,'quickname','quickbrid')">
						&nbsp;&nbsp;브랜드ID : <input type="text" class="formTxt" value="<%=vQuickBrID%>" id="quickbrid" name="quickbrid" style="width:27%" readonly />
						<% End If %>
					</td>
				</tr>
				<tr>
						...
						<p class="tPad05">
							<div class="selectLink">
								...
								<ul style="display:none;" id="selectbtnLinkm">
									...
									<li onclick="populateTextBoxL('/category/category_itemPrd.asp?itemid=상품코드','m');">/category/category_prd.asp?itemid=상품코드</li>
									<li onclick="populateTextBoxL('/category/category_list.asp?disp=카테고리','m');">/shopping/category_list.asp?disp=카테고리</li>
