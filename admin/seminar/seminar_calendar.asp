...
function popSeminarRoom(){
	window.open('popSeminarRoom.asp','roomcode','width=410,height=600');
}
...
<tr><td><input type="button" value="관리" onclick="popSeminarRoom();" class="button"></td></tr>
</table>
<% End If %>
<table border="0" cellpadding="0" cellspacing="0">
<tr>
	<td>
		...
		<tr align="center" valign="middle">
			<td height="25">
				<A HREF="seminar_calendar.asp?ThisDate=<%=PrevThisYearDate%>&menupos=<%=menupos%>">◀</A>&nbsp;&nbsp;<%=ThisYear%>&nbsp;&nbsp;
				<A HREF="seminar_calendar.asp?ThisDate=<%=NextThisYearDate%>&menupos=<%=menupos%>">▶</A>&nbsp;
				<A HREF="seminar_calendar.asp?ThisDate=<%=PrevThisMonthDate%>&menupos=<%=menupos%>">◀</A>&nbsp;&nbsp;<%=ThisMonth%>&nbsp;&nbsp;
				<A HREF="seminar_calendar.asp?ThisDate=<%=NextThisMonthDate%>&menupos=<%=menupos%>">▶</A>
			</td>
		</tr>
	</table>
	<br>
  	...
  	Response.Write "<tr>"&vbCR			' vbCR = ch<13>  : 엔터
  		...
  		ELSE
  			...
  			ELSE
  				...
  				Response.Write "<td height=80 align=left valign=top id=cell" + Cstr(PrintDay) + " onMouseOver='myColor(" + Cstr(PrintDay) + ")' onMouseOut='myColorOut(" + Cstr(PrintDay) + ")' style='cursor:hand; background-color:rgb(255, 255, 204);' onclick=GoSeminar('" + Cstr(DateSerial(Cstr(ThisYear),Cstr(ThisMonth),Cstr(PrintDay))) + "')><table class=verdana-small><tr><td><font color=#990000><b>"&PrintDay&"</b></font></td>"
