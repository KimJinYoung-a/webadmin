...
<table width="800" border="0" cellpadding="5" cellspacing="0" bgcolor="#CCCCCC">
	...
	<tr>
		...
		구분선택 : <select name="divcd">
			<option value='0' <% if divcd = "0" then response.write "selected" %>>맞교환</option>
			<option value='1' <% if divcd = "1" then response.write "selected" %>>누락재발송</option>
			<option value='2' <% if divcd = "2" then response.write "selected" %>>서비스발송</option>
		</select>
		</td>
