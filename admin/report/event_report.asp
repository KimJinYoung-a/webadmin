...
eventid = request("eventid2")
end if

if grouptype="" then grouptype="T"
...
<table width="800" border="0" cellpadding="5" cellspacing="0" bgcolor="#CCCCCC">
	...
	<tr>
		...
		<input type="radio" name="egubun" value="T" <% If egubun = "T" Then response.write "checked" %> disabled >이벤트명<input type="radio" name="egubun" value="N" <% If egubun = "N" Or egubun = ""  Then response.write "checked" %>>이벤트번호<br>
		<!-- &nbsp;이벤트 선택 : -->
		<% 'SelectBoxEvent eventid %>
		&nbsp;이벤트 번호 : <input type="text" name="eventid2" size="10" value="<%= eventid %>">

		정렬타입 : <input type="radio" name="grouptype" value="T" <% If grouptype = "T" Then response.write "checked" %>  >날짜별<input type="radio" name="grouptype" value="N" <% If grouptype = "N" Or grouptype = ""  Then response.write "checked" %>>상품별<br>
		</td>
...
<%
totalprice = totalprice + ototal.FMasterItemList(i).FTotalPrice
totalea = totalea + ototal.FMasterItemList(i).FTotalEa
%>
<% next %>
    <tr bgcolor="#FFFFFF">
    	<td width="100" align="center">총합</td>
    	<td width="100" align="right"><%= FormatNumber(totalprice,0) %></td>
    	<td width="100" align="right"><%= totalea %>개</td>
    </tr>
<% end if %>
</table>
<% end if %>
