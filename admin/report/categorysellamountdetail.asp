...
oreport.SearchCategorySellrePortDetail
...
<table width="100%" border="0" cellspacing="1" cellpadding="3" bgcolor="#EFBE00">
        ...
        <tr bgcolor="#FFFFFF">
		  <td width="140" height="10" class="a">
          	<a href="categorysellamountsubdetail.asp?menupos=221&codelarge=<%= codelarge %>&codemid=<%= oreport.FItemList(i).FCMid %>&order_desum=<%= order_desum %>&yyyy1=<%= yyyy1 %>&mm1=<%= Format00(2,mm1) %>&dd1=<%= Format00(2,dd1) %>&yyyy2=<%= yyyy2 %>&mm2=<%= Format00(2,mm2) %>&dd2=<%= Format00(2,dd2) %>&oldlist=<%= oldlist %>"><%= oreport.FItemList(i).FCMName %> (<%= oreport.FItemList(i).FCmid %>)</a>
          </td>
