...
SET oChart = new CChart
	...
	vArr1 = oChart.fnTimeMeachul_trend_DW
...
function popTimeBestDetail(yyyymmdd,hh,ichannel, ordtype){
    var frm = document.frm1;
    if (ordtype=="1") ordtype="C"
    if (ordtype=="2") ordtype="S"
    
    var iURL = "/admin/dataanalysis/chart/timeorder_trend_bestitembytime.asp?startdate="+yyyymmdd+"&hh="+hh
    iURL=iURL+"&channel="+ichannel+"&ordtype="+ordtype+"&rdsitegrp=<%=rdsitegrp%>";
    
    var popwin = window.open(iURL,"popTimeBestDetail","width=1200,height=600,scrollbars=yes,resizable=yes")
    popwin.focus();
}
...
<tr bgcolor="#FFFFFF">
    ...
    <td width="560">
        ...
        <tr bgcolor="#FFFFFF" align="right">
            ...
            %>
            <td <% if (k=0) then response.write "onclick=""popTimeBestDetail('"&LEFT(vArr1(iDataSetPosArr2(k),i),10)&"','"&RIGHT(vArr1(iDataSetPosArr2(k),i),2)&"','"&vChannel&"','"&sTp&"')"" style=""cursor:pointer"" " end if %> >
                <% 
...
<tr bgcolor="#FFFFFF" >
    ...
    <td width="560" style="padding-top:20px">
        ...
        <tr bgcolor="#FFFFFF" align="right">
            ...
            %>
            <td <% if (k=0) then response.write "onclick=""popTimeBestDetail('"&LEFT(vArr1(iDataSetPosArrCh(ii)(k),i),10)&"','"&RIGHT(vArr1(iDataSetPosArrCh(ii)(k),i),2)&"','"&titleArrChCd(ii)&"','"&sTp&"')"" style=""cursor:pointer"" " end if %> >
                <% 
                if SumArrType(k)<=1 then
