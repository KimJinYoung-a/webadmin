...
Dim vSDate, vEDate, vChannel, rdsitegrp, vpValue, vOrdType, sTp, vpUpType
...
SET oChart = new CChart
	...
	vArr2 = oChart.fnOutSiteTop_DW
	
	oChart.FPageSize = 5
	vArr1 = oChart.fnOutSiteTop_Trend_DW
SET oChart = nothing

Dim iChartCaption : iChartCaption = "제휴사별 주문건수"
...
if (vOrdType="S") then
    iDataSetPosArr = Array(3)
    iDataSetHeadArr = Array("구매총액")
    
    iChartCaption = "제휴사별 구매총액"
    yAxisName = "구매총액"
end if

Dim pTypeName : pTypeName = "제휴사" ''getpTypeName(vpType)
...
<tr bgcolor="#FFFFFF">
    ...
    <td width="500">
        ...
        <tr bgcolor="#FFFFFF" align="right">
			...
			<td><a href="/admin/maechul/statistic/statistic_item_dw.asp?menupos=1726&syear=<%=year(vSDate)%>&smonth=<%=month(vSDate)%>&sday=<%=day(vSDate)%>&eyear=<%=year(vEDate)%>&emonth=<%=month(vEDate)%>&eday=<%=day(vEDate)%>&sitename=<%=vArr2(0,i)%>&sorting=<%=CHKIIF(vOrdType="C","itemno","itemcost") %>" target="_blank">보기</s></td>
        </tr>
        <% next %>
        </table>
    </td>
