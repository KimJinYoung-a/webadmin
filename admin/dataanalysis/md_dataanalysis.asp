			...
			if (channeltype[i] == tmpchanneltype){	//해당 되는 채널를 그린다
				...
				if (charttype[i] == 'barchart'){	//바차트 그리기
					...
					if (option1[i]!=''){	//등록한 옵션1이 있을경우
						...
						chartoption1 = {'orientation':'horizontal','isStacked':false}
					}
					if (ispretype){	//비교 체크 선택시
...
<% if C_ADMIN_AUTH then %>
	...
	<input type='text' name='goapiurl' id='goapiurl' size=160 value='<%= goapiurl %>' readonly>]<br><br>
<% else %>
	<input type='hidden' name='goapiurl' id='goapiurl' value='<%= goapiurl %>' readonly>
<% end if %>
...
<% '/목표 %>
<!-- #include virtual="/admin/dataanalysis/md_purpose.asp"-->
	...
	<% for i = 0 to cchart.FResultCount -1 %>
		<tr bgcolor="#FFFFFF">
			<td align="left" colspan=2>
				<Br>
				<% if cchart.FItemList(i).fcharttype="simpletablechart" or cchart.FItemList(i).fcharttype="datatablechart" then %><Br><Br><Br><% end if %>

				<div id='<%=cchart.FItemList(i).fcharttype %>Container_<%=cchart.FItemList(i).fchartidx %>' ><%= chkiif(i=0,"<img src='http://fiximage.10x10.co.kr/icons/loading16.gif' width=20 height=20>","") %></div>
