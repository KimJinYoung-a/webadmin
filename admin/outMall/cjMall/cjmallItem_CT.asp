...
Dim failCntExists, optExists, diffPrc, optnotExists
...
If (session("ssBctID")="kjy8517") Then
	...
	JYSQL = JYSQL & " db_item.dbo.tbl_item "
	JYSQL = JYSQL & " where itemid in ( "
	JYSQL = JYSQL & " 	select itemid from [db_item].[dbo].tbl_OutMall_regedoption where mallid = 'cjmall' and itemoption = '0000' and outmallsellyn = 'N' and lastupdate < '2013-10-07' group by itemid "
	JYSQL = JYSQL & " ) "
...
'	rsget.Open JYSQL, dbget
'	If Not(rsget.EOF or rsget.BOF) Then
'		For qq = 1 to rsget.RecordCount
'			if qq = rsget.RecordCount Then
'				ww = ww & trim(rsget("itemid"))
'			Else
'				ww = ww & trim(rsget("itemid"))&","
'			End If
'			rsget.MoveNext
...
function popManageOptAddPrc(iitemid,mngOptAdd){
    var pwin = window.open("/admin/outMall/popOptionAddPrcSet.asp?itemid="+iitemid+'&mallid=cjmall&mngOptAdd='+mngOptAdd,"popOptionAddPrc","width=800,height=600,scrollbars=yes,resizable=yes");
	pwin.focus();
}
...
function NotInMakerid(){
	var popwin=window.open('/admin/outMall/JaehyuMall_Not_In_Makerid.asp?mallgubun=cjmall','notin','width=300,height=500,scrollbars=yes,resizable=yes');
	popwin.focus();
}
// 등록제외 상품
function NotInItemid(){
	var popwin=window.open('/admin/outMall/JaehyuMall_Not_In_Itemid.asp?mallgubun=cjmall','notinItem','width=500,height=500,scrollbars=yes,resizable=yes');
	popwin.focus();
}
//카테고리 관리
function pop_CateManager() {
	var pCM = window.open("/admin/outMall/cjmall/popcjmallCateList.asp","popCateMancjMall","width=1000,height=600,scrollbars=yes,resizable=yes");
	pCM.focus();
}
//상품분류 관리
function pop_prdDivManager() {
	var pCM2 = window.open("/admin/outMall/cjmall/popcjmallprdDivList.asp","popprdDivcjMall","width=1000,height=600,scrollbars=yes,resizable=yes");
	pCM2.focus();
}
// 선택된 상품 일괄 등록
function CjregIMSI(isreg) {
	...
	if (isreg){
		if (confirm('CjMall에 선택하신 ' + chkSel + '개 상품을 예정 등록 하시겠습니까?')){
			...
			document.frmSvArr.action = "/admin/outMall/cjmall/actCjMallReq_CT.asp"
...
function CjSellynSubmit(yn){
	...
	if(yn == true){
		...
		xLink.location.href = "/admin/outMall/cjmall/actCjMallReq_CT.asp?cmdparam=LIST&sday="+document.getElementById('theday').value+"";
	}else{
		xLink.location.href = "/admin/outMall/cjmall/actCjMallReq_CT.asp?cmdparam=DayLIST&sday="+document.getElementById('sday').value+"";
	}
...
function popCjCommCDSubmit() {
	...
	xLink.location.href = "/admin/outMall/cjmall/actCjMallReq_CT.asp?cmdparam=cjmallCommonCode&CommCD="+ccd+"";
}
function popOutMallEtcLink(itemid,mallid,poomok){
    var popwin = window.open('/admin/outMall/common/popOutMallEtcLink.asp?mallid='+mallid+'&itemid='+itemid+'&poomok='+poomok+'','popOutMallEtcLink','width=900,height=700,scrollbars=yes,resizable=yes');
    popwin.focus();
}
</script>
    ...
    <td style="padding:5 0 5 0">
	    ...
	    <tr>
	    	<td valign="top">
				...
				<% If (session("ssBctID")="icommang") or (session("ssBctID")="kjy8517") or (session("ssBctID")="cogusdk") or (session("ssBctID")="areum531") or (session("ssBctID")="therthis") or (session("ssBctID")="joohyun49") then %>
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="#CCCCCC">
<tr bgcolor="#FFFFFF">
	<td colspan="16" align="right" height="30">page: <%= FormatNumber(page,0) %> / <%= FormatNumber(cjMall.FTotalPage,0) %> 총건수: <%= FormatNumber(cjMall.FTotalCount,0) %></td>
</tr>
