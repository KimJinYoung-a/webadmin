...
<!--
	...
	function jsSetEvtCont(cd,title,subtitle,sale,csale,sdate,edate){
		 ...
		 opener.document.frmcontents.StartDate.value=sdate;
		 opener.document.frmcontents.EndDate.value=edate;
		 window.close();
	}
...
<tr>
	<td>
		...
		END If
			...
			%>
		<tr bgcolor="#FFFFFF" onClick="jsSetEvtCont(<%=arrList(0,intLoop)%>,'<%=title%>','<%=db2html(arrList(14,intLoop))%>','<%=db2html(arrList(12,intLoop))%>','<%=db2html(arrList(13,intLoop))%>','<%=sdate%>','<%=arrList(6,intLoop)%>');" style="cursor:hand;" onMouseOver="this.style.backgroundColor='#FFFFEC'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
