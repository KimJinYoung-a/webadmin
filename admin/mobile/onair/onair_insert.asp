...
' Discription : 모바일 onair
' History : 2013.12.15 이종화
'###############################################
%>
    ...
    $("#sDt").datepicker({
    	...
    	onClose: function( selectedDate ) {
    		$( "#eDt" ).datepicker( "option", "minDate", selectedDate );
			 document.frm.onairtitle.value = selectedDate +" "+ sTime + " 시작 onair 입니다";
			
			 if (document.frm.gubun[3].checked)
			 {
				nextdate(selectedDate);// 노출기간 다음날 표기 바꾸기
			 }
    	}
    });
    $("#eDt").datepicker({
...
function jsautotime(v){
	...
	if (v == 1){
		...
		frm.onairtitle.value= frm.sDt.value +" "+ frm.sTm.value + " 시작 onair 입니다";
	}else if (v == 2){
		frm.sDt.value = ndate;
		frm.eDt.value = ndate;
...
<tr bgcolor="#FFFFFF">
	<% If idx = ""  Then %>
	<td colspan="5" align="center" height="35">등록 진행 중 입니다.</td>
	<% Else %>
	<td bgcolor="#FFF999" colspan="5" align="center" height="35">수정 진행 중 입니다.</td>
	<% End If %>
</tr>
<% If idx <> ""  Then %>
<tr bgcolor="#FFFFFF">
...
<% End If %>
<tr bgcolor="#FFFFFF">
    <td bgcolor="#FFF999" align="center" width="15%">구분</td>
    <td colspan="3">
		&nbsp;<label for="t1">8시</label><input type="radio" id= "t1" name="gubun" value="1" onclick="jsautotime(1)" <%=chkiif(gubun="1" Or gubun="" ,"checked","")%>/>
		<label for="t2">12시</label><input type="radio" id= "t2" name="gubun" value="2" onclick="jsautotime(2)" <%=chkiif(gubun="2","checked","")%>/>
		<label for="t3">18시</label><input type="radio" id= "t3" name="gubun" value="3" onclick="jsautotime(3)" <%=chkiif(gubun="3","checked","")%>/>
		<label for="t4">23시</label><input type="radio" id= "t4" name="gubun" value="4" onclick="jsautotime(4)" <%=chkiif(gubun="4","checked","")%>/>
    </td>
</tr>
<tr bgcolor="#FFFFFF">
...
<tr bgcolor="#FFFFFF">
    ...
    <td colspan="3">
		<input type="text" name="onairtitle" size="100" value="<%If idx="" then%><%=sDt%>&nbsp;<%=sTm%>&nbsp;시작 onair 입니다<% Else %><%=onairtitle%><% End if%>" />
    </td>
</tr>
<tr bgcolor="#FFFFFF">
...
<tr bgcolor="#FFFFFF">
    <td bgcolor="#FFF999" align="center">타임쿠폰</td>
    <td width="40%">
		설명 : <input type="text" name="ctitle" size="30" value="<%=ctitle%>" /><br/>
		쿠폰종류 : <select name="cgubun" >
							<option value="I" <%=chkiif(cgubun="I","selected","")%>>상품쿠폰</option>
							<option value="B" <%=chkiif(cgubun="B","selected","")%>>보너스쿠폰쿠폰</option>
					   </select>
    </td>
	<td width="25%">
		할인률% : <input type="text" name="cper" size="5" value="<%=cper%>" />%
	</td>
	<td width="25%">
		쿠폰번호 : <input type="text" name="cnum" size="7" value="<%=cnum%>" />
	</td>
</tr>
<tr bgcolor="#FFFFFF" align="center">
    <td colspan="5"><input type="button" value=" 취 소 " onClick="jsgolist();"/><input type="button" value=" 저 장 " onClick="jsSubmit();"/></td>
</tr>
</form>
</table>
