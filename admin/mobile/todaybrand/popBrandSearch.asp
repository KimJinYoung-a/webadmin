...
function selectThis(makerid, dgncomment, brandimage){
    var arrItem, splitvar;
    opener.<%= frmName %>.makerid.value = makerid;
    opener.<%= frmName %>.subcopy.value = dgncomment;
    if (brandimage!="<%=staticImgUrl%>/brandstreet/main/"){
		brandimage = brandimage.replace("<%=staticImgUrl%>/brandstreet/main/","http://imgstatic.10x10.co.kr/brandstreet/main/");
	...
	$.ajax({
		type: "POST",
		url: "ajaxBrandBestItemSearch.asp",
		data: "makerid="+makerid+"&itemcnt=3",
		cache: false,
		success: function(message) {
			if(message) {
                arrItem = eval("[" + message + "]");

				$.each(arrItem, function(index, value){
					splitvar = value.split("|");
					if(index==2){
						$("#moreimg",opener.document).val(splitvar[1].replace("//webimage.10x10.co.kr/","http://webimage.10x10.co.kr/"));
						$("#moreimgsrc",opener.document).attr('src', splitvar[1]);
					}
					else{
						$("#itemid"+(index+1),opener.document).val(splitvar[0]);
						$("#lyItemInfo"+(index+1),opener.document).css("display", "");
                		$("#lyItemInfo"+(index+1),opener.document).html("<img src='" + splitvar[1] + "' height='70'/><br/>");
					}
				});
				window.close();
			} else {
...
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	...
	<% end if %>
		...
		<td><a href="javascript:selectThis('<%= opartner.FPartnerList(i).Fid %>','<%= replace(opartner.FPartnerList(i).Fdgncomment,"'","`") %>','<%= opartner.FPartnerList(i).Fbrandimage %>')"><%= opartner.FPartnerList(i).FID %></a></td>
		<td>
			<%= opartner.FPartnerList(i).FSocName_Kor %><br>
			<%= opartner.FPartnerList(i).FSocName %>
		</td>
		<td><a href="javascript:selectThis('<%= opartner.FPartnerList(i).Fid %>','<%= replace(opartner.FPartnerList(i).Fdgncomment,"'","`") %>','<%= opartner.FPartnerList(i).Fbrandimage %>')"><%= opartner.FPartnerList(i).Fcompany_name %></a></td>
