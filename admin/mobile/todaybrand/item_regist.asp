...
formName = requestCheckvar(request("formName"),16)
target = requestCheckvar(request("target"),10)
...
function addItem(itemid, itemimg, itemname, saleper, couponper){
	<% if target="itemid1" then %>		
	$("#itemid1",opener.document).val(itemid);
	$("#lyItemInfo1",opener.document).html("<img src='" + itemimg + "' height='70' id='item1img' />");
	$("#lyItemInfo1",opener.document).fadeIn();
	<% else %>
	$("#itemid2",opener.document).val(itemid);
	$("#lyItemInfo2",opener.document).html("<img src='" + itemimg + "' height='70' id='item2img' />");
	$("#lyItemInfo2",opener.document).fadeIn();
	<% end if %>
	window.close();
}
$(function(){
...
%>
	...
	<td>
		<%If oitem.FItemList(i).Fitemdiv="21" Then%><font color="#0000ff">[딜]</font><%End If%>&nbsp;<% =oitem.FItemList(i).Fitemname %>
