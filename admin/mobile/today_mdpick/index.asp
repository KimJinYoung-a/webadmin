...
function popupMainPreview(){			
	var popwin; 		
	popwin = window.open("/admin/sitemaster/main_preview.asp", "popup_main_preview", "width=500,height=400,scrollbars=yes,resizable=yes");
	popwin.focus();
}
-->
</script>
...
<form name="frm" method="post">
	...
	<tr align="center" bgcolor="#FFFFFF" >
		...
		<td width="50" bgcolor="<%= adminColor("gray") %>"><input type="button" class="button_s" value="미리보기" onClick="popupMainPreview()"></td>
...
<tr  height="30" align="center" bgcolor="<%=chkIIF(mdpickList.FItemList(i).Fisusing="Y","#FFFFFF","#F0F0F0")%>">
	...
	<td><%=chkiif(mdpickList.FItemList(i).Fuserlevelgubun="1","모든사용자","vip이상")%></td>	
	<!-- <td>
		<%
	...
	<td onclick="jsmodify('<%=mdpickList.FItemList(i).Fidx%>');" style="cursor:pointer;" align="left">
		<% 
			If mdpickList.FItemList(i).Fstartdate <> "" And mdpickList.FItemList(i).Fenddate Then 
				...
				If Clng(datediff("d", now() , mdpickList.FItemList(i).Fenddate)) < 0 Or Clng(datediff("h", now() , mdpickList.FItemList(i).Fenddate )) < 0  Then 
					Response.write " <span style=""color:red"">(종료)</span>"
				ElseIf Clng(datediff("d", mdpickList.FItemList(i).Fenddate , now())) < 1  Then '오늘날짜

					If Clng(datediff("h", now() , mdpickList.FItemList(i).Fenddate )) >= 0 And Clng(datediff("h", now() , mdpickList.FItemList(i).Fenddate )) < 24 Then ' 오늘
					Response.write " <span style=""color:red"">(약 "& Clng(datediff("h", now() , mdpickList.FItemList(i).Fenddate )) &" 시간후 종료)</span>"
					Else  ' 시작전
