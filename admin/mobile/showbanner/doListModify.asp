...
<%
	...
	for i=1 to request.form("chkIdx").count
		...
		Select Case mode
			Case "main"
				strSql = strSql & "Update [db_sitemaster].[dbo].tbl_mobile_showbanner_list Set "
				strSql = strSql & " viewno='" & sSortNo & "'"
				strSql = strSql & " Where showidx='" & sIdx & "';" & vbCrLf
			Case "sub"
				strSql = strSql & "Update [db_sitemaster].[dbo].tbl_mobile_showbanner_subitem Set "
				strSql = strSql & " sortnum='" & sSortNo & "'"
				strSql = strSql & " ,isusing='" & sIsUsing & "'"
				strSql = strSql & " Where showitemidx='" & sIdx & "';" & vbCrLf
		end Select
	next
