...
Dim sDt, sTm, eDt, eTm , gubun , onairtitle , saleper , prevDate , saleprice
...
If idx <> "" then
	dim oOnAirList
	set oOnAirList = new Cjustoneday
	...
	saleper			=	oOnAirList.FOneItem.Fsaleper
	saleprice		=	oOnAirList.FOneItem.Fsaleprice

	set oOnAirList = Nothing
End If

Dim oSubItemList
set oSubItemList = new Cjustoneday
...
<script type='text/javascript'>
	...
	function jsSubmit(){
		...
		if (getByteLength(frm.onairtitle.value) >= 50) {
			alert("제목을 짧게 입력하세요(" + getByteLength(frm.onairtitle.value) + "/50)");
			return;
		}
	...
	function jsgolist(){
		self.location.href="/admin/mobile/justoneday/";
	}
	$(function(){
	//달력대화창 설정
    ...
    $("#sDt").datepicker({
    	...
    	onClose: function( selectedDate ) {
    		$( "#eDt" ).datepicker( "option", "minDate", selectedDate );
			 document.frm.onairtitle.value = selectedDate +" 시작 justoneday 입니다";
...
function jsautotime(v){
	...
	if (v == 1){
		...
		frm.onairtitle.value= frm.sDt.value +" 시작 justoneday 입니다";
	}else if (v == 2){
		frm.sDt.value = ndate;
		frm.eDt.value = ndate;
...
<% if idx <> "" then %>
    var popwin = window.open("/admin/itemmaster/pop_itemAddInfo.asp?sellyn=Y&usingyn=Y&defaultmargin=0&acURL=/admin/mobile/justoneday/doSubRegItemCdArray.asp?listidx=<%=idx%>", "popup_item", "width=800,height=500,scrollbars=yes,resizable=yes");
    popwin.focus();
<% else %>
	alert("템플릿 컨텐츠 정보를 먼저 등록해주세요.");
<% end if %>
...
<form name="frm" method="post" action="dojustoneday.asp">
...
<tr bgcolor="#FFFFFF">
	<% If idx = ""  Then %>
	<td colspan="4" align="center" height="35">등록 진행 중 입니다.</td>
	<% Else %>
	<td bgcolor="#FFF999" colspan="4" align="center" height="35">수정 진행 중 입니다.</td>
	<% End If %>
</tr>
<% If idx <> ""  Then %>
<tr bgcolor="#FFFFFF">
...
<tr bgcolor="#FFFFFF">
    ...
    <td colspan="3">
		<input type="text" name="onairtitle" size="100" value="<%If idx="" then%><%=sDt%>&nbsp;시작 justoneday 입니다<% Else %><%=onairtitle%><% End if%>" />
    </td>
</tr>
...
<tr bgcolor="#FFFFFF">
    <td bgcolor="#FFF999" align="center">할인률</td>
	<td><input type="text" name="saleper" value="<%=saleper%>"/>%</td>
	<td bgcolor="#FFF999" align="center">할인가</td>
	<td><input type="text" name="saleprice" value="<%=saleprice%>"/></td>
</tr>
...
<tr align="center" bgcolor="<%=chkIIF(oSubItemList.FItemList(lp).FIsUsing="Y","#FFFFFF","#F3F3F3")%>#FFFFFF">
	...
	<td>
		<img src="<%=oSubItemList.FItemList(lp).Fextraimg%>" height='50' />
	</td>
