...
<%
'###############################################
' PageName : hkw_insert.asp
' Discription : 모바일 hot keyword
...
If idx <> "" then
	dim hkwobannerOne
	set hkwobannerOne = new CMainbanner
	hkwobannerOne.FRectIdx = idx
	hkwobannerOne.GetOneContents()

	kword				=	hkwobannerOne.FOneItem.Fkword
	ktitle				=	hkwobannerOne.FOneItem.Fktitle
	kcontents			=	hkwobannerOne.FOneItem.Fkcontents
	kurl_mo				=	hkwobannerOne.FOneItem.Fkurl_mo
	kurl_app			=	hkwobannerOne.FOneItem.Fkurl_app
	appdiv				=	hkwobannerOne.FOneItem.Fappdiv
	appcate				=	hkwobannerOne.FOneItem.fappcate
	sortnum				=	hkwobannerOne.FOneItem.Fsortnum
	mainStartDate		=	hkwobannerOne.FOneItem.Fstartdate
	mainEndDate			=	hkwobannerOne.FOneItem.Fenddate 
	isusing				=	hkwobannerOne.FOneItem.Fisusing
	subImage1			=	hkwobannerOne.FOneItem.Fkwimg
	ordertext			=	hkwobannerOne.FOneItem.Fordertext
	itemid				=	hkwobannerOne.FOneItem.Fitemid
	itemname			=	hkwobannerOne.FOneItem.Fitemname
	smallimage			=	hkwobannerOne.FOneItem.Fsmallimage

	set hkwobannerOne = Nothing
End If 
...
//		}
		...
		if (frm.kurl_mo.value.indexOf("검색어") > 0 || frm.kurl_mo.value.indexOf("카테고리") > 0 || frm.kurl_mo.value.indexOf("이벤트번호") > 0 || frm.kurl_mo.value.indexOf("상품코드") > 0 || frm.kurl_mo.value.indexOf("브랜드아이디") > 0){
			alert("링크 값을 확인 해주세요.");
			frm.kurl_mo.focus();
			return;
		}
...
<form name="frm" method="post" action="<%=uploadUrl%>/linkweb/mobile/hkwbanner_proc.asp" enctype="multipart/form-data" style="margin:0px;">
