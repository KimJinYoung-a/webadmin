...
Dim qdate
Dim startdate , enddate , gubun , isusing , gcode
...
Select Case mode
	 Case "subadd"
		'subitem 신규 등록
		sqlStr = "Insert Into [db_sitemaster].[dbo].tbl_mobile_cate_mdpick_item " &_
	 ...
	 Case "submodify"
		'subitem 수정
		sqlStr = "Update [db_sitemaster].[dbo].tbl_mobile_cate_mdpick_item " &_
				" Set itemid='" & itemid & "'" &_
				" 	,sortnum='" & sortnum & "'" &_
				" 	,isusing='" & isusing & "'" &_
				" Where subidx=" & subidx
		...
		'// 페이지정보 최종 수정자 업데이트
		sqlStr = "Update [db_sitemaster].[dbo].tbl_mobile_cate_mdpick_list " + VbCrlf
...
'		response.end
	...
	Case "add"
		'신규 등록
		sqlStr = "Insert Into [db_sitemaster].[dbo].tbl_mobile_cate_mdpick_list " &_
					" (mdpicktitle, startdate , enddate , adminid , gnbcode ) values " &_
					" ('" & mdpicktitle &"'" &_
					" ,'" & startdate &"'" &_
					" ,'" & enddate &"'" &_
					" ,'" & session("ssBctId") &"'" &_
	...
	Case "modify"
		'내용 수정
		sqlStr = "Update [db_sitemaster].[dbo].tbl_mobile_cate_mdpick_list " &_
...
<!--
	// 목록으로 복귀
	alert("저장했습니다.");
	self.location = "index.asp?menupos=<%=menupos%>&prevDate=<%=qdate%>&gcode=<%=gcode%>";
//-->
</script>
<% End If %>
