...
Dim sDt, sTm, eDt, eTm , gubun , mdpicktitle , prevDate , gcode
...
If idx <> "" then
	...
	gcode				=	mdpickList.FOneItem.Fgnbcode
...
<script type='text/javascript'>
	...
	function jsgolist(){
		self.location.href="/admin/mobile/topmdpick/";
	}
	$(function(){
	//달력대화창 설정
...
<% if idx <> "" then %>
    var popwin = window.open("/admin/itemmaster/pop_itemAddInfo.asp?sellyn=Y&usingyn=Y&defaultmargin=0&acURL=/admin/mobile/topmdpick/doSubRegItemCdArray.asp?listidx=<%=idx%>", "popup_item", "width=800,height=500,scrollbars=yes,resizable=yes");
    popwin.focus();
<% else %>
	alert("템플릿 컨텐츠 정보를 먼저 등록해주세요.");
<% end if %>
...
<tr bgcolor="#FFFFFF">
    <td bgcolor="#FFF999" align="center" width="15%">GNB 선택</td>
    <td>
