...
<%
'###########################################################
' Description :  모바일 카테고리 메인 브랜드 작성/수정
...
dim idx, poscode, reload
	...
	dim oMainContents
		...
		oMainContents.GetOneBrandContents
...
<script language='javascript'>
	...
	function SaveMainContents(frm){
        ...
        if (frm.makerid.value.length<1){
	        alert('브랜드 아이디를 입력 하세요.');
	        frm.makerid.focus();
	        return;
	    }

        if (frm.mainimg.value.length<1){
	        alert('이미지를 업로드 해 주세요.');
	        frm.mainimg.focus();
	        return;
	    }

        if (frm.sub_copy.value.length<1){
	        alert('서브 카피를 입력 하세요.');
	        frm.sub_copy.focus();
	        return;
	    }
...
<div class="popWinV17">
	<h1>등록</h1>
	<form name="frmcontents" method="post" action="<%=uploadUrl%>/linkweb/doMobileCateBrandReg.asp" onsubmit="return false;" enctype="multipart/form-data">
			...
			<div class="ftLt col6">
				<table class="tbType1 writeTb">
                    ...
                    <tr>
						<th width="160">브랜드</th>
						<td>
							<input type="text" name="makerid" id="txtBrandName" class="formTxt" size=10 value="<%= oMainContents.FOneItem.Fmakerid %>"> <input type="button" value="브랜드 검색" onClick="jsSearchBrandIDNew(this.form.name,'makerid');"/>
						</td>
					</tr>
					<tr>
						<th width="160">이미지</th>
						<td>
							<input type="file" name="file1" value="" size="32" maxlength="32" class="formFile" accept="image/*" onchange="fileInfo(this)"> <input type="button" value="이미지 불러오기" onClick="jsSearchBrandImage(this.form.name);"/>
							<input type="hidden" name="mainimg" id="imgURL" value="<%= imgURL %>">
							<% if oMainContents.FOneItem.Fidx<>"" then %>
							<br>
								<% If imgURL = "" Then %>
					...
					<tr>
					  ...
					  <td><input type="text" name="sub_copy" value="<%=oMainContents.FOneItem.Fsub_copy%>" class="formTxt" style="width:94%" maxlength="128"> </td>
					</tr>
					<tr>
						<th width="160">링크값</th>
						<td>
				...
				<div class="lPad30 vTop" id="today_preview">
					...
					<section class="" style="width:375px;">
						<div class="thumbnail" id="img_box">
							<% If imgURL="" Then %>
							<img src="/images/admin_login_logo2.png" id="mainimg" />
							<% Else %>
