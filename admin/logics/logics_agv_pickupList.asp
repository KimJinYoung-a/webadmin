...
function ChulgoInput() {
	location.href="logics_agv_pickupinput.asp?menupos=<%= menupos %>";
}

function ChulgoEdit(masteridx) {
	location.href="logics_agv_pickupedit.asp?menupos=<%= menupos %>&idx=" + masteridx;
}
...
// CS맞교환 등록
function popcschange(){
	var popwin = window.open('<%= logicsUrl %>/V2/onLine/m_re_chulgo.asp?searchflag=B001&research=on&','cschange','width=1280,height=960,scrollbars=yes,resizable=yes');
	popwin.focus();
}

// 미배송 등록
function popmibeasongitem(){
	var popwin = window.open('/admin/ordermaster/misendlist.asp','mibeasongitem','width=1280,height=960,scrollbars=yes,resizable=yes');
	popwin.focus();
}

// 기타출고 등록
function popetcchulgo(){
	var popwin = window.open('/admin/newstorage/culgolist.asp?notalinkcode=on','etcchulgo','width=1280,height=960,scrollbars=yes,resizable=yes');
	popwin.focus();
}

// 업체반품 등록
function popminusorder(){
	var popwin = window.open('/admin/newstorage/orderlist.asp?minusjumun=on','minusorder','width=1280,height=960,scrollbars=yes,resizable=yes');
	popwin.focus();
}
...
<tr>
	...
	<td align="right">
		<input type="button" value="CS맞교환 등록" onclick="popcschange();" class="button" >
		<input type="button" value="미배송 등록" onclick="popmibeasongitem();" class="button" >
		<input type="button" value="기타출고 등록" onclick="popetcchulgo();" class="button" >
		<input type="button" value="업체반품 등록" onclick="popminusorder();" class="button" >
		<input type="button" class="button" value="엑셀등록" onclick="uploadexcel();">
		<input type="button" value="수기등록" onclick="ChulgoInput();" class="button" >
	</td>
</tr>
</table>
<!-- 액션 끝 -->
...
<% if oAGV.FResultCount >0 then %>
	...
	<tr bgcolor="#FFFFFF" height=24>
		...
		<td align="center">
		  	<%= oAGV.FItemList(i).FpickingOrderNo %>
		</td>
		<td>
