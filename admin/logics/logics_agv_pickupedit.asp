...
set oPickupDetail = new CAGVItems
	...
	oPickupDetail.GetPickupDetailList
...
<% end if %>
	...
	frmDetail.action = "logics_agv_pickup_process.asp";
	frmDetail.submit();
}
...
<% end if %>
	...
	if (frmDetail.chk.length) {
		for (var i = 0; i < frmDetail.chk.length; i++) {
            ...
            itemnoarr = itemnoarr + ',' + frmDetail.itemno[i].value;
		}
	} else {
		didxarr = didxarr + ',' + frmDetail.chk.value;
        itemnoarr = itemnoarr + ',' + frmDetail.itemno.value;
	}

    frmMaster.didxarr.value = didxarr;
    frmMaster.itemnoarr.value = itemnoarr;

    if (confirm("저장하시겠습니까?") != true) {
        return;
	}

    frmMaster.action = "logics_agv_pickup_process.asp";
	frmMaster.mode.value = "modi";
	frmMaster.submit();
}

function jsGotoList() {
    location.replace('logics_agv_pickupList.asp?menupos=<%= menupos %>');
}
...
function jsSendPickup() {
    ...
    url = url + '/agv/api.asp?mode=agvpickup&masteridx=<%= masteridx %>';
...
function jsSendPickupCancel() {
    ...
    url = url + '/agv/api.asp?mode=agvpickupdel&masteridx=<%= masteridx %>';
...
<table width="100%" border="0" align="center" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	...
	<tr align="center" bgcolor="#FFFFFF" height="30">
		...
		<td align="left">
            ...
            <input type="button" class="button" value=" 전송하기 " onclick="jsSendPickup()">
            <% end if %>
            <input type="button" class="button" value=" 전송취소 " onclick="jsSendPickupCancel()">
        </td>
		<td bgcolor="<%= adminColor("tabletop") %>">피킹지시 번호</td>
		<td align="left"><%= oPickupMaster.FOneItem.FpickingOrderNo %></td>
	</tr>
	<tr align="center" bgcolor="#FFFFFF" height="30">
...
<table width="100%" border="0" align="center" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	...
	<tr bgcolor="#FFFFFF">
		...
		<td>
            <input type="text" class="text" name="itemno" value="<%= oPickupDetail.FItemList(i).Fitemno %>" size="4" style="text-align:right;">

        </td>
        <td><%= oPickupDetail.FItemList(i).Fpickupno %></td>
        <td align="center"><div id="agvstock_<%= oPickupDetail.FItemList(i).FskuCd %>">-</div></td>
        <td>
			<% 'if oPickupDetail.FItemList(i).FskuCd="" or isnull(oPickupDetail.FItemList(i).FskuCd) then %>
				<!--<font color="red">AGV재고없음</font>-->
			<% 'end if %>
