...
select case mode
    '/////////// 수정시 mode값(agvregarr) 와 엑셀일괄등록(imgstatic/linkweb/item/agv/upload_AGV_item_excel.asp)도 같이 수정하셔야 합니다./////////
    case "write"
        ...
        '// 신규저장
		sqlStr = " select * from [db_aLogistics].[dbo].[tbl_agv_stock_invest_master] where 1=0"
		...
		for i=0 to UBound(itemgubunarr) - 1
			if (trim(itemgubunarr(i)) <> "") then
				...
				sqlStr = " insert into [db_aLogistics].[dbo].[tbl_agv_stock_invest_detail] " + VBCrlf
				sqlStr = sqlStr + " (masteridx, itemgubun, itemid, itemoption) " + VBCrlf
				sqlStr = sqlStr + " values('" & masteridx & "', '" & itemgubun & "'," & itemid & ", '" & itemoption & "') " & VBCrlf
	    ...
	    response.write "<script>location.replace('logics_agv_stockInvestList.asp?menupos="+menupos+"')</script>"
    case "deldetail"
        chk= chk + ",,"
	    chk = Split(chk, ",")
        ...
        for i=0 to UBound(chk) - 1
            if (trim(chk(i)) <> "") then
                sqlStr = " update [db_aLogistics].[dbo].[tbl_agv_stock_invest_detail] "
    ...
    case "adddetail"
		...
		for i=0 to UBound(itemgubunarr)
			if (trim(itemgubunarr(i)) <> "") then
                ...
                sqlStr = sqlStr + " select top 1 idx from [db_aLogistics].[dbo].[tbl_agv_stock_invest_detail] "
                sqlStr = sqlStr + " where masteridx = " & masteridx & " and itemgubun = '" & itemgubun & "' and itemid = " & itemid & " and itemoption = '" & itemoption & "' "
                sqlStr = sqlStr + " ) "
                sqlStr = sqlStr + " begin "
                sqlStr = sqlStr + "	insert into [db_aLogistics].[dbo].[tbl_agv_stock_invest_detail] " + VBCrlf
				sqlStr = sqlStr + " 	(masteridx, itemgubun, itemid, itemoption) " + VBCrlf
				sqlStr = sqlStr + " 	values('" & masteridx & "', '" & itemgubun & "'," & itemid & ", '" & itemoption & "') " & VBCrlf
                sqlStr = sqlStr + " end "
    ...
    '/////////// 수정시 mode값(write) 와 엑셀일괄등록(imgstatic/linkweb/item/agv/upload_AGV_item_excel.asp)도 같이 수정하셔야 합니다./////////
    ' AGV인터페이스 에 저장 처리
    case "agvregarr"
        ' 주문서관리
        ...
        response.write "	}else{"
        response.write "	    var popwin = window.open('/admin/logics/logics_agv_stockInvestList.asp','addreg','width=1280,height=960,scrollbars=yes,resizable=yes');"
...
function iteminforeg(masteridx)
    ...
    sqlStr = sqlStr + " from [db_aLogistics].[dbo].[tbl_agv_stock_invest_detail] as d with(noLock)" & vbcrlf
    ...
    sqlStr = sqlStr + " 	and oi.itemid in ( select itemid from [db_aLogistics].[dbo].[tbl_agv_stock_invest_detail] with(noLock) where deldt is NULL and masteridx = "& masteridx &" )" & vbcrlf
    ...
    sqlStr = sqlStr + " 	and ooi.itemid in ( select itemid from [db_aLogistics].[dbo].[tbl_agv_stock_invest_detail] with(noLock) where deldt is NULL and masteridx = "& masteridx &" )" & vbcrlf
