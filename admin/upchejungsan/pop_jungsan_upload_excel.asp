...
bufStr = "<table><tr><td>은행</td><td>계좌</td><td>정산금액</td><td>업체명</td><td>사업자등록번호</td><td>(주)텐바이텐</td></tr>"
...
for i=0 to ojungsan.FresultCount-1
	...
	if (searchtype = "withholding") then
		bufStr = bufStr & "</td><td class='txt'>" & ojungsan.FItemList(i).GetTotalWithHoldingJungSanSum
	else
		bufStr = bufStr & "</td><td class='txt'>" & ojungsan.FItemList(i).GetTotalSuplycash
	end if
	bufStr = bufStr & "</td><td class='txt'>" & Replace(Left(Trim(Replace(ojungsan.FItemList(i).Fcompany_name, " ", "")),9), ",", " ")
	bufStr = bufStr & "</td><td class='txt'>"&ojungsan.FItemList(i).Fcompany_no&"</td>"
