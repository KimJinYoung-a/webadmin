...
//계좌 추가등록
function jsViewAddAcc(){
	document.all.dvAddAcc.style.display="block";
}

//계좌추가등록처리
function jsAASubmit(){
	document.frmAA.submit();
}
</script>
...
<tr>
	...
	<tr>
			...
			<% if Not isReadOnly then %>
				<input type="button" class="button" value="저장" onClick="jsSubmitCust();">
			<% end if%>
			</td>
		</tr>
	<tr>
	...
	<tr>
		...
		<tr>
			<td  colspan="3">
		  <input type="button" class="button" value="계좌추가등록" onClick="jsViewAddAcc();"> 
		  <div id="dvAddAcc" style="display:none;"> 
		  	<form name="frmAA" method="post" action="">
		  	...
		  	<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a" border="0"  bgcolor="<%= adminColor("tablebg") %>">
				<tr>
					<td  bgcolor="<%= adminColor("tabletop") %>" align="center" width="80">은행명</td>
					<td  bgcolor="#FFFFFF">
						<select name="selBC" class="select"> 
							<option value="">--선택--</option>
							<%IF isArray(arrBank) THEN
								For intB = 0 To UBound(arrBank,2)
							%>
							<option value="<%=arrBank(0,intB)%>" ><%=arrBank(1,intB)%></option>
							<%
								Next
								END IF%>
						</select>
					...
					<td bgcolor="<%= adminColor("tabletop") %>" align="center" width="80"> 계좌번호:</td>
					<td   bgcolor="#FFFFFF"> <input type="text" name="sAN" value="" size="20" class="text" ></td>
					<td  bgcolor="<%= adminColor("tabletop") %>" align="center" width="80">예금주</td>
					<Td  bgcolor="#FFFFFF"><input type="text" name="sSN" value="" size="10" class="text" ></td>
				</tr>
			</table>
			</form>
			<input type="button" value="등록" class="button" onClick="jsAASubmit();">
		  </div>
			</td>
		</tr>
		</table>
