...
function jsSetPart(){
	...
	$('#frmReg input[name="chkV"]').each(function(i) {
		if ($(this).prop("checked") == true){
			sNM=$(this).next().val();	// document.frmReg.sNM[i].value;
			hidPM=$(this).next().next().next().val();	// document.frmReg.hidPM[i].value

			if(iCount==0){
				opener.document.frm.iP.value = $(this).val();
				opener.document.frm.sP.value = sNM;
			}else{
				opener.document.frm.iP.value = opener.document.frm.iP.value +","+ $(this).val();
				opener.document.frm.sP.value = opener.document.frm.sP.value +","+ sNM;
			}

			strT = strT+  "<tr><td  width='140' align='center' style='border-bottom:1px solid #BABABA;border-right:1px solid #BABABA;'>"+ hidPM +"</td><td width='140'  align='center' style='border-bottom:1px solid #BABABA;border-right:1px solid #BABABA;'>"+ sNM +"</td><td align='center' width='200'  style='border-bottom:1px solid #BABABA;border-right:1px solid #BABABA;'><input type='text' name='mPM' id='mPM'  value='' size='20' style='text-align:right;' onKeyUp=\"jsSetMoney('m','"+iCount+"','<%=iType%>');auto_amount(this.form,this)\" onKeypress=\"num_check()\" >원</td><td align='center' width=200  style='border-bottom:1px solid #BABABA;'><input type='text' name='iPM' id='iPM' value='' size='4'  style='text-align:right;' onKeyUp=jsSetMoney('i','"+iCount+"','<%=iType%>')>%</td></tr>";
			iCount = iCount  + 1;
		}
	});
	// 표준코드로 변경후 주석처리
	//for(i=0;i<$('#frmReg input[name="chkV"]').length;i++){
...
<form name="frmReg" id="frmReg" method="post" style="margin:0px;">
<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a">
<tr>
	<td>
		<!-- 상단 띠 시작 -->
