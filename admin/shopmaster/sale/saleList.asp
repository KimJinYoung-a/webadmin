...
'Call fnSetEventCommonCode '공통코드 어플리케이션 변수에 세팅
 ...
 isaleType      = requestCheckVar(Request("selST"),1)       '할인타입
 arrList = ""
...
<input type="hidden" name="menupos" value="<%=menupos%>">
	...
	<input type="hidden" name="iC">
  	<tr align="center" bgcolor="#FFFFFF" >
		...
		<option value="1" <%if isaleType= "1" THEN %>selected<%END IF%>>일반 할인</option>
		<option value="2" <%if isaleType = "2" THEN %>selected<%END IF%>>타임 특가</option>
		</select>		
		</td>
...
<table width="100%" border="0" align="center" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
    ...
    <tr align="center" bgcolor="#FFFFFF">    
    	...
    	<td align="left">&nbsp;<a href="javascript:jsMod(<%=arrList(0,intLoop)%>)" title="할인 정보수정"><%=db2html(ReplaceBracket(arrList(1,intLoop)))%></a></td>    	
    	...
    	<td><%if arrList(17,intLoop)=2 then%><font color="orange">타임특가</font><%else%>일반할인<%end if%></td>
    	...
    	<td> 
    			<%IF chkState = 1 THEN%><input type="button" value="실시간적용" class="button" onClick="jsSetRealSale(<%=arrList(0,intLoop)%>,<%=chkState%>);"> <%END IF%>
    	</td>    			
    	<td>
	...
	%>
	<tr>
		<td colspan="13" align="center" bgcolor="#FFFFFF">등록된 내용이 없습니다.</td>
	</tr>
	<%END IF%>
</table>    
<!-- 페이징처리 -->
