...
<%
'####################################################
' Description : 사은품 가져오기
' History : 2008.12.11 김정인 생성
'####################################################
%>
...
<!--
	...
	// 선택사은품 등록
	function jsSelImport(gc){
		document.getElementById('frameim').src='/admin/shopmaster/gift/frmGiftImport.asp?gC='+gc;
	}
//-->
</script>
<!---- 검색 ---->
<table width="100%" align="center" cellpadding="5" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
	...
	<tr  bgcolor="#FFFFFF">
		...
		&nbsp;배송:
		<select name="selDelivery" class="select" onChange="javascript:document.frmSearch.submit();">
...
<table width="100%" border="0" align="center" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
    ...
    %>
    <tr align="center" bgcolor="#FFFFFF">
    	<td nowrap><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%=arrList(0,intLoop)%></a></td>
    	...
    	<td align="left">&nbsp;<a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%=db2html(arrList(1,intLoop))%></a></td>
    	<td><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%=db2html(arrList(5,intLoop))%></a></td>
    	...
    	<td><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%=fnGetCommCodeArrDesc(arrgifttype,arrList(6,intLoop))%></a></td>
    	<td nowrap><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%=formatnumber(arrList(7,intLoop),0)%></a></td>
    	<td nowrap><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%=formatnumber(arrList(8,intLoop),0)%></a></td>
    	<td nowrap><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%=arrList(11,intLoop)%></a></td>
    	<td><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%IF arrList(9,intLoop) > 0 THEN%>[<%=arrList(9,intLoop)%>]<%=arrList(19,intLoop)%><%END IF%></a></td>
    	<td nowrap><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="<%IF arrList(22,intLoop) <> "" THEN %><%=arrList(22,intLoop)%><%END IF%>"><%=arrList(13,intLoop)%></a></td>
    	<td nowrap><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="<%IF arrList(23,intLoop) <> "" THEN %><%=arrList(23,intLoop)%><%END IF%>"><%=arrList(14,intLoop)%></a></td>
    	<td nowrap><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%=fnGetCommCodeArrDesc(arrgiftstatus,arrList(15,intLoop))%></a></td>
    	<td nowrap><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%IF arrList(12,intLoop) > 0 THEN%><%=arrList(12,intLoop)%><%END IF%></a></td>
    		<td nowrap><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%IF arrList(21,intLoop)="Y" THEN%><font color="#F08050">업체</font><%ELSE%><font color="#5080F0">텐바이텐</font><%END IF%></a></td>
    	<td nowrap><a href="javascript:jsSelImport(<%=arrList(0,intLoop)%>)" title="사은품 정보수정"><%=FormatDate(arrList(16,intLoop),"0000.00.00")%></a></td>
    	<td nowrap><input type="button" class="button" value="선택" onClick="jsSelImport(<%=arrList(0,intLoop)%>)"></td>
    </tr>
	<% Next
	ELSE
	%>
...
<% IF application("Svr_Info")="Dev" THEN %>
	<iframe name="frameim" id="frameim" src="" frameborder="0" width="100%" height="300"></iframe>
<% else %>
	<iframe name="frameim" id="frameim" src="" frameborder="0" width="0" height="0"></iframe>
<% end if %>
<!-- 표 하단바 끝-->
