...
Dim CUST_CD   : CUST_CD =  requestCheckvar(request.Form("CUST_CD"),13)   ''2017/04/11 수정.
...
ELSEIF (mode="regCardMeaip") then
    ...
    For i=LBound(chk2) to UBound(chk2)
        ...
        IF (opExpDailyCardidx<>"") THEN
            ...
            sqlStr = "db_SCM_LINK.dbo.sp_SCM2ERP_CARD_SALE_INPUT"
            ...
            if (RetErr<1) then
                rw "key:"&opExpDailyCardidx&":ERR:["&RetErr&"]"&retErrStr
            else
...
ELSEIF (mode="sendDocErp") then
    ...
    if (chkTaxKey="") and (taxKeyArr<>"") then chkTaxKey = taxKeyArr
    ...
    For i=LBound(chkTaxKey) to UBound(chkTaxKey)
        ...
        IF (taxKey<>"") THEN
            ...
            IF IsArray(ArrVal) then
            	...
            	RMK         = LEFT(ArrVal(19,0),60) ''LEFT/60추가 2015/04/06
            ...
            sqlStr = "select arap_gb, arap_nm from db_partner.dbo.tbl_TMS_BA_ARAP_CD"
            ...
            IF (arap_cd="390") or (arap_cd="20") or (arap_cd="174") THEN     '''선급금(390) 팀운영비(20) 전도금대체(174) 으로 등록된것은 잘못된거인듯..
                response.write "<script>alert('수지 항목 확인 요망 - ["&arap_cd&"] "&arap_nm&"');location.href='"&ref&"'</script>"
        	    dbget.Close()
        	    response.end
            ENd IF

            IF (Not (IsNULL(erpLinkType) or (erpLinkType=""))) then
...
'                END IF
            ...
            if (cust_cd="") or (IsNULL(cust_cd)) then
                if (Not IsICheByArrayProc) then
                    ...
                    retVal = fnGetOrMakeCUST(iCorpNo,taxKey,cust_cd)
                    ...
                    elseif (retVal=-9) then
                        response.write "<script>alert('[경영]재무회계>>ERP연동 ,거래처 등록 후 사용요망. 사업자번호 :"&iCorpNo&" 거래처 자동생성 불가.');location.href='"&ref&"'</script>"
            	        dbget.Close()
            	        response.end
                    end if
                end if
            ...
            if (LEN(replace(iCorpNo,"-",""))=13) then
                response.write "<script>alert('사업자번호 :"&iCorpNo&" 거래처 전송 불가. [임시]');location.href='"&ref&"'</script>"
    	        dbget.Close()
    	        response.end
            end if
...
'            end if
            ...
            sqlStr = "db_SCM_LINK.dbo.sp_SCM2ERP_DocInputByTaxKey"
            ...
            ELSE
                ...
                sqlStr = sqlStr & " ,erpLinkKey="&RetErr
                sqlStr = sqlStr & " where taxKey='"&taxKey&"'" & vbCRLF
                ...
                IF (matchType="9") then
                    ...
                    sqlStr = sqlStr & " ,erpDocLinkKey=(CASE WHEN erpDocLinkKey is NULL THEN "&RetErr&" ELSE erpDocLinkKey END)"
