...
if (mode="del") and (itemid<>"") then
	if (itemoption = "") then
		sqlStr = " delete from [db_item].[dbo].tbl_baljureg_item " + VbCrlf
...
if (mode="add") and (itemid<>"") and (divcd<>"") then
	if (Len(itemid) <= 8) then
		sqlStr = " insert into [db_item].[dbo].tbl_baljureg_item " + VbCrlf
		...
		sqlStr = sqlStr + " left join [db_item].[dbo].tbl_baljureg_item b on i.itemid=b.itemid and b.itemoption = '' " + VbCrlf
		...
		if BF_IsMaybeTenBarcode(itemid) then
			...
			sqlStr = " insert into [db_item].[dbo].tbl_baljureg_item " + VbCrlf
			sqlStr = sqlStr + " (itemid, itemoption, divcd, reguserid) "+ VbCrlf
			sqlStr = sqlStr + " select i.itemid, o.itemoption, '" & divcd & "', '" + reguserid + "'" + VbCrlf
			sqlStr = sqlStr + " from [db_item].[dbo].tbl_item i" + VbCrlf
			sqlStr = sqlStr + " join db_item.dbo.tbl_item_option o on i.itemid = o.itemid "
			sqlStr = sqlStr + " left join [db_item].[dbo].tbl_baljureg_item b on i.itemid=b.itemid and b.itemoption = o.itemoption " + VbCrlf
			sqlStr = sqlStr + " where i.itemid = " & bf_itemid & " and o.itemoption = '" & bf_itemoption & "' "
			sqlStr = sqlStr + " and b.itemid is null"
		else
			'// 범용바코드
			sqlStr = " insert into [db_item].[dbo].tbl_baljureg_item " + VbCrlf
			sqlStr = sqlStr + " (itemid, itemoption, divcd, reguserid) "+ VbCrlf
			sqlStr = sqlStr + " select i.itemid, s.itemoption, '" & divcd & "', '" + reguserid + "'" + VbCrlf
			sqlStr = sqlStr + " from [db_item].[dbo].tbl_item i" + VbCrlf
			sqlStr = sqlStr + " join db_item.dbo.tbl_item_option_stock s on i.itemid = s.itemid "
			sqlStr = sqlStr + " left join [db_item].[dbo].tbl_baljureg_item b on i.itemid=b.itemid and b.itemoption = s.itemoption " + VbCrlf
			sqlStr = sqlStr + " where s.barcode='" + CStr(itemid) + "'"
			sqlStr = sqlStr + " and b.itemid is null"
		end if
	end if
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="a" bgcolor="<%= adminColor("topbar") %>">
	...
	<tr height="25" valign="top">
        ...
        <td>
        	...
        	상품코드(전체옵션)/바코드(위탁옵션) : <input type="text" name="itemid" value="<%= itemid %>" Maxlength="20" size="13" AUTOCOMPLETE="off" onKeyPress="if (event.keyCode == 13){ AddItem(frmbar); return false;}">
...
<table width="100%" border="0" align="center" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
    ...
    <tr align="center" bgcolor="#FFFFFF">
		...
		<td>
			<%
			if (odanpumbalju.FItemList(i).Fmwdiv = "U") then
				response.write "<font color='red'><b>업배</b></font>"
			else
				response.write "텐배"
			end if
			%>
