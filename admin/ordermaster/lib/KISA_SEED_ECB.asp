...
private SUB Class_Initialize
encrypt = 0
Set seed_key = NEW KISA_SEED_KEY_4ECB
Call arrayinit(ecb_buffer, 0, 4)
buffer_length = 0
END SUB

public FUNCTION memcpy_int2ecb_buffer(byref src, src_offset, length)
Call memcpy_int2int(ecb_buffer, src, src_offset, length)
end FUNCTION

public FUNCTION set_byte_for_int_ecb_buffer(b_offset, value)
Call set_byte_for_int(ecb_buffer, b_offset, value)
end FUNCTION

public FUNCTION KISA_SEED_Encrypt_Block_forECB_ecb_buffer(byref v_out, out_offset, byref ks)
Call KISA_SEED_ECB.KISA_SEED_Encrypt_Block_forECB(ecb_buffer, 0, v_out, out_offset, ks)
end FUNCTION


END CLASS

CLASS KISA_SEED_ECB_C
...
public function KISA_SEED_Encrypt_Block_forECB(byref v_in, in_offset, byref v_out, out_offset, byref ks)
Dim LR(3)
Dim T(1)
...
private function KISA_SEED_Decrypt_Block_forECB(byref v_in, in_offset, byref v_out, out_offset, byref ks )
Dim LR(3)
Dim T(1)
Dim K
...
public function chartoint32_for_SEED_ECB(byref v_in, inLen)
	Dim data
	Dim len
	Dim i
...
int32tochar_for_SEED_ECB = data

end function


'SEED_ECB_init
'@brief SEED ECB 알고리즘 초기화 함수
'@param pInfo : ECB 내부에서 사용되는 구조체로써 유저가 변경하면 안된다.(메모리 할당되어 있어야 한다.)
'@param enc : 암호화 및 복호화 모드 지정
'@param pbszUserKey : 사용자가 지정하는 입력 키(16 byte)
'@return 0: pInfo 또는 pbszUserKey 포인터가 null일 경우,
'        1: 성공
public function SEED_ECB_init(byref pInfo, enc, byref pbszUserKey)
	Dim ABCD(3)
	Dim T(1)
	Dim SK
	...
	if isempty(pInfo) or isempty(pbszUserKey) then
		SEED_ECB_init = 0
		exit function
	end if
...
public function SEED_ECB_Process( byref pInfo, byref v_in, inLen, byref v_out, byref outLen )
	Dim nCurrentCount
	Dim in_offset
	Dim out_offset
	...
	if KISA_ENCRYPT = pInfo.encrypt then
		...
		Do While nCurrentCount <= inLen
			Call KISA_SEED_Encrypt_Block_forECB( v_in, in_offset, v_out, out_offset, pInfo.seed_key )
		...
		Call pInfo.memcpy_int2ecb_buffer( v_in, in_offset, pInfo.buffer_length )
	else
		in_offset = 0
		out_offset = 0
		pdwXOR_offset = 0
		...
		Do While nCurrentCount <= inLen
			Call KISA_SEED_Decrypt_Block_forECB( v_in, in_offset, v_out, out_offset, pInfo.seed_key )
		...
		outLen = nCurrentCount - BLOCK_SIZE_SEED
		Call pInfo.memcpy_int2ecb_buffer( v_out, out_offset-BLOCK_SIZE_SEED_INT, BLOCK_SIZE_SEED)
	end if
...
public function SEED_ECB_Close( byref pInfo, byref v_out, out_offset, byref outLen )
	Dim nPaddngLeng
	Dim i
	...
	if KISA_ENCRYPT = pInfo.encrypt then
		...
		for i=pInfo.buffer_length to BLOCK_SIZE_SEED-1
			Call pInfo.set_byte_for_int_ecb_buffer(i, nPaddngLeng)
		Next
		Call pInfo.KISA_SEED_Encrypt_Block_forECB_ecb_buffer( v_out, out_offset/4, pInfo.seed_key )

		outLen = BLOCK_SIZE_SEED

		SEED_ECB_Close = 1
	Else
		nPaddngLeng = get_byte_for_int(pInfo.ecb_buffer, BLOCK_SIZE_SEED-1)
...
public function SEED_ECB_Encrypt( byref pbszUserKey, byref message, message_offset, message_length )
	...
	Call SEED_ECB_init( info, KISA_ENCRYPT, pbszUserKey)
	...
	Call SEED_ECB_Process( info, data, nPlainTextLen, outbuf, nRetOutLeng )
	Call SEED_ECB_Close( info, outbuf, nRetOutLeng, nPaddingLeng )


	cdata = int32tochar_for_SEED_ECB(outbuf, nRetOutLeng+nPaddingLeng)
	Call arraycopy(pbszCipherText, cdata, nRetOutLeng+nPaddingLeng)

	SEED_ECB_Encrypt = pbszCipherText
...
public function SEED_ECB_Decrypt(byref pbszUserKey, byref message, message_offset, message_length)
	...
	Call SEED_ECB_init( info, KISA_DECRYPT, pbszUserKey)

	outlen = ( ((int)(nCipherTextLen/16))) *4
	ReDim outbuf(outlen-1)
	Call arrayinit(outbuf, 0, outlen)
	data = chartoint32_for_SEED_ECB(pbszCipherText, nCipherTextLen)

	Call SEED_ECB_Process( info, data, nCipherTextLen, outbuf, nRetOutLeng )
	If SEED_ECB_Close( info, outbuf, nRetOutLeng, nPaddingLeng ) = 1 Then
...
dim KISA_SEED_ECB
Set KISA_SEED_ECB = NEW KISA_SEED_ECB_C
%>
