...
<%
dim ojumun, page, daydiff, orderserial
...
function delitems(upfrm){
	...
	var ret = confirm('선택한 무통장 주문을 삭제하시겠습니까?');
	...
	<tr align="center" bgcolor="#FFFFFF" >
		...
		<td align="left">
			<select class="select" name="daydiff">
			    <!-- option value="5" <% if daydiff="5" then response.write "selected" %> >5일 이후</option -->
			...
			주문번호 : <input type="text" name="orderserial" value="<%=orderserial%>" size="20" maxlength="20">
		</td>
...
<% else %>
	...
	<tr align="center" bgcolor="#FFFFFF">
		...
		<td><%= printUserId(ojumun.FItemList(i).FUserID,2,"**") %></td>
		...
		<td><%= ojumun.FItemList(i).FSumPaymentEtc %></td>
