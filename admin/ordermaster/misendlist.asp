...
dim dplusFrom, dplusTo, realstockFrom, realstockTo, sellyn, danjongyn
    ...
    realstockFrom	= requestCheckVar(request("realstockFrom"), 32)
    realstockTo		= requestCheckVar(request("realstockTo"), 32)
    sellyn			= requestCheckVar(request("sellyn"), 32)
    danjongyn		= requestCheckVar(request("danjongyn"), 32)
...
set obalju = New CBalju
    ...
    if IsNumeric(realstockFrom) then
        obalju.FRectRealstockFrom = realstockFrom
    end if
    if IsNumeric(realstockTo) then
        obalju.FRectRealstockTo = realstockTo
    end if
    obalju.FRectSellYN = sellyn
    obalju.FRectDanjongYN = danjongyn

	''obalju.GetMiSendOrderDetailAll
	obalju.GetMiSendOrderDetailAll_NEW
...
function confirmSubmitNew() {
    ...
    if (confirm('저장 하시겠습니까?')){
		frmSubmit.action="/admin/ordermaster/domisendlist.asp";
...
function popMisendJumunByItem(makerid, itemid, itemoption) {
	var popwin = window.open('misendmaster_top.asp?designer=' + makerid + '&itemid=' + itemid + '&itemoption=' + itemoption + '&lackItemOnly=Y&inputyn=Y','popmisendjumunbyitemid','width=1600 height=600 scrollbars=yes resizable=yes')
    popwin.focus();
}
...
<tr align="center" bgcolor="#FFFFFF" >
	<td align="left" height="25">
        ...
        * 실사재고 :
		<input type="text" class="text" name="realstockFrom" value="<%= realstockFrom %>" size="4" maxlength="4">
		~
		<input type="text" class="text" name="realstockTo" value="<%= realstockTo %>" size="4" maxlength="4">
        * 판매여부 :
        <select class="select" name="sellyn">
            <option value=""></option>
            <option value="Y" <%= CHKIIF(sellyn="Y", "selected", "") %>>판매중</option>
            <option value="S" <%= CHKIIF(sellyn="S", "selected", "") %>>일시품절</option>
            <option value="N" <%= CHKIIF(sellyn="N", "selected", "") %>>판매안함</option>
        </select>
        * 단종여부 :
        <select class="select" name="danjongyn">
            <option value=""></option>
            <option value="N" <%= CHKIIF(danjongyn="N", "selected", "") %>>생산중</option>
            <option value="Y" <%= CHKIIF(danjongyn="Y", "selected", "") %>>단종</option>
            <option value="S" <%= CHKIIF(danjongyn="S", "selected", "") %>>재고부족</option>
            <option value="M" <%= CHKIIF(danjongyn="M", "selected", "") %>>MD품절</option>
        </select>
	</td>
</tr>
<tr align="center" bgcolor="#FFFFFF" >
...
<!-- 액션 시작 -->
<form name="frmSubmit" method="post" action="/admin/ordermaster/domisendlist.asp" onsubmit="return false;" style="margin:0px;">
...
<tr>
	<td align="left">
		...
		<input type="button" class="csbutton" value="과거내역삭제" onclick="deloldMisend()">
		&nbsp;
		-->
		<input type="button" class="csbutton" value="미배송사유저장" onclick="confirmSubmitNew();">
		<input type="button" class="csbutton" value=" 선택상품AGV인터페이스저장" onclick="regAGVArr();">
	</td>
	<td align="right">
...
<tr align="center" bgcolor="<%= adminColor("tabletop") %>">
    ...
    <td width="2"></td>
	<td width="35">주문<br />수량<br />(상품<br />준비)</td>
	<td width="35">미배<br />등록</td>
    <td width="2"></td>
...
<% end if %>
    ...
    <td></td>
	<td>
        <%= obalju.FBaljuDetailList(i).FItemNo %>
        <% if (obalju.FBaljuDetailList(i).FItemNo <> obalju.FBaljuDetailList(i).Fipkumdiv5*-1) then %>
        <br /><font color="red">(<%= obalju.FBaljuDetailList(i).Fipkumdiv5*-1 %>)</font>
        <% end if %>
    </td>
	<td>
		<font color="red"><%= obalju.FBaljuDetailList(i).FItemLackNo %></font>
		<input type="hidden" id="agvitemno<%= i %>" name="agvitemno<%= i %>" value="<%= obalju.FBaljuDetailList(i).FItemLackNo %>">
	</td>
    <td></td>
	<td><a href="javascript:popViewCurrentStock('10', <%= obalju.FBaljuDetailList(i).FItemID %>, '<%= obalju.FBaljuDetailList(i).FItemOption %>')"><%= obalju.FBaljuDetailList(i).Frealstock %></a></td>
	...
	<td align=center><a href="javascript:popMisendJumunByItem('<%= obalju.FBaljuDetailList(i).Fmakerid %>', '<%= obalju.FBaljuDetailList(i).FItemID %>', '<%= obalju.FBaljuDetailList(i).FItemOption %>')">보기</a></td>
