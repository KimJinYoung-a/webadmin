...
for i = 0 to UBound(chk)
	if (trim(chk(i)) <> "") then
        ...
        ''dbget.Execute sqlStr				'// 결제완료 상태 주문에 동시입력 기능 비활성화, 2021-01-12, skyer9
        ...
        sqlStr = sqlStr + " 	and T.itemlackno > 0 "			''미배등록 상품만
        ''rw sqlStr
        ''dbget.close : response.end
		...
		if (slcode = "05") then
            ...
            sqlStr = sqlStr + " and itemlackno > 0 "
            ''rw sqlStr
