...
Class cDispCate
	...
	Public Sub GetDispCateList()
		...
		sqlStr = sqlStr & " FROM [db_item].[dbo].[tbl_display_cate2] AS c " & vbCrLf
...
'rw sqlStr
	...
	Public Sub GetDispCateListSort()
		...
		If FRectCateCode <> "" Then
			sqlStr = "SELECT c.catename FROM [db_item].[dbo].[tbl_display_cate2] AS c WHERE c.catecode = '" & FRectCateCode & "' "
			rsget.Open sqlStr,dbget,1
			If not rsget.EOF Then
		...
		sqlStr = "SELECT c.catecode, c.catename, c.useyn, c.sortNo FROM [db_item].[dbo].[tbl_display_cate2] AS c "
	...
	Dim sqlStr, i, addsql
		...
		sqlStr = sqlStr & " , (select count(catecode) from [db_item].[dbo].[tbl_display_cate2] where Left(catecode," & Len(FRectCateCode) & ") = '" & FRectCateCode & "' AND useyn = 'Y' and depth = '" & (Len(FRectCateCode)/3)+1 & "') as downcatecount "
		sqlStr = sqlStr &"FROM [db_item].[dbo].[tbl_display_cate2] AS c WHERE c.catecode = '" & FRectCateCode & "'"
		rsget.Open sqlStr,dbget,1
		If Not rsget.Eof Then
	...
	Public Sub GetDispCateItemList()
		...
		'이전카테고리 필터링
		if FSearchDispCateOld<>"" then
			addsql = addsql & " AND i.itemid IN (Select itemid From db_item.dbo.tbl_display_cate_item Where Left(catecode," & Len(FSearchDispCateOld) & ") = '" & FSearchDispCateOld & "') "
		end if
		...
		sqlStr = sqlStr & " 	LEFT JOIN [db_item].[dbo].[tbl_display_cate2_item] AS i2 on i.itemid = i2.itemid " & vbCrLf
...
'rw sqlStr
		...
		sqlStr = sqlStr & " 		SELECT '|^|' + convert(varchar,dci.catecode) + '$' + ([db_item].[dbo].[getCate2CodeFullDepthName](dci.catecode)) " & vbCrLf
		sqlStr = sqlStr & " 		+ case when dci.isDefault = 'y' then '[기본]' else '' end " & vbCrLf
		sqlStr = sqlStr & " 		FROM [db_item].[dbo].[tbl_display_cate2] AS dc " & vbCrLf
		sqlStr = sqlStr & " 			INNER JOIN [db_item].[dbo].[tbl_display_cate2_item] AS dci on dc.catecode = dci.catecode " & vbCrLf
...
'rw sqlStr
	...
	Dim sqlStr, i, addsql
		...
		sqlStr = sqlStr & "	FROM [db_item].[dbo].[tbl_display_cate2_item] AS ci "
		sqlStr = sqlStr & "		INNER JOIN [db_item].[dbo].[tbl_display_cate2] AS c ON ci.catecode = c.catecode "
...
Function fnStandardDispCateSelectBox(depth, catecode, selname, selectedcode, onchange)
	...
	sqlStr = sqlStr & " FROM [db_item].[dbo].[tbl_display_cate2] "
