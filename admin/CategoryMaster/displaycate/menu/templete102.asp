...
function popCate(c){
	var catepop = window.open("pop_cate.asp?catecode=102&depth=2&inputname="+c+"","catepop","width=850,height=527, scrollbars=yes, resizable=yes");
	catepop.focus();
}

function jsItemReg(i){
	var itempop = window.open("pop_item.asp?catecode=102&itemid="+i+"","itempop","width=400,height=300, scrollbars=yes, resizable=yes");
	itempop.focus();
}
...
<%
	...
	End IF
%>
    var popCreateTemp = window.open("<%=vWWW%>/chtml/dispcate/menu_make_xml.asp?catecode=102&gb=temp","popCreateTemp","width=1200 height=930 scrollbars=yes resizable=yes");
	popCreateTemp.focus();
}
...
function jsSaveCateMenu(){
	...
	$("input[name=cate12code]").val($("input[name=cate12]").val());
		...
		<div class="gnbSub">
			...
			<div class="ftLt">
				<dl>
					...
					<dd>
						<ul>
							<li>12. <input type="text" name="cate12" value="<%=cate(11)%>" size="17" style="cursor:pointer;" onClick="jsSearchBrandIDn(this.form.name,'cate12');" readonly></li>
