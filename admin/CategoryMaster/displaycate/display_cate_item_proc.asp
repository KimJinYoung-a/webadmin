...
<%
	...
	If vAction = "update" OR vAction = "delete" Then
		vQuery = "SELECT count(catecode) FROM [db_item].[dbo].[tbl_display_cate_item] WHERE itemid = '" & vItemID & "'"
	...
	If vAction = "update" Then
		...
		ElseIf vTemp > 1 Then
			vQuery = "SELECT catecode FROM [db_item].[dbo].[tbl_display_cate_item] WHERE itemid = '" & vItemID & "' AND isDefault = 'y'"
			rsget.Open vQuery,dbget,1
			vTemp = rsget(0)
			rsget.close()
			If CStr(vTemp) = CStr(vCateCode) AND vIsDefault = "n" Then
				vQuery = "SELECT TOP 1 catecode FROM [db_item].[dbo].[tbl_display_cate_item] where itemid = '" & vItemID & "' AND isDefault = 'n' ORDER BY depth ASC, sortno ASC"
				rsget.Open vQuery,dbget,1
				vTemp = rsget(0)
				rsget.close()
				vQuery = "UPDATE [db_item].[dbo].[tbl_display_cate_item] SET isDefault = 'y' WHERE catecode = '" & vTemp & "' AND itemid = '" & vItemID & "' " & vbCrLf
			...
			If CStr(vTemp) <> CStr(vCateCode) AND vIsDefault = "y" Then		'### 이미 y가 있는데 다른카테고리를 y로 지정할경우 일단 같은 itemid 모두 n으로 변경.
				vQuery = "UPDATE [db_item].[dbo].[tbl_display_cate_item] SET isDefault = 'n' WHERE itemid = '" & vItemID & "'"
		...
		vQuery = "IF EXISTS(SELECT catecode FROM [db_item].[dbo].[tbl_display_cate_item] WHERE catecode = '" & vCateCode & "' AND itemid = '" & vItemID & "') " & vbCrLf
		vQuery = vQuery & "	BEGIN " & vbCrLf
		vQuery = vQuery & "		UPDATE [db_item].[dbo].[tbl_display_cate_item] SET " & vbCrLf
	...
	ElseIf vAction = "delete" Then
		...
		vQuery = "DELETE [db_item].[dbo].[tbl_display_cate_item] WHERE catecode = '" & vCateCode & "' AND itemid = '" & vItemID & "' " & vbCrLf
		dbget.execute vQuery
