...
function RefreshCategoryEventBanner(){
    if (confirm('적용하시겠습니까?')){
		 ...
		 refreshFrm.action = "<%=CHKIIF(application("Svr_Info")="Dev",wwwURL,"http://www1.10x10.co.kr")%>/chtml/dispcate/catemain_eventbanner_make.asp";
		 refreshFrm.submit();
    }
}
...
<table width="100%" align="center" cellpadding="0" cellspacing="0" class="a" style="padding-top:10;">
	<tr height="50">
		...
		<td align="right">		
			<input type="button" value="아이템 추가" onclick="self.location='/admin/categorymaster/category_main_EventBanner_input.asp?mode=add&catecode=<%= vCateCode %>&menupos=<%= menupos %>'" class="button">					
		</td>
	</tr>
</table>
<!-- 액션 끝 -->
...
<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
	...
	<tr align="center" bgcolor="<%= adminColor("tabletop") %>">
		...
		<td align="center">이미지//모바일카테고리 리스트 이미지(2014)</td>
    ...
    <% if omd.FItemList(i).fisusing = "Y" then %>
		...
		<td align="center">
		<% If Left(omd.FItemList(i).Fevt_bannerimg,1) = "S" Then %>
			<img src="http://webimage.10x10.co.kr/image/icon1/<%= GetImageSubFolderByItemid(omd.FItemList(i).Fevt_itemid) %>/<%= omd.FItemList(i).Fevt_bannerimg %>" width="100" border="0">
		<% Else %>
			<img src="<%= omd.FItemList(i).Fevt_bannerimg %>" width="100" border="0">
		<% End If %>
		&nbsp;&nbsp;&nbsp;<img src="<%= omd.FItemList(i).Fevt_molistbanner %>" width="200" height="90" border="0">
		</td>
