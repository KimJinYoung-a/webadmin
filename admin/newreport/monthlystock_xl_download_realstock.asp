...
' Description : 재고자산
' History : 이상구 생성
'			2023.05.04 한용민 수정(실사재고 추가, 재고 계산이 복잡해서 전체셀에 코맨트 넣음)
'###########################################################
%>
...
Dim FileName: FileName = "MonthlyRealStockAsset_"&sDateName&".csv"
dim fso, tFile
...
Function WriteMakeFile(tFile, arrList)
    ...
    For intLoop=0 to iRow
		...
		bufstr = bufstr & "," & arrList(65,intLoop)		' 기초재고(실사재고)

		if (bPriceGbn = "P") then
			bufstr = bufstr & "," & arrList(66,intLoop)
			...
			' 오차( (기초재고(실사재고)+매입(입고)+이동+판매+당월출고1+당월출고2+CS출고+당월기타출고)-실사재고+당월오차 )
			bufstr = bufstr & "," & ((arrList(65,intLoop) + arrList(12,intLoop) + arrList(56,intLoop) + arrList(14,intLoop) + (arrList(16,intLoop)+(arrList(56,intLoop)*-1)) + (arrList(18,intLoop)-arrList(20,intLoop)) + arrList(22,intLoop) + arrList(20,intLoop))-arrList(30,intLoop)+arrList(26,intLoop))*-1
			bufstr = bufstr & "," & ((arrList(66,intLoop) + arrList(13,intLoop) + arrList(57,intLoop) + arrList(15,intLoop) + (arrList(17,intLoop)+(arrList(57,intLoop)*-1)) + (arrList(19,intLoop)-arrList(21,intLoop)) + arrList(23,intLoop) + arrList(21,intLoop))-arrList(31,intLoop)+arrList(27,intLoop))*-1
			...
			bufstr = bufstr & "," & arrList(67,intLoop)
			bufstr = bufstr & "," & arrList(12,intLoop)		' 매입(입고)
			...
			' 오차( (기초재고(실사재고)+매입(입고)+이동+판매+당월출고1+당월출고2+CS출고+당월기타출고)-실사재고+당월오차 )
			bufstr = bufstr & "," & ((arrList(65,intLoop) + arrList(12,intLoop) + arrList(56,intLoop) + arrList(14,intLoop) + (arrList(16,intLoop)+(arrList(56,intLoop)*-1)) + (arrList(18,intLoop)-arrList(20,intLoop)) + arrList(22,intLoop) + arrList(20,intLoop))-arrList(30,intLoop)+arrList(26,intLoop))*-1
			bufstr = bufstr & "," & ((arrList(67,intLoop) + arrList(33,intLoop) + arrList(58,intLoop) + arrList(34,intLoop) + (arrList(35,intLoop)+(arrList(58,intLoop)*-1)) + (arrList(36,intLoop)-arrList(37,intLoop)) + arrList(38,intLoop) + arrList(37,intLoop))-arrList(42,intLoop)+arrList(40,intLoop))*-1
			bufstr = bufstr & "," & arrList(30,intLoop)		' 실사재고
		...
		bufstr = bufstr & "," & arrList(68,intLoop)		' 상품명
		bufstr = bufstr & "," & arrList(69,intLoop)		' 옵션명
...
IF FTotCnt > 0 THEN
	...
	headLine = "YYYY-MM,재고위치,상품구분,상품코드,옵션코드,바코드,부서구분,브랜드ID,구매유형,재고매입구분,ON매입구분,과세구분,기초재고(실사재고),,매입,,이동,,판매,,당월출고1,,당월출고2,,당월기타출고,,CS출고,,당월오차,,오차,,실사재고,,누적오차,,당월불량,,누적불량,,최종입고월,상품명,옵션명,대카테고리코드,대카테고리명,중간카테고리코드,중간카테고리명,사이즈X,사이즈Y,사이즈Z,판매여부"
