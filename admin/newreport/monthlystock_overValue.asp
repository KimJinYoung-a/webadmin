...
dim yyyy1,mm1,isusing,sysorreal, research, newitem, minusinc, bPriceGbn, vatyn, mwgubun, buseo, itemgubun, mygubun
dim purchasetype, stplace, shopid, swSppPrc, etcjungsantype, dispCate, showUpbae, nowdate, ojaego, i
dim subTotBuySum1, subTotBuySum2, subTotBuySum3, subTotBuySum4, subTotBuySum5, subTotBuySum6, subTotBuySum7, subTotBuySum8
dim subTotBuySum11, subTotBuySum12, subTotBuySum13, subTotBuySum14, subTotBuySum, subTotOverValueSum, sub_totStockNo
dim totBuySum1, totBuySum2, totBuySum3, totBuySum4, totBuySum5, totBuySum6, totBuySum7, totBuySum8, totBuySum11, totBuySum12
dim totBuySum13, totBuySum14, totBuySum, totOverValueSum, tot_totStockNo, totno, totbuy, subTotno, subTotbuy '', totavgBuy, offtotavgBuy
dim totPreno, totPrebuy , subPreno, subPrebuy, totIpno,totIpBuy , subIpno, subIpBuy, totLossno, totLossBuy , subLossno, subLossBuy
dim iURL
...
function reActMonthSummary() {
	...
	var popwin = window.open('/admin/newreport/do_stocksummary.asp?mode=stockovervalue&yyyymm=' + yyyymm,'reActMonthSummary','width=600,height=600');
	popwin.focus();
}
...
<tr>
	<td align="left">
		...
		* 매장(매입구분별) = 물류매입상품은 물류입고일, 그 이외 상품은 매장입고일.
	</td>
	<td align="right" valign="bottom">
		<% if C_ADMIN_AUTH or (session("ssAdminPsn")="8") or (session("ssBctId") = "aimcta") or (session("ssBctId") = "gogo27") then %>
			<input type="button" class="button" value="재고월령재작성" onclick="reActMonthSummary();">

			<% If stplace = "L" OR stplace = "T" OR stplace = "M" Then %>
				* 다운로드는 재고자산(월별) FIX 매뉴에서 가능
			<% End If %>
		<% end if %>
	</td>
</tr>
...
if (ojaego.FItemList(i).Fitemgubun <> "75") and (ojaego.FItemList(i).Fitemgubun <> "80") and (ojaego.FItemList(i).Fitemgubun <> "85") then
	...
	iURL = "monthlystock_overValue_detail.asp?menupos="& menupos &"&mwgubun="& ojaego.FItemList(i).FMaeIpGubun &"&yyyy1="& yyyy1 &"&mm1="& mm1 &"&isusing="& isusing &"&newitem="& newitem &"&itemgubun="&ojaego.FItemList(i).Fitemgubun&"&vatyn="&vatyn
	...
	iURL = iURL + "&sysorreal=" & sysorreal + "&mygubun=" & mygubun & "&etcjungsantype="&etcjungsantype & "&disp="&dispCate & "&showUpbae="&showUpbae
%>
	<tr align="center" bgcolor="#FFFFFF">
...
<input type="hidden" name="v2" value="N">
</form>
