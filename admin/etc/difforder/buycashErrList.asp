...
<%
Dim oOrder, research, i, page, itemid, makerid, nowsDate, iSD, sellchnl
research	= requestCheckvar(request("research"),2)
itemid  	= request("itemid")
makerid		= requestCheckvar(request("makerid"),32)
iSD			= request("iSD")
sellchnl	= request("sellchnl")
'텐바이텐 상품코드 엔터키로 검색되게
If itemid<>"" then
...
SET oOrder = new COrder
	...
	oOrder.FRectSellChannelDiv	= sellchnl
	oOrder.getBuycashErrList
%>
...
function pop_extsitejungsan(vItemid, vItemCost){
	var pCM5;
	pCM5 = window.open("/admin/etc/extsitejungsan_check.asp?itemid="+vItemid+"&mallsellcash="+vItemCost,"pop_jungsan");
	pCM5.location.href="/admin/etc/extsitejungsan_check.asp?itemid="+vItemid+"&mallsellcash="+vItemCost;
	pCM5.focus();
}
...
<tr align="center" <% If oOrder.FItemList(i).FOmwdiv = "U" Then response.write "bgcolor= 'PINK'" Else response.write "bgcolor= '#FFFFFF'" End If %> >
	...
	<td style="cursor:pointer;" onclick="HighlightRow(this);pop_extsitejungsan('<%=oOrder.FItemList(i).FItemID%>', '<%= oOrder.FItemList(i).FItemcost %>' );"><%= oOrder.FItemList(i).FItemcost %></td>
	<td><%= oOrder.FItemList(i).FReducedprice %></td>
	<td><%= oOrder.FItemList(i).FBuycash %></td>
	<td>
	<% if(oOrder.FItemList(i).FSitename="10x10") and (isNULL(oOrder.FItemList(i).FItemcouponidx)) then %>
	<strong><%= oOrder.FItemList(i).FChkMargin %></strong>
	<% else %>
	<%= oOrder.FItemList(i).FChkMargin %>
	<% end if %>
	</td>
