...
Else
	o11st.get11stRegedItemList		'그 외 리스트
End If
...
<tr align="center" bgcolor="#FFFFFF" >
	...
	<td align="left">
		...
		&nbsp;
		<a href="http://soffice.11st.co.kr" target="_blank">11번가Admin바로가기</a>
		<%
			...
			End If
		%>
		&nbsp;<a href="http://openapi.11st.co.kr/openapi/OpenApiGuide.tmall" target="_blank">11번가 개발가이드</a>
		<br>
		상품코드 : <textarea rows="2" cols="20" name="itemid" id="itemid"><%=replace(itemid,",",chr(10))%></textarea>
		&nbsp;
		상품명: <input type="text" class="text" name="itemname" value="<%= itemname %>" size="32" maxlength="32">
		&nbsp;
		11번가 상품코드 : <textarea rows="2" cols="20" name="st11GoodNo" id="itemid"><%=replace(st11GoodNo,",",chr(10))%></textarea>
		<br>
		<!-- #include virtual="/common/module/categoryselectbox.asp"-->
		<br>
		등록여부 :
		<select name="ExtNotReg" class="select" <%=Chkiif(isReged <> "", "disabled","") %> >
			<option value="A" <%= CHkIIF(ExtNotReg="A","selected","") %> >11번가 등록시도
			<option value="W" <%= CHkIIF(ExtNotReg="W","selected","") %> >11번가 등록예정이상
			<option value="D" <%= CHkIIF(ExtNotReg="D","selected","") %> >11번가 등록완료(전시)
		</select>&nbsp;
...
<tr align="center" bgcolor="#FFFFFF" >
	<td align="left">
		<label><input onClick="checkisReged(this)" type="checkbox" name="expensive10x10" <%= ChkIIF(expensive10x10="on","checked","") %> ><font color=red>11번가 가격<텐바이텐 판매가</font>상품보기</label>
		&nbsp;
		...
		<% End If %>
		<br>
		<label><input onClick="checkisReged(this)" type="checkbox" name="st11Yes10x10No" <%= ChkIIF(st11Yes10x10No="on","checked","") %> ><font color=red>11번가판매중&텐바이텐품절</font>상품보기</label>
		&nbsp;
		<label><input onClick="checkisReged(this)" type="checkbox" name="st11No10x10Yes" <%= ChkIIF(st11No10x10Yes="on","checked","") %> ><font color=red>11번가품절&텐바이텐판매가능</font>(판매중,한정>=10) 상품보기</label>
		<br>
		<label><input onClick="checkisReged(this)" type="checkbox" name="st11KeepSell" <%= ChkIIF(st11KeepSell="on","checked","") %> ><font color=red>판매유지</font> 해야할 상품보기</label>
		&nbsp;
...
* 제휴판매가 : 할인가(기준마진 미만인 경우 정상가), 원단위 올림처리(11번가는 원단위 안씀)<br />
...
<tr bgcolor="#FFFFFF">
    <td style="padding:5 0 5 0">
	    ...
	    <tr>
	    	<td valign="top">
				실제상품 등록 :
				<input class="button" type="button" id="btnRegSel" value="등록" onClick="st11SelectRegProcess();">
				<br><br>
				실제상품 수정 :
				<input class="button" type="button" id="btnEditPrice" value="가격" onClick="st11PriceEditProcess();">&nbsp;&nbsp;
				<input class="button" type="button" id="btnStock" value="재고조회" onClick="st11checkStockProcess();">&nbsp;&nbsp;
				<input class="button" type="button" id="btnStock" value="수정" onClick="st11EditProcess();">&nbsp;&nbsp;
				<input class="button" type="button" id="btnChkStat" value="조회" onClick="st11ChkStatProcess();">&nbsp;&nbsp;
			<% If (session("ssBctID")="kjy8517") Then %>
				&nbsp;&nbsp;<input class="button" type="button" id="btnODelete" value="상품삭제" onClick="st11DeleteProcess();" style=font-weight:bold>
			<% End If %>
			<% If (session("ssBctID")="kjy8517") OR (session("ssBctID")="icommang") Then %>
				...
				<select name="CommCD" class="select" id="CommCD">
					...
					<option value="outboundarea">출고지주소코드
					<option value="inboundarea">반품교환지주소코드
				</select>
				<input class="button" type="button" id="btnCommcd" value="공통코드확인" onClick="fn11stCommCDSubmit();" >
			<% End If %>
			</td>
			<td align="right" valign="top">
				...
				<input class="button" type="button" id="btnSellYn" value="변경" onClick="st11SellYnProcess(frmReg.chgSellYn.value);">
			</td>
		</tr>
		</table>
    </td>
...
<tr height="25" bgcolor="FFFFFF">
	<td colspan="16">
		검색결과 : <b><%= FormatNumber(o11st.FTotalCount,0) %></b>
		&nbsp;
		페이지 : <b> <%= FormatNumber(page,0) %> / <%= FormatNumber(o11st.FTotalPage,0) %></b>
	</td>
	<td align="right">
...
<% End If %>
	...
	<td width="140">11st등록일<br>11st최종수정일</td>
	<td width="70">텐바이텐<br>판매가</td>
	<td width="70">텐바이텐<br>마진</td>
	<td width="70">품절여부</td>
	<td width="70">주문제작<br>여부</td>
	<td width="70">11st<br>가격및판매</td>
	<td width="70">11st<br>상품번호</td>
	...
	<%
				...
				If Not IsNULL(o11st.FItemList(i).FSpecialPrice) Then
					If (now() >= o11st.FItemList(i).FStartDate) And (now() <= o11st.FItemList(i).FEndDate) Then
						response.write "<font color='orange'><strong>(특)" & formatNumber(o11st.FItemList(i).FSpecialPrice,0)&"</strong></font><br />"
					End If
				End If
