...
Class C11stItem
	...
	Public Function getSt11StatName
	    If IsNULL(FSt11StatCd) then FSt11StatCd=-1
		Select Case FSt11StatCd
			CASE -9 : getSt11StatName = "미등록"
			CASE -1 : getSt11StatName = "등록실패"
			CASE 0 : getSt11StatName = "<font color=blue>등록예정</font>"
			CASE 1 : getSt11StatName = "전송시도"
			CASE 3 : getSt11StatName = "<font color=red>OnSale변경전</font>"
			CASE 7 : getSt11StatName = ""
			CASE ELSE : getSt11StatName = FSt11StatCd
		End Select
	End Function
...
Class C11st
	...
	Public FRectdiffPrc
	Public FRectSt11Yes10x10No
	Public FRectSt11No10x10Yes
	Public FRectSt11KeepSell
	...
	Public Sub get11stRegedItemList
		Dim i, sqlStr, addSql
		'브랜드검색
		...
		'11st 상품번호 검색
        If (FRectSt11GoodNo <> "") then
            If Right(Trim(FRectSt11GoodNo) ,1) = "," Then
            	FRectSt11GoodNo = Replace(FRectSt11GoodNo,",,",",")
            	addSql = addSql & " and J.st11GoodNo in ('" & replace(Left(FRectSt11GoodNo, Len(FRectSt11GoodNo)-1),",","','") & "')"
            Else
				FRectSt11GoodNo = Replace(FRectSt11GoodNo,",,",",")
            	addSql = addSql & " and J.st11GoodNo in ('" & replace(FRectSt11GoodNo,",","','") & "')"
            End If
        End If
		...
		Select Case FRectExtNotReg
		    Case "A"	'전송시도중오류
				addSql = addSql & " and J.st11StatCd = 1"
			Case "W"	'등록예정이상
				addSql = addSql & " and J.st11StatCd >= 0"
			Case "D"	'등록완료(전시)
			    addSql = addSql & " and J.st11StatCd = 7"
				addSql = addSql & " and J.st11GoodNo is Not Null"
		End Select
		...
		If (FRectNotinmakerid <> "") then
			If (FRectNotinmakerid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = '11st1010') "
			ElseIf (FRectNotinmakerid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = '11st1010') "
			End If
		End If
		...
		If (FRectNotinitemid <> "") then
			If (FRectNotinitemid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = '11st1010') "
			ElseIf (FRectNotinitemid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = '11st1010') "
			End If
		End If
		...
		If (FRectExcTrans <> "") then
			If (FRectExcTrans = "Y") Then
				...
				addSql = addSql & " or i.makerid in (Select makerid From [db_temp].dbo.tbl_jaehyumall_not_in_makerid Where mallgubun='11st1010') "
				addSql = addSql & " or i.itemid in (Select itemid From [db_temp].dbo.tbl_jaehyumall_not_in_itemid Where mallgubun='11st1010') "
			...
			ElseIf (FRectExcTrans = "F") Then
				addSql = addSql & " and i.makerid not in (Select makerid From [db_temp].dbo.tbl_jaehyumall_not_in_makerid Where mallgubun='11st1010') "
				addSql = addSql & " and i.itemid not in (Select itemid From [db_temp].dbo.tbl_jaehyumall_not_in_itemid Where mallgubun='11st1010') "
		...
		If (FRectexpensive10x10 <> "") Then
			addSql = addSql & " and J.st11Price is Not Null and J.st11Price < i.sellcash"
		End If

		'가격상이전체보기
		If FRectdiffPrc <> "" Then
			addSql = addSql & " and J.st11Price is Not Null and i.sellcash <> J.st11Price "
		End If

		'GSShop판매 10x10 품절
		If (FRectSt11Yes10x10No <> "") Then
			addSql = addSql & " and i.sellyn<>'Y'"
			addSql = addSql & " and J.st11SellYn='Y'"
		End If

		'11st품절&텐바이텐판매가능(판매중,한정>=10) 상품보기
		If FRectSt11No10x10Yes <> "" Then
			addSql = addSql & " and (J.st11SellYn= 'N' and i.sellyn='Y' and (i.limityn='N' or (i.limityn='Y' and i.limitno-i.limitsold>"&CMAXLIMITSELL&")))"
		End If

		'수정요망상품보기(최종업데이트일 기준)
		If FRectReqEdit <> "" Then
			addSql = addSql & " and J.st11LastUpdate < i.lastupdate "
		End If
		...
		If (FRectOrdType = "LU") Then
		    ...
		    addSql = addSql & " and Left(i.lastupdate, 10) <> Left(J.st11LastUpdate, 10) "
		End If
		...
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
		    sqlStr = sqlStr & " 	LEFT JOIN db_etcmall.dbo.tbl_11st_regitem as J WITH (NOLOCK) "
		Else
		    sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_11st_regitem as J WITH (NOLOCK) "
	    END IF
		sqlStr = sqlStr & " 		on i.itemid=J.itemid "
		sqlStr = sqlStr & "	LEFT Join db_etcmall.dbo.tbl_11st_cate_mapping as c WITH (NOLOCK) on c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small "
		sqlStr = sqlStr & " LEFT join db_user.dbo.tbl_user_c uc WITH (NOLOCK) on i.makerid = uc.userid"
		sqlStr = sqlStr & " LEFT JOIN db_etcmall.dbo.tbl_outmall_mustPriceItem as mi with (nolock) on mi.itemid = i.itemid and mi.mallgubun = '11st1010' "
		sqlStr = sqlStr & " LEFT JOIN [db_temp].dbo.tbl_schedule_not_in_itemid as sc with (nolock) on sc.itemid = i.itemid and sc.mallgubun = '11st1010' "
		sqlStr = sqlStr & " LEFT JOIN db_partner.dbo.tbl_partner_addInfo as f on f.partnerid = '11st1010' "
		...
		sqlStr = sqlStr & "	, J.st11Regdate, J.st11LastUpdate, isnull(J.st11GoodNo, '') as st11GoodNo, J.st11Price, J.st11SellYn, J.regUserid, IsNULL(J.st11StatCd,-9) as st11StatCd "
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new C11stItem
					...
					FItemList(i).FSt11Regdate		= rsget("st11Regdate")
					FItemList(i).FSt11LastUpdate	= rsget("st11LastUpdate")
					FItemList(i).FSt11GoodNo		= rsget("st11GoodNo")
					FItemList(i).FSt11Price			= rsget("st11Price")
					FItemList(i).FSt11SellYn		= rsget("st11SellYn")
					FItemList(i).FRegUserid			= rsget("regUserid")
					FItemList(i).FSt11StatCd		= rsget("st11StatCd")
    ...
    Public Sub get11streqExpireItemList
		...
		sqlStr = sqlStr & " JOIN db_etcmall.dbo.tbl_11st_regitem as J on J.itemid = i.itemid and J.st11Goodno is not null "
		...
		sqlStr = sqlStr & "	, i.sellYn, i.sailyn, i.LimitYn, i.LimitNo, i.LimitSold, i.deliverytype, i.optionCnt"
		sqlStr = sqlStr & "	, J.st11Regdate, J.st11LastUpdate, J.st11GoodNo, J.st11Price, J.st11SellYn, J.regUserid, IsNULL(J.st11StatCd,-9) as st11StatCd "
		sqlStr = sqlStr & "	, Case When isnull(c.depthCode, 0) = 0 Then 0 Else 1 End as mapcnt "
		sqlStr = sqlStr & " , J.regedOptCnt, J.rctSellCNT, J.accFailCNT, J.lastErrStr "
		sqlStr = sqlStr & " ,uc.defaultdeliverytype, uc.defaultfreeBeasongLimit"
	...
	Public Sub getTen11stCateList
		...
		sqlStr = sqlStr & " 	FROM db_etcmall.dbo.tbl_11st_cate_mapping as cm  "  & VBCRLF
		sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_11st_category as cc on cc.depthCode = cm.depthCode  "  & VBCRLF
		...
		sqlStr = sqlStr & " ,T.depthCode, T.Depth1Nm,  T.Depth2Nm, T.Depth3Nm, T.Depth4Nm, W.itemcnt "  & VBCRLF
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_cate_small as s " & VBCRLF
		sqlStr = sqlStr & " LEFT JOIN (  "  & VBCRLF
		sqlStr = sqlStr & " 	SELECT cm.depthCode, cm.tenCateLarge,cm.tenCateMid, cm.tenCateSmall,cc.Depth1Nm,cc.Depth2Nm,cc.Depth3Nm,cc.Depth4Nm "  & VBCRLF
		sqlStr = sqlStr & " 	FROM db_etcmall.dbo.tbl_11st_cate_mapping as cm "  & VBCRLF
	...
	Public Sub get11stCateList
		...
		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(*) as cnt, CEILING(CAST(Count(*) AS FLOAT)/" & FPageSize & ") as totPg " & VBCRLF
		sqlStr = sqlStr & " FROM db_etcmall.dbo.tbl_11st_category " & VBCRLF
