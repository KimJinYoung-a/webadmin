...
Set oWmpfashion = new CWmpfashion
	...
	oWmpfashion.FCurrPage = 1
	oWmpfashion.FRectCDL = cdl
	oWmpfashion.FRectCDM = cdm
	oWmpfashion.FRectCDS = cds
...
<!--
	...
	function fnSaveForm() {
		...
		if(frm.CateKey.value=="") {
			alert("매칭할 위메프W패션 카테고리를 선택해주세요.");
			return;
		}

		if(confirm("선택하신 카테고리로 매칭하시겠습니까?")) {
			frm.mode.value="saveCate";
			frm.action="procWmpfashion.asp";
			frm.submit();
		}
	}
	...
	// 위메프W패션 카테고리 검색
	function fnSearchWmpfashionCate(disptpcd) {
		var pFCL = window.open("","popWmpfashionCate","width=1000,height=700,scrollbars=yes,resizable=yes");
		pFCL.focus();
		srcFrm.target="popWmpfashionCate";
		srcFrm.action="popFindWmpfashionCate.asp";
		srcFrm.submit();
	}
//-->
</script>
...
<tr valign="top">
	...
	<font color="red"><strong>위메프W패션 카테고리 매칭</strong></font></td>
...
<tr height="25">
	...
	<td bgcolor="#FFFFFF">[<%=cdl%>] <%=oWmpfashion.FItemList(0).FtenCDLName%></td>
</tr>
<tr height="25">
	<td width="80" align="center" bgcolor="#DDDDFF">중분류</td>
	<td bgcolor="#FFFFFF">[<%=cdm%>] <%=oWmpfashion.FItemList(0).FtenCDMName%></td>
</tr>
<tr height="25">
	<td width="80" align="center" bgcolor="#DDDDFF">소분류</td>
	<td bgcolor="#FFFFFF">[<%=cds%>] <%=oWmpfashion.FItemList(0).FtenCDSName%></td>
</tr>
</table>
...
<tr height="5" valign="top">
	<td width="10" align="right" background="/images/tbl_blue_round_04.gif"></td>
	<td align="left"><img src="/images/icon_arrow_down.gif" border="0" vspace="5" align="absmiddle"> 위메프W패션 카테고리 매칭 정보</td>
	<td width="10" align="left" background="/images/tbl_blue_round_05.gif"></td>
</tr>
</table>
<!-- 표 중간바 끝-->
...
<tr height="25">
	...
	<td bgcolor="#FFFFFF">
		...
		<input type="button" value="검색" class="button" onClick="fnSearchWmpfashionCate();">
	</td>
</tr>
...
<tr height="25">
    <td id="brTT" width="80" align="center" bgcolor="#DDDDFF" rowspan="<%= oWmpfashion.FResultCount + 1 %>" >등록된<br>카테고리</td>
	<td bgcolor="#FFFFFF" height="1"></td>
</tr>
<% For i = 0 to oWmpfashion.FResultCount - 1 %>
<% If Not IsNULL(oWmpfashion.FItemList(i).FCateKey) Then %>
	<%
		If oWmpfashion.FItemList(i).FDepth4Name = "" Then
			isNull4DeprgNm = oWmpfashion.FItemList(i).FDepth3Name
		Else
			isNull4DeprgNm = oWmpfashion.FItemList(i).FDepth4Name
		End If
	%>
<tr>
    <td bgcolor="#F2F2F2"><b><span id="selBr"><%=isNull4DeprgNm%> [<%=oWmpfashion.FItemList(i).FCateKey%>] <%=oWmpfashion.FItemList(i).FCateKey%></span></b>
    &nbsp;&nbsp;&nbsp;&nbsp;<img src="/images/icon_delete.gif" width="45" height="20" border="0" onclick="fnDelForm('<%=oWmpfashion.FItemList(i).FCateKey%>')" style="cursor:pointer" align="absmiddle">
    </td>
</tr>
<% End If %>
<% Next %>
