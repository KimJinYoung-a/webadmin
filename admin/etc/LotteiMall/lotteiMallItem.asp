...
function LotteRegProcess(){
    if (confirm('롯데iMall에 미등록된 상품을 일괄 등록 하시겠습니까?\n\n※롯데iMall과의 통신상태에 따라 시간이 다소 걸릴 수 있습니다.')){
...
<tr>
	<td class="a">
		...
		<option value="W" <%= CHkIIF(LotteNotReg="W","selected","") %> >롯데iMall 등록예정
		...
		-->
		<!--
		<input type="checkbox" name="onreginotmapping" <%= ChkIIF(onreginotmapping="on","checked","") %> ><font color=red>롯데iMall 등록&미패칭 카테고리</font>상품보기
		&nbsp;
		-->
	</td>
	...
	<td>
		...
		<tr>
			...
			-->
				...
				&nbsp;&nbsp;
				<input class="button" type="button" id="btnChkReal" value="선택상품 [승인] 확인" onClick="LotteRealItemMappingChecked();" <%= CHKIIF((TRUE) or (LotteNotReg="F"),"","disabled") %> >
...
<tr bgcolor="#FFFFFF">
    <td style="padding:5 0 5 0">
	    ...
	    <tr>
	    	<td valign="top">
	    	    <!--
			    <input class="button" type="button" id="btnEditAll" value="수정된 상품 롯데iMall으로 일괄수정 [10건씩]" onClick="LotteEditProcess();">
			    &nbsp;&nbsp;
			    <input class="button" type="button" id="btnRegAll" value="미등록 상품 롯데iMall으로 일괄등록 [10건씩]" onClick="LotteRegProcess();">
			    -->
			    <br><br>
			    <input class="button" type="button" id="btnEditSel" value="선택상품정보/가격 수정" onClick="LotteSelectEditProcess();">
			    &nbsp;&nbsp;
			    <input class="button" type="button" id="btnEditDanpum" value="선택상품단품/판매상태 수정" onClick="LotteSelectSaleStatEditProcess();">
			    &nbsp;&nbsp;
			    <input class="button" type="button" id="btnRegSel" value="선택상품 실 등록" onClick="LotteSelectRegProcess(true);">
			    <br><br>
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="#CCCCCC">
<tr bgcolor="#FFFFFF">
	<td colspan="16" align="right" height="30">page: <%= FormatNumber(page,0) %> / <%= FormatNumber(oiMall.FTotalPage,0) %> 총건수: <%= FormatNumber(oiMall.FTotalCount,0) %></td>
</tr>
...
<tr bgcolor="#FFFFFF" height="20">
    ...
    <br><a href="javascript:popManageOptAddPrc('<%=oiMall.FItemList(i).FItemID%>');"><font color="red">옵션금액</font>(<%=oiMall.FItemList(i).FoptAddPrcRegType%>)</a>
    <% end if %>
    </td>
