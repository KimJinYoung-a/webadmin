...
Set okakaogift = new Ckakaogift
	...
	okakaogift.FCurrPage = 1
	okakaogift.FRectCDL = cdl
	okakaogift.FRectCDM = cdm
	okakaogift.FRectCDS = cds
	okakaogift.getTenkakaogiftCateList

If okakaogift.FResultCount <= 0 Then
...
<!--
	...
	function fnSaveForm() {
		...
		if(frm.depthcode.value=="") {
			alert("매칭할 kakaogift 카테고리를 선택해주세요.");
			return;
		}

		if(confirm("선택하신 카테고리로 매칭하시겠습니까?")) {
			frm.mode.value="saveCate";
			frm.action="prockakaogift.asp";
			frm.submit();
		}
	}
    ...
    function fnDelForm(iDspNo) {
		var frm = document.frmAct;
		if (iDspNo=="") {
		    alert("삭제할 kakaogift 카테고리를 선택해주세요.");
			return;
		}
	...
	// kakaogift 카테고리 검색
	function fnSearchkakaogiftCate(disptpcd) {
		var pFCL = window.open("","popkakaogiftCate","width=1000,height=700,scrollbars=yes,resizable=yes");
		pFCL.focus();
		srcFrm.target="popkakaogiftCate";
		srcFrm.action="popFindkakaogiftCate.asp";
		srcFrm.submit();
	}
//-->
</script>
...
<tr valign="top">
	...
	<font color="red"><strong>kakaogift 카테고리 매칭</strong></font></td>
...
<tr height="25">
	...
	<td bgcolor="#FFFFFF">[<%=cdl%>] <%=okakaogift.FItemList(0).FtenCDLName%></td>
</tr>
<tr height="25">
	<td width="80" align="center" bgcolor="#DDDDFF">중분류</td>
	<td bgcolor="#FFFFFF">[<%=cdm%>] <%=okakaogift.FItemList(0).FtenCDMName%></td>
</tr>
<tr height="25">
	<td width="80" align="center" bgcolor="#DDDDFF">소분류</td>
	<td bgcolor="#FFFFFF">[<%=cds%>] <%=okakaogift.FItemList(0).FtenCDSName%></td>
</tr>
</table>
...
<tr height="5" valign="top">
	<td width="10" align="right" background="/images/tbl_blue_round_04.gif"></td>
	<td align="left"><img src="/images/icon_arrow_down.gif" border="0" vspace="5" align="absmiddle"> kakaogift 카테고리 매칭 정보</td>
	<td width="10" align="left" background="/images/tbl_blue_round_05.gif"></td>
</tr>
</table>
<!-- 표 중간바 끝-->
...
<tr height="25">
	...
	<td bgcolor="#FFFFFF">
		...
		<input type="button" value="검색" class="button" onClick="fnSearchkakaogiftCate();">
	</td>
</tr>
...
<tr height="25">
    <td id="brTT" width="80" align="center" bgcolor="#DDDDFF" rowspan="<%= okakaogift.FResultCount + 1 %>" >등록된<br>카테고리</td>
	<td bgcolor="#FFFFFF" height="1"></td>
</tr>
<% For i = 0 to okakaogift.FResultCount - 1 %>
<% If Not IsNULL(okakaogift.FItemList(i).FDepthCode) Then %>
<tr>
    <td bgcolor="#F2F2F2"><b><span id="selBr"><%=okakaogift.FItemList(i).getLastDepthName%> [<%=okakaogift.FItemList(i).FDepthCode%>] <%=okakaogift.FItemList(i).FDepthCode%></span></b>
    &nbsp;&nbsp;&nbsp;&nbsp;<img src="/images/icon_delete.gif" width="45" height="20" border="0" onclick="fnDelForm('<%=okakaogift.FItemList(i).FDepthCode%>')" style="cursor:pointer" align="absmiddle">
    </td>
</tr>
<% End If %>
<% Next %>
...
<input type="hidden" name="needinfodiv" value="">
