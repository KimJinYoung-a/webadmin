...
function getCjMallfirstItemoption(byval iitemid)
	...
	if (ret="") then
		sqlStr = "select top 1 itemoption from db_outmall.dbo.tbl_item_option where itemid = '"&iitemid&"' and isusing = 'Y' and optsellyn = 'Y' order by itemoption asc"
		rsCTget.Open sqlStr, dbCTget
		If Not(rsCTget.EOF or rsCTget.BOF) Then
			ret = rsCTget("itemoption")
		Else
...
Function regCjMallOneItem(byval iitemid, byRef ierrStr)
	...
	If (xmlStr = "") Then
		...
		sqlStr = sqlStr & " LEFT JOIN db_outmall.dbo.tbl_cjmall_regItem as R on i.itemid=R.itemid " & VBCRLF
		sqlStr = sqlStr & " LEFT JOIN db_outmall.dbo.tbl_jaehyumall_not_in_itemid as N on i.itemid=N.itemid and N.mallgubun = 'cjmall' " & VBCRLF
		sqlStr = sqlStr & " LEFT JOIN db_outmall.dbo.tbl_cjMall_prdDiv_mapping as PD on PD.tencatelarge = i.cate_large and PD.tencatemid = i.cate_mid and PD.tencatesmall = i.cate_small " & VBCRLF
		sqlStr = sqlStr & " LEFT JOIN db_outmall.dbo.tbl_OutMall_CateMap_Summary as c on c.mallid='cjmall' and c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small " & VBCRLF
		sqlStr = sqlStr & " WHERE i.itemid = "&iitemid
		rsCTget.Open sqlStr,dbCTget,1
		If Not(rsCTget.EOF or rsCTget.BOF) Then
			If (rsCTget("asCjmallStatCD") >= 3) Then
				ierrStr = ierrStr & " - 기등록상품"&" :: 상태["&rsCTget("asCjmallStatCD")&"]"
			End If

			If (rsCTget("sellyn") <> "Y") Then
			    ierrStr = ierrStr & " - 품절상태"
			End If

			If (rsCTget("mapCnt") = "0") Then
			    ierrStr = ierrStr & " - 카테고리미매칭상태"
			End If

			If (rsCTget("CddKey") = "") Then
			    ierrStr = ierrStr & " - 상품미분류상태"
			End If

			If (rsCTget("limityn") = "Y") and (rsCTget("limitno") - rsCTget("limitsold") < CMAXLIMITSELL) Then
				ierrStr = ierrStr & " - 한정수량 부족 ("&rsCTget("limitno")-rsCTget("limitsold")&") 개 남음"
				cause = "limitErr"
			End If

			If (rsCTget("Nitemid") <> "0") Then
		...
		If (ierrStr = "등록불가") Then
			...
			sqlStr = sqlStr & " FROM db_AppWish.dbo.tbl_item_option" & VBCRLF
			sqlStr = sqlStr & " WHERE itemid="&iitemid & VBCRLF
			sqlStr = sqlStr & " and isusing='Y'" & VBCRLF
			sqlStr = sqlStr & " GROUP BY itemid"
			rsCTget.Open sqlStr,dbCTget,1
			If Not(rsCTget.EOF or rsCTget.BOF) Then
'				If (rsCTget("optAddCNT") > 0) Then
'					ierrStr = ierrStr & " - 옵션추가 금액 존재상품 등록불가"
'					cause = "optAddPrcExist"
'				End If

				If (rsCTget("optCnt") - rsCTget("optNotSellCnt") < 1) Then
		...
		If (cause <> "") Then
			''제약조건 체크해야..
			sqlStr = ""
			sqlStr = sqlStr & "INSERT INTO db_outmall.dbo.tbl_jaehyumall_not_in_itemid " & VBCRLF
			sqlStr = sqlStr & "(itemid, mallgubun, bigo) " & VBCRLF
			sqlStr = sqlStr & " SELECT i.itemid, '"&CMALLNAME&"', '"&cause&"'" & VBCRLF
			sqlStr = sqlStr & " FROM db_AppWish.dbo.tbl_item as i" & VBCRLF
			sqlStr = sqlStr & " LEFT JOIN db_outmall.dbo.tbl_jaehyumall_not_in_itemid as n on i.itemid = n.itemid " & VBCRLF
...
'			zClassId= SubNodes.getElementsByTagName("ns1:zClassId")(0).Text	'hsk
        ...
        ''if (taxYn="Y") then purchvat=purchvat*1.1
			...
			If (OverLapNo <> Xvpn) Then
				...
				strSql = strSql & " FROM db_outmall.dbo.tbl_cjmall_regitem as R " & VBCRLF
				strSql = strSql & " INNER JOIN db_AppWish.dbo.tbl_item as i on R.itemid=i.itemid " & VBCRLF
			...
			If Xitemcode <> "" Then
				strSql = ""
				strSql = strSql & " UPDATE db_outmall.dbo.tbl_OutMall_regedoption  " & VBCRLF
...
'2014-09-15 11:12 김진영 on error 문 추가 / cj API 변경으로 Left => instr로 변경
	...
	'reged옵션 테이블에 데이터 꼽기
			...
			sqlStr = sqlStr & " FROM db_AppWish.dbo.tbl_item as i " & VBCRLF
			sqlStr = sqlStr & " LEFT JOIN db_AppWish.[dbo].tbl_item_option as o on i.itemid = o.itemid and o.isusing = 'Y' " & VBCRLF
			sqlStr = sqlStr & " WHERE i.itemid = "&prdno&" " & VBCRLF
			sqlStr = sqlStr & " ORDER BY o.itemoption ASC "
			rsCTget.Open sqlStr,dbCTget,1
			If Not(rsCTget.EOF or rsCTget.BOF) Then
				If rsCTget.RecordCount = 1 AND IsNull(rsCTget("itemoption")) Then
					Titemid			= rsCTget("itemid")
					Titemoption 	= "0000"
					Toptionname		= "단일상품"
					Tlimitno		= rsCTget("limitno")
					Tlimitsold		= rsCTget("limitsold")
					Tlimityn		= rsCTget("limityn")
					Tsellyn			= rsCTget("sellyn")
					...
					sqlStr = sqlStr & " VALUES " & VBCRLF
					sqlStr = sqlStr & " ('"&Titemid&"',  '"&Titemoption&"', 'cjmall', '', '"&html2db(Toptionname)&"', '"&rsCTget("sellyn")&"', '"&rsCTget("limityn")&"', '"&Titemsu&"', '0', getdate()) "
					dbCTget.Execute sqlStr
				Else
					For i = 1 to rsCTget.RecordCount
						...
						Tlimitno		= rsCTget("limitno")
						Tlimitsold		= rsCTget("limitsold")
						Titemoption		= rsCTget("itemoption")
						Toptionname		= rsCTget("optionname")
						Toptlimitno		= rsCTget("optlimitno")
						Toptlimitsold	= rsCTget("optlimitsold")
						Toptsellyn		= rsCTget("optsellyn")
						Toptlimityn		= rsCTget("optlimityn")
						Toptaddprice	= rsCTget("optaddprice")
						Tsellcash		= rsCTget("sellcash")
		...
		ElseIf mode = "EDT" Then  '' 결과값으로 저장요망
			...
			sqlStr = sqlStr & " FROM db_AppWish.dbo.tbl_item as i " & VBCRLF
			sqlStr = sqlStr & " LEFT JOIN db_AppWish.[dbo].tbl_item_option as o on i.itemid = o.itemid and o.isusing = 'Y' " & VBCRLF
			sqlStr = sqlStr & " LEFT JOIN [db_outmall].[dbo].tbl_OutMall_regedoption as R on i.itemid = R.itemid and R.itemoption = o.itemoption and R.mallid='"&CMALLNAME&"'" & VBCRLF   ''R.mallid='"&CMALLNAME&"' 추가
			sqlStr = sqlStr & " WHERE i.itemid = "&prdno&" " & VBCRLF
			sqlStr = sqlStr & " ORDER BY o.itemoption ASC "
			rsCTget.Open sqlStr,dbCTget,1
			If Not(rsCTget.EOF or rsCTget.BOF) Then
				For i = 1 to rsCTget.RecordCount
					If IsNull(rsCTget("outmallOptCode")) Then
						Titemid			= rsCTget("itemid")
						Tlimitno		= rsCTget("limitno")
						Tlimitsold		= rsCTget("limitsold")
						Titemoption		= rsCTget("itemoption")
		...
		If (prdno <> "") Then
			Dim MustPrice
			sqlStr = ""
			sqlStr = sqlStr & " SELECT sellcash, buycash, orgprice FROM db_AppWish.dbo.tbl_item where itemid = "&prdno&"  " & VBCRLF
			rsCTget.Open sqlStr,dbCTget,1
			If Not(rsCTget.EOF or rsCTget.BOF) Then
				If CLng(10000 - rsCTget("buycash") / rsCTget("sellcash") * 100 * 100) / 100 < 15 Then
					MustPrice = rsCTget("orgprice")
				Else
					MustPrice = rsCTget("sellcash")
				End If
			End If
			rsCTget.Close
...
function IsOptionAddPriceExistItem(iitemid)
    ...
    sqlStr = "select count(*) as CNT from db_AppWish.dbo.tbl_item_option"&VbCRLF
    sqlStr = sqlStr&" where itemid="&iitemid&VbCRLF
    sqlStr = sqlStr&" and optAddprice<>0"&VbCRLF

    rsCTget.Open sqlStr,dbCTget,1
	if (Not rsCTget.Eof) then
		IsOptionAddPriceExistItem = rsCTget("CNT")>0
	end if
	rsCTget.Close
