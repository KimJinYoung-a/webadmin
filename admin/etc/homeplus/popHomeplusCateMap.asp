...
<!--
	...
	function fnSaveForm() {
		...
		if(frm.depthcode.value=="") {
			alert("매칭할 Homeplus 카테고리를 선택해주세요.");
			return;
		}

		if(confirm("선택하신 카테고리로 매칭하시겠습니까?")) {
			frm.mode.value="brandCate";
	...
	function fnSearchCJCate(disptpcd) {
		var pFCL = window.open("","popHomeplusCate","width=900,height=700,scrollbars=yes,resizable=yes");
		pFCL.focus();
		srcFrm.target="popHomeplusCate";
		srcFrm.action="popFindHomeplusCate.asp";
		srcFrm.submit();
	}
//-->
</script>
...
<tr valign="top">
	...
	<font color="red"><strong>Homeplus 카테고리 매칭</strong></font></td>
...
<tr height="5" valign="top">
	<td width="10" align="right" background="/images/tbl_blue_round_04.gif"></td>
	<td align="left"><img src="/images/icon_arrow_down.gif" border="0" vspace="5" align="absmiddle"> Homeplus 전시 카테고리 매칭 정보 <!--(변경하려면 삭제후 저장) 1:1 카테고리로.--></td>
	<td width="10" align="left" background="/images/tbl_blue_round_05.gif"></td>
</tr>
</table>
<!-- 표 중간바 끝-->
...
<tr height="25">
    <td id="brTT" width="80" align="center" bgcolor="#DDDDFF" rowspan="<%= oHomeplus.FResultCount + 1 %>" >등록된<br>카테고리</td>
	<td bgcolor="#FFFFFF" height="1"></td>
</tr>
<% For i = 0 to oHomeplus.FResultCount - 1 %>
<% If Not IsNULL(oHomeplus.FItemList(i).FDepthCode) Then %>
<tr>
    <td bgcolor="#F2F2F2"><b><span id="selBr"><%=oHomeplus.FItemList(i).FDepth6Nm%> [<%=oHomeplus.FItemList(i).FDepthCode%>] <%=oHomeplus.FItemList(i).FDepthCode%></span></b>
    &nbsp;&nbsp;&nbsp;&nbsp;<img src="/images/icon_delete.gif" width="45" height="20" border="0" onclick="fnDelForm('<%=oHomeplus.FItemList(i).FDepthCode%>')" style="cursor:pointer" align="absmiddle">
    </td>
</tr>
<% End If %>
<% Next %>
...
<input type="hidden" name="categbn" value="branddisp">
</form>
