...
If cmdparam = "DELETE" Then
	...
	sqlStr = sqlStr &" SELECT 'bindmall1010', i.itemid, r.bindmallGoodNo, isNull(r.bindmallRegdate, getdate()), getdate(), r.lastErrStr " & VBCRLF
	sqlStr = sqlStr &" FROM db_item.dbo.tbl_item as i " & VBCRLF
	sqlStr = sqlStr &" JOIN db_etcmall.dbo.tbl_bindmall_regitem as r on i.itemid = r.itemid " & VBCRLF
	sqlStr = sqlStr &" WHERE i.itemid in (" & arrItemid & ") "
	dbget.Execute(sqlStr)

	sqlStr = ""
	sqlStr = sqlStr & " DELETE FROM db_etcmall.dbo.tbl_bindmall_regitem WHERE itemid in (" & arrItemid & ")" & VbCrlf
	dbget.Execute sqlStr,AssignedRow
	rw AssignedRow&"건 상품 삭제"

	sqlStr = ""
	sqlStr = sqlStr & " DELETE FROM db_item.dbo.tbl_OutMall_regedoption WHERE itemid in (" & arrItemid & ") and mallid = 'bindmall' "
	dbget.Execute sqlStr,AssignedRow
	rw AssignedRow&"건 옵션 삭제"

	sqlStr = ""
	sqlStr = sqlStr & " DELETE FROM db_etcmall.dbo.tbl_outmall_API_Que WHERE itemid in (" & arrItemid & ") and mallid = 'bindmall' "
	dbget.Execute sqlStr,AssignedRow
...
<script type="text/javascript">
	...
	function arrSubmit(ino){
        ...
        document.frmSvArr.action = '/admin/etc/wconcept/wconceptActProc.asp';
