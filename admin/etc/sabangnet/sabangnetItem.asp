...
Else
	oSabangnet.getSabangnetRegedItemList		'그 외 리스트
End If
%>
...
// 관리 카테고리를 사방넷에 등록..최초 1회만 해놓으면 될 것 같음
function sabangnetCategoryRegProcess(){
	if (confirm('sabangnet에 10x10의 관리카테고리를 등록하시겠습니까?')){
...
function sabangnetGosiInfoSelectProcess(){
	if (confirm('sabangnet에 상품 고시 정보를 호출하겠습니까?')){
		document.frmSvArr.target = "xLink";
...
function sabangnetREGProcess() {
    ...
    if (confirm('sabangnet에 선택하신 ' + chkSel + '개 상품을 등록 하시겠습니까?')){
        document.frmSvArr.target = "xLink";
...
function sabangnetEditPriceProcess(){
    ...
    if (confirm('선택하신 ' + chkSel + '개 가격 및 판매상태를 수정 하시겠습니까?')){
        document.frmSvArr.target = "xLink";
...
function sabangnetShopDataProcess(){
    ...
    if (confirm('선택하신 ' + chkSel + '개 쇼핑몰별 DATA를 수정 하시겠습니까?')){
        document.frmSvArr.target = "xLink";
...
function sabangnetEditProcess(){
    ...
    if (confirm('선택하신 ' + chkSel + '개 전체 수정 하시겠습니까?')){
        document.frmSvArr.target = "xLink";
...
<tr align="center" bgcolor="#FFFFFF" >
	...
	<td align="left">
		...
		&nbsp;
		<a href="http://s400.sabangnet.co.kr" target="_blank">사방넷Admin바로가기</a>
		<%
		...
		&nbsp;
		사방넷 상품코드 : <textarea rows="2" cols="20" name="sabangnetGoodNo" id="itemid"><%= replace(replace(sabangnetGoodNo,",",chr(10)), "'", "")%></textarea>
		<br>
		<!-- #include virtual="/common/module/categoryselectbox.asp"-->
		<br>
		등록여부 :
		<select name="ExtNotReg" class="select" <%=Chkiif(isReged <> "", "disabled","") %> >
			<option value="A" <%= CHkIIF(ExtNotReg="A","selected","") %> >사방넷 전송시도 중 오류
			<option value="W" <%= CHkIIF(ExtNotReg="W","selected","") %> >사방넷 등록예정
			<option value="D" <%= CHkIIF(ExtNotReg="D","selected","") %> >사방넷 등록완료(전시)
		</select>&nbsp;
...
<tr align="center" bgcolor="#FFFFFF" >
	<td align="left">
		<label><input onClick="checkisReged(this)" type="checkbox" name="expensive10x10" <%= ChkIIF(expensive10x10="on","checked","") %> ><font color=red>사방넷 가격<텐바이텐 판매가</font>상품보기</label>
		&nbsp;
		...
		<% End If %>
		<br>
		<label><input onClick="checkisReged(this)" type="checkbox" name="sabangnetYes10x10No" <%= ChkIIF(sabangnetYes10x10No="on","checked","") %> ><font color=red>사방넷판매중&텐바이텐품절</font>상품보기</label>
		&nbsp;
		<label><input onClick="checkisReged(this)" type="checkbox" name="sabangnetNo10x10Yes" <%= ChkIIF(sabangnetNo10x10Yes="on","checked","") %> ><font color=red>사방넷품절&텐바이텐판매가능</font>(판매중,한정>=10) 상품보기</label>
		<br>
...
* 전송제외상품 : 등록제외브랜드, 등록제외상품, 제휴몰사용안함, 업체착불, 딜상품, 꽃배달, 화물배달, 티켓(강좌) 상품, 판매가(할인가) 1만원 미만
...
<tr bgcolor="#FFFFFF">
    <td style="padding:5 0 5 0">
	    ...
	    <tr>
	    	<td valign="top">
				실제상품 등록 :
				<input class="button" type="button" id="btnREG" value="등록" onClick="sabangnetREGProcess();" style=color:red;font-weight:bold>&nbsp;&nbsp;
				<br><br>
				실제상품 수정 :
				<input class="button" type="button" id="btnEdit" value="수정" onClick="sabangnetEditProcess();" style=color:blue;font-weight:bold>&nbsp;&nbsp;
				<input class="button" type="button" id="btnEditPrice" value="가격 및 상태" onClick="sabangnetEditPriceProcess();">
			<% If (session("ssBctID")="kjy8517") Then %>
				&nbsp;&nbsp;<input class="button" type="button" id="btnEditPrice" value="쇼핑몰DATA수정" onClick="sabangnetShopDataProcess();">
			<% End If %>
			...
			<td align="right" valign="top">
				...
				<Select name="chgSellYn" class="select">
					<option value="N">일시중지</option>
					<option value="Y">판매</option>
				</Select>(으)로
				<input class="button" type="button" id="btnSellYn" value="변경" onClick="sabangnetSellYnProcess(frmReg.chgSellYn.value);">
			</td>
		</tr>
		</table>
    </td>
...
<tr height="25" bgcolor="FFFFFF">
	<td colspan="19">
		검색결과 : <b><%= FormatNumber(oSabangnet.FTotalCount,0) %></b>
		&nbsp;
		페이지 : <b> <%= FormatNumber(page,0) %> / <%= FormatNumber(oSabangnet.FTotalPage,0) %></b>
	</td>
</tr>
<tr align="center" bgcolor="<%= adminColor("tabletop") %>">
	...
	<td width="140">사방넷 등록일<br>사방넷 최종수정일</td>
	<td width="70">텐바이텐<br>판매가</td>
	<td width="70">텐바이텐<br>마진</td>
	<td width="70">품절여부</td>
	<td width="70">주문제작<br>여부</td>
	<td width="70">사방넷<br>가격및판매</td>
	<td width="70">사방넷<br>상품번호</td>
	...
	<%
				...
				If Not IsNULL(oSabangnet.FItemList(i).FSpecialPrice) Then
					If (now() >= oSabangnet.FItemList(i).FStartDate) And (now() <= oSabangnet.FItemList(i).FEndDate) Then
						response.write "<font color='orange'><strong>(특)" & formatNumber(oSabangnet.FItemList(i).FSpecialPrice,0)&"</strong></font><br />"
					End If
				End If
