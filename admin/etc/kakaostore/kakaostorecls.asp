...
Class CKakaostoreItem
	...
	Public FLimitNo
	Public FLimitSold
	Public FkakaostoreRegdate
	Public FkakaostoreLastUpdate
	Public FkakaostoreGoodNo
	Public FkakaostorePrice
	Public FkakaostoreSellYn
	Public FRegUserid
	Public FkakaostoreStatCd
	Public FCateMapCnt
	...
	Public FCertKc
	Public FCertFood
	Public FMinorPurchasable
	Public FSupplementTypes
	...
	Public Function getkakaostoreStatName
	    If IsNULL(FkakaostoreStatCd) then FkakaostoreStatCd=-1
		Select Case FkakaostoreStatCd
			CASE -9 : getkakaostoreStatName = "미등록"
			CASE -1 : getkakaostoreStatName = "등록실패"
			CASE 0 : getkakaostoreStatName = "<font color=blue>등록예정</font>"
			CASE 1 : getkakaostoreStatName = "전송시도"
			CASE 3 : getkakaostoreStatName = "<font color=red>OnSale변경전</font>"
			CASE 7 : getkakaostoreStatName = ""
			CASE ELSE : getkakaostoreStatName = FkakaostoreStatCd
		End Select
	End Function
...
Class CKakaostore
	...
	Public FRectdiffPrc
	Public FRectkakaostoreYes10x10No
	Public FRectkakaostoreNo10x10Yes
	Public FRectkakaostoreKeepSell
	...
	'// kakaostore 상품 목록 // 수정시 조건이 달라야 함..
	Public Sub getkakaostoreRegedItemList
		Dim i, sqlStr, addSql
		'브랜드검색
		...
		'kakaostore 상품번호 검색
        If (FRectkakaostoreGoodNo <> "") then
            If Right(Trim(FRectkakaostoreGoodNo) ,1) = "," Then
            	FRectkakaostoreGoodNo = Replace(FRectkakaostoreGoodNo,",,",",")
            	addSql = addSql & " and J.kakaostoreGoodNo in ('" & replace(Left(FRectkakaostoreGoodNo, Len(FRectkakaostoreGoodNo)-1),",","','") & "')"
            Else
				FRectkakaostoreGoodNo = Replace(FRectkakaostoreGoodNo,",,",",")
            	addSql = addSql & " and J.kakaostoreGoodNo in ('" & replace(FRectkakaostoreGoodNo,",","','") & "')"
            End If
        End If
		...
		Select Case FRectExtNotReg
		    Case "A"	'전송시도중오류
				addSql = addSql & " and J.kakaostoreStatCd = 1"
			Case "W"	'등록예정이상
				addSql = addSql & " and J.kakaostoreStatCd >= 0"
		    Case "I"	'이미지만 완료
				addSql = addSql & " and isnull(J.kakaostoreGoodNo, '') = '' "
				addSql = addSql & " and J.APIaddImg = 'Y'"
			Case "D"	'등록완료(전시)
			    addSql = addSql & " and J.kakaostoreStatCd = 7"
				addSql = addSql & " and J.kakaostoreGoodNo is Not Null"
		End Select
		...
		If (FRectNotinmakerid <> "") then
			If (FRectNotinmakerid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'kakaostore') "
			ElseIf (FRectNotinmakerid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'kakaostore') "
			End If
		End If
		...
		If (FRectNotinitemid <> "") then
			If (FRectNotinitemid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'kakaostore') "
			ElseIf (FRectNotinitemid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'kakaostore') "
			End If
		End If
		...
		If (FRectExcTrans <> "") then
			If (FRectExcTrans = "Y") Then
				...
				addSql = addSql & " or i.makerid in (Select makerid From [db_temp].dbo.tbl_jaehyumall_not_in_makerid Where mallgubun='kakaostore') "
				addSql = addSql & " or i.itemid in (Select itemid From [db_temp].dbo.tbl_jaehyumall_not_in_itemid Where mallgubun='kakaostore') "
			...
			ElseIf (FRectExcTrans = "F") Then
				addSql = addSql & " and i.makerid not in (Select makerid From [db_temp].dbo.tbl_jaehyumall_not_in_makerid Where mallgubun='kakaostore') "
				addSql = addSql & " and i.itemid not in (Select itemid From [db_temp].dbo.tbl_jaehyumall_not_in_itemid Where mallgubun='kakaostore') "
		...
		If (FRectExtSellYn<>"") then
			If (FRectExtSellYn = "YN") Then
				addSql = addSql & " and J.kakaostoreSellYn <> 'X'"
			Else
				addSql = addSql & " and J.kakaostoreSellYn='" & FRectExtSellYn & "'"
			End if
		End If
        ...
        'kakaostore가격 < 10x10 가격
		If (FRectexpensive10x10 <> "") Then
			addSql = addSql & " and J.kakaostorePrice is Not Null and J.kakaostorePrice < i.sellcash"
		End If

		'가격상이전체보기
		If FRectdiffPrc <> "" Then
			addSql = addSql & " and J.kakaostorePrice is Not Null and i.sellcash <> J.kakaostorePrice "
		End If

		'GSShop판매 10x10 품절
		If (FRectkakaostoreYes10x10No <> "") Then
			addSql = addSql & " and i.sellyn<>'Y'"
			addSql = addSql & " and J.kakaostoreSellYn='Y'"
		End If

		'kakaostore품절&텐바이텐판매가능(판매중,한정>=10) 상품보기
		If FRectkakaostoreNo10x10Yes <> "" Then
			addSql = addSql & " and (J.kakaostoreSellYn= 'N' and i.sellyn='Y' and (i.limityn='N' or (i.limityn='Y' and i.limitno-i.limitsold>"&CMAXLIMITSELL&")))"
		End If

		'수정요망상품보기(최종업데이트일 기준)
		If FRectReqEdit <> "" Then
			addSql = addSql & " and J.kakaostoreLastUpdate < i.lastupdate "
		End If
		...
		If (FRectOrdType = "LU") Then
		    ...
		    addSql = addSql & " and Left(i.lastupdate, 10) <> Left(J.kakaostoreLastUpdate, 10) "
		End If
		...
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
		    sqlStr = sqlStr & " 	LEFT JOIN db_etcmall.dbo.tbl_kakaostore_regitem as J WITH (NOLOCK) "
		Else
		    sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_kakaostore_regitem as J WITH (NOLOCK) "
	    END IF
		sqlStr = sqlStr & " 		on i.itemid=J.itemid "
		sqlStr = sqlStr & "	LEFT Join db_etcmall.dbo.tbl_kakaostore_cate_mapping as c WITH (NOLOCK) on c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small "
		sqlStr = sqlStr & " LEFT join db_user.dbo.tbl_user_c uc WITH (NOLOCK) on i.makerid = uc.userid"
		sqlStr = sqlStr & " LEFT JOIN db_etcmall.dbo.tbl_outmall_mustPriceItem as mi with (nolock) on mi.itemid = i.itemid and mi.mallgubun = 'kakaostore' "
		sqlStr = sqlStr & " LEFT JOIN [db_temp].dbo.tbl_schedule_not_in_itemid as sc with (nolock) on sc.itemid = i.itemid and sc.mallgubun = 'kakaostore' "
		sqlStr = sqlStr & " LEFT JOIN db_partner.dbo.tbl_partner_addInfo as f on f.partnerid = 'kakaostore' "
		...
		sqlStr = sqlStr & "	, J.kakaostoreRegdate, J.kakaostoreLastUpdate, isnull(J.kakaostoreGoodNo, '') as kakaostoreGoodNo, J.kakaostorePrice, J.kakaostoreSellYn, J.regUserid, IsNULL(J.kakaostoreStatCd,-9) as kakaostoreStatCd "
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new CKakaostoreItem
					...
					FItemList(i).FkakaostoreRegdate		= rsget("kakaostoreRegdate")
					FItemList(i).FkakaostoreLastUpdate	= rsget("kakaostoreLastUpdate")
					FItemList(i).FkakaostoreGoodNo		= rsget("kakaostoreGoodNo")
					FItemList(i).FkakaostorePrice		= rsget("kakaostorePrice")
					FItemList(i).FkakaostoreSellYn		= rsget("kakaostoreSellYn")
					FItemList(i).FRegUserid				= rsget("regUserid")
					FItemList(i).FkakaostoreStatCd		= rsget("kakaostoreStatCd")
    ...
    ''' 등록되지 말아야 될 상품..
    Public Sub getkakaostorereqExpireItemList
		Dim sqlStr, addSql, i
        If FRectMakerid <> "" Then
			addSql = addSql & " and i.makerid='" & FRectMakerid & "'"
		End if
		...
		sqlStr = sqlStr & " JOIN db_etcmall.dbo.tbl_kakaostore_regitem as J on J.itemid = i.itemid and J.kakaostoreGoodno is not null "
		sqlStr = sqlStr & " JOIN db_item.dbo.tbl_item_contents as ct on i.itemid = ct.itemid"
		sqlStr = sqlStr & "	LEFT Join db_etcmall.dbo.tbl_kakaostore_cate_mapping as c on c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small "
		...
		sqlStr = sqlStr & "	, i.sellYn, i.sailyn, i.LimitYn, i.LimitNo, i.LimitSold, i.deliverytype, i.optionCnt"
		sqlStr = sqlStr & "	, J.kakaostoreRegdate, J.kakaostoreLastUpdate, J.kakaostoreGoodNo, J.kakaostorePrice, J.kakaostoreSellYn, J.regUserid, IsNULL(J.kakaostoreStatCd,-9) as kakaostoreStatCd "
	...
	'// 텐바이텐-kakaostore 카테고리 리스트
	Public Sub getTenkakaostoreCateList
		...
		sqlStr = sqlStr & " 	SELECT cm.CateKey, cm.tenCateLarge,cm.tenCateMid, cm.tenCateSmall, cc.name, cc.catename "  & VBCRLF
		sqlStr = sqlStr & " 	FROM db_etcmall.dbo.tbl_kakaostore_cate_mapping as cm  "  & VBCRLF
		sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.[tbl_kakaostore_totalcategory] as cc on cc.CateKey = cm.CateKey  "  & VBCRLF
		...
		sqlStr = sqlStr & " ,T.CateKey, T.name, T.catename, W.itemcnt "  & VBCRLF
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_cate_small as s " & VBCRLF
		sqlStr = sqlStr & " LEFT JOIN (  "  & VBCRLF
		sqlStr = sqlStr & " 	SELECT cm.CateKey, cm.tenCateLarge,cm.tenCateMid, cm.tenCateSmall,cc.name, cc.catename "  & VBCRLF
		sqlStr = sqlStr & " 	FROM db_etcmall.dbo.tbl_kakaostore_cate_mapping as cm "  & VBCRLF
	...
	Public Sub getkakaostoreCateList
		Dim sqlStr, addSql, i

		If FsearchName <> "" Then
			addSql = addSql & " and (name like '%" & FsearchName & "%')"
		End If

		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(*) as cnt, CEILING(CAST(Count(*) AS FLOAT)/" & FPageSize & ") as totPg " & VBCRLF
		sqlStr = sqlStr & " FROM db_etcmall.dbo.[tbl_kakaostore_totalcategory] " & VBCRLF
		...
		sqlStr = sqlStr & " FROM db_etcmall.dbo.[tbl_kakaostore_totalcategory] " & VBCRLF
		sqlStr = sqlStr & " WHERE 1 = 1 " & addSql
		sqlStr = sqlStr & " order by name ASC "
		rsget.pagesize = FPageSize
		rsget.CursorLocation = adUseClient
		rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
		FResultCount = rsget.RecordCount-(FPageSize*(FCurrPage-1))
		...
		If not rsget.EOF Then
			...
			Do until rsget.eof
				Set FItemList(i) = new CKakaostoreItem
					...
					FItemList(i).FName					= rsget("name")
					FItemList(i).FCatename				= rsget("catename")
					FItemList(i).FCertKc				= rsget("certKc")
					FItemList(i).FCertFood				= rsget("certFood")
					FItemList(i).FMinorPurchasable		= rsget("minorPurchasable")
					FItemList(i).FSupplementTypes		= rsget("supplementTypes")
				i = i + 1
				rsget.moveNext
			Loop
		End If
