...
If cmdparam = "DELETE" Then
	...
	sqlStr = sqlStr &" SELECT 'kakaostore', i.itemid, r.kakaostoreGoodNo, isNull(r.kakaostoreRegdate, getdate()), getdate(), r.lastErrStr " & VBCRLF
	sqlStr = sqlStr &" FROM db_item.dbo.tbl_item as i " & VBCRLF
	sqlStr = sqlStr &" JOIN db_etcmall.dbo.tbl_kakaostore_regitem as r on i.itemid = r.itemid " & VBCRLF
	sqlStr = sqlStr &" WHERE i.itemid in (" & arrItemid & ") "
	dbget.Execute(sqlStr)

	sqlStr = ""
	sqlStr = sqlStr & " DELETE FROM db_etcmall.dbo.tbl_kakaostore_Image WHERE itemid in (" & arrItemid & ")" & VbCrlf
	dbget.Execute sqlStr,AssignedRow
	rw AssignedRow&"건 이미지 삭제"

	sqlStr = ""
	sqlStr = sqlStr & " DELETE FROM db_etcmall.dbo.tbl_kakaostore_regitem WHERE itemid in (" & arrItemid & ")" & VbCrlf
	dbget.Execute sqlStr,AssignedRow
	rw AssignedRow&"건 상품 삭제"

	sqlStr = ""
	sqlStr = sqlStr & " DELETE FROM db_item.dbo.tbl_kakaostore_regedoption WHERE itemid in (" & arrItemid & ") "
	dbget.Execute sqlStr,AssignedRow
	rw AssignedRow&"건 옵션 삭제"

	sqlStr = ""
	sqlStr = sqlStr & " DELETE FROM db_etcmall.dbo.tbl_outmall_API_Que WHERE itemid in (" & arrItemid & ") and mallid = 'kakaostore' "
	dbget.Execute sqlStr,AssignedRow
...
<script type="text/javascript">
	...
	function arrSubmit(ino){
        ...
        document.frmSvArr.action = '/admin/etc/kakaostore/kakaostoreActProc.asp';
