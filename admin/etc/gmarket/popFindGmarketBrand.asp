...
page		= request("page")
srcKwd		= Trim(request("srcKwd"))

If page = ""	Then page = 1
'// 목록 접수
Set oGmarket = new CGmarket
	oGmarket.FPageSize = 50
	oGmarket.FCurrPage = page
	oGmarket.FsearchName = srcKwd
	oGmarket.getGmarketBrandList
%>
...
<!--
	...
	// 상품분류 선택
	function fnSelDispBrand(bCode, dpmakername) {
		opener.document.frmAct.Brandcode.value=bCode;
		opener.document.getElementById("BrRow").style.display="";
		opener.document.getElementById("selBr").innerHTML= dpmakername;
		self.close();
	}
//-->
</script>
...
<tr valign="top">
	...
	<font color="red"><strong>Gmarket 브랜드 검색</strong></font></td>
...
%>
<tr align="center" height="25" onClick="fnSelDispBrand('<%= oGmarket.FItemList(i).FBrandCode %>', '<%= oGmarket.FItemList(i).FMakername %>');" style="cursor:pointer" title="브랜드 선택" bgcolor="#FFFFFF">
	<td><%= oGmarket.FItemList(i).FMakername %></td>
	<td><%= oGmarket.FItemList(i).FBrandCode %></td>
</tr>
<%
		Next
	End If
