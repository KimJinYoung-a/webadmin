...
Public Function fnebayCommonCode(iccd, goodsGrpCd)
	Dim obj, istrParam, iRbody, strObj, action
	...
	Select Case iccd
		...
		Case "placepolicy"					action = "PLACE_POLICY"
		Case "dispatchpolicy"				action = "DISPATCH_POLICY"
		Case "parcel"						action = "PARCELCODE"
		Case "origin"						action = "ORIGIN"
		...
		Case "addon"						action = "ADDON_SERVICE"
		...
		Set strObj = JSON.parse(iRbody)
			...
			ElseIf action = "OPTION_POLICY" Then
				...
				Else
					rw strObj.result.message
				End If
			ElseIf action = "PRODUCT_NOTICE_INFO_DETAIL" Then
				If IsNull(strObj.result.resultCode) Then
					Set infoCodes = strObj.result.codes
						For i = 0 to infoCodes.length - 1
							rw "코드 : " & infoCodes.get(i).officialNoticeItemelementCode
							rw "상세명 : " &  infoCodes.get(i).officialNoticeItemelementName
							rw "가이드 : " &  infoCodes.get(i).guideText
							rw "필수여부 : " &  infoCodes.get(i).isExtraMark
							rw "------------------------------------------------"
						Next
					Set infoCodes = nothing
				End If
