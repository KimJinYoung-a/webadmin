...
<!--
	...
	// ssg 표준카테고리 매칭 팝업
	function popSSGStdCateMap(cdl,cdm,cds,dno) {
		var pCM = window.open("popssgStdCateMap.asp?cdl="+cdl+"&cdm="+cdm+"&cds="+cds+"&dspNo="+dno,"popCateMap","width=1600,height=600,scrollbars=yes,resizable=yes");
		pCM.focus();
	}
//-->
</script>
...
<%
	Else
		For i = 0 to ossg.FresultCount - 1
			Depth1Nm = ossg.FItemList(i).FDepth1Nm
			Depth2Nm = ossg.FItemList(i).FDepth2Nm
			Depth3Nm = ossg.FItemList(i).FDepth3Nm
			Depth4Nm = ossg.FItemList(i).FDepth4Nm
			If Depth4Nm = "" Then
			...
			End If
%>
<tr align="center" height="25" bgcolor="<%= CHKIIF(IsNull(ossg.FItemList(i).FStdDepthCode),"#CCCCCC","#FFFFFF") %>">
	...
	<% If ossg.FItemList(i).FStdDepthCode="" OR isNull(ossg.FItemList(i).FStdDepthCode) Then %>
	<td colspan="4"><input type="button" class="button" value="ssg 표준카테고리 매칭" onClick="popSSGStdCateMap('<%= ossg.FItemList(i).FtenCateLarge %>','<%= ossg.FItemList(i).FtenCateMid %>','<%= ossg.FItemList(i).FtenCateSmall %>','')"></td>
	<% Else %>
	<td title="<%=cateAllNm%>" onClick="popSSGStdCateMap('<%= ossg.FItemList(i).FtenCateLarge %>','<%= ossg.FItemList(i).FtenCateMid %>','<%= ossg.FItemList(i).FtenCateSmall %>','<%=ossg.FItemList(i).FStdDepthCode%>')" style="cursor:pointer"><%= ossg.FItemList(i).FStdDepthCode %></td>
	<td title="<%=cateAllNm%>" onClick="popSSGStdCateMap('<%= ossg.FItemList(i).FtenCateLarge %>','<%= ossg.FItemList(i).FtenCateMid %>','<%= ossg.FItemList(i).FtenCateSmall %>','<%=ossg.FItemList(i).FStdDepthCode%>')" style="cursor:pointer"><%= Chkiif(ossg.FItemList(i).FDepth4Nm="", ossg.FItemList(i).FDepth3Nm, ossg.FItemList(i).FDepth4Nm) %></td>
