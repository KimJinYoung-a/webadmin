...
<!--
	...
	// 상품분류 선택
	function fnSelDispCate(stdcode, dp6nm) {
	   // alert(stdcode)
...
<%
	Else
		For i = 0 to ossg.FresultCount - 1
			...
			End If
%>
<tr align="center" height="25" onClick="fnSelDispCate('<%= ossg.FItemList(i).FStdDepthCode %>', '<%= replace(isNull4DpethNm, "'", "`") %>')" style="cursor:pointer" title="카테고리 선택" bgcolor="#FFFFFF">
	<td><%= ossg.FItemList(i).FStdDepthCode %></td>
