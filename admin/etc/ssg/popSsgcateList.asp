...
<!--
	...
	function popCjCateMap(cdl,cdm,cds,dno) {
		var pCM = window.open("popssgCateMap.asp?cdl="+cdl+"&cdm="+cdm+"&cds="+cds+"&dspNo="+dno,"popCateMap","width=1600,height=600,scrollbars=yes,resizable=yes");
		pCM.focus();
	}
//-->
</script>
...
<%
	Else
		For i = 0 to ossg.FresultCount - 1
			...
			End If
%>
<tr align="center" height="25" bgcolor="<%= CHKIIF(IsNull(ossg.FItemList(i).FDepthCode),"#CCCCCC","#FFFFFF") %>">
	...
	<% If ossg.FItemList(i).FDepthCode="" OR isNull(ossg.FItemList(i).FDepthCode) Then %>
	<td colspan="4"><input type="button" class="button" value="ssg 카테 매칭" onClick="popCjCateMap('<%= ossg.FItemList(i).FtenCateLarge %>','<%= ossg.FItemList(i).FtenCateMid %>','<%= ossg.FItemList(i).FtenCateSmall %>','')"></td>
	<% Else %>
	<td ><%= ossg.FItemList(i).getSiteNoToSiteName %></td>
	<td title="<%=cateAllNm%>" onClick="popCjCateMap('<%= ossg.FItemList(i).FtenCateLarge %>','<%= ossg.FItemList(i).FtenCateMid %>','<%= ossg.FItemList(i).FtenCateSmall %>','<%=ossg.FItemList(i).FDepthCode%>')" style="cursor:pointer"><%= ossg.FItemList(i).FDepthCode %></td>
	<td title="<%=cateAllNm%>" onClick="popCjCateMap('<%= ossg.FItemList(i).FtenCateLarge %>','<%= ossg.FItemList(i).FtenCateMid %>','<%= ossg.FItemList(i).FtenCateSmall %>','<%=ossg.FItemList(i).FDepthCode%>')" style="cursor:pointer"><%= Chkiif(ossg.FItemList(i).FDepth4Nm="", ossg.FItemList(i).FDepth3Nm, ossg.FItemList(i).FDepth4Nm) %></td>
