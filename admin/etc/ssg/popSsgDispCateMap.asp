...
<!--
	...
	function fnSaveForm() {
		...
		if(frm.DispCtgId.value=="") {
			alert("매칭할 SSG 전시 카테고리를 선택해주세요.");
			return;
		}

		if(confirm("선택하신 카테고리로 매칭하시겠습니까?")) {
			frm.mode.value="saveCate";
			frm.action="procDispSsg.asp";
			frm.submit();
		}
	}

    function fnDelForm(iDspNo, iSiteNo) {
		var frm = document.frmAct;
		if (iDspNo=="") {
		    alert("삭제할 SSG 표준 카테고리를 선택해주세요.");
			return;
		}

		if(confirm("현재 매칭된 카테고리를 연결해제 하시겠습니까?\n\n※ 상품 또는 카테고리가 삭제되는 것은 아니며, 연결된 정보만 삭제됩니다.")) {
			frm.mode.value="delCate";
			frm.DispCtgId.value=iDspNo;
			frm.siteNo.value=iSiteNo;
			frm.action="procDispSsg.asp";
			frm.submit();
		}
	}
	...
	function fnSearchSsgStdCate(disptpcd) {
		...
		srcFrm.action="popFindSsgDispCate.asp";
		srcFrm.submit();
	}
//-->
</script>
...
<tr valign="top">
	...
	<font color="red"><strong>SSG 표준 카테고리 매칭</strong></font></td>
...
<tr height="25">
	...
	<td bgcolor="#FFFFFF">
		...
		<input type="button" value="검색" class="button" onClick="fnSearchSsgStdCate();">
	</td>
</tr>
...
<% If Not IsNULL(oSsg.FItemList(i).FDispCtgId) Then %>
<%
	isNull4DeprgNm = oSsg.FItemList(i).FDispCtgNm
%>
<tr>
    <td bgcolor="#F2F2F2"><b><span id="selBr">[<%= ossg.FItemList(i).getSiteNoToSiteName %>]  <%=isNull4DeprgNm%> [<%=oSsg.FItemList(i).FDispCtgId%>] <%=oSsg.FItemList(i).FDispCtgId%></span></b>
    &nbsp;&nbsp;&nbsp;&nbsp;<img src="/images/icon_delete.gif" width="45" height="20" border="0" onclick="fnDelForm('<%=oSsg.FItemList(i).FDispCtgId%>','<%=oSsg.FItemList(i).FSiteNo%>')" style="cursor:pointer" align="absmiddle">
    </td>
</tr>
<% End If %>
<% Next %>
