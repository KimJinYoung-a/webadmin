...
ELSE
    ...
    elseif (mode="EditAll") then
        'dataUrl = "http://webadmin.10x10.co.kr/admin/etc/interparkXML/iparkEditItems.xml"
        dataUrl = "http://webadmin.10x10.co.kr/admin/etc/interparkXML/newRegedItem.asp?mode=EditPrd&brandid=" & request("brandid")
...
dim replyXML
dim i
dim ErrMsg, sqlStr, SuccCnt, pErrMsg
...
'            sqlStr = sqlStr & " where itemid=" & ioriginPrdNoList(i).firstChild.nodeValue
            ...
            '''=(CASE WHEN (i.deliveryType=9) and (i.Sellcash>IsNULL(c.defaultfreeBeasongLimit,0)) THEN 2"
            '''sqlStr = sqlStr & "                                 WHEN (i.deliveryType=9) THEN 6 else 2 END)"
            ...
            sqlStr = sqlStr & " and i.itemid="& ioriginPrdNoList(i).firstChild.nodeValue & VbCrlf
...
'                dbget.execute sqlStr
            ...
            pErrMsg =  "[" & ioriginPrdNoList(i).firstChild.nodeValue & "]" & iresultMessageList(i).firstChild.nodeValue
            ErrMsg = ErrMsg & pErrMsg & VbCrlf
             
            if (mode="DelPrd") or (mode="DelSoldOut") then
                ''인터 파크에서 사용 금지 한 경우
                if (iresultMessageList(i).firstChild.nodeValue="인증전이나 판매금지의 판매상태를 임의로 변경할 수 없습니다.") _
                    or (iresultMessageList(i).firstChild.nodeValue="판매금지, TNS판매금지 상태의 상품은 상품정보수정이 불가능합니다.") then
                    sqlStr = "delete from [db_item].[dbo].tbl_interpark_reg_item"
                    sqlStr = sqlStr & " where itemid=" & ioriginPrdNoList(i).firstChild.nodeValue
                ...
                '''임시
                if (iresultMessageList(i).firstChild.nodeValue="상품정보가 존재하지 않습니다.") then
                    sqlStr = "delete from [db_item].[dbo].tbl_interpark_reg_item"
                    sqlStr = sqlStr & " where itemid=" & ioriginPrdNoList(i).firstChild.nodeValue
            ...
            if (mode="EditAll") then  
                ''''if (iresultMessageList(i).firstChild.nodeValue="해당 상품의 수수료 설정이 잘못 되었습니다. 다시 확인해 주세요.") or (iresultMessageList(i).firstChild.nodeValue="특판행사상품은 옵션정보를 수정할 수 없습니다.") or (iresultMessageList(i).firstChild.nodeValue="인증전이나 판매금지의 판매상태를 임의로 변경할 수 없습니다.") or (iresultMessageList(i).firstChild.nodeValue="상품정보가 존재하지 않습니다.") then
                    ...
                    sqlStr = " insert into db_log.dbo.tbl_interparkEdit_log" & VbCrlf
                    sqlStr = sqlStr & " (itemid, interparkprdno,sellcash,buycash,sellyn, ErrMsg, errCode)" & VbCrlf
                    sqlStr = sqlStr & " select R.itemid, R.interparkprdno, i.sellcash,i.buycash,i.sellyn,convert(varchar(100),'"&pErrMsg&"') as ErrMsg, '"&iecodeList(i).firstChild.nodeValue&"' as errCode" & VbCrlf
                    sqlStr = sqlStr & "  from db_item.dbo.tbl_interpark_reg_item R" & VbCrlf
                    sqlStr = sqlStr & " 	Join db_item.dbo.tbl_item i" & VbCrlf
                    sqlStr = sqlStr & " 	on R.itemid=i.itemid" & VbCrlf
                    sqlStr = sqlStr & " where R.itemid=" & ioriginPrdNoList(i).firstChild.nodeValue & VbCrlf
                    ...
                    IF (InStr(pErrMsg,"API 스펙정의에")>0) then
                        ''Nothing  ''다시시도하면 될수 있음..;;
                        sqlStr = " DECLARE @ECNT int" &VbCRLF
                        sqlStr = sqlStr & " select @ECNT=count(*) from (" &VbCRLF
                        sqlStr = sqlStr & " 	select top 2 * from db_log.dbo.tbl_interparkEdit_log" &VbCRLF
                        sqlStr = sqlStr & " 	where itemid="& ioriginPrdNoList(i).firstChild.nodeValue & VbCrlf
                        sqlStr = sqlStr & " 	order by logidx desc" &VbCRLF
                        sqlStr = sqlStr & " ) T" &VbCRLF
                        sqlStr = sqlStr & " where T.Errcode='ECODE309'" &VbCRLF
                        
                        sqlStr = sqlStr & " IF (@ECNT=2)" &VbCRLF
                        sqlStr = sqlStr & " BEGIN" &VbCRLF
                        sqlStr = sqlStr & "     update [db_item].[dbo].tbl_interpark_reg_item" & VbCrlf
                        sqlStr = sqlStr & "     set interparklastupdate=getdate()" & VbCrlf
                        sqlStr = sqlStr & "     where itemid=" & ioriginPrdNoList(i).firstChild.nodeValue & VbCrlf
