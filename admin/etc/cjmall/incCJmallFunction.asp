...
function getCjMallPrdNoByItemID(byval iitemid)
    ...
    sqlStr = " select isNULL(cjmallprdno,'') as cjmallprdno from db_item.dbo.tbl_cjmall_regitem where itemid="&iitemid

    rsget.Open sqlStr, dbget
	If Not(rsget.EOF or rsget.BOF) Then
		ret = rsget("cjmallprdno")
	End If
	rsget.close
...
''2013-09-24 김진영 17:45 하단 프로시저 수정(단일상품일 때 자꾸 품절로 내려가서 단일상품의 경우 IF처리함)
				...
				sqlStr = "select count(*) as cnt from db_temp.dbo.tbl_jaehyumall_not_in_itemid where mallgubun = 'cjmall' and itemid =" & iitemid
				rsget.Open sqlStr, dbget
				If Not(rsget.EOF or rsget.BOF) Then
					notitemId = rsget("cnt")
				End If
				...
				sqlStr = "select count(*) as cnt from db_item.dbo.tbl_item as i join [db_temp].dbo.tbl_jaehyumall_not_in_makerid as m on i.makerid = m.makerid where i.itemid = "& iitemid&" and m.mallgubun = 'cjmall'"
				rsget.Open sqlStr, dbget
				If Not(rsget.EOF or rsget.BOF) Then
...
Function saveSellYNItemResult(retDoc, mode, prdno, smdcd)
	...
	If mode = "MDT" Then
		...
		If (Not (retDoc is Nothing)) Then
			...
			Dim sellynCnt
			sqlStr = ""
			sqlStr = sqlStr & " SELECT count(*) as cnt FROM db_item.dbo.tbl_Outmall_regedoption WHERE itemid="&prdno&" and mallid = 'cjmall' and outmallSellyn = 'Y' "
			rsget.Open sqlStr, dbget
				sellynCnt = rsget("cnt")
			rsget.Close
            sqlStr = ""
            sqlStr = sqlStr & " update db_item.dbo.tbl_cjmall_regItem set cjmallLastUpdate = getdate()" & VBCRLF
			If (sellynCnt > 0) Then
				sqlStr = sqlStr & " ,cjmallSellyn = 'Y' " & VBCRLF
...
function saveORDOneTmp(OutMallOrderSerial,SellDate,matchItemID,matchItemOption,partnerItemName,partnerOptionName _
        ...
        , byref ierrCode, byref ierrStr, itemCd )
    ...
    paramInfo = Array(Array("@RETURN_VALUE",adInteger,adParamReturnValue,,0) _
    	...
    	,Array("@outMallGoodsNo"	,adVarchar, adParamInput,16, itemCd) _
		...
		,Array("@overseasPrice"	,adCurrency, adParamInput,, "") _
		,Array("@overseasDeliveryPrice"	,adCurrency, adParamInput,, "") _
		,Array("@overseasRealPrice"	,adCurrency, adParamInput,, "") _
		,Array("@beasongNum11st"	,adCurrency, adParamInput,16, "") _
	)
