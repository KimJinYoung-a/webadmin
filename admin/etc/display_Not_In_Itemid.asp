...
SET oOutMall = new cOutmall
	...
	strSql = "SELECT Count(A.idx) FROM [db_temp].[dbo].[tbl_display_not_in_itemid] AS A "
	...
	strSql = "SELECT Top 15 A.itemid, I.itemname, A.mallgubun FROM [db_temp].[dbo].[tbl_display_not_in_itemid] AS A "
	strSql = strSql & "		INNER JOIN [db_item].[dbo].[tbl_item] AS I ON A.itemid = I.itemid "
	strSql = strSql & "Where 1=1 "
	If targetmall2 = "sel"  Then
		strSql = strSql & " AND A.mallgubun in ('" & tmpMallList & "') "
	...
	If vItemName <> ""  Then
		strSql = strSql & " AND I.itemname Like '%" & vItemName & "%' "
	End If
	strSql = strSql & "		AND A.idx NOT IN(SELECT TOP "&iDelCnt&" X.idx FROM [db_temp].[dbo].[tbl_display_not_in_itemid] AS X "
...
<form name="frm" action="display_Not_In_itemid.asp" methd="post" style="margin:0px;">
...
Function Proc()
	...
	If vAction = "insert" Then
					...
					for j = 0 to UBound(arrItemid)
						if Trim(arrItemid(j)) <> "" then
							...
							strSql = "	DECLARE @Temp CHAR(1) " & _
									 "	If NOT EXISTS(SELECT * FROM [db_temp].dbo.tbl_display_not_in_itemid Where mallgubun = '" & vMallGubun & "' AND itemid = '" & vItemid & "') " & _
									 "		BEGIN " & _
							 		 "			INSERT INTO [db_temp].dbo.tbl_display_not_in_itemid(itemid,mallgubun) VALUES('" & vItemid & "','" & vMallGubun & "') " & _
							 		 "		END	"
		...
		ElseIf targetmall = "sel" Then
			...
			Response.Write "<script>alert('처리되었습니다.');location.href='display_Not_In_itemid.asp?mallgubun=" & vMallGubun & "&cp=" & vCurrPage & "&targetmall=" & targetmall & "&arrtargetmall=" & arrtargetmall & "';</script>"
			Response.End
		Else
			arrItemid = Split(arrItemid, ",")
			for j = 0 to UBound(arrItemid)
	...
	ElseIf vAction = "delete" Then
		...
		strSql = "DELETE [db_temp].dbo.tbl_display_not_in_itemid WHERE mallgubun = '" & vMallGubun & "' AND itemid IN(" & vItemid & ")"
		dbget.execute strSql
	End IF

	Response.Write "<script>alert('처리되었습니다.');location.href='display_Not_In_itemid.asp?mallgubun=" & vMallGubun & "&cp=" & vCurrPage & "';</script>"
	Response.End
End Function
%>
