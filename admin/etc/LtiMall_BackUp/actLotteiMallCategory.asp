				...
				If CInt(CateCnt) > 0 Then
					Set CateInfo = xmlDOM.getElementsByTagName("CategoryInfoList")
						For each SubNodes in CateInfo
							...
							'MD존재여부 확인
							strSql = "Select count(DispNo) From db_temp.dbo.tbl_lotteiMall_Category Where DispNo='" & DispNo & "'"
							rsget.Open strSql,dbget,1
							If rsget(0) > 0 Then
								'// 존재 -> 사용함
								strSql = "update db_temp.dbo.tbl_lotteiMall_Category Set isUsing='Y', groupCode='" & arrMDGrNo(lp) & "', disptpcd='"&disp_tp_cd&"' Where DispNo='" & DispNo & "'"
								dbget.Execute(strSql)
							Else
								'// 없음 -> 신규등록
