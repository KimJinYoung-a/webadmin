...
Class CLotteiMallItem
	...
	Public Function getLotteItemStatCd()
	    Select Case FLotteStatCd
			...
			CASE ELSE
			    getLotteItemStatCd = FLotteStatCd
		End Select
	End Function
...
class CLotteiMall
	...
	Public Sub getLTiMallRegedItemList
		...
		If FRectLTiMallTmpGoodNo <> "" Then
			addSql = addSql & " and m.LTiMallTmpGoodNo = '" & FRectLTiMallTmpGoodNo & "'"
		End If
		...
		If FRectEventid <> "" Then
			addSql = addSql & " and i.itemid in (Select itemid From [db_event].[dbo].tbl_eventitem Where evt_code='" & FRectEventid & "')"
		End If
		...
		Select Case FRectLotteNotReg
		    ...
		    Case "A"	'전송시도
				addSql = addSql & " and m.LTiMallStatCd=1"
			Case "F"	'등록완료(임시)
			    addSql = addSql & " and m.LTiMallStatCd=20"
...
'		    addSql = addSql & " and isNULL(m.optAddPrcCnt,0)=0"
        ...
        if (FRect10000_Over<>"") then
            addSql = addSql & " and i.sellcash > 10000"
...
''rw sqlStr
		...
		sqlStr = sqlStr & "	, Ct.infoDiv, m.optAddPrcCnt, m.optAddPrcRegType, i.itemdiv"
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_item as i "
		If (FRectLotteNotReg <> "M") and (FRectLotteNotReg <> "") Then
		    sqlStr = sqlStr & " 	JOIN db_item.dbo.tbl_LTiMall_regItem as m "
		Else
		...
		If (FRectOrdType = "LS") AND (FRectLotteNotReg = "F") Then
			sqlStr = sqlStr & " ORDER BY m.lastStatCheckDate, m.LtiMallLastupdate"
