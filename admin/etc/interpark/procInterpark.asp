...
CateKey = requestCheckvar(Request("CateKey"),100)
...
Select Case mode
	Case "saveCate"
        ...
        If categbn = "cate" Then
	        sqlStr = "SELECT COUNT(*) as cnt FROM db_etcmall.dbo.tbl_interpark_cate_mapping "  & VbCrlf
		...
		If joongBok = False Then
			sqlStr = ""
			sqlStr = sqlStr & " INSERT INTO db_etcmall.dbo.tbl_interpark_cate_mapping  " & VbCrlf
		...
		End If
	Case "delCate"
		sqlStr = "DELETE FROM db_etcmall.dbo.tbl_interpark_cate_mapping " & VbCrlf
		sqlStr = sqlStr& " WHERE tenCateLarge='" & cdl & "'" & VbCrlf
		sqlStr = sqlStr& " 	and tenCateMid='" & cdm & "'" & VbCrlf
		sqlStr = sqlStr& " 	and tenCateSmall='" & cds & "'" & VbCrlf
		dbget.execute(sqlStr)
