...
<%
Dim oEzwel, i, dispCate
dispCate	= request("catecode")
...
<!--
	...
	function fnSaveForm() {
		...
		if(frm.depthcode.value=="") {
			alert("매칭할 Ezwel 카테고리를 선택해주세요.");
			return;
		}

		if(confirm("선택하신 카테고리로 매칭하시겠습니까?")) {
			frm.mode.value="saveCate";
			frm.action="procDispezwel.asp";
			frm.submit();
		}
	}
	...
	// Ezwel 카테고리 검색
	function fnSearchCate() {
		var pFCL = window.open("","popezwelCate","width=900,height=700,scrollbars=yes,resizable=yes");
		pFCL.focus();
		srcFrm.target="popezwelCate";
		srcFrm.action="popFindNewezwelCate.asp";
		srcFrm.submit();
	}
//-->
</script>
...
<tr valign="top">
	...
	<font color="red"><strong>Ezwel 카테고리 매칭</strong></font></td>
...
<tr height="25">
	<td width="80" align="center" bgcolor="#DDDDFF">[<%=dispCate%>]</td>
	<td bgcolor="#FFFFFF"><%=oEzwel.FItemList(0).FCateName%></td>
</tr>
</table>
<!-- 표 중간바 시작-->
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="a" bgcolor="F4F4F4">
<tr height="5" valign="top">
	<td width="10" align="right" background="/images/tbl_blue_round_04.gif"></td>
	<td align="left"><img src="/images/icon_arrow_down.gif" border="0" vspace="5" align="absmiddle"> Ezwel 카테고리 매칭 정보</td>
	<td width="10" align="left" background="/images/tbl_blue_round_05.gif"></td>
</tr>
</table>
<!-- 표 중간바 끝-->
...
<tr height="25">
	...
	<td bgcolor="#FFFFFF">
		...
		<input type="button" value="검색" class="button" onClick="fnSearchCate();">
	</td>
</tr>
...
<tr height="25">
    <td id="brTT" width="80" align="center" bgcolor="#DDDDFF" rowspan="<%= oEzwel.FResultCount + 1 %>" >등록된<br>카테고리</td>
	<td bgcolor="#FFFFFF" height="1"></td>
</tr>
<% For i = 0 to oEzwel.FResultCount - 1 %>
<% If Not IsNULL(oEzwel.FItemList(i).FDepthCode) Then %>
<tr>
    <td bgcolor="#F2F2F2"><b><span id="selBr"><%=oEzwel.FItemList(i).FDepth4Nm%> [<%=oEzwel.FItemList(i).FDepthCode%>] <%=oEzwel.FItemList(i).FDepthCode%></span></b>
    &nbsp;&nbsp;&nbsp;&nbsp;<img src="/images/icon_delete.gif" width="45" height="20" border="0" onclick="fnDelForm('<%=oEzwel.FItemList(i).FDepthCode%>')" style="cursor:pointer" align="absmiddle">
    </td>
</tr>
<% End If %>
<% Next %>
...
<input type="hidden" name="catecode" value="<%= dispCate %>">
