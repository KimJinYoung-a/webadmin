...
IF (prctp="3") then
	Call fnEzwelDlvFinish(ord_no, ord_dtl_sn)
	dbget.Close() 
	response.end
End IF

Call fnEzwelSongjangUpload(ord_no, ord_dtl_sn, hdc_cd, inv_no, retCode, iMessage, failCount, ORG_ord_no)
...
If NOT(IsSuccss) Then
	...
	If failCount > 0 Then
		...
		response.write "    var uri = 'ezwelActProc.asp?act=updateSendState&ord_no='+ORG_ord_no+'&ord_dtl_sn='+ord_dtl_sn+'&updateSendState='+selectValue;"&VbCRLF
