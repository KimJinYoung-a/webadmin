...
Set oLfmall = new CLfmall
	...
	oLfmall.FCurrPage = 1
	oLfmall.FRectCDL = cdl
	oLfmall.FRectCDM = cdm
	oLfmall.FRectCDS = cds
...
<!--
	...
	function fnSaveForm() {
		...
		if(frm.itemkindcode.value=="") {
			alert("매칭할 Lfmall 상품분류를 선택해주세요.");
			return;
		}

		if(confirm("선택하신 상품분류로 매칭하시겠습니까?")) {
			frm.mode.value="saveNoti";
			frm.action="proclfmall.asp";
			frm.submit();
		}
	}
    ...
    function fnDelForm(iDspNo) {
		var frm = document.frmAct;
		if (iDspNo=="") {
		    alert("삭제할 Lfmall 상품분류를 선택해주세요.");
			return;
		}

		if(confirm("현재 매칭된 상품분류를 연결해제 하시겠습니까?\n\n※ 상품 또는 상품분류가 삭제되는 것은 아니며, 연결된 정보만 삭제됩니다.")) {
			frm.mode.value="delNoti";
			frm.itemkindcode.value=iDspNo;
			frm.action="proclfmall.asp";
			frm.submit();
		}
	}
	...
	// Lfmall 상품분류 검색
	function fnSearchLfmallNoti(disptpcd) {
		var pFCL = window.open("","popLfmallNoti","width=1000,height=700,scrollbars=yes,resizable=yes");
		pFCL.focus();
		srcFrm.target="popLfmallNoti";
		srcFrm.action="popFindlfmallNoti.asp";
		srcFrm.submit();
	}
//-->
</script>
...
<tr valign="top">
	...
	<font color="red"><strong>Lfmall 상품분류 매칭</strong></font></td>
...
<tr height="25">
	...
	<td bgcolor="#FFFFFF">[<%=cdl%>] <%=oLfmall.FItemList(0).FtenCDLName%></td>
</tr>
<tr height="25">
	<td width="80" align="center" bgcolor="#DDDDFF">중분류</td>
	<td bgcolor="#FFFFFF">[<%=cdm%>] <%=oLfmall.FItemList(0).FtenCDMName%></td>
</tr>
<tr height="25">
	<td width="80" align="center" bgcolor="#DDDDFF">소분류</td>
	<td bgcolor="#FFFFFF">[<%=cds%>] <%=oLfmall.FItemList(0).FtenCDSName%></td>
</tr>
</table>
...
<tr height="5" valign="top">
	<td width="10" align="right" background="/images/tbl_blue_round_04.gif"></td>
	<td align="left"><img src="/images/icon_arrow_down.gif" border="0" vspace="5" align="absmiddle"> Lfmall 상품분류 매칭 정보</td>
	<td width="10" align="left" background="/images/tbl_blue_round_05.gif"></td>
</tr>
</table>
<!-- 표 중간바 끝-->
...
<tr height="25">
	...
	<td bgcolor="#FFFFFF">
		상품분류명 <input type="text" name="srcKwd" class="text">
		<input type="button" value="검색" class="button" onClick="fnSearchLfmallNoti();">
	</td>
</tr>
...
<tr height="25">
    <td id="brTT" width="80" align="center" bgcolor="#DDDDFF" rowspan="<%= oLfmall.FResultCount + 1 %>" >등록된<br>상품분류</td>
	<td bgcolor="#FFFFFF" height="1"></td>
</tr>
<% For i = 0 to oLfmall.FResultCount - 1 %>
<% If Not IsNULL(oLfmall.FItemList(i).FItemkindcode) Then %>
<tr>
    <td bgcolor="#F2F2F2"><b><span id="selBr"><%=oLfmall.FItemList(i).FItemkindname%> [<%=oLfmall.FItemList(i).FItemkindcode%>] <%=oLfmall.FItemList(i).FItemkindcode%></span></b>
    &nbsp;&nbsp;&nbsp;&nbsp;<img src="/images/icon_delete.gif" width="45" height="20" border="0" onclick="fnDelForm('<%=oLfmall.FItemList(i).FItemkindcode%>')" style="cursor:pointer" align="absmiddle">
    </td>
</tr>
<% End If %>
<% Next %>
...
<input type="hidden" name="nitypecd" value="">
<input type="hidden" name="mode" value="saveNoti">
<input type="hidden" name="categbn" value="noti">
</form>
