...
<tr valign="top">
	...
	<font color="red"><strong>Hmall 카테고리 검색</strong></font></td>
...
<%
	Else
		For i = 0 to oHmall.FresultCount - 1
			...
			End If
%>
<tr align="center" height="25" onClick="fnSelDispCate('<%= oHmall.FItemList(i).FCateKey %>', '<%= replace(isNullDpethNm, "'", "`") %>')" style="cursor:pointer" title="카테고리 선택" bgcolor="#FFFFFF">
	<td><%= oHmall.FItemList(i).FCateKey %></td>
	<td><%= oHmall.FItemList(i).Fdepth1Name %></td>
	<td><%= oHmall.FItemList(i).Fdepth2Name %></td>
	<td><%= oHmall.FItemList(i).Fdepth3Name %></td>
	<td><%= oHmall.FItemList(i).Fdepth4Name %></td>
</tr>
<%
		Next
	End If
