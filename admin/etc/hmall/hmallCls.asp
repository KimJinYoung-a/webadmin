...
Class CHmallItem
	...
	Public Function getHmallStatName
	    If IsNULL(FHmallStatcd) then FHmallStatcd=-1
		Select Case FHmallStatcd
			CASE -9 : getHmallStatName = "미등록"
			CASE -1 : getHmallStatName = "등록실패"
			CASE 0 : getHmallStatName = "<font color=blue>등록예정</font>"
			CASE 3 : getHmallStatName = "승인예정"
			CASE 2 : getHmallStatName = "반려"
			CASE 4 : getHmallStatName = "취소"
			CASE 1 : getHmallStatName = "전송시도"
			CASE 7 : getHmallStatName = ""
			CASE ELSE : getHmallStatName = FHmallStatcd
		End Select
	End Function
...
Class CHmall
	...
	Public FRectMakerid
	Public FRectHmallGoodNo
	Public FRectHmallGoodNo2
	Public FRectMatchCate
	Public FRectMatchIMG
	Public FRectoptExists
	Public FRectoptnotExists
	Public FRectHmallNotReg
	Public FRectMinusMigin
	Public FRectExpensive10x10
	Public FRectdiffPrc
	Public FRectHmallYes10x10No
	Public FRectHmallNo10x10Yes
	...
	'// HMall 상품 목록 // 수정시 조건이 달라야 함..
	Public Sub getHmallRegedItemList
		Dim i, sqlStr, addSql
		'브랜드검색
		...
		'Hmall 상품번호 검색
        If (FRectHmallGoodNo <> "") then
            If Right(Trim(FRectHmallGoodNo) ,1) = "," Then
            	FRectItemid = Replace(FRectHmallGoodNo,",,",",")
            	addSql = addSql & " and J.HmallGoodNo2 in (" & Left(FRectHmallGoodNo & "", Len(FRectHmallGoodNo)-1) & ")"
            Else
				FRectHmallGoodNo = Replace(FRectHmallGoodNo,",,",",")
            	addSql = addSql & " and J.HmallGoodNo2 in (" & FRectHmallGoodNo & ")"
            End If
        End If
		...
		Select Case FRectExtNotReg
		    Case "A"	'전송시도중오류
				addSql = addSql & " and J.HmallStatCd = 1"
			Case "W"	'등록예정이상
				addSql = addSql & " and J.HmallStatCd >= 0"
			Case "C"	'반려
				addSql = addSql & " and J.HmallStatCd = 2"
			Case "E"	'취소
				addSql = addSql & " and J.HmallStatCd = 4"
			Case "F"	'등록후 승인대기(임시)
				addSql = addSql & " and J.HmallStatCd = 3"
				addSql = addSql & " and J.HmallGoodNo is Not Null"
			Case "D"	'등록완료(전시)
			    addSql = addSql & " and J.HmallStatCd = 7"
				addSql = addSql & " and J.HmallGoodNo is Not Null"
		End Select
		...
		'HMall 이미지 오류 여부
		Select Case FRectMatchIMG
			Case "Y"	'등록
				addSql = addSql & " and ((isnull(J.APIaddImg, '') = 'Y') and (isnull(J.APIconfirmImg, '') = 'Y'))  "
			Case "N"	'미등록
				addSql = addSql & " and ((isnull(J.APIaddImg, '') <> 'Y') OR (isnull(J.APIconfirmImg, '') <> 'Y'))  "
		End Select

        'HMall가격 < 10x10 가격
		If (FRectexpensive10x10 <> "") Then
			addSql = addSql & " and J.HmallPrice is Not Null and J.HmallPrice < i.sellcash"
		End If

		'가격상이전체보기
		If FRectdiffPrc <> "" Then
			addSql = addSql & " and J.HmallPrice is Not Null and i.sellcash <> J.HmallPrice "
		End If

		'HMall판매 10x10 품절
		If (FRectHmallYes10x10No <> "") Then
			addSql = addSql & " and i.sellyn<>'Y'"
			addSql = addSql & " and J.HmallSellYn='Y'"
		End If

		'HMall품절&텐바이텐판매가능(판매중,한정>=10) 상품보기
		If FRectHmallNo10x10Yes <> "" Then
			addSql = addSql & " and (J.HmallSellYn= 'N' and i.sellyn='Y' and (i.limityn='N' or (i.limityn='Y' and i.limitno-i.limitsold>"&CMAXLIMITSELL&")))"
		End If

		'수정요망상품보기(최종업데이트일 기준)
		If FRectReqEdit <> "" Then
			addSql = addSql & " and J.HmallLastUpdate < i.lastupdate "
		End If
		...
		If (FRectOrdType = "LU") Then
		    ...
		    addSql = addSql & " and Left(i.lastupdate, 10) <> Left(J.HmallLastUpdate, 10) "
		End If
		...
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
		    sqlStr = sqlStr & " 	LEFT JOIN db_etcmall.dbo.tbl_hmall_regitem as J with (nolock) "
		Else
		    sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_hmall_regitem as J with (nolock) "
	    END IF
		sqlStr = sqlStr & " 		on i.itemid=J.itemid "
		sqlStr = sqlStr & "	LEFT JOIN db_etcmall.[dbo].[tbl_hmall_cate_mapping] as c with (nolock) on c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small "
		...
		sqlStr = sqlStr & "	, J.hmallRegdate, J.hmallLastUpdate, isnull(J.hmallGoodNo, '') as hmallGoodNo, isnull(J.hmallGoodNo2, '') as hmallGoodNo2, J.hmallPrice, J.hmallSellYn, J.regUserid, IsNULL(J.hmallStatCd,-9) as hmallStatCd "
		sqlStr = sqlStr & "	, Case When isnull(c.cateKey, '') = '' Then 0 Else 1 End as mapcnt "
		sqlStr = sqlStr & " , J.regedOptCnt, J.rctSellCNT, J.accFailCNT, J.lastErrStr, J.setMargin "
		...
		sqlStr = sqlStr & "	, isnull(J.APIaddImg, 'N') as APIaddImg, isnull(J.APIconfirmImg, 'N') as APIconfirmImg, sc.idx as notSchIdx, p.purchasetype "
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_item as i with (nolock) "
		sqlStr = sqlStr & " JOIN db_item.dbo.tbl_item_contents as ct with (nolock) on i.itemid = ct.itemid"
		sqlStr = sqlStr & " JOIN db_partner.dbo.tbl_partner as p with (nolock) on i.makerid = p.id"
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new CHmallItem
					...
					FItemList(i).FHmallGoodNo2				= rsget("hmallGoodNo2")
					...
					FItemList(i).FAPIconfirmImg				= rsget("APIconfirmImg")
    ...
    Public Sub getHmallreqExpireItemList
		...
		sqlStr = sqlStr & " JOIN db_etcmall.dbo.tbl_hmall_regitem as J on J.itemid = i.itemid and J.hmallGoodno is not null "
...
'			exit sub
	...
	'// 텐바이텐-hmall 카테고리 리스트
	Public Sub getTenHmallCateList
		Dim sqlStr, addSql, i
		...
		sqlStr = sqlStr & " 	FROM db_etcmall.dbo.tbl_hmall_cate_mapping as cm  "  & VBCRLF
		sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_hmall_category as cc on cc.CateKey = cm.CateKey  "  & VBCRLF
	...
	Public Sub getHmallCateList
		...
		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(*) as cnt, CEILING(CAST(Count(*) AS FLOAT)/" & FPageSize & ") as totPg " & VBCRLF
		sqlStr = sqlStr & " FROM db_etcmall.dbo.tbl_hmall_category " & VBCRLF
		sqlStr = sqlStr & " WHERE 1 = 1 "
		sqlStr = sqlStr & " and infNotfBsicCd <> '' "
		sqlStr = sqlStr & addSql
		rsget.Open sqlStr,dbget,1
			FTotalCount = rsget("cnt")
			FTotalPage = rsget("totPg")
	...
	Public Sub getHmallSectList
		Dim sqlStr, addSql, i

		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(*) as cnt, CEILING(CAST(Count(*) AS FLOAT)/" & FPageSize & ") as totPg " & VBCRLF
		sqlStr = sqlStr & " FROM db_etcmall.[dbo].[tbl_hmall_sectId] " & VBCRLF
		sqlStr = sqlStr & " WHERE 1 = 1 "
		sqlStr = sqlStr & addSql
		rsget.Open sqlStr,dbget,1
			FTotalCount = rsget("cnt")
		...
		sqlStr = sqlStr & " WHERE 1 = 1 "
		sqlStr = sqlStr & addSql
		sqlStr = sqlStr & " order by sectNmPath, sectId ASC "
		rsget.pagesize = FPageSize
		rsget.Open sqlStr,dbget,1
		FResultCount = rsget.RecordCount-(FPageSize*(FCurrPage-1))
		Redim preserve FItemList(FResultCount)
		...
		If not rsget.EOF Then
			...
			Do until rsget.eof
				Set FItemList(i) = new CHmallItem
					FItemList(i).FSectId		= rsget("sectId")
					FItemList(i).FSectNmPath	= rsget("sectNmPath")
					FItemList(i).FVenDispYn		= rsget("venDispYn")
					FItemList(i).FRegdate		= rsget("regdate")
				i = i + 1
				rsget.moveNext
			Loop
	...
	Public Function getHmallSectMapList
		Dim sqlStr, addSql, i

		sqlStr = ""
		sqlStr = sqlStr & " SELECT m.sectId1, m.sectId2, v.nmlarge, v.nmmid "
		sqlStr = sqlStr & " FROM db_etcmall.[dbo].[tbl_hmall_sepcial_cate_mapping] m  "
		sqlStr = sqlStr & " JOIN db_item.dbo.vw_category v on m.tenCateLarge = v.cdlarge and m.tenCateMid = v.cdmid "
		sqlStr = sqlStr & " GROUP BY m.sectId1, m.sectId2, v.nmlarge, v.nmmid "
		sqlStr = sqlStr & " order by v.nmlarge, v.nmmid "
		rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
		If not rsget.EOF Then
			getHmallSectMapList = rsget.getRows()
		End If
		rsget.Close
	End Function
