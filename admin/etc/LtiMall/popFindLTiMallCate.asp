...
<%
Dim oiMall, i, page, isMapping, srcDiv, srcKwd, grpCd
Dim disptpcd
...
<!--
	...
	function refreshLotteCate(disptpcd) {
		if(confirm("전시 카테고리를 롯데아이몰 서버에서 내려받아 갱신하시겠습니까?\n\n※ 1.통신상태에따라 다소 시간이 많이 걸릴 수 있습니다.\n※ 2.현재등록 되어있는 카테고리를 리셋하고 다시 롯데아이몰의 정보를 가져오는 것이므로 신중하게 결정하세요.")) {
			document.getElementById("btnRefresh").disabled=true;
			xLink.location.href="actLotteiMallCategory.asp?disptpcd="+disptpcd;
		}
	}
...
<tr valign="top">
	...
	<font color="red"><strong>롯데아이몰 카테고리 검색</strong></font></td>
...
%>
<tr align="center" height="25" onClick="fnSelCate('<%= oiMall.FItemList(i).FDispNo %>','<%=oiMall.FItemList(i).FDispNm%>')" style="cursor:pointer" title="카테고리 선택" bgcolor="<%=chkIIF(oiMall.FItemList(i).FisUsing="Y","#FFFFFF","#DDDDDD")%>">
	<td><%= oiMall.FItemList(i).getDisptpcdName %></td>
