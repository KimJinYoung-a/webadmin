...
Class CNvstoregiftItem
	...
	Public FLimitNo
	Public FLimitSold
	Public FNvstoregiftRegdate
	Public FNvstoregiftLastUpdate
	...
	Public Function getNvstoregiftStatName
	    If IsNULL(FNvstoregiftStatCd) then FNvstoregiftStatCd=-1
		Select Case FNvstoregiftStatCd
			CASE -9 : getNvstoregiftStatName = "미등록"
			CASE -1 : getNvstoregiftStatName = "등록실패"
			CASE 0 : getNvstoregiftStatName = "<font color=blue>등록예정</font>"
			CASE 1 : getNvstoregiftStatName = "전송시도"
			CASE 7 : getNvstoregiftStatName = ""
			CASE ELSE : getNvstoregiftStatName = FNvstoregiftStatCd
		End Select
	End Function
...
Class CNvstoregift
	...
	Public FRectMakerid
	Public FRectNvstoregiftGoodNo
	Public FRectMatchCate
	Public FRectoptExists
	Public FRectoptnotExists
	Public FRectNvstoregiftNotReg
	Public FRectMinusMigin
	Public FRectExpensive10x10
	Public FRectdiffPrc
	Public FRectNvstoregiftYes10x10No
	Public FRectNvstoregiftNo10x10Yes
	...
	Public Sub getNvstoregiftRegedItemList
		Dim i, sqlStr, addSql
		'브랜드검색
		...
		'스토어팜 상품번호 검색
        If (FRectNvstoregiftGoodNo <> "") then
            If Right(Trim(FRectNvstoregiftGoodNo) ,1) = "," Then
            	FRectNvstoregiftGoodNo = Replace(FRectNvstoregiftGoodNo,",,",",")
            	FRectNvstoregiftGoodNo = Replace(FRectNvstoregiftGoodNo,"''","'")
            	addSql = addSql & " and J.nvstoregiftGoodNo in (" & Left(FRectNvstoregiftGoodNo, Len(FRectNvstoregiftGoodNo)-1) & ")"
            Else
				FRectNvstoregiftGoodNo = Replace(FRectNvstoregiftGoodNo,",,",",")
				FRectNvstoregiftGoodNo = Replace(FRectNvstoregiftGoodNo,"''","'")
            	addSql = addSql & " and J.nvstoregiftGoodNo in (" & FRectNvstoregiftGoodNo & ")"
            End If
        End If
		...
		Select Case FRectExtNotReg
			Case "Q"	''등록실패
				addSql = addSql & " and J.nvstoregiftStatCd = -1"
			Case "J"	'등록예정이상
				addSql = addSql & " and J.nvstoregiftStatCd >= 0"
		    Case "A"	'전송시도중오류
				addSql = addSql & " and J.nvstoregiftStatCd = 1"
		    Case "I"	'이미지만 완료
				addSql = addSql & " and isnull(J.nvstoregiftGoodNo, '') = '' "
				addSql = addSql & " and J.APIaddImg = 'Y'"
			Case "D"	'등록완료(전시)
			    addSql = addSql & " and J.nvstoregiftStatCd = 7"
				addSql = addSql & " and J.nvstoregiftGoodNo is Not Null"
		End Select
		...
		If (FRectNotinmakerid <> "") then
			If (FRectNotinmakerid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'nvstoregift') "
			ElseIf (FRectNotinmakerid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'nvstoregift') "
			End If
		End If
		...
		If (FRectNotinitemid <> "") then
			If (FRectNotinitemid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'nvstoregift') "
			ElseIf (FRectNotinitemid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'nvstoregift') "
			End If
		End If
		...
		If (FRectExcTrans <> "") then
			If (FRectExcTrans = "Y") Then
				...
				addSql = addSql & " or exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'nvstoregift') "
				addSql = addSql & " or exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'nvstoregift') "
...
'				addSql = addSql & " and uc.isExtUsing='Y' "											'// 2018-12-03 김진영 수정 // 제휴판매안함이라도 스토어팜 판매 가능이면 판매
		...
		If (FRectExtSellYn<>"") then
			If (FRectExtSellYn = "YN") Then
				addSql = addSql & " and J.nvstoregiftSellYn <> 'X'"
			Else
				addSql = addSql & " and J.nvstoregiftSellYn='" & FRectExtSellYn & "'"
			End if
		End If
		...
		If (FRectexpensive10x10 <> "") Then
			addSql = addSql & " and J.nvstoregiftPrice is Not Null and J.nvstoregiftPrice < i.sellcash"
		End If

		'가격상이전체보기
		If FRectdiffPrc <> "" Then
			addSql = addSql & " and J.nvstoregiftPrice is Not Null and i.sellcash <> J.nvstoregiftPrice "
		End If

		'스토어팜 판매 10x10 품절
		If (FRectNvstoregiftYes10x10No <> "") Then
			addSql = addSql & " and i.sellyn<>'Y'"
			addSql = addSql & " and J.nvstoregiftSellYn='Y'"
		End If

		'스토어팜 품절&텐바이텐판매가능(판매중,한정>=10) 상품보기
		If FRectNvstoregiftNo10x10Yes <> "" Then
			addSql = addSql & " and (J.nvstoregiftSellYn= 'N' and i.sellyn='Y' and (i.limityn='N' or (i.limityn='Y' and i.limitno-i.limitsold>"&CMAXLIMITSELL&")))"
		End If

		'수정요망상품보기(최종업데이트일 기준)
		If FRectReqEdit <> "" Then
			addSql = addSql & " and J.nvstoregiftLastUpdate < i.lastupdate "
		End If
		...
		If (FRectOrdType = "LU") Then
		    ...
		    addSql = addSql & " and Left(i.lastupdate, 10) <> Left(J.nvstoregiftLastUpdate, 10) "
		End If
		...
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
		    sqlStr = sqlStr & " 	LEFT JOIN db_etcmall.[dbo].[tbl_nvstoregift_regItem] as J with (nolock) "
		Else
		    sqlStr = sqlStr & " 	JOIN db_etcmall.[dbo].[tbl_nvstoregift_regItem] as J with (nolock) "
	    END IF
		...
		sqlStr = sqlStr & " LEFT JOIN [db_temp].dbo.tbl_schedule_not_in_itemid as sc with (nolock) on sc.itemid = i.itemid and sc.mallgubun = 'nvstoregift' "
		sqlStr = sqlStr & " WHERE 1 = 1  "
		If (FRectIsReged <> "N" and FRectExtNotReg <> "Q")  Then		'// 미등록도 아니고 등록실패도 아니면 조건 없음
			If FRectIsReged = "Q" Then							'스케줄링에서만 사용
				sqlStr = sqlStr & " and J.nvstoregiftGoodNo is Not Null "
				If (FRectExcTrans <> "N") Then
		...
		sqlStr = sqlStr & "	, J.nvstoregiftRegdate, J.nvstoregiftLastUpdate, J.nvstoregiftGoodNo, J.nvstoregiftPrice, J.nvstoregiftSellYn, J.regUserid, IsNULL(J.nvstoregiftStatCd,-9) as nvstoregiftStatCd "
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new CnvstoregiftItem
					...
					FItemList(i).FNvstoregiftRegdate		= rsget("nvstoregiftRegdate")
					FItemList(i).FNvstoregiftLastUpdate		= rsget("nvstoregiftLastUpdate")
					FItemList(i).FNvstoregiftGoodNo			= rsget("nvstoregiftGoodNo")
					FItemList(i).FNvstoregiftPrice			= rsget("nvstoregiftPrice")
					FItemList(i).FNvstoregiftSellYn			= rsget("nvstoregiftSellYn")
					FItemList(i).FRegUserid					= rsget("regUserid")
					FItemList(i).FNvstoregiftStatCd			= rsget("nvstoregiftStatCd")
    ...
    ''' 등록되지 말아야 될 상품..
    Public Sub getNvstoregiftreqExpireItemList
		Dim sqlStr, addSql, i
		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(i.itemid) as cnt, CEILING(CAST(Count(i.itemid) AS FLOAT)/" & FPageSize & ") as totPg "
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_item as i "
		sqlStr = sqlStr & " JOIN db_etcmall.[dbo].[tbl_nvstoregift_regItem] as m on i.itemid=m.itemid and m.nvstoregiftGoodNo is Not Null and m.nvstoregiftSellYn = 'Y' "     ''' nvstoregift 판매중인거만.
		...
		sqlStr = sqlStr & "	, m.nvstoregiftRegdate, m.nvstoregiftLastUpdate, m.nvstoregiftGoodNo, m.nvstoregiftPrice, m.nvstoregiftSellYn, m.regUserid, m.nvstoregiftStatCd "
