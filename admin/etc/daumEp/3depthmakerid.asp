...
If mode = "I" Then
	...
	sqlStr = sqlStr & " LEFT JOIN db_outmall.dbo.tbl_EpShop_makerid_3depthName as m on m.makerid = c.userid AND m.mallgubun = 'daumEP' "
	sqlStr = sqlStr & " WHERE c.userid = '"&chgNmmakerid&"' "
	rsCTget.Open sqlStr,dbCTget,1
	If rsCTget("userCNT") = 0 Then
		response.write "<script language='javascript'>alert('텐바이텐에 등록된 브랜드가 아닙니다.');location.href='/admin/etc/daumEp/3depthmakerid.asp?menupos="&menupos&"';</script>"
	ElseIf rsCTget("notmakerCNT") > 0 Then
		response.write "<script language='javascript'>alert('이미 등록된 브랜드 입니다.');location.href='/admin/etc/daumEp/3depthmakerid.asp?menupos="&menupos&"';</script>"
	Else
		sqlStr = ""
		...
		sqlStr = sqlStr & " ('"&chgNmmakerid&"', 'daumep', '"&html2db(depthname)&"', 'Y' ,getdate(), '"&session("ssBctID")&"') "
		...
		response.write "<script language='javascript'>alert('저장하였습니다.');location.href='/admin/etc/daumEp/3depthmakerid.asp?menupos="&menupos&"';</script>"
	End If
	rsCTget.Close
ElseIf mode = "U" Then
	Dim cnt, tmpIsusing
	...
	For i = 0 to cnt
		...
		dbCTget.execute sqlStr
	Next
	response.write "<script language='javascript'>alert('수정하였습니다.');location.href='/admin/etc/daumEp/3depthmakerid.asp?menupos="&menupos&"';</script>"
End If
