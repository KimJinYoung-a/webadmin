...
iisql = iisql & " where mallid='skstoa' " & VbCRLF
...
If mode = "I" Then
	...
	iisql = ""
	iisql = iisql & " IF EXISTS (SELECT iniVal FROM db_etcmall.dbo.tbl_outmall_ini WHERE mallid='skstoa' and inikey='pass') "
	iisql = iisql & " 	BEGIN "
	iisql = iisql & "		UPDATE db_etcmall.dbo.tbl_outmall_ini "
	iisql = iisql & "		SET iniVal = '"& ipass &"' "
	iisql = iisql & "		WHERE mallid='skstoa'  "
	iisql = iisql & "		and inikey='pass' "
	iisql = iisql & " 	END "
	iisql = iisql & " ELSE "
	iisql = iisql & " 	BEGIN "
	iisql = iisql & "		INSERT INTO db_etcmall.dbo.tbl_outmall_ini (mallid, inikey, iniVal, lastupdate) VALUES "
	iisql = iisql & "		('skstoa', 'pass', '"& ipass &"', GETDATE()) "
	iisql = iisql & " 	END "
	dbget.execute iisql
...
function saveProcess(){
	if(confirm("SKSTOA 비밀번호와 같아야 합니다.\n동일하지 않을 시 모든 API에 오류가 발생합니다.\n\n저장하겠습니까?")){
