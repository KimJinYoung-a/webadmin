...
<!-- #include virtual="/lib/classes/3pl/csasCls.asp" -->
<%
				...
				<tr height="24">
				    ...
				    <td colspan="4" bgcolor="#FFFFFF">
	                  	...
	                  	<% if (ojumun.FOneItem.FAccountDiv="100") and (ojumun.FOneItem.FIpkumDiv >= 4) then %>
							<% if ojumun.FOneItem.Fpaygatetid<>"" then %>
								<% if (ojumun.FOneItem.Fpggubun = "KA") then %>
		                  		...
		                  		<% elseif (Left(ojumun.FOneItem.Fpaygatetid,9)="IniTechPG") or (Left(ojumun.FOneItem.Fpaygatetid,9)="INIMX_CAR") or (Left(ojumun.FOneItem.Fpaygatetid,9)="INIMX_ISP") or (Left(ojumun.FOneItem.Fpaygatetid,6)="Stdpay") then %>
		                  		    <!-- 이니시스 전표 -->
						...
						<% if (ojumun.FOneItem.FAccountDiv="20") and (ojumun.FOneItem.FIpkumDiv >= 4) then %>
								<% if (Left(ojumun.FOneItem.Fpaygatetid,9)="IniTechPG") or (ojumun.FOneItem.Fjumundiv="9") or (ojumun.FOneItem.Fpggubun = "NP") or (ojumun.FOneItem.Fpggubun = "PY") then %>
						...
						<% if (ojumun.FOneItem.FAccountDiv <> "7") and (ojumun.FOneItem.FAccountDiv <> "20") and (ojumun.FOneItem.FIpkumDiv >= 4) and (ojumun.FOneItem.FsumPaymentEtc > 0 or ((ojumun.FOneItem.Fpggubun="NP" or ojumun.FOneItem.Fpggubun="PY") and (ojumun.FOneItem.GetPaperType = "R" or ojumun.FOneItem.GetPaperType = "S"))) then %>
					        <% if ojumun.FOneItem.IsPaperRequestExist then %>
					        	<% if ojumun.FOneItem.IsPaperFinished then %>
					        		<% if ojumun.FOneItem.GetPaperType = "R" then %>
										<!-- INICIS현금영수증 : 발행완료 -->
		...
		<td align="left">
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
				...
				<tr height="24">
				    ...
				    <td colspan="4" bgcolor="#FFFFFF">
						<% if C_CriticInfoUserLV1 or C_CriticInfoUserLV2 then %>
				    	<textarea id="idReqZipAddr" class="textarea_ro" rows="1" cols="100" readonly><%= ojumun.FOneItem.FReqZipAddr %>&nbsp;<%= ojumun.FOneItem.FReqAddress %></textarea>
						<% end if %>
	                </td>
				</tr>
				<tr height="24">
				    ...
				    <td colspan="4" bgcolor="#FFFFFF">
						<% if C_CriticInfoUserLV1 or C_CriticInfoUserLV2 then %>
				        <textarea id="idComment" class="textarea_ro" rows="1" cols="100" readonly><%= ojumun.FOneItem.FComment %></textarea>
						<% end if %>
				    </td>
				</tr>
			</table>
	                     ...
	                     <table height="270" width="100%" border=0 cellspacing=0 cellpadding=0 class=a bgcolor="FFFFFF">
	                        <tr height="100%">
	                            <td colspan="13" style="vertical-align: text-top;">
	                    	        <iframe name="orderdetail" src="ordermaster_itemlist.asp?orderserial=<%= orderserial %>" border="0" frameborder="no" frameSpacing=0  width="100%" height="100%" leftmargin="0"></iframe>
	                            </td>
	                        <tr>
	                    </table>
				    </td>
