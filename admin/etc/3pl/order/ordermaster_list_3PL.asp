...
dim searchfield, orderserial, username, userhp, etcfield, etcstring
...
dim tplcompanyid, checkTplcompanyid
...
checkTplcompanyid = request("checkTplcompanyid")
...
if (Len(orderserial) <> 17) then
	'// 제휴몰 주문번호 -> 주문번호
	outmallorderserial = orderserial
	Call GetOrderserialWithOutmallOrderserial_3PL(outmallorderserial, orderserial)
	if (orderserial = "") then
...
if (checkTplcompanyid = "Y") then
	ojumun.FRectTplcompanyid = tplcompanyid
end if
...
''검색조건 없을때 최근 N건 검색
ojumun.QuickSearchOrderList_3PL
...
function GotoOrderDetail(orderserial) {
	...
	ifrm.location.href = "ordermaster_detail_3PL.asp?orderserial=" + orderserial;
}
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="F4F4F4">
	<tr height="50">
        <td>
            ...
            &nbsp;
			<input type="checkbox" name="checkTplcompanyid" value="Y" <% if checkTplcompanyid="Y" then response.write "checked" %> onClick="ChangeFormBgColor(frm)">
			고객사 :
			<% CALL drawPartner3plCompany("tplcompanyid", tplcompanyid,"") %>
        </td>
...
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	...
	<% end if %>
		...
		<td><%= GetUsernameWithAsterisk(Left(ojumun.FItemList(ix).FReqName, 4), useAsterisk) %></td>
