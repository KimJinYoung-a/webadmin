...
dim checkYYYYMMDD, checkJumunDiv, checkJumunSite, checkJumunItem, checkSongjangno
dim yyyy1,yyyy2,mm1,mm2,dd1,dd2
dim jumundiv, jumunsite, jumunitem
dim research
dim AlertMsg, v6MonthAgo
dim outmallorderserial
dim songjangno
dim useAsterisk : useAsterisk = True
...
if (Len(orderserial) >= 12) or (Len(orderserial) <= 10) then		'2017-03-08 김진영..Len(orderserial) < 10에서 Len(orderserial) <= 10으로 수정 | '2018-03-14 김진영..orderserial) >= 12로 수정
	'// 제휴몰 주문번호 -> 주문번호
...
set oCTPLPartner = New CTPLPartner
	oCTPLPartner.FCurrPage					= 1
	oCTPLPartner.FRectUseYN					= "Y"
	oCTPLPartner.FPageSize					= 100

oCTPLPartner.GetTPLPartnerList
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="F4F4F4">
	<tr height="50">
        <td>
    		...
    		<input type="text" class="text" name="orderserial" value="<%= orderserial %>" size="22" maxlength="32" onKeyPress="if (event.keyCode == 13) document.frm.submit();" onFocus="ChangeCheckbox('searchfield', 'orderserial'); FocusAndSelect(frm, frm.orderserial);">
    		...
    		특정사이트 : <% SelectBoxPartnerCompanyID "jumunsite", jumunsite, "Y" %>
			&nbsp;
...
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	...
	<% end if %>
		...
		<td>
			<a href="?searchfield=orderserial&orderserial=<%= ojumun.FItemList(ix).Forderserial %>"><%= ojumun.FItemList(ix).Forderserial %></a>
		</td>
		<td><font color="<%= ojumun.FItemList(ix).SiteNameColor %>"><%= ojumun.FItemList(ix).Fpartnercompanyname %></font></td>
		<td><acronym title="<%= ojumun.FItemList(ix).Frdsite %>"><%= Left(ojumun.FItemList(ix).Frdsite,8) %></acronym></td>
		<td align="left">
