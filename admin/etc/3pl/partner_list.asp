...
function jsPopModi(idx) {
	var popwin = window.open("pop_partner_modify.asp?idx=" + idx,"jsPopModi","width=400 height=170 scrollbars=auto resizable=yes");
	popwin.focus();
}
...
<% if (oCTPLPartner.FResultCount > 0) then %>
    ...
    <tr align="center" bgcolor="<%= CHKIIF(oCTPLPartner.FItemList(i).Fuseyn<>"Y", "#DDDDDD", "#FFFFFF")%>" height="25">
  		<td><a href="javascript:jsPopModi(<%= oCTPLPartner.FItemList(i).Fpartnercompanyid %>)"><%= oCTPLPartner.FItemList(i).Fpartnercompanyid %></a></td>
		<td><a href="javascript:jsPopModi(<%= oCTPLPartner.FItemList(i).Fpartnercompanyid %>)"><%= oCTPLPartner.FItemList(i).Fpartnercompanyname %></a></td>
