...
<%
Dim ogsshop, i, page, infodiv, CateName, searchName
...
Set ogsshop = new CGSShop
	...
	ogsshop.FInfodiv	= infodiv
	ogsshop.FCateName	= CateName
	ogsshop.FsearchName	= searchName
	ogsshop.FRectIsMapping	= isMapping
	ogsshop.getTenGsshopprdDivList
%>
...
<!--
	...
	// gsshop 상품분류 매칭 팝업
	function popGSprddivMap(mode,infodiv,cdl,cdm,cds,dno) {
		var pCM = window.open("popgsshopPrddivMap.asp?mode="+mode+"&infodiv="+infodiv+"&cdl="+cdl+"&cdm="+cdm+"&cds="+cds,"popprdDivMap","width=600,height=400,scrollbars=yes,resizable=yes");
		pCM.focus();
	}
...
<%
	Else
		For i = 0 to ogsshop.FresultCount - 1
			prdDivAllNm = ogsshop.FItemList(i).Fcdl_Name & ">" & ogsshop.FItemList(i).Fcdm_Name & ">" & ogsshop.FItemList(i).Fcds_Name & ">" & ogsshop.FItemList(i).Fcdd_Name
			
%>
<tr align="center" height="25" bgcolor="#FFFFFF">
	<td><%= ogsshop.FItemList(i).Finfodiv %></td>
	<td><%= ogsshop.FItemList(i).FtenCDLName %></td>
	<td><%= ogsshop.FItemList(i).FtenCDMName %></td>
	<td><%= ogsshop.FItemList(i).FtenCDSName %></td>
	<td onclick="javascript:pop_itemmodi('<%= ogsshop.FItemList(i).FtenCateLarge %>','<%= ogsshop.FItemList(i).FtenCateMid %>','<%= ogsshop.FItemList(i).FtenCateSmall %>','<%= ogsshop.FItemList(i).Finfodiv %>');" style="cursor:pointer;"><%= ogsshop.FItemList(i).Ficnt %></td>
	<% If ogsshop.FItemList(i).FDivcode="" OR isNull(ogsshop.FItemList(i).FDivcode) Then %>
	<td colspan="3"><input type="button" class="button" value="gsshop 상품분류 매칭" onClick="popGSprddivMap('I','<%= ogsshop.FItemList(i).Finfodiv %>','<%= ogsshop.FItemList(i).FtenCateLarge %>','<%= ogsshop.FItemList(i).FtenCateMid %>','<%= ogsshop.FItemList(i).FtenCateSmall %>')"></td>
	<% Else %>
	<td title="<%=prdDivAllNm%>" onClick="popGSprddivMap('U','<%= ogsshop.FItemList(i).Finfodiv %>','<%= ogsshop.FItemList(i).FtenCateLarge %>','<%= ogsshop.FItemList(i).FtenCateMid %>','<%= ogsshop.FItemList(i).FtenCateSmall %>')" style="cursor:pointer"><%= ogsshop.FItemList(i).FDivcode %></td>
	<td title="<%=prdDivAllNm%>" onClick="popGSprddivMap('U','<%= ogsshop.FItemList(i).Finfodiv %>','<%= ogsshop.FItemList(i).FtenCateLarge %>','<%= ogsshop.FItemList(i).FtenCateMid %>','<%= ogsshop.FItemList(i).FtenCateSmall %>')" style="cursor:pointer"><%= ogsshop.FItemList(i).Fcdd_Name %></td>
	<td><%=prdDivAllNm%></td>
	<% End If %>
</tr>
<%
