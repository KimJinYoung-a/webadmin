...
<!--
	...
	// GSShop MDID 매칭 팝업
	function popMdidMap(ckey, v) {
		var pCM = window.open("popgshopmdidMap.asp?catekey="+ckey+"&reged="+v,"popMdidMap","width=600,height=400,scrollbars=yes,resizable=yes");
		pCM.focus();
	}
//-->
</script>
...
<tr valign="top">
	...
	<font color="red"><strong>GSShop MDID 관리</strong></font></td>
...
<tr align="center" height="25" bgcolor="#E8E8FF">
	<td colspan="5">GSShop 전문 전시카테고리</td>
	<td rowspan="2">GSShop MDID</td>
</tr>
...
<tr align="center" height="25" bgcolor="<%= CHKIIF(IsNull(ogsshop.FItemList(i).FBrandcd),"#CCCCCC","#FFFFFF") %>">
	<td><%= ogsshop.FItemList(i).FL_NAME %></td>
	<td><%= ogsshop.FItemList(i).FM_NAME %></td>
	<td><%= ogsshop.FItemList(i).FS_NAME %></td>
	<td><%= ogsshop.FItemList(i).FD_NAME %></td>
	<td><%= ogsshop.FItemList(i).FCatekey %></td>
	<% If ogsshop.FItemList(i).FMdid = "" Then %>
	<td colspan="6"><input type="button" class="button" value="GSShop MDID 매칭" onClick="popMdidMap('<%= ogsshop.FItemList(i).FCatekey %>', '')"></td>
	<% Else %>
	<td style="cursor:pointer;" onclick="popMdidMap('<%= ogsshop.FItemList(i).FCatekey %>', 'Y')"><%= ogsshop.FItemList(i).FMdid %></td>
	<% End If %>
</tr>
<%
		Next
