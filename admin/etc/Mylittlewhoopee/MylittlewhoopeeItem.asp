...
Else
	oMylittlewhoopee.getMylittlewhoopeeRegedItemList		'그 외 리스트
End If
%>
...
<tr align="center" bgcolor="#FFFFFF" >
	...
	<td align="left">
		...
		&nbsp;
		스토어팜 상품코드 : <textarea rows="2" cols="20" name="MylittlewhoopeeGoodNo" id="itemid"><%= replace(replace(MylittlewhoopeeGoodNo,",",chr(10)), "'", "")%></textarea>
		<br>
...
<tr align="center" bgcolor="#FFFFFF" >
	<td align="left">
		...
		<% End If %>
		<br>
		<label><input onClick="checkisReged(this)" type="checkbox" name="MylittlewhoopeeYes10x10No" <%= ChkIIF(MylittlewhoopeeYes10x10No="on","checked","") %> ><font color=red>스토어팜판매중&텐바이텐품절</font>상품보기</label>
		&nbsp;
		<label><input onClick="checkisReged(this)" type="checkbox" name="MylittlewhoopeeNo10x10Yes" <%= ChkIIF(MylittlewhoopeeNo10x10Yes="on","checked","") %> ><font color=red>스토어팜품절&텐바이텐판매가능</font>(판매중,한정>5) 상품보기</label>
		<br>
...
<tr bgcolor="#FFFFFF">
    <td style="padding:5 0 5 0">
	    ...
	    <tr>
	    	<td valign="top">
				실제상품 등록 :
				<input class="button" type="button" id="btnRegImgSel" value="이미지" onClick="MylittlewhoopeeSelectImageRegProcess();" style=color:red;font-weight:bold>&nbsp;&nbsp;
			<% If (session("ssBctID")="kjy8517") OR (session("ssBctID")="icommang") OR (session("ssBctID")="yhj0613") OR (session("ssBctID")="hrkang97") Then %>
				...
				<input class="button" type="button" id="btnRegSel" value="상품" onClick="MylittlewhoopeeSelectRegItemProcess();">&nbsp;&nbsp;
				<input class="button" type="button" id="btnRegOptSel" value="옵션" onClick="MylittlewhoopeeSelectOPTRegProcess();">&nbsp;&nbsp;
				<input class="button" type="button" id="btnDelSel" value="삭제" onClick="MylittlewhoopeeSelectDelProcess();">&nbsp;&nbsp;
				<% Else %>
				<input class="button" type="button" id="btnDelSel" value="삭제" onClick="MylittlewhoopeeSelectDelProcess();">&nbsp;&nbsp;
				<% End If %>
			<% End If %>
				<input class="button" type="button" id="btnReg" value="상품+옵션" onClick="MylittlewhoopeeSelectRegProcess();" style=color:red;font-weight:bold>
				<br><br>
				실제상품 수정 :
				<input class="button" type="button" id="btnReg" value="수정" onClick="MylittlewhoopeeSelectEDITProcess();" style=color:blue;font-weight:bold>
				<br><br>
			<% If (session("ssBctID")="kjy8517") OR (session("ssBctID")="icommang") Then %>
				실제상품 조회 :
				<input class="button" type="button" id="btnSchitem" value="상품" onClick="MylittlewhoopeeSelectItemSearchProcess();">&nbsp;&nbsp;
				<input class="button" type="button" id="btnSchOpt" value="옵션" onClick="MylittlewhoopeeSelectOptionSearchProcess();">&nbsp;&nbsp;
				<br><br>
				공통코드 검색 :
				...
				<input class="button" type="button" id="btnCommcd" value="공통코드확인" onClick="MylittlewhoopeeCommCDSubmit();" >
			<% End If %>
			</td>
			<td align="right" valign="top">
				...
				<input class="button" type="button" id="btnSellYn" value="변경" onClick="MylittlewhoopeeSellYnProcess(frmReg.chgSellYn.value);">
			</td>
		</tr>
		</table>
    </td>
...
<tr height="25" bgcolor="FFFFFF">
	<td colspan="19">
		검색결과 : <b><%= FormatNumber(oMylittlewhoopee.FTotalCount,0) %></b>
		&nbsp;
		페이지 : <b> <%= FormatNumber(page,0) %> / <%= FormatNumber(oMylittlewhoopee.FTotalPage,0) %></b>
	</td>
</tr>
...
<tr align="center" <% If oMylittlewhoopee.FItemList(i).FNotSchIdx <> "" Then response.write "bgcolor= 'PINK'" Else response.write "bgcolor= '#FFFFFF'" End If %>>
	...
	<%
				...
				If Not IsNULL(oMylittlewhoopee.FItemList(i).FSpecialPrice) Then
					If (now() >= oMylittlewhoopee.FItemList(i).FStartDate) And (now() <= oMylittlewhoopee.FItemList(i).FEndDate) Then
						response.write "<font color='orange'><strong>(특)" & formatNumber(oMylittlewhoopee.FItemList(i).FSpecialPrice,0)&"</strong></font><br />"
					End If
				End If
	...
	<td align="center">
		<%= Chkiif(oMylittlewhoopee.FItemList(i).FAPIaddImg="Y","<font color='BLUE'>"&oMylittlewhoopee.FItemList(i).FAPIaddImg&"</font>", "<font color='RED'>미등록</font>") %>
	</td>
</tr>
<% Next %>
<tr height="20">
