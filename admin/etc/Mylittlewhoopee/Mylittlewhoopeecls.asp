...
Class CMylittlewhoopeeItem
	...
	Public FLimitNo
	Public FLimitSold
	Public FMylittlewhoopeeRegdate
	Public FMylittlewhoopeeLastUpdate
	...
	Public Function getMylittlewhoopeeStatName
	    If IsNULL(FMylittlewhoopeeStatCd) then FMylittlewhoopeeStatCd=-1
		Select Case FMylittlewhoopeeStatCd
			CASE -9 : getMylittlewhoopeeStatName = "미등록"
			CASE -1 : getMylittlewhoopeeStatName = "등록실패"
			CASE 0 : getMylittlewhoopeeStatName = "<font color=blue>등록예정</font>"
			CASE 1 : getMylittlewhoopeeStatName = "전송시도"
			CASE 7 : getMylittlewhoopeeStatName = ""
			CASE ELSE : getMylittlewhoopeeStatName = FMylittlewhoopeeStatCd
		End Select
	End Function
...
Class CMylittlewhoopee
	...
	Public FRectMakerid
	Public FRectMylittlewhoopeeGoodNo
	Public FRectMatchCate
	Public FRectoptExists
	Public FRectoptnotExists
	Public FRectMylittlewhoopeeNotReg
	Public FRectMinusMigin
	Public FRectExpensive10x10
	Public FRectdiffPrc
	Public FRectMylittlewhoopeeYes10x10No
	Public FRectMylittlewhoopeeNo10x10Yes
	...
	Public Sub getMylittlewhoopeeRegedItemList
		Dim i, sqlStr, addSql
		'브랜드검색
		...
		'스토어팜 상품번호 검색
        If (FRectMylittlewhoopeeGoodNo <> "") then
            If Right(Trim(FRectMylittlewhoopeeGoodNo) ,1) = "," Then
            	FRectMylittlewhoopeeGoodNo = Replace(FRectMylittlewhoopeeGoodNo,",,",",")
            	FRectMylittlewhoopeeGoodNo = Replace(FRectMylittlewhoopeeGoodNo,"''","'")
            	addSql = addSql & " and J.MylittlewhoopeeGoodNo in (" & Left(FRectMylittlewhoopeeGoodNo, Len(FRectMylittlewhoopeeGoodNo)-1) & ")"
            Else
				FRectMylittlewhoopeeGoodNo = Replace(FRectMylittlewhoopeeGoodNo,",,",",")
				FRectMylittlewhoopeeGoodNo = Replace(FRectMylittlewhoopeeGoodNo,"''","'")
            	addSql = addSql & " and J.MylittlewhoopeeGoodNo in (" & FRectMylittlewhoopeeGoodNo & ")"
            End If
        End If
		...
		Select Case FRectExtNotReg
			Case "Q"	''등록실패
				addSql = addSql & " and J.MylittlewhoopeeStatCd = -1"
			Case "J"	'등록예정이상
				addSql = addSql & " and J.MylittlewhoopeeStatCd >= 0"
		    Case "A"	'전송시도중오류
				addSql = addSql & " and J.MylittlewhoopeeStatCd = 1"
		    Case "I"	'이미지만 완료
				addSql = addSql & " and isnull(J.MylittlewhoopeeGoodNo, '') = '' "
				addSql = addSql & " and J.APIaddImg = 'Y'"
			Case "D"	'등록완료(전시)
			    addSql = addSql & " and J.MylittlewhoopeeStatCd = 7"
				addSql = addSql & " and J.MylittlewhoopeeGoodNo is Not Null"
		End Select
		...
		If (FRectNotinmakerid <> "") then
			If (FRectNotinmakerid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'Mylittlewhoopee') "
			ElseIf (FRectNotinmakerid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'Mylittlewhoopee') "
			End If
		End If
		...
		If (FRectNotinitemid <> "") then
			If (FRectNotinitemid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'Mylittlewhoopee') "
			ElseIf (FRectNotinitemid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'Mylittlewhoopee') "
			End If
		End If
		...
		If (FRectExcTrans <> "") then
			If (FRectExcTrans = "Y") Then
				...
				addSql = addSql & " or exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'Mylittlewhoopee') "
				addSql = addSql & " or exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'Mylittlewhoopee') "
...
'				addSql = addSql & " and uc.isExtUsing='Y' "											'// 2018-12-03 김진영 수정 // 제휴판매안함이라도 스토어팜 판매 가능이면 판매
		...
		If (FRectExtSellYn<>"") then
			If (FRectExtSellYn = "YN") Then
				addSql = addSql & " and J.MylittlewhoopeeSellYn <> 'X'"
			Else
				addSql = addSql & " and J.MylittlewhoopeeSellYn='" & FRectExtSellYn & "'"
			End if
		End If
		...
		If (FRectexpensive10x10 <> "") Then
			addSql = addSql & " and J.MylittlewhoopeePrice is Not Null and J.MylittlewhoopeePrice < i.sellcash"
		End If

		'가격상이전체보기
		If FRectdiffPrc <> "" Then
			addSql = addSql & " and J.MylittlewhoopeePrice is Not Null and i.sellcash <> J.MylittlewhoopeePrice "
		End If

		'스토어팜 판매 10x10 품절
		If (FRectMylittlewhoopeeYes10x10No <> "") Then
			addSql = addSql & " and i.sellyn<>'Y'"
			addSql = addSql & " and J.MylittlewhoopeeSellYn='Y'"
		End If

		'스토어팜 품절&텐바이텐판매가능(판매중,한정>=10) 상품보기
		If FRectMylittlewhoopeeNo10x10Yes <> "" Then
			addSql = addSql & " and (J.MylittlewhoopeeSellYn= 'N' and i.sellyn='Y' and (i.limityn='N' or (i.limityn='Y' and i.limitno-i.limitsold>"&CMAXLIMITSELL&")))"
		End If

		'수정요망상품보기(최종업데이트일 기준)
		If FRectReqEdit <> "" Then
			addSql = addSql & " and J.MylittlewhoopeeLastUpdate < i.lastupdate "
		End If
		...
		If (FRectOrdType = "LU") Then
		    ...
		    addSql = addSql & " and Left(i.lastupdate, 10) <> Left(J.MylittlewhoopeeLastUpdate, 10) "
		End If
		...
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
		    sqlStr = sqlStr & " 	LEFT JOIN db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem] as J with (nolock) "
		Else
		    sqlStr = sqlStr & " 	JOIN db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem] as J with (nolock) "
	    END IF
		...
		sqlStr = sqlStr & " LEFT JOIN [db_temp].dbo.tbl_schedule_not_in_itemid as sc with (nolock) on sc.itemid = i.itemid and sc.mallgubun = 'Mylittlewhoopee' "
		sqlStr = sqlStr & " WHERE 1 = 1  "
		If (FRectIsReged <> "N" and FRectExtNotReg <> "Q")  Then		'// 미등록도 아니고 등록실패도 아니면 조건 없음
			If FRectIsReged = "Q" Then							'스케줄링에서만 사용
				sqlStr = sqlStr & " and J.MylittlewhoopeeGoodNo is Not Null "
				If (FRectExcTrans <> "N") Then
		...
		sqlStr = sqlStr & "	, J.MylittlewhoopeeRegdate, J.MylittlewhoopeeLastUpdate, J.MylittlewhoopeeGoodNo, J.MylittlewhoopeePrice, J.MylittlewhoopeeSellYn, J.regUserid, IsNULL(J.MylittlewhoopeeStatCd,-9) as MylittlewhoopeeStatCd "
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new CMylittlewhoopeeItem
					...
					FItemList(i).FMylittlewhoopeeRegdate		= rsget("MylittlewhoopeeRegdate")
					FItemList(i).FMylittlewhoopeeLastUpdate		= rsget("MylittlewhoopeeLastUpdate")
					FItemList(i).FMylittlewhoopeeGoodNo			= rsget("MylittlewhoopeeGoodNo")
					FItemList(i).FMylittlewhoopeePrice			= rsget("MylittlewhoopeePrice")
					FItemList(i).FMylittlewhoopeeSellYn			= rsget("MylittlewhoopeeSellYn")
					FItemList(i).FRegUserid						= rsget("regUserid")
					FItemList(i).FMylittlewhoopeeStatCd			= rsget("MylittlewhoopeeStatCd")
    ...
    ''' 등록되지 말아야 될 상품..
    Public Sub getMylittlewhoopeereqExpireItemList
		Dim sqlStr, addSql, i
		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(i.itemid) as cnt, CEILING(CAST(Count(i.itemid) AS FLOAT)/" & FPageSize & ") as totPg "
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_item as i "
		sqlStr = sqlStr & " JOIN db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem] as m on i.itemid=m.itemid and m.MylittlewhoopeeGoodNo is Not Null and m.MylittlewhoopeeSellYn = 'Y' "     ''' Mylittlewhoopee 판매중인거만.
		...
		sqlStr = sqlStr & "	, m.MylittlewhoopeeRegdate, m.MylittlewhoopeeLastUpdate, m.MylittlewhoopeeGoodNo, m.MylittlewhoopeePrice, m.MylittlewhoopeeSellYn, m.regUserid, m.MylittlewhoopeeStatCd "
