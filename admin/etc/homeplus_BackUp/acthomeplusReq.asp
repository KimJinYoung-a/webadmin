...
'					response.write BinaryToText(objXML.ResponseBody, "euc-kr")
		...
		If retCode = "E0000" Then	'성공(E0000)
			...
			Set optlist = xmlDOM.SelectNodes("soapenv:Envelope/soapenv:Body/createNewProductResponse/ns1:createNewProductReturn/ns1:ITEMRESULT/ns1:ITEMRESULT")
...
Function HomeplusOneItemEdit(iitemid, iHomeplusGoodNo, byRef iErrStr, strParam, mode)
	...
	Dim strRst, strSql, pregitemname
...
'			response.end
		...
		If retCode = "E0000" Then	'성공(E0000)이면 실패횟수 초기화
			...
			strSql = strSql & " FROM db_outmall.dbo.tbl_homeplus_regItem A "& VbCRLF
			strSql = strSql & " JOIN db_AppWish.dbo.tbl_item B on A.itemid = B.itemid "& VbCRLF
...
Function HomeplusOneItemOPTEdit(iitemid, iHomeplusGoodNo, byRef iErrStr, strParam, imustprice, mode)
	Dim xmlStr : xmlStr = strParam
	Dim objXML, xmlDOM, retCode, iMessage
	Dim strRst, strSql
...
If (cmdparam = "RegSelect") Then				'선택상품 실제 등록
	...
	Set oHomeplus = new CHomeplus
		...
		For i = 0 to (oHomeplus.FResultCount - 1)
			...
			Else
				CALL Fn_AcctFailTouch("homeplus", oHomeplus.FItemList(i).Fitemid, iErrStr)
				iErrStr = "["&oHomeplus.FItemList(i).Fitemid&"] : 옵션검사 실패 | 한정옵션의 개수가 5개 이하일 수 있음"
				retErrStr = retErrStr & iErrStr
			End If
		Next
			...
			On Error Resume Next
 			If HomeplusLoginAPI2() Then
				...
				Else
					...
					getMustprice = ""
					getMustprice = oHomeplus.FItemList(i).fngetMustPrice()

					Call (HomeplusOneItemOPTEdit(oHomeplus.FItemList(i).Fitemid, oHomeplus.FItemList(i).FHomeplusGoodNo, iErrStr, strParam, getMustprice, "updateProduct"))
					If iErrStr <> "" Then
						rw iErrStr
					End If
