...
'				strRst = strRst &"	<d_RELEASE_DATE></d_RELEASE_DATE>"									'출시일자 / 출시일자 (YYYYMMDD)
	...
	Public Function getHomeplusOptionParamToEDT
		...
		strSql = "exec db_outmall.dbo.sp_Ten_OutMall_optEditParamList_homeplus 'homeplus'," & Fitemid
		...
		strSql = "SELECT COUNT(*) as cnt FROM db_outmall.dbo.tbl_jaehyumall_not_in_itemid where mallgubun = 'homeplus' and itemid =" & Fitemid
		rsCTget.Open strSql, dbCTget
		If Not(rsCTget.EOF or rsCTget.BOF) Then
		...
		strSql = "SELECT COUNT(*) as cnt FROM db_Appwish.dbo.tbl_item as i join [db_outmall].dbo.tbl_jaehyumall_not_in_makerid as m on i.makerid = m.makerid where i.itemid = "& Fitemid&" and m.mallgubun = 'homeplus'"
		rsCTget.Open strSql, dbCTget
		If Not(rsCTget.EOF or rsCTget.BOF) Then
...
Class CHomeplus
	...
	Public Sub getHomeplusRegedItemList
		...
		Select Case FRectHomeplusNotReg
			Case "M"	'미등록
			    addSql = addSql & " and G.itemid is NULL and (i.limityn='N' or (i.limityn='Y' and i.limitno-i.limitsold>5)) "
			Case "Q"	''등록실패
		...
		If (FRectHomeplusNotReg <> "M") and (FRectHomeplusNotReg <> "") Then
		    sqlStr = sqlStr & " 	JOIN db_outmall.dbo.tbl_Homeplus_regitem as G "
		Else
		    sqlStr = sqlStr & " 	LEFT JOIN db_outmall.dbo.tbl_Homeplus_regitem as G "
	    END IF
		sqlStr = sqlStr & " 		on i.itemid=G.itemid "
		sqlStr = sqlStr & "	LEFT Join db_outmall.dbo.tbl_homeplus_brandCategory_mapping as c on c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small "
		sqlStr = sqlStr & " LEFT JOIN db_outmall.dbo.tbl_homeplus_prdDiv_mapping as pm on pm.tenCateLarge = i.cate_large and pm.tenCateMid = i.cate_mid and pm.tenCateSmall = i.cate_small and ct.infodiv = pm.infodiv "
		...
		Else
    		...
    		sqlStr = sqlStr & "	and i.makerid not in (Select makerid From [db_outmall].dbo.tbl_jaehyumall_not_in_makerid Where mallgubun='"&CMALLNAME&"') "	'등록제외 브랜드
    		sqlStr = sqlStr & "	and i.itemid not in (Select itemid From [db_outmall].dbo.tbl_jaehyumall_not_in_itemid Where mallgubun='"&CMALLNAME&"') "		'등록제외 상품
		...
		sqlStr = sqlStr & " , G.regedOptCnt, G.rctSellCNT, G.accFailCNT, G.lastErrStr "
		sqlStr = sqlStr & " ,uc.defaultdeliverytype, uc.defaultfreeBeasongLimit, isnull(pm.hDIVISION, '') as hDIVISION "
		sqlStr = sqlStr & "	, Ct.infoDiv, G.optAddPrcCnt, G.optAddPrcRegType, i.itemdiv "
		sqlStr = sqlStr & " FROM db_AppWish.dbo.tbl_item as i "
		sqlStr = sqlStr & " JOIN db_AppWish.dbo.tbl_item_contents as ct on i.itemid = ct.itemid"
		If (FRectHomeplusNotReg <> "M") and (FRectHomeplusNotReg <> "") Then
		    sqlStr = sqlStr & " 	JOIN db_outmall.dbo.tbl_Homeplus_regitem as G "
		...
		If not rsCTget.EOF Then
			...
			Do until rsCTget.EOF
				Set FItemList(i) = new CHomeplusItem
					...
					FItemList(i).FHomeplusRegdate			= rsCTget("HomeplusRegdate")
					FItemList(i).FHomeplusLastUpdate		= rsCTget("HomeplusLastUpdate")
					FItemList(i).FHomeplusGoodNo			= rsCTget("HomeplusGoodNo")
					FItemList(i).FHomeplusPrice				= rsCTget("HomeplusPrice")
					FItemList(i).FHomeplusSellYn			= rsCTget("HomeplusSellYn")
					FItemList(i).FregUserid					= rsCTget("regUserid")
					FItemList(i).FHomeplusStatCd			= rsCTget("HomeplusStatCd")
	                ...
	                FItemList(i).FhDIVISION					= rsCTget("hDIVISION")
    ...
    Public Sub getHomeplusreqExpireItemList
		...
		sqlStr = sqlStr & " SELECT count(i.itemid) as cnt, CEILING(CAST(Count(i.itemid) AS FLOAT)/" & FPageSize & ") as totPg "
		sqlStr = sqlStr & " FROM db_AppWish.dbo.tbl_item as i "
		sqlStr = sqlStr & " JOIN db_outmall.dbo.tbl_homeplus_regitem as m on i.itemid=m.itemid and m.HomeplusGoodNo is Not Null and m.HomeplusSellYn = 'Y' "     ''' Homeplus 판매중인거만.
		...
		sqlStr = sqlStr & " LEFT JOIN (Select tenCateLarge, tenCateMid, tenCateSmall, count(*) as mapCnt From db_outmall.dbo.tbl_homeplus_brandCategory_mapping Group by tenCateLarge, tenCateMid, tenCateSmall ) as cm on cm.tenCateLarge=i.cate_large and cm.tenCateMid=i.cate_mid and cm.tenCateSmall=i.cate_small "
	...
	Public Sub getHomeplusEditedItemList
		...
		strSql = strSql & " JOIN db_outmall.dbo.tbl_Homeplus_regitem as m on i.itemid = m.itemid "
		strSql = strSql & " LEFT JOIN db_outmall.dbo.tbl_homeplus_prdDiv_mapping as pm on pm.tenCateLarge=i.cate_large and pm.tenCateMid=i.cate_mid and pm.tenCateSmall=i.cate_small and c.infodiv = pm.infodiv "
		strSql = strSql & " LEFT JOIN db_outmall.dbo.tbl_homeplus_cate_mapping as hm on hm.tenCateLarge=i.cate_large and hm.tenCateMid=i.cate_mid and hm.tenCateSmall=i.cate_small and c.infodiv = hm.infodiv "
		strSql = strSql & " LEFT JOIN db_outmall.dbo.tbl_homeplus_brandCategory_mapping as bm on bm.tenCateLarge=i.cate_large and bm.tenCateMid=i.cate_mid and bm.tenCateSmall=i.cate_small "
		strSql = strSql & " LEFT JOIN (Select tenCateLarge, tenCateMid, tenCateSmall, count(*) as mapCnt From db_outmall.dbo.tbl_Homeplus_cate_mapping Group by tenCateLarge, tenCateMid, tenCateSmall ) as cm on cm.tenCateLarge=i.cate_large and cm.tenCateMid=i.cate_mid and cm.tenCateSmall=i.cate_small "
		...
		If not rsCTget.EOF Then
			Do until rsCTget.EOF
				Set FItemList(i) = new CHomeplusItem
	                ...
	                FItemList(i).FhDIVISION			= rsCTget("hDIVISION")
	                FItemList(i).FhGROUP			= rsCTget("hGROUP")
	                FItemList(i).FhDEPT				= rsCTget("hDEPT")
	                FItemList(i).FhCLASS			= rsCTget("hCLASS")
	                FItemList(i).FhSUBCLASS			= rsCTget("hSUBCLASS")
	                FItemList(i).FDeliveryType		= rsCTget("deliveryType")
	                FItemList(i).FdepthCode			= rsCTget("depthCode")
	                FItemList(i).FbrandDepthCode	= rsCTget("brandDepthCode")
	...
	Public Sub getHomeplusNotRegItemList
		...
		strSql = strSql & " FROM db_AppWish.dbo.tbl_item as i "
		strSql = strSql & " JOIN db_AppWish.dbo.tbl_item_contents as c on i.itemid=c.itemid "
		strSql = strSql & " JOIN (Select tenCateLarge, tenCateMid, tenCateSmall, count(*) as mapCnt From db_outmall.dbo.tbl_homeplus_brandCategory_mapping Group by tenCateLarge, tenCateMid, tenCateSmall ) as cm on cm.tenCateLarge=i.cate_large and cm.tenCateMid=i.cate_mid and cm.tenCateSmall=i.cate_small "
		strSql = strSql & " LEFT JOIN db_outmall.dbo.tbl_homeplus_prdDiv_mapping as pm on pm.tenCateLarge=i.cate_large and pm.tenCateMid=i.cate_mid and pm.tenCateSmall=i.cate_small and c.infodiv = pm.infodiv "
		strSql = strSql & " LEFT JOIN db_outmall.dbo.tbl_homeplus_cate_mapping as hm on hm.tenCateLarge=i.cate_large and hm.tenCateMid=i.cate_mid and hm.tenCateSmall=i.cate_small and c.infodiv = hm.infodiv "
		strSql = strSql & " LEFT JOIN db_outmall.dbo.tbl_homeplus_brandCategory_mapping as bm on bm.tenCateLarge=i.cate_large and bm.tenCateMid=i.cate_mid and bm.tenCateSmall=i.cate_small "
		strSql = strSql & " LEFT JOIN db_outmall.dbo.tbl_homeplus_regItem R on i.itemid=R.itemid"
		strSql = strSql & " LEFT JOIN db_AppWish.dbo.tbl_user_c uc on i.makerid = uc.userid"
		...
		strSql = strSql & "	and i.itemid not in (Select itemid From db_outmall.dbo.tbl_homeplus_regItem where homeplusStatCD>3) "
