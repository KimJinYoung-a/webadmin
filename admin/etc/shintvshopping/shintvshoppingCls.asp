...
Class CShintvshoppingItem
	...
	Public FLimitNo
	Public FLimitSold
	Public FShintvshoppingRegdate
	Public FShintvshoppingLastUpdate
	...
	Public FIdx
	Public FMidx
	Public FShipCostCode

	Public Function getShintvshoppingStatName
	    If IsNULL(FShintvshoppingStatcd) then FShintvshoppingStatcd=-1
		Select Case FShintvshoppingStatcd
			CASE -9 : getShintvshoppingStatName = "미등록"
			CASE -1 : getShintvshoppingStatName = "등록실패"
			CASE 0 : getShintvshoppingStatName = "<font color=blue>등록예정</font>"
			CASE 3 : getShintvshoppingStatName = "승인예정"
			CASE 2 : getShintvshoppingStatName = "반려"
			CASE 4 : getShintvshoppingStatName = "취소"
			CASE 1 : getShintvshoppingStatName = "전송시도"
			CASE 7 : getShintvshoppingStatName = ""
			CASE ELSE : getShintvshoppingStatName = FShintvshoppingStatcd
		End Select
	End Function
...
Class CShintvshopping
	...
	Public FRectMakerid
	Public FRectShintvshoppingGoodNo
	Public FRectShintvshoppingGoodNo2
	Public FRectMatchCate
	Public FRectoptExists
	Public FRectoptnotExists
	Public FRectShintvshoppingNotReg
	Public FRectMinusMigin
	Public FRectExpensive10x10
	Public FRectdiffPrc
	Public FRectShintvshoppingYes10x10No
	Public FRectShintvshoppingNo10x10Yes
	...
	Public FRectShintvshoppingDelOptErr
	...
	Public FRectShipCostCode

	'// Shintvshopping 상품 목록 // 수정시 조건이 달라야 함..
	Public Sub getShintvshoppingRegedItemList
		Dim i, sqlStr, addSql
		'브랜드검색
		...
		'Shintvshopping 상품번호 검색
        If (FRectShintvshoppingGoodNo <> "") then
            If Right(Trim(FRectShintvshoppingGoodNo) ,1) = "," Then
            	FRectShintvshoppingGoodNo = Replace(FRectShintvshoppingGoodNo,",,",",")
            	FRectShintvshoppingGoodNo = Replace(FRectShintvshoppingGoodNo,"''","'")
            	addSql = addSql & " and J.ShintvshoppingGoodNo in (" & Left(FRectShintvshoppingGoodNo & "", Len(FRectShintvshoppingGoodNo)-1) & ")"
            Else
				FRectShintvshoppingGoodNo = Replace(FRectShintvshoppingGoodNo,",,",",")
				FRectShintvshoppingGoodNo = Replace(FRectShintvshoppingGoodNo,"''","'")
            	addSql = addSql & " and J.ShintvshoppingGoodNo in (" & FRectShintvshoppingGoodNo & ")"
            End If
        End If
		...
		Select Case FRectExtNotReg
		    Case "A"	'전송시도중오류
				addSql = addSql & " and J.ShintvshoppingStatCd = 1"
			Case "W"	'등록예정이상
				addSql = addSql & " and J.ShintvshoppingStatCd >= 0"
			Case "C"	'반려
				addSql = addSql & " and J.ShintvshoppingStatCd = 2"
			Case "E"	'취소
				addSql = addSql & " and J.ShintvshoppingStatCd = 4"
			Case "F"	'등록후 승인대기(임시)
				addSql = addSql & " and J.ShintvshoppingStatCd = 3"
				addSql = addSql & " and J.ShintvshoppingGoodNo is Not Null"
			Case "D"	'등록완료(전시)
			    addSql = addSql & " and J.ShintvshoppingStatCd = 7"
				addSql = addSql & " and J.ShintvshoppingGoodNo is Not Null"
		End Select
		...
		If (FRectExtSellYn<>"") then
			If (FRectExtSellYn = "YN") Then
				addSql = addSql & " and J.ShintvshoppingSellYn <> 'X'"
			Else
				addSql = addSql & " and J.ShintvshoppingSellYn='" & FRectExtSellYn & "'"
			End if
		End If
		...
		'Shintvshopping 카테고리 매칭 여부
		Select Case FRectMatchCate
			Case "Y"	'매칭완료
				addSql = addSql & " and isnull(c.tgroup, '') <> ''"
			Case "N"	'미매칭
				addSql = addSql & " and isnull(c.tgroup, '') = ''"
		End Select

        'Shintvshopping가격 < 10x10 가격
		If (FRectexpensive10x10 <> "") Then
			addSql = addSql & " and J.ShintvshoppingPrice is Not Null and J.ShintvshoppingPrice < i.sellcash"
		End If

		'가격상이전체보기
		If FRectdiffPrc <> "" Then
			addSql = addSql & " and J.ShintvshoppingPrice is Not Null and i.sellcash <> J.ShintvshoppingPrice "
		End If

		'Shintvshopping판매 10x10 품절
		If (FRectShintvshoppingYes10x10No <> "") Then
			addSql = addSql & " and i.sellyn<>'Y'"
			addSql = addSql & " and J.ShintvshoppingSellYn='Y'"
		End If

		'Shintvshopping품절&텐바이텐판매가능(판매중,한정>=10) 상품보기
		If FRectShintvshoppingNo10x10Yes <> "" Then
			addSql = addSql & " and (J.ShintvshoppingSellYn= 'N' and i.sellyn='Y' and (i.limityn='N' or (i.limityn='Y' and i.limitno-i.limitsold>"&CMAXLIMITSELL&")))"
		End If

		'수정요망상품보기(최종업데이트일 기준)
		If FRectReqEdit <> "" Then
			addSql = addSql & " and J.ShintvshoppingLastUpdate < i.lastupdate "
		End If
		...
		If (FRectOrdType = "LU") Then
		    ...
		    addSql = addSql & " and Left(i.lastupdate, 10) <> Left(J.ShintvshoppingLastUpdate, 10) "
		End If
		...
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
		    sqlStr = sqlStr & " 	LEFT JOIN db_etcmall.dbo.tbl_shintvshopping_regitem as J with (nolock) "
		Else
		    sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_shintvshopping_regitem as J with (nolock) "
	    END IF
		sqlStr = sqlStr & " 		on i.itemid=J.itemid "
		sqlStr = sqlStr & "	LEFT JOIN db_etcmall.[dbo].[tbl_shintvshopping_cate_mapping] as c with (nolock) on c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small "
		...
		sqlStr = sqlStr & "	, J.ShintvshoppingRegdate, J.ShintvshoppingLastUpdate, isnull(J.ShintvshoppingGoodNo, '') as ShintvshoppingGoodNo, J.ShintvshoppingPrice, J.ShintvshoppingSellYn, J.regUserid, IsNULL(J.ShintvshoppingStatCd,-9) as ShintvshoppingStatCd "
		sqlStr = sqlStr & "	, Case When isnull(c.tgroup, '') = '' Then 0 Else 1 End as mapcnt "
		sqlStr = sqlStr & " , J.regedOptCnt, J.rctSellCNT, J.accFailCNT, J.lastErrStr, J.reglevel "
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new CShintvshoppingItem
					...
					FItemList(i).FShintvshoppingRegdate				= rsget("ShintvshoppingRegdate")
					FItemList(i).FShintvshoppingLastUpdate			= rsget("ShintvshoppingLastUpdate")
					FItemList(i).FShintvshoppingGoodNo				= rsget("ShintvshoppingGoodNo")
					FItemList(i).FShintvshoppingPrice				= rsget("ShintvshoppingPrice")
					FItemList(i).FShintvshoppingSellYn				= rsget("ShintvshoppingSellYn")
					FItemList(i).FRegUserid					= rsget("regUserid")
					FItemList(i).FShintvshoppingStatcd				= rsget("ShintvshoppingStatCd")
    ...
    Public Sub getHmallreqExpireItemList
		...
		sqlStr = sqlStr & " JOIN db_etcmall.dbo.tbl_shintvshopping_regitem as J on J.itemid = i.itemid and J.hmallGoodno is not null "
...
'			exit sub
	...
	'// 텐바이텐-Shintvshopping 카테고리 리스트
	Public Sub getTenShintvshoppingCateList
...
'					addSql = addSql & " and T.Catekey='" & FRectKeyword & "'"
		...
		sqlStr = sqlStr & " 	SELECT cm.lgroup, cm.mgroup, cm.sgroup, cm.dgroup, cm.tgroup, cc.lgroupName, cc.mgroupName, cc.sgroupName, cc.dgroupName, cc.tgroupName, cm.tenCateLarge,cm.tenCateMid, cm.tenCateSmall "
		sqlStr = sqlStr & " 	FROM db_etcmall.dbo.tbl_shintvshopping_cate_mapping as cm "
		sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_shintvshopping_category as cc on cm.lgroup = cc.lgroup and cm.mgroup = cc.mgroup and cm.sgroup = cc.sgroup and cm.dgroup = cc.dgroup and cm.tgroup = cc.tgroup "
		...
		sqlStr = sqlStr & " ,T.lgroup, T.mgroup, T.sgroup, T.dgroup, T.tgroup, T.lgroupName, T.mgroupName, T.sgroupName, T.dgroupName, T.tgroupName, W.itemcnt "  & VBCRLF
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_cate_small as s " & VBCRLF
		sqlStr = sqlStr & " LEFT JOIN (  "  & VBCRLF
		sqlStr = sqlStr & " 	SELECT cm.lgroup, cm.mgroup, cm.sgroup, cm.dgroup, cm.tgroup, cc.lgroupName, cc.mgroupName, cc.sgroupName, cc.dgroupName, cc.tgroupName, cm.tenCateLarge,cm.tenCateMid, cm.tenCateSmall "
		sqlStr = sqlStr & " 	FROM db_etcmall.dbo.tbl_shintvshopping_cate_mapping as cm "
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new CShintvshoppingItem
					...
					FItemList(i).FTgroup			= rsget("tgroup")
					...
					FItemList(i).FTgroupName		= rsget("tgroupName")
					FItemList(i).FLastDepthNm		= rsget("tgroupName")
	...
	Public Sub getShintvshoppingCateList
		Dim sqlStr, addSql, i
		If FsearchName <> "" Then
			addSql = addSql & " and ( "
			...
			addSql = addSql & " 	or tgroupName like '%" & FsearchName & "%' "
			addSql = addSql & " )"
		End If

		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(*) as cnt, CEILING(CAST(Count(*) AS FLOAT)/" & FPageSize & ") as totPg " & VBCRLF
		sqlStr = sqlStr & " FROM db_etcmall.[dbo].[tbl_shintvshopping_category] "
		sqlStr = sqlStr & " WHERE 1=1 "
		sqlStr = sqlStr & addSql
		rsget.CursorLocation = adUseClient
		rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
		...
		sqlStr = ""
		sqlStr = sqlStr & " SELECT DISTINCT TOP " & CStr(FPageSize*FCurrPage) & " lgroup, mgroup, sgroup, dgroup, tgroup, lgroupName, mgroupName, sgroupName, dgroupName, tgroupName "
		sqlStr = sqlStr & " FROM db_etcmall.[dbo].[tbl_shintvshopping_category] "
		sqlStr = sqlStr & " WHERE 1=1 "
		sqlStr = sqlStr & addSql
		sqlStr = sqlStr & " ORDER BY lgroup, mgroup, sgroup, dgroup, tgroup, lgroupName, mgroupName, sgroupName, dgroupName, tgroupName "
		rsget.pagesize = FPageSize
		rsget.CursorLocation = adUseClient
		rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
		FResultCount = rsget.RecordCount-(FPageSize*(FCurrPage-1))
	...
	Public Sub getShintvshoppingBeasongCodeItemList
		Dim sqlStr, addSql, i
		...
		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(*) as cnt, CEILING(CAST(Count(*) AS FLOAT)/" & FPageSize & ") as totPg  " & VBCRLF
		sqlStr = sqlStr & " FROM db_etcmall.[dbo].[tbl_shintvshopping_beasongCodeItem_master] "
		...
		sqlStr = sqlStr & " idx, startDate, endDate, shipCostCode, isusing, regdate "
		sqlStr = sqlStr & " FROM db_etcmall.[dbo].[tbl_shintvshopping_beasongCodeItem_master] "
		sqlStr = sqlStr & " WHERE 1 = 1 " & addSql
		...
		If not rsget.EOF Then
			...
			Do until rsget.eof
				Set FItemList(i) = new CShintvshoppingItem
					...
					FItemList(i).FStartDate		= rsget("startDate")
					FItemList(i).FEndDate		= rsget("endDate")
					FItemList(i).FShipCostCode	= rsget("shipCostCode")
	...
	Public Sub getShintvshoppingshipCostCodeItemOneItem
	    Dim i, sqlStr, addSql
		sqlStr = ""
		sqlStr = sqlStr & " SELECT TOP 1 idx, startDate, endDate, shipCostCode, isusing "
		sqlStr = sqlStr & " FROM db_etcmall.[dbo].[tbl_shintvshopping_beasongCodeItem_master] "
		...
		If not rsget.EOF Then
			...
			FOneItem.FshipCostCode	= rsget("shipCostCode")
	...
	Public Sub getssgMarginItemDetailList
		...
		If FRectShipCostCode <> "" Then
			addSql = addSql & " and r.setMargin = '"& FRectShipCostCode &"' "
		End If
		...
		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(*) as cnt, CEILING(CAST(Count(*) AS FLOAT)/" & FPageSize & ") as totPg  " & VBCRLF
		sqlStr = sqlStr & " FROM db_etcmall.[dbo].[tbl_shintvshopping_beasongCodeItem_master] as m "
		sqlStr = sqlStr & " JOIN db_etcmall.[dbo].[tbl_shintvshopping_beasongCodeItem_detail] as d on m.idx = d.midx "
		sqlStr = sqlStr & " WHERE 1 = 1 " & addSql
		rsget.CursorLocation = adUseClient
        rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
			FTotalCount = rsget("cnt")
		...
		sqlStr = ""
		sqlStr = sqlStr & " SELECT TOP " & CStr(FPageSize*FCurrPage)
		sqlStr = sqlStr & " d.idx, d.midx, d.itemid, isNull(m.shipCostCode, 0) as shipCostCode "
		sqlStr = sqlStr & " FROM db_etcmall.[dbo].[tbl_shintvshopping_beasongCodeItem_master] as m "
		sqlStr = sqlStr & " JOIN db_etcmall.[dbo].[tbl_shintvshopping_beasongCodeItem_detail] as d on m.idx = d.midx "
		sqlStr = sqlStr & " WHERE 1 = 1 " & addSql
