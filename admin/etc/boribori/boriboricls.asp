...
Class CBoriboriItem
	...
	Public FLimitNo
	Public FLimitSold
	Public FboriboriRegdate
	Public FboriboriLastUpdate
	Public FboriboriGoodNo
	Public FboriboriPrice
	Public FboriboriSellYn
	Public FRegUserid
	Public FboriboriStatCd
	Public FBoriboriRegisterStep
	Public FCateMapCnt
	Public FBrandMapCnt
	...
	Public Function getboriboriStatName
	    If IsNULL(FboriboriStatCd) then FboriboriStatCd=-1
		Select Case FboriboriStatCd
			CASE -9 : getboriboriStatName = "미등록"
			CASE -1 : getboriboriStatName = "등록실패"
			CASE 0 : getboriboriStatName = "<font color=blue>등록예정</font>"
			CASE 1 : getboriboriStatName = "전송시도"
			CASE 3 : 
				If FBoriboriRegisterStep = "1" Then
					getboriboriStatName = "<font color=red>등록중(STEP1)</font>"
				ElseIf FBoriboriRegisterStep = "2" Then
					getboriboriStatName = "<font color=red>등록중(STEP2)</font>"
				ElseIf FBoriboriRegisterStep = "3" Then
					getboriboriStatName = "<font color=red>등록중(STEP3)</font>"
				End If
			CASE 7 : getboriboriStatName = ""
			CASE ELSE : getboriboriStatName = FboriboriStatCd
		End Select
	End Function
...
Class CBoribori
	...
	Public FRectdiffPrc
	Public FRectboriboriYes10x10No
	Public FRectboriboriNo10x10Yes
	Public FRectboriboriKeepSell
	...
	'// boribori 상품 목록 // 수정시 조건이 달라야 함..
	Public Sub getboriboriRegedItemList
		Dim i, sqlStr, addSql
		'브랜드검색
		...
		'boribori 상품번호 검색
        If (FRectboriboriGoodNo <> "") then
            If Right(Trim(FRectboriboriGoodNo) ,1) = "," Then
            	FRectboriboriGoodNo = Replace(FRectboriboriGoodNo,",,",",")
            	addSql = addSql & " and J.boriboriGoodNo in ('" & replace(Left(FRectboriboriGoodNo, Len(FRectboriboriGoodNo)-1),",","','") & "')"
            Else
				FRectboriboriGoodNo = Replace(FRectboriboriGoodNo,",,",",")
            	addSql = addSql & " and J.boriboriGoodNo in ('" & replace(FRectboriboriGoodNo,",","','") & "')"
            End If
        End If
		...
		Select Case FRectExtNotReg
		    Case "A"	'전송시도중오류
				addSql = addSql & " and J.boriboriStatCd = 1"
			Case "W"	'등록예정이상
				addSql = addSql & " and J.boriboriStatCd >= 0"
			Case "D"	'등록완료(전시)
			    addSql = addSql & " and J.boriboriStatCd = 7"
				addSql = addSql & " and J.boriboriGoodNo is Not Null"
		End Select
		...
		If (FRectNotinmakerid <> "") then
			If (FRectNotinmakerid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'boribori1010') "
			ElseIf (FRectNotinmakerid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'boribori1010') "
			End If
		End If
		...
		If (FRectNotinitemid <> "") then
			If (FRectNotinitemid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'boribori1010') "
			ElseIf (FRectNotinitemid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'boribori1010') "
			End If
		End If
		...
		If (FRectExcTrans <> "") then
			If (FRectExcTrans = "Y") Then
				...
				addSql = addSql & " or i.makerid in (Select makerid From [db_temp].dbo.tbl_jaehyumall_not_in_makerid Where mallgubun='boribori1010') "
				addSql = addSql & " or i.itemid in (Select itemid From [db_temp].dbo.tbl_jaehyumall_not_in_itemid Where mallgubun='boribori1010') "
			...
			ElseIf (FRectExcTrans = "F") Then
				addSql = addSql & " and i.makerid not in (Select makerid From [db_temp].dbo.tbl_jaehyumall_not_in_makerid Where mallgubun='boribori1010') "
				addSql = addSql & " and i.itemid not in (Select itemid From [db_temp].dbo.tbl_jaehyumall_not_in_itemid Where mallgubun='boribori1010') "
		...
		If (FRectExtSellYn<>"") then
			If (FRectExtSellYn = "YN") Then
				addSql = addSql & " and J.boriboriSellYn <> 'X'"
			Else
				addSql = addSql & " and J.boriboriSellYn='" & FRectExtSellYn & "'"
			End if
		End If
		...
		Select Case FRectMatchBrand
			Case "Y"	'매칭완료
				addSql = addSql & " and isnull(b.prdGroupNo, '') <> ''"
			Case "N"	'미매칭
				addSql = addSql & " and isnull(b.prdGroupNo, '') = ''"
		End Select
		...
		If (FRectexpensive10x10 <> "") Then
			addSql = addSql & " and J.boriboriPrice is Not Null and J.boriboriPrice < i.sellcash"
		End If

		'가격상이전체보기
		If FRectdiffPrc <> "" Then
			addSql = addSql & " and J.boriboriPrice is Not Null and i.sellcash <> J.boriboriPrice "
		End If

		'GSShop판매 10x10 품절
		If (FRectboriboriYes10x10No <> "") Then
			addSql = addSql & " and i.sellyn<>'Y'"
			addSql = addSql & " and J.boriboriSellYn='Y'"
		End If

		'boribori품절&텐바이텐판매가능(판매중,한정>=10) 상품보기
		If FRectboriboriNo10x10Yes <> "" Then
			addSql = addSql & " and (J.boriboriSellYn= 'N' and i.sellyn='Y' and (i.limityn='N' or (i.limityn='Y' and i.limitno-i.limitsold>"&CMAXLIMITSELL&")))"
		End If

		'수정요망상품보기(최종업데이트일 기준)
		If FRectReqEdit <> "" Then
			addSql = addSql & " and J.boriboriLastUpdate < i.lastupdate "
		End If
		...
		If (FRectOrdType = "LU") Then
		    ...
		    addSql = addSql & " and Left(i.lastupdate, 10) <> Left(J.boriboriLastUpdate, 10) "
		End If
		...
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
		    sqlStr = sqlStr & " 	LEFT JOIN db_etcmall.dbo.tbl_boribori_regitem as J WITH (NOLOCK) "
		Else
		    sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_boribori_regitem as J WITH (NOLOCK) "
	    END IF
		sqlStr = sqlStr & " 		on i.itemid=J.itemid "
		sqlStr = sqlStr & "	LEFT Join db_etcmall.dbo.tbl_boribori_cate_mapping as c WITH (NOLOCK) on c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small "
		sqlStr = sqlStr & "	LEFT Join db_etcmall.dbo.tbl_boribori_brand_mapping as b WITH (NOLOCK) on i.makerid = b.makerid "
		sqlStr = sqlStr & " LEFT join db_user.dbo.tbl_user_c uc WITH (NOLOCK) on i.makerid = uc.userid"
		sqlStr = sqlStr & " LEFT JOIN db_etcmall.dbo.tbl_outmall_mustPriceItem as mi with (nolock) on mi.itemid = i.itemid and mi.mallgubun = 'boribori1010' "
		sqlStr = sqlStr & " LEFT JOIN [db_temp].dbo.tbl_schedule_not_in_itemid as sc with (nolock) on sc.itemid = i.itemid and sc.mallgubun = 'boribori1010' "
		sqlStr = sqlStr & " LEFT JOIN db_partner.dbo.tbl_partner_addInfo as f on f.partnerid = 'boribori1010' "
		...
		sqlStr = sqlStr & "	, J.boriboriRegdate, J.boriboriLastUpdate, isnull(J.boriboriGoodNo, '') as boriboriGoodNo, J.boriboriPrice, J.boriboriSellYn, J.regUserid, IsNULL(J.boriboriStatCd,-9) as boriboriStatCd, IsNULL(J.registerStep, '') as registerStep "
		sqlStr = sqlStr & "	, Case When isnull(c.CateKey, '') = '' Then 0 Else 1 End as mapcnt "
		sqlStr = sqlStr & "	, Case When isnull(b.prdGroupNo, '') = '' Then 0 Else 1 End as brandmapcnt "
		...
		sqlStr = sqlStr & "	, Ct.infoDiv, J.optAddPrcCnt, J.optAddPrcRegType, mi.mustPrice as specialPrice, mi.startDate, mi.endDate, sc.idx as notSchIdx, isNull(f.outmallstandardMargin, "& CMAXMARGIN &") as outmallstandardMargin, p.purchasetype "
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_item as i WITH (NOLOCK) "
		sqlStr = sqlStr & " JOIN db_item.dbo.tbl_item_contents as ct WITH (NOLOCK) on i.itemid = ct.itemid"
		sqlStr = sqlStr & " JOIN db_partner.dbo.tbl_partner as p with (nolock) on i.makerid = p.id"
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new CBoriboriItem
					...
					FItemList(i).FboriboriRegdate		= rsget("boriboriRegdate")
					FItemList(i).FboriboriLastUpdate	= rsget("boriboriLastUpdate")
					FItemList(i).FboriboriGoodNo		= rsget("boriboriGoodNo")
					FItemList(i).FboriboriPrice		= rsget("boriboriPrice")
					FItemList(i).FboriboriSellYn		= rsget("boriboriSellYn")
					FItemList(i).FRegUserid				= rsget("regUserid")
					FItemList(i).FboriboriStatCd		= rsget("boriboriStatCd")
					FItemList(i).FBoriboriRegisterStep	= rsget("registerStep")
    ...
    ''' 등록되지 말아야 될 상품..
    Public Sub getboriborireqExpireItemList
		Dim sqlStr, addSql, i
        If FRectMakerid <> "" Then
			addSql = addSql & " and i.makerid='" & FRectMakerid & "'"
		End if
		...
		sqlStr = sqlStr & " JOIN db_etcmall.dbo.tbl_boribori_regitem as J on J.itemid = i.itemid and J.boriboriGoodno is not null "
		sqlStr = sqlStr & " JOIN db_item.dbo.tbl_item_contents as ct on i.itemid = ct.itemid"
		sqlStr = sqlStr & "	LEFT Join db_etcmall.dbo.tbl_boribori_cate_mapping as c on c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small "
		sqlStr = sqlStr & "	LEFT Join db_etcmall.dbo.tbl_boribori_brand_mapping as b WITH (NOLOCK) on i.makerid = b.makerid "
		...
		sqlStr = sqlStr & "	, i.sellYn, i.sailyn, i.LimitYn, i.LimitNo, i.LimitSold, i.deliverytype, i.optionCnt"
		sqlStr = sqlStr & "	, J.boriboriRegdate, J.boriboriLastUpdate, J.boriboriGoodNo, J.boriboriPrice, J.boriboriSellYn, J.regUserid, IsNULL(J.boriboriStatCd,-9) as boriboriStatCd "
	...
	'// 텐바이텐-boribori 카테고리 리스트
	Public Sub getTenboriboriCateList
		...
		sqlStr = sqlStr & " 	SELECT cm.CateKey, cm.tenCateLarge,cm.tenCateMid, cm.tenCateSmall, cc.categoryName "  & VBCRLF
		sqlStr = sqlStr & " 	FROM db_etcmall.dbo.tbl_boribori_cate_mapping as cm  "  & VBCRLF
		sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.[tbl_boribori_category] as cc on cc.CateKey = cm.CateKey  "  & VBCRLF
		...
		sqlStr = sqlStr & " ,T.CateKey, T.categoryName, W.itemcnt "  & VBCRLF
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_cate_small as s " & VBCRLF
		sqlStr = sqlStr & " LEFT JOIN (  "  & VBCRLF
		sqlStr = sqlStr & " 	SELECT cm.CateKey, cm.tenCateLarge,cm.tenCateMid, cm.tenCateSmall,cc.categoryName "  & VBCRLF
		sqlStr = sqlStr & " 	FROM db_etcmall.dbo.tbl_boribori_cate_mapping as cm "  & VBCRLF
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new CBoriboriItem
					...
					FItemList(i).FCategoryName		= rsget("categoryName")
	...
	Public Sub getboriboriCateList
		Dim sqlStr, addSql, i

		If FsearchName <> "" Then
			addSql = addSql & " and (categoryName like '%" & FsearchName & "%')"
		End If

		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(*) as cnt, CEILING(CAST(Count(*) AS FLOAT)/" & FPageSize & ") as totPg " & VBCRLF
		sqlStr = sqlStr & " FROM db_etcmall.dbo.[tbl_boribori_category] " & VBCRLF
		...
		sqlStr = sqlStr & " FROM db_etcmall.dbo.[tbl_boribori_category] " & VBCRLF
		sqlStr = sqlStr & " WHERE 1 = 1 " & addSql
		sqlStr = sqlStr & " order by categoryName ASC "
		rsget.pagesize = FPageSize
		rsget.CursorLocation = adUseClient
		rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
		FResultCount = rsget.RecordCount-(FPageSize*(FCurrPage-1))
