...
<!-- #include virtual="/admin/etc/shodocEp/epShopCls.asp"-->
<%
...
If mode = "I" Then
	...
	sqlStr = sqlStr & " LEFT JOIN db_outmall.dbo.tbl_EpShop_not_in_itemid as ni on i.itemid = ni.itemid AND ni.mallgubun = 'shodocep' "
	sqlStr = sqlStr & " WHERE i.itemid in ("&NOTitemid&") "
	rsCTget.Open sqlStr,dbCTget,1
	If rsCTget("userCNT") <> SavearrCnt Then
		response.write "<script language='javascript'>alert('텐바이텐에 등록된 상품코드가 아닙니다.');location.href='/admin/etc/shodocEp/notinitemid.asp?menupos="&menupos&"';</script>"
	ElseIf rsCTget("notItemCNT") > 0 Then
		response.write "<script language='javascript'>alert('이미 등록된 상품코드 입니다.');location.href='/admin/etc/shodocEp/notinitemid.asp?menupos="&menupos&"';</script>"
	Else
		sqlStr = ""
		sqlStr = sqlStr & " INSERT INTO db_temp.dbo.tbl_EpShop_not_in_itemid (itemid, mallgubun, isusing, regdate, regid) "
		sqlStr = sqlStr & " SELECT itemid, 'shodocep', 'Y' ,getdate(), '"&session("ssBctID")&"' FROM db_item.dbo.tbl_item "
		...
		sqlStr = sqlStr & " SELECT itemid, 'shodocep', 'Y' ,getdate(), '"&session("ssBctID")&"' FROM db_AppWish.dbo.tbl_item "
		sqlStr = sqlStr & " WHERE itemid in ("&NOTitemid&") "
		dbCTget.Execute sqlStr
		response.write "<script language='javascript'>alert('저장하였습니다.');location.href='/admin/etc/shodocEp/notinitemid.asp?menupos="&menupos&"';</script>"
	End If
	rsCTget.Close
ElseIf mode = "U" Then
	Dim cnt, tmpIsusing
	...
	For i = 0 to cnt
		...
		dbCTget.execute sqlStr
	Next
	response.write "<script language='javascript'>alert('수정하였습니다.');location.href='/admin/etc/shodocEp/notinitemid.asp?menupos="&menupos&"';</script>"
End If
...
<!-- #include virtual="/admin/etc/shodocEp/inc_shodocHead.asp" -->
<!-- 검색 시작 -->
