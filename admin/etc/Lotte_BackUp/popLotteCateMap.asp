...
<%
	...
	dispNo = oLotte.FItemList(0).FDispNo
	dispNm = oLotte.FItemList(0).FDispNm
	dispFull = oLotte.FItemList(0).FDispLrgNm
	if Not(oLotte.FItemList(0).FDispMidNm="" or isNull(oLotte.FItemList(0).FDispMidNm)) then dispFull = dispFull & " > " & oLotte.FItemList(0).FDispMidNm
	if Not(oLotte.FItemList(0).FDispSmlNm="" or isNull(oLotte.FItemList(0).FDispSmlNm)) then dispFull = dispFull & " > " & oLotte.FItemList(0).FDispSmlNm
	if Not(oLotte.FItemList(0).FDispThnNm="" or isNull(oLotte.FItemList(0).FDispThnNm)) then dispFull = dispFull & " > " & oLotte.FItemList(0).FDispThnNm
%>
...
<!--
	...
	function fnSaveForm() {
		...
		if(frm.dspNo.value=="") {
			alert("매칭할 롯데닷컴 카테고리를 선택해주세요.");
			return;
		}
		if(confirm("선택하신 카테고리로 매칭하시겠습니까?")) {
			frm.mode.value="save";
			frm.action="procLotteCateMapping.asp";
			frm.submit();
		}
	}
	...
	function fnSearchLotteCate() {
		...
		srcFrm.action="popFindLotteCate.asp";
		srcFrm.submit();
	}
//-->
</script>
...
<tr valign="top">
	...
	<font color="red"><strong>롯데닷컴 카테고리 매칭</strong></font></td>
...
<tr height="25">
	...
	<td bgcolor="#FFFFFF">[<%=cdl%>] <%=oLotte.FItemList(0).FtenCDLName%></td>
</tr>
<tr height="25">
	<td width="80" align="center" bgcolor="#DDDDFF">중분류</td>
	<td bgcolor="#FFFFFF">[<%=cdm%>] <%=oLotte.FItemList(0).FtenCDMName%></td>
</tr>
<tr height="25">
	<td width="80" align="center" bgcolor="#DDDDFF">소분류</td>
	<td bgcolor="#FFFFFF">[<%=cds%>] <%=oLotte.FItemList(0).FtenCDSName%></td>
</tr>
</table>
<!-- 표 중간바 시작-->
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="a" bgcolor="F4F4F4">
<tr height="5" valign="top">
	<td width="10" align="right" background="/images/tbl_blue_round_04.gif"></td>
	<td align="left"><img src="/images/icon_arrow_down.gif" border="0" vspace="5" align="absmiddle"> 롯데닷컴 전시 카테고리 매칭 정보</td>
	<td width="10" align="left" background="/images/tbl_blue_round_05.gif"></td>
</tr>
</table>
<!-- 표 중간바 끝-->
