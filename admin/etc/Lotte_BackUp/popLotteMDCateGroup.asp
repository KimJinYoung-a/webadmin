...
<%
	...
	oLotte.FRectMdCode = mdCode
	oLotte.getLotte_MDGrpList
%>
<script language="javascript">
<!--
	// 상품군 갱신
	function refreshMDGroup() {
		if(confirm("MD상품군을 롯데닷컴 서버에서 내려받아 갱신하시겠습니까?\n\n※ 통신상태에따라 다소 시간이 걸릴 수 있습니다.")) {
			document.getElementById("btnRefresh").disabled=true;
			xLink.location.href="actLotteMDCateGroup.asp?mdcd=<%=mdCode%>";
		}
	}
...
<tr valign="top">
	...
	<font color="red"><strong>롯데닷컴 MD상품군 목록</strong></font></td>
...
<tr align="center" height="25" bgcolor="<%=chkIIF(oLotte.FItemList(i).FisUsing="Y","#FFFFFF","#DDDDDD")%>">
	<td><%= oLotte.FItemList(i).FgroupCode %></td>
	<td><%= oLotte.FItemList(i).FSuperGroupName %></td>
	<td><%= oLotte.FItemList(i).FGroupName %></td>
</tr>
<%
		next
	end if
