...
function etcmallSellYnProcess(chkYn, imallid) {
	...
	}else if (imallid == 'bindmall1010'){
		if (confirm('선택하신 ' + chkSel + '개 상품의 판매여부를 "' + strSell + '"(으)로 수정 하시겠습니까?\n\n※bindmall과의 통신상태에 따라 시간이 다소 걸릴 수 있습니다.')){
	        document.frmSvArr.target = "xLink";
	        document.frmSvArr.cmdparam.value = "EditSellYn";
	        document.frmSvArr.chgSellYn.value = chkYn;
...
<tr align="center" bgcolor="#FFFFFF" >
	...
	<% ElseIf mallid = "boribori1010" or mallid = "bindmall1010" or mallid = "benepia1010" Then %>
		<select name="apiAction" class="select">
			<option value="">전체</option>
			<option value="REG"		 <%= Chkiif(apiAction = "REG", "selected", "")%> >상품등록</option>
			<option value="EDIT"	 <%= Chkiif(apiAction = "EDIT", "selected", "")%> >상품수정</option>
	...
	<% Else %>
		<select name="apiAction" class="select">
			...
			<option value="INFODIV"	 <%= Chkiif(apiAction = "INFODIV", "selected", "")%> >정보고시수정</option>
			<option value="CHKSTAT"  <%= Chkiif(apiAction = "CHKSTAT", "selected", "")%> >신규상품조회</option>
...
<br />
<input type="button" value="10일 이전Log" class="button" onclick="location.replace('/admin/etc/que/popQueLogbackupList.asp?mallid=<%= mallid %>');">
<br />
<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
...
<% If mallid = "11st1010" or mallid = "interpark" or mallid = "gmarket1010" or mallid = "ssg" or mallid = "auction1010" or mallid = "kakaostore" or mallid = "boribori1010" or mallid = "benepia1010" Then %>
