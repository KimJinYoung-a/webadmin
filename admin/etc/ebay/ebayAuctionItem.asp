...
Dim bestOrdMall, auctionGoodNo, g9GoodNo, extsellyn, ExtNotReg, isReged, MatchCate, MatchBrand, optAddPrcRegTypeNone, notinmakerid, notinitemid, MatchG9, sellpriceChk, deliverytype, mwdiv, exctrans
Dim expensive10x10, diffPrc, auctionYes10x10No, auctionNo10x10Yes, auctionKeepSell, reqEdit, reqExpire, failCntExists, priceOption, isextusing, scheduleNotInItemid
Dim page, i, research, auctionGoodNoArray
Dim oEbay, gubun
...
End If
	...
	oEbay.FRectAuctionGoodNo			= auctionGoodNo
	...
	oEbay.FRectAuctionYes10x10No		= auctionYes10x10No
	oEbay.FRectAuctionNo10x10Yes		= auctionNo10x10Yes
	oEbay.FRectAuctionKeepSell		= auctionKeepSell
...
If isReged = "R" Then						'품절처리요망 상품보기 리스트
	oEbay.getAuctionreqExpireItemList
Else
	oEbay.getAuctionRegedItemList			'그 외 리스트
End If
%>
...
<tr align="center" bgcolor="#FFFFFF" >
	...
	<td align="left">
		...
		<select name="gubun" class="select" onchange="goMallgubun(this.value);";>
			<option value="auction" selected>옥션</option>
		...
		&nbsp;
		<a href="http://www.esmplus.com/Home/Home" target="_blank">ebay Admin바로가기</a>
		<%
...
<form name="frmReg" method="post" action="ebayAuctionitem.asp" style="margin:0px;">
...
<tr align="center" bgcolor="#FFFFFF">
	...
	<td><img src="<%= oEbay.FItemList(i).Fsmallimage %>" width="50" onClick="popOutMallEtcLink('<%= oEbay.FItemList(i).FItemID %>','auction1010','<%=oEbay.FItemList(i).FinfoDiv%>')" style="cursor:pointer"></td>
	<td align="center">
		<a href="<%=wwwURL%>/<%=oEbay.FItemList(i).FItemID%>" target="_blank"><%= oEbay.FItemList(i).FItemID %></a>
		<% If oEbay.FItemList(i).FAuctionStatcd <> 7 Then %>
		<br><%= oEbay.FItemList(i).getAuctionStatName %>
		<% End If %>
	...
	<td align="center"><%= oEbay.FItemList(i).FAuctionRegdate %><br><%= oEbay.FItemList(i).FAuctionLastUpdate %></td>
	<td align="right">
	...
	<td align="center">
	<%
		If (oEbay.FItemList(i).FAuctionStatCd > 0) Then
			If Not IsNULL(oEbay.FItemList(i).FAuctionPrice) Then
				If (oEbay.FItemList(i).Fsellcash <> oEbay.FItemList(i).FAuctionPrice) Then
	%>
					<strong><%= formatNumber(oEbay.FItemList(i).FAuctionPrice,0) %></strong><br>
	<%
				Else
					response.write formatNumber(oEbay.FItemList(i).FAuctionPrice,0)&"<br>"
				End If
				...
				If (oEbay.FItemList(i).FSellyn="Y" and oEbay.FItemList(i).FAuctionSellYn<>"Y") or (oEbay.FItemList(i).FSellyn<>"Y" and oEbay.FItemList(i).FAuctionSellYn="Y") Then
	%>
					<strong><%= oEbay.FItemList(i).FAuctionSellYn %></strong>
	<%
				Else
					response.write oEbay.FItemList(i).FAuctionSellYn
				End If
			End If
		End If
	%>
	...
	<td align="center">
	<%
		If Not(IsNULL(oEbay.FItemList(i).FAuctionGoodNo)) Then
			Response.Write "<a target='_blank' href='http://itempage3.auction.co.kr/detailview.aspx?itemNo="&oEbay.FItemList(i).FAuctionGoodNo&"'>"&oEbay.FItemList(i).FAuctionGoodNo&"</a>"
		End If
	%>
	</td>
	<td align="center"><%= oEbay.FItemList(i).Freguserid %></td>
	<td align="center">
		<a href="javascript:popManageOptAddPrc('<%=oEbay.FItemList(i).FItemID%>','0');"><%= oEbay.FItemList(i).FoptionCnt %>:<%= oEbay.FItemList(i).FregedOptCnt %></a>
		<br>
		<input type="button" class="button" value="일자" onclick="javascript:popAuctionDate('<%=oEbay.FItemList(i).FItemID%>');">
	</td>
...
<% AuctionGoodNoArray = AuctionGoodNoArray & oEbay.FItemList(i).FAuctionGoodNo & VBCRLF %>
<% Next %>
<% If (session("ssBctID")="kjy8517") Then %>
