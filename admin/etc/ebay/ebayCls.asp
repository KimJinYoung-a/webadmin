...
'			End If
	...
	Public Function getAuctionStatName
		...
		Select Case FAuctionStatCd
			CASE -9 : getAuctionStatName = "미등록"
			CASE -1 : getAuctionStatName = "등록실패"
			CASE 0 : getAuctionStatName = "<font color=blue>등록예정</font>"
			CASE 1 : getAuctionStatName = "전송시도"
			CASE 3 : getAuctionStatName = "<font color=red>OnSale변경전</font>"
...
Class CEbay
	...
	Public Sub getGmarketRegedItemList
		...
		sqlStr = sqlStr & "	, isnull(J.APIadditem, 'N') as APIadditem, isnull(J.APIaddgosi, 'N') as APIaddgosi, isnull(J.APIaddopt, 'N') as APIaddopt, displayDate, mi.mustPrice as specialPrice, mi.startDate, mi.endDate, sc.idx as notSchIdx "
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_item as i WITH (NOLOCK) "
		sqlStr = sqlStr & " JOIN db_item.dbo.tbl_item_contents as ct WITH (NOLOCK) on i.itemid = ct.itemid"
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
			sqlStr = sqlStr & " 	LEFT JOIN db_etcmall.dbo.tbl_gmarket_regItem as J WITH (NOLOCK) "
	...
	Public Sub getAuctionRegedItemList
		...
		'Auction 품절&텐바이텐판매가능(판매중,한정>=10) 상품보기
		...
		sqlStr = sqlStr & " JOIN db_item.dbo.tbl_item_contents as ct with (nolock) on i.itemid = ct.itemid"
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
		    sqlStr = sqlStr & " 	LEFT JOIN db_etcmall.dbo.tbl_auction1010_regItem as J with (nolock) "
		Else
		    sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_auction1010_regItem as J with (nolock) "
	    END IF
		sqlStr = sqlStr & " 		on i.itemid=J.itemid "
		...
		sqlStr = sqlStr & " ,uc.defaultdeliverytype, uc.defaultfreeBeasongLimit"
		sqlStr = sqlStr & "	, Ct.infoDiv, J.optAddPrcCnt, J.optAddPrcRegType"
		sqlStr = sqlStr & "	, Displaydate, mi.mustPrice as specialPrice, mi.startDate, mi.endDate "
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_item as i with (nolock) "
		sqlStr = sqlStr & " JOIN db_item.dbo.tbl_item_contents as ct with (nolock) on i.itemid = ct.itemid"
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
			sqlStr = sqlStr & " 	LEFT JOIN db_etcmall.dbo.tbl_auction1010_regItem as J with (nolock) "
	...
	Public Sub getTenEbayCateList
		...
		If FRectIsMapping = "Y" Then
			addSql = addSql & " and T.cateCode is Not null "
		ElseIf FRectIsMapping = "N" Then
			addSql = addSql & " and T.cateCode is null "
		End if

		If FRectKeyword<>"" Then
			Select Case FRectSDiv
				Case "CCD"	'ebay 전시코드 검색
					addSql = addSql & " and T.cateCode='" & FRectKeyword & "'"
		...
		sqlStr = sqlStr & " 	SELECT cm.cateCode, cm.tenCateLarge,cm.tenCateMid, cm.tenCateSmall, cc.cateName2, cc.cateName, cc.gubun " & VBCRLF
		sqlStr = sqlStr & " 	FROM db_etcmall.[dbo].[tbl_ebay_cate_mapping] as cm " & VBCRLF
		sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_ebay_siteCategory as cc on cc.cateCode = cm.cateCode " & VBCRLF
		sqlStr = sqlStr & " 	GROUP BY cm.cateCode, cm.tenCateLarge,cm.tenCateMid, cm.tenCateSmall, cc.cateName2, cc.cateName, cc.gubun " & VBCRLF
		...
		sqlStr = sqlStr & " ,code_nm as small_nm ,T.cateCode, T.cateName2, T.cateName, T.gubun " & VBCRLF
		sqlStr = sqlStr & " FROM db_item.dbo.tbl_cate_small as s " & VBCRLF
		sqlStr = sqlStr & " LEFT JOIN ( " & VBCRLF
		sqlStr = sqlStr & " 	SELECT cm.cateCode, cm.tenCateLarge,cm.tenCateMid, cm.tenCateSmall, cc.cateName2, cc.cateName, cc.gubun " & VBCRLF
		sqlStr = sqlStr & " 	FROM db_etcmall.[dbo].[tbl_ebay_cate_mapping] as cm " & VBCRLF
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new CEbayItem
					...
					FItemList(i).FCateName2			= rsget("cateName2")
	...
	Public Sub getESMCateList
		Dim sqlStr, addSql, i
		If FRectSearchName <> "" Then
			addSql = addSql & " and ( (e.SDCategoryName like '%"&FRectSearchName&"%') "
			'addSql = addSql & "		or (s.cateName like '%"&FRectSearchName&"%') "
			addSql = addSql & " )"
		End If

		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(*) as cnt, CEILING(CAST(Count(*) AS FLOAT)/" & FPageSize & ") as totPg FROM ( " & VBCRLF
		sqlStr = sqlStr & " 	SELECT e.SDCategoryCode, e.SDCategoryName, s.gubun, s.cateCode, s.cateName " & VBCRLF
		sqlStr = sqlStr & " 	FROM db_etcmall.[dbo].[tbl_ebay_esmCategory] as e " & VBCRLF
		sqlStr = sqlStr & "		JOIN db_etcmall.[dbo].[tbl_ebay_matchCategory] as m on e.SDCategoryCode = m.SDCategoryCode " & VBCRLF
		sqlStr = sqlStr & " 	JOIN db_etcmall.[dbo].[tbl_ebay_siteCategory] as s on m.siteCateCode = s.cateCode " & VBCRLF
		sqlStr = sqlStr & "		WHERE 1 = 1 " & addSql
		sqlStr = sqlStr & " 	GROUP BY e.SDCategoryCode, s.gubun, e.SDCategoryName, s.cateCode, s.cateName "
		sqlStr = sqlStr & "	) T "
		rsget.CursorLocation = adUseClient
        rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
			FTotalCount = rsget("cnt")
		...
		sqlStr = ""
		sqlStr = sqlStr & " SELECT TOP " & CStr(FPageSize*FCurrPage) & " e.SDCategoryCode, e.SDCategoryName, s.gubun, s.cateCode, s.cateName " & VBCRLF
		sqlStr = sqlStr & " FROM db_etcmall.[dbo].[tbl_ebay_esmCategory] as e " & VBCRLF
		sqlStr = sqlStr & " JOIN db_etcmall.[dbo].[tbl_ebay_matchCategory] as m on e.SDCategoryCode = m.SDCategoryCode " & VBCRLF
		sqlStr = sqlStr & " JOIN db_etcmall.[dbo].[tbl_ebay_siteCategory] as s on m.siteCateCode = s.cateCode " & VBCRLF
		sqlStr = sqlStr & " WHERE 1 = 1 " & addSql
		sqlStr = sqlStr & " ORDER BY SDCategoryCode, s.gubun, s.cateCode " & VBCRLF
		rsget.pagesize = FPageSize
		rsget.CursorLocation = adUseClient
        rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
		FResultCount = rsget.RecordCount-(FPageSize*(FCurrPage-1))
		...
		If not rsget.EOF Then
			...
			Do until rsget.eof
				Set FItemList(i) = new CEbayItem
					FItemList(i).FSDCategoryCode	= rsget("SDCategoryCode")
					FItemList(i).FSDCategoryName	= rsget("SDCategoryName")
					FItemList(i).FGubun				= rsget("gubun")
					FItemList(i).FCateCode			= rsget("cateCode")
					FItemList(i).FCateName			= rsget("cateName")
				i = i + 1
