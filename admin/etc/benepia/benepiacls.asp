...
Class CBenepiaItem
	...
	Public FLimitNo
	Public FLimitSold
	Public FbenepiaRegdate
	Public FbenepiaLastUpdate
	Public FbenepiaGoodNo
	Public FbenepiaPrice
	Public FbenepiaSellYn
	Public FRegUserid
	Public FbenepiaStatCd
	Public FCateMapCnt
	...
	Public Function getbenepiaStatName
	    If IsNULL(FbenepiaStatCd) then FbenepiaStatCd=-1
		Select Case FbenepiaStatCd
			CASE -9 : getbenepiaStatName = "미등록"
			CASE -1 : getbenepiaStatName = "등록실패"
			CASE 0 : getbenepiaStatName = "<font color=blue>등록예정</font>"
			CASE 1 : getbenepiaStatName = "전송시도"
			CASE 3 : getbenepiaStatName = "<font color=red>승인대기</font>"
			CASE 7 : getbenepiaStatName = ""
			CASE ELSE : getbenepiaStatName = FbenepiaStatCd
		End Select
	End Function
...
Class CBenepia
	...
	Public FRectdiffPrc
	Public FRectbenepiaYes10x10No
	Public FRectbenepiaNo10x10Yes
	Public FRectbenepiaKeepSell
	...
	'// benepia 상품 목록 // 수정시 조건이 달라야 함..
	Public Sub getbenepiaRegedItemList
		Dim i, sqlStr, addSql
		'브랜드검색
		...
		'benepia 상품번호 검색
        If (FRectbenepiaGoodNo <> "") then
            If Right(Trim(FRectbenepiaGoodNo) ,1) = "," Then
            	FRectbenepiaGoodNo = Replace(FRectbenepiaGoodNo,",,",",")
            	addSql = addSql & " and J.benepiaGoodNo in ('" & replace(Left(FRectbenepiaGoodNo, Len(FRectbenepiaGoodNo)-1),",","','") & "')"
            Else
				FRectbenepiaGoodNo = Replace(FRectbenepiaGoodNo,",,",",")
            	addSql = addSql & " and J.benepiaGoodNo in ('" & replace(FRectbenepiaGoodNo,",","','") & "')"
            End If
        End If
		...
		Select Case FRectExtNotReg
		    Case "A"	'전송시도중오류
				addSql = addSql & " and J.benepiaStatCd = 1"
			Case "W"	'등록예정이상
				addSql = addSql & " and J.benepiaStatCd >= 0"
			Case "R"	'승인대기
				addSql = addSql & " and J.benepiaStatCd = 3 "
			Case "D"	'등록완료(전시)
			    addSql = addSql & " and J.benepiaStatCd = 7"
				addSql = addSql & " and J.benepiaGoodNo is Not Null"
		End Select
		...
		If (FRectNotinmakerid <> "") then
			If (FRectNotinmakerid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'benepia') "
			ElseIf (FRectNotinmakerid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'benepia') "
			End If
		End If
		...
		If (FRectNotinitemid <> "") then
			If (FRectNotinitemid = "Y") Then
				addSql = addSql & " and exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'benepia') "
			ElseIf (FRectNotinitemid = "N") Then
				addSql = addSql & " and not exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'benepia') "
			End If
		End If
		...
		If (FRectExcTrans <> "") then
			If (FRectExcTrans = "Y") Then
				...
				addSql = addSql & " or i.makerid in (Select makerid From [db_temp].dbo.tbl_jaehyumall_not_in_makerid Where mallgubun='benepia') "
				addSql = addSql & " or i.itemid in (Select itemid From [db_temp].dbo.tbl_jaehyumall_not_in_itemid Where mallgubun='benepia') "
			...
			ElseIf (FRectExcTrans = "F") Then
				addSql = addSql & " and i.makerid not in (Select makerid From [db_temp].dbo.tbl_jaehyumall_not_in_makerid Where mallgubun='benepia') "
				addSql = addSql & " and i.itemid not in (Select itemid From [db_temp].dbo.tbl_jaehyumall_not_in_itemid Where mallgubun='benepia') "
		...
		If (FRectExtSellYn<>"") then
			If (FRectExtSellYn = "YN") Then
				addSql = addSql & " and J.benepiaSellYn <> 'X'"
			Else
				addSql = addSql & " and J.benepiaSellYn='" & FRectExtSellYn & "'"
			End if
		End If
		...
		If (FRectexpensive10x10 <> "") Then
			addSql = addSql & " and J.benepiaPrice is Not Null and J.benepiaPrice < i.sellcash"
		End If

		'가격상이전체보기
		If FRectdiffPrc <> "" Then
			addSql = addSql & " and J.benepiaPrice is Not Null and i.sellcash <> J.benepiaPrice "
		End If

		'GSShop판매 10x10 품절
		If (FRectbenepiaYes10x10No <> "") Then
			addSql = addSql & " and i.sellyn<>'Y'"
			addSql = addSql & " and J.benepiaSellYn='Y'"
		End If

		'benepia품절&텐바이텐판매가능(판매중,한정>=10) 상품보기
		If FRectbenepiaNo10x10Yes <> "" Then
			addSql = addSql & " and (J.benepiaSellYn= 'N' and i.sellyn='Y' and (i.limityn='N' or (i.limityn='Y' and i.limitno-i.limitsold>"&CMAXLIMITSELL&")))"
		End If

		'수정요망상품보기(최종업데이트일 기준)
		If FRectReqEdit <> "" Then
			addSql = addSql & " and J.benepiaLastUpdate < i.lastupdate "
		End If
		...
		If (FRectOrdType = "LU") Then
		    ...
		    addSql = addSql & " and Left(i.lastupdate, 10) <> Left(J.benepiaLastUpdate, 10) "
		End If
		...
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
		    sqlStr = sqlStr & " 	LEFT JOIN db_etcmall.dbo.tbl_benepia_regitem as J WITH (NOLOCK) "
		Else
		    sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_benepia_regitem as J WITH (NOLOCK) "
	    END IF
		sqlStr = sqlStr & " 		on i.itemid=J.itemid "
		sqlStr = sqlStr & "	LEFT Join db_etcmall.dbo.tbl_benepia_cate_mapping as c WITH (NOLOCK) on c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small "
		sqlStr = sqlStr & " LEFT join db_user.dbo.tbl_user_c uc WITH (NOLOCK) on i.makerid = uc.userid"
		sqlStr = sqlStr & " LEFT JOIN db_etcmall.dbo.tbl_outmall_mustPriceItem as mi with (nolock) on mi.itemid = i.itemid and mi.mallgubun = 'benepia1010' "
		sqlStr = sqlStr & " LEFT JOIN [db_temp].dbo.tbl_schedule_not_in_itemid as sc with (nolock) on sc.itemid = i.itemid and sc.mallgubun = 'benepia1010' "
		sqlStr = sqlStr & " LEFT JOIN db_partner.dbo.tbl_partner_addInfo as f on f.partnerid = 'benepia' "
		...
		sqlStr = sqlStr & "	, J.benepiaRegdate, J.benepiaLastUpdate, isnull(J.benepiaGoodNo, '') as benepiaGoodNo, J.benepiaPrice, J.benepiaSellYn, J.regUserid, IsNULL(J.benepiaStatCd,-9) as benepiaStatCd "
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				Set FItemList(i) = new CbenepiaItem
					...
					FItemList(i).FbenepiaRegdate		= rsget("benepiaRegdate")
					FItemList(i).FbenepiaLastUpdate	= rsget("benepiaLastUpdate")
					FItemList(i).FbenepiaGoodNo		= rsget("benepiaGoodNo")
					FItemList(i).FbenepiaPrice		= rsget("benepiaPrice")
					FItemList(i).FbenepiaSellYn		= rsget("benepiaSellYn")
					FItemList(i).FRegUserid				= rsget("regUserid")
					FItemList(i).FbenepiaStatCd		= rsget("benepiaStatCd")
    ...
    ''' 등록되지 말아야 될 상품..
    Public Sub getbenepiareqExpireItemList
		Dim sqlStr, addSql, i
        If FRectMakerid <> "" Then
			addSql = addSql & " and i.makerid='" & FRectMakerid & "'"
		End if
		...
		sqlStr = sqlStr & " JOIN db_etcmall.dbo.tbl_benepia_regitem as J on J.itemid = i.itemid and J.benepiaGoodno is not null "
		sqlStr = sqlStr & " JOIN db_item.dbo.tbl_item_contents as ct on i.itemid = ct.itemid"
		sqlStr = sqlStr & "	LEFT Join db_etcmall.dbo.tbl_benepia_cate_mapping as c on c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small "
		...
		sqlStr = sqlStr & "	, i.sellYn, i.sailyn, i.LimitYn, i.LimitNo, i.LimitSold, i.deliverytype, i.optionCnt"
		sqlStr = sqlStr & "	, J.benepiaRegdate, J.benepiaLastUpdate, J.benepiaGoodNo, J.benepiaPrice, J.benepiaSellYn, J.regUserid, IsNULL(J.benepiaStatCd,-9) as benepiaStatCd "
	...
	'// 텐바이텐-benepia 카테고리 리스트
	Public Sub getTenbenepiaCateList
		...
		sqlStr = sqlStr & " 	FROM db_etcmall.dbo.tbl_benepia_cate_mapping as cm  "  & VBCRLF
		sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.[tbl_benepia_totalcategory] as cc on cc.CateKey = cm.CateKey  "  & VBCRLF
		...
		sqlStr = sqlStr & " ,T.CateKey, T.name, T.catename "  & VBCRLF
	...
	Public Sub getbenepiaCateList
		...
		sqlStr = ""
		sqlStr = sqlStr & " SELECT count(*) as cnt, CEILING(CAST(Count(*) AS FLOAT)/" & FPageSize & ") as totPg " & VBCRLF
		sqlStr = sqlStr & " FROM db_etcmall.dbo.[tbl_benepia_totalcategory] " & VBCRLF
