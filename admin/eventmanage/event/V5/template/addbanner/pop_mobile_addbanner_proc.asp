...
Sub fnevtaddimgcnt()
	Dim imgcnt : imgcnt = 0
	sqlStr = "SELECT count(*) FROM db_event.dbo.tbl_event_mobile_addbanner where evt_code = '"&eventid&"' and isusing = 'Y' and menuidx=" & menuidx
...
Select Case mode
	 Case "SI"
		...
		sqlStr = "IF NOT EXISTS(SELECT idx FROM db_event.dbo.tbl_event_multi_contents WHERE menuidx=" & menuidx  & " and device='M')" & vbCrLf
		sqlStr = sqlStr & "	BEGIN" & vbCrLf
		sqlStr = sqlStr & "		Insert Into db_event.dbo.tbl_event_multi_contents(menuidx, device , imgurl)" & vbCrLf
		sqlStr = sqlStr & "  	values('" & menuidx  & "','M','" & bimg &"')" & vbCrLf
		sqlStr = sqlStr & "	END"
		dbget.Execute(sqlStr)
	...
	Case "SU"
		...
		for i=1 to request.form("chkIdx").count
			...
			sqlStr = sqlStr & " ,blink='" & Trim(sBlink) & "'"
	...
	Case "SD" '삭제
		...
		sqlStr = "IF NOT EXISTS(SELECT top 1 idx FROM db_event.dbo.tbl_event_mobile_addbanner WHERE menuidx=" & menuidx  & ")" & vbCrLf
		sqlStr = sqlStr & "	BEGIN" & vbCrLf
		sqlStr = sqlStr & "		DELETE FROM db_event.dbo.tbl_event_multi_contents WHERE menuidx=" & menuidx  & " AND device='M'" & vbCrLf
		sqlStr = sqlStr & "	END"
		dbget.Execute(sqlStr)
...
<!--
	...
	self.location = "pop_mobile_addbanner.asp?eC=<%=eventid%>&menuidx=<%=menuidx%>";
//-->
</script>
