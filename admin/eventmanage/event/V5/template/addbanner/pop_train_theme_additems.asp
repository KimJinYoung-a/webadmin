...
<%
'###############################################
' PageName : pop_train_theme_additems.asp
' Discription : I형 추가 테마 기차형 배너 등록(아이템 선택)
...
<%
dim eC, page, dispCate, itemname, i, stype, maxDepth, searchdiv, catesort, itemid, itemarr, GroupItemCheck, menuidx
...
elseif searchdiv="brand" Then
oitem.FRectMakerid = itemname
Else
...
oitembanner.FRectGroupType = "T"
oitembanner.FRectIDX = menuidx
oitembanner.GetMultiTrainThemeItemList
...
function MainWindowReloadClose(){
	opener.location.reload();
	self.close();
}

function MainWindowReload(){
	document.frmArrupdate.mode.value="MR";
	document.frmArrupdate.target="FrameCKP";
...
<div class="popWinV17">
	...
	<div class="popContainerV17">
		<div class="contL">
			...
			<input type='hidden' name='menuidx' id='menuidx'value="<%=menuidx%>">
			<div id="unitType01" class="unitPannel">
				<div class="searchWrap" style="border-top:none;">
					...
					상품코드 : <textarea rows="3" cols="10" name="itemid" id="itemid"><%=itemarr%></textarea>
		...
		<form name="frmArrupdate" method="post" action="dotrainthemeadditem.asp">
			...
			<div class="tbListWrap">
				...
				<% For i=0 To oitembanner.FresultCount-1 %>
					<li id='del<%= oitembanner.FItemList(i).Fitemid %>'>
						...
						<p class='lt'><span class='textOverflow'><%= DDotFormat(oitembanner.FItemList(i).Fitemname,20) %></span></p>
					</li>
				<% Next %>
				</ul>
				<% Else %>
	...
	<div class="popBtnWrap">
		<input type="button" value="저장" onclick="MainWindowReload();" class="cRd1" style="width:100px; height:30px;" />
...
$(function(){
	$("#additem").click(function(){
		...
		if($("#items input:checkbox[name='cksel']:checked").length<1){
			...
			alert("같은 상품이 있습니다.");
			return false;
		}else if((Number($("#check").val())+Number($("#items input:checkbox[name='cksel']:checked").length))>32){
			alert("상품 배너 추가 갯수는 32개 입니다.");
			return false;
		}else{
			// 행추가
			var oRow;
