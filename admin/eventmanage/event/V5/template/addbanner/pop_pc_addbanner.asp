...
<%
'###############################################
' PageName : pop_pc_addbanner.asp
' Discription : PC slide insert
...
function saveList() {
	...
	if(confirm("지정하신 목록의 선택 정보를 저장하시겠습니까?")) {
		frm.action="pop_pc_addbanner_proc.asp";
		frm.submit();
	}
}
...
function simgsubmit(){ 
	...
	if (!frm.btitle.value){
		alert("Alt값을 입력 해주세요");frm.btitle.focus();return false;
	}
...
<div class="slideRegister adminMob bnrRegister">
	<h1>배너 등록 (PC Web)&nbsp;&nbsp;/&nbsp;&nbsp;<a href="/admin/eventmanage/event/v5/template/addbanner/pop_mobile_addbanner.asp?eC=<%=eCode%>&menuidx=<%=menuidx%>" style="color:#FFFFFF">(MOBILE)</a> <input type="button" class="btn" value="배경/레이아웃 설정" onclick="fnMasterInfoSet();"></h1>
	<div class="register">
		<dl>
			<dd>
				<form name="slideimgfrm" method="post" action="pop_pc_addbanner_proc.asp" style="margin:0px;">
				...
				<div class="insertImg">
					<table class="tbType1 listTb">
						...
						<tr>
							<td>
								<span><input type="radio" name="gubun" value="1" id="gt" checked /> <label for="gt">상</label></span>
								<span class="lMar05"><input type="radio" name="gubun" value="2" id="gm" disabled /> <label for="gm">중</label></span>
								<span class="lMar05"><input type="radio" name="gubun" value="3" id="gb" disabled /> <label for="gb">하</label></span>
							</td>
							<td>
							...
							<td>
								<div class="selectLink">
									...
									<ul style="display:none;">
										...
										<li onclick="populateTextBox('/deal/deal.asp?itemid=상품코드');">/deal/deal.asp?itemid=상품코드 (O)</li>
							...
							<!--<td>
								<p>시작일 : <input type="text" onclick="jsPopCal('bst_date');" value="<%=sDt%>" style="width:85px; cursor:pointer;" name="bst_date" readonly> ~ 종료일 : <input type="text" value="<%=eDt%>" onclick="jsPopCal('bed_date');" style="width:85px; cursor:hand;" name="bed_date" readonly></p>
								<p class="tMar05"><input type="checkbox" name="bdate_flag" value="Y" onclick="jscalcopy('bst_date','bed_date','bdate_flag');" checked /> 이벤트 기간 내내 노출</p>
							</td>-->
				...
				<div class="tMar20">
					<table class="tbType1 listTb">
						...
						<% 
							...
							sqlStr = sqlStr & " from db_event.dbo.tbl_event_pc_addbanner where evt_code = '"& eCode &"'" & vbcrlf
						...
						<tr class="<%=chkIIF(rsget("isusing")="N","bgGry1","")%>">
							<td><input type="checkbox" name="chkIdx" value="<%=rsget("idx")%>" /><input type="hidden" name="viewidx" value="<%=rsget("gubun")%>"></td>
							<td>
								<span><input type="radio" name="gubun<%=rsget("idx")%>" value="1" <%=chkiif(rsget("gubun")=1,"checked","")%> id="gt<%=rsget("idx")%>" checked /> <label for="gt<%=rsget("idx")%>">상</label></span>
								<span class="lMar05"><input type="radio" name="gubun<%=rsget("idx")%>" value="2" <%=chkiif(rsget("gubun")=2,"checked","")%> id="gm<%=rsget("idx")%>" disabled /> <label for="gm<%=rsget("idx")%>">중</label></span>
								<span class="lMar05"><input type="radio" name="gubun<%=rsget("idx")%>" value="3" <%=chkiif(rsget("gubun")=3,"checked","")%> id="gb<%=rsget("idx")%>" disabled /> <label for="gb<%=rsget("idx")%>">하</label></span>
							...
							<!--<td>
								<p>시작일 : <input type="text" onclick="jsPopCal_2('bst_date<%=rsget("idx")%>','bdate_flag<%=rsget("idx")%>');" value="<%=rsget("bst_date")%>" style="width:85px; cursor:pointer;" name="bst_date<%=rsget("idx")%>"> ~ 종료일 : <input type="text" onclick="jsPopCal_2('bed_date<%=rsget("idx")%>','bdate_flag<%=rsget("idx")%>');" value="<%=rsget("bed_date")%>" style="width:85px; cursor:pointer;" name="bed_date<%=rsget("idx")%>"></p>
...
<form name="frmdel" method="POST" action="pop_pc_addbanner_proc.asp" style="margin:0px;">
<input type="hidden" name="sDt" value="<%=sDt%>"/>
<input type="hidden" name="eDt" value="<%=eDt%>"/>
<input type="hidden" name="eventid" value="<%=eCode%>"/>
<input type="hidden" name="menuidx" value="<%=menuidx%>"/>
