...
Dim mode , idx , strSql , sqlStr , isarrow
	...
	If eCode <> "" Then 
		strSql = "SELECT top 1 idx , topimg , btmimg , topaddimg, videosize, videolink , isarrow " & vbcrlf
		strSql = strSql & " from db_event.dbo.tbl_event_slide_template where evt_code = '"& eCode &"'" & vbcrlf
		strSql = strSql & " and device = 'M' and menuidx=" & menuidx 
...
<div class="slideRegister adminMob">
	<h1>이미지 템플릿 슬라이드 등록 (MOBILE)&nbsp;&nbsp;/&nbsp;&nbsp;<a href="/admin/eventmanage/event/v5/template/slide/pop_pcweb_slide.asp?eC=<%=eCode%>&menuidx=<%=menuidx%>" style="color:#FFFFFF">(PC WEB)</a></h1>
	<%'슬라이드 화면 불러오기%>
