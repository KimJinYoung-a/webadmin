...
dim arrGroupMo, dispCate, sellyn
...
function jsSortImgSize() {
	var frm;
	var sValue, sSort, sImgSize,sDisp ;
	frm = document.fitem;
	sValue = "";
	sSort = "";
	sImgSize = ""; 
	...
	if (frm.chkI.length > 1){
		...
		document.frmSortImgSize.itemsort.value=document.fitem.itemsort.value;
	...
	function jsExcelDown(iCurrpage){
		document.fitem.target="_blank";
		document.fitem.action="/admin/eventmanage/event/V5/popup/eventitemlist_excel.asp";
		document.fitem.submit();
		document.fitem.action="/admin/eventmanage/event/V5/popup/eventitem_regist.asp";
	}
</script>
<style type="text/css">
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="a"  style="padding-top:10px">
		...
		<td style="padding-bottom:10">  
			<table width="100%" border="0" align="left" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
				<tr>
					...
					<td width="30%" bgcolor="#FFFFFF" style="padding: 0 0 0 5"><%=eCode%>&nbsp;&nbsp; <a href="<%=vwwwUrl%>/event/eventmain.asp?eventid=<%=eCode%>" target="_blank">[미리보기]</a></td>
	...
	<tr>
	    <td><div class="tab btmLine">
				<ul style="margin-left:-1px">
					<li class="col11  selected">PC_WEB 상품등록</li>
					<li class="col11" onclick="location.href='eventitem_regist_mo.asp?eC=<%=eCode%>&menupos=<%=menupos%>'">Mobile / APP 상품등록</a></li> 
				</ul>
			</div> 
		</td>
	</tr>	
	<tr><!-- 검색--->
		<td>     
		    <table cellspacing="5"  bgcolor="FAECC5" width="100%" class="a" cellpadding="0">
		        <tr>
		            <td bgcolor="#FFFFFF"> 
            			<form name="fsearch" method="post" action="eventitem_regist.asp"> 
            						...
            						<tr>
            							<td style="white-space:nowrap;">그룹: 
            								<select name="selG" onChange="jsSearch();" class="select">
            						       	...
            						       	%>
            						       		<option value="<%=arrGroup(0,intLoop)%>" <%IF Cstr(strG) = Cstr(arrGroup(0,intLoop)) THEN %> selected<%END IF%> <%if not arrGroup(8,intLoop) then%>style="color:gray;"<%end if%>><%IF arrGroup(5,intLoop) <> 0 THEN%>└&nbsp;<%END IF%><%=arrGroup(0,intLoop)%>(<%=arrGroup(1,intLoop)%>) <%if not arrGroup(8,intLoop) then%>-[전시안함]<%end if%></option>
            						     	<%	Next 
            						    	END IF%>	
            						       	</select> 	
            			       			</td> 
										...
										<td style="white-space:nowrap;padding-left:5px;">전시카테고리 : <!-- #include virtual="/common/module/dispCateSelectBox.asp"--></td>
            						</tr> 
            						<tr>
            						    <td colspan="4">
											<label>상품명: <input type="text" class="text" name="itemname" value="<%= itemname %>" size="32" maxlength="20" /></label>
											&nbsp;
											<label>판매: <% drawSelectBoxSellYN "sellyn", sellyn %></label>
										</td>
            						</tr>
            			        	</table>        			        	
            			        </td>
                      			    ...
                      			    <tr height="35">			      
                      			        <td align="left">       	
                          			       	...
                          			       	<%
                          			       		For intLoop = 0 To UBound(arrGroup,2)
                          			       	%>
                          			       		<option value=" <%=arrGroup(0,intLoop)%>" <%if not arrGroup(8,intLoop) then%>style="color:gray;"<%end if%>><%IF arrGroup(5,intLoop) <> 0 THEN%>└&nbsp;<%END IF%><%=arrGroup(0,intLoop)%>(<%=arrGroup(1,intLoop)%>)<%if not arrGroup(8,intLoop) then%> -[전시안함]<%end if%></option>
                          			    	<%	Next 
                          			    	  ELSE	
                          			    	%>
                      			    	...
                      			    	<td align="right"> 
										  	<input type="button" value="엑셀 다운" onclick="jsExcelDown();"  class="button">
                          ...
                          <tr>
                      	    <td> 
                      			<table width="100%" border="0" align="center" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>"> 
                      			    ...
                      			    <tr align="center" bgcolor="<%= adminColor("tabletop") %>">
                      					...
                      					<td>순서</td>  
                      			    	<td>이미지크기<br>
                      			    	    <% sbGetVarOptEventCodeValue "selimgsize","itemimgsize", "", false, "onchange=""jsSizeChg(this.value)""" %> 
                      			    	</td>
									...
									<tbody id="subList">
                      			    ...
                      			    <tr align="center" bgcolor="<%if  arrList(29,intLoop) then%>#FFFFFF<%else%>gray<%end if%>">    
                      			    	...
                      			    	<td>
										<% If arrList(30,intLoop)="21" Then %>
										<% if (Not IsNull(arrList(12,intLoop)) ) and (arrList(12,intLoop)<>"") then %>
                      					 <img src="http://webimage.10x10.co.kr/image/small/<%=arrList(12,intLoop)%>">
                      					<%end if%>
										<% Else %>
										<% if (Not IsNull(arrList(12,intLoop)) ) and (arrList(12,intLoop)<>"") then %>
										...
										<%end if%>
                      			    	...
                      			    	<td class="sortablearrow"><%=db2html(arrList(3,intLoop))%></td>
                      			    	<td align="left" class="sortablearrow">&nbsp;<%=db2html(arrList(4,intLoop))%></td>
                      			    	<td><%
                      		...
                      		%></td>
                      					<td><%if arrList(18,intLoop)="Y" then%>
                      						...
                      						<!-- <font color=#F08050><%' formatnumber(((arrList(7,intLoop)-arrList(9,intLoop))/arrList(7,intLoop))*100,0)%>%</font> 2016-06-17 10:16 김진영 위 할인으로 수정..위가 프론트 노출 계산식-->	
                      						<%end if%>
                      						<%if arrList(22,intLoop)="Y" then 
                      						if arrList(23,intLoop)="1" or arrList(23,intLoop)="2" then
                      					        if arrList(25,intLoop)=0 or isNull(arrList(25,intLoop)) then
                      			    	...
                      			    	<td class="sortablearrow"><input type="text" name="sSort" value="<%=arrList(2,intLoop)%>" size="4" style="text-align:right;"></td>
                      			    	<td class="sortablearrow"><% sbGetVarOptEventCodeValue "itemimgsize","itemimgsize", arrList(27,intLoop), false, "" %></td> 
    ...
    <tr>
	    <td   align="right" height="30"><a href="index.asp?menupos=<%=menupos%>&<%=strparm%>"><img src="/images/icon_list.gif" border="0"></a></td>			        
	</tr>
</table>
<%
