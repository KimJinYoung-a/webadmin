...
function jsSerach(){
	...
	frm = document.frm;
	frm.target = "_self";
	frm.action ="multi3_eventitem_regist.asp";
	frm.submit();
}
...
function NextPage(ipage){
	...
	document.frm.action ="multi3_eventitem_regist.asp";
	document.frm.submit();
}
function addItem(itemid, itemimg, itemname){
	
	<% if itemIdx <> "" then %>
	if(confirm("상품을 변경하시겠습니까?")){				
	var frm = window.opener.document.contentfrm;
	frm.itemid[<%=itemIdx%>].value = itemid;		
	frm.item_name[<%=itemIdx%>].value=itemname;	
	//이미지		
	frm.item_img[<%=itemIdx%>].value=itemimg;
	frm.item_imgsrc[<%=itemIdx%>].src=itemimg;
	}
	<% else %>
	var frm = window.opener.document.itenFrm;	
	if(confirm("상품을 추가하시겠습니까?")){
		frm.itemid.value=itemid;		
		frm.item_name.value=itemname;	
		//이미지		
		frm.item_img.value=itemimg;
		frm.item_imgsrc.src=itemimg;							
	}		
	<% end if %>
	window.close();
}
...
<% if oitem.FresultCount > 0 then %>
	...
	<tr class="a" onclick="addItem('<%=oitem.FItemList(i).FItemId%>','<%= oitem.FItemList(i).Fbasicimage %>','<% =oitem.FItemList(i).Fitemname %>');" height="25" bgcolor="#FFFFFF" style="cursor:pointer;" onmouseover="this.style.backgroundColor='#D8D8D8'" onmouseout="this.style.backgroundColor=''">	
