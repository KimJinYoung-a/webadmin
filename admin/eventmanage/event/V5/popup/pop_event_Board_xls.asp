...
' Description :  이벤트 게시판 참여자 Excel 옵션선택 팝업
...
<!--
	function chkForm()
	{
		...
		if(frm.Sdate.value.length<10) {
			alert("게시판 참여 시작일을 입력해주세요.");
			frm.Sdate.focus();
			return false;
		}

		if(frm.Edate.value.length<10) {
			alert("게시판 참여 종료일을 입력해주세요.");
			frm.Edate.focus();
			return false;
		}
...
<tr height="23">
	<td colspan="2" bgcolor="#F3F3F5"><b>이벤트 게시판 참여자 다운로드 옵션 선택</b></td>
</tr>
<tr>
