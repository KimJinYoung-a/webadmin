...
sGimg = requestCheckVar(Request("sGimg"),128)
tGMap = html2db(Trim(Request("tGMap")))

if sGimg <> "" then
	if checkNotValidHTML(sGimg) then
...
CASE "GU"
		if eChannel = "P" then
			strSql = "UPDATE  [db_event].[dbo].[tbl_eventitem_group]"&_
					" SET evtgroup_desc = '" & eGDesc & "'"&_
					" ,evtgroup_sort = '" & eGSort & "'"&_
					" ,evtgroup_sort_mo = '" & eGSort & "'"&_
					" ,evtgroup_img = '" & sGimg & "'"&_
					" ,evtgroup_link = '" & tGMap & "'"&_
					" ,evtgroup_isDisp = '" & eGIsDisp & "'"&_
					" WHERE evtgroup_code ="&evtgroup_code											
			dbget.execute strSql
		else
			strSql = "UPDATE  [db_event].[dbo].[tbl_eventitem_group]"&_
					" SET evtgroup_desc_mo = '" & eGDesc & "'"&_
					" ,evtgroup_sort_mo = '" & eGSort & "'"&_
					" ,evtgroup_img_mo = '" & sGimg & "'"&_
					" ,evtgroup_link_mo = '" & tGMap & "'"&_
					" ,evtgroup_isDisp_mo = '" & eGIsDisp & "'"&_
					" WHERE evtgroup_code ="&evtgroup_code											
			dbget.execute strSql
		end if
		vChangeContents = vChangeContents & "- 이벤트(" & eCode & ") 그룹 이미지등록,수정 = " & evtgroup_code & vbCrLf
	  ...
	  dim eGDescArr, eGBArr, eGLKArr
	    ...
	    IF isARRay(eGCodeArr) THEN
    	    For intLoop = 0 To UBound(eGCodeArr)
    	      ...
    	      if eChannel = "P" then
				...
				strSql = strSql&", evtgroup_sort_mo = "&trim(eSortArr(intLoop))
        	            if ePGCodeArr(intLoop) = 0 then
	    ...
	    ELSE
	       if eChannel = "P" then
        	    ...
        	    strSql = strSql&", evtgroup_sort_mo = "&eSortArr
				strSql = strSql&", evtgroup_pcode = 0"   
        	    strSql = strSql&"  , evtgroup_depth =   "&newDepth 
...
CASE "F" '//기본폴더   
	    ...
	    Next
			...
			Call sbAlertMsg (strMsg,  "pop_eventitem_group.asp?eC="&eCode&"&eCh="&eChannel&"&sTarget="&sTarget, "self")
			response.end
CASE "C"	 '그룹전체복사
