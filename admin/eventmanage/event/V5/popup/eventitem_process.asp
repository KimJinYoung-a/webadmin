...
Select Case mode
	...
	END IF	
		...
		END IF		 
			...
			sqlStr = sqlStr & "     left outer join  [db_event].[dbo].tbl_eventitem  as ei on i.itemid = ei.itemid and ei.evt_code =  "&eCode& " AND ei.evtgroup_code = " & sGroup & VbCrlf
			sqlStr = sqlStr & " where  ei.itemid is null "& addSql  
		 	dbget.execute sqlStr
	    
	    
			sqlStr =" if exists( select ei.itemid from db_item.dbo.tbl_item as i inner join  [db_event].[dbo].tbl_eventitem  as ei on i.itemid = ei.itemid and ei.evt_code =  "&eCode&" and ei.evtitem_isusing = 0 AND ei.evtgroup_code = " & sGroup & addSql & " )"
			sqlStr = sqlStr & " update ei set evtitem_isusing = 1 "& VbCrlf
			sqlStr = sqlStr & " from db_item.dbo.tbl_item as i "  & VbCrlf
			sqlStr = sqlStr & " inner join  [db_event].[dbo].tbl_eventitem  as ei "& VbCrlf
			sqlStr = sqlStr & "  on i.itemid = ei.itemid and ei.evt_code =  "&eCode&" and ei.evtitem_isusing = 0 AND ei.evtgroup_code = " & sGroup & addSql
			dbget.execute sqlStr

			'이벤트 아이템 할인율 체크
			sqlStr = "EXEC db_event.dbo.usp_SCM_Event_ItemSaleCouponInfo_Upd " & CStr(eCode)
			dbget.execute sqlStr
	...
	Case "D" '// 선택상품 삭제
			sqlStr = "Delete From  [db_event].[dbo].tbl_eventitem "&_
					"	WHERE evt_code = "&eCode&" and itemid in ("&itemidarr&") " & chkIIF(strG<>"", " AND evtgroup_code = " & strG, "")
			dbget.execute sqlStr
		...
		'// 정렬순서 저장  
    		...
    		tmpDisp = "NULL"		
				...
				if eChannel ="P" then
                    ...
                    sqlStr =	sqlStr&	"	WHERE evt_code = "&eCode&" and itemid =" & itemidarr(i) & chkIIF(strG<>"", " AND evtgroup_code = " & strG, "")
                    dbget.execute sqlStr
			next   

	    sqlStr = "UPDATE [db_event].dbo.tbl_event_display SET evt_itemsort="&itemsort&" WHERE evt_code = "&eCode 
		dbget.execute sqlStr 
