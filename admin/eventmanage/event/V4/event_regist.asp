...
dim arrText,intT
dim blnReqPublish, cbrand, tmp_cdl
 dim chkeCmt, chkeIps, chkeGf, chkeBS 
eCode = Request("eC")
ekind = Request("eK")

response.redirect "/admin/eventmanage/event/v5/event_register.asp"
response.end
...
//	  	}
         ...
         if(document.all.dvCmt.style.display ==""){
	  	...
	  	//유형선택 여부 확인
	  	if(!frm.pc_evttype.value && frm.blnWeb.checked){
		  	alert("PC 이벤트 유형을 선택해 주세요");
		  	frm.pc_evttype.focus();
		  	return false;
	  	}
		//유형선택 여부 확인
	  	if(!frm.mo_evttype.value){
		  	alert("모바일 이벤트 유형을 선택해 주세요");
		  	frm.mo_evttype.focus();
		  	return false;
	  	}
...
//-- jsLastEvent : 지난 이벤트 불러오기 --//
    ...
    function jsChkSale(){
	    ...
	    if(( frm.eventkind.options[frm.eventkind.selectedIndex].value == 1 ||  frm.eventkind.options[frm.eventkind.selectedIndex].value == 23 )   && (document.frmEvt.chSale.checked)){
	...
	function TnCheckbrandType(type){
		if (type =="O"){
            document.all.btype1.style.display="";
			document.all.btype2.style.display="none";
        }else{
			document.all.btype1.style.display="none";
            document.all.btype2.style.display="";
	    }
	}
		...
		<table width="100%" border="0" align="left" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
		   ...
		   <!-- <tr>
		   	...
		   	<tr id="eNameTr_A">
		   		...
		   		<td bgcolor="#FFFFFF">
		   			<input type="text" name="sEN" id="sEN" size="50" maxlength="120" value="<%=ename%>">
		   		</td>
		   	</tr>
		   	...
		   	<tr>
				<td bgcolor="<%= adminColor("tabletop") %>">당첨 발표일<b style="color:red">*</b></td>
		   		<td  bgcolor="#FFFFFF">
		   			<input type="text" id="priceDt" name="sPD" size="10">
		   	...
		   	<tr id="spSale" style="display:<%if not ((ekind="1" or ekind="23") and (esale)) then %>none<%end if%>;<%if esale then%>color:red;<%end if%>">
				<td bgcolor="<%= adminColor("tabletop") %>"><font color="black">상품 할인율</font><b style="color:red">*</b></td>
		   		<td  bgcolor="#FFFFFF">
		   			<input type="text" name="sSP" value="<%=eSalePer%>" size="10" class="text" >(예:40%~)
		   		</td>
		   	</tr>
			<tr id="spCSale" style="display:<%if not ((ekind="1" or ekind="23") and (ecoupon)) then %>none<%end if%>;<%if ecoupon then%>color:green;<%end if%>">
				<td bgcolor="<%= adminColor("tabletop") %>"><font color="black">쿠폰 할인율</font><b style="color:red">*</b></td>
		   		<td  bgcolor="#FFFFFF">
		   			<input type="text" name="sCSP" value="<%=eSalePer%>" size="10" class="text" >(예:40%~)
		   		</td>
		   	</tr>
		</table>
	</td>
...
<tr>
	<td> 
		<table width="100%" border="0" align="left" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
			<tr>
		   	...
		   	<tr>
		   		...
		   		<td bgcolor="#FFFFFF">
		   		<!-- #include virtual="/common/module/dispCateSelectBoxDepth.asp"-->&nbsp;<input type="checkbox" name="nocate" id="nocate">카테고리 선택 안함
		   		</td>
		   	</tr>
			<tr>
		   		<td bgcolor="<%= adminColor("tabletop") %>">브랜드</td>
		   		<td bgcolor="#FFFFFF">
					<input type="radio" name="cbrand" value="O" onclick="TnCheckbrandType('O');" <%IF cbrand="O" THEN%>checked<%END IF%>>단일브랜드 <input type="radio" name="cbrand" value="M" onclick="TnCheckbrandType('M');" <%IF cbrand="M" THEN%>checked<%END IF%>>다중 브랜드
		   			<div id="btype1" style="display:"><% drawSelectBoxDesignerwithName "ebrand", ebrand %></div>
					<div id="btype2" style="display:none">*하단의 PC/Mobile Group 관리에서 브랜드를 설정해주세요.</div>
		   		</td>
		   	</tr>
		   	<tr>
		   		<td bgcolor="<%= adminColor("tabletop") %>">상품정렬방법</td>
		   		<td bgcolor="#FFFFFF"> 
		...
		<table width="100%" border="0" align="left" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
		   	<tr>
		   		<td width="100" bgcolor="<%= adminColor("tabletop") %>"><b>정렬번호</b></td>
		   		<td bgcolor="#FFFFFF">
	...
	<td> 
		<table width="100%" border="0" align="left" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
			...
			<!-- <tr>
		   	...
		   	<tr>    
		   		<td bgcolor="<%= adminColor("tabletop") %>">퍼블리싱</td>
				...
				<td valign="top">
					...
					<table width="100%" border="0" align="left" class="a" cellpadding="5" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
						...
						<tr id="pcevttype" style="display:">
					   		...
					   		<td bgcolor="#FFFFFF">
								<%sbGetOptEventCodeValue "pc_evttype","",True,""%>
					   		</td>
						</tr> 
						<tr>
						...
						<tr> 
							<td bgcolor="#FAECC5">이벤트명<b style="color:red">*</b></td>
					   		<td bgcolor="#FFFFFF">
								<input type="text" name="title_pc" id="title_pc" size="70" maxlength="64" value="">
					   		</td>
						</tr>
						<tr> 
							<td bgcolor="#FAECC5">서브카피<b style="color:red">*</b></td>
					   		<td bgcolor="#FFFFFF">  
					   			<textarea name="subcopyK" id="subcopyK" style="width:100%; height:80px;" onclick="if(this.value=='한글')this.value='';" onblur="if(this.value=='')this.value='한글';" value="<%=subcopyK%>"><%=chkiif(subcopyK="","한글",subcopyK)%></textarea>
				...
				<td  valign="top">
					...
					<table width="100%" border="0" align="left" class="a" cellpadding="5" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
						...
						<tr id="moevttype" style="display:">
					   		...
					   		<td bgcolor="#FFFFFF">
								<%sbGetOptEventCodeValue "mo_evttype","",True,""%>
					   		</td>
						</tr> 
						<tr>
							<td width="100" bgcolor="#e3f1fb">리스트 구성<b style="color:red">*</b></td>
							<td bgcolor="#FFFFFF">
						...
						<tr>
							<td bgcolor="#e3f1fb">이벤트명<b style="color:red">*</b></td>
					   		<td bgcolor="#FFFFFF">
								<input type="text" name="title_mo" id="title_mo" size="70" maxlength="64" value="">
					   		 </td>
					   	</tr> 
						<tr>
							<td bgcolor="#e3f1fb">서브카피<b style="color:red">*</b></td>
					   		<td bgcolor="#FFFFFF"> <input type="text" name="subsEN" id="subsEN" size="70" maxlength="70" value="<%=enamesub%>" OnKeyUp="jsAddByte(this);"> <input type="text" name="subSize" size="3" value="" class="text_ro" style="text-align:right" readonly>Byte 
...
<tr>
	<td width="100%" align="right" >
		<input type="image" src="/images/icon_save.gif" onClick="jsEvtSubmit(this.form);return false;">
