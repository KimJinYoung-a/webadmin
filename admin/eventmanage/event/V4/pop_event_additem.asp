...
function jsSerach(){
	...
	frm = document.frm;
	frm.target = "_self";
	frm.action ="pop_event_additem.asp";
	frm.submit();
}
...
function TnBannerSelectItemCode(itemid,itemname){
    $("#itemid<%=idx%>",opener.document).val(itemid);
    $("#itemname<%=idx%>",opener.document).val(itemname);
	$("#title<%=idx%>",opener.document).val(itemname);
    self.close();
}
...
<% if oitem.FresultCount > 0 then %>
	...
	<tr class="a" height="25" bgcolor="#FFFFFF" onClick="TnBannerSelectItemCode(<% =oitem.FItemList(i).Fitemid %>,'<% =oitem.FItemList(i).Fitemname %>');">
