...
Dim tmpType, endlessview, videoLink, videoFullLink
Dim eType, isConfirm, bannerTypeDiv, bannerCouponTxt, bannerGubun
...
Dim gift_img2, gift_text3, gift_img3, usinginfo, using_text1, using_contents1, using_text2, using_contents2, using_text3, using_contents3, upback, title_pc, title_mo, eventtype_pc, eventtype_mo
dim brandname, dgncomment, GroupItemCheck, GroupItemPriceView, vtemplateBannerCnt, ix, iconnew, iconbest, hvideotype, GroupItemType, brandid, tempSalePer, tempSaleCPer
dim eval_isusing, eval_text, eval_freebie_img, eval_start, eval_end

comm_isusing = requestCheckVar(Request.Form("comm_isusing"),1)
comm_text = html2db(Request.Form("comm_text"))
...
title_pc = html2db(Request.Form("title_pc"))
title_mo = html2db(Request.Form("title_mo"))
endlessview = requestCheckVar(Request.Form("endlessview"),1)
bannerTypeDiv = requestCheckVar(Request.Form("bannerTypeDiv"),1)
bannerCouponTxt = requestCheckVar(Request.Form("bannerCouponTxt"),16)
bannerGubun = requestCheckVar(Request.Form("bannerGubun"),1)
brandname = requestCheckVar(Request.Form("brandname"),32)
dgncomment = Request.Form("dgncomment")
...
if Request.Form("videoLink")<>"" then
videoLink = requestCheckVar(Request.Form("videoLink"),256)
end if
if Request.Form("videoLinkPC")<>"" then
videoLink = requestCheckVar(Request.Form("videoLinkPC"),256)
end if

vtemplateBannerCnt = Request.Form("bidx").Count
...
If eventtype_pc="" Then eventtype_pc=eType
If eventtype_mo="" Then eventtype_mo=eType
If eType="" Then eType=eventtype_pc
If eventtype_mo = "90" Then eType=eventtype_mo
 ...
 if isApp = "" then isApp = 0
	...
	eName 		= html2db(Request.Form("sEN"))
...
Case "I"
	...
	END IF
		...
		strSql = "INSERT INTO [db_event].[dbo].[tbl_event_display] "&_
				...
				"	,designerid2, dsn_state1, dsn_state2, mdtheme, mdthememo, themecolor, themecolormo, textbgcolor, textbgcolormo, mdbntype, mdbntypemo, SalePer, SaleCPer, endlessview, eventtype_pc, eventtype_mo, bannerTypeDiv, bannerCouponTxt, bannerGubun, videoLink, videoFullLink, videoType)" & vbCrlf&_
				...
				" ,'"&edgid2&"','"&edgstat1&"','"&edgstat2&"','"&mdtheme&"','"&mdthememo&"','"&DFcolorCD&"','"&DFcolorCDMo&"','"&DFcolorCD2&"','"&DFcolorCDMo2&"','"&bntype&"','"&bntypemo&"','" & eSalePer &"','" & eSaleCPer &"','" & endlessview &"','" & eventtype_pc &"','" & eventtype_mo &"','" & bannerTypeDiv &"','" & bannerCouponTxt &"','" & bannerGubun & "','" & videoLink &"','" & videoFullLink &"','" & hvideotype & "')"
		dbget.execute strSql
  ...
  '================ 이벤트 모바일 상품이벤트 =================
  '2015-11-04 이종화 추가
	If mdthememo="5" And (isMobile Or isApp) Then '상품이벤트
	...
	strSql = strSql& " VALUES("&eCode&", '"& evt_tagkind &"','"& evt_tagopt1 &"','"& etc_opt1 &"','"& etc_opt2 &"')"&vbCrlf 
	strSql = strSql& "end"
	dbget.execute strSql
		IF Err.Number <> 0 Then
			dbget.RollBackTrans 
...
CASE "U"
	...
	IF not (rsget.EOF or rsget.BOF) THEN 
			strSql = "UPDATE [db_event].[dbo].[tbl_event_display] SET "&_
					...
					"	,textbgcolor =  '"& DFcolorCD2 & "' , textbgcolormo =  '" & DFcolorCDMo2 & "', mdbntype='" & bntype & "', mdbntypemo='" & bntypemo & "', SalePer='" & eSalePer & "', SaleCPer='" & eSaleCPer & "', endlessview='" & endlessview & "'"&_
					"	, eventtype_pc='" & eventtype_pc & "', eventtype_mo='" & eventtype_mo & "',bannerTypeDiv =  '"& bannerTypeDiv & "' , bannerCouponTxt =  '" & bannerCouponTxt & "', bannerGubun='" & bannerGubun & "'"&_
					"   , videoLink='" & videoLink & "', videoFullLink='" & videoFullLink & "', videoType='" & hvideotype & "'" &_
					"	WHERE evt_code ="&eCode  
			dbget.execute strSql 
	END IF
	rsget.close
	...
	IF not (rsget.EOF or rsget.BOF) THEN 
			...
			strSql = strSql & " ,brandname='" & brandname & "'"
			strSql = strSql & " ,BrandContents='" & dgncomment & "'"
			If GroupItemPriceView <> "" Then
			strSql = strSql & " ,GroupItemPriceView='" & GroupItemPriceView & "'"
			End If
			If GroupItemCheck <> "" Then
			strSql = strSql & " ,GroupItemCheck='" & GroupItemCheck & "'"
			End If
			If GroupItemType <> "" Then
			strSql = strSql & " ,GroupItemType='" & GroupItemType & "'"
			End If
...
'	Response.end
	...
	End if 	 
 	'---------------------------------------------
	'H형 템플릿 이미지 수정
	if vtemplateBannerCnt > 1 then
		For ix = 0 To vtemplateBannerCnt-1
			strSql = "UPDATE [db_event].[dbo].[tbl_event_manual_group]"&vbCrlf
			strSql = strSql & " SET viewidx='" & Request.Form("viewidx")(ix+1) & "'" & vbCrlf
			strSql = strSql & " ,grouptype='" & GroupItemType & "'" & vbCrlf
			if Request.Form("title") <> "" then
			strSql = strSql & " ,title='" & Request.Form("title")(ix+1) & "'" & vbCrlf
			strSql = strSql & " ,groupcode='" & Request.Form("groupcode")(ix+1) & "'" & vbCrlf
			end if
			if Request.Form("itemid") <> "" then
			strSql = strSql & " ,itemid='" & Request.Form("itemid")(ix+1) & "'" & vbCrlf
			end if
			if Request.Form("itemname") <> "" then
			strSql = strSql & " ,itemname='" & Request.Form("itemname")(ix+1) & "'" & vbCrlf
			end if
			if Request.Form("iconnew")<>"" then
			strSql = strSql & " ,iconnew='" & Request.Form("iconnew")(ix+1) & "'" & vbCrlf
			end if
			if Request.Form("iconbest")<>"" then
			strSql = strSql & " ,iconbest='" & Request.Form("iconbest")(ix+1) & "'" & vbCrlf
			end if
			if Request.Form("imgurl") <> "" then
			strSql = strSql & " ,imgurl='" & Request.Form("imgurl")(ix+1) & "'" & vbCrlf
			end if
			if Request.Form("brandid")<>"" then
			strSql = strSql & " ,makerid='" & Request.Form("brandid")(ix+1) & "'" & vbCrlf
			end if
			strSql = strSql & "	WHERE evt_code = " & eCode & vbCrlf
			strSql = strSql & " and idx=" & Request.Form("bidx")(ix+1)
			dbget.execute strSql
		next
	end if
	...
	IF Err.Number = 0 THEN
		...
		IF 	(egift = 1 AND igiftcnt < 1) THEN	'사은품이벤트이나 사은품이 등록이 안된경우
			...
			ElseIf upback="T" Then
				Response.write "<script>parent.FnPCEventTypeChangeMove();</script>"
			Else
