...
<%
	...
	'신형 툴로 보내기
	response.redirect "http://webadmin.10x10.co.kr/admin/eventmanage/event/v5/?menupos=3953"
	response.end
...
''	edgnm2 		= requestCheckVar(Request("sdg2nm"),32)		'서브 디자이너
	...
	strparm  = "isWeb="&blnWeb&"&isMobile="&blnMobile&"&isApp="&blnApp&"&selDate="&sDate&"&iSD="&sSdate&"&iED="&sEdate&"&selEvt="&sEvt&"&sEtxt="&strTxt&"&selC="&sCategory&"&selCM="&sCateMid&"&eventstate="&sState&"&eventkind="&sKind&"&sDgId="&edgid&"&sMdId="&emdid&"&spsid="&epsid&"&sdpid="&edpid&_
				"&sdgnm="&edgnm&"&smdnm="&emdnm&"&spsnm="&epsnm&"&sdpnm="&edpnm&"&ebrand="&ebrand&"&chSale="&esale&"&chGift="&egift&"&chCoupon="&ecoupon&"&chOnlyTen="&eonlyten&"&disp="&dispCate&"&chDiary="&eDiary&"&sDg2Id="&edgid2&"&sdg2nm="&edgnm2&"&designerstatus="&edgstat1&"&designerstatus="&edgstat2&"&eventtype_pc="&etype_pc&"&eventtype_mo="&etype_mo
	'#######################################
...
<!-- 
	...
	function jsExcelDown(iCurrpage){
		...
		document.frmEvt.action="/admin/eventmanage/event/V4/event_excel.asp";
		document.frmEvt.submit();
	}
//-->
</script>
  	...
  	<tr align="center" bgcolor="#FFFFFF" >
		...
		<td align="left">
			...
			<tr>
				...
				<td style="border-top:1px solid <%= adminColor("tablebg") %>;">
						<%sbGetOptCommonCodeArr "eventtype_pc", etype_pc, True,True,"onChange='document.frmEvt.submit();'"%> &nbsp; 
						<%sbGetOptCommonCodeArr "eventtype_mo", etype_mo, True,True,"onChange='document.frmEvt.submit();'"%> &nbsp; 
...
'					sMoblie =  "<a href='" & mobileUrl & "/street/street_brandday.asp?eventid=" & arrList(0,intLoop) & "' target='_blank'>"  
    	...
    	<td><a href="event_modify.asp?eC=<%=arrList(0,intLoop)%>&menupos=<%=menupos%>&<%=strparm%>"><%=fnGetCommCodeArrDesc(arreventkind,arrList(1,intLoop))%></a></td>
    	<td>
			<a href="event_modify.asp?eC=<%=arrList(0,intLoop)%>&menupos=<%=menupos%>&<%=strparm%>">
			<% If arrList(45,intLoop)>0 Then %>
			PC:<%=fnGetCommCodeArrDesc(arreventtype,arrList(44,intLoop))%><br>
			M/A:<%=fnGetCommCodeArrDesc(arreventtype,arrList(45,intLoop))%>
			<% Else %>
