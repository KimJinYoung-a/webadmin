...
<%
dim eC, page, dispCate, itemname, i, stype, maxDepth, searchdiv, catesort, itemid, itemarr, GroupItemCheck
...
oitembanner.GetTrainThemeItemList
...
$(function(){
	$("#additem").click(function(){
		...
		if($("#items input:checkbox[name='cksel']:checked").length<1){
			...
			alert("같은 상품이 있습니다.");
			return false;
		}else if((Number($("#check").val())+Number($("#items input:checkbox[name='cksel']:checked").length))>8){
			alert("상품 배너 추가 갯수는 8개 입니다.");
			return false;
		}else{
			// 행추가
			var oRow;
