				...
				<div class="insertImg">
					<table class="tbType1 listTb">
						...
						<tr>
							...
							<td>
								<p>시작일 : <input type="text" onclick="jsPopCal('bst_date');" style="width:75px; cursor:pointer;" name="bst_date" readonly> ~ 종료일 : <input type="text" onclick="jsPopCal('bed_date');" style="width:75px; cursor:hand;" name="bed_date" readonly></p>
				...
				<div class="tMar20">
					<table class="tbType1 listTb">
						...
						<% 
							...
							sqlStr = "SELECT idx , gubun , bimg , btitle , blink , bdate_flag , bst_date , bed_date , isusing "
							sqlStr = sqlStr & " from db_event.dbo.tbl_event_pc_addbanner where evt_code = '"& eCode &"' "
							sqlStr = sqlStr & " order by gubun asc , idx asc " 
							rsget.Open sqlStr,dbget,1
							if Not(rsget.EOF or rsget.BOF) Then
								Do Until rsget.eof
						%>
						<tr class="<%=chkIIF(rsget("isusing")="N" Or (CStr(Date()) > CStr(rsget("bed_date"))),"bgGry1","")%>">
							...
							<td>
								<p>시작일 : <input type="text" onclick="jsPopCal_2('bst_date<%=rsget("idx")%>','bdate_flag<%=rsget("idx")%>');" value="<%=rsget("bst_date")%>" style="width:75px; cursor:pointer;" name="bst_date<%=rsget("idx")%>"> ~ 종료일 : <input type="text" onclick="jsPopCal_2('bed_date<%=rsget("idx")%>','bdate_flag<%=rsget("idx")%>');" value="<%=rsget("bed_date")%>" style="width:75px; cursor:pointer;" name="bed_date<%=rsget("idx")%>"></p>
