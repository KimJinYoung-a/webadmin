...
function selectThis(makerid, socname, dgncomment){
<% if idx>0 then %>
	$("#brandid<%=idx%>",opener.document).val(makerid);
    window.close();
<% else %>
    opener.<%= frmName %>.ebrand.value = makerid;
    opener.<%= frmName %>.brandname.value = socname;
    opener.<%= frmName %>.dgncomment.value = dgncomment;
    window.close();
<% end if %>
}
</script>
...
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	...
	<% end if %>
		...
		<td><a href="javascript:selectThis('<%= opartner.FPartnerList(i).Fid %>','<%= opartner.FPartnerList(i).Fsocname %>','<%= db2html(Replace(opartner.FPartnerList(i).Fdgncomment,"'","")) %>')"><%= opartner.FPartnerList(i).FID %></a></td>
		<td>
			<%= opartner.FPartnerList(i).FSocName_Kor %><br>
			<%= opartner.FPartnerList(i).FSocName %>
		</td>
		<td><a href="javascript:selectThis('<%= opartner.FPartnerList(i).Fid %>','<%= opartner.FPartnerList(i).Fsocname %>','<%= db2html(Replace(opartner.FPartnerList(i).Fdgncomment,"'","")) %>')"><%= opartner.FPartnerList(i).Fcompany_name %></a></td>
