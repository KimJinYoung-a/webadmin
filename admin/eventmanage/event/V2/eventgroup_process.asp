...
Dim eGIsDisp, vChangeContents, vSCMChangeSQL
...
CASE "I" 
		...
		END IF
    		...
    		rsget.Close
		 	 ...
		 	 strSql = "UPDATE db_event.dbo.tbl_eventitem_Group set evtgroup_code_mo = evtgroup_code where evt_code = "&eCode&" and evtgroup_code = "&evtgroup_code
	        dbget.execute strSql
...
CASE "U" 
     ...
     if eChannel = "P" then
		strSql = "UPDATE  [db_event].[dbo].[tbl_eventitem_group] SET evtgroup_desc ='"&eGDesc&"', evtgroup_sort='"&eGSort&"',"&_
					"	evtgroup_pcode = "&ePCode&" , evtgroup_isDisp ="&eGIsDisp&_
					" WHERE evtgroup_code ="&evtgroup_code	 	
	 else
	    strSql = "UPDATE  [db_event].[dbo].[tbl_eventitem_group] SET evtgroup_desc_mo ='"&eGDesc&"', evtgroup_sort_mo='"&eGSort&"',"&_
					"	evtgroup_pcode_mo = "&ePCode&" , evtgroup_isDisp_mo ="&eGIsDisp&_
					" WHERE evtgroup_code ="&evtgroup_code
	 end if															
		dbget.execute strSql
...
CASE "D" 
		...
		strSql = "UPDATE  [db_event].[dbo].[tbl_eventitem] SET evtitem_isusing = 0	"&_ 
					" WHERE evtgroup_code ="&evtgroup_code	 												
		dbget.execute strSql	
	    ...
	    IF isARRay(eGCodeArr) THEN
    	    For intLoop = 0 To UBound(eGCodeArr)
    	      ...
    	      if eChannel = "P" then
        	    ...
        	    strSql =  strSql&" where evtgroup_code = "&trim(eGCodeArr(intLoop))&"  and evt_code = "&eCode		'2017-03-27 김진영 위 주석 아래로 처리
...
Case "A"	 '전시설정
    if eChannel = "P" then
        strSql = " UPDATE [db_event].[dbo].[tbl_eventitem_group] set evtgroup_isDisp = "&eGIsDisp&" where  evtgroup_code = "&evtgroup_code& " and evt_code =" &eCode
        dbget.execute strSql 
	else
