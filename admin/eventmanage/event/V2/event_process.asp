...
Dim sPartnerid, eLinkCode, eCommentTitle,sOpenDate,sCloseDate, eItemListType, sImgregdate, eCCId
...
dim eSalePer , sgroup_m , sgroup_w
...
eSalePer = requestCheckVar(Request.Form("sSP"),20) 
...
endlessview = requestCheckVar(Request.Form("endlessview"),1)
 ...
 if isApp = "" then isApp = 0
    ...
    if ((eKind = "1" or  ekind="23" ) and (eSale = "1" or eCoupon="1") and (eSalePer <> "" or eSalePer <> "0" )) then eName = eName &"|"&eSalePer
...
Case "I"
	...
	END IF
		...
		strSql = "INSERT INTO [db_event].[dbo].[tbl_event_display] "&_
				...
				"	,designerid2, dsn_state1, dsn_state2, endlessview)" & vbCrlf&_
				...
				" ,'"&edgid2&"','"&edgstat1&"','"&edgstat2&"','" & endlessview&"')"
		dbget.execute strSql
...
CASE "U"
	...
	IF not (rsget.EOF or rsget.BOF) THEN 
			strSql = "UPDATE [db_event].[dbo].[tbl_event_display] SET "&_
					...
					"   ,designerid2 = '"& edgid2 &"' , dsn_state1 =  '"& edgstat1  &"' , dsn_state2 =  '"& edgstat2 & "', endlessview='" & endlessview & "'"&_
					"	WHERE evt_code ="&eCode  
