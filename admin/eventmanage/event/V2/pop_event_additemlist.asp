...
dim itemid, itemname, makerid, sellyn, usingyn, danjongyn, mwdiv, limityn, vatyn, sailyn,deliverytype, keyword,couponyn
...
<table width="100%" height="40" align="center" cellpadding="3" cellspacing="1" class="a" border="0">	
	<tr>
		<td  valign="bottom">		
				<select name="selGroup" class="select">		
					<option value="0"> 그룹 미지정 </option>
				<%IF isArray(arrGroup) THEN %>
				<%	
					For intLoop = 0 To UBound(arrGroup,2)
				%>
					<option value=" <%=arrGroup(0,intLoop)%>" <%if Cstr(egcode) = Cstr(arrGroup(0,intLoop)) then%>selected<%end if%> <%if not arrGroup(8,intLoop) then%>style="color:gray;"<%end if%>><%IF arrGroup(5,intLoop) <> 0 THEN%>└&nbsp;<%END IF%><%=arrGroup(0,intLoop)%>(<%=arrGroup(1,intLoop)%>) <%if not arrGroup(8,intLoop) then%> -[전시안함]<%end if%></option>
				<%	Next 
				END IF
				%>					    						    	
			 	</select>
