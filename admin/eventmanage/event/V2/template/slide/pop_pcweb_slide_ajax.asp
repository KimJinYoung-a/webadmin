...
<div class="<%=gubuncls%>">
	...
	<div class="evtSection swiper">
		<div class="swiper-container" <% If pcadd1 <>"" Then %>style="background-image:url(<%=pcadd1%>);"<% End If %> id="spanpcadd1_bg">
			...
			<% 
				...
				sqlStr = sqlStr & " from db_event.[dbo].[tbl_event_slide_addimage] where evt_code = '"& eCode &"' "
				sqlStr = sqlStr & " and isusing = 'Y' and device ='W' "
	...
	<div class="evtSection evtBtm" <% If btmaddimg <>"" Then %>style="background-image:url(<%=btmaddimg%>);"<% End If %> id="spanbtmaddimg_bg">
		<div id="spanbtmimg">
		<% If btmYN = "Y" Then %>
			<%IF btmimg <> "" THEN %>
			<img src="<%=btmimg%>" alt="" />
