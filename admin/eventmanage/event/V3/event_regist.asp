...
dim blnReqPublish, cbrand, tmp_cdl
 dim chkeCmt, chkeIps, chkeGf, chkeBS, catesort
eCode = Request("eC")
ekind = Request("eK")
...
blnIteminfo = True
	...
	strparm  = "selDate="&sDate&"&iSD="&sSdate&"&iED="&sEdate&"&selEvt="&sEvt&"&sEtxt="&strTxt&"&selC="&sCategory&"&selCM="&sCateMid&"&eventstate="&sState&"&eventkind="&sKind&"&sDgId="&edgid&"&sMdId="&emdid&"&sMdNm="&eMDnm&_
				"&ebrand="&ebrand&"&chSale="&esale&"&chGift="&egift&"&chCoupon="&ecoupon&"&chOneplusone="&eOneplusone&"&chFreedelivery="&eFreedelivery&"&chBookingsell="&eBookingsell&"&disp="&dispcate&"&chOnlyTen="&eonlyten&"&chDiary="&eDiary&"&catesort="&catesort
	'#######################################
...
//-- jsLastEvent : 지난 이벤트 불러오기 --//
	...
	function chkHtmlTag(o) {
		var v = o.value;
		if((/(<([^>]+)>)/ig).test(v)){
			alert("내용에 HTML TAG를 사용하실 수 없습니다..");
			o.value = v.replace(/(<([^>]+)>)/ig,""); 
		}
	}
		...
		<table width="100%" border="0" align="left" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
		   ...
		   <tr>
		   	...
		   	<tr id="eNameTr_A">
		   		...
		   		<td bgcolor="#FFFFFF">
		   			<input type="text" name="sEN" id="sEN" size="50" maxlength="120" value="<%=ename%>" OnKeyUp="chkHtmlTag(this);" />
		   		</td>
		   	</tr>
		   	...
		   	<tr id="spSale" style="display:<%if not ((ekind="1" or ekind="23") and (esale)) then %>none<%end if%>;<%if esale then%>color:red;<%end if%>">
				<td bgcolor="<%= adminColor("tabletop") %>"><font color="black">상품 할인율</font><b style="color:red">*</b></td>
		   		<td  bgcolor="#FFFFFF">
		   			~[<input type="text" name="sSP" id="sSP" value="<%=eSalePer%>" size="10" class="text" >]% (숫자만 입력 가능 특수문자 자동입력)
		   		</td>
		   	</tr>
			<tr id="spCSale" style="display:<%if not ((ekind="1" or ekind="23") and (ecoupon)) then %>none<%end if%>;<%if ecoupon then%>color:green;<%end if%>">
				<td bgcolor="<%= adminColor("tabletop") %>"><font color="black">쿠폰 할인율</font><b style="color:red">*</b></td>
		   		<td  bgcolor="#FFFFFF">
		   			~[<input type="text" name="sCSP" id="sCSP" value="<%=eSalePer%>" size="10" class="text" >]% (숫자만 입력 가능 특수문자 자동입력)
		   		</td>
		   	</tr>
		</table>
	</td>
					...
					<table width="100%" border="0" align="left" class="a" cellpadding="5" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
						...
						<tr> 
							<td bgcolor="#FAECC5">서브카피<b style="color:red">*</b></td>
					   		<td bgcolor="#FFFFFF">  
					   			<textarea name="subcopyK" style="width:100%; height:80px;" onclick="if(this.value=='한글')this.value='';" onblur="if(this.value=='')this.value='한글';" value="<%=subcopyK%>"><%=chkiif(subcopyK="","한글",subcopyK)%></textarea>
