...
<!-- #include virtual="/lib/classes/event/_eventManageCls_V3.asp"-->
...
<%
	...
	Dim sDate,sSdate,sEdate, sEvt,strTxt, sCategory, sCateMid ,sState,sKind,esale,egift,ecoupon,ebrand,eonlyten,etype,isConfirm,eMng
	Dim strparm
	...
	dim blnReqPublish ,sSort, chComm, chBbs, chItemps, isblogurl
	dim isResearch, mdtheme, catesort, dispcategroup

	response.write "<script>location.href='/admin/eventmanage/event/v4/'</script>"
...
''	edgnm2 		= requestCheckVar(Request("sdg2nm"),32)		'서브 디자이너
	...
	catesort       = requestCheckvar(request("catesort"),1)
	...
	strparm  = "isWeb="&blnWeb&"&isMobile="&blnMobile&"&isApp="&blnApp&"&selDate="&sDate&"&iSD="&sSdate&"&iED="&sEdate&"&selEvt="&sEvt&"&sEtxt="&strTxt&"&selC="&sCategory&"&selCM="&sCateMid&"&eventstate="&sState&"&eventkind="&sKind&"&sDgId="&edgid&"&sMdId="&emdid&"&spsid="&epsid&"&sdpid="&edpid&_
				"&sdgnm="&edgnm&"&smdnm="&emdnm&"&spsnm="&epsnm&"&sdpnm="&edpnm&"&ebrand="&ebrand&"&chSale="&esale&"&chGift="&egift&"&chCoupon="&ecoupon&"&chOnlyTen="&eonlyten&"&disp="&dispCate&"&chDiary="&eDiary&"&sDg2Id="&edgid2&"&sdg2nm="&edgnm2&"&designerstatus="&edgstat1&"&designerstatus="&edgstat2&"&eventtype="&etype&"&catesort="&catesort
	'#######################################
 	if sSort = "" then sSort = "CD"
 	if blnReqPublish= "" then blnReqPublish = False     
 	    
	If catesort="1" Then
		dispcategroup="103,104,121,122,120,112,119,115,110"
	ElseIf catesort="2" Then
		dispcategroup="101,102,124,117,116,125,118"
	Else
		dispcategroup=""
	End If
	'데이터 가져오기
...
''		cEvtList.FSedid2   	= edgid2
		...
		cEvtList.FDispCateGroup=dispcategroup
...
<!-- 
	...
	function fnSelectPart(objvalue){
		document.frmEvt.submit();
	}
	//20181105 멀티3번 최종원
  	...
  	<tr align="center" bgcolor="#FFFFFF" >
		...
		<td align="left">
			...
			<tr>
				...
				<td colspan="4">
					전시 카테고리 : <select name="catesort" class="select" onchange="fnSelectPart(this.value);">
					<option value="">선택</option>
					<option value="1"<% If catesort="1" Then Response.write " selected"%>>1파트</option>
					<option value="2"<% If catesort="2" Then Response.write " selected"%>>2파트</option>
					</select>
			...
			<tr>
				...
				<td style="border-top:1px solid <%= adminColor("tablebg") %>;">
						<%sbGetOptCommonCodeArr "eventtype", eType, True,True,"onChange='document.frmEvt.submit();'"%> &nbsp; 
						<input type="checkbox" name="blnCnfm" <%=chkIIF(Cstr(isConfirm)="1","checked","")%> value="1">승인완료
				</td>
				<td style="border-top:1px solid <%= adminColor("tablebg") %>;">이벤트 테마:</td>
				<td style="border-top:1px solid <%= adminColor("tablebg") %>;">
					<select name="mdtheme" class="select">
						<option value="" selected>선택</option>
						<option value="2"<% If mdtheme="2" Then Response.write " selected"%>>이미지</option>
						<option value="3"<% If mdtheme="3" Then Response.write " selected"%>>상품</option>
						<option value="1"<% If mdtheme="1" Then Response.write " selected"%>>텍스트</option>
					</select>
				</td>
			...
			<tr>
				...
				<td colspan="2" style="border-top:1px solid <%= adminColor("tablebg") %>;">
				이벤트 기능 : 
				<input type="checkbox" name="chComm"<% If chComm="1" Then Response.write " checked"%> value="1">코멘트
		   		<input type="checkbox" name="chBbs"<% If chBbs="1" Then Response.write " checked"%> value="1">게시판
		   		<input type="checkbox" name="chItemps"<% If chItemps="1" Then Response.write " checked"%> value="1">상품후기
		   		<input type="checkbox" name="isblogurl"<% If isblogurl="1" Then Response.write " checked"%> value="1">Blog URL
				</td>
			</tr>
			<tr>
			...
			<tr> 
				...
				<td style="border-top:1px solid <%= adminColor("tablebg") %>;" colspan="5"><%sbGetOptCommonCodeArr "eventmanager", eMng, True,True,"onChange='document.frmEvt.submit();'"%> &nbsp; </td>
			</tr>
			</table>
        </td>
...
<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a"  >
    <tr height="40" valign="bottom">
        <td align="left">
        	<input type="button" value="새로등록" onclick="jsGoUrl('/admin/eventmanage/event/V4/event_regist.asp?menupos=<%=menupos%>&<%=strParm%>');" class="button">
	    </td>
	    <td align="right">
...
<table width="100%" border="0" align="center" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	<tr bgcolor="#FFFFFF" height="25">
		<td colspan="21">검색결과 : <b><%=iTotCnt%></b>&nbsp;&nbsp;페이지 : <b><%=iCurrpage%> / <%=iTotalPage%></b></td>
	</tr>
...
'					Response.Write "<td><a href='" & strURL & "/street/street_brandday.asp?eventid=" & arrList(0,intLoop) & "' target='_blank'>" & arrList(0,intLoop) & "</a></td>"
				...
				Case "26"		'모바일
					Response.Write "<td><a href=""javascript:jsOpen('" & strURL & "/event/eventmain.asp?eventid=" &  arrList(0,intLoop) & "','M');"" target='_blank'>" & arrList(0,intLoop) & "</a></td>"
				Case Else		'쇼핑찬스 및 기타
