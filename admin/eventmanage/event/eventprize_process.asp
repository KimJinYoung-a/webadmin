...
sType 		= Request.Form("evtprizetype")
...
'--------------------------------------------------------
   ...
   '기본
	...
	If Right(Request.Form("sW"),1) = "," Then
	...
	dAStartDate = Request.Form("dASDate")
	dAEndDate 	= Request.Form("dAEDate")
	If sType = "1" Then
	...
	'당첨자 메지시
	chkSms = request("chkSMS")
	smsCont = request("smsCont")
	...
	vChangeContents = vChangeContents & "- 당첨자 = " & Request.Form("sW") & vbCrLf
	...
	'STAFF 제외 처리
	if date>="2022-07-01" and sType<>"5" then		'22년 7월부터 STAFF검사, 테스터 제외
		For oo = 0 to UBound(arrwinner)
			if fnIsStaffUser(html2db(Trim(arrwinner(oo)))) then
				'### 수정 로그 저장(event)
				vChangeContents = "- 이벤트(" & eCode & ") 당첨자 등록 중 STAFF 존재." & vbCrLf
				vChangeContents = vChangeContents & "- 구분 = " & sType & ", 등수 = " & iranking & ", 등수별칭 = " & srankname & vbCrLf
				vChangeContents = vChangeContents & "- STAFF ID = " & Trim(arrwinner(oo)) & vbCrLf
				vSCMChangeSQL = "INSERT INTO [db_log].[dbo].[tbl_scm_change_log](userid, gubun, pk_idx, menupos, contents, refip) "
				vSCMChangeSQL = vSCMChangeSQL & "VALUES('" & session("ssBctId") & "', 'event', '" & eCode & "', '9079', "
				vSCMChangeSQL = vSCMChangeSQL & "'" & html2db(vChangeContents) & "', '" & Request.ServerVariables("REMOTE_ADDR") & "')"
				dbget.execute(vSCMChangeSQL)

				Call sbAlertMsg ("직원 아이디는 등록할 수 없습니다. 다시 입력하세요", "back", "")
...
'------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	'### STAFF 여부 확인 ###################
	Function fnIsStaffUser(ByVal userid)
		strSql = "SELECT count(*) FROM db_user.dbo.tbl_logindata WHERE userlevel=7 AND userid='" & userid & "'"
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly, adCmdText

		'결과 반환
		fnIsStaffUser = rsget(0)>0
