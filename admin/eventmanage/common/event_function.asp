...
<%
 	For intLoop =0 To UBound(arrList,2)
		if arrList(1,intLoop) <> "150px" then
%>
	<option value="<%=arrList(0,intLoop)%>" <%If CStr(selValue) = CStr(arrList(0,intLoop)) THEN%>selected<%END IF%>>
	<%
		If arrList(1,intLoop) = "100px" Then
		...
		ElseIf arrList(1,intLoop) = "150px" Then
			Response.Write "180px"
		ElseIf arrList(1,intLoop) = "153px" Then'4칸
			Response.Write "240px"
		ElseIf arrList(1,intLoop) = "200px" Then'2칸
			Response.Write "400px"
		ElseIf arrList(1,intLoop) = "155px" Then'3칸
			Response.Write "270px"
		ElseIf arrList(1,intLoop) = "160px" Then'3칸
			Response.Write "320px"
		Else
			Response.Write arrList(1,intLoop)
		End If
...
'-----------------------------------------------------------------------
 Sub sbGetMKTid(ByVal selName, ByVal sIDValue, ByVal sScript)
  Dim strSql, arrList, intLoop
   strSql = " SELECT userid, username"
   strSql = strSql & " FROM db_partner.dbo.tbl_user_tenbyten "
   strSql = strSql & " WHERE part_sn in('11','14')  and   isUsing=1  and (statediv ='Y' or (statediv ='N' and datediff(dd,retireday,getdate())<=0)) and userid <> '' order by part_sn, posit_sn, empno"	' and  posit_sn>='4' and  posit_sn<='8'
...
<%			End If
  ...
  '-----------------------------------------------------------------------
' 17. sbGetMKTid2 :담당MKT,MD 리스트가져오기 (MKT 강수영 추가)
' 2017.06.05 김진영 생성
'-----------------------------------------------------------------------
 Sub sbGetMKTid2(ByVal selName, ByVal sIDValue, ByVal sScript)
  Dim strSql, arrList, intLoop
   strSql = " SELECT userid, username"
   strSql = strSql & " FROM db_partner.dbo.tbl_user_tenbyten "
   strSql = strSql & " WHERE part_sn in('11','14') and  ((posit_sn>='4' and  posit_sn<='8') OR userid = 'ksy92630' ) and   isUsing=1  and (statediv ='Y' or (statediv ='N' and datediff(dd,retireday,getdate())<=0)) and userid <> '' order by part_sn, posit_sn, empno"
