...
'-----------------------------------------------------------------------
 Sub sbGetDesignerid(ByVal selName, ByVal sIDValue, ByVal sScript)
  Dim strSql, arrList, intLoop
   strSql = "SELECT t.userid, t.username, t.posit_sn, t.empno from " 
   strSql = strSql & "(SELECT userid,  username, posit_sn, empno"
   strSql = strSql & "  from db_partner.dbo.tbl_user_tenbyten with (noLock) "
   strSql = strSql & " WHERE part_sn ='12' and isUsing=1"
   strSql = strSql & " and (statediv ='Y' or (statediv ='N' and datediff(dd,retireday,getdate())<=0)) and userid <> ''"
   strSql = strSql & " UNION"
   strSql = strSql & " SELECT userid,  username, posit_sn, empno"
   strSql = strSql & "  from db_partner.dbo.tbl_user_tenbyten with (noLock) "
   strSql = strSql & " WHERE isUsing=1 and userid in ('cunhng','sehee0226')"
   strSql = strSql & " and (statediv ='Y' or (statediv ='N' and datediff(dd,retireday,getdate())<=0)) and userid <> '') as t"
   strSql = strSql & " order by t.posit_sn, t.empno"
   ...
   rsget.close
%>
	<select name="<%=selName%>" <%=sScript%> class="formControl">
...
%>
	<select name="<%=sType%>" id="<%=sType%>" <%=sScript%> class="formControl">
	<%IF sViewOpt THEN%>
	<option value="">선택</option>
    <%END IF%>
<%
...
'--------------------------------------------------------------------------------
 Sub sbGetOptGiftCodeValue(ByVal sType, ByVal selValue, ByVal sViewOpt, ByVal sScript,ByVal eCode)
   Dim arrList, intLoop
 	...
 	IF  isNull(selValue) THEN selValue = ""
%>
    <select name="<%=sType%>" class="formControl" <%=sScript%>>
	<% if selValue="1" then %>
...
%>
	<select name="<%=sType%>" <%=sScript%> class="formControl">
	<%IF sViewOpt THEN%>
	<option value="">선택</option>
    <%END IF%>
<%
...
%>
	...
	<option value="">선택</option>
    <%END IF%>
	<% if sType="eventstate" and (session("ssBctId")="violetiris" or C_MD_AUTH or C_ADMIN_AUTH) and selValue="1" then %>
	<option value="0">등록대기</option>
	<option value="3">이미지등록요청</option>
	<% end if %>
<%
	if isarray(arrList) then
 	For intLoop =0 To UBound(arrList,2) 
...
' 2019.05.10 
'---------------------------------------------------------------------------------
Sub getEventKindList(ByVal ekind)
    dim arrEvtkinds : arrEvtkinds = array("1","13","16","5","28","12","31","33","23","32","17")
    dim arrEvtkindTexts : arrEvtkindTexts = array("쇼핑찬스","상품이벤트","브랜드이벤트","컬쳐스테이션","마케팅이벤트","SNS/PLAY/HITCHHIKER","BRAND WEEK","월페이퍼","테스터이벤트","패션매거진","다이어리이벤트")
    dim i
    dim tempHtml

    For i = 0 To ubound(arrEvtkinds)
        tempHtml = tempHtml & "<div class='formInline'>"
        tempHtml = tempHtml & "<label class='formCheckLabel'>"
        tempHtml = tempHtml & "<input type='radio' class='formCheckInput' name='eventkind' value='"&arrEvtkinds(i)&"' "& chkiif(Cstr(ekind) = Cstr(arrEvtkinds(i)) , " checked" , "") &" onclick='fnItemEventSet(this.value);'>"
        tempHtml = tempHtml & arrEvtkindTexts(i)
        tempHtml = tempHtml & "<i class='inputHelper'></i>"
...
'-----------------------------------------------------------------------
' 1. sbGetPublisherid :웹디자인팀 부서번호(12)로 디자이너이름 리스트가져오기
' 2007.02.07 정윤정 생성
'2011.01.18 정윤정 수정 : 부서정보 테이블변경(tbl_partner -> tbl_user_tenbyten)
'-----------------------------------------------------------------------
 Sub sbGetPublisherid(ByVal selName, ByVal sIDValue, ByVal sScript)
  Dim strSql, arrList, intLoop
   strSql = " SELECT userid,  username"
   strSql = strSql & "  from db_partner.dbo.tbl_user_tenbyten with (noLock) "
   strSql = strSql & " WHERE part_sn ='7' and posit_sn='12' and isUsing=1 and (statediv ='Y' or (statediv ='N' and datediff(dd,retireday,getdate())<=0)) and userid <> ''  order by posit_sn, empno"
...
'####################################################
' Page : /lib/event_function.asp
' Description :  이벤트 이미지 복사하기용 권한 설정(임시)
' History : 2019.09.11 정태훈
'####################################################
'
'-----------------------------------------------------------------------
Function fnCheckAuthImageCopy(ByVal AdminID)
	Dim strSql, arrList, intLoop
	strSql = " SELECT userid,  username"
	strSql = strSql & "  from db_partner.dbo.tbl_user_tenbyten with (noLock) "
	strSql = strSql & " WHERE part_sn ='12' and isUsing=1 and (statediv ='Y' or (statediv ='N' and datediff(dd,retireday,getdate())<=0)) and userid='" & AdminID & "'"

	rsget.Open strSql,dbget
	IF not rsget.eof THEN
		fnCheckAuthImageCopy = true
	else
		if C_MD_AUTH or C_ADMIN_AUTH then
			fnCheckAuthImageCopy = true
		else
			fnCheckAuthImageCopy = false
		end if
	End IF
	rsget.close
End Function
...
'--------------------------------------------------------------------------------
' 14.sbGetNewOptCommonCodeArr(변수명, 선택값, 선택구문,  사용유무, 전시유무, 스크립트)
' : 특정종류의 공통코드값의 배열에서 select 처리
' 2019.09.30 정태훈 생성 (기존 함수에 전시 유무 선택 포함)
'--------------------------------------------------------------------------------
Sub sbGetNewOptCommonCodeArr(ByVal sType, ByVal selValue, ByVal sViewOpt, ByVal blnUse, ByVal dispYn, ByVal sScript)
   Dim arrCode, intLoop, iValue
   	arrCode= fnSetNewCommonCodeArr(sType, blnUse, dispYn)
 	iValue  = selValue
 	IF  isNull(selValue) THEN 	selValue = ""
 	IF selValue = ""  THEN	iValue = 0
%>
...
' 2019.09.30 정태훈 생성 (기존 함수에 전시 유무 선택 포함)
'---------------------------------------------------------------------------------
Sub getNewEventKindList(ByVal sType, ByVal selValue, ByVal blnUse, ByVal dispYn)
   Dim arrCode, i, tempHtml
   	arrCode= fnSetNewCommonCodeArr(sType, blnUse, dispYn)
 	IF  isNull(selValue) THEN 	selValue = ""
	tempHtml=""
	IF isArray(arrCode) THEN
		For i = 0 To UBound(arrCode,2)
			tempHtml = tempHtml & "<div class='formInline'>" & vbcrlf
			tempHtml = tempHtml & "<label class='formCheckLabel'>" & vbcrlf
			tempHtml = tempHtml & "<input type='radio' class='formCheckInput' name='eventkind' value='"&arrCode(0,i)&"' "& chkiif(Cstr(selValue) = Cstr(arrCode(0,i)) , " checked" , "") &" onclick='fnItemEventSet(this.value);'>" & vbcrlf
			tempHtml = tempHtml & arrCode(1,i) & vbcrlf
			tempHtml = tempHtml & "<i class='inputHelper'></i>" & vbcrlf
			tempHtml = tempHtml & "</label>" & vbcrlf
			tempHtml = tempHtml & "</div>" & vbcrlf
		Next
	End IF
    response.write tempHtml
End Sub
...
'-----------------------------------------------------------------------
' fnSetNewCommonCodeArr : 이벤트 공통코드 가져오기
' 2019.09.30 정태훈 생성 (기존 함수에 전시 유무 선택 포함)
'-----------------------------------------------------------------------
 Function fnSetNewCommonCodeArr(ByVal code_type, ByVal blnUse, ByVal dispYn)
	Dim strSql, arrList, intLoop, strAdd
	Dim intI, intJ, arrCode(), strtype
	strAdd = ""
	IF blnUse THEN
		strAdd=strAdd & " and code_using ='Y'"
	END IF
	IF dispYn THEN
		strAdd=strAdd & " and code_dispYN ='Y'"
	END IF
	strSql = " SELECT code_value, code_desc,code_dispYN FROM [db_event].[dbo].[tbl_event_commoncode] WHERE code_type='"&code_type&"'" & strAdd&_
			" Order by code_dispYN desc, code_type, code_sort "
	rsget.Open strSql,dbget
	IF not rsget.EOF THEN
		fnSetNewCommonCodeArr = rsget.getRows()
	END IF
	rsget.close
End Function
%>