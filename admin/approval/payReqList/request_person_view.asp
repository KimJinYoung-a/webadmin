...
Dim ipayDocIdx,ipaydockind,svatkind,dissuedate,sitemname,mtotprice,msupplyprice,mvatprice,setaxkey,sDocbigo,sattachfile, erpDocLinkType, erpDocLinkKey
...
set clsPay = new CPayRequest
 ...
 '//증빙서류
  clsPay.fnGetEappPayDoc 
	...
	erpDocLinkType = clsPay.FerpDocLinkType
	erpDocLinkKey = clsPay.FerpDocLinkKey
...
<tr>
	<td>
		...
		<input type="hidden" name="returnurl" value="/admin/approval/payReqList/request_person_view.asp">
		...
		<tr>
			<td> 
				...
				<tr align="center">	
					...
					<td bgcolor="#FFFFFF"><a href="javascript:jsPopView('/admin/approval/eapp/confirmpayrequest.asp?iridx=<%=ireportidx%>&ipridx=<%=arrProc(0,intP)%>&ias=<%=arrProc(5,intP)%>');">상세보기 >></a></td>
				</tr>
				<%Next%>
				</table>
			</td>
					...
					<td>
						...
						<%
							If ipaytype = "1" Then
								Response.Write "외화금액: "
								IF blnMod=0 THEN
									Response.Write sCurrencyType & "&nbsp;" & sCurrencyPrice
								ELSE
									rsget.Open " select currencyUnit ,exchangeRate ,basedate ,currencyChar from db_shop.dbo.tbl_shop_exchangeRate where currencyUnit = '" & sCurrencyType & "'",dbget,1
									Response.Write rsget("currencyUnit") & "[" & rsget("currencyChar") & "]"
									rsget.close()
								END IF
							End IF
						%>
		...
		<tr>
			<td align="right" style="padding:7px 4px 7px 4px;">
			<% If isNull(erpDocLinkKey) = True OR erpDocLinkKey = "" Then %>
			<input type="button" value="증빙서류 내용수정" style="color:blue;" class="button" onClick="jsPayDocUpdate();">
			<% Else %>
			<b>※ 이미 ERP로 처리된 결제입니다.</b>
			<% End If %>
			</td>
		</tr>
		</table>
...
<tr>
	<td style="padding:2px 4px 7px 4px;">
		<table width="100%" align="left" cellpadding="5" cellspacing="1" class="a" border="0" bgcolor=#BABABA>
		<tr bgcolor="<%= adminColor("tabletop") %>" align="center">
...
<tr>
	<td style="padding:1px 4px 7px 4px;">
		<table width="100%" align="left" cellpadding="5" cellspacing="1" class="a" border="0" bgcolor=#BABABA>
		<tr bgcolor="<%= adminColor("tabletop") %>" align="center">
...
<tr>
	<td style="padding:1px 4px 7px 4px;">
		...
		<tr>
			...
			<%ELSE%>
			<table border="0" cellpadding="3" cellspacing="0" class="a" width="760"><tr><td>&nbsp;</td></tr></table>
			<%END IF%>
			</div>
			</td>
		</tr> 
