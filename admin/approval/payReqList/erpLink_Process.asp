...
IF (LTp="A") THEN
  For i=UBound(chk) to LBound(chk) STEP -1   ''역으로 등록.
    ...
    IF (payIdx<>"") Then
    	...
    	sqlStr = "db_SCM_LINK.dbo.sp_SCM2ERP_payREQ" 
...
ELSEIF (LTp="AF") THEN          '''입금확정 File - 업체 정기결제.
    IF (ipFileNo<>"") then
    	...
    	sqlStr = "db_SCM_LINK.dbo.sp_SCM2ERP_ICheFile" 
        ...
        IF application("Svr_Info")="Dev" THEN
            sqlStr ="db_SCM_LINK.dbo.sp_ERP_RESULT_BY_LINKTYPE_TEST (4,"&RetErr&") "
        ELSE
            sqlStr ="db_SCM_LINK.dbo.sp_ERP_RESULT_BY_LINKTYPE (4,"&RetErr&") "
        END IF
...
ELSEIF (LTp="C") THEN                                           ''''운영비 전송.
  For i=UBound(chk) to LBound(chk) STEP -1   ''역으로 등록.
    ...
    IF (payIdx<>"") Then
    	...
    	sqlStr = "db_SCM_LINK.dbo.sp_SCM2ERP_SLMAST_INPUT" 
        ...
        ELSE
            ...
            ELSE
                sqlStr ="db_SCM_LINK.dbo.sp_ERP_RESULT_BY_LINKTYPE (3,"&RetErr&") "
            END IF
...
ELSEIF (LTp="D") THEN                                           ''''체크카드내역 전송.
  For i=UBound(chk) to LBound(chk) STEP -1   ''역으로 등록.
    ...
    IF (payIdx<>"") Then
    	...
    	sqlStr = "db_SCM_LINK.dbo.sp_SCM2ERP_SLMAST_INPUT_CARD" 
