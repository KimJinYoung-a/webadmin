...
<script type="text/javascript">
	...
	//등록처리 
	function jsSetId(sUserID, sUserNm){
		opener.eval("document.frmReg.sId").value = sUserID;
		opener.eval("document.frmReg.sNm").value = sUserNm;
		self.close();
	}
		...
		<tr>
			<td><%dim cid1, cid2, cid3, cid4, oldcid1, oldcid2, oldcid3, oldcid4 %> 
				...
				<% if cid4  <>  oldcid4 and cid4 <> 0 then	 
					...
					<%end if%>	 
						<div id="divU" style="padding:0 0 0 1;background:white;" onClick="jsSetId('<%=arrList(4,intLoop)%>','<%=arrList(5,intLoop)%>');">
