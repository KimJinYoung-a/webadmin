...
Dim blnMod
Dim mSumPrice, mSumRealPrice,soutBankName
...
''IF serpLinkType = "S" THEN blnMod = 0   '' 증빙 전송 후면 수정 불가.

if Not (C_ADMIN_AUTH or C_MngPart or C_PSMngPart) then blnMod = 0
%>
<html>
<head>
...
<script type="text/javascript">
	...
	if(ichkVal == 1 || ichkVal == 2){
 	...
 	//경영지원관리항목 수정
 	function jsFSUpdate(){
 			if(jsChkBlank(document.frmFS.dPD.value) ){
			alert("결제예정일을 입력해주세요");
			return;
		}

		//if(jsChkBlank(document.frmFS.selOB.value) ){
		//	alert("출금은행을 선택해주세요");
		//	return;
		//}
		...
		{
			document.frmFS.rdoTD[0].checked = true;
		}

        if ((document.frmFS.frcfin)&&(document.frmFS.frcfin.checked)&&(document.frmFS.dprld.value.length<1)){
            alert('강제 완료 하시려면 결제(입금)일을 입력하세요.');
            return;
        }
        
 		 if(confirm("수정  하시겠습니까?")){
 		document.frmFS.submit();
 	}
 	}

 	function jsEditPayType(comp){
 	    var frm = comp.form;
 	    frm.hidM.value="DP"
 	    if(confirm("결제방법을 수정  하시겠습니까?")){
     		document.frm.submit();
     	}
 	}

    function jsEditReqtitle(comp){
        var frm = comp.form;
 	    frm.hidM.value="ET"
 	    if(confirm("자금용도명칭을 수정  하시겠습니까?")){
     		document.frm.submit();
     	}
    }

 	function jsEditPayState(comp){
 	    var frm = comp.form;
 	    frm.hidM.value="SC"
 	...
 	function jsDelPayReq(comp){
 	    var frm = comp.form;
 	    frm.hidM.value="SD"
 	    if(confirm("결제 요청서를 삭제  하시겠습니까?")){
     		document.frm.submit();
     	}
 	}

	function jsPayARAPUpdate(){
		if(!document.frm.iaidx.value) {
 			alert("수지항목을 선택해주세요");
 			return;			
		}

 	    document.frm.hidM.value="AU";
 	    if(confirm("수지항목을 적용하시겠습니까?")){
     		document.frm.submit();
     	}

	}

 	function jsPayAnbunUpdate(){
 	    var mTotPrice = 0;
	...
	{
		if (dType == 0){
		document.frmFS.dPD.value = "<%=date()%>";
		}else{
		document.frmFS.dPD.value = '<%=dateadd("d",date(),1)%>';	
		}
	} 
...
<tr>
	<td>
		...
		<tr>
			<td>
				...
				<tr>
					...
					<% if (ipayrequeststate="5") then %> <!-- 반려인경우 삭제 가능 -->
					<input type="button" value="삭제" class="button" style="color:blue;" onClick="jsDelPayReq(this)">
					<% end if %>
					</td>
				</tr>
				</table>
		...
		<tr>
			<td>
				...
				<tr align="center"  bgcolor="#FFFFFF">
					...
					<td>
					    <% if(ipayrequeststate=9) then %>
					    <%=fnGetPayType(ipaytype)%>
					    <% else %>
					    <select name="selPT" onChange="jsChFC();" class="select">
					    <% CALL sboptPayType(ipaytype)%>
					    </select>
					    <input type="button" value="수정" class="button" style="color:blue;" onClick="jsEditPayType(this)">
					    <% end if %>
					</td>
					<td>
				...
				<tr>
					...
					<% if blnMod=1 then %>
					<input type="text" id="sprt" name="sPayrequesttitle" value="<%=sPayrequesttitle%>" size="50" class="text">
					<input type="button" value="수정" class="button" style="color:blue;" onClick="jsEditReqtitle(this)">
					<% else %>
					<%=sPayrequesttitle%>
					<% end if %>
					</td>
		...
		<%IF blnMod = 1 THEN%>
		<tr>
			<td align="right"  bgcolor="#FFFFFF" ><input type="button" value="수지항목 적용" style="color:blue;" class="button" onClick="jsPayARAPUpdate();"></td>
		</tr>
		<% end if %>
		<tr>
			<td>
		...
		<%IF blnMod = 1 THEN%>
		<tr>
			<td align="right"  bgcolor="#FFFFFF" ><input type="button" value="부서별 안분비율 수정" style="color:blue;" class="button" onClick="jsPayAnbunUpdate();"></td>
		</tr>
		<% end if %>
		</form>
		...
		<tr>
			<td>
				...
				<tr bgcolor="#FFFFFF" align="center">
					<td><input type="text" name="dPD" size="10" value="<%IF dpaydate <> "" THEN%><%=formatdate(dpaydate,"0000-00-00")%><%END IF%>" size="10" ><img src="/images/calicon.gif" align="absmiddle" border="0" onClick="jsPopCal('dPD');"  style="cursor:hand;">
					...
					<%Dim intY, intM%>
					<td><input type="text" name="dprld" size="10" value="<%IF dpayrealdate <> "" THEN%><%=formatdate(dpayrealdate,"0000-00-00")%><%END IF%>" size="10" ><img src="/images/calicon.gif" align="absmiddle" border="0" onClick="jsPopCal('dprld');"  style="cursor:hand;"></td>
		...
		<tr>
			<td align="right">
			<% if (ipayrequeststate="8") then %>
			    <input type="checkbox" name="frcfin" >강제완료 변경 &nbsp;
			<% end if %>
			<input type="button" value="경영지원관리항목 내용수정" style="color:blue;" class="button" onClick="jsFSUpdate();"></td>
		</tr>
		</table>
	</form>
	</td>
