...
<%
Dim clseapp
Dim sadminId,ireportState , iAuthState,ireportidx
...
set clseapp = new CEApproval
	...
	arrList = clseapp.fnGetEAppReceiveList
...
<!--  
	function jsConfirm(iridx){
		top.eappDetail.location.href = "confirmeapp.asp?iridx="+iridx+"&iRM=<%=iRectMenu%>";
	}
...
<tr> 
	<td valign="top">
		...
		<tr> 
			<td height="25"><font color="#4E9FC6"><b>받은결재함> 결재문서<%IF ireportState > 0 THEN%>> <%=fnGetMenu("R1",ireportState,iAuthState)%><%END IF%></b></font></td>
	 	</tr>
		<tr>
			<td>
		...
		<tr>
			<td> 
		 	<!----------------- 리스트 --------------------------->  
				...
				%>
				<tr id="t<%=arrList(0,intLoop)%>" bgcolor="#FFFFFF" align="center" onclick="jsConfirm(<%=arrList(0,intLoop)%>);ChangeColor(this,'#CEF6EC','FFFFFF');" style="cursor:hand;"> 
					<td><%=arrList(0,intLoop)%></a></td> 
					<td><%=arrList(1,intLoop)%></td>
					<td nowrap><%=formatnumber(arrList(2,intLoop),0)%></td> 
					<td nowrap><%=formatdate(arrList(5,intLoop),"0000-00-00")%></td>
					<td nowrap><%=arrList(10,intLoop)%></td> 
				</tr>
				<%	
					Next
					ELSE	
