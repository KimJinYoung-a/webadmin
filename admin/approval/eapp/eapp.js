...
//파일 다운로드(문서양식)
function jsEdmsDownload(sDownURL, sRFN, sFN){
var winFD = window.open(sDownURL+"/linkweb/edms/procDownload.asp?sRFN="+sRFN+"&sFN="+sFN,"popFD","");
winFD.focus();
}
...
function jsRegID(iMode){ 
		...
		var winRI = window.open('/admin/approval/eapp/popSetAuthID.asp?iM='+iMode+'&iLAID='+ilastApprovalid+'&sjn='+document.frm.hidJN.value+'&idpid='+idpid+'&icid1='+icid1+'&icid2='+icid2+'&icid3='+icid3+'&icid4='+icid4 ,'popAL','width=800, height=550, resizable=yes, scrollbars=yes');
		winRI.focus();
}
...
function jsPopView(sPage){
	 	 var winNew = window.open(sPage,"popNew","width=1200, height=800,scrollbars=yes, resizable=yes");
		 winNew.focus();
	}
...
//결재등록
	function jsEappConfirm(iState){ 
		...
		if (iState == 1){
            ...
            if (sRectAuthType=="L"){
                appStr = "최종"+appStr;
            }else if(sRectAuthType=="F"){ //최송합의승인
                appStr = "최종합의"+appStr;
...
function jsPayEappSubmit(	sMode, reportprice ,arapcd){ 
	...
	if (document.frm.selPT.options[document.frm.selPT.selectedIndex].value ==2){//계좌이체 
	  	 if(!document.frm.san.value){
	  	 	alert("결제방법이 계좌이체일 경우 계좌번호를 입력해주세요");
	  	 	return;
	  	 }
	  } 
...
function jsGoScm(sURL, iIdx){
	var winScm = window.open(sURL+iIdx,"newScm","width=1200, height=800, resizable=yes, scrollbars=yes");
	winScm.focus();
}
