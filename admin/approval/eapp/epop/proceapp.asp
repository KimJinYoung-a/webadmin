...
reportPrice	= requestCheckvar(Request("mRP"),20)
...
'//2020-07-21 태훈 추가 (사번 로그인 등록)
if sUserid="" or isnull(sUserid) then
	sUserid = session("ssBctSn")
	adminID = session("ssBctSn")
	strSql = "exec [db_partner].[dbo].[usp_SCM_eappReport_MemberID_GetSet] '" & CStr(adminID) & "'"
...
Case "I"
	...
	IF returnValue > 0 THEN
		...
		With objCmd
			.ActiveConnection = dbget
			.CommandType = adCmdText
			.CommandText = "{?= call db_partner.[dbo].[sp_Ten_eappPartMoney_insert]( "&reportIdx&" ,0,'"&trim(eapppartIdx(i))&"','"&partMoney(i)&"')}"
			.Parameters.Append .CreateParameter("RETURN_VALUE", adInteger, adParamReturnValue)
			.Execute, , adExecuteNoRecords
			End With
		    returnValue = objCmd(0).Value
