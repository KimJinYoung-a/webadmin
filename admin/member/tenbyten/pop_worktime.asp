			...
			if ((eval("document.frmPay.hidWD"+i).value*1 == 7) || (i == LastDayOfThisMonth)) {
				if (blnCWH > 1) {
				    ...
				    if (i*1!=LastDayOfThisMonth*1){ //추가
					    alert("주휴일을 설정해주세요...." + itwt + " " + blnCWH + " " +LastDayOfThisMonth+ " [" +i+"] "+hidCWHD);
					    return false;
					}
				}
	...
	<%'--------------- 현재 달 급여 설정 start ----------------
		...
		<tr   bgcolor="#FFFFFF"  align="center" >
			...
			<td>
				...
				<option value="6" <%IF iworktype ="6" THEN%>selected<%END IF%> style="color:red">주휴일(무)</option>
				<option value="7" <%IF iworktype ="7" THEN%>selected<%END IF%> style="color:red">주휴일(유)</option>
