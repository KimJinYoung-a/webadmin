...
Sub sbOptCodeType2(ByVal selCodeType)
    Dim arrSelCode(19), i
    arrSelCode(0)	= Split("jobkind|직무","|")
    arrSelCode(1)	= Split("placekind|근무지","|")
	...
	<td colspan="2"><!--//코드 등록 및 수정-->	
		...
		<tr>
			<td>	
				...
				<tr>
					<td bgcolor="#EFEFEF"   align="center">코드 전시여부</td>
					<td bgcolor="#FFFFFF"><input type="radio" value="Y" name="rdoD" <%IF sCodeDispYN ="Y" THEN%>checked<%END IF%>>전시 
					<input type="radio" value="N" name="rdoD" <%IF  sCodeDispYN ="N" THEN%>checked<%END IF%>>전시안함 </td>
				</tr>
				<tr>
