...
function selectThis(selval,selNm){
	opener.<%= frmName %>.<%= compName %>.value = selval;
	try{
		opener.document.getElementById("txtBrandName").innerText = selNm;
	} catch(e) {}
	try{
		opener.chcwflag(opener.frmMaster.shopid.value);
	} catch(e) {}
	window.close();
}
</script>
...
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	...
	<% end if %>
		...
		<td><a href="javascript:selectThis('<%= opartner.FPartnerList(i).FID %>','<%=Replace(opartner.FPartnerList(i).FSocName,"'","")%>')"><%= opartner.FPartnerList(i).FID %></a></td>
		<td>
			<%= opartner.FPartnerList(i).FSocName_Kor %><br>
			<%= opartner.FPartnerList(i).FSocName %>
		</td>
		<td><a href="javascript:selectThis('<%= opartner.FPartnerList(i).FID %>','<%=Replace(opartner.FPartnerList(i).FSocName,"'","")%>')"><%= opartner.FPartnerList(i).Fcompany_name %></a></td>
