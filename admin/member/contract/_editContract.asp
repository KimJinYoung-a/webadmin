...
if isEcContract then
	...
	ecCtrState =  fnViewEcContInfo(oneContract.FOneItem.FecCtrSeq, oneContract.FOneItem.FBcompany_no, oneContract.FOneItem.FecBUser, acctoken, strErrMsg)
		...
		if ecCtrState <> GetContractEcStateName(oneContract.FOneItem.FCtrState) then
			dim sqlStr
...
<table width="100%" border="0" cellspacing="1" cellpadding="4" class="a" bgcolor="#BABABA">
	<form name="frmecView" method="post" action="<%=FecURL%>/w20/contractView.do" > 
...
<form name="frmCtrEdt" method="post" action="ctrReg_Process.asp">
...
<td colspan="3"><%if isEcContract then%>전자(<%=onecontract.FOneItem.FecCtrSeq%>)<%else%>수기<%end if%></td>
</tr>
<tr bgcolor="#FFFFFF" >
