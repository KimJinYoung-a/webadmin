...
function regContract(itype){
    ...
    // 특약은 현재 DocuSign에서만 사용 가능함.
    // 일단 사용 안함
    /*
    if (itype!=3){
        if (frm.chkCT13.checked){
            alert('특약 계약서는 DocuSign 등록 시에만 사용이 가능합니다.');
            frm.chkCT13.checked=false;
            $("#specialAppointmentArea").attr("style", "display:none;");
            return;            
        }
    }
    // DocuSign 등록 시 특약계약서 체크하면 특약내용 validation 체크
    if (itype==3){
        if (frm.chkCT13.checked){
            if (frm.$$CONTENTS_CONTS$$.value.length==0){
                alert('특약내용을 입력하세요.');
                frm.$$CONTENTS_CONTS$$.focus();
                return;
            }
        }
    }
   ...
   // if (frm.defaultCtrKey.value!=""){
    ...
    if(itype==2 ) {
        ...
        // DocuSign
        $("#dsLyr").css('top', winH-$("#dsLyr").height());
        $("#dsLyr").css('left', winW/2-$("#dsLyr").width()/2);
        $("#dsDiv").show();
    } else {
        // 수기계약
...
function jsDsSubmit(){
    ...
    if(!document.frmDs.dsAEmail.value){
        alert("계약서를 전달할 이메일을 입력해주세요.");
        return;
    }

    if(!document.frmDs.dsAName.value){
        alert("업체 계약자명을 입력해주세요.");
        return;
    }

    if (confirm('계약서를 신규 등록 하시겠습니까?')){
        frm.signtype.value = 3;
        frm.mngEmail.value = document.frmDs.dsAEmail.value;
        frm.ecBUser.value = document.frmDs.dsAName.value;
        frm.action="ctrReg_Process.asp";
        $("#docuSignSubmitButton").attr('disabled',true);
        frm.submit();
    }
}
...
function jsSetDocuSignState(){
	$("#btnDocuSignSubmit").prop("disabled", true);
	document.frmDocuState.submit();
}
...
function saAreaCheck(a) {
    if (a.checked) {
        $("#specialAppointmentArea").attr("style", "display:'';");
    } else {
        $("#specialAppointmentArea").attr("style", "display:none;");
    }
...
<p>
<form name="frmEcState" method="post"  action="ctrReg_Process.asp">
<input type="hidden" name="mode" value="ecstate">
<input type="hidden" name="groupid" value="<%=groupid%>">
</form>
<form name="frmDocuState" method="post"  action="ctrReg_Process.asp">
<input type="hidden" name="mode" value="docustate">
<input type="hidden" name="groupid" value="<%=groupid%>">
</form>
...
<input type="hidden" name="mngEmail" value="">
<input type="hidden" name="ecBUser" value="">
<tr bgcolor="#FFFFFF">
...
<div style="text-align:right;padding:5px;">
    <span style="left-margin:10px;"><input type="button" id="btnDocuSignSubmit" class="button" value="DocuSign 상태Update" onClick="jsSetDocuSignState('<%=groupid%>');"></span>
...
<tr bgcolor="#FFFFFF" align="center">
	<td>
    <%
        if oDftContractList.FItemList(i).GetSignType<>"" then
            response.write oDftContractList.FItemList(i).GetSignType &_
                chkIIF(not(oDftContractList.FItemList(i).FsignType="H" or oDftContractList.FItemList(i).FsignType="D"),"(" & oDftContractList.FItemList(i).FecCtrSeq & ")","")
        Else
            if oDftContractList.FItemList(i).FecCtrSeq <> "" or not isNull(oDftContractList.FItemList(i).FecCtrSeq ) or oDftContractList.FItemList(i).FecCtrSeq <> "0"  then'
                response.write "전자(" & oDftContractList.FItemList(i).FecCtrSeq & ")"
            else
                response.write "수기"
            end if
        end if
...
<tr bgcolor="#FFFFFF" align="center">
	<td>
    <%
        if oAddContractList.FItemList(i).GetSignType<>"" then
            response.write oAddContractList.FItemList(i).GetSignType &_
                chkIIF(not(oAddContractList.FItemList(i).FsignType="H" or oAddContractList.FItemList(i).FsignType="D"),"(" & oAddContractList.FItemList(i).FecCtrSeq & ")","")
        Else
            if oAddContractList.FItemList(i).FecCtrSeq <> "" or not isNull(oAddContractList.FItemList(i).FecCtrSeq ) or oAddContractList.FItemList(i).FecCtrSeq <> "0"  then'
                response.write "전자(" & oAddContractList.FItemList(i).FecCtrSeq & ")"
            else
                response.write "수기"
            end if
        end if
...
<tr bgcolor="#FFFFFF" align="center">
	<td>
    <%
        if oAddContractListOF.FItemList(i).GetSignType<>"" then
            response.write oAddContractListOF.FItemList(i).GetSignType &_
                chkIIF(not(oAddContractListOF.FItemList(i).FsignType="H" Or oAddContractListOF.FItemList(i).FsignType="D"),"(" & oAddContractListOF.FItemList(i).FecCtrSeq & ")","")&""
        Else
            if oAddContractListOF.FItemList(i).FecCtrSeq <> "" or not isNull(oAddContractListOF.FItemList(i).FecCtrSeq ) or oAddContractListOF.FItemList(i).FecCtrSeq <> "0"  then'
                response.write "전자(" & oAddContractListOF.FItemList(i).FecCtrSeq & ")"
            else
                response.write "수기"
            end if
        end if
...
<p>
<table width="100%" border="0" cellspacing="1" cellpadding="4" class="a" style="border:1px solid #aaa;">
<tr bgcolor="#FFFFFF">
    <td width="20%">&nbsp;</td>
    <td align="center">
        <input type="button" class="button" value="신규 계약 등록 [수기계약]" onClick="regContract(1)">
        <input type="button" class="button" id="docuSignSubmitButton" name="docuSignSubmitButton" value="신규 계약 등록 [DocuSign]" onClick="regContract(3)">
    </td>
    <%'// 2022.04.06 유플러스 전자서명 등록 버튼 제거 %>
    <td width="20%" style="text-align:right;"><!--input type="button" class="button" value="신규 계약 등록 [U+전자서명]" onClick="regContract(2)"--></td>
</tr>
</table>
...
<% end if %>
<style type="text/css">
#ecDiv, #dsDiv {display:none; width:100%; height:100%; position:fixed; left:0; top:0; z-index:900000;}
#ecLyr, #dsLyr {display:; width:600px;height:100px;position:absolute; left:50%; top:50%; margin:0px 0 0 0px; background:#efefef; padding:50px; z-index:999999;}
#mask, #mask2 {display:; width:100%; height:100%;position:absolute; left:0; top:0; z-index:9000; background:url(http://webadmin.10x10.co.kr/images/mask_bg.png) left top repeat;}
 </style>
...
<div id="dsDiv">
	<div id="dsLyr">
		<form name="frmDs" method="post" >
		<div>
            <table cellspacing="1" cellpadding="4" class="a" bgcolor="#BABABA" width="600">
            <tr bgcolor="#FFFFFF" >
                <td bgcolor="#DDDDFF" width="20%" align="center" rowspan="2" >업체 담당자<br />(계약자)</td>
                <td bgcolor="#DDDDFF" width="20%" align="center"  >이메일</td>
                <td ><input type="text" class="text" name="dsAEmail" value="" size="40" /> </td>
            </tr>
            <tr bgcolor="#FFFFFF" >
                <td bgcolor="#DDDDFF" width="10%" align="center"  >이름</td>
                <td ><input type="text" class="text" name="dsAName" value="" size="20" /></td>
            </tr>
            </table>
        </div>
        <div style="width:100%;text-align:center;padding:5px;">
            <input type="button" class="button" value="취소" onClick="jsEcCancel();" />
            <input type="button" class="button" value="전자계약등록" onClick="jsDsSubmit();" />
        </div>
        </form>
	</div>
	<div id="mask2"></div>
