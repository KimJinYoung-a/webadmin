...
<%
dim ContractType , detailKey , i ,mode, sqlStr, ContractContents, ContractName ,onoffgubun
...
if (mode="editProtoType") then
    ...
    sqlStr = sqlStr & " , onoffgubun='" & onoffgubun & "'" & VbCrlf
...
elseif (mode="regProtoType") then
    ...
    sqlStr = sqlStr & " (ContractName,ContractContents)" & VbCrlf
...
function preViewContractProtoType(ContractType){
    var popwin = window.open('preViewProtoType.asp?ContractType=' + ContractType,'preViewProtoType','width=900,height=600,scrollbars=yes,resizable=yes');
    popwin.focus();
}


function DocDownloadContractProtoType(ContractType){
    var popwin = window.open('DocDownloadpreViewProtoType.asp?ContractType=' + ContractType,'DocDownloadpreViewProtoType','width=900,height=600,scrollbars=yes,resizable=yes');
    popwin.focus();
}
...
return;
    ...
    if (frm.onoffgubun.value.length<1){
        alert('계약서 구분을 선택 하세요.');
        frm.onoffgubun.focus();
        return;
    }
...
<% end if %>
    ...
    <td><%= ocontractProtoType.FItemList(i).fonoffgubun %></td>
    ...
    <td>
        ...
        &nbsp;
        <a href="javascript:DocDownloadContractProtoType('<%= ocontractProtoType.FItemList(i).FContractType %>');"><img src="/images/btn_word.gif" width="70" border="0"></a></td>
</tr>
<% next %>
</table>
<br>
...
%>
	...
	<tr bgcolor="#FFFFFF">
	    ...
	    <td colspan="2">
	        <% drawonoffgubun "onoffgubun" , onecontractProtoType.FOneItem.Fonoffgubun %>
	    </td>
	</tr>
	<tr bgcolor="#FFFFFF">
...
%>
	...
	<tr bgcolor="#FFFFFF">
	    <td width="100">계약서 구분</td>
	    <td colspan="2">
	        <% drawonoffgubun "onoffgubun" , onoffgubun %>
	    </td>
	</tr>
	<tr bgcolor="#FFFFFF">
	    <td colspan="3">
	...
	<tr bgcolor="#FFFFFF" height="30">
	    <td align="center" colspan="3"><input type="button" value="신규저장" onClick="checkNSubmit(frmSvr);"  class="button"></td>
	</tr>
	</form>
	</table>
