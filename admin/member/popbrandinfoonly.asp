...
<%
dim itableWidth : itableWidth="800px"
...
<% if (C_SYSTEM_Part or C_OP or C_ADMIN_AUTH or C_CSPowerUser or C_MngPart) and opartner.FOneItem.Fpartnerusing = "Y" then %>
function jsLogin(makerid) {
	if (confirm('[관리자권한]\n\n업체 어드민으로 로그인하시겠습니까?\n(직원어드민 로그인은 풀립니다.)') != true) { return; }

	var pop = window.open("<%= partnerScmUrl %>/admin/etc/forceBrandSSn.asp?makerid="+makerid,"jsLogin","width=1500, height=800,scrollbars=yes,resizable=yes");
	pop.focus();
}
<% end if %>
</script>
...
<table width="100%" style="max-width:<%= itableWidth %>;" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
<colgroup>
	<col width="15%" />
	<col width="35%" />
	<col width="15%" />
...
<tr height="25" bgcolor="<%= adminColor("tabletop") %>">
	<td colspan=4><b>* 사업자정보요약</b></td>
</tr>
<tr height="25">
	<td width="100" bgcolor="<%= adminColor("tabletop") %>">그룹코드</td>
	<td bgcolor="#FFFFFF" >
		<%= ogroup.FOneItem.FGroupId %>
		<% if (prevmonthgroupid <> "") and (prevmonthgroupid <> ogroup.FOneItem.FGroupId) then %>
		&lt;- <font color="red"><%= prevmonthgroupid %></font>
		<% if C_ADMIN_AUTH or C_OFF_AUTH or C_MD_AUTH then %>
		<input type="button" class="button_s" value="변경" onClick="jsModiPrevMonthGroupID('<%= opartner.FOneItem.FID %>')">
		<font color="blue"><b>*</b></font>
		<% end if %>
		<% end if %>
	</td>
	<td width="90" bgcolor="<%= adminColor("tabletop") %>">등록일</td>
	<td bgcolor="#FFFFFF" >
		<%= ogroup.FOneItem.FRegdate %>
	</td>
</tr>
<tr height="25">
	...
	<td bgcolor="#FFFFFF" >
		<%= ogroup.FOneItem.Fceoname %>
	</td>
</tr>
<tr height="25">
	<td bgcolor="<%= adminColor("tabletop") %>">사업자번호</td>
	<td bgcolor="#FFFFFF" >
		<%= socialnoReplace(ogroup.FOneItem.Fcompany_no) %>
	</td>
	<td bgcolor="<%= adminColor("tabletop") %>">과세구분</td>
	<td bgcolor="#FFFFFF" >
		<%= ogroup.FOneItem.Fjungsan_gubun %>
	</td>
</tr>
<tr height="25">
...
<tr>
	...
	<td bgcolor="<%= adminColor("tabletop") %>">Fax</td>
	<td bgcolor="#FFFFFF" ><%= ogroup.FOneItem.Fcompany_fax %></td>
</tr>
-->
<tr bgcolor="#FFFFFF">
	<td height="25" colspan="4">**담당자정보**</td>
</tr>
<tr>
...
<tr>
	...
	<td bgcolor="#FFFFFF"><a href="mailto:<%= ogroup.FOneItem.fmanager_email %>"><%= ogroup.FOneItem.fmanager_email %></a></td>
...
<tr>
	...
	<td bgcolor="#FFFFFF"><a href="mailto:<%= ogroup.FOneItem.Fjungsan_email %>"><%= ogroup.FOneItem.Fjungsan_email %></a></td>
...
<tr>
	...
	<td colspan="3" bgcolor="#FFFFFF"><%= opartner.FOneItem.FID %>
		<% if C_ADMIN_AUTH or C_SYSTEM_Part or C_CSUser or C_MD or C_OFF_part or C_logics_Part then %>
			<span style="padding-left:10px;"><input type="button" class="button" value="비밀번호 변경" onClick="jsOpenPW('<%=opartner.FOneItem.FID%>','P');"></span>
		<%END IF%>
	</td>
</tr>
<tr>
...
<tr>
	...
	<td bgcolor="#FFFFFF" >
		<select name="socname_use" class="select">
			<option value="K" <%=chkIIF(opartner.FOneItem.Fsocname_use="K","selected","")%>>브랜드명(KR)</option>
			<option value="E" <%=chkIIF(opartner.FOneItem.Fsocname_use="E","selected","")%>>브랜드명(EN)</option>
		</select>
	</td>
	...
	<td bgcolor="#FFFFFF" >
		...
		&nbsp;&nbsp;
		<% if C_ADMIN_AUTH or C_OFF_AUTH or C_MD_AUTH then %>
		<input type="button" class="button" value="수정" onClick="PopBrandAdminUsingChange('<%= opartner.FOneItem.FID %>');" /><font color="blue"><b>*</b></font>
		<% end if %>
		<% if (C_SYSTEM_Part or C_OP or C_ADMIN_AUTH or C_CSPowerUser or C_MngPart) and opartner.FOneItem.Fpartnerusing = "Y" then %>
		&nbsp;
		<input type="button" class="button" value="로그인" onClick="jsLogin('<%= opartner.FOneItem.FID %>');" /><font color="red"><b>*</b></font>
		<% end if %>
	</td>
</tr>
<tr bgcolor="#FFFFFF">
...
<tr >
	...
	<td bgcolor="#FFFFFF" >
	<input type="text" name="prtidx" value="<%= opartner.FOneItem.getRackCode %>" size="8" maxlength="8">
	(4 or 8자리 Fix)</td>
	</td>
</tr>
<tr height=25"">
...
<% if (pcuserdiv="9999_02") or (pcuserdiv="9999_15") then %>
	<table width="100%" style="max-width:<%= itableWidth %>;" align="center" cellpadding="1" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
    <colgroup>
        <col width="15%" />
        <col width="15%" />
        <col width="20%" />
	...
	<tr>
		...
		<td bgcolor="#FFFFFF">
			<label><input type=radio name="isusing" value="Y" <% if opartner.FOneItem.Fisusing="Y" then response.write "checked" %> > Y</label>
			<label><input type=radio name="isusing" value="N" <% if opartner.FOneItem.Fisusing="N" then response.write "checked" %> > N</label>
		</td>
		<td rowspan="2" bgcolor="<%= adminColor("pink") %>">스트리트<br>표시여부<br>(브랜드운영관련)</td>
		<td bgcolor="#FFFFFF">텐바이텐</td>
		<td bgcolor="#FFFFFF">
			<label><input type=radio name="streetusing" value="Y" <% if opartner.FOneItem.Fstreetusing="Y" then response.write "checked" %> > Y</label>
			<label><input type=radio name="streetusing" value="N" <% if opartner.FOneItem.Fstreetusing="N" then response.write "checked" %> > N<label>
		</td>
	</tr>
	<tr>
	    <td bgcolor="#FFFFFF">텐바이텐 OFF</td>
		<td bgcolor="#FFFFFF">
			<label><input type=radio name="isoffusing" value="Y" <% if opartner.FOneItem.Fisoffusing="Y" then response.write "checked" %> > Y</label>
			<label><input type=radio name="isoffusing" value="N" <% if opartner.FOneItem.Fisoffusing="N" then response.write "checked" %> > N</label>
		</td>
		<td bgcolor="#FFFFFF">커뮤니티(상품Q/A)</td>
		<td bgcolor="#FFFFFF">
			<label><input type=radio name="specialbrand" value="Y" <% if opartner.FOneItem.Fspecialbrand="Y" then response.write "checked" %>> Y</label>
			<label><input type=radio name="specialbrand" value="N" <% if opartner.FOneItem.Fspecialbrand="N" then response.write "checked" %>> N</label>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td bgcolor="#FFFFFF" colspan="2">
			<label><input type=radio name="onlyflg" value="Y" <% if opartner.FOneItem.Fonlyflg="Y" then response.write "checked" %> > Y</label>
			<label><input type=radio name="onlyflg" value="N" <% if opartner.FOneItem.Fonlyflg="N" then response.write "checked" %> > N</label>
		</td>
		<td bgcolor="<%= adminColor("pink") %>">Artist 노출</td>
		<td bgcolor="#FFFFFF" colspan="2">
			<label><input type=radio name="artistflg" value="Y" <% if opartner.FOneItem.Fartistflg="Y" then response.write "checked" %> > Y</label>
			<label><input type=radio name="artistflg" value="N" <% if opartner.FOneItem.Fartistflg="N" then response.write "checked" %> > N</label>
		</td>
	</tr>
	<tr>
		<td bgcolor="<%= adminColor("pink") %>">K-Design 노출</td>
		<td bgcolor="#FFFFFF" colspan="2">
			<label><input type=radio name="kdesignflg" value="Y" <% if opartner.FOneItem.Fkdesignflg="Y" then response.write "checked" %> > Y</label>
			<label><input type=radio name="kdesignflg" value="N" <% if opartner.FOneItem.Fkdesignflg="N" then response.write "checked" %> > N</label>
		</td>
	...
	<tr >
		<td bgcolor="<%= adminColor("pink") %>">온라인대표<br>관리카테고리</td>
	...
	<tr >
		<td bgcolor="<%= adminColor("pink") %>">온라인대표<br>전시카테고리</td>
		...
		<td bgcolor="#FFFFFF" colspan=2>
			<%= fnStandardDispCateSelectBox(1,"", "standardmdcatecode", opartner.FOneItem.fstandardmdcatecode, "")%>
		</td>
	</tr>
	<tr >
...
<% if (pcuserdiv="9999_14") then %>
	...
	<tr>
		...
		<td bgcolor="#FFFFFF">
		    <label><input type=radio name="isusing" value="Y" <%= CHKIIF(opartner.FOneItem.Fisusing="Y","checked","") %> > Y</label>
		    <label><input type=radio name="isusing" value="N" <%= CHKIIF(opartner.FOneItem.Fisusing="N","checked","") %> > N</label>
		</td>
		<td bgcolor="<%= adminColor("pink") %>">스트리트<br>표시여부<br>(브랜드운영관련)</td>
		<td bgcolor="#FFFFFF">
		    <label><input type=radio name="streetusing" value="Y" <%= CHKIIF(opartner.FOneItem.Fstreetusing="Y","checked","") %> > Y</label>
		    <label><input type=radio name="streetusing" value="N" <%= CHKIIF(opartner.FOneItem.Fstreetusing="N","checked","") %> > N</label>
		</td>
	</tr>
	<tr >
		<td width="120" bgcolor="#DDDDFF" rowspan="2">강좌 진행 여부</td>
		<td bgcolor="#FFFFFF" rowspan="2">
			<label><input type="radio" name="lec_yn" value="Y" <%= CHKIIF(opartner.FOneItem.Flec_yn="Y","checked","") %> onClick="clickLec(this)"> Y</label>
			<label><input type="radio" name="lec_yn" value="N" <%= CHKIIF(opartner.FOneItem.Flec_yn="N","checked","") %> onClick="clickLec(this)"> N</label>
		</td>
	...
	<tr >
		...
		<td  bgcolor="#FFFFFF" width="200" >
			<label><input type="radio" name="diy_yn" value="Y"  <%= CHKIIF(opartner.FOneItem.Fdiy_yn="Y","checked","") %> onClick="clickDiy(this);"> Y</label>
			<label><input type="radio" name="diy_yn" value="N"  <%= CHKIIF(opartner.FOneItem.Fdiy_yn="N","checked","") %>  onClick="clickDiy(this);"> N</label>
		</td>
...
<% if (pcuserdiv="9999_02") or (pcuserdiv="9999_14") then %>
<!-- 	<table width="<%= itableWidth %>" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>"> -->
<!--  -->
<!-- 	<% if (application("Svr_Info")="Dev") then %> -->
<!-- 		<form name="frmetc" method=post action="http://testpartner.10x10.co.kr/linkweb/doprofileimageadmin.asp" enctype="multipart/form-data"> -->
<!-- 	<% else %> -->
...
<!-- 	<tr> -->
<!-- 		<td bgcolor="<%= adminColor("sky") %>" colspan=3><b>3.브랜드 기타정보 (웹페이지 표시정보)</b></td> -->
<!-- 	</tr> -->
<!-- 	<tr> -->
<!-- 		<td width="90" bgcolor="<%= adminColor("sky") %>" >아이콘</td> -->
<!-- 		<td bgcolor="#FFFFFF"> -->
<!-- 		<img name=iconimg src="<%= opartner.FOneItem.getSocIconUrl %>" ><br> -->
<!-- 		(브랜드 로고는 30x30 픽셀로 지정되 있습니다.)-->
<!-- 		<input type="file" class="button" name="file3" size="60" onchange="ChangeIcon(this,frmetc.iconimg);"> -->
<!-- 		</td> -->
<!-- 	</tr> -->
<!-- 	<tr> -->
...
<!-- 		(브랜드 로고는 150x100 픽셀로 지정되어 있습니다.)<br> -->
<!-- 		<input type="file" class="button" name="file1" size="60" onchange="ChangeLogo(this,frmetc.logoimg);"> -->
<!-- 		</td> -->
<!-- 	</tr> -->
<!-- 	<tr> -->
<!-- 		<td bgcolor="<%= adminColor("sky") %>" >배너</td> -->
<!-- 		<td bgcolor="#FFFFFF"> -->
<!-- 		<img name=titleimg src="<%= opartner.FOneItem.getTitleImgUrl %>" width=<%=720/2%> height=<%=220/2%>><br> -->
<!-- 		(배너이미지는 720x220 픽셀로 지정되어 있습니다.)<br> -->
<!-- 		<input type="file" class="button" name="file2" size="60" onchange="ChangeTitle(this,frmetc.titleimg);"> -->
<!-- 		<% If opartner.FOneItem.getTitleImgUrl <> "http://webimage.10x10.co.kr/image/brandlogo/" Then%> -->
...
<!-- 	 -->
<!-- 	<tr> -->
<!-- 		<td bgcolor="<%= adminColor("sky") %>" >디자이너<br>코멘트</td> -->
<!-- 		<td bgcolor="#FFFFFF"> -->
<!-- 			<textarea name="dgncomment" class="textarea" cols="75" rows="6"><%= opartner.FOneItem.Fdgncomment %></textarea> -->
<!-- 		</td> -->
<!-- 	</tr> -->
<!-- 	<tr> -->
<!-- 		<td colspan="3" bgcolor="#FFFFFF" height="25">**연관 브랜드**</td> -->
<!-- 	</tr> -->
<!-- 	<tr> -->
<!-- 		<td align="center" bgcolor="<%= adminColor("sky") %>" >브랜드</td> -->
<!-- 		<td bgcolor="#FFFFFF" colspan="2"> -->
<!-- 		<% -->
<!-- 			If opartner.FOneItem.Fsamebrand <> "" Then -->
<!-- 				drawSelectBoxDesignerwithName2 "ebrand", Split(opartner.FOneItem.Fsamebrand,"|")(0), Split(opartner.FOneItem.Fsamebrand,"|")(1) -->
<!-- 			Else -->
<!-- 				drawSelectBoxDesignerwithName2 "ebrand", opartner.FOneItem.Fsamebrand, "" -->
<!-- 			End If -->
<!-- 		%> -->
<!-- 		</td> -->
<!-- 	</tr> -->
<!-- 	<tr> -->
<!-- 		<td colspan="3" align=center bgcolor="#FFFFFF"><input type="button" class="button" value="브랜드 기타정보 저장" onclick="SaveBrandEtcInfo(frmetc);"></td> -->
<!-- 	</tr> -->
<!-- 	 -->
<!-- 	</form> -->
<!-- 	</table> -->
	...
	<tr>
		...
		<input type="file" class="button" name="file4" size="60" onchange="ChangeTitle(this,frmetc.brandimg);">
