...
// 추가 이미지 변경
	...
	function TnSwitchImageBG(imagesrc,btn){
		document.getElementById("IimageMain").src = imagesrc;
...
<tr>
    ...
    <tr>
    	<td valign="top" style="padding-left:20px;">
    		...
    		<tr>
    			<td><img src="<% = getFirstAddimage %>" id="IimageMain" width="400" height="400"></td>
    		</tr>
    		<tr>
    			<td style="padding-top:7px;">
    				...
    				<tr>
    					...
    					<% end if %>
    					<% if ImageExists(oitem.FOneItem.FMaskImage) then %>
    					<td style="padding-right:5px;">
    						<table border="0" cellspacing="1" cellpadding="1"  onclick="TnSwitchImageBG('<%= oitem.FOneItem.FMaskImage %>','btnM');" style="cursor:pointer" bgcolor="#CCCCCC">
    						<tr>
    							<td width="38" height="38"  id="btnM"><img src="<%= oitem.FOneItem.FMaskImage %>" width=38 height=38></td>
    						</tr>
    						</table>
    					</td>
    					<% end if %>
    					<% IF oAdd.FResultCount > 0 THEN %>
    					<% FOR i= 0 to oAdd.FResultCount  %>
    					...
    					<td style="padding:15px 0 9px 0;border-bottom:1px solid #eaeaea;">
    						...
    						<tr>
    							...
    							<td>
    								<table border="0" cellspacing="0" cellpadding="0">
    								<tr>
    									<td class="black11pxb" style="padding:2px 5px 0 0;"><%= FormatNumber(oitem.FOneItem.getOrgPrice,0) %><% if oitem.FOneItem.IsMileShopitem then %> Point<% else %> 원<% end  if %></td>
	...
	<td style="padding:20px 20px 10px 20px;">
		...
		<tr>
			<td style="padding-bottom:30px;">
				...
				<%
					If addEx.FResultCount = 0 Or IsNull(addEx.FResultCount) Then 
				%>
				<tr>
					<td>
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<td>
						<div class="bgBox01">
							<div class="pdtInforList">
								...
								%>
								<span><em><%=addEx.FItem(i).FInfoname%></em> : <%=addEx.FItem(i).FInfoContent%></span>
								<% 
										Next 
									End If 
								%>
							</div>
							<% if oItem.FOneItem.IsSafetyYN then %>
							<!-- 안전인증대상 일때만 추가 -->
							<div class="pdtInforList abroadMsg">
								<span><em>안전인증대상</em> : <%=oItem.FOneItem.IsSafetyDIV%>&nbsp;<%=oItem.FOneItem.FsafetyNum%></span>
							</div>
							<% End If %>
							<!-- //안전인증대상 일때만 추가 -->
						</div>
				...
				<tr>
					<td style="padding-top:30px;" class="gray11px02">
						...
						<% end if %>
						<% if ImageExists(oItem.FOneItem.FMainImage2) then %>
							<img src="<% = oItem.FOneItem.FMainImage2 %>" border="0" id="filemain2" style="cursor:pointer;" onclick="ViewOrgImage('<% = oItem.FOneItem.FMainImage2 %>');" >
						<% end if %>
						<% if ImageExists(oItem.FOneItem.FMainImage3) then %>
							<img src="<% = oItem.FOneItem.FMainImage3 %>" border="0" id="filemain3" style="cursor:pointer;" onclick="ViewOrgImage('<% = oItem.FOneItem.FMainImage3 %>');" >
						<% end if %>
						<% if ImageExists(oItem.FOneItem.FMainImage4) then %>
							<img src="<% = oItem.FOneItem.FMainImage4 %>" border="0" id="filemain3" style="cursor:pointer;" onclick="ViewOrgImage('<% = oItem.FOneItem.FMainImage4 %>');" >
						<% end if %>
						<% if ImageExists(oItem.FOneItem.FMainImage5) then %>
							<img src="<% = oItem.FOneItem.FMainImage5 %>" border="0" id="filemain3" style="cursor:pointer;" onclick="ViewOrgImage('<% = oItem.FOneItem.FMainImage5 %>');" >
						<% end if %>
						<% if ImageExists(oItem.FOneItem.FMainImage6) then %>
							<img src="<% = oItem.FOneItem.FMainImage6 %>" border="0" id="filemain3" style="cursor:pointer;" onclick="ViewOrgImage('<% = oItem.FOneItem.FMainImage6 %>');" >
						<% end if %>
						<% if ImageExists(oItem.FOneItem.FMainImage7) then %>
							<img src="<% = oItem.FOneItem.FMainImage7 %>" border="0" id="filemain3" style="cursor:pointer;" onclick="ViewOrgImage('<% = oItem.FOneItem.FMainImage7 %>');" >
						<% end if %>
						<% if ImageExists(oItem.FOneItem.FMainImage8) then %>
							<img src="<% = oItem.FOneItem.FMainImage8 %>" border="0" id="filemain3" style="cursor:pointer;" onclick="ViewOrgImage('<% = oItem.FOneItem.FMainImage8 %>');" >
						<% end if %>
						<% if ImageExists(oItem.FOneItem.FMainImage9) then %>
							<img src="<% = oItem.FOneItem.FMainImage9 %>" border="0" id="filemain3" style="cursor:pointer;" onclick="ViewOrgImage('<% = oItem.FOneItem.FMainImage9 %>');" >
						<% end if %>
						<% if ImageExists(oItem.FOneItem.FMainImage10) then %>
							<img src="<% = oItem.FOneItem.FMainImage10 %>" border="0" id="filemain3" style="cursor:pointer;" onclick="ViewOrgImage('<% = oItem.FOneItem.FMainImage10 %>');" >
						<% end if %>
					</td>
				</tr>
				</table>
