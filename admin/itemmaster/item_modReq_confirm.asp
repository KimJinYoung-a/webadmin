...
<%
'###########################################################
' Description : 업체배송상품_상품수정승인상품 리스트
' Hieditor : 2014.03.21 정윤정 생성
'###########################################################
%>
...
function jsSort(sValue,i){
 ...
 function jsChkSubmit(sMode){
	...
	document.frmF.editidx.value = "";
	document.frmF.edittype.value = "";
...
function jsCancel(){
 ...
 <form name="frmR" method="post" action="/admin/itemmaster/item_modReq_confirm_proc.asp">
 ...
 <form name="frmF" method="post" action="/admin/itemmaster/item_modReq_confirm_proc.asp">
...
<tr bgcolor="#FFFFFF" align="center">
	...
	<td rowspan="2"><img src="<%=webImgUrl%>/image/small/<%=GetImageSubFolderByItemid(arrList(0,intLoop))%>/<%=arrList(11,intLoop)%>"></td>
	...
	<td align="left"  <% IF arrList(35,intLoop) = "N" THEN%>bgcolor="#DDFFDD"<%ELSE%>rowspan="2"<%END IF%>>
		...
		<%=arrList(2,intLoop)%>
		<%END IF%>
		&nbsp;&nbsp;<a href="<%=wwwUrl%>/shopping/category_prd.asp?itemid=<%=arrList(0,intLoop)%>" target="_blank"><font color="blue">확인하기</font></a></td>
