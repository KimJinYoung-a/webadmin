...
<%
'####################################################
' Page : /admin/itemmaster/deal/dealitem_regist.asp
' Description :  딜 상품 등록 메인
...
function jsCopyItem(){
	var winItemCopy;
	winItemCopy = window.open('pop_dealitem_copy.asp?idx=<%=idx%>','popCopy','width=800,height=620');
	winItemCopy.focus();
}
//그룹검색
function jsSearch(){ 
...
function jsExcelDown(iCurrpage){
	...
	document.fitem.action="dealitemlist_excel.asp";
	document.fitem.submit();
}
//상품 복사 후 페이지 새로고침
function jsItemCopyAfter(){
	opener.fnLoadItems();
	opener.fnItemSelectboxLoad();
	location.reload();
}
//상품 삭제 후 새로고침
function jsDeleteItemReload(){
	location.reload();
}
// 상품 순서 일괄 저장
function jsItemSortSave() {
	var frm;
...
//정렬
function jsChSort(){
	document.fitem.sorting.value="Y";
	document.fitem.submit();	
}
function jsAddNewItem(){
	var groupSelect = document.fsearch.selG.value;
	var winpop = window.open('/admin/itemmaster/deal/pop_deal_additemlist.asp?idx=<%=idx%>&groupSelect='+groupSelect+'&stype=w','winItempop','width=1024,height=768,scrollbars=yes,resizable=yes');
}
function jsItemAddLoad(){
	opener.fnLoadItems();
	opener.fnItemSelectboxLoad();
}
            						...
            						<tr>
            							<td style="white-space:nowrap;">그룹: 
            								<select name="selG" onChange="jsSearch();" class="select">
            						        	<option value=""<% IF strG = "" THEN %> selected<% END IF %>>미지정</option>
                      			...
                      			<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="a"  >		 
                      			    <tr height="35">			      
                      			        <td align="left">       	
                          			       	...
                          			       	<% For intLoop = 0 To UBound(arrGroup,2) %>
                          			       		<option value=" <%=arrGroup(0,intLoop)%>"><%=arrGroup(0,intLoop)%>(<%=arrGroup(1,intLoop)%>)</option>
                          			    	<% Next %>
                          			    	<% ELSE %>
                          			    	<option value="">--그룹없음--</option>
                          			    	<% END IF %>
                          			       	</select>
                      			       		<input type="button" value="선택그룹이동" onclick="jsMoveGroup();" class="button">
                      			       			&nbsp; 	<input type="button" value="그룹추가" onclick="jsAddGroup('<%=idx%>','');" class="button">
												&nbsp; 	<input type="button" value="상품 복사" onclick="jsCopyItem();" class="button">
                      			    	</td>
                      			    	<td align="right">
										  	<input type="button" value="엑셀 다운" onclick="jsExcelDown();"  class="button">
                      			    	    <input type="button" value="선택상품 수정" onclick="jsItemSortSave();" class="button">&nbsp; 
                      			...
                      			<table width="100%" border="0" align="center" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>"> 
									...
									IF isArray(arrList) THEN 
                      			    ...
                      			    <tr align="center" bgcolor="#FFFFFF">    
                      			    	...
                      			    	<td><%=db2html(arrList(19,intLoop))%></td>
										...
										<td class="sortablearrow"><%=db2html(arrList(24,intLoop))%></td>
										...
										<% else %>
                      			    	<td class="sortablearrow"><input type="text" name="sSort" value="<%=arrList(20,intLoop)%>" size="4" style="text-align:right;"></td>
										<% end if %>
                      			    	<td class="sortablearrow"><input type="button" value="삭제" onClick="jsDel(1,<%=arrList(0,intLoop)%>);" class="button"></td>	
                      			    </tr>   
                      				<%	
										Next
                      				ELSE
...
<!-- 선택삭제--->
<form name="frmDel" method="post" action="dodealitemdel.asp">
...
<!-- 순서 및 이미지크기 변경--->
<form name="frmSortImgSize" method="post" action="dodealitemgroup.asp">
