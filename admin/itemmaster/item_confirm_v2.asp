		...
		<td><!-- action ---------------------------------->
		 ...
		 <!--input type="button" class="button" value="오픈예약"--></td>
						<td align="right">
							...
							<%if dispcate <> "" and makerid <> ""  then%>
							<input type="button" class="button" id="btnSubmit" value="  일괄승인  " onClick="jsMultiWaitState(1);" style="background-color:#F29661;">
							<%end if%>
						</td>
					</tr>
				</table>
				...
				<tr bgcolor="<%if arrList(16,intLoop) <> 7 THEN%>#DDDDFF<%else%>#FFFFFF<%end if%>" align="center">
					...
					<td><font color="<%=GetCurrStateColor(arrList(7,intLoop))%>"><%=GetCurrStateName(arrList(7,intLoop))%></font>
							...
							<% elseif  (arrList(7,intLoop)="1") or (arrList(7,intLoop)="5") then%>
						 	&nbsp;<input id="btnApp" name="btnApp" type="button" class="button" value="▶승인" style="color:blue;" onclick="jsApproval('<%=arrList(0,intLoop)%>','<%=arrList(2,intLoop)%>','<%=arrList(16,intLoop)%>')">
							<% end if %>
