...
<%
    ...
    vArr0 = cMI.fnGetItemTotalCountByDispmdcatecode
	...
	If vGubun = "1" Then
		...
		vArr1 = cMI.fnGetItemSellStdateByDispmdcatecode
	ElseIf vGubun = "2" Then
		'vArr1 = cMI.fnGetUserCitemSellStdateByDisp
		vArr1 = cMI.fnGetUserCitemSellStdateByDispmdcatecode
	End If
	...
	If vCompareKey = "o" Then
		...
		If vGubun = "1" Then
			...
			vArr2 = cMI.fnGetItemSellStdateByDispmdcatecode
		ElseIf vGubun = "2" Then
			'vArr2 = cMI.fnGetUserCitemSellStdateByDisp
			vArr2 = cMI.fnGetUserCitemSellStdateByDispmdcatecode
		End If
	Else
	End If
	Set cMI = Nothing
...
function jsGoSearch(){
	...
	$("#realbtn").hide();
	$("#loadingbtn").show();
	frm1.action = "makerid_itemid_v2.asp";
	frm1.submit();
}
...
<div class="contSectFix scrl">
	<div class="contHead">
		<div class="locate"><h2>DATAMART&gt;&gt;<strong>신상품/신규브랜드 현황v2</strong></h2></div>
		<div class="helpBox">
	...
	<div class="searchWrap2">
		...
		<input type="button" class="resetBtn" value="초기화" onClick="location.href='/admin/datamart/mng/makerid_itemid_v2.asp';" />
	</div>
	</form>
	...
	<div class="pad20">
		<div class="tPad10">
			<div class="overHidden pad10">
				<div class="ftLt">
					브랜드리스트의 <strong class="fontred">전시카테고리담당MD</strong> 기준으로 집계됩니다.
					<% If vCompareKey = "o" Then %>
						<br>비교일 대비 일별 증감율이 높을 경우 <strong class="fontred">붉은색</strong>으로, 낮을 경우 <strong class="fontblue">푸른색</strong>으로 표기
					<% End If %>
				</div>
