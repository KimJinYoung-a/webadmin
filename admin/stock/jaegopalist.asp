...
<%
dim page, makerid , stats 
	Page = Request("Page") 						
		if Page = "" then 						
		Page = 1 
		end if
	makerid = html2db(request("makerid"))		
	stats = request("stats")					
...
Sub Drawstats(selectboxname, stats)		
	...
	userquery = userquery + " where statecd<>'' "
...
<script language="javascript">
	...
	function banyoung(frm){
		if (confirm('반영하시겠습니까?')){
		frm.mode.value = 'banyoung'
		frm.submit();
		}
		}
	...
	if (!CheckSelected()){
		...
		var frm;
			...
			var aa;
			aa = window.open("jaegoprint.asp?idx=" +tot, "jaegoprint","width=800,height=600,scrollbars=yes,resizable=yes");
			aa.focus();
	}
	
	function jaegoadd(){
		var edit = window.open("jaegoadd.asp", "jaegoadd" , 'width=1024,height=768,scrollbars=yes,resizable=yes');
		edit.focus();
		}
	
	function jaegoadd_brand(){
		var edit = window.open("jaegoadd_brand.asp", "jaegoadd_brand" , 'width=1024,height=768,scrollbars=yes,resizable=yes');
		edit.focus();
		}	
</script>
...
<table width="100%" align="center" cellpadding="0" cellspacing="0" class="a" style="padding-top:10;">
	<tr>
		<td align="left">
        	<input type="button" class="button" value="신규등록[상품]" onclick="javascript:jaegoadd();">&nbsp;&nbsp;
        	<input type="button" class="button" value="신규등록[브랜드]" onclick="javascript:jaegoadd_brand();">
		</td>
		<td align="right">	
...
<table width="100%" align="center" cellpadding="0" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
	...
	<% for i=0 to oip.FResultCount - 1 %>
		<form action="jaegocheck_process.asp" name="frmBuyPrc<%=i%>" method="get">			<!--for문 안에서 i 값을 가지고 루프-->
		<input type="hidden" name="mode">    
		...
		<!--비고란구분시작 -->	
				<% if oip.flist(i).fbigo = 0 then%>		
					<input type="button" class="button" value="삭제" onclick="DelMe(frmBuyPrc<%=i%>);"> 
				<% elseif oip.flist(i).fbigo =1 then%>
					<input type="button" class="button" value="삭제" onclick="DelMe(frmBuyPrc<%=i%>);"> <!--/ <input type="button" value="재고입력" onclick="insert(frm<%=i%>);">-->
				<% elseif oip.flist(i).fbigo =5 then%>
					<input type="button" class="button" value="삭제" onclick="DelMe(frmBuyPrc<%=i%>);"> <input type="button" value="반영" onclick="banyoung(frmBuyPrc<%=i%>);">
				<% elseif oip.flist(i).fbigo =7 then%>
					<input type="button" class="button" value="삭제" onclick="DelMe(frmBuyPrc<%=i%>);">
				<% elseif oip.flist(i).fbigo =8 then%>
					<input type="button" class="button" value="삭제" onclick="DelMe(frm<%=i%>);"> <input type="button" class="button" value="반영" onclick="banyoung(frm<%=i%>);">
				<% end if %><input type="hidden" value="<%= oip.flist(i).freguserid %>" name="jisiname"><input type="hidden" value="<%= oip.flist(i).fitemgubun %>" name="itemgubun">		
		<!--비고란구분끝 -->
