...
Select Case mode
	Case "add"
		'// 신규 등록
		rsget.Open "Select count(JustDate) from [db_sitemaster].[dbo].tbl_just1day where JustDate='" & justDate & "' And itemid='"&itemid&"' ", dbget, 1
		if rsget(0)>0 then
	...
	End If
		'저스트 원데이 저장
		sqlStr = "Insert Into [db_sitemaster].[dbo].tbl_just1day " &_
	...
	Case "edit"
		'// 내용 수정
		sqlStr = "Update [db_sitemaster].[dbo].tbl_just1day " &_
				...
				" 	,img3='" & img3 & "'" &_
				" 	,img4='" & img4 & "'" &_
				" Where justDate='" & justDate & "' And itemid='"&itemid&"' "
		dbget.Execute(sqlStr)
	...
	Case "delete"
		...
		if justDate>cStr(date()) then
			...
			'저스트원데이 완전 삭제
			sqlStr = sqlStr & "delete [db_sitemaster].[dbo].tbl_just1day " &_
