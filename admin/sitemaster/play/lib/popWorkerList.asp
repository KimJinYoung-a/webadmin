...
{
	var o_wname = opener.document.inputfrm.doc_workername;
	var o_wid = opener.document.inputfrm.selMId;
	var chktemp = opener.document.forms["inputfrm"].elements["selMId"];
...
{
	document.getElementById("temp_workerlist").value = opener.document.inputfrm.doc_workername.value;
}

function goPartSelect(part)
{
	document.location.href = "PopWorkerList.asp?didx=<%=iDoc_Idx%>&worker=<%=sWorker%>&team=" + part + "";
}
</script>
