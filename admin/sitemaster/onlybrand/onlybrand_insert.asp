...
<script type='text/javascript'>
	function jsSubmit(){
		...
		if (frm.bannerlink.value.indexOf("브랜드아이디") > 0){
			alert("배너 링크 값을 확인 해주세요.");
			frm.bannerlink.focus();
			return;
		}

		if (frm.bannerlink.value.indexOf("이벤트번호") > 0){
			alert("배너 링크 값을 확인 해주세요.");
			frm.bannerlink.focus();
			return;
		}
...
function putLinkText(key) {
	...
	switch(key) {
		case 'brand':
			frm.bannerlink.value='/street/street_brand_sub06.asp?makerid=브랜드아이디';
			break;

		case 'event':
			frm.bannerlink.value='/event/eventmain.asp?eventid=이벤트아이디';
			break;
	}
}
...
<table width="900" cellpadding="2" cellspacing="1" class="a" bgcolor="#3d3d3d">
	...
	<tr bgcolor="#FFFFFF" id="tmpstyle1">
		<td bgcolor="#DDDDFF" align="center" width="15%" id="imagetitle">배너 이미지</td>
		<td><input type="button" name="wimg" value="배너 이미지 등록" onClick="jsSetImg('onlyBrand','<%=bannerImg%>','bannerImg','onlyBrandimg')" class="button">
	...
	<tr bgcolor="#FFFFFF" id="tmpstyle2">
		<td bgcolor="#DDDDFF"  align="center" width="15%">배너 링크</td>
		<td>
			...
			- <span style="cursor:pointer" onClick="putLinkText('event')">이벤트 링크 : /event/eventmain.asp?eventid=<font color="darkred">이벤트아이디</font></span><br>
		</td>
	</tr>
	<tr bgcolor="#FFFFFF">
	...
	<tr bgcolor="#FFFFFF">
		<td bgcolor="#FFF999" align="center" width="15%">작업자 요청사항</td>
		<td>
