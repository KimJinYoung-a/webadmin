...
function AutoInsert() {
	...
	var inHtml = "<tr>"
				...
				+ "		<input type='button' id='A"+i+"' value='이미지첨부' onClick=jsPopAddImg('mobile',"+i+"); class='button'>"
				+ "		<input type='hidden' name='imgmobile' value=''>"
				+ "		<span id='divmobile'></span><br><br><font color='blue'>+ map name='add"+i+"Map'</font>"
...
<form name="frmReg" method="post" action="procDF_MImage.asp" onSubmit="return jsDFSubmit();">  	
	...
	<td colspan="2">
		...
		Dim i, is_Empty
			...
			For i = 1 To iTotImgCnt
				If is_Empty <> "x" Then
					IF iDFSeq <> "" THEN strImgUrl =arrImg(24,i,3) : strLink =arrImg(24,i,4)
				End If
		%>
				<tr bgcolor="#FFFFFF">
					<td style="padding:5 5 5 5">
						<input type="button" id="A<%=i%>" value="이미지첨부" onClick="jsPopAddImg('mobile',<%=i%>);" class="button">
						<input type="hidden" name="imgmobile" value="<%=strImgUrl%>">
						<span id="divmobile">
						<%IF strImgUrl <> "" THEN%>  	  
							<a href="javascript:jsImgView('<%=strImgUrl%>')"><img src="<%=strImgUrl%>" border="0" width="200" height="100"></a> <a href="javascript:jsDelImg('mobile',<%=i%>);"><img src='/images/i_delete.gif' border='0'></a>
						<%END IF%>  		  	  
						</span>   
						<br><br>
