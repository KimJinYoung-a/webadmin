...
<%
'####################################################
' Page : /admin/sitemaster/popManageColorCode.asp
' Description :  상품 컬러 코드등록
' History : 2011.04.13 이종화
'####################################################
%>
...
<div style="padding: 0 5 5 5"> <img src="/images/icon_arrow_link.gif" align="absmiddle"> 색상코드 관리</div>
<table width="350" border="0" class="a" cellpadding="3" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
<form name="frmImg" method="post" action="<%= uploadImgUrl %>/linkweb/sitemaster/itemColorCodeProcess.asp" enctype="MULTIPART/FORM-DATA" onSubmit="return jsUpload();">
...
<% end if %>
<tr>
	<td bgcolor="<%= adminColor("tabletop") %>">코드명</td>
