...
<!-- #include virtual="/lib/classes/sitemasterclass/_just1DayCls.asp"-->
<%
'###############################################
...
//	}
	...
	if(frm.mode.value=="add"&&frm.itemOptCnt.value>0&&frm.limitNo.value>0) {
		if(confirm("옵션이 존재하는 상품일 입니다.\n입력하신 한정개수가 옵션에 자동으로 반영되지 않으므로, 시작 전에 상품정보에서 옵션 한정정보를 직접 입력하셔야 정상적으로 판매가 가능합니다.")) {
			if(frm.saleSuplyCash.value*1>frm.salePrice.value*1){
				if(confirm("매입급액이 판매 금액 보다 큽니다. 계속 진행하시겠습니까?")){
					frm.submit();
				}else{
					return;
				}
...
<form name="inputfrm" method="post" action="_doJust1Day_Process.asp">
...
<tr>
	<td align="center" bgcolor="<%= adminColor("tabletop") %>">작업요청사항</td>
	<td bgcolor="#FFFFFF">
		<textarea name="justDesc" class="textarea" cols="80" rows="3"></textarea>
	</td>
</tr>
<!-- 2015-04-24 이상준 대리 요청으로 하단 이미지 입력부분 주석처리 및 상단 간략설명 영역을 작업요청사항 명칭으로 변경함 -->
...
<tr id="saleinfo" style="display:<% If fmainitem.FItemList(0).Fitemdiv="21" Then Response.write "none" %>">
	<td align="center" bgcolor="<%= adminColor("tabletop") %>">할인정보</td>
	<td bgcolor="#FFFFFF">
...
<tr align="center" bgcolor="<%= adminColor("tabletop") %>" height="30">
	...
	<td bgcolor="#FFFFFF" align="left">
		...
		<div align="right" id="image1div"><% IF fmainitem.FItemList(0).Fimg1<>"" THEN %><img src="<%=webImgUrl%>/just1day/<%= fmainitem.FItemList(0).Fimg1 %>" width=50 height=50 ><% End IF %></div>
	</td>
</tr>
<tr align="center" bgcolor="<%= adminColor("tabletop") %>" height="30">
	<td>이미지 #1</td>
	<td bgcolor="#FFFFFF" align="left">
		...
		<div align="right" id="image2div"><% IF fmainitem.FItemList(0).Fimg2<>"" THEN %><img src="<%=webImgUrl%>/just1day/<%= fmainitem.FItemList(0).Fimg2 %>" width=50 height=50 ><% End IF %></div>
	</td>
</tr>
<tr align="center" bgcolor="<%= adminColor("tabletop") %>" height="30">
	<td>이미지 #2</td>
	<td bgcolor="#FFFFFF" align="left">
		...
		<div align="right" id="image3div"><% IF fmainitem.FItemList(0).Fimg3<>"" THEN %><img src="<%=webImgUrl%>/just1day/<%= fmainitem.FItemList(0).Fimg3 %>" width=50 height=50 ><% End IF %></div>
	</td>
</tr>
<tr align="center" bgcolor="<%= adminColor("tabletop") %>" height="30">
	<td>이미지 #3</td>
	<td bgcolor="#FFFFFF" align="left">
		...
		<div align="right" id="image4div"><% IF fmainitem.FItemList(0).Fimg4<>"" THEN %><img src="<%=webImgUrl%>/just1day/<%= fmainitem.FItemList(0).Fimg4 %>" width=50 height=50 ><% End IF %></div>
	</td>
</tr>
