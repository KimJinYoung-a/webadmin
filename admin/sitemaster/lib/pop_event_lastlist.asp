...
<!--
	...
	//부모창에 값 넘기기
	function jsSetEvtCont(cd,title,subtitle,sale,csale,sale2){
		opener.document.frmcontents.Evt_Code<%=num%>.value=cd;
		opener.document.frmcontents.Evt_Title<%=num%>.value=title;
		opener.document.frmcontents.Evt_Subcopy<%=num%>.value=subtitle;
		
		if(sale2!=""&&sale==""&&csale=="")
		{
			opener.document.frmcontents.Evt_Discount<%=num%>.value=sale2;
		}
		else if (sale==""&&csale!=""&&sale2=="")
		{
			opener.document.frmcontents.Evt_Discount<%=num%>.value="~"+csale+"%";
		}
		else if(sale2==""&&sale==""&&csale=="")
		{
			opener.document.frmcontents.Evt_Discount<%=num%>.value="";
		}
		else
		{
			opener.document.frmcontents.Evt_Discount<%=num%>.value="~"+sale+"%";
		}
		 window.close();
	}
...
<tr>
	<td>
		...
		<%IF isArray(arrList) THEN 
			...
			End If
			%>
		<tr bgcolor="#FFFFFF" onClick="jsSetEvtCont(<%=arrList(0,intLoop)%>,'<%=title%>','<%=db2html(arrList(14,intLoop))%>','<%=db2html(arrList(12,intLoop))%>','<%=db2html(arrList(13,intLoop))%>','<%=sale%>');" style="cursor:hand;" onMouseOver="this.style.backgroundColor='#FFFFEC'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
