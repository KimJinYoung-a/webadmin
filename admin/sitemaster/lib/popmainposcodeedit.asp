...
oposcodeList.FRectGubun=gubun
oposcodeList.FRectUsing=useyn
...
function setDefault() {
self.location.href="popmainposcodeedit.asp?gubun=" + document.all("Lgb").value + "&useyn=" + document.all("useyn").value;
}
</script>
...
<tr bgcolor="#FFFFFF">
    ...
    <td>
        <input type="text" name="orderbynum" value="<%= oposcode.FOneItem.Forderbynum %>" maxlength="2" size="5">
    </td>
</tr>
<tr bgcolor="#FFFFFF">
    <td width="150" bgcolor="#DDDDFF">그룹(필드명gubun)</td>
    <td>
    	<% call DrawGroupGubunCombo("gubun", oposcode.FOneItem.Fgubun, "") %>
    </td>
</tr>
<tr bgcolor="#FFFFFF">
...
<table width="660" cellpadding="2" cellspacing="1" class="a" bgcolor="#3d3d3d">
<tr bgcolor="#FFFFFF">
    <td colspan="2"><% call DrawGroupGubunCombo("Lgb", gubun, "onchange='setDefault();'") %></td>
    <td colspan="4" align="right">
    	<select name="useyn" class="select" onchange='setDefault();'>
    	<option value="Y" <%=chkIIF(useyn="Y","selected","")%>>사용</option>
    	<option value="N" <%=chkIIF(useyn="N","selected","")%>>사용안함</option>
    	</select> &nbsp; / &nbsp;
    	<a href="?poscode="><img src="/images/icon_new_registration.gif" border="0" align="absbottom"></a>
    </td>
</tr>
<tr bgcolor="#DDDDFF">
...
<% end if %>
    ...
    <td ><a href="?poscode=<%= oposcodeList.FItemList(i).FposCode %>&gubun=<%=gubun%>&useyn=<%=useyn%>&page=<%= page %>"><%= oposcodeList.FItemList(i).FposName %></a></td>
...
<tr bgcolor="#FFFFFF">
    ...
    <% if oposcodeList.HasPreScroll then %>
		<a href="?gubun=<%=gubun%>&useyn=<%=useyn%>&page=<%= oposcodeList.StarScrollPage-1 %>">[pre]</a>
	<% else %>
		[pre]
	<% end if %>
	...
	<% for i=0 + oposcodeList.StarScrollPage to oposcodeList.FScrollCount + oposcodeList.StarScrollPage - 1 %>
		...
		<% else %>
		<a href="?gubun=<%=gubun%>&useyn=<%=useyn%>&page=<%= i %>">[<%= i %>]</a>
		<% end if %>
	<% next %>

	<% if oposcodeList.HasNextScroll then %>
		<a href="?gubun=<%=gubun%>&useyn=<%=useyn%>&page=<%= i %>">[next]</a>
	<% else %>
		[next]
	<% end if %>
    </td>
