...
If mode = "add" Then '//날짜 체크
	Dim itemcount
	sqlStr = "select count(*) from [db_sitemaster].[dbo].tbl_pc_main_just1day_list where startdate = '"& startdate &"' and isusing = 'Y'" 
...
Select Case mode
	 Case "subadd"
		'subitem 신규 등록
		sqlStr = "Insert Into [db_sitemaster].[dbo].tbl_pc_main_just1day_item " &_
	 ...
	 Case "submodify"
		'subitem 수정
		sqlStr = "Update [db_sitemaster].[dbo].tbl_pc_main_just1day_item " &_
				" Set itemid='" & itemid & "'" &_
				" 	,sortnum='" & sortnum & "'" &_
				" 	,isusing='" & isusing & "'" &_
				" Where subidx=" & subidx
...
'		response.end
	...
	Case "modify"
		'내용 수정
		sqlStr = "Update [db_sitemaster].[dbo].tbl_pc_main_just1day_list " &_
				" Set title='" & title & "'" &_
