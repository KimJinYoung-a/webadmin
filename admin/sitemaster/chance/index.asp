...
<!-- 검색 시작 -->
<form name="frm" method="post" style="margin:0px;" action="/admin/sitemaster/chance/index.asp">
...
<tr  height="30" align="center" bgcolor="<%=chkIIF(just1DayList.FItemList(i).Fisusing="Y","#FFFFFF","#F0F0F0")%>">
    ...
    <td onclick="jsmodify('<%=just1DayList.FItemList(i).Fidx%>');" style="cursor:pointer;">
		<%
			Select Case Trim(just1DayList.FItemList(i).Fis1day)
				Case "Y"
					response.write "just1day"
				Case "H"
					response.write "연휴특가"
				Case "W"
					response.write "주말특가"
