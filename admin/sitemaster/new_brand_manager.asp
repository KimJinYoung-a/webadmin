...
dim oMainContents
	...
	oMainContents.GetMainNewBrandList
...
function AddNewMainContents(idx){
    var popwin = window.open('/admin/sitemaster/lib/popmainnewbrandedit.asp?idx=' + idx+'&<%=strParm%>','mainposcodeedit','width=800,height=600,scrollbars=yes,resizable=yes');
    popwin.focus();
}
...
<% end if %>
    ...
    <td align="center">
	<%= oMainContents.FItemList(i).FBrandID %> [<%= oMainContents.FItemList(i).FBrandName %>]
	</td>
    <td align="center">
    	<a href="javascript:AddNewMainContents('<%= oMainContents.FItemList(i).Fidx %>');"><img src="<%= oMainContents.FItemList(i).FMain_Image %>" width="182" border="0"></a>
    </td>
	<td align="center"><%= oMainContents.FItemList(i).FMainCopy %></td>
