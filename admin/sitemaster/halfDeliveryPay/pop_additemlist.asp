...
frm.sType.value = sType;
	   	    	...
	   	    	for(i=0;i<frm.chkitem.length;i++){
	   	    		if(frm.chkitem[i].checked) {
						ck=ck+1;	   	    			
	   	    			...
	   	    			if (frm.itemdataarr.value==""){
	   	    			 frm.itemdataarr.value =  frm.viewitemdata[i].value;
	   	    			}else{
	   	    			 frm.itemdataarr.value = frm.itemdataarr.value + "," +frm.viewitemdata[i].value;
	   	    			}						   
	   	    		}	
	   	    	}
	...
	//alert(frm.itemidarr.value);
	//alert(frm.itemdataarr.value);	
	document.domain ="10x10.co.kr";
	setTimeout(() => {
		opener.document.all.viewitemdataparent.value = frm.itemdataarr.value;
	}, 50);	
	setTimeout(() => {
		opener.parent.viewUserAddItemListData();
		window.close();	
	}, 100);		
...
<form name="frm" method=post>	
	...
	<input type="hidden" name="itemdataarr" >
...
<% if oitem.FresultCount > 0 then %>
	...
	<input type="hidden" name="viewitemdata" value="<%= oitem.FItemList(i).FSmallImage&"|"&oitem.FItemList(i).FItemId&"|"&oitem.FItemList(i).Fmakerid&"|"&replace(oitem.FItemList(i).Fitemname,","," ")&"|"&getBeadalDivname(oitem.FItemList(i).FdefaultDeliveryType)&"|"&oitem.FItemList(i).FdefaultFreeBeasongLimit&"|"&oitem.FItemList(i).FdefaultDeliverPay %>">
