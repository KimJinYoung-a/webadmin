...
'           2019.12.11 원승현 저스트원데이 전용 이벤트 리스트 팝업창 생성
'####################################################
%>
...
<!--
	...
	function jsSetEvtCont(cd,title,subtitle,sale,csale,sale2){
		...
		opener.document.frm.linkurl<%=num%>.value='/event/eventmain.asp?eventid='+cd;
		opener.document.frm.title<%=num%>.value=title;
		opener.document.frm.workertext<%=num%>.value=subtitle;
		
		if(sale2!=""&&sale==""&&csale=="")
		{
			opener.document.frm.saleper<%=num%>.value=sale2;
		}
		else if (sale==""&&csale!=""&&sale2=="")
		{
			opener.document.frm.saleper<%=num%>.value="~"+csale+"%";
		}
		else if(sale2==""&&sale==""&&csale=="")
		{
			opener.document.frm.saleper<%=num%>.value="";
		}
		else
		{
			opener.document.frm.saleper<%=num%>.value="~"+sale+"%";
		}
		 window.close();
	}
