...
<%
	...
	Dim sEvt,strTxt,sKind , gcode ,  sDt , eDt
	...
	set cEvtList = new ClsEvent	
		...
		cEvtList.FSeTxt = strTxt '검색어	
 		...
 		arrList = cEvtList.fnGetEventLastList_v2	'데이터목록 가져오기
...
<!--
	...
	//부모창에 값 넘기기
	function jsSetEvtCont(evt_code, evt_copy, bnr_img){		
	  if(typeof(opener.document) == "object"){
		if(confirm("추가 하시겠습니까?")){
			var frm = opener.document.frm  
			frm.evt_code.value = evt_code;
			frm.evttitle.value = evt_copy.split('|')[0];
			frm.evtimg.value = bnr_img;	
			window.opener.document.getElementById("evtimgsrc").src = bnr_img;
			window.close();
		}		
	  }	
	}
...
<input type="hidden" name="gcode" value="<%=request("gcode")%>">
<tr>
	<td>
...
<tr>
	<td>
		...
		<%IF isArray(arrList) THEN 
			...
			%>
		<tr bgcolor="#FFFFFF" onClick="jsSetEvtCont('<%=arrList(0,intLoop)%>', '<%=db2html(arrList(4,intLoop))%>', '<%=arrList(12,intLoop)%>');" style="cursor:hand;" onMouseOver="this.style.backgroundColor='#FFFFEC'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
