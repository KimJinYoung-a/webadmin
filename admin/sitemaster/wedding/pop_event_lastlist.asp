...
<%
	...
	dim blnWeb, blnMobile, blnApp, isWeb, isMobile, isApp, num, title, sale,arrtitle
...
<!--
	...
	//부모창에 값 넘기기
	function jsSetEvtCont(cd,title,subtitle,sale,csale,startdate,enddate){
		opener.document.frmcontents.Evt_Code.value=cd;
		opener.document.frmcontents.Evt_Title.value=title;
		opener.document.frmcontents.Evt_Subcopy.value=subtitle;
		if (csale!="0" && csale!="")
		{
			opener.document.frmcontents.Evt_Coupon.value="~"+csale+"%";
		}
		if (sale!="0" && sale!="")
		{
			opener.document.frmcontents.Evt_Discount.value="~"+sale+"%";
		}
		opener.document.frmcontents.StartDate.value=startdate;
		opener.document.frmcontents.EndDate.value=enddate;
		window.close();
	}
...
<tr>
	<td>
		...
		<%IF isArray(arrList) THEN 
			...
			If (instr(db2html(arrList(4,intLoop)),"|")>0) Then
				arrtitle = split(db2html(arrList(4,intLoop)),"|")
				title = arrtitle(0)
				sale = arrtitle(1)
			Else
			...
			End If
			%>
		<tr bgcolor="#FFFFFF" onClick="jsSetEvtCont(<%=arrList(0,intLoop)%>,'<%=db2html(title)%>','<%=db2html(arrList(14,intLoop))%>','<% If isNull(arrList(12,intLoop)) Then %>0<% Else %><%=db2html(arrList(12,intLoop))%><% End If %>','<% If isNull(arrList(13,intLoop)) Then %>0<% Else %><%=db2html(arrList(13,intLoop))%><% End If %>','<%=arrList(5,intLoop)%>','<%=arrList(6,intLoop)%>');" style="cursor:hand;" onMouseOver="this.style.backgroundColor='#FFFFEC'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
