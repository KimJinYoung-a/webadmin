...
function selectThis(makerid, dgncomment, brandeng, brandkor){
    //alert(makerid);
    opener.<%= frmName %>.makerid.value = makerid;
    opener.<%= frmName %>.subcopy.value = dgncomment;
    opener.<%= frmName %>.bannernameeng.value = brandeng;
    opener.<%= frmName %>.bannernamekor.value = brandkor;
    opener.<%= frmName %>.linkurl.value = "/street/street_brand_sub06.asp?makerid="+makerid;
    window.close();
}
</script>
...
<table width="100%" border="0" align="center" class="a" cellpadding="2" cellspacing="1" bgcolor="<%= adminColor("tablebg") %>">
	...
	<% end if %>
		...
		<td><a href="javascript:selectThis('<%= opartner.FPartnerList(i).Fid %>','<%= replace(opartner.FPartnerList(i).Fdgncomment,"'","`") %>','<%= replace(opartner.FPartnerList(i).Fsocname,"'","`") %>','<%= opartner.FPartnerList(i).Fsocname_kor %>')"><%= opartner.FPartnerList(i).FID %></a></td>
		<td>
			<%= opartner.FPartnerList(i).FSocName_Kor %><br>
			<%= opartner.FPartnerList(i).FSocName %>
		</td>
		<td><a href="javascript:selectThis('<%= opartner.FPartnerList(i).Fid %>','<%= replace(opartner.FPartnerList(i).Fdgncomment,"'","`") %>','<%= replace(opartner.FPartnerList(i).Fsocname,"'","`") %>','<%= opartner.FPartnerList(i).Fsocname_kor %>')"><%= opartner.FPartnerList(i).Fcompany_name %></a></td>
