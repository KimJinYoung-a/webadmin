...
function choiceevt(evtidx){
	opener.eval('document.all.divsub'+<%=num%>).innerHTML = "기획전코드 & 상품코드 : <input type='text' name='gubunvalue' value='"+evtidx+"' size=10 maxlength=10>";
	self.close();
}
...
<% end if %>
	...
	<td align="center">
		<input type="button" class="button" value="선택" onclick="choiceevt('<%= oevent.FItemList(i).Fevtidx %>');">		
	</td>
</tr>
<% next %>
<tr>
