...
<%
dim idx,title,subcopy,state,banner_img,startdate,enddate,isusing,regdate,comment
...
'//이벤트정보
set oevent = new ClsStyleLife
	oevent.frectidx = idx
	
	if idx <> "" then
		oevent.fnGetTheme_item()
...
function jsSerach(){
	...
	frm = document.frm;
	frm.target = "_self";
	frm.action ="stylelife_theme_item.asp";
	frm.submit();
}
...
function NextPage(ipage){
	...
	document.frm.action ="stylelife_theme_item.asp";
	document.frm.submit();
}

//새상품추가
function addevtnewItem(idx,cd1){
	var addevtnewItem = window.open('/admin/stylepick/pop_themeitemAddInfo.asp?idx='+idx+'&cd1='+cd1+'&menupos=<%=menupos%>','addevtnewItem','width=1024,height=768,scrollbars=yes,resizable=yes');
	addevtnewItem.focus();
}

//이전페이지로
function backpage(idx){
	location.href="/admin/stylepick/stylelife_theme.asp?menupos=<%=menupos%>";
}
...
<tr bgcolor="#FFFFFF" align="center">
	<td bgcolor="<%= adminColor("tabletop") %>">테마번호</td>
	<td><%= idx %></td>
	<td bgcolor="<%= adminColor("tabletop") %>">스타일</td>
	<td><%= catename %></td>
...
<tr>
	<td align="left">
		<% if state <> "9" then %>
			<input type="button" value="상품추가" onclick="addevtnewItem('<%=idx%>','<%=cd1%>');" class="button">
			<input type="button" value="선택상품삭제" onClick="SelectItemsdel()" class="button">
			<input type="button" value="테마목록으로" onclick="backpage();" class="button">
		<% end if %>
	</td>
	<td align="right">			
	</td>
...
<tr align="center" bgcolor="#FFFFFF" onmouseout="this.style.backgroundColor='#FFFFFF'" onmouseover="this.style.backgroundColor='#F1F1F1'">
	...
	<td align="center">
		<%= CHKIIF(oitem.FItemList(i).fcd1="0P0","스타일픽",oitem.FItemList(i).fcd1name) %><%= oitem.FItemList(i).fcd2name %><%= oitem.FItemList(i).fcd3name %>
