...
<%
dim mode , sqlstr , i , menupos ,idx,cd1 , title, subcopy, state, startdate, enddate
dim partMDid ,partWDid , isusing , comment ,weekly_banner_img , weekly_title_img, lastadminid ,opendate ,closedate
dim strAdd ,itemidarr ,totalcount ,tmpitemid , tmpitem ,evtitemidxarr
	...
	weekly_banner_img = request("weekly_banner_img")
	weekly_title_img = request("weekly_title_img")
...
if mode = "eventedit" then
	...
	if idx = "" then
		...
		sqlstr = "insert into db_giftplus.dbo.tbl_stylelife_weekly" + vbcrlf
		sqlstr = sqlstr & " (title, cd1, state ,banner_img , title_img,startdate ,comment" + vbcrlf
		sqlstr = sqlstr & " ,lastadminid , partMDid ,partWDid) values (" + vbcrlf
		sqlstr = sqlstr & " '"&html2db(title)&"', '" & cd1 & "',"&state&",'"&html2db(weekly_banner_img)&"','"&html2db(weekly_title_img)&"'" + vbcrlf
		sqlstr = sqlstr & " ,'"&html2db(startdate)&" 00:00:00', '"&html2db(comment)&"'" + vbcrlf
		sqlstr = sqlstr & " ,'"&lastadminid&"','"&partMDid&"','"&partWDid&"'" + vbcrlf
		sqlstr = sqlstr & " )"
	...
	'//수정
		...
		sqlstr = "update db_giftplus.dbo.tbl_stylelife_weekly set" + vbcrlf
		sqlstr = sqlstr & " title = '"&html2db(title)&"'" + vbcrlf
		sqlstr = sqlstr & " ,cd1 = '" & cd1 & "'" + vbcrlf
		sqlstr = sqlstr & " ,state = '"&state&"'" + vbcrlf
		sqlstr = sqlstr & " ,banner_img = '"&html2db(weekly_banner_img)&"'" + vbcrlf
		sqlstr = sqlstr & " ,title_img = '"&html2db(weekly_title_img)&"'" + vbcrlf
...
elseif mode = "cimage" then
	
	sqlstr = "delete db_giftplus.dbo.tbl_stylelife_weekly_img where idx = '" & idx & "' " & vbCrLf
	For i=0 To 30
		If IsNull(Request("c"&idx&"weekly_content_img"&i&"")) Then
			Exit For
		Else
			If Request("c"&idx&"weekly_content_img"&i&"") <> "" Then
				sqlstr = sqlstr & "insert into db_giftplus.dbo.tbl_stylelife_weekly_img(idx,imgurl) values('" & idx & "','" & Request("c"&idx&"weekly_content_img"&i&"") & "') " & vbCrLf
			End IF
		End IF
	Next
	dbget.Execute sqlStr
	...
	'response.write "	location.href='/admin/stylepick/stylelife_weekly_CImg.asp?idx="&idx&"&menupos="&menupos&"';"
	response.write "	location.href='" & wwwUrl & "/chtml/stylelife/make_stylelife_weekly.asp?idx="&idx&"';"
	response.write	"</script>"
	dbget.close()	:	response.End
end if	
%>	
