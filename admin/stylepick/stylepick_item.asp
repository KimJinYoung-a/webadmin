...
<%
Dim cd1, cd2 ,cd3,i,page,isusing ,oitem,deliverytype,sailyn,couponyn
...
function SelectItemsdel(){	
	...
	if(typeof(frm.chkitem) !="undefined"){
		if(!frm.chkitem.length){
			...
			frm.itemidxarr.value = frm.chkitem.value;
			itemcount = 1;
		}else{
		
			for(i=0;i<frm.chkitem.length;i++){
				if(frm.chkitem[i].checked) {	   	    			
					if (frm.itemidxarr.value==""){
						frm.itemidxarr.value = frm.chkitem[i].value;				
					}else{
						frm.itemidxarr.value = frm.itemidxarr.value + "," +frm.chkitem[i].value;
					} 
	...
	frm.mode.value = "itemdel";
	frm.submit();
	frm.itemidxarr.value = "";
...
function category(){
	var category = window.open('/admin/stylepick/stylepick_category.asp?menupos=<%=menupos%>','category','width=1024,height=768,scrollbars=yes,resizable=yes');
	category.focus();
}
...
function NextPage(ipage){
	...
	document.frm.action ="stylepick_item.asp";
	document.frm.submit();
}

//새상품추가
function addnewItem(){
	var addnewItem = window.open('/admin/stylepick/pop_itemAddInfo.asp','addnewItem','width=1024,height=768,scrollbars=yes,resizable=yes');
	addnewItem.focus();
}
...
<tr>
	...
	<td align="right">	
		<input type="button" class="button" value="카테고리등록" onclick="category();">
	</td>
</tr>
</table>
<!-- 액션 끝 -->
...
<% end if %>
	...
	<td align="center">webadmin.10x10.co.krem.FItemList(i).Flimityn,"yn") %></td>
