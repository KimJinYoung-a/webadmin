...
function addItem(itemid, itemimg, itemname, saleper, couponPer){		
	...
	// console.log(itemimg);
	// return false;
	couponPer = couponPer == 0 ? "" : 
						  couponPer > 100 ? "쿠폰" + couponPer  : "쿠폰" + couponPer + "%"
		...
		frm.coupon_per.value=couponPer
...
%>
	...
	<tr class="a" onclick="addItem('<%=oitem.FItemList(i).FItemId%>','<%=itemImg%>','<% =oitem.FItemList(i).Fitemname %>', '<%=chkIIF( tempSalePer = 0, "", tempSalePer&"%")%>', '<%=oitem.FItemList(i).FitemCouponValue%>');" height="25" bgcolor="#FFFFFF" style="cursor:pointer;" onmouseover="this.style.backgroundColor='#D8D8D8'" onmouseout="this.style.backgroundColor=''">	
