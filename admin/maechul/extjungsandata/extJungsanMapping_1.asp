...
<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
	...
	<Tr bgcolor="#ffffff" align="center">  
		...
		<td colspan="3" width="15%">scm <%=nextDate%></td>
		<td colspan="3" width="15%">scm 미매칭</td>
		<td colspan="3" bgcolor="#eeeeee"><b>scm 합계</b></td>
	</tr>	
	<Tr bgcolor="#ffffff" align="center">     
		<td>상품</td>
		<td>배송비</td>
	...
	<tr bgcolor="#e3f1fb" align="right">
		...
		<td><a href="javascript:jsGoDetail('<%=sellsite%>','I','<%=left(nextDate,4)%>','<%=mid(nextDate,6,2)%>','<%=left(arrList(0,intLoop),4)%>','<%=mid(arrList(0,intLoop),6,2)%>');">
			<span <%if arrList(3,intLoop)<>arrList(7,intLoop) then%>style ="color:red"<%end if%>><%=formatnumber(arrList(3,intLoop),0)%></span></a>
		</td>
		<td><a href="javascript:jsGoDetail('<%=sellsite%>','D','<%=left(nextDate,4)%>','<%=mid(nextDate,6,2)%>','<%=left(arrList(0,intLoop),4)%>','<%=mid(arrList(0,intLoop),6,2)%>');">
			<span <%if arrList(11,intLoop)<>arrList(15,intLoop) then%>style ="color:red"<%end if%>><%=formatnumber(arrList(11,intLoop),0)%></span></a>
		</td>
		<td ><a href="javascript:jsGoDetail('<%=sellsite%>','A','<%=left(nextDate,4)%>','<%=mid(nextDate,6,2)%>','<%=left(arrList(0,intLoop),4)%>','<%=mid(arrList(0,intLoop),6,2)%>');">
			<span <%if (arrList(3,intLoop)+arrList(11,intLoop))<>(arrList(7,intLoop)+arrList(15,intLoop)) then%>style ="color:red"<%end if%>><%=formatnumber( (arrList(3,intLoop)+arrList(11,intLoop)),0)%></span></a>
		</td>	
	...
	<tr bgcolor="#ffDDDD" align="right">   
	  ...
	  <td>
		...
		<td><a href="javascript:jsGoDetail('<%=sellsite%>','I','<%=left(nextDate,4)%>','<%=mid(nextDate,6,2)%>','<%=left(arrList(0,intLoop),4)%>','<%=mid(arrList(0,intLoop),6,2)%>');">
			<span <%if arrList(3,intLoop)<>arrList(7,intLoop) then%>style ="color:red"<%end if%>><%=formatnumber(arrList(7,intLoop),0)%></span></a>
		</td>
		<td><a href="javascript:jsGoDetail('<%=sellsite%>','D','<%=left(nextDate,4)%>','<%=mid(nextDate,6,2)%>','<%=left(arrList(0,intLoop),4)%>','<%=mid(arrList(0,intLoop),6,2)%>');">
			<span <%if arrList(11,intLoop)<>arrList(15,intLoop) then%>style ="color:red"<%end if%>><%=formatnumber(arrList(15,intLoop),0)%></span></a>
		</td>
		<td>
			<a href="javascript:jsGoDetail('<%=sellsite%>','A','<%=left(nextDate,4)%>','<%=mid(preDate,6,2)%>','<%=left(arrList(0,intLoop),4)%>','<%=mid(arrList(0,intLoop),6,2)%>');">
			<span <%if (arrList(3,intLoop)+arrList(7,intLoop))<>(arrList(11,intLoop)+arrList(15,intLoop)) then%>style ="color:red"<%end if%>><%=formatnumber(arrList(11,intLoop)+arrList(15,intLoop),0)%></span></a>
		</td>
		<td><a href="javascript:jsGoDetail('<%=sellsite%>','I','','','<%=left(arrList(0,intLoop),4)%>','<%=mid(arrList(0,intLoop),6,2)%>');">
	...
	<tr bgcolor="#e3f1fb" align="right">
		...
		<td><span <%if scm4 <> om4 then%>style ="color:red"<%end if%>><strong><%=formatnumber(scm4,0)%></strong></span></td>
		<td><span <%if scm4_d <> om4_d then%>style ="color:red"<%end if%>><strong><%=formatnumber(scm4_d,0)%></strong></span></td>
		<td><span <%if (scm4+scm4_d) <> (om4+om4_d) then%>style ="color:red"<%end if%>><strong><%=formatnumber(scm4+scm4_d,0)%></strong></span></td>
	...
	<tr  bgcolor="ffdddd" align="right">
		...
		<td><span <%if scm4 <> om4 then%>style ="color:red"<%end if%>><strong><%=formatnumber(om4,0)%></strong></span></td>
		<td><span <%if scm4_d <> om4_d then%>style ="color:red"<%end if%>><strong><%=formatnumber(om4_d,0)%></strong></span></td>
		<td><span <%if (scm4+scm4_d) <> (om4+om4_d) then%>style ="color:red"<%end if%>><strong><%=formatnumber(om4+om4_d,0)%></strong></span></td>	
