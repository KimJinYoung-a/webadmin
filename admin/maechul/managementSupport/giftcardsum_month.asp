...
function pop_sell_day(yyyy1, mm1, dd1, yyyy2, mm2, dd2){
	var pop_sell_day = window.open('/admin/maechul/managementsupport/giftcardsum_day.asp?yyyy1='+yyyy1+'&mm1='+mm1+'&dd1='+dd1+'&yyyy2='+yyyy2+'&mm2='+mm2+'&dd2='+dd2+'&onoffgubun=<%=onoffgubun%>&menupos=<%=menupos%>','pop_sell_day','width=1024,height=768,scrollbars=yes,resizable=yes');
	pop_sell_day.focus();
}
...
<tr bgcolor="#FFFFFF" align="center" onmouseover=this.style.background="#f1f1f1"; onmouseout=this.style.background="#FFFFFF";>
	...
	<td>
		<a href="javascript:pop_sell_list('<%= left(csell.fitemlist(i).fYYYYMM,4) %>','<%= mid(csell.fitemlist(i).fYYYYMM,6,2) %>','01','<%= left(csell.fitemlist(i).fYYYYMM,4) %>','<%= mid(csell.fitemlist(i).fYYYYMM,6,2) %>','<%= LastDayOfThisMonth(left(csell.fitemlist(i).fYYYYMM,4),mid(csell.fitemlist(i).fYYYYMM,6,2)) %>','');" onfocus="this.blur()">
