...
<tr height="25" bgcolor="FFFFFF">
	<td colspan="25">
		검색결과 : <b><%= csell.FresultCount %></b> ※ 총 200건까지 검색 됩니다.
	</td>
</tr>
<tr bgcolor="<%= adminColor("tabletop") %>" align="center">
    <td height="25" width="60" rowspan="2">날짜</td>
    <td rowspan="2">이월잔액<br>(A)</td>
    <td rowspan="2">ON<br>적립액(B)</td>
	<td rowspan="2">OFF-&gt;ON<br>전환적립(C)</td>
    <td colspan="2">고객사용액<br>(D)</td>
    <td rowspan="2">회원탈퇴<br>(E)</td>
    <td rowspan="2">소멸<br>(F)</td>
    <td rowspan="2">월변동<br>(B+C+D+E+F)</td>
    <td rowspan="2">월잔액(R)</td>
	<td rowspan="2">사용률<br>(S=D/(B+C))</td>
	<td rowspan="2">최근60개월<br>사용률<br>(V)</td>

	<td rowspan="2">충당액<br>(X=R*V)</td>
	<td rowspan="2">원가율<br>(Y)</td>
	<td rowspan="2">최종충당액<br>(Z=X*Y)</td>
	<td rowspan="2">월별충당액</td>
...
<%
dim totbeforeremainpoint, totgainpoint, totspendpoint, totofflineshiftpoint, totuseroutpoint, totdelpoint, totremaincash, totacademyspendpoint
...
totacademyspendpoint = totacademyspendpoint + csell.fitemlist(i).FacademySpendPoint
...
<tr bgcolor="#FFFFFF" align="center" onmouseover=this.style.background="#f1f1f1"; onmouseout=this.style.background="#FFFFFF";>
	...
	<td>
		...
		<%= FormatNumber(csell.fitemlist(i).fspendpoint-csell.fitemlist(i).FacademySpendPoint,0) %></a>
	</td>
	<td>
	    <%= FormatNumber(csell.fitemlist(i).FacademySpendPoint,0) %>
	    </td>
	<td>
	...
	<td>
		<%= FormatNumber(csell.fitemlist(i).fremaincash-csell.fitemlist(i).fbeforeremainpoint,0) %>
	</td>
	<td>
	...
	<input type="hidden" name="mode" value="modOnCostpricepercent">
...
<tr bgcolor="<%= adminColor("tabletop") %>" align="center">
    ...
    <td><%= FormatNumber(totspendpoint-totacademyspendpoint,0) %></td>
    <td><%= FormatNumber(totacademyspendpoint,0) %></td>
    <td><%= FormatNumber(totuseroutpoint,0) %></td>
    <td><%= FormatNumber(totdelpoint,0) %></td>
    <td><%= FormatNumber(totremaincash-totbeforeremainpoint,0) %></td>
