...
if (mode="getonpgdata") then
    ...
    nextDT = replace(dateadd("d",1,LEFT(yyyymmdd,4)&"-"&MID(yyyymmdd,5,2)&"-"&MID(yyyymmdd,7,2)),"-","")
	...
	if (InStr(objData, "NO DATA") > 0) then
		...
		response.write "<br><a href='?mode=getonpgdata&yyyymmdd="&nextDT&"'>"&nextDT&"</a>"
		response.end
	end if
	...
	'// ========================================================================
	sqlStr = " delete from db_datamart.dbo.tbl_TMP_onlineApp_log_tmp "
	...
	for i = 0 to UBound(objData)
		...
		if (objLine(0) = "B") then
			...
			sqlStr = " insert into db_datamart.dbo.tbl_TMP_onlineApp_log_tmp(PGgubun, PGkey, PGCSkey, sitename, appDivCode, appMethod, appDate, cancelDate, appPrice, commPrice, commVatPrice, jungsanPrice, ipkumdate, PGuserid) "
	...
	sqlStr = " insert into db_datamart.dbo.tbl_TMP_onlineApp_log(PGgubun, PGkey, PGCSkey, sitename, appDivCode, appMethod, appDate, cancelDate, appPrice, commPrice, commVatPrice, jungsanPrice, ipkumdate, PGuserid, PGmeachulDate) "
	...
	sqlStr = sqlStr + " from "
	sqlStr = sqlStr + " 	db_datamart.dbo.tbl_TMP_onlineApp_log_tmp t "
	sqlStr = sqlStr + " 	left join db_datamart.dbo.tbl_TMP_onlineApp_log l "
	sqlStr = sqlStr + " 	on "
	sqlStr = sqlStr + " 		1 = 1 "
...
elseif (TRUE) then
    response.write "TT-"&mode&":미지정"
    response.end
elseif (mode="matchoneorder") then
	...
	if Not(db3_rsget.EOF or db3_rsget.BOF) then
	    IsMatched = Not (db3_rsget("orderserial") = "")
	end if
	db3_rsget.Close
...
elseif (mode="addActLog") then
    ...
    db3_rsget.Open sqlStr,db3_dbget,1
	if Not(db3_rsget.EOF or db3_rsget.BOF) then
	    PGCSkey = "-" + Format00(3, db3_rsget("cnt"))
	end if
	db3_rsget.Close
...
elseif (mode="getpaycoT") Then
	...
	'// ========================================================================
	sqlStr = " delete from db_datamart.dbo.tbl_onlineApp_log_tmp "
	...
	for i = 0 to UBound(objData)
		...
		If (UBound(objLine) > 0) Then
			If (IsNumeric(objLine(0))) Then
				...
				sqlStr = sqlStr + " 	from db_datamart.dbo.tbl_onlineApp_log_tmp "
...
elseif (mode="getpaycoS") Then
	...
	sqlStr = sqlStr + " 	and l.PGgubun = t.PGgubun "
	sqlStr = sqlStr + " 	and l.PGkey = t.PGkey "
	sqlStr = sqlStr + " 	and l.appDivCode = t.appDivCode "
	...
	if  (Not IsAutoScript) then
		response.write "<script>alert('정산일자 : " + CStr(yyyymmdd) + " [9]');</script>"
	end If
...
elseif (mode="getonpgdatauplus") then
	...
	if (yyyymmdd = "") then
		...
		db3_rsget.Open sqlStr,db3_dbget,1
		if Not(db3_rsget.EOF or db3_rsget.BOF) then
			if Not IsNull(db3_rsget("lastipkumdate")) then
				lastipkumdate = db3_rsget("lastipkumdate")
			end if
		end if
		db3_rsget.Close
		...
		if (UBound(objLine) >= 0) then
			if (objLine(0) = "D") then
				...
				if (i >= 1) then
					...
					if (IsDuplicate = True) Or (PGkey = "16010512377") Or (PGkey = "16010731258") Or (PGkey = "16062463881") then
						sqlStr = " select count(*) as cnt "
						sqlStr = sqlStr + " from "
						...
						if Not(db3_rsget.EOF or db3_rsget.BOF) Then
							If db3_rsget("cnt") > 0 Then
								subPgKey = "-" & Format00(2, db3_rsget("cnt"))
							End If
						end if
						db3_rsget.Close
...
elseif (mode = "matchpgdata") then
	...
	sqlStr = sqlStr + " 	and l.PGkey in ('INIMX_CARDteenxteen920160228010833251223', 'INIMX_CARDteenxteen920151222001139501963', 'INIMX_CARDteenxteen920150818001110200385', 'INIMX_CARDteenxteen920150826221741101162','IniTechPG_teenxteen420160329142003393763') "
	...
	'sqlStr = sqlStr + " 	and l.canceldate >= '2016-11-01' "		'/날짜도 바꿔주고
...
elseif (mode = "makeadvprc") then
	...
	if not db3_rsget.EOF  then
		response.write "중복 ERROR : " & db3_rsget("pgkey")
		db3_rsget.close
		db3_dbget.close()
		response.end
	end if
...
//location.replace('<%= refer %>');
    ...
    <% if (mode="getonpgdata") then %>
    <a href="?mode=getonpgdata&yyyymmdd=<%=nextDT%>"><%=nextDT%></a>
    <% end if %>
<% end if %>
