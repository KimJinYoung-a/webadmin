...
function jsSubmit(){
	...
	if ($("input[name=gubun]:checked").val() == 'partcancel') {
		if ($("input[name=orgpgcskey]").val() == '') {
			alert('PG사CSKEY 를 입력하세요.');
			return;
		}
	...
	if ($("input[name=gubun]:checked").val() == 'del') {
		if ($("input[name=orgpgcskey]").val() == '') {
			alert('PG사CSKEY 를 입력하세요.');
			return;
		}
...
<input type="hidden" name="mode" value="addhandOn" />
...
<tr align="center" bgcolor="#FFFFFF" height="30">
	<td width="100" bgcolor="<%= adminColor("tabletop") %>">원PG사CSKEY:</td>
	<td align="left">
		<input type="text" class="text" name="orgpgcskey" size="32">
	</td>
</tr>
<tr align="center" bgcolor="#FFFFFF" height="30">
	...
	<td align="left">
		...
		-->
		<input type="radio" name="gubun" value="partcancel"> 금액 0원 부분취소건
		&nbsp;
