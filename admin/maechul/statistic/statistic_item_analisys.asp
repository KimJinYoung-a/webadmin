...
response.write "page moved..<br><br>"
response.write "&nbsp;&nbsp;&nbsp;<a href='statistic_item_dw.asp?"&request.serverVariables("QUERY_STRING")&"'>go &gt;&gt;</a>"
response.end
%>
	...
	<td align="left">
		...
		<tr>
			<td colspan="4"> 리스트타입:
			    ...
			    &nbsp;&nbsp;
			    <input type="checkbox" name="chkchn" value="1" <%=CHKIIF(chkchn="1","checked","")%>>채널상세보기
			    &nbsp;
				...
				&nbsp;
				회사명 <input type="text" name="crect" value="<%= crect %>" Maxlength="32" size="20">
			</td>
		</tr>
	    </table>
	</td>
...
<table width="100%" align="center" cellpadding="2" cellspacing="1" class="a" >
	...
	<td>
		 ...
		 <%IF chkchn = "1"  then%>
			 <tr bgcolor="<%= adminColor("tabletop") %>">
		      <%IF sVType = 2  then%>
    			...
    			<td align="center" colspan="4">MOB</td>
    			<td align="center" colspan="4">APP</td>
    			<td align="center" colspan="4">제휴</td>
    			<td align="center" colspan="4">해외몰</td>
        ...
        <tr align="center" bgcolor="<%= adminColor("tabletop") %>" height="30">
            <td colspan="<%IF chkImg = 1 then%><%if sVType="2" then%>5<%else%>4<%end if%><% else %><%if sVType="2" then%>4<%else%>3<%end if%><%end if%>" align="center">총계</td>
