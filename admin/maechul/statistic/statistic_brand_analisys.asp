...
<tr bgcolor="#FFFFFF">
	...
	<td  align="center" <%if chkChannel ="1" then%>rowspan="7"<%end if%>><a href="/admin/maechul/statistic/statistic_item_analisys.asp?menupos=1726&date_gijun=<%=vDateGijun%>&syear=<%=vSYear%>&smonth=<%=vSMonth%>&sday=<%=vSDay%>&eyear=<%=vEYear%>&emonth=<%=vEMonth%>&eday=<%=vEDay%>&ebrand=<%= cStatistic.FList(i).FMakerID %>" target="_blank">[상품상세]</a></td>
</tr>
<%if chkChannel ="1" then%>
