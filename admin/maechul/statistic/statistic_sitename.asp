...
function detailStatistic(y1,m1,d1,y2,m2,d2,sitename,date_gijun,sellchnl)
{
	var detailpop = window.open("/admin/maechul/statistic/statistic_daily.asp?syear="+y1+"&smonth="+m1+"&sday="+d1+"&eyear="+y2+"&emonth="+m2+"&eday="+d2+"&sitename="+sitename+"&date_gijun="+date_gijun+"&sellchnl="+sellchnl+"","detailpop","width=1000,height=780,scrollbars=yes,resizable=yes");
	detailpop.focus();
}
...
%>
	<tr bgcolor="#FFFFFF">
		...
		<td align="center"><%= cStatistic.flist(i).getSellbizName %></td>
		...
		<td align="center" >
			[<a href="javascript:detailStatistic('<%=vSYear%>','<%=vSMonth%>','<%=vSDay%>','<%=vEYear%>','<%=vEMonth%>','<%=vEDay%>','<%= cStatistic.flist(i).FSiteName %>','<%= vDateGijun %>','<%= sellchnl %>')">일별</a>]
		</td>
