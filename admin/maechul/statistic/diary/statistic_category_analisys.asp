...
Dim mwdiv, inc3pl, vCateMRate,vTot_CateMRate, dispCate, maxDepth, chkChannel, linkcate, linkdispcate
dim dy ,diaryyear
...
<tr bgcolor="#FFFFFF">
	...
	<td  align="center" <%if chkChannel ="1" then%>rowspan="3"<%end if%>><a href="/admin/maechul/statistic/diary/statistic_item_analisys.asp?menupos=1834&date_gijun=<%=vDateGijun%>&syear=<%=vSYear%>&smonth=<%=vSMonth%>&sday=<%=vSDay%>&eyear=<%=vEYear%>&emonth=<%=vEMonth%>&eday=<%=vEDay%><%=linkcate&linkdispcate%>" target="_blank">[상품상세]</a></td>
</tr>
<%if chkChannel ="1" then%>
