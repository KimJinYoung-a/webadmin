...
<%
Dim i, cStatistic, vSiteName, vDateGijun, v6MonthDate, vSYear, vSMonth, vSDay, vEYear, vEMonth, vEDay, vSorting, chkChannel,vBrandID
...
<tr bgcolor="#FFFFFF">
	...
	<td align="center" <%if chkChannel ="1" then%>rowspan="3"<%end if%>><%= cStatistic.FList(i).fpurchasetypename %></td>
	<%if chkChannel ="1" then%>
	...
	<td  align="center" <%if chkChannel ="1" then%>rowspan="3"<%end if%>><a href="/admin/maechul/statistic/diary/statistic_item_analisys.asp?menupos=1834&date_gijun=<%=vDateGijun%>&syear=<%=vSYear%>&smonth=<%=vSMonth%>&sday=<%=vSDay%>&eyear=<%=vEYear%>&emonth=<%=vEMonth%>&eday=<%=vEDay%>&ebrand=<%= cStatistic.FList(i).FMakerID %>" target="_blank">[상품상세]</a></td>
</tr>
<%if chkChannel ="1" then%>
...
<tr align="center" bgcolor="<%= adminColor("tabletop") %>" height="30">
	<td align="center" colspan="<%if chkChannel ="1" then%>3<%else%>2<%end if%>">총계</td>
