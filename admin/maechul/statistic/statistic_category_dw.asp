...
<%
Dim i, cStatistic, vSiteName, vDateGijun, v6MonthDate, vEDay, vCateL, vCateM	' , vSYear, vSMonth, vSDay, vEYear, vEMonth
...
Dim incStockAvg, vShowDate, iDtlItemLink
Dim vTot_OrderCnt, vTot_ItemNO, vTot_OrgitemCost, vTot_ItemcostCouponNotApplied, vTot_ItemCost, vTot_BuyCash, vTot_itemsku
...
dim vTot_upcheJungsan, vTot_avgipgoPrice, vTot_overValueStockPrice, vstartdate, venddate, isSendGift
...
vParam = CurrURL() & "?menupos="&Request("menupos")&"&date_gijun="&vDateGijun&"&startdate="&vstartdate&"&enddate="&venddate&"&sitename="&vSiteName&"&isBanpum="&vIsBanPum&"&ebrand="&vBrandID&"&purchasetype="&vPurchasetype&"&is6ago="&v6Ago&"&mwdiv="&mwdiv&"&categbn="&categbn&"&sellchnl="&sellchnl&"&chkChl="&chkChannel&"&rdsite="&rdsite&"&incStockAvg="&incStockAvg&"&isSendGift="&isSendGift
...
function popCateSellDetail(cdl,cdm,cds,dispcate){
    window.open("/admin/maechul/statistic/statistic_item_dw.asp?menupos=1726&date_gijun=<%=vDateGijun%>&startdate=<%= vstartdate %>&enddate=<%= venddate %>&cdl="+cdl+"&cdm="+cdm+"&cds="+cds+"&disp="+dispcate+"&isSendGift="+isSendGift,'','');
}
...
<input type="hidden" name="cdl" value="<%= vCateL %>">
<input type="hidden" name="cdm" value="<%= vCateM %>">
<input type="hidden" name="cds" value="<%= vCateS %>">
		...
		<tr>
			<td height="25">
				* 기간 :
				...
				%>
				&nbsp;&nbsp;
				* 채널구분 : <% drawSellChannelComboBox "sellchnl",sellchnl %>
                &nbsp;&nbsp;
                * 주문구분 :
		...
		<tr>
		    <td>
				...
				* 구매유형 : 
				...
				&nbsp;&nbsp;
				<label><input type="radio" name="categbn" value="M" <%=CHKIIF(categbn="M","checked","")%> >관리카테고리</label>
				&nbsp;&nbsp;
				<label><input type="radio" name="categbn" value="D" <%=CHKIIF(categbn="D","checked","")%> >전시카테고리</label>
				...
				<% end if%>
				<label><input type="checkbox" name="chkChl" value="1" <%if chkChannel ="1" then%>checked<%end if%>>채널상세보기</label>
			    &nbsp;&nbsp;
			    <label><input type="checkbox" name="showordcnt" value="on" <%= CHKIIF(showordcnt="on","checked","") %> >주문건수 표시</label>
			    &nbsp;&nbsp;
				...
				&nbsp;&nbsp;
			    <label><input type="checkbox" name="showdate" value="Y" <%=CHKIIF(vShowDate<>"","checked","")%>>일자표시</label>
				&nbsp;&nbsp;
...
<tr 	bgcolor=<%if chkChannel ="1" then%>"#e3f1fb"<%else%>"#FFFFFF"<%end if%>>
	...
	' Response.Write " <a href=""javascript:popCateSellDetail('"&cStatistic.FList(i).FCateL&"','"&cStatistic.FList(i).FCateM&"','"&cStatistic.FList(i).FCateS&"','"&cStatistic.FList(i).FCateX&"')"">(상세)</a>"
		...
		iDtlItemLink= "/admin/maechul/statistic/statistic_item_dw.asp?menupos=1726&date_gijun="&vDateGijun&"&startdate="& vstartdate &"&enddate="& venddate &linkcate&linkdispcate&"&incStockAvg="&incStockAvg&"&sitename="&vSitename&"&mwdiv="&mwdiv&"&isSendGift="&isSendGift
        iDtlItemLink= iDtlItemLink & "&ebrand="&vBrandID
	...
	<% if chkChannel <> "1" then %>
		<td align="center"><%= NullOrCurrFormat(CDbl(cStatistic.FList(i).fitemsku)) %></td>
	<% end if %>
	<% if (NOT C_InspectorUser) then %>
	...
	<td  align="center" <%if chkChannel ="1" then%>rowspan="6"<%end if%>><a href="<%=iDtlItemLink%>" target="_blank">[상품상세]</a></td>
</tr>
...
<tr align="center" bgcolor="<%= adminColor("tabletop") %>" height="30">
	...
	<% if chkChannel <> "1" then %>
		<td align="center"><%=NullOrCurrFormat(vTot_itemsku)%></td>
	<% end if %>
	<% if (NOT C_InspectorUser) then %>
