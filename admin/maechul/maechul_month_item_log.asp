...
dim searchfield, searchtext, dategbn, actDivCode, makerid, excptdlv, exceptSite, vPurchasetype
dim excTPL
dim searchGbn
...
set oCMaechulLog = new CMaechulLog
	...
	if (oCMaechulLog.FRecttargetGbn = "ON") and (useNewDB <> "") then
		oCMaechulLog.FRectGrpBy = "mwch"
	end if

    if showOrder = "on" then
        oCMaechulLog.FRectGrpBy = "mwordr"
    end if
...
if (oCMaechulLog.FRectGrpBy = "mwch") then
	showChannel = True
end if
%>
...
function jsSetYYYYMM4() {
	...
	if (frm.dategbn.value == "chulgoDate") {
		frm.nxmonthfixed.disabled = false;
	}else{
		frm.nxmonthfixed.checked = false;
		frm.nxmonthfixed.disabled = true;
	}
	...
	<td align="left">
		* 날짜 :
		<select class="select" name="dategbn" onChange="jsSetYYYYMM4()">
			...
			<!--
			<option value="actDateAndChulgoDate" <%=CHKIIF(dategbn="actDateAndChulgoDate","selected","")%> >결제일자(처리일자) + 출고일자</option>
			-->
		</select>
		&nbsp;
...
<tr align="center" bgcolor="#FFFFFF" >
	<td align="left">
		...
		* 검색조건 :
		...
		&nbsp;&nbsp;
		* <input type="checkbox" name="excptdlv" <%=CHKIIF(excptdlv<>"","checked","")%> > 배송비/포장비 제외
		&nbsp;&nbsp;
		* <input type="checkbox" name="useNewDB" <%=CHKIIF(useNewDB<>"","checked","")%> > DW디비사용
        &nbsp;&nbsp;
		* <input type="checkbox" name="showOrder" <%=CHKIIF(showOrder<>"","checked","")%> > 주문번호 표시
		&nbsp;&nbsp;
		* <input type="checkbox" name="nxmonthfixed" <%=CHKIIF(nxmonthfixed<>"" and dategbn="chulgoDate","checked","")%> <%=CHKIIF(dategbn<>"chulgoDate"," disabled","")%>> 정산이월내역만
	</td>
</tr>
</table>
</form>
...
<tr>
	<td align="left">
		...
		※ 매출구분 : 온라인, DW디비사용을 체크하면 <font color="red">채널구분</font>이 표시됩니다.
	</td>
	<td align="right">
	</td>
</tr>
...
<tr align="center" bgcolor="FFFFFF" onmouseover=this.style.background="F1F1F1"; onmouseout=this.style.background="FFFFFF";>
	...
	<% if showChannel then %>
	<td><%= getSellChannelDivName(oCMaechulLog.FItemList(i).Fbeadaldiv) %></td>
	<% else %>
	<td><%= getSellChannelDivName_Not_ON(oCMaechulLog.FItemList(i).FtargetGbn, oCMaechulLog.FItemList(i).Fsitename) %></td>
	<% end if %>
    ...
    <% if showOrder = "on" then %>
    <td><%= oCMaechulLog.FItemList(i).Forderserial %></td>
    <% end if %>
	<% if (C_InspectorUser = False) then %>
