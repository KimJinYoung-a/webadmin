...
set Omaechul_list = new Cmaechul_list
	...
	Omaechul_list.fmaechul_month_sum()	
...
set Omaechul_list_old = new Cmaechul_list
	...
	Omaechul_list_old.fmaechul_month_sum()	
end if 
...
<form name="frm" method="get">
	...
	<tr height="25" valign="top">
		...
		<td background="/images/tbl_blue_round_06.gif">
			...
			<font color="red"><strong>텐바이텐 <%= yyyy2 %>년 월별 매출통계</strong></font> 
		</td>			
...
dim upchepartDeliverBuySum_totalsum
...
<% if Omaechul_list.ftotalcount > 0 then %>
	<table width="100%" border="0" align="center" class="a" cellpadding="0" cellspacing="1" bgcolor="#BABABA">
		<tr bgcolor="#DDDDFF">
			...
			<td align="center"><strong>결제총액</strong></td>
			<td align="center">매입가</td>
			<td align="center">텐배송비</td>
			<td align="center">업체조건배송비</td>
		...
		<tr bgcolor="#FFFFFF">
			<td align="center"><%= Omaechul_list.flist(i).forderdate %></td>
			<% if datecancle <> "" then %>
			...
			<% upchepartDeliverBuySum_totalsum = upchepartDeliverBuySum_totalsum + Omaechul_list.flist(i).fupchepartDeliverBuySum %>
		...
		<td align="right"><%= totalcount_totalsum %></td>
		...
		<td align="center"><%= CurrFormat(subtotalprice_totalsum) %></td>
		<td align="right"><%= CurrFormat(totalbuysum_totalsum) %></td>
		<td align="right"><%= CurrFormat(tendeliversum_totalsum) %></td>
		<td align="right"><%= NullOrCurrFormat(upchepartDeliverBuySum_totalsum) %></td>
		<td align="center"><%= CurrFormat(tendeliverCount_totalsum) %></td>
		<td align="right"><%= CurrFormat(sunsuik_totalsum) %></td>
		<td align="center">
		<% if (totalsum_totalsum<>0) and Not IsNULL(sunsuik_totalsum) then %>
		...
		upchepartDeliverBuySum_totalsum =0
		sunsuik_totalsum = 0
		magin_totalsum = 0
		%>
	</tr>		
...
<% if olddata = "no" then %>
		<% if Omaechul_list_old.ftotalcount > 0 then %>					
			...
			<tr bgcolor="#FFFFFF">
				<td align="center"><%= Omaechul_list_old.flist(i).forderdate %></td>
				<% if datecancle <> "" then %>
				...
				<td align="right"><%= CurrFormat(Omaechul_list_old.flist(i).ftendeliverBuysum) %></td>
				<td align="right"><%= NullOrCurrFormat(Omaechul_list_old.flist(i).fupchepartDeliverBuySum) %></td>
			...
			<% tendeliversum_totalsum = tendeliversum_totalsum + Omaechul_list_old.flist(i).ftendeliverBuysum %>
			<% tendeliverCount_totalsum = tendeliverCount_totalsum + Omaechul_list_old.flist(i).ftendeliverCount %>
			<% upchepartDeliverBuySum_totalsum = upchepartDeliverBuySum_totalsum + Omaechul_list_old.flist(i).fupchepartDeliverBuySum %>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="a" bgcolor="F4F4F4">
    <tr valign="top" height="25">
        ...
        <input type="button" value="엑셀출력" onclick="javascript:excelprint('<%=olddata%>','<%=yyyy1%>','<%=yyyy2%>','<%=dateview1%>','<%=datecancle%>','<%=bancancle%>','<%=accountdiv%>','<%=sitename%>','<%=ipkumdatesucc%>','<%=menupos%>');">
