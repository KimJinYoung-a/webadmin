...
objLink.getDiaryLinkedItem idx
...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="a" bgcolor="#9d9d9d">
	...
	<tr bgcolor="#FFFFFF">
		<td align="center"> <%= objLink.FItemList(intLoop).FItemid %></td><td><%= objLink.FItemList((intLoop)).FItemName %><a href="diary_reg_LInkItem_proc.asp?mode=del&idx=<%= idx %>&itemid=<%= objLink.FItemList(intLoop).FItemid %>">[삭제]</a></td>
	</tr>
		<% Next %>
	<% else %>
	<tr bgcolor="#FFFFFF">
	...
	<form name="regfrm" method="post" action="diary_reg_LInkItem_proc.asp">
