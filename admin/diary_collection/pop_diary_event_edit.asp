...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="a" bgcolor="#9d9d9d">
	...
	<tr bgcolor="#FFFFFF">
		<td align="center" width="120" bgcolor="<%= adminColor("topbar") %>"><b>번호</b></td>
		<td><%= objevt.DiaryPrd.FBanneridx %></td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td align="center" bgcolor="<%= adminColor("topbar") %>"><b>배너위치</b></td>
		<td>
			<select name="bannerType" onchange="jsSelevtType(this.value)">
				<option value="multi" <% if objevt.DiaryPrd.FBannerType="multi" then response.write "selected" %> >메인 멀티</option>
				<option value="left" <% if objevt.DiaryPrd.FBannerType="left" then response.write "selected" %>>좌측 메뉴</option>
				<option value="power" <% if objevt.DiaryPrd.FBannerType="power" then response.write "selected" %>>파워이벤트</option>
				...
				<option value="dzone" <% if objevt.DiaryPrd.FBannerType="dzone" then response.write "selected" %>>디자인존</option>
				<option value="tdayitem" <% if objevt.DiaryPrd.FBannerType="tdayitem" then response.write "selected" %>>Today`s Item</option>
				<option value="evtmain" <% if objevt.DiaryPrd.FBannerType="evtmain" then response.write "selected" %>>이벤트메인 배너</option>
				<option value="othermall_left" <% if objevt.DiaryPrd.FBannerType="othermall_left" then response.write "selected" %>>[외부몰]좌측메뉴</option>
				<option value="othermall_multi" <% if objevt.DiaryPrd.FBannerType="othermall_multi" then response.write "selected" %>>[외부몰]메인멀티</option>
				<option value="othermall_right" <% if objevt.DiaryPrd.FBannerType="othermall_right" then response.write "selected" %>>[외부몰]우측메뉴</option>
			</select>
		</td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td align="center" bgcolor="<%= adminColor("topbar") %>"><b>이벤트 코드</b></td>
		<td><input type="text" name="evtcode" size="10" maxlength="10" value="<%= objevt.DiaryPrd.FEvtCode %>" />(없을시 공란)</td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td align="center" bgcolor="<%= adminColor("topbar") %>"><b>URL</b></td>
		<td><!--<input type="checkbox" name="mapusing" onclick="fnmapusing(this.checked);" <% if objevt.DiaryPrd.FbannerMapUsing="Y" then response.write "checked" %>>이미지맵 사용<br>-->
	...
	<tr bgcolor="#FFFFFF">
		...
		<td>
			<label><input type="radio" name="isusing" value="Y" <% if objevt.DiaryPrd.FBannerUsing="Y" then response.write "checked"%> /> 사용 </label>
			<label><input type="radio" name="isusing" value="N" <% if objevt.DiaryPrd.FBannerUsing="N" then response.write "checked"%>/> 사용안함 </label>
		</td>
	</tr>
	</form>
</table>
