...
<tr>
	<td align="center">
		  ...
		  <form method="post" name="f" action="cscenter_qna_board_act2.asp" onsubmit="return false">
		  ...
		  <% if boardqna.results(0).userid <> "" then %>
			  <tr>
				  <td class="a" height="5" colspan="2">고객등급 : <%= boardqna.results(0).GetUserLevelStr %></td>
			  </tr>
		  <!--
				<tr>
		  ...
		  <% else %>
				<tr>
				  ...
				  <select name="gubun" onchange="TnChangeCompliment(this.options[this.selectedIndex].value);">
					 <option value="">선택</option>
					 <option value="01">흐린날</option>
					 <option value="02">비</option>
					 <option value="03">눈</option>
					 <option value="11">봄</option>
					 <option value="12">여름</option>
					 <option value="13">가을</option>
					 <option value="20">일반</option>
					 <option value="31">오전</option>
					 <option value="33">월요일</option>
					 <option value="34">금요일</option>
					 <option value="35">주말</option>
					 <option value="36">월초</option>
					 <option value="37">월말</option>
					 <option value="40">명절</option>
				  </select>
				  </td>
				</tr>
				<tr>
				  <td colspan="2"><textarea name="replycontents" cols="80" rows="10"></textarea></td>
				</tr>
		  <% end if %>
				<tr>
				  ...
				  <% else %>
				  <a href="cscenter_qna_board_list.asp">목록으로 이동</a>
				  <% end if %>
				  </td>
				</tr>
		  </form>
...
<tr>
	<td align="center"><br>
		  <% if boardqna.results(0).userid <> "" or boardqna.results(0).orderserial <> "" then %>
		  <table cellpadding="0" cellspacing="0" bordercolordark="White" bordercolorlight="black" border="1" width="580">
		  <tr>
...
<!--
 ...
 function TnChangeCompliment(SelectGubun){
	ComplimentFrame.location.href="/admin/board/compliment_select.asp?gubun=" + SelectGubun;
 }

 function TnChangeText(str){
var basictext;
basictext = "안녕하세요.고객님\n"
basictext = basictext + "텐바이텐 고객행복센터입니다.\n"
basictext = basictext + "(내용)\n"
basictext = basictext + "만족스런답변이 되셨는지요\n\n"

	if(str == ''){
		document.f.replycontents.value = basictext;
	}
	else{
		document.f.replycontents.value = basictext + str;
	}
 }
<% if boardqna.results(0).replyuser="" then %>
TnChangeText('');
