...
<input type="hidden" name="userid" value="<%=session("ssBctId")%>">
	<tr>
		...
		<td bgcolor="white" style="padding:0">
			<select name="gubun">
				...
				<option value="00" <% if boardevent.results(0).gubun = "00" then response.write "selected" %>>전체</option>
				<option value="01" <% if boardevent.results(0).gubun = "01" then response.write "selected" %>>대학로점</option>
				<option value="02" <% if boardevent.results(0).gubun = "02" then response.write "selected" %>>잠실점</option>
			</select>
		</td>
	</tr>	
	<tr>
		...
		<td bgcolor="white" style="padding:0">
				<input name="title" style="width:450" maxlength="40" style="border:1 solid" value="<% = boardevent.results(0).title %>">
		</td>
	</tr>
	<tr>
		<td width="100" align="center" style="color:white">내용</td>
		<td bgcolor="white" style="padding:0">
				<textarea name="contents" cols="50" rows="15"><% = boardevent.results(0).contents %></textarea>
	...
	<tr>
		...
		<td bgcolor="white" style="padding:0">
				<input type="text" name="enddate" style="width:200" maxlength="40" style="border:1 solid" value="<% = boardevent.results(0).enddate %>">(2004-07-30)
		</td>
	</tr>
	<tr>
		<td width="100" align="center" style="color:white">사용유무</td>
		<td bgcolor="white" style="padding:0">&nbsp;<input type="radio" name="isusing" value="Y" <% if boardevent.results(0).isusing = "Y" then response.write "checked" %>>사용 <input type="radio" name="isusing" value="N" <% if boardevent.results(0).isusing = "N" then response.write "checked" %>>사용안함</td>
	</tr>
	<tr>
