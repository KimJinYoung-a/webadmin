...
response.redirect "partner_notice_read.asp?idx=" + request("idx") + "&menupos=89"  
end if
...
<form method="POST" name="boardform" action="partner_notice_modify.asp?idx=<% =Request("idx")%>&pgsize=<% =Request("pgsize")%>&page=<% =Request("page")%>&menupos=79" name="boardform">
        ...
        <table width="750" border="0" cellpadding="3" cellspacing="1">
          <tr> 
            ...
            <td width="407" height="7">
              <select name="name">
                <option value="winnie" <% if nboard.FRectName="winnie" then response.write "selected" %>>최은희</option>
                <option value="moon" <% if nboard.FRectName="moon" then response.write "selected" %>>이문재</option>
              </select>
            </td>
          </tr>
          <tr> 
