...
//	if (CheckMember() == true){
		...
		if(chk == true){
			location = "partner_notice.asp?menupos=79&delmode=delete&page=<%=page%>&deletelist=" + deletelist;
		}
		else{
...
function gotowrite(){
location.href="partner_notice_write.asp?idx=<% =request("idx") %>&page=<% =request("page") %>&pgsize=<% =request("pgsize") %>&menupos=79"
}
...
<tr>
	<td colspan="5" height="25" align="right">검색결과 : 총 <font color="red"><% = nboard.FTotalCount %></font>개&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
</tr>
<tr >
...
<input type="hidden" name="menupos" value="<%= menupos %>">
<tr class="a">
	<td align="center"><input type="checkbox" name="deletebox" value="<%= nboard.BoardItem(ix).FRectIdx  %>"></td>
	<td align="center" height="25"><%= nboard.BoardItem(ix).FRectIdx  %></a></td>
	<td align="center"><a href="partner_notice_read.asp?idx=<%= nboard.BoardItem(ix).FRectIdx  %>&page=<% =page %>&pgsize=<% =pgsize %>&menupos=89"><%= nboard.BoardItem(ix).FRectTitle %></a></td>
...
<table>
<tr>
	<td><input type="button" value="선택항목삭제" onclick="qnadelete();"></td>
	<td><input type="button" value="공지사항쓰기" onclick="gotowrite();"></td>
</tr>
</table>
<%
set nboard = Nothing
