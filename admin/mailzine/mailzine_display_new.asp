...
if (omail.FOneItem.Fregtype = "2") then
	if (Not IsNumeric(omail.FOneItem.Fevt_code)) then
		Call PrintErrorAndStop("잘못된 주말특가 이벤트코드입니다. : '" & omail.FOneItem.Fevt_code & "'")
	end if
	...
	if (DateDiff("d", cEvtCont.FESDay, cEvtCont.FEEDay) < 2) then
		Call PrintErrorAndStop("잘못된 주말특가 이벤트코드입니다. : '" & omail.FOneItem.Fevt_code & "'" & "<br />주말특가 이벤트기간이 3일 미만입니다.")
	end if
	...
	if (maxpercentage = "") then
		Call PrintErrorAndStop("잘못된 주말특가 이벤트코드입니다. : '" & omail.FOneItem.Fevt_code & "'" & "<br />상품 최대 할인가 입력안됨.")
	end if
...
if (omail.FOneItem.Fregtype = "3") or (omail.FOneItem.Fregtype = "4") then
	...
	if (Not IsNumeric(omail.FOneItem.Fevt_code)) then
		Call PrintErrorAndStop("잘못된 메인 기획전 이벤트코드입니다. : '" & omail.FOneItem.Fevt_code & "'")
	end if
...
if (omail.FOneItem.Fregtype = "2") or (omail.FOneItem.Fregtype = "3") or (omail.FOneItem.Fregtype = "4") then
	...
	if Not IsArray(evtList) then
		Call PrintErrorAndStop("잘못된 기획전 목록입니다. : '" & omail.FOneItem.Fevt_code & "'" & "<br />기획전 목록 없음.")
	end if

	if (UBound(evtList, 2) - LBound(evtList, 2)) < 7 then
		Call PrintErrorAndStop("잘못된 기획전 목록입니다. : '" & omail.FOneItem.Fevt_code & "'" & "<br />기획전 목록이 8개 미만.")
	end if
...
if (omail.FOneItem.Fregtype = "3") then
	...
	if (UBound(evtList, 2) - LBound(evtList, 2)) < 11 then
		Call PrintErrorAndStop("잘못된 기획전 목록입니다. : '" & omail.FOneItem.Fevt_code & "'" & "<br />기획전 목록이 4개 미만.")
	end if
...
if (omail.FOneItem.Fregtype = "5") then
	...
	if Not IsArray(evtList) then
		Call PrintErrorAndStop("잘못된 기획전 목록입니다[0]. : '" & omail.FOneItem.Fevt_code & "'" & "<br />기획전 목록 없음.")
	end if

	if (UBound(evtList, 2) - LBound(evtList, 2)) < 1 then
		Call PrintErrorAndStop("잘못된 기획전 목록입니다[1]. : '" & omail.FOneItem.Fevt_code & "'" & "<br />기획전 목록이 1개 미만.")
	end if
...
if (omail.FOneItem.Fregtype = "2") or (omail.FOneItem.Fregtype = "4") or (omail.FOneItem.Fregtype = "5") then
	...
	if (omail.FOneItem.Fimgmap2 = "") then
		Call PrintErrorAndStop("잘못된 엠디픽 목록입니다[0]. : '" & omail.FOneItem.Fimgmap2 & "'" & "<br />엠디픽 목록 없음.")
	end if
	...
	if Not IsArray(evtList) then
		Call PrintErrorAndStop("잘못된 엠디픽 목록입니다[1]. : '" & omail.FOneItem.Fimgmap2 & "'" & "<br />엠디픽 목록 없음.")
	end if

	if (omail.FOneItem.Fregtype = "2") and (UBound(evtList, 2) - LBound(evtList, 2)) < 11 then
		Call PrintErrorAndStop("잘못된 엠디픽 목록입니다[2]. : '" & omail.FOneItem.Fimgmap2 & "'" & "<br />엠디픽 목록이 12개 미만.")
	end if

	if (omail.FOneItem.Fregtype = "4") and (UBound(evtList, 2) - LBound(evtList, 2)) < 5 then
		Call PrintErrorAndStop("잘못된 엠디픽 목록입니다[3]. : '" & omail.FOneItem.Fimgmap2 & "'" & "<br />엠디픽 목록이 6개 미만.")
	end if
...
if (omail.FOneItem.Fregtype = "2") or (omail.FOneItem.Fregtype = "3") or (omail.FOneItem.Fregtype = "4") or (omail.FOneItem.Fregtype = "5") then
	...
	if omail.FOneItem.Fimgmap3 = "" then
		...
		if Not IsArray(evtList) then
			Call PrintErrorAndStop("잘못된 저스트원데이입니다. : '" & cEvtCont.FRectItemid & "'" & "<br />저스트원데이 없음.")
		end if
...
tailHTML = tailHTML + "											<td style=""margin:0; padding:67px 37px 8px 37px; font-family:'맑은고딕','Malgun Gothic','돋움', dotum, sans-serif; font-size:18px !important; line-height:27px; letter-spacing:-0.3px; color:#838383; text-align:left;"">" & vbCrLf
tailHTML = tailHTML + "												<p style=""margin:0; padding:0; font-family:'맑은고딕','Malgun Gothic','돋움', dotum, sans-serif; font-size:18px !important; line-height:27px; letter-spacing:-0.3px; color:#838383; text-align:left;"">* 본 메일은 정보통신망 이용촉진 및 정보보호 등에 관한 법률시행규칙에 의거 &nbsp;${TODAYSTR} 기준으로 메일수신 동의하셨기에 발송되는 발송전용메일입니다.</p>" & vbCrLf
tailHTML = tailHTML + "												<p style=""margin:0; padding:0; font-family:'맑은고딕','Malgun Gothic','돋움', dotum, sans-serif; font-size:18px !important; line-height:27px; letter-spacing:-0.3px; color:#838383; text-align:left;"">* 본 메일은 발신 전용이며 회신 시 답장을 받을 수 없습니다.</p>" & vbCrLf
tailHTML = tailHTML + "												<p style=""margin:0; padding:0; font-family:'맑은고딕','Malgun Gothic','돋움', dotum, sans-serif; font-size:18px !important; line-height:27px; letter-spacing:-0.3px; color:#838383; text-align:left;"">* 더 이상 수신을 원하지 않으시면 수신거부 버튼을 클릭해주세요.</p>" & vbCrLf
tailHTML = tailHTML + "												<p style=""margin:0; padding:0; font-family:'맑은고딕','Malgun Gothic','돋움', dotum, sans-serif; font-size:18px !important; line-height:27px; letter-spacing:-0.3px; color:#838383; text-align:left;"">* 개인정보 변경 시 적용에 2~3일이 소요될 수 있는 점 양해 부탁드립니다.</p>" & vbCrLf
...
tailHTML = tailHTML + "										</tr>" & vbCrLf
tailHTML = tailHTML + "										<tr>" & vbCrLf
tailHTML = tailHTML + "											<td style=""margin:0; padding:8px 35px; font-family:'맑은고딕','Malgun Gothic','돋움', dotum, sans-serif; font-size:18px !important; line-height:27px; letter-spacing:-0.3px; color:#838383; text-align:left;"">(03082) 서울시 종로구 대학로 57 홍익대학교 대학로캠퍼스 교육동 14층 텐바이텐<br />대표이사 : 최은희 / 사업자등록번호 : 211-87-00620 / 통신판매업신고 : 제01-1968호 / 개인정보 보호 및 청소년 보호책임자 : 이문재<br />고객행복센터 TEL : <a href=""tel:1644-6030"" style=""color:#838383; text-decoration:none; font-style:bold;""><b>1644-6030</b></a> / E-mail : <a href=""mailto:customer@10x10.co.kr"" style=""color:#838383; text-decoration:none; font-style:bold;""><b>customer@10x10.co.kr</b></a></td>" & vbCrLf
tailHTML = tailHTML + "										</tr>" & vbCrLf
tailHTML = tailHTML + "										<tr>" & vbCrLf
...
<% if (typeGubun = "code") then %>
	...
	<tr bgcolor="FFFFFF">
		<td>
			<input type="text" name="title" size="100" class="input" readonly value="(광고) <% = omail.FOneItem.Ftitle %>"><br>
	...
	<div align="center">
		...
		<% if IsNull(omail.FOneItem.FreservationDATE) or (omail.FOneItem.FreservationDATE = "") then %>
			발송예약 설정이후에만 메일러 전송가능합니다.
		<% else %>
			<input type="button" class="button" style="width=200px; height=70px;" value=" 메일러 전송하기 " onClick="jsSendMailer();">
		<% end if %>
		<br />
	...
	function jsSendMailer() {
		<% if (Left(Now(),10) >= Replace(omail.FOneItem.Fregdate, ".", "-")) then %>
			alert("오늘 이전날짜의 이메일은 전송할 수 없습니다.");
		...
		<% else %>
			alert('텐바이텐 전지역, 전체맴버, 디자인완성 이후에만 전송가능합니다.');
		<% end if %>
	}
