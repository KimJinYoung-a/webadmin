...
<table align="center" width="670" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
	<td align="left" style="padding:25px 0 0 0; font-family:Dotum; color:#888; font-size:11px; line-height:16px;">* 본 메일은 정보통신망 이용촉진 및 정보보호 등에 관한 법률시행규칙에 의거&nbsp;<%= Year(Now) %>년 <%= month(Now) %>월 <%= day(Now) %>일 기준으로 회원님의 메일수신에 동의하셨기에 발송되는 발송전용메일입니다.<br />
	<% if area = "finger_all" or area = "finger_metropolitan" then %>
	* 본 메일은 발신 전용이며 회신 시 답장을 받을 수 없습니다.<br />
	<% end if %>
	* 더 이상 수신을 원하지 않으시면 수신거부 버튼을 클릭해주세요.<br />
	* 개인정보변경 시 시스템 적용에 2~3일 반영시간이 소요될 수 있으니 이 점 양해 부탁드립니다.<br />
	<a href='http://www.10x10.co.kr/member/mailzine/reject_mailzine.asp?M_ID=${TMS_M_EMAIL}' target='_blank' onFocus='this.blur()'>[수신거부]</a> (To unsubscribe this e-mail, <a href='http://www.10x10.co.kr/member/mailzine/reject_mailzine.asp?M_ID=${TMS_M_EMAIL}' target='_blank' onFocus='this.blur()'>click HERE</a>)</td>
  </tr>
  <tr>
