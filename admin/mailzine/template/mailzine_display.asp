...
dim title, regdate, isusing, gubun
...
dim evtList, prevEvenOdd, currEvenOdd, imgURL, bestHTML, newHTML, TENTENIMAGE600
dim ix, i, j, k, member, typeGubun, cEvtCont, combineHTML, manuallyHTML,mailergubun
...
If isArray(ArrTemplateInfo) Then
    For ix=0 To UBound(ArrTemplateInfo,2)
        ...
        if ArrTemplateInfo(0, ix)="20" or ArrTemplateInfo(0, ix)="21" or ArrTemplateInfo(0, ix)="22" or ArrTemplateInfo(0, ix)="23" then
            ...
            if ArrTemplateDetailInfo(0, ix)<>"" and not(isnull(ArrTemplateDetailInfo(0, ix))) then
        ...
        if ArrTemplateInfo(0, ix)="24" then
            ...
            weekendHTML = weekendHTML + "													<tr>" & vbCrLf
            if maxpercentage<"1" then
            weekendHTML = weekendHTML + "														<td></td>" & vbCrLf
            else
...
tailHTML1 = Replace(tailHTML, "${REJECTLINK}", "http://www.10x10.co.kr/member/mailzine/reject_mailzine.asp?M_ID=${TMS_M_EMAIL}")
tailHTML2 = Replace(tailHTML, "${REJECTLINK}", "http://www.10x10.co.kr/member/mailzine/notmember_del.asp?usermail=[$email]&site=10x10")
...
<% if (typeGubun = "code") then %>
	...
	<tr bgcolor="FFFFFF">
		<td>
			...
			<textarea name="mailcontents1" rows="15" class="input" readonly style="width:100%;"><%
            response.write headerHTML
            response.write combineHTML
            response.write tailHTML1
            %></textarea>
            <textarea name="mailcontents2" rows="15" class="input" readonly style="width:100%;"><%
            response.write headerHTML
            response.write combineHTML
            response.write tailHTML2
            %></textarea>
		</td>
	</tr>
	</table>
	...
	<div align="center">
		...
		<% else %>
			<input type="button" class="button" style="width=200px; height=70px;" value="<%= mailergubun %> 메일러 전송하기 " onClick="jsSendMailer();">
		<% end if %>
		<br />
	...
	function jsSendMailer() {
		...
		<% if (omail.FOneItem.Farea = "ten_all") and (omail.FOneItem.Fmemgubun = "member_all") and (omail.FOneItem.Fgubun = "5") then %>
			var idx, member;
	...
	<form name="frmAct" method="post" action="/admin/mailzine/template/mailzine_process.asp" style="margin:0px;">
    ...
    <% IF application("Svr_Info")="Dev" THEN %>
	    <iframe name="iframe_mailer" width="100%" height="300" border="0" frameborder="0"></iframe>
    <% else %>
        <iframe name="iframe_mailer" width="100%" height="300" border="0" frameborder="0"></iframe>
    <% end if %>
<%
...
htmlForDB = htmlForDB & vbCrLf & newHTML
htmlForDB = htmlForDB & vbCrLf & bestHTML
