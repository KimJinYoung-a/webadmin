...
Class CJumunMasterItem
	...
	public function getJungsanSum
	    getJungsanSum = FTotalSum - getEnuiSum - getDlvPaySum
    end function
...
Class CJumunMaster
    ...
    public Sub GetOneBetweenJumunMaster
        dim sqlStr
        sqlStr = ""
		...
		sqlStr = sqlStr & " JOIN db_order.dbo.tbl_order_detail D on D.orderserial=m.orderserial and D.cancelyn<>'Y'" ''--부분취소 재낌.
		sqlStr = sqlStr & " WHERE m.ipkumdiv>3"
		sqlStr = sqlStr & " and m.jumundiv<>6"
		sqlStr = sqlStr & " and m.beadaldiv = 8 "
		sqlStr = sqlStr & " and m.sitename = '10x10' "
		sqlStr = sqlStr & " and m.rdsite = 'betweenshop' "
	...
	Public Sub betShopJumunList()
		...
		sqlStr = sqlStr & " 	,Case When m.cancelyn = 'Y' Then '0' else m.totalsum end as totalsum"
		sqlStr = sqlStr & " 	,m.subtotalprice,isNULL(m.miletotalprice,0) as miletotalprice "
		sqlStr = sqlStr & " 	,Case When m.cancelyn = 'Y' Then '0' else m.tencardspend end as tencardspend, m.allatdiscountprice"
		sqlStr = sqlStr & " 	,sum(CASE WHEN d.itemid<>0 then d.itemcost*d.itemno else 0 END) as itemcostSum"
		sqlStr = sqlStr & " 	,sum(CASE WHEN d.itemid<>0 then d.reducedprice*d.itemno else 0 END) as reducedpriceSum"
		sqlStr = sqlStr & " 	,sum(CASE WHEN d.itemid<>0 and d.vatinclude='Y' then d.reducedprice*d.itemno else 0 END) as reducedpriceVatSum"
		sqlStr = sqlStr & " 	,sum(CASE WHEN d.itemid=0 and m.cancelyn <> 'Y' then d.itemcost*d.itemno else 0 END) as dlvcostSum"
		sqlStr = sqlStr & " 	,sum(CASE WHEN d.itemid=0 and d.bonuscouponidx is Not NULL then (d.itemcost-d.reducedprice)*d.itemno else 0 END) as dlvcostCpnSum"
		sqlStr = sqlStr & " 	FROM db_order.dbo.tbl_order_master m"
		sqlStr = sqlStr & " 	JOIN db_order.dbo.tbl_order_detail D on D.orderserial=m.orderserial and D.cancelyn<>'Y'" ''--부분취소 재낌.
		sqlStr = sqlStr & " 	WHERE m.ipkumdiv>3"
		...
		sqlStr = sqlStr & " 	GROUP BY m.orderserial, m.totalsum, m.subtotalprice, isNULL(m.miletotalprice,0), m.tencardspend, m.allatdiscountprice, m.cancelyn "
		sqlStr = sqlStr & " ) T"
		rsget.Open sqlStr,dbget,1
		If not rsget.EOF Then
	...
	public sub SearchJumunDetail(byval orderserial)
		...
		sqlStr = sqlStr + " , d.currstate, d.songjangdiv, d.songjangno, d.beasongdate, d.isupchebeasong, d.issailitem" & vbcrlf
