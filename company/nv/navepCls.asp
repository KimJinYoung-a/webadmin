...
Class CJumunMasterItem
    ...
    ''public FreducedpriceVatSum  '' 제거 2015/04/27
    public FreducedpriceNoVatSum
...
'        getJungsanTargetNoVatSum = getJungsanTargetSum*10/11
'    end function
...
'    end function
    ...
    public function getJungsanTargetNoVatSum
        getJungsanTargetNoVatSum = FreducedpriceNoVatSum
    end function

    public function getJungsanTargetSum()
        getJungsanTargetSum = FreducedpriceSum
    end function
	...
	'/사용금지 		'/공용펑션에 공용함수 쓸것.		'/2016.07.21 한용민
...
Class CJumunMaster
    ...
    public Sub GetOneNavEpJumunMaster
        dim sqlStr
        sqlStr = ""
        ...
        'sqlStr = sqlStr & " ,sum(CASE WHEN d.itemid<>0 and d.vatinclude='Y' then d.reducedprice*d.itemno else 0 END) as reducedpriceVatSum"
        sqlStr = sqlStr & " ,(CASE WHEN d.itemid=0 THEN 0 WHEN d.vatinclude='Y' then Round((d.reducedprice*d.itemno)/1.1,0) else d.reducedprice*d.itemno END) as reducedpriceNoVatSum"
        ...
        sqlStr = sqlStr & " 	and G.gubun = 'nvshop'"
        ...
        sqlStr = sqlStr & " group by m.idx,m.orderserial,m.totalsum,m.subtotalprice,isNULL(m.miletotalprice,0),m.tencardspend,m.allatdiscountprice,m.beadaldate,m.ipkumdiv,m.jumundiv, m.ipkumdate, m.regdate, m.cancelyn, m.canceldate, m.jungsanflag, m.rdsite,d.itemid, d.vatinclude, d.reducedPrice, d.itemno"
		...
		If not rsget.EOF Then
			set FOneItem = new CJumunMasterItem
                ...
                ''FOneItem.FreducedpriceVatSum  = Null2Zoro(rsget("reducedpriceVatSum"))
                FOneItem.FreducedpriceNoVatSum  = Null2Zoro(rsget("reducedpriceNoVatSum"))
    ...
    ''2015/04/27 면세분 디비에서 쿼리로 변경
    Public Sub navEpJumunList_nofix()
		dim sqlStr
		dim wheredetail
		dim i
		
		if (dateDiff("d",FRectRegStart,FRectRegEnd)>31) then
            response.write "<font color=red>검색 기간을 1달 초과하여 조회 할 수 없음.</font>"
            exit sub
        end if
		...
		if (FRectRegEnd<>"") then
		    ...
		    elseif (FRectMType="fx") then   
		        ...
		        wheredetail = wheredetail + " and m.ipkumdiv='8'"
		...
		if (FRectMeCode <> "") then
			'wheredetail = wheredetail + " and G.rdsite in ('"&FRectMeCode&"','mobile_"&FRectMeCode&"')"  ''모바일쪽 매출관련
			'2013-12-18 김진영..아래것으로 해야 매출코드 별(웹이나 모바일 함께가 아닌) 정산금액이 나오는..)
			'2013-12-18 17:49 채현아주임님 요청....WEB_ALL과 MOBILE_ALL  김진영 추가 
			'2015-04-27 WEB_ALL과 MOBILE_ALL 구분 변경 서동석 
			If FRectMeCode = "WEB_ALL" Then
				''wheredetail = wheredetail + " and Left(G.rdsite, 6) = 'nvshop' "
				wheredetail = wheredetail + " and G.ismobile=0"
			ElseIf FRectMeCode = "MOBILE_ALL" Then
				''wheredetail = wheredetail + " and Left(G.rdsite, 6) = 'mobile' "
				wheredetail = wheredetail + " and G.ismobile=1"
			Else
		...
		''sqlStr = sqlStr & " ,sum(reducedpriceVatSum) as reducedpriceVatSum"
		sqlStr = sqlStr & " ,sum(reducedpriceNoVatSum) as reducedpriceNoVatSum"
        ...
        sqlStr = sqlStr & " ,sum(CASE WHEN d.itemid<>0 then d.reducedprice*d.itemno else 0 END) as reducedpriceSum"
        ''sqlStr = sqlStr & " ,sum(CASE WHEN d.itemid<>0 and d.vatinclude='Y' then d.reducedprice*d.itemno else 0 END) as reducedpriceVatSum"
        sqlStr = sqlStr & " ,sum(CASE WHEN d.itemid=0 then 0 WHEN d.vatinclude='Y' then Round((d.reducedprice*d.itemno)/1.1,0) else d.reducedprice*d.itemno END) as reducedpriceNoVatSum"
        sqlStr = sqlStr & " ,sum(CASE WHEN d.itemid=0 then d.itemcost*d.itemno else 0 END) as dlvcostSum"
        sqlStr = sqlStr & " ,sum(CASE WHEN d.itemid=0 and d.bonuscouponidx is Not NULL then (d.itemcost-d.reducedprice)*d.itemno else 0 END) as dlvcostCpnSum"
        sqlStr = sqlStr & " from db_order.dbo.tbl_order_master m"
        sqlStr = sqlStr & " 	JOIN db_item.dbo.tbl_Outmall_RdsiteGubun as G "
...
'response.write sqlStr &"<BR>"
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				set FMasterItemList(i) = new CJumunMasterItem
                    ...
                    ''FMasterItemList(i).FreducedpriceVatSum  = Null2Zoro(rsget("reducedpriceVatSum"))
                    FMasterItemList(i).FreducedpriceNoVatSum  = Null2Zoro(rsget("reducedpriceNoVatSum"))
	...
	Public Sub navEpJumunList_fixed()
		dim sqlStr
		dim wheredetail
		dim i
		...
		if (FRectOrderSerial<>"") then
			wheredetail = wheredetail + " and j.orderserial='" + FRectOrderSerial + "'"
		end if

	    wheredetail = wheredetail + " and (CASE WHEN isNULL(cancelDT,'')='' THEN fixedDate ELSE cancelDT END)>='" + CStr(FRectRegStart) + "'"
	    wheredetail = wheredetail + " and (CASE WHEN isNULL(cancelDT,'')='' THEN fixedDate ELSE cancelDT END)<'" + CStr(FRectRegEnd) + "'"
        ...
        sqlStr = sqlStr & " ,sum(j.suppprc) as suppprcSum"
        sqlStr = sqlStr & " ,sum(j.commissoin) as commiSum"
        sqlStr = sqlStr & " from db_jungsan.dbo.tbl_nvshop_jungsan_detail J"
        sqlStr = sqlStr & " 	JOIN db_item.dbo.tbl_Outmall_RdsiteGubun as G "
        sqlStr = sqlStr & " 	on J.rdsite =G.rdsite and G.gubun = 'nvshop' "
...
'response.write sqlStr
		...
		If not rsget.EOF Then
            ...
            FOneItem.FreducedpriceNoVatSum  = Null2Zoro(rsget("suppprcSum"))
            FOneItem.FcommiSum         = Null2Zoro(rsget("commiSum"))
	    end if
		rsget.Close
        ...
        sqlStr = sqlStr & " where 1=1" 
        sqlStr = sqlStr & wheredetail
        sqlStr = sqlStr & " order by rDate,orderserial"
...
'response.write sqlStr &"<BR>"
		...
		If not rsget.EOF Then
			...
			Do until rsget.EOF
				set FMasterItemList(i) = new CJumunMasterItem
					FMasterItemList(i).Forderserial		    = rsget("orderserial")
					FMasterItemList(i).Fregdate			    = rsget("rDate")
					FMasterItemList(i).Fbeadaldate		    = rsget("fixedDate")
					FMasterItemList(i).Fcanceldate          = rsget("cancelDt")
					if (FMasterItemList(i).Fcanceldate<>"") then FMasterItemList(i).Fbeadaldate = FMasterItemList(i).Fcanceldate
					    
					FMasterItemList(i).Frdsite              = rsget("rdsite")
                    FMasterItemList(i).FreducedpriceNoVatSum  = Null2Zoro(rsget("suppprc"))
                    
                    FMasterItemList(i).FitemNOptionName  = rsget("itemNOptionName")
                    FMasterItemList(i).Fitemno           = rsget("itemno")
                    FMasterItemList(i).Fismobile         = rsget("ismobile")
                    FMasterItemList(i).FcommiSum         = rsget("commissoin")
                    FMasterItemList(i).Fcommpro          = rsget("commpro")
                    FMasterItemList(i).FordStatName      = rsget("ordStatName")
	...
	''기존방식 //사용안함
	Public Sub navEpJumunList()
	  if (FRectMType="fx") then
	    Call navEpJumunList_fixed
	  else
	    Call navEpJumunList_nofix
	  end if
	  Exit Sub
