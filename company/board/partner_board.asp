...
function gotowrite(){
location.href="partner_board_write.asp?page=<% =request("page") %>&pgsize=<% =request("pgsize") %>&menupos=94"
}
...
<STRIKE><%= nboard.BoardItem(ix).FRectTitle %></STRIKE>
<% else %>
	<a href="partner_board_read.asp?idx=<%= nboard.BoardItem(ix).FRectIdx  %>&page=<% =page %>&pgsize=<% =pgsize %>&menupos=94"><%= nboard.BoardItem(ix).FRectTitle %></a>
<% end if %>
	</td>
