...
function GotoCallHistoryMemoMidify(iid,iorderserial, sitename){
    popCallRing('','','',iid,iorderserial,'', sitename);
}
...
<tr align="center" bgcolor="#FFFFFF" >
	...
	<td align="left">
		...
		<select class="select" name="sitename">
			<option value=""></option>
			<option value="academy" <%= CHKIIF(sitename="academy", "selected", "") %> >강좌</option>
			<option value="diyitem" <%= CHKIIF(sitename="diyitem", "selected", "") %> >DIY</option>
		</select>
		고객정보 :
...
<tr align="center" bgcolor="#FFFFFF" >
	<td align="left">
		...
		&nbsp;
		<input type="checkbox" class="checkbox" name="mmgubunexclude" value="4,5" <% if (mmgubunexclude <> "") then %>checked<% end if %>> SMS/이메일 안내 메모 제외
		&nbsp;
...
<tr align="center" bgcolor="#FFFFFF" height="25">
    ...
    <td>
    	...
    	<% end if %>
    	<%= printtel(ocsmemo.FItemList(i).FphoneNumber) %>
    </td>
    ...
    <td align="left">
        ...
        <a href="javascript:GotoCallHistoryMemoMidify('<%= ocsmemo.FItemList(i).Fid %>','<%= ocsmemo.FItemList(i).Forderserial %>','<%= ocsmemo.FItemList(i).Fsitename %>')">
