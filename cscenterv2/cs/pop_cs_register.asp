...
set oupchebeasongpay = new COrderMaster
	if (orderserial<>"") and (divcd = "A004") then
...
dim orgpayPrice
orgpayPrice = 0
	if (orderserial<>"") and (ocsaslist.FOneItem.IsCancelProcess or ocsaslist.FOneItem.IsReturnProcess) then
		orgpayPrice = getOrgPayPrice(orderserial)
	end if
...
var IsRegState = <%= LCase(IsRegState) %>;

var CDEFAULTBEASONGPAY = 2500;
var SELECTEDUPCHEDEFAULTBEASONGPAY = 2500;
var Fdivcd = "<%= divcd %>";
...
function CheckOverItemSelected() {
	var frm = document.frmaction;

	if ((Fdivcd == "A004") || (Fdivcd == "A010")) {
		if (frm.orderdetailidx.length>1){
			for (var j=0;j<frm.orderdetailidx.length;j++){
				if (frm.orderdetailidx[j].checked == true) {
					if (frm.itemno[j].value*1 < (frm.regitemno[j].value*1 + frm.prevreturnno[j].value*1)) {
						alert("중복접수입니다.");
						frm.orderdetailidx[j].checked = false;
					}
				}
			}
		}else{
			if (frm.orderdetailidx.checked == true) {
				if (frm.itemno.value*1 < (frm.regitemno.value*1 + frm.prevreturnno.value*1)) {
					alert("중복접수입니다.");
					frm.orderdetailidx.checked = false;
				}
			}
		}
	}
...
function CheckSelect(comp){
    ...
    if (comp.name!="Deliverdetailidx"){
        if (comp.checked){
			if ((Fdivcd == "A004") || (Fdivcd == "A010")) {
				//반품,회수
				var upbaeitemYN = eval("frm.isupchebeasong_" + chkidx).value;
				var makerid = eval("frm.makerid_" + chkidx).value;

				if ((Fdivcd == "A004") && upbaeitemYN == "N") {
					alert("접수불가 : 핑거스배송 상품은 회수신청으로 등록하세요.");
					comp.checked = false;
					AnCheckClick(comp);
					return;
				}
				if ((Fdivcd == "A010") && upbaeitemYN == "Y") {
					alert("접수불가 : 업체배송 상품은 반품접수로 등록하세요.");
					comp.checked = false;
					AnCheckClick(comp);
					return;
				}

				if ((Fdivcd == "A004") && (comp.checked == true)) {
					comp.checked = false;
					var prevmakerid = GetSelectedMakerID(frm);
					comp.checked = true;

					if ((prevmakerid != "") && (prevmakerid != makerid)) {
						alert("접수불가 : 하나의 브랜드만 선택가능합니다.");
						comp.checked = false;
						AnCheckClick(comp);
						return;
					}
...
function CheckTenItemAllSelected(frm) {
    if (frm.orderdetailidx.length>1){
        for (var j=0;j<frm.orderdetailidx.length;j++){
			if (frm.cancelyn[j].value != "Y") {
				if ((frm.odlvtype[j].value=="1")||(frm.odlvtype[j].value=="4")){
					if (frm.orderdetailidx[j].checked != true) {
						if (frm.itemno[j].value != frm.prevreturnno[j].value) {
							return false;
						}
					} else {
						if (frm.itemno[j].value != (frm.regitemno[j].value*1 + frm.prevreturnno[j].value*1)) {
							return false;
						}
					}
				}
		...
		if (frm.cancelyn.value != "Y") {
			if ((frm.odlvtype.value=="1")||(frm.odlvtype.value=="4")){
				if (frm.orderdetailidx.checked != true) {
					if (frm.itemno.value != frm.prevreturnno.value) {
						return false;
					}
				} else {
					if (frm.itemno.value != (frm.regitemno.value*1 + frm.prevreturnno.value*1)) {
						return false;
					}
				}
			}
...
function CheckUpcheItemAllSelected(frm, upbeaMakerid) {
	var itemMakerid;
...
function CheckBeasongPay(frm, makerid) {
	// 전체취소
	// 전체반품(변심반품제외)
	var gubun02 = frm.gubun02.value;

    if ((IsReturnProcess) && ((gubun02 == "CD01") || (gubun02 == ""))) {
		return false;
    }

	if (makerid.length < 1) {
		return CheckTenItemAllSelected(frm);
	} else {
		return CheckUpcheItemAllSelected(frm, makerid);
	}
}

function CheckClickBeasongPay(frm, makerid, chk) {
	var checkNeed = false;
	chk.checked = CheckBeasongPay(frm, makerid);
	AnCheckClick(chk);
}
...
function CheckUpcheDeliverPay(frm){
	...
	if (frm.Deliverdetailidx.length > 1) {
		for (var i = 0; i < frm.Deliverdetailidx.length; i++) {
			...
			if (frm.DeliverCancelyn[i].value == "N") {
				CheckClickBeasongPay(frm, upbeaMakerid, frm.Deliverdetailidx[i]);
			}
		}
	} else {
		upbeaMakerid = frm.DeliverMakerid.value;
		if (frm.DeliverCancelyn.value == "N") {
			CheckClickBeasongPay(frm, upbeaMakerid, frm.Deliverdetailidx);
		}
	}
}

function GetSelectedMakerID(frm) {
	var itemMakerid;
    if (frm.orderdetailidx.length>1){
        for (var j=0;j<frm.orderdetailidx.length;j++){
			if (frm.orderdetailidx[j].checked == true) {
				if ((frm.odlvtype[j].value=="1")||(frm.odlvtype[j].value=="4")){
					return "10x10logistics";
				} else {
					return frm.makerid[j].value;
				}
			}
        }
    }else{
...
function GetUpcheJumunDeliverPay(frm, makerid){
    if (!frm.Deliverdetailidx) return 0;
    if ((!IsCancelProcess)&&(!IsReturnProcess)) return 0;

	if (frm.Deliverdetailidx.length > 1) {
		for (var i = 0; i < frm.Deliverdetailidx.length; i++) {
			if ((makerid == "10x10logistics") && (frm.DeliverMakerid[i].value == "")) {
				return frm.Deliveritemcost[i].value*1;
			} else if (frm.DeliverMakerid[i].value == makerid) {
				return frm.Deliveritemcost[i].value*1;
			}
		}
	} else {
		if ((makerid == "10x10logistics") && (frm.DeliverMakerid.value == "")) {
			return frm.Deliveritemcost.value*1;
		} else if (frm.DeliverMakerid.value == makerid) {
			return frm.Deliveritemcost.value*1;
		}
	}
...
function GetUpcheDeliverPay(makerid) {
	...
	if ((makerid == "10x10logistics") || (makerid == "")) {
...
function CheckDeliverPay(frm) {
	...
	if (IsCancelProcess != true && IsReturnProcess != true) { return; }

	frm.milereturn.checked = allselected;
    // frm.couponreturn.checked = allselected;

	if (IsReturnProcess) {
		makerid = GetSelectedMakerID(frm);
		...
		if (value_gubun02=="CD01") {
			CheckTenItemAllSelected(frm)
			CheckUpcheItemAllSelected(frm)
			SELECTEDUPCHEDEFAULTBEASONGPAY = GetUpcheDeliverPay(makerid);

			if (makerid == "10x10logistics") {
				if ((GetUpcheJumunDeliverPay(frm, makerid) == 0) && (CheckTenItemAllSelected(frm) == true)) {
					frm.ckreturnpay.checked = true;
					CheckDoubleCheck(frm, frm.ckreturnpayHalf);
				} else {
					frm.ckreturnpayHalf.checked = true;
					CheckDoubleCheck(frm, frm.ckreturnpayHalf);
				}
			} else {
				if ((GetUpcheJumunDeliverPay(frm, makerid) == 0) && (CheckUpcheItemAllSelected(frm, makerid) == true)) {
					frm.ckreturnpay.checked = true;
					CheckDoubleCheck(frm, frm.ckreturnpayHalf);
				} else {
					frm.ckreturnpayHalf.checked = true;
    ...
    if ((IsCancelProcess)||(IsReturnProcess)){
        if (frm.subtotalprice!=undefined){
            ...
            if (frm.returnmethod){
				...
				if (orgpayPrice != 0) {
					if (orgpayPrice != frm.canceltotal.value*1) {
						if ((frm.returnmethod.value=="R020") || (frm.returnmethod.value=="R100")) {
							alert('취소금액과 최초결제금액이 다릅니다.\n\n다른 환불수단을 선택하세요.');
							<% if (session("ssBctId")<>"icommang") and (session("ssBctId")<>"iroo4") then %>
							return;
							<% end if %>
						}
					} else if (orgpayPrice == frm.canceltotal.value*1) {
						if ((frm.returnmethod.value=="R022") || (frm.returnmethod.value=="R120")) {
							alert('취소금액과 최초결제금액이 같습니다.\n\n카드취소 또는 실시간이체취소를 선택하세요.');
							<% if (session("ssBctId")<>"icommang") and (session("ssBctId")<>"iroo4") then %>
							return;
							<% end if %>
						}
...
function CsRegCancelBeasongPayProc(frm, orderdetailidx) {
	...
	for (var i = 0; i < frm.length; i++) {
		e = frm.elements[i];
		if (e.name == "Deliverdetailidx") {
			if (e.value*1 == orderdetailidx*1) {
				e.checked = true;
				AnCheckClick(e);
				CalculateAndApplyItemCostSum(frm);
				break;
...
function IsAllSelected(frm){
	...
	if (frm.regitemno.length == undefined) {
		if (frm.orderdetailidx.checked != true) {
			if (frm.prevreturnno.value != frm.itemno.value) {
				return false;
			}
		} else {
			if ((frm.regitemno.value*1 + frm.prevreturnno.value*1) != frm.itemno.value*1) {
				return false;
			}
		}
	} else {
		for (var i = 0; i < frm.regitemno.length; i++) {
			if (frm.orderdetailidx[i].checked != true) {
				if (frm.prevreturnno[i].value != frm.itemno[i].value) {
					return false;
				}
			} else {
				if ((frm.regitemno[i].value*1 + frm.prevreturnno[i].value*1) != frm.itemno[i].value*1) {
					return false;
				}
			}
		}
...
function SaveCheckedItemList(frm) {
    ...
    } else if (upchedeliverPayStr.length>0) {
		frm.detailitemlist.value = frm.detailitemlist.value + "|" + upchedeliverPayStr
	}
	//--------------------------------------------------
	...
	if ((Fdivcd=="A009")||(Fdivcd=="A002")||(Fdivcd=="A003")||(Fdivcd=="A005")||(Fdivcd=="A006")||(Fdivcd=="A700")){
		...
		if (!checkitemExists && (upchedeliverPayStr.length<1)){
...
function CalculateAndApplyItemCostSum(frm) {
    ...
    for (var i = 0; i < frm.length; i++) {
        ...
        if ((ischecked == true) && (e.name == "discountAssingedCost")) {
            if ((e.value * 0) == 0) {
                discountAssingedCost = e.value;
            } else {
                discountAssingedCost = 0;
            }

			percentBonusCouponDiscount = itemcost - discountAssingedCost;
        }
	...
	frm.remaincouponsum.value = frm.orgcouponsum.value*1 - frm.refundcouponsum.value*1;
...
//    }
    ...
    if (frm.ckreturnpay!=undefined){
        if (frm.ckreturnpayHalf.checked){
            refunddeliverypay = SELECTEDUPCHEDEFAULTBEASONGPAY*-1;

        }else if (frm.ckreturnpay.checked){
            refunddeliverypay = SELECTEDUPCHEDEFAULTBEASONGPAY*2*-1;
        ...
        <tr bgcolor="#F4F4F4">
                ...
                <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="#BABABA">
                        <tr height="20" align="center" bgcolor="#F4F4F4">
                          ...
                          <% if (ocsaslist.FOneItem.IsCancelProcess) then %>
                          	  취소/원주문
                          <% else %>
							  접수/원주문<% if (IsRegState and ocsaslist.FOneItem.IsReturnProcess) then %><br>(기존반품)<% end if %>
                          <% end if %>
                          </td>
                          <td width="60">판매가</td>
						  <td width="60">쿠폰가</td>
            ...
            <% for i=0 to ocsOrderDetail.FResultCount-1 %>
                ...
                %>
                        ...
                        <% end if %>
							...
							<input type="hidden" name="DeliverdiscountAssingedCost" value="<%= ocsOrderDetail.FItemList(i).FdiscountAssingedCost %>">
                            <% end if %>
                            </td>
                            <td>배송비</td>
                            ...
                            <td align="left">
								(<%= baesongmethodstr %>)
								<% if (IsRegState and (ocsOrderDetail.FItemList(i).FCancelyn<>"Y") and divcd="A008") then %>
								&nbsp;&nbsp;
								<input type="button" class="button" value="배송비취소" onClick="CsRegCancelBeasongPayProc(frmaction, <%= ocsOrderDetail.FItemList(i).Forderdetailidx %>);" <% if (ocsOrderDetail.FItemList(i).FdiscountAssingedCost = 0) or (ocsOrderDetail.FItemList(i).Fprevreturnno <> 0) or (ocsOrderDetail.FItemList(i).ForderDetailcurrstate = "7") then %>disabled<% end if %> >
								<% end if %>
							</td>
                            <td >
								<%= ocsOrderDetail.FItemList(i).Fitemno %>
								<% if (IsRegState and ocsaslist.FOneItem.IsReturnProcess) then %>
								<% if (ocsOrderDetail.FItemList(i).Fprevreturnno <> 0) then %>
								<br>(<%= ocsOrderDetail.FItemList(i).Fprevreturnno %>)
								<% end if %>
								<% end if %>
							</td>
                            <td align="right"><%= FormatNumber(ocsOrderDetail.FItemList(i).Fitemcost,0) %></td>
							<td align="right">
								<% if ocsOrderDetail.FItemList(i).FdiscountAssingedCost<>ocsOrderDetail.FItemList(i).Fitemcost then %>
								<font color="blue"><%= FormatNumber(ocsOrderDetail.FItemList(i).FdiscountAssingedCost,0) %></font>
								<% else %>
								<%= FormatNumber(ocsOrderDetail.FItemList(i).FdiscountAssingedCost,0) %>
								<% end if %>
							</td>
                            <td></td>
							<input type="hidden" name="DeliverCancelyn" value="<%= ocsOrderDetail.FItemList(i).FCancelyn %>">
                        </tr>
                <% else %>
                        <%
                            if (ocsOrderDetail.FItemList(i).FCancelyn<>"Y") then
                        ...
                        %>
                        	...
                        	<td>
								...
								<% if (IsRegState and ocsaslist.FOneItem.IsReturnProcess) then %>
								<% if (ocsOrderDetail.FItemList(i).Fprevreturnno <> 0) then %>
								<br><b>(<%= ocsOrderDetail.FItemList(i).Fprevreturnno %>)</b>
								<% end if %>
								<% end if %>
								<% if ocsOrderDetail.FItemList(i).Fprevreturnno = "" or IsNull(ocsOrderDetail.FItemList(i).Fprevreturnno) then ocsOrderDetail.FItemList(i).Fprevreturnno = 0 end if %>
								<input type="hidden" name="prevreturnno" value="<%= ocsOrderDetail.FItemList(i).Fprevreturnno %>">
                        	</td>
                        	<input type="hidden" name="itemcost" value="<%= ocsOrderDetail.FItemList(i).Fitemcost %>">
							<input type="hidden" name="discountAssingedCost" value="<%= ocsOrderDetail.FItemList(i).FdiscountAssingedCost %>">
                        	<!-- 국민카드 할인으로인해 변경함 -->
                        	...
                        	<td align="right"><font color="gray"><%= FormatNumber(ocsOrderDetail.FItemList(i).Fitemcost,0) %></font></td>
                           	...
                           	<% else %>
                           	<td align="right">
                           	    <%= FormatNumber(ocsOrderDetail.FItemList(i).Fitemcost,0) %>
                           	</td>
                           	<% end if %>
							<td align="right">
								<% if ocsOrderDetail.FItemList(i).FdiscountAssingedCost<>ocsOrderDetail.FItemList(i).Fitemcost then %>
                            ...
                            <input type="hidden" name="isupchebeasong_<%= distinctid %>" value="<%= ocsOrderDetail.FItemList(i).Fisupchebeasong %>">
							<input type="hidden" name="makerid_<%= distinctid %>" value="<%= ocsOrderDetail.FItemList(i).Fmakerid %>">
							...
							<input type="hidden" name="cancelyn" value="<%= ocsOrderDetail.FItemList(i).FCancelyn %>">
            ...
            <% next %>
            	<tr bgcolor="FFFFFF" height="20">
            	    <td colspan="7"></td>
            	    <td>상품합계금액</td>
            	    <td align="right"><input type="text" name="disporgitemcostsum" value="<%= orgitemcostsum %>" size="7" readonly style="text-align:right;border: 1px solid #CCCCCC;" ></td>
            	    <td colspan="2"></td>
            	</tr>
        ...
        <% if (IsCancelInfoDisplay) then %>
            ...
            <tr bgcolor="FFFFFF">
        		...
        		<td align="right" width="70">
					<%= FormatNumber(orefund.FOneItem.Forgitemcostsum,0) %>
					<input type="hidden" name="orgitemcostsum" value="<%= orefund.FOneItem.Forgitemcostsum %>">
				</td>
        	...
        	<tr bgcolor="FFFFFF">
        		<td>주문시 배송비</td>
        		<td><div id="beasongpayAssign" ><input <% if (IsFinishProcState) then response.write "disabled" %> type="checkbox" name="ckbeasongpayAssign" <% if (ckAll<>"") or (orefund.FOneItem.Frefundbeasongpay>0) then response.write "checked" %> value="" onclick="CalculateAndApplyItemCostSum(frmaction);" disabled><font color="red">환급</font></div></td>
        		<td align="right">
        	...
        	<tr bgcolor="FFFFFF">
        		...
        		<td></td>
        		<td align="right">
					<%= FormatNumber(orefund.FOneItem.Forgcouponsum * -1,0) %>
					<input type="hidden" name="orgcouponsum" value="<%= orefund.FOneItem.Forgcouponsum * -1 %>">
				</td>
        		<td align="right">
        	...
        	<tr bgcolor="FFFFFF">
        		...
        		<td>
					<input type="checkbox" name="ckreturnpay" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)" <% if (IsFinishProcState) then response.write "disabled" %>>
        			왕복배송비
            		<br>
            		<input type="checkbox" name="ckreturnpayHalf" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)" <% if (IsFinishProcState) then response.write "disabled" %>>
            		회수배송비
					<br>
					<input type="checkbox" name="ckreturnpayZero" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)" <% if (IsFinishProcState) then response.write "disabled" %>>
            		차감안함
        		</td>
        		<td></td>
        	...
        	<tr bgcolor="FFFFFF">
        		<td colspan="4" style="padding-left: 10; padding-top: 5; padding-bottom: 5; ">
					* 단순변심 + 브랜드전체반품 + 무료배송 : 업체왕복배송비<br>
					* 기타 단순변심반품 : 회수배송비<br>
					* 상품불량반품 등 : 차감없음<br>
            ...
            <tr bgcolor="FFFFFF">
        		...
        		<td width="120"></td>
        		<td align="right" width="70">
					<%= FormatNumber(orgitemcostsum,0) %>
					<input type="hidden" name="orgitemcostsum" value="<%= orgitemcostsum %>">
				</td>
        	...
        	<tr bgcolor="FFFFFF">
        		<td>주문시 배송비</td>
        		<td><div id="beasongpayAssign" ><input type="checkbox" name="ckbeasongpayAssign" <% if (ckAll<>"") then response.write "checked" %>  value="" onclick="CalculateAndApplyItemCostSum(frmaction);" disabled><font color="red">배송비전체 환급</font></div></td>
        		<td align="right">
        	...
        	<tr bgcolor="FFFFFF">
        		...
        		<td></td>
        		<td align="right">
					<%= FormatNumber(oordermaster.FOneItem.Ftencardspend * -1,0) %>
					<input type="hidden" name="orgcouponsum" value="<%= oordermaster.FOneItem.Ftencardspend*-1 %>">
				</td>
        		<td align="right">
        		    <input class="text_ro" type="text" name="refundcouponsum" value="0" size="9" style="text-align:right" readonly>
        		</td>
        		<td align="right">
        		    <input class="text_ro" type="text" name="remaincouponsum" value="<%= oordermaster.FOneItem.Ftencardspend*-1 %>" size="9" style="text-align:right" readonly>
        		</td>
        	</tr>
        	...
        	<tr bgcolor="FFFFFF">
        		...
        		<td>
            		...
            		회수배송비
					<br>
					<input type="checkbox" name="ckreturnpayZero" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)">
            		차감안함
        		</td>
        		<td></td>
    ...
    <td bgcolor="#FFFFFF" valign="top" align="left">
        ...
        <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="a" bgcolor="BABABA">
	        ...
	        <tr bgcolor="#FFFFFF" height="25">
	            ...
	            <td>
	                <% if (orgpayPrice <> 0) then %>
					최초결제 : <%= FormatNumber(orgpayPrice,0) %>원
					<% end if %>
	            </td>
	        </tr>
	        <tr bgcolor="#FFFFFF">
	            ...
	            <td>
	                <%
					'// /cscenterv2/lib/classes/cs/cs_aslistcls.asp
...
<tr>
        ...
        <% if (ocsaslist.FOneITem.FDeleteyn="N") then %>
            ...
            <% else %>
                ...
                <input class="csbutton" type="button" value=" 접수내용 수정 " onClick="CsRegEditProc(frmaction)" onFocus="blur()">
                    <% if (IsUpcheConfirmState) then %>
                    &nbsp;&nbsp;&nbsp;&nbsp;
                    <input class="csbutton" type="button" value="접수 상태로 변경" onClick="CsUpcheConfirm2RegProc(frmaction)" onFocus="blur()">
                    <% end if %>
                <% end if %>
            <% end if %>
    ...
    <% elseif (IsStateFinished) then %>
		...
		<% elseif C_ADMIN_AUTH then %>
			<input class="csbutton" type="button" value="[관리자]접수 상태로 변경" onClick="CsRegStateChg(frmaction)" onFocus="blur()">
		<% end if %>
    <% end if %>
    </td>
</tr>
