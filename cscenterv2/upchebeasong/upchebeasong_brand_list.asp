...
function PopUpchebeasongList(imakerid, idetailstate){
    var popwin = window.open("upchebeasonglist.asp?makerid=" + imakerid + "&cdl=<%= cdl %>&yyyy1=<%= yyyy1 %>&mm1=<%= mm1 %>&dd1=<%= dd1 %>&yyyy2=<%= yyyy2 %>&mm2=<%= mm2 %>&dd2=<%= dd2 %>&detailstate=" + idetailstate,"PopUpchebeasongList","width=1000 height=800 scrollbars=yes resizable=yes");
    popwin.focus();
}
...
function popUpcheMisend(makerID, dplusOver, currState)
{
	...
	var url = "/cscenterv2/upchebeasong/upchemibeasonglist.asp?menupos=246&MisendState=";
	url += "&makerID=" + makerID;
...
<table width="100%" align="center" cellpadding="3" cellspacing="1" class="a" bgcolor="<%= adminColor("tablebg") %>">
    ...
    %>
	<tr align="center" bgcolor="#FFFFFF">
	    <td height="40"><a href="javascript:popSimpleBrandInfo('<%= ojumun.FMasterItemList(i).FMakerid %>');"><%= ojumun.FMasterItemList(i).FMakerid %></a></td>
	    ...
	    <td>
	        ...
	        <% if ojumun.FMasterItemList(i).FNDayMiBaljuCnt>0 then %>
				<font color="red"><b><%= ojumun.FMasterItemList(i).FNDayMiBaljuCnt %></b></font>
				<% else %>
	    ...
	    <td>
			...
			<% if ojumun.FMasterItemList(i).FNDayMiBeasongCnt>0 then %>
				<font color="red"><b><%= ojumun.FMasterItemList(i).FNDayMiBeasongCnt %></b></font>
	        <% else %>
	    ...
	    <td><%= ojumun.FMasterItemList(i).FDeliverPhone %><br><%= ojumun.FMasterItemList(i).FDeliverHp %></td>
	</tr>
	<% end if %>
	<% next %>
	<tr align=center bgcolor="#FFFFFF">
