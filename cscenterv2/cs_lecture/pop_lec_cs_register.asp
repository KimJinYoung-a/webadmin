...
function CsRegProc(frm){
    ...
    if (IsCancelProcess){
    	...
    	if ((frm.cklecturepay0.checked == false) && (frm.ckmaterialpay20.checked == false) && (frm.ckmaterialpay10.checked == false)) {
    		alert('차감액을 선택하세요.');
    		return;
    	}

        if(confirm("강좌확정 후 일부환불 접수 하시겠습니까?")){
            frm.submit();
        }
        /*
...
function CalculateAndApplyItemCostSum(frm) {
	...
	if ((IsReturnProcess == true) && (frm.cklecturepay0.checked != true) && (frm.ckmaterialpay20.checked != true) && (frm.ckmaterialpay10.checked != true)) {
		alert("차감방식을 먼저 선택하세요.");
		return;
	}
    ...
    for (var i = 0; i < frm.length; i++) {
        ...
        if ((ischecked == true) && (e.name == "matinclude_yn")) {
            matinclude_yn = e.value;
        }

        if ((ischecked == true) && (e.name == "matcostAdded")) {
            if ((e.value * 0) == 0) {
                matcostAdded = e.value;
            } else {
                matcostAdded = 0;
            }
        }
        ...
        if (e.name == "dummystopper") {
            if (ischecked == true) {
				...
				if (IsReturnProcess != true) {
					...
					if (frm.cklecturepay0.checked == true) {
						// 강좌료 전액환불
						// 재료비 포함 환불로 변경(skyer9, 2014-09-02)
						//if (matinclude_yn == "C") {
						//	refunditemcostsum = refunditemcostsum + ((itemcost - matcostAdded) * regitemno * 1);
						//} else {
			                refunditemcostsum = refunditemcostsum + (itemcost * regitemno * 1);
						//}
		                allatitemdiscountSum = allatitemdiscountSum + (allatitemdiscount * regitemno * 1);
		                percentBonusCouponDiscountSum = percentBonusCouponDiscountSum + (percentBonusCouponDiscount * regitemno * 1);
					} else if ((frm.ckmaterialpay20.checked == true) && (matinclude_yn == "C")) {
						// 재료비환불 20% 차감
						refunditemcostsum = refunditemcostsum + parseInt(matcostAdded * 0.8 * regitemno * 1);

					} else if ((frm.ckmaterialpay10.checked == true) && (matinclude_yn == "C")) {

						// 재료비환불 10% 차감
						refunditemcostsum = refunditemcostsum + parseInt(matcostAdded * 0.9 * regitemno * 1);

					} else {
						refunditemcostsum = 0
					}

	                allatitemdiscountSum = 0;
	                percentBonusCouponDiscountSum = 0;
				}
...
function CheckDoubleCheck(frm,comp){
    if (comp.name=="cklecturepay0"){
        if (frm.cklecturepay0.checked){
            frm.ckmaterialpay20.checked = false;
            frm.ckmaterialpay10.checked = false;
        }
    } else if (comp.name=="ckmaterialpay20"){
        if (frm.ckmaterialpay20.checked){
            frm.cklecturepay0.checked = false;
            frm.ckmaterialpay10.checked = false;
        }
    } else if (comp.name=="ckmaterialpay10"){
        if (frm.ckmaterialpay10.checked){
            frm.cklecturepay0.checked = false;
            frm.ckmaterialpay20.checked = false;
        }
    }
...
<form name="frmaction" method="post" action="pop_lec_cs_process.asp">
                ...
                %>
                        ...
                        <% end if %>
                            ...
                            <% if (True) or (ocsOrderDetail.FItemList(i).IsUpcheParticleDeliverPayCodeItem) then %>
                                <% if (IsRegState) then %>
                                <input type="checkbox" name="Deliverdetailidx" value="<%= ocsOrderDetail.FItemList(i).Forderdetailidx %>" <% if (Not ocsOrderDetail.FItemList(i).IsCheckAvailItem(oordermaster.FOneItem.FIpkumDiv,oordermaster.FOneItem.FCancelYn,divcd)) then %> disabled<% end if %> onClick="AnCheckClick(this); CalculateAndApplyItemCostSum(frmaction);">
                                <% else %>
                ...
                <% else %>
                        <%
							...
							if (ocsaslist.FOneItem.IsReturnProcess) then
								if (ocsOrderDetail.FItemList(i).Fmatinclude_yn = "C" and orefund.FOneItem.Frefundmatdiv = "2") then
									regitemcostsum = regitemcostsum + CLng(FormatNumber(ocsOrderDetail.FItemList(i).GetDefaultRegNo(IsRegState)*ocsOrderDetail.FItemList(i).FmatcostAdded * 0.8, 0))
								elseif (ocsOrderDetail.FItemList(i).Fmatinclude_yn = "C" and orefund.FOneItem.Frefundmatdiv = "1") then
									regitemcostsum = regitemcostsum + CLng(FormatNumber(ocsOrderDetail.FItemList(i).GetDefaultRegNo(IsRegState)*ocsOrderDetail.FItemList(i).FmatcostAdded * 0.9, 0))
								else
									regitemcostsum = regitemcostsum + 0
								end if
							else
                        ...
                        %>
                        	...
                        	<td>
                        	    <% if (Not IsRegState) then %>
                        	        ...
                        	        <input type="text" name="regitemno" value="<%= ocsOrderDetail.FItemList(i).GetDefaultRegNo(IsRegState) %>" size="2" style="text-align:center" onKeyUp="CheckMaxItemNo(this, '<%= ocsOrderDetail.FItemList(i).FItemNo %>'); CalculateAndApplyItemCostSum(frmaction);" >
                        	        <% else %>
                        	        ...
                        	        <% end if %>
                        	    <% else %>
                        	    <input type="text" name="regitemno" value="<%= ocsOrderDetail.FItemList(i).GetDefaultRegNo(IsRegState) %>" size="2" style="text-align:center" onKeyUp="CheckMaxItemNo(this, '<%= ocsOrderDetail.FItemList(i).FItemNo %>'); CalculateAndApplyItemCostSum(frmaction);" <% if Not ocsOrderDetail.FItemList(i).IsItemNoEditEnabled(divcd) then response.write "style='text-align:center;background-color:#DDDDFF;' readonly" %> >
                        	    <% end if %>
                        	    /
                        	...
                        	<input type="hidden" name="itemcost" value="<%= ocsOrderDetail.FItemList(i).Fitemcost %>"><!-- 강좌 할인액 -->
                        	<input type="hidden" name="matcostAdded" value="<%= ocsOrderDetail.FItemList(i).FmatcostAdded %>"><!-- 재료비 -->
                        	<input type="hidden" name="matinclude_yn" value="<%= ocsOrderDetail.FItemList(i).Fmatinclude_yn %>"><!-- 포함여부(C : 포함) -->
                        	<input type="hidden" name="refundstate" value="<%= ocsOrderDetail.FItemList(i).Frefundstate %>"><!-- 환불상태 -->
							...
							<td>
								<%= db2html(ocsOrderDetail.FItemList(i).Fentryname) %><br>
								<%= db2html(ocsOrderDetail.FItemList(i).Fentryhp) %>
							</td>
							<td align="right" >
								<% if ocsOrderDetail.FItemList(i).Fmatinclude_yn = "C" then %>
								<%= FormatNumber((ocsOrderDetail.FItemList(i).FcouponNotAsigncost - ocsOrderDetail.FItemList(i).FmatcostAdded),0) %>
								<% else %>
								<%= FormatNumber(ocsOrderDetail.FItemList(i).FcouponNotAsigncost,0) %>
								<% end if %>
							</td>
							<td align="right" >
								<%= FormatNumber(ocsOrderDetail.FItemList(i).FmatcostAdded,0) %><br>
								<% if ocsOrderDetail.FItemList(i).Fmatinclude_yn = "C" then %>
								[선납]
								<% else %>
								[현장]
							...
							<td align="right"><%= FormatNumber(ocsOrderDetail.FItemList(i).FcouponNotAsigncost,0) %></td>
		                   	<td align="right">
		                   		<% if (ocsOrderDetail.FItemList(i).FCancelyn="Y") then %>
		                   			<font color="gray"><%= FormatNumber(ocsOrderDetail.FItemList(i).FreducedPrice,0) %></font>
		                   		<% elseif ocsOrderDetail.FItemList(i).FItemNo < 1 then %>
									<font color="red"><%= FormatNumber(ocsOrderDetail.FItemList(i).FreducedPrice,0) %></font>
		                   		<% else %>
			                   	    <% if ocsOrderDetail.FItemList(i).Fissailitem="Y" then %>
				                   	    <span title="세일상품" style="cursor:hand"><font color="red"><b><%= FormatNumber(ocsOrderDetail.FItemList(i).FreducedPrice,0) %></b></font></span>
			                   	    <% elseif ocsOrderDetail.FItemList(i).Fissailitem="P" then %>
				                   	    <span title="플러스세일상품" style="cursor:hand"><font color="purple"><%= FormatNumber(ocsOrderDetail.FItemList(i).FreducedPrice,0) %></font></span>
			                   	    <% elseif ocsOrderDetail.FItemList(i).IsBonusCouponDiscountItem then %>
				                   	    <span title="비율보너스쿠폰할인상품" style="cursor:hand">
				                   	    <font color="blue">
				                   	        <%= FormatNumber(ocsOrderDetail.FItemList(i).FreducedPrice,0) %><br>
				                   	        <font color="#000000">(<%= FormatNumber(ocsOrderDetail.FItemList(i).FreducedPrice,0) %>)</font>
				                   	    </font>
				                   	    </span>
			                   	    <% elseif ocsOrderDetail.FItemList(i).IsItemCouponDiscountItem then %>
				                   	    <span title="상품보너스쿠폰할인상품" style="cursor:hand"><font color="green"><b><%= FormatNumber(ocsOrderDetail.FItemList(i).FreducedPrice,0) %></b></font></span>
			                   	    <% else %>
				                   	    <span title="정상가격" style="cursor:hand"><font color="#000000"><%= FormatNumber(ocsOrderDetail.FItemList(i).FreducedPrice,0) %></font></span>
			                   	    <% end if %>
		                   		<% end if %>
		                	</td>
			                <td align="right" width="70">
			                	<span title="<%= ocsOrderDetail.FItemList(i).GetRefundStateName %>" style="cursor:hand"><font color="<%= ocsOrderDetail.FItemList(i).GetRefundStateColor %>"><%= FormatNumber(ocsOrderDetail.FItemList(i).GetRefundPrice,0) %></font></span>
			            	</td>
                            <td align="center">
    ...
    <td bgcolor="#FFFFFF" width="500" valign="top">
        ...
        <% if (IsCancelInfoDisplay) then %>
        	...
        	<tr bgcolor="FFFFFF">
        		<td>차감액</td>
        		<td>
	        		<input <% if ((IsFinishProcState or orefund.FOneItem.Frefundmatdiv <> "") and (IsFinishProcState or orefund.FOneItem.Frefundmatdiv <> "L")) then response.write "disabled" %>  type="checkbox" name="cklecturepay0" value="L" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)" <% if (orefund.FOneItem.Frefundmatdiv = "L") then response.write "checked" %> > <font color="red">전액 환불</font><br>
	        		<input <% if ((IsFinishProcState or orefund.FOneItem.Frefundmatdiv <> "") and (IsFinishProcState or orefund.FOneItem.Frefundmatdiv <> "2")) then response.write "disabled" %>  type="checkbox" name="ckmaterialpay20" value="2" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)" <% if (orefund.FOneItem.Frefundmatdiv = "2") then response.write "checked" %> > -20% 재료비 차감<br>
	        		<input <% if ((IsFinishProcState or orefund.FOneItem.Frefundmatdiv <> "") and (IsFinishProcState or orefund.FOneItem.Frefundmatdiv <> "1")) then response.write "disabled" %>  type="checkbox" name="ckmaterialpay10" value="1" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)"  <% if (orefund.FOneItem.Frefundmatdiv = "1") then response.write "checked" %> > -10% 재료비 차감
        		</td>
        		<td></td>
        	...
        	<tr bgcolor="FFFFFF">
        		<td>차감액</td>
        		<td>
            		<input type="checkbox" name="cklecturepay0" value="L" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)">  <font color="red">전액 환불</font><br>
            		<input type="checkbox" name="ckmaterialpay20" value="2" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)"> -20% 재료비 차감<br>
            		<input type="checkbox" name="ckmaterialpay10" value="1" onClick="CheckDoubleCheck(frmaction,this);CalculateAndApplyItemCostSum(frmaction)"> -10% 재료비 차감
        		</td>
        		<td></td>
