...
function GetDefaultTitle(divcd, id, orderserial)
    ...
    sqlStr = sqlStr + " left join " & TABLE_CS_COMMON_CODE & " C"
...
function RegCSUpcheAddJungsanPay(iasid, iadd_upchejungsandeliverypay, iadd_upchejungsancause, buf_requiremakerid)
    ...
    sqlStr = " insert into [db_academy].[dbo].[tbl_academy_as_upcheAddjungsan]"
...
function EditCSUpcheAddJungsanPay(iasid, iadd_upchejungsandeliverypay, iadd_upchejungsancause, buf_requiremakerid)
    ...
    sqlStr = " IF EXISTS( select * from [db_academy].[dbo].[tbl_academy_as_upcheAddjungsan] where asid=" & iasid & ")" & VbCrlf
    sqlStr = sqlStr + " BEGIN" & VbCrlf
    sqlStr = sqlStr + "     update [db_academy].[dbo].[tbl_academy_as_upcheAddjungsan]" & VbCrlf
    ...
    sqlStr = sqlStr + "     BEGIN" & VbCrlf
    sqlStr = sqlStr + "         insert into [db_academy].[dbo].[tbl_academy_as_upcheAddjungsan]" & VbCrlf
...
function updateUserMileage(byVal userid)
	...
	'==============================================================
	'주문마일리지 요약 재계산
    sqlStr = " select IsNull(sum(totalmileage), 0) as totmile, IsNull(sum(case when sitename = 'academy' and ipkumdiv < 7 then totalmileage when sitename = 'diyitem' and ipkumdiv < 8 then totalmileage else 0 end),0) as michulgoMile " + VbCrlf
...
''         end if
    ...
    ''전자보증서 발급된 경우 취소
    if (InsureCd="0") and (IsAllCancel) then
...
function AddMinusOrder(id, orderserial)
	...
	neworderserial = "Y" & Mid(replace(CStr(DateSerial(Year(now),month(now),Day(now))),"-",""),4,256)
    ...
    sqlStr = sqlStr + " ,pggubun=O.pggubun" & vbCrlf  ''2016/09/09 추가
...
'	end if
	...
	sqlStr = sqlStr + " (masteridx, orderserial,itemid,itemoption,itemno," & vbCrlf
    sqlStr = sqlStr + " itemcost," & FIELD_ITEMVAT & ",mileage,reducedPrice,itemname," & vbCrlf
    sqlStr = sqlStr + " itemoptionname,makerid,buycash,vatinclude,isupchebeasong,issailitem,oitemdiv,omwdiv,odlvType,currstate,beasongdate,upcheconfirmdate)" & vbCrlf
    sqlStr = sqlStr + " select " & CStr(iid) & vbCrlf
    sqlStr = sqlStr + " ,'" & neworderserial & "'" & vbCrlf
    sqlStr = sqlStr + " ,d.itemid" & vbCrlf
    sqlStr = sqlStr + " ,d.itemoption" & vbCrlf
