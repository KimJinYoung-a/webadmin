...
''' 발주시 사은품 작성. Table : [db_order].[dbo].tbl_order_gift_balju => [db_order].[dbo].tbl_order_gift
''' 관련 Procedure [db_order].[dbo].ten_order_Gift_Maker : 발주번호로 사은품 목록 생성.
...
Class COrderGift
    ...
    public FRectBaljuid				'입력 받아올 발주id
    ...
    public function GetOneOrderGiftlist()
		...
		sqlStr = sqlStr & " o.*, g.evt_code, g.gift_name, g.gift_itemname, k.giftkind_name , g.giftkind_givecnt, e.evt_name, g.giftkind_cnt as giftkind_orgcnt, g.makerid "
		sqlStr = sqlStr & " from " & TABLE_ORDER_GIFT & " o"
...
'response.write 		sqlStr
    ...
    public Sub GeteventOrderGiftcount()			'이벤트(사은품) 발주리스트 페이지 ( 그룹:합계 )
        dim sqlStr,i

		sqlStr = "select "
			if frectdate_display <> "on" then				                '날짜표시가 x 일경우
				if frectdateview1 = "no" then				                '발주일 기준
					sqlStr = sqlStr & " convert(varchar(10),bm.baljudate,21) as convdate,"
				elseif frectdateview1 = "yes" then			                    '주문일 기준 = tbl_order_gift 에 넣은날짜
        ...
        if (FRectBaljuid = "") and (FRecteventid = "") and (frectdateview="") and (FRectgift_code="") then		' 이벤트 아이디나 발주 아이디 / 날짜 없으면 뿌리지 않음.
    ...
    public Sub GeteventOrderGiftList()			'이벤트(사은품) 발주리스트 페이지 ( 그룹:내역 )
        dim sqlStr,i
