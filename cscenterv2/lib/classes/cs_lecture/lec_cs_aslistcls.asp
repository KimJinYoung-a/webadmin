...
function drawSelectBoxCSCommCombo(selectBoxName,selectedId,groupCode,onChangefunction)
   ...
   <%
       ...
       sqlStr = sqlStr + " from  "
       sqlStr = sqlStr + " [db_academy].[dbo].tbl_academy_cs_lecutre_comm_code "
...
'        end if
    ...
    public function IsItemNoEditEnabled(byval idivcd)
        ...
        elseif (fnIsReturnProcess(idivcd)) then
            ''반품 접수
			if IsNull(Frefundstate) then Frefundstate = ""
			if Frefundstate <> "0" then IsItemNoEditEnabled=true
    ...
    public function IsCheckAvailItem(byval iIpkumdiv, byval iMasterCancelYn, byval idivcd)
        ...
        elseif (fnIsReturnProcess(idivcd)) then
            ''반품 접수
            if IsNull(Frefundstate) then Frefundstate = ""
            if (iIpkumdiv>=4) and Frefundstate <> "0" then IsCheckAvailItem=true
...
Class CCSASRefundInfoItem
    ...
    public Frefundmatdiv        ''강좌 재료비환불 구분(1:10%, 2:20%)
...
Class CCSReturnAddress
    ...
    public sub GetBrandReturnAddress()
        ...
        sqlStr = sqlStr + " from [db_partner].[dbo].tbl_partner p, [db_user].[dbo].tbl_user_c c "
...
Class CCSASMasterItem
    ...
    public FlectureStartDate	'강좌시작일
    ...
    public function IsAsRegAvail(byval iIpkumdiv, byval iCancelYn, byref descMsg)
        ...
        elseif (IsReturnProcess) then
            if Not ((iIpkumdiv=4) or (iIpkumdiv=7) or (iIpkumdiv=8)) then
                IsAsRegAvail = false
                descMsg      = "결재 완료/출고 완료/ 일부 출고 상태가 아닙니다. - 환불 접수 불가능 "
                exit function
            end if

            if (iCancelYn<>"N") then
                IsAsRegAvail = false
                descMsg      = "취소된 거래입니다. - 환불 접수 불가능 "
                exit function
            end if
...
Class CCSASList
    ...
    public Sub GetOneRefundInfo()
		...
		sqlStr = sqlStr + " , (CASE WHEN r.encmethod='PH1' THEN IsNull(db_academy.dbo.uf_DecAcctPH1(r.encaccount), '') ELSE '' END) as decaccount "
        sqlStr = sqlStr + " from " & TABLE_CS_REFUND & " r"
        sqlStr = sqlStr + "     left join [db_academy].[dbo].tbl_academy_cs_lecutre_comm_code C1"
        ...
        if Not rsget.Eof then
            ...
            FOneItem.Frefundmatdiv      	= rsget("refundmatdiv")
        end if
        rsget.Close
    end Sub
    ...
    public Sub GetCSASMasterList()
        ...
        sqlStr = sqlStr + " Left Join [db_academy].[dbo].tbl_academy_cs_lecutre_comm_code C2"
        sqlStr = sqlStr + "  on A.gubun01=C2.comm_cd"
        sqlStr = sqlStr + " Left Join [db_academy].[dbo].tbl_academy_cs_lecutre_comm_code C3"
        sqlStr = sqlStr + "  on A.gubun02=C3.comm_cd"
        sqlStr = sqlStr + " Left Join [db_academy].[dbo].tbl_academy_cs_lecutre_comm_code C4"
...
'            FOneItem.Frefundresult        = rsget("refundresult")
    ...
    public Sub GetOrderDetailByCsDetail()
		...
		sqlStr = "select d." & FIELD_DETAILIDX & " as orderdetailidx, d.orderserial,d.itemid,d.itemoption,d.itemno,d.itemcost, d.buycash,d.reducedPrice, d.entryname, d.entryhp, d.matcostAdded, d.matinclude_yn, d.couponNotAsigncost, d.refundstate"
		...
		sqlStr = sqlStr + " ,i.smallimg as smallimage "
...
''            end if
            ...
            FItemList(i).Forgprice          = rsget("couponNotAsigncost")
            FItemList(i).Fissailitem        = rsget("issailitem")
            FItemList(i).Fbonuscouponidx    = rsget("bonuscouponidx")
            FItemList(i).Fitemcouponidx     = rsget("itemcouponidx")
    ...
    public Sub GetCsDetailList()
		...
		sqlStr = sqlStr + " ,d.reducedprice as discountAssingedCost, d.oitemdiv, d.odlvType, d.issailitem, d." & FIELD_ITEMCOUPONIDX & " as itemcouponidx, d.bonuscouponidx,d.reducedPrice, d.entryname, d.entryhp, d.matcostAdded, d.matinclude_yn, d.couponNotAsigncost, d.refundstate"
		...
		sqlStr = sqlStr + " left join [db_academy].[dbo].tbl_lec_item i "
		sqlStr = sqlStr + "  on c.itemid=i.idx"
		sqlStr = sqlStr + " Left Join [db_academy].[dbo].tbl_academy_cs_lecutre_comm_code C2"
        sqlStr = sqlStr + "  on c.gubun01=C2.comm_cd"
        sqlStr = sqlStr + " Left Join [db_academy].[dbo].tbl_academy_cs_lecutre_comm_code C3"
        sqlStr = sqlStr + "  on c.gubun02=C3.comm_cd"
