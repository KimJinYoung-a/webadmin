...
class CPartnerGroup
	...
	public Sub GetGroupInfoListByBrand
		...
		sqlStr = sqlStr + " from " & TABLE_PARTNER_GROUP & " g ,"
		sqlStr = sqlStr + " " & TABLE_PARTNER & " p "
		...
		sqlStr = sqlStr + " from " & TABLE_PARTNER & " p,"
		sqlStr = sqlStr + " " & TABLE_PARTNER_GROUP & " g"
	...
	public Sub GetGroupInfoList
		...
		sqlStr = sqlStr + " from " & TABLE_PARTNER_GROUP & ""
		...
		if FRectSocno<>"" then
		    sqlStr = sqlStr + " and company_no='" + FRectSocno + "'"
		end if
	...
	public Sub GetOneGroupInfo
		dim sqlStr
		sqlStr = "select top 1 * from " & TABLE_PARTNER_GROUP & ""
		...
		if FOneItem.Fgroupid<>"" then
			sqlStr = "select id, isusing from " & TABLE_PARTNER & ""
...
class CPartnerUser
    ...
    public sub GetAcademyPartnerList()
		...
		sqlStr = sqlStr + " from " & TABLE_USER_C & " c"
		sqlStr = sqlStr + "     left join " & TABLE_PARTNER & " p "
	...
	public Sub GetPartnerQuickSearch()
		...
		sqlStr = sqlStr + " p.company_upjong, p.company_uptae, IsNULL(p.groupid,'') as groupid, p.subid, p.password as ppass"
		sqlStr = sqlStr + " from " & TABLE_USER_C & " c"
		sqlStr = sqlStr + " left join " & TABLE_PARTNER & " p on c.userid=p.id"
		sqlStr = sqlStr + " where c.userid<>''" + vbCrlf
	...
	public Sub GetOutBrandList()
		...
		sqlStr = sqlStr + " " & TABLE_USER_C & " c"
		sqlStr = sqlStr + " left join " & TABLE_PARTNER & " p on c.userid=p.id"
		sqlStr = sqlStr + " left join ( "
		sqlStr = sqlStr + " 	select makerid, count(itemid) as cnt"
		sqlStr = sqlStr + " 	from " & TABLE_ITEM & ""
...
'response.write sqlStr
	...
	public Sub GetOnePartnerNUser()
		...
		sqlStr = sqlStr + " left join " & TABLE_PARTNER & " p on c.userid=p.id"
		sqlStr = sqlStr + " left join ( "
		sqlStr = sqlStr + " select makerid, count(itemid) as cnt from " & TABLE_ITEM & " where makerid='" + FRectDesignerID + "'"
		sqlStr = sqlStr + " group by makerid "
		sqlStr = sqlStr + " ) as T on c.userid=T.makerid"
		''택배사 명,전화 추가.
		sqlStr = sqlStr + " left join " & TABLE_SONGJANG_DIV & " s"
...
'response.write sqlstr
	...
	public Sub GetPartnerNUserCList()
		...
		sqlStr = sqlStr + " from " & TABLE_USER_C & " c" + vbCrlf
		sqlStr = sqlStr + " left join " & TABLE_PARTNER & " p on c.userid=p.id"
		sqlStr = sqlStr + " where c.userdiv < 22"
	    ...
	    elseif FRectManagerName<>"" then
			...
			if FrectIsUsing="on" then
				sqlStr = sqlStr + " and c.isusing='Y'"
			elseif FrectIsUsing="off_new" then
		...
		sqlStr = sqlStr + " from " & TABLE_USER_C & " c"
		sqlStr = sqlStr + " left join " & TABLE_PARTNER & " p on c.userid=p.id"
		sqlStr = sqlStr + " where  c.userdiv < 22" + vbCrlf
	...
	public function duplicateUserID(byval userid)
		dim sqlStr
		sqlStr = "select count(id) as cnt from " & TABLE_PARTNER & ""
	...
	public Sub addNewPartner(byval userid,userpass,username,usermail,userdiv, discountrate,commission,bigo)
		...
		sqlStr = "insert into " & TABLE_PARTNER & "" + vbCrlf
        ...
        public Sub editPartner(byval userid,userpass,username,usermail,userdiv, isusing, discountrate,commission,bigo)
	        dim sqlStr
		sqlStr = "update " & TABLE_PARTNER & "" + vbCrlf
	...
	public Sub GetOnePartner(byval userid)
		...
		sqlStr = "select top 1 * from " & TABLE_PARTNER & ""
		sqlStr = sqlStr + " where id='" + CStr(userid) + "'"
		rsget.Open sqlStr,dbget,1
		FTotalCount = rsget.RecordCount
		FResultCount = rsget.RecordCount
	...
	public Function UpdateDesignerSet(byval idesignerid, isusing, isextusing, isb2b)
		dim sqlStr
		sqlStr = "update " & TABLE_USER_C & "" + vbCrlf
...
'====================================================
	...
	public Sub GetDesignerList()
		...
		''총 갯수.
		''#################################################
		sqlStr = "select Count(userid) as cnt from " & TABLE_USER_C & " c, [db_user].[dbo].tbl_user_div d" + vbCrlf
		...
		''현재 페이지 리스트.
		''#################################################
		sqlStr = "select top " + CStr(FPageSize) + " c.*, d.divename from " & TABLE_USER_C & " c, [db_user].[dbo].tbl_user_div d" + vbCrlf
		sqlStr = sqlStr + " where c.userdiv=d.divcode" + vbCrlf
		sqlStr = sqlStr + " and c.userid not in (" + vbCrlf
		sqlStr = sqlStr + " select top " + CStr((FCurrPage-1)*FPageSize)  + " c.userid from " & TABLE_USER_C & " c, [db_user].[dbo].tbl_user_div d" + vbCrlf
		sqlStr = sqlStr + " where c.userdiv=d.divcode" + vbCrlf
	...
	public Sub GetPartnerList(byval ix)
		...
		''총 갯수.
		''#################################################
		sqlStr = "select Count(id) as cnt from " & TABLE_PARTNER & "" + vbCrlf
		...
		''현재 페이지 리스트.
		''#################################################
		sqlStr = "select top " + CStr(FCurrPage*FPageSize) + " * from " & TABLE_PARTNER & "" + vbCrlf
		sqlStr = sqlStr + " where id<>''" + vbCrlf
		if FRectUserDiv<>"" then
			sqlStr = sqlStr + " and userdiv='" + FRectUserDiv + "'"
		end if
