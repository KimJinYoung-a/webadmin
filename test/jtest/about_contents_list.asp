...
set opart = new CAbouthitchhiker
	...
	opart.getHitchhikerList
iTotCnt = opart.FTotalCount
%>

<script type="text/javascript">
	function conwrite(contentsidx){
		var conwrite = window.open('/test/jtest/about_contents_write.asp?contentsidx='+contentsidx,'about_contents_write','width=800,height=768,scrollbars=yes,resizable=yes');
		conwrite.focus();
	}
	function sizewrite(){
...
<table width="100%" cellpadding="3" cellspacing="1" class="a" bgcolor="<%=adminColor("tablebg")%>">
	...
	<% if opart.FResultCount > 0 then %>
		...
		<tr align="center" bgcolor="#FFFFFF" onmouseover=this.style.background="#F1F1F1"; onmouseout=this.style.background='#FFFFFF'; height="30">
			...
			<td><%= opart.FItemList(i).Fcon_title %></td><!--타이틀-->
